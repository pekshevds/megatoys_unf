...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
				Объект.Организация = Справочники.Организации.ПредопределеннаяОрганизация();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьФорму()
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя1.Заголовок = 
			Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя1.Заголовок
			+ " ("
			+ Формат(НачалоКвартала(Объект.Дата),"ДФ=MMMM")+")";
		
		Элементы.ПоказателиЕНВДВыработкаДней1.Заголовок = 
			Элементы.ПоказателиЕНВДВыработкаДней1.Заголовок
			+ " ("
			+ Формат(НачалоКвартала(Объект.Дата),"ДФ=MMMM")+")";
			
		Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя2.Заголовок = 
			Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя2.Заголовок
			+ " ("
			+ Формат(ДобавитьМесяц(НачалоКвартала(Объект.Дата),1),"ДФ=MMMM")+")";
			
		Элементы.ПоказателиЕНВДВыработкаДней2.Заголовок = 
			Элементы.ПоказателиЕНВДВыработкаДней2.Заголовок
			+ " ("
			+ Формат(ДобавитьМесяц(НачалоКвартала(Объект.Дата),1),"ДФ=MMMM")+")";
			
		Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя3.Заголовок = 
			Элементы.ПоказателиЕНВДЗначениеФизическогоПоказателя3.Заголовок
			+ " ("
			+ Формат(КонецКвартала(Объект.Дата),"ДФ=MMMM")+")";
			
		Элементы.ПоказателиЕНВДВыработкаДней3.Заголовок = 
			Элементы.ПоказателиЕНВДВыработкаДней3.Заголовок
			+ " ("
			+ Формат(КонецКвартала(Объект.Дата),"ДФ=MMMM")+")";
	КонецЕсли;
