...
//+mega
&НаКлиенте
Процедура mega_ЗагрузкаПоAPIOzonLK01(Команда)
	
	ПараметрыПодключенияКЛичномуКабинету = mega_Маркеплейсы.ПолучитьпараметрыПодключенияКОзонЛК1();
	
	АдресТаблицыНеизвестныхАктикулов = ЗагрузкаПоAPIOzonНаСервере(ПараметрыПодключенияКЛичномуКабинету);
	ТаблицаНеизвестныхАктикулов = ПолучитьТаблицуНеизвестныхАктикуловВВидеТабличногоДокумента(АдресТаблицыНеизвестныхАктикулов);
	ТаблицаНеизвестныхАктикулов.Показать();
КонецПроцедуры

&НаКлиенте
Процедура mega_ЗагрузкаПоAPIOzonLK02(Команда)
	
	ПараметрыПодключенияКЛичномуКабинету = mega_Маркеплейсы.ПолучитьпараметрыПодключенияКОзонЛК2();
...
&НаКлиенте
Процедура mega_ЗагрузкаПоAPIWBLK1(Команда)
	
	ПараметрыПодключенияКЛичномуКабинету = mega_Маркеплейсы.ПолучитьпараметрыПодключенияКВБЛК1();
	
	АдресТаблицыНеизвестныхАктикулов = ЗагрузкаПоAPIWBНаСервере(ПараметрыПодключенияКЛичномуКабинету);
	ТаблицаНеизвестныхАктикулов = ПолучитьТаблицуНеизвестныхАктикуловВВидеТабличногоДокумента(АдресТаблицыНеизвестныхАктикулов);
	ТаблицаНеизвестныхАктикулов.Показать();
КонецПроцедуры

&НаКлиенте
Процедура mega_ЗагрузкаИзExcelЯндекс(Команда)
	
	Режим = РежимДиалогаВыбораФайла.Открытие;
	
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
	ДиалогОткрытияФайла.ПолноеИмяФайла = "Отчет.xlsx";
...
&НаКлиенте
Процедура mega_ЗагрузкаИзExcelДетскийМир(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ДиалогОткрытияФайлаОтчетаДетскийМирЗавершение", ЭтотОбъект);
	ДиалогОткрытияФайла.Показать(Оповещение);
КонецПроцедуры
//-mega
...
//+mega 
&НаСервереБезКонтекста
Функция ПолучитьТаблицуНеизвестныхАктикуловВВидеТабличногоДокумента(АдресТаблицыНеизвестныхАктикулов)
	ТаблицаНеизвестныхАктикулов = ПолучитьИзВременногоХранилища(АдресТаблицыНеизвестныхАктикулов);
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	Макет = Документы.ОтчетКомиссионера.ПолучитьМакет("mega_ПФ_MXL_ТаблицаНеизвестныхАктикулов");
	
	ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	Для Каждого ТекСтрока Из ТаблицаНеизвестныхАктикулов Цикл 
...
Процедура ДиалогОткрытияФайлаОтчетаЯндексЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено И ВыбранныеФайлы.Количество() > 0 Тогда
		...
		АдресТаблицыНеизвестныхАктикулов = ЗагрузитьДанныеФайлаExcelЯндексВТабличнуюЧастьЗапасы(ПоместитьВоВременноеХранилище(ДвоичныеДанные));
		ТаблицаНеизвестныхАктикулов = ПолучитьТаблицуНеизвестныхАктикуловВВидеТабличногоДокумента(АдресТаблицыНеизвестныхАктикулов);
		ТаблицаНеизвестныхАктикулов.Показать();
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ДиалогОткрытияФайлаОтчетаДетскийМирЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныеФайлы <> Неопределено И ВыбранныеФайлы.Количество() > 0 Тогда
		ПутьКфайлу = ВыбранныеФайлы[0];
		ДвоичныеДанные = Новый ДвоичныеДанные(ПутьКфайлу);
		
		АдресТаблицыНеизвестныхАктикулов = ЗагрузитьДанныеФайлаExcelДетскийМирВТабличнуюЧастьЗапасы(ПоместитьВоВременноеХранилище(ДвоичныеДанные));
		ТаблицаНеизвестныхАктикулов = ПолучитьТаблицуНеизвестныхАктикуловВВидеТабличногоДокумента(АдресТаблицыНеизвестныхАктикулов);
		ТаблицаНеизвестныхАктикулов.Показать();
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция ЗагрузитьДанныеФайлаExcelЯндексВТабличнуюЧастьЗапасы(АдресВоВременномХранилище)
	...
	ТекущаяСтрокаПокупатели = Объект.Покупатели.Добавить();
	ТекущаяСтрокаПокупатели.Покупатель = Объект.Контрагент;
	ТекущаяСтрокаПокупатели.КлючСвязи = СоздатьНовыйКлючСвязи("Покупатели");
	...
	ТаблицаНеизвестныхАктикулов = ИнициализацияТаблицыАртикулов();	
	
	ИтогВсего = 0;
	
	НомерСтроки = 15;
	СтрНомерСтроки = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 1, НомерСтроки, 1).ТекущаяОбласть.Текст);
	Пока СтрНомерСтроки <> "" Цикл 
		
		Количество = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 6, НомерСтроки, 6).ТекущаяОбласть.Текст);
		Попытка
			
			Сумма = Число(СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 18, НомерСтроки, 18).ТекущаяОбласть.Текст));
		Исключение
			
			Сумма = 0;
		КонецПопытки;
						
		Артикул = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 4, НомерСтроки, 4).ТекущаяОбласть.Текст);		
		Номенклатура = Справочники.Номенклатура.НайтиПоРеквизиту("Артикул", Артикул);
		Если Не ЗначениеЗаполнено(Номенклатура) Тогда
			ВыборкаНоменклатуры = Справочники.НоменклатураПоставщиков.НоменклатураПоДаннымНоменклатурыПоставщика(
				Объект.Контрагент, , Артикул);
		...
		НоваяСтрокаТовары.КлючСвязи = ТекущаяСтрокаПокупатели.КлючСвязи;
		
		Если ЗначениеЗаполнено(Номенклатура) Тогда 
			
			СтруктураДанные.Вставить("Номенклатура", Номенклатура);
			СтруктураДанные.Вставить("Характеристика",	 НоваяСтрокаТовары.Характеристика);
			СтруктураДанные.Вставить("Партия",	 НоваяСтрокаТовары.Партия);
			
			СтруктураДанные = ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные);
			
			НоваяСтрокаТовары.Номенклатура = Номенклатура;
			НоваяСтрокаТовары.ЕдиницаИзмерения = СтруктураДанные.ЕдиницаИзмерения;
			НоваяСтрокаТовары.СтранаПроисхождения = СтруктураДанные.СтранаПроисхождения;
			НоваяСтрокаТовары.НомерГТД = СтруктураДанные.НомерГТД;
			НоваяСтрокаТовары.Количество = Количество;
			НоваяСтрокаТовары.Сумма = Сумма;
			НоваяСтрокаТовары.СтавкаНДС = СтруктураДанные.СтавкаНДС;
			
			НоваяСтрокаТовары.Цена = НоваяСтрокаТовары.Сумма / ?(НоваяСтрокаТовары.Количество = 0, 1, НоваяСтрокаТовары.Количество);
			...
			Если Объект.СпособРасчетаКомиссионногоВознаграждения = ПредопределенноеЗначение("Перечисление.СпособыРасчетаКомиссионногоВознаграждения.ПроцентОтСуммыПродажи") Тогда
				НоваяСтрокаТовары.СуммаВознаграждения = Объект.ПроцентКомиссионногоВознаграждения * НоваяСтрокаТовары.Сумма / 100;
			ИначеЕсли Объект.СпособРасчетаКомиссионногоВознаграждения = ПредопределенноеЗначение("Перечисление.СпособыРасчетаКомиссионногоВознаграждения.ПроцентОтРазностиСуммПродажиИПоступления") Тогда
				НоваяСтрокаТовары.СуммаВознаграждения = Объект.ПроцентКомиссионногоВознаграждения * (НоваяСтрокаТовары.Сумма - НоваяСтрокаТовары.СуммаПередачи) / 100;
			Иначе
				НоваяСтрокаТовары.СуммаВознаграждения = 0;
			КонецЕсли;
			
			СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(Объект.СтавкаНДСВознаграждения);
			
			НоваяСтрокаТовары.СуммаНДСВознаграждения = ?(Объект.СуммаВключаетНДС,
												НоваяСтрокаТовары.СуммаВознаграждения - (НоваяСтрокаТовары.СуммаВознаграждения) / ((СтавкаНДС + 100) / 100),
												НоваяСтрокаТовары.СуммаВознаграждения * СтавкаНДС / 100);				
			
			//Партии
			НоваяСтрокаТовары.ИспользоватьПартии = СтруктураДанные.ИспользоватьПартии;
			НоваяСтрокаТовары.ПроверятьЗаполнениеПартий = СтруктураДанные.ПроверятьЗаполнениеПартий;
			
			Если СтруктураДанные.ИспользоватьПартии Тогда
				Если ПодборНоменклатурыИзСписка И ТипЗнч(СтруктураВыбораНоменклатуры) = Тип("Структура") Тогда
					НоваяСтрокаТовары.Партия = СтруктураВыбораНоменклатуры.Партия;
					СтруктураВыбораНоменклатуры.Партия = Неопределено;
				Иначе
					НоваяСтрокаТовары.Партия = СтруктураДанные.Партия;
				КонецЕсли;
			КонецЕсли;
			// Конец Партии  
			
			НоваяСтрокаТовары.ЦенаПередачи = НоваяСтрокаТовары.Цена;
			НоваяСтрокаТовары.СуммаПередачи = НоваяСтрокаТовары.Сумма;
			НоваяСтрокаТовары.СуммаНДСПередачи = НоваяСтрокаТовары.СуммаНДС;
		Иначе
			
			НоваяСтрока = ТаблицаНеизвестныхАктикулов.Добавить();
			...
			НоваяСтрокаТовары.Количество = Количество;
			НоваяСтрокаТовары.Сумма = Сумма;
			НоваяСтрокаТовары.СуммаПередачи = Сумма;
		...
		ИтогВсего = ИтогВсего + НоваяСтрокаТовары.Всего;
		
		НомерСтроки = НомерСтроки + 1;
		СтрНомерСтроки = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 1, НомерСтроки, 1).ТекущаяОбласть.Текст);
	КонецЦикла;
	
	ТекущаяСтрокаПокупатели.Всего = ИтогВсего;
	
	УдалитьФайлы(ИмяВременногоФайла);
	
	ТаблицаНеизвестныхАктикулов.Свернуть("Артикул", "Количество,КоличествоВозврат,Сумма,СуммаВозврат");
	Возврат ПоместитьВоВременноеХранилище(ТаблицаНеизвестныхАктикулов);
КонецФункции

&НаСервере
Функция ЗагрузитьДанныеФайлаExcelДетскийМирВТабличнуюЧастьЗапасы(АдресВоВременномХранилище)
	...
	Пока СтрНомерСтроки <> "" Цикл 
		...
		Количество = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 13, НомерСтроки, 13).ТекущаяОбласть.Текст);
		Попытка
			
			Сумма = Число(СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 24, НомерСтроки, 24).ТекущаяОбласть.Текст));
		Исключение
		...
		Артикул = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 7, НомерСтроки, 7).ТекущаяОбласть.Текст);		
		Номенклатура = Справочники.Номенклатура.НайтиПоРеквизиту("Артикул", Артикул);
		Если Не ЗначениеЗаполнено(Номенклатура) Тогда
			ВыборкаНоменклатуры = Справочники.НоменклатураПоставщиков.НоменклатураПоДаннымНоменклатурыПоставщика(
				Объект.Контрагент, , Артикул);
...
&НаСервере
Функция ПолучитьОтчетОПродажахПоРеализацииОзон(Заголовки, ПериодДень, ОписаниеОшибки = "") 
			
	HTTPСоединение = Новый HTTPСоединение(mega_Маркеплейсы.ПолучитьСерверОзон(), 443,,,,, Новый ЗащищенноеСоединениеOpenSSL());
	
	Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
	HTTPЗапрос = Новый HTTPЗапрос("/v1/finance/realization", Заголовки);
		
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	
	Структура = Новый Структура;
	Структура.Вставить("date", Формат(ПериодДень, "ДФ=yyyy-MM"));
...
&НаСервере
Функция ПолучитьОтчетОПродажахПоРеализацииВБ(Заголовки, ПериодДень, ОписаниеОшибки = "") 
	
	НачалоПериода = НачалоНедели(ПериодДень);
	КонецПериода = КонецНедели(ПериодДень);
	
	HTTPСоединение = Новый HTTPСоединение(mega_Маркеплейсы.ПолучитьСерверВБ(), 443,,,,, Новый ЗащищенноеСоединениеOpenSSL());
	
	Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		
	HTTPЗапрос = Новый HTTPЗапрос(СтрШаблон("/api/v1/supplier/reportDetailByPeriod?dateFrom=%1&dateTo=%2", XMLСтрока(НачалоПериода), XMLСтрока(КонецПериода)), Заголовки);
	...
	ОписаниеОшибки = ТелоКакСтрока;
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "Ошибка работы с сервером WB, возможно истек срок годности токена.";
	Возврат Неопределено;
КонецФункции

&НаСервере
Функция СоздатьНовыйКлючСвязи(ИмяТабличнойЧасти)
...
&НаСервере
Функция ИнициализацияТаблицыАртикулов()
	...
	ТаблицаЗначений.Колонки.Добавить("КоличествоВозврат", Новый ОписаниеТипов("Число",
												 Новый КвалификаторыЧисла(15, 3, ДопустимыйЗнак.Любой)));
	...
	ТаблицаЗначений.Колонки.Добавить("Цена", Новый ОписаниеТипов("Число",
												 Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Любой)));
	ТаблицаЗначений.Колонки.Добавить("СуммаВозврат", Новый ОписаниеТипов("Число",
												 Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Любой)));
	Возврат ТаблицаЗначений;
КонецФункции

&НаСервере
Функция ЗагрузкаПоAPIOzonНаСервере(ПараметрыПодключенияКЛичномуКабинету)
	...
	ТекущаяСтрокаПокупателиВозвраты = Объект.ПокупателиВозвраты.Добавить();
	ТекущаяСтрокаПокупателиВозвраты.Покупатель = Объект.Контрагент;
	ТекущаяСтрокаПокупателиВозвраты.КлючСвязи = СоздатьНовыйКлючСвязи("Покупатели");
	
	
	ТаблицаИзвестныхАктикулов = ИнициализацияТаблицыАртикулов();
	ТаблицаНеизвестныхАктикулов = ИнициализацияТаблицыАртикулов();
	
	ОписаниеОшибки = "";
	ОтчетОПродажахПоРеализации = ПолучитьОтчетОПродажахПоРеализацииОзон(
		ПараметрыПодключенияКЛичномуКабинету, Объект.Дата, ОписаниеОшибки);
	
	Если ОтчетОПродажахПоРеализации = Неопределено Тогда 
	...
	Для Каждого ТекСтрока Из ОтчетОПродажахПоРеализации Цикл 
		
		НоваяСтрока = ТаблицаИзвестныхАктикулов.Добавить();			
		НоваяСтрока.Артикул 			= ТекСтрока.Получить("offer_id");
		НоваяСтрока.Количество 			= ТекСтрока.Получить("sale_qty");
		НоваяСтрока.КоличествоВозврат	= ТекСтрока.Получить("return_qty");
		НоваяСтрока.Сумма 				= ТекСтрока.Получить("sale_amount");//ТекСтрока.Получить("price") * НоваяСтрока.Количество;
		НоваяСтрока.Цена				= НоваяСтрока.Сумма / ?(НоваяСтрока.Количество = 0, 1, НоваяСтрока.Количество);
		НоваяСтрока.СуммаВозврат		= ТекСтрока.Получить("return_amount");//ТекСтрока.Получить("return_price_seller");
	КонецЦикла;
	
	ТаблицаИзвестныхАктикулов.Свернуть("Артикул,Цена", "Количество,КоличествоВозврат,Сумма,СуммаВозврат");
	...
	ИтогВсего = 0;
	ИтогВсегоВозврат = 0;
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаИзвестныхАктикулов Цикл
		
		Номенклатура = Справочники.Номенклатура.НайтиПоРеквизиту("Артикул", СтрокаТабличнойЧасти.Артикул);
		Если Не ЗначениеЗаполнено(Номенклатура) Тогда
			ВыборкаНоменклатуры = Справочники.НоменклатураПоставщиков.НоменклатураПоДаннымНоменклатурыПоставщика(
				Объект.Контрагент, , СтрокаТабличнойЧасти.Артикул);
		...
		Если НЕ ЗначениеЗаполнено(Номенклатура) Тогда
			...
			ЗаполнитьЗначенияСвойств(ТаблицаНеизвестныхАктикулов.Добавить(), СтрокаТабличнойЧасти);
		КонецЕсли;
		...
		Если СтрокаТабличнойЧасти.Количество > 0 Тогда 
			...
			Если ЗначениеЗаполнено(Номенклатура) Тогда 
				...
				//НоваяСтрокаТовары.mega_Артикул = ТекСтрока.Артикул;
				//
				НоваяСтрокаТовары.Номенклатура = Номенклатура;
				НоваяСтрокаТовары.ЕдиницаИзмерения = СтруктураДанные.ЕдиницаИзмерения;
				НоваяСтрокаТовары.СтранаПроисхождения = СтруктураДанные.СтранаПроисхождения;
				НоваяСтрокаТовары.НомерГТД = СтруктураДанные.НомерГТД;
				НоваяСтрокаТовары.Количество = СтрокаТабличнойЧасти.Количество;
				НоваяСтрокаТовары.Сумма = СтрокаТабличнойЧасти.Сумма;
				НоваяСтрокаТовары.СтавкаНДС = СтруктураДанные.СтавкаНДС;
			...
			Иначе
				...
				НоваяСтрокаТовары.Всего = НоваяСтрокаТовары.Сумма + ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС);
		...
		Если СтрокаТабличнойЧасти.КоличествоВозврат > 0 Тогда 
		
			НоваяСтрокаТовары = Объект.ЗапасыВозвраты.Добавить();	
			НоваяСтрокаТовары.КлючСвязи = ТекущаяСтрокаПокупателиВозвраты.КлючСвязи;
			Если ЗначениеЗаполнено(Номенклатура) Тогда 
				...
				НоваяСтрокаТовары.СтранаПроисхождения = СтруктураДанные.СтранаПроисхождения;
				НоваяСтрокаТовары.НомерГТД = СтруктураДанные.НомерГТД;
				НоваяСтрокаТовары.Количество = СтрокаТабличнойЧасти.КоличествоВозврат;
				НоваяСтрокаТовары.Сумма = СтрокаТабличнойЧасти.СуммаВозврат;
				НоваяСтрокаТовары.СтавкаНДС = СтруктураДанные.СтавкаНДС;
			...
			ИтогВсегоВозврат = ИтогВсегоВозврат + НоваяСтрокаТовары.Всего;
		КонецЕсли;		
	КонецЦикла; 
	
	ТекущаяСтрокаПокупатели.Всего = ИтогВсего;
	ТекущаяСтрокаПокупателиВозвраты.Всего = ИтогВсегоВозврат;
...
&НаСервере
Функция ЗагрузкаПоAPIWBНаСервере(ПараметрыПодключенияКЛичномуКабинету)
	...
	ТаблицаИзвестныхАктикулов = ИнициализацияТаблицыАртикулов();
	ТаблицаНеизвестныхАктикулов = ТаблицаИзвестныхАктикулов.Скопировать();
	
	ОписаниеОшибки = "";
	ОтчетОПродажахПоРеализации = ПолучитьОтчетОПродажахПоРеализацииВБ(
		ПараметрыПодключенияКЛичномуКабинету, Объект.Дата, ОписаниеОшибки);
	...
	ДопустимыеВидыОпераций.Добавить("Продажа");
	ДопустимыеВидыОпераций.Добавить("Возврат");
	
	Для Каждого ТекСтрока Из ОтчетОПродажахПоРеализации Цикл 
		
		ТипДокумента = ТекСтрока.Получить("doc_type_name");	
		Если ДопустимыеВидыОпераций.Найти(ТипДокумента) = Неопределено Тогда
			
			Продолжить;
		КонецЕсли;
				
		Объект.Комментарий = ТекСтрока.Получить("realizationreport_id");
					
		НоваяСтрока 					= ТаблицаИзвестныхАктикулов.Добавить();
		НоваяСтрока.Артикул 			= ТекСтрока.Получить("sa_name");
		Если ТипДокумента = "Продажа" Тогда 
			
			НоваяСтрока.Количество 			= ТекСтрока.Получить("quantity");		
			НоваяСтрока.Сумма 				= ТекСтрока.Получить("retail_amount");
			НоваяСтрока.Цена				= НоваяСтрока.Сумма / ?(НоваяСтрока.Количество = 0, 1, НоваяСтрока.Количество);
		ИначеЕсли ТипДокумента = "Возврат" Тогда
			
			НоваяСтрока.КоличествоВозврат 	= ТекСтрока.Получить("quantity");
			НоваяСтрока.СуммаВозврат		= ТекСтрока.Получить("retail_amount");
		КонецЕсли;
