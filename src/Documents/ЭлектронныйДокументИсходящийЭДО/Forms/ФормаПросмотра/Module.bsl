...
Процедура ЗаполнитьДанныеНовогоЭлектронногоДокумента(Основание, ВидДокумента)
	...
	// Если не было предварительной подготовки данных, то документ не может иметь тип "Договорной документ"
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент Тогда
...
Процедура РаспознатьДокумент(ЭлементСхемы)
	...
	Если ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент
...
Процедура УстановитьОформлениеНастроекФормы(ЭлементСхемы)
	...
	ИначеЕсли ЭлементСхемы.ФормированиеПоОбъектуУчета Тогда
		...
		ВыводитьПодсказкуПДФ_А3 = Обработки.ФорматДоговорнойДокумент101.ЭтоФайлФормата(ЭлементСхемы.ИмяФайла);
	...
	Если ВыводитьПодсказкуПДФ_А3 Тогда
		Элементы.НадписьПодсказкаДоговорнойДокументПДФ_А3.Заголовок = 
			Обработки.ФорматДоговорнойДокумент101.ТекстСправки();
	КонецЕсли;
	Элементы.ГруппаПодсказкаДоговорнойДокументПДФ_А3.Видимость = ВыводитьПодсказкуПДФ_А3;
	
	Если Не ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент Тогда
...
Процедура НачатьЗаписьДокумента(ОповещениеОЗавершении)
	...
	Если РезультатЗаписи.ТребуетсяФормализованныйФормат Тогда
