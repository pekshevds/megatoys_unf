...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	,Табель.ПодписьРуководителя.Должность КАК ДолжностьРуководителя
		|	,Табель.ПодписьРуководителя.РасшифровкаПодписи КАК РасшифровкаПодписиРуководителя
		|	,Табель.ПодписьГлавногоБухгалтера.Должность КАК ДолжностьГлавногоБухгалтера
		|	,Табель.ПодписьГлавногоБухгалтера.РасшифровкаПодписи КАК РасшифровкаПодписиГлавногоБухгалтера
		|	,Табель.ПодписьОтветственного.Должность КАК ДолжностьОтветственного
		|	,Табель.ПодписьОтветственного.РасшифровкаПодписи КАК РасшифровкаПодписиОтветственного
		|ИЗ Документ.Табель КАК Табель
		|ГДЕ Табель.Ссылка = &ТекущийДокумент";
		...
		Если Шапка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням Тогда
			...
			|		ПО ТабельОтработанноеВремяПоДням.Сотрудник.Физлицо = ФИОФизЛицСрезПоследних.ФизическоеЛицо
			|ГДЕ
			|	ТабельОтработанноеВремяПоДням.Ссылка = &Ссылка
			|
			|УПОРЯДОЧИТЬ ПО
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.Фамилия) Тогда
					...
					ФамилияИмяОтчество.Вставить("Фамилия", Выборка.Фамилия);
					ФамилияИмяОтчество.Вставить("Имя", Выборка.Имя);
					ФамилияИмяОтчество.Вставить("Отчество", Выборка.Отчество);
					ФИО = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ФамилияИмяОтчество);
		...
		Иначе
			...
			|	Документ.Табель.ОтработанноеВремяЗаПериод КАК ТабельОтработанноеВремяЗаПериод
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ПериодРегистрации, ) КАК ФИОФизЛицСрезПоследних
			|		ПО ТабельОтработанноеВремяЗаПериод.Сотрудник.Физлицо = ФИОФизЛицСрезПоследних.ФизическоеЛицо
			|ГДЕ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Табель") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Табель", НСтр("ru = 'Табель учета рабочего времени'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
