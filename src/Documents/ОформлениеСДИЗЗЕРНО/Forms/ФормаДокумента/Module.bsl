...
&НаКлиенте
Процедура НомерПротоколаИспытанийПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект,, "НомерПротоколаИспытаний");
	ОтключитьОтметкуНезаполненного();
КонецПроцедуры

&НаКлиенте
Процедура ДатаПротоколаИспытанийПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект,, "ДатаПротоколаИспытаний");
	ОтключитьОтметкуНезаполненного();
КонецПроцедуры
...
Процедура ТоварыПартияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		ОбновитьДанные(ВыбранноеЗначение,
			Новый Структура("ИмяПоля, ИдентификаторСтроки", "ТоварыПартия", Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор()));
...
&НаКлиенте
Процедура ТоварыНомерПротоколаИспытанийПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект,, "ТоварыНомерПротоколаИспытаний");
	ОтключитьОтметкуНезаполненного();
КонецПроцедуры

&НаКлиенте
Процедура ТоварыДатаПротоколаИспытанийПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект,, "ТоварыДатаПротоколаИспытаний");
	ОтключитьОтметкуНезаполненного();
КонецПроцедуры
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("Товары") Или Инициализация Тогда
		...
		СтруктураРеквизитов.Вставить("НомерПротоколаИспытаний");
	...
	Если СтруктураРеквизитов.Свойство("НомерПротоколаИспытаний")
		Или СтруктураРеквизитов.Свойство("ТоварыНомерПротоколаИспытаний")
		Или СтруктураРеквизитов.Свойство("ДатаПротоколаИспытаний")
		Или СтруктураРеквизитов.Свойство("ТоварыДатаПротоколаИспытаний") Тогда
		
		ОтметкаНезаполненногоТовары = Не ЗначениеЗаполнено(Объект.НомерПротоколаИспытаний) Или Не ЗначениеЗаполнено(Объект.ДатаПротоколаИспытаний);
		Элементы.ТоварыНомерПротоколаИспытаний.АвтоОтметкаНезаполненного = ОтметкаНезаполненногоТовары;
		Элементы.ТоварыДатаПротоколаИспытаний.АвтоОтметкаНезаполненного  = ОтметкаНезаполненногоТовары;
		
		Если Не ЗначениеЗаполнено(Объект.НомерПротоколаИспытаний) И Не ЗначениеЗаполнено(Объект.ДатаПротоколаИспытаний) Тогда
			ОтметкаНезаполненного = Ложь;
			Для Каждого СтрокаТовары Из Объект.Товары Цикл
				Если Не ЗначениеЗаполнено(СтрокаТовары.НомерПротоколаИспытаний) Тогда
					ОтметкаНезаполненного = Истина;
					Прервать;
				КонецЕсли;
				Если Не ЗначениеЗаполнено(СтрокаТовары.ДатаПротоколаИспытаний) Тогда
					ОтметкаНезаполненного = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Элементы.НомерПротоколаИспытаний.АвтоОтметкаНезаполненного = ОтметкаНезаполненного;
			Элементы.ДатаПротоколаИспытаний.АвтоОтметкаНезаполненного  = ОтметкаНезаполненного;
		Иначе
			Элементы.ДатаПротоколаИспытаний.АвтоОтметкаНезаполненного  = ЗначениеЗаполнено(Объект.НомерПротоколаИспытаний);
			Элементы.НомерПротоколаИспытаний.АвтоОтметкаНезаполненного = ЗначениеЗаполнено(Объект.ДатаПротоколаИспытаний);
		КонецЕсли;
