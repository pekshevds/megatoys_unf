...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства) Экспорт
	...
	ИнициализироватьДанныеДокументаДвиженияДенежныхСредств(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства);
...
Процедура ИнициализироватьДанныеДокументаДенежныеСредстваПрогноз(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУдалитьДенежныеСредстваПрогноз", Результат);
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура ИнициализироватьДанныеДокументаДвиженияДенежныхСредств(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Ссылка.СценарийПланирования КАК СценарийПланирования,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Счет = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Касса)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА ТаблицаДокумента.Ссылка.Проект
	|		ИНАЧЕ ТаблицаДокумента.Проект
	|	КОНЕЦ КАК Проект,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	...
	|	0 КАК СуммаРасходВал,
	|	0 КАК СуммаРасход,
	|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.Бюджет) КАК ХозяйственнаяОперация
	|ИЗ
	|	Документ.Бюджет.Поступления КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	...
	|	ТаблицаДокумента.Сумма,
	|	ТаблицаДокумента.Сумма,
	|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.Бюджет)
	|ИЗ
	|	Документ.Бюджет.Выбытия КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДвиженияДенежныхСредств", Результат);
						 ...
						 ИЛИ ОбходПоСчетамУчета.СуммаРасходов <> 0 Тогда
							...
							Если СуммаРаспределили = 0 Тогда
								...
								ТекстСообщения = СтрШаблон(НСтр(
									"ru = 'Расчет финансового результата: Счет учета ""%1"", не имеет базы распределения.'",
