...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
		...
		Иначе
			Если НастройкиЗагрузкиДанных.ЗаказЗаполнен Тогда
				ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Резерв", НСтр(
					"ru = 'Резерв'"), ОписанияТиповПолей.ОписаниеТиповСтрока25,
					ОписанияТиповПолей.ОписаниеТиповЧисло15_2, , , Ложь);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;

	ВидимостьЗаказа = (ЭтоЗапасы И НастройкиЗагрузкиДанных.Свойство("ПоложениеЗаказаПокупателя")
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЗаказПокупателя", НСтр(
		"ru = 'Заказ (покупатель)'"), ОписанияТиповПолей.ОписаниеТиповСтрока50, ОписаниеТиповКолонка, , , , ,
		ВидимостьЗаказа);
...
//+mega
Процедура ДополнитьДаннымиСпецификации(ТаблицаЗначений, Спецификация, Количество) 
	
	Для Каждого СтрокаТабличнойЧастиСостав Из Спецификация.Состав Цикл 
		
		Если СтрокаТабличнойЧастиСостав.ТипСтрокиСостава <> Перечисления.ТипыСтрокСоставаСпецификации.Материал Тогда 
			Продолжить;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаТабличнойЧастиСостав.Спецификация) Тогда
			
			ДополнитьДаннымиСпецификации(ТаблицаЗначений, СтрокаТабличнойЧастиСостав.Спецификация, СтрокаТабличнойЧастиСостав.Количество / СтрокаТабличнойЧастиСостав.КоличествоПродукции * Количество);
		Иначе                                                
			
			НоваяСтрока = ТаблицаЗначений.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТабличнойЧастиСостав);
			НоваяСтрока.Количество = СтрокаТабличнойЧастиСостав.Количество / СтрокаТабличнойЧастиСостав.КоличествоПродукции * Количество;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ДополнитьТаблицуЗапасы(ТаблицаЗапасы)
	
	КопияТаблицаЗапасы = ТаблицаЗапасы.Скопировать();
	КопияТаблицаЗапасы.Очистить();
	
	Для Каждого СтрокаТабличнойЧастиЗапасы Из ТаблицаЗапасы Цикл

		Если ЗначениеЗаполнено(СтрокаТабличнойЧастиЗапасы.Спецификация) Тогда
			
			ДополнитьДаннымиСпецификации(КопияТаблицаЗапасы, СтрокаТабличнойЧастиЗапасы.Спецификация, СтрокаТабличнойЧастиЗапасы.Количество);
		Иначе
			
			НоваяСтрока = КопияТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТабличнойЧастиЗапасы);
		КонецЕсли;
	КонецЦикла;
	
	КопияТаблицаЗапасы.Свернуть("Номенклатура", "Количество");
	ТаблицаЗапасы = КопияТаблицаЗапасы.Скопировать();
	ТаблицаЗапасы.Сортировать("Номенклатура");
КонецПроцедуры 

Процедура ДополнитьТаблицуЗапасыОстатками(ТаблицаЗапасы, СтруктурнаяЕдиницаОстатков)
		
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("ТаблицаЗапасы", ТаблицаЗапасы);
	Запрос.Параметры.Вставить("СтруктурнаяЕдиницаОстатков", СтруктурнаяЕдиницаОстатков);
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	втТаблицаЗапасы.Номенклатура.Артикул КАК Артикул,
		|	втТаблицаЗапасы.Номенклатура.Штрихкод КАК Штрихкод,
		|	втТаблицаЗапасы.Номенклатура.Код КАК Код,
		|	втТаблицаЗапасы.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ЕСТЬNULL(втТаблицаЗапасы.Количество, 0) КАК Количество,
		|	ЕСТЬNULL(ЗапасыНаСкладахОстаткиНаСкладах.КоличествоОстаток, 0) КАК НаСкладах,
		|	ЕСТЬNULL(ЗапасыНаСкладахОстаткиНаСкладе.КоличествоОстаток, 0) КАК НаСкладе
		|ИЗ
		|	втТаблицаЗапасы КАК втТаблицаЗапасы
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(, СтруктурнаяЕдиница <> &СтруктурнаяЕдиницаОстатков) КАК ЗапасыНаСкладахОстаткиНаСкладах
		|	ПО втТаблицаЗапасы.Номенклатура = ЗапасыНаСкладахОстаткиНаСкладах.Номенклатура
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(, СтруктурнаяЕдиница = &СтруктурнаяЕдиницаОстатков) КАК ЗапасыНаСкладахОстаткиНаСкладе
		|	ПО втТаблицаЗапасы.Номенклатура = ЗапасыНаСкладахОстаткиНаСкладе.Номенклатура
        |	
		|	УПОРЯДОЧИТЬ ПО
		|	втТаблицаЗапасы.Номенклатура.Наименование";
	
	РезультатЗапроса = Запрос.Выполнить();	
	ТаблицаЗапасы = РезультатЗапроса.Выгрузить();
КонецПроцедуры

Функция СформироватьАнализВозможностиОбеспеченияПотребности(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)Экспорт 
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	|	,ЗаказНаПроизводство.СтруктурнаяЕдиница КАК Подразделение
	|	,ЗаказНаПроизводство.Организация.Префикс КАК Префикс 
	|	,ЗаказНаПроизводство.mega_СтруктурнаяЕдиницаОстатков КАК СтруктурнаяЕдиницаОстатков
	|	
	// ::: Табличная часть "Запасы" (Материалы)
	|	,ЗаказНаПроизводство.Запасы.(
	|		НомерСтроки КАК НомерСтроки	
	...
	Пока Шапка.Следующий() Цикл
		...
		НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента,
			Шапка.Номер, Шапка.Префикс);
		ДатаДокумента = Формат(Шапка.ДатаДокумента, "ДЛФ=DD");
		ДанныеПечати.Вставить("ТекстЗаголовка", СтрШаблон(НСтр("ru ='Анализ возможности обеспечения потребности № %1 от %2'"), НомерДокумента,
			ДатаДокумента));
		...
		ТаблицаЗапасы = Шапка.ТаблицаЗапасы.Выгрузить();
		ДополнитьТаблицуЗапасы(ТаблицаЗапасы);
		//@skip-check query-in-loop
		ДополнитьТаблицуЗапасыОстатками(ТаблицаЗапасы, Шапка.СтруктурнаяЕдиницаОстатков);
		НомерСтроки = 1;
		Для Каждого СтрокаТабличнойЧастиЗапасы Из ТаблицаЗапасы Цикл
			
			ОбластьМакетаСтрока = Макет.ПолучитьОбласть("Строка");		
			Если СтрокаТабличнойЧастиЗапасы.Количество > СтрокаТабличнойЧастиЗапасы.НаСкладе Тогда 
				
				ОбластьМакетаСтрока = Макет.ПолучитьОбласть("СтрокаОтрицательная");
			КонецЕсли;
			
			ОбластьМакетаСтрока.Параметры.Заполнить(СтрокаТабличнойЧастиЗапасы);
...
Функция СформироватьЗаданиеНаПроизводство(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)Экспорт 
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		Количество КАК Количество,
	|		mega_Комментарий КАК Комментарий
	|	) КАК ТаблицаЗапасы
	|ИЗ
	...
	Пока Шапка.Следующий() Цикл
		...
		НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента,
			Шапка.Номер, Шапка.Префикс);
		ДатаДокумента = Формат(Шапка.ДатаДокумента, "ДЛФ=DD");
		ДанныеПечати.Вставить("ТекстЗаголовка", СтрШаблон(НСтр("ru ='Задание на производство № %1 от %2'"), НомерДокумента,
			ДатаДокумента));
			
		ДанныеПечати.Вставить("Подразделение", Шапка.Подразделение);
		...
		Для Каждого СтрокаТабличнойЧастиЗапасы Из ТаблицаЗапасы Цикл
			...
			Если СокрЛП(СтрокаТабличнойЧастиЗапасы.Комментарий) <> "" Тогда 
				
				ОбластьМакетаСтрокаКомментарий = Макет.ПолучитьОбласть("СтрокаКомментарий");		
						
				ОбластьМакетаСтрокаКомментарий.Параметры.Заполнить(СтрокаТабличнойЧастиЗапасы);				
				ТабличныйДокумент.Вывести(ОбластьМакетаСтрокаКомментарий);
			КонецЕсли;
...
Функция СформироватьЗаданиеНаПроизводствоИсполнителям(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)Экспорт 
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	"ВЫБРАТЬ
	|	ЗаказНаПроизводствоПродукция.Ссылка КАК Ссылка,
	|	ЗаказНаПроизводствоПродукция.mega_Сотрудник КАК Сотрудник,
	|	ЗаказНаПроизводствоПродукция.Номенклатура КАК Номенклатура,
	|	ЗаказНаПроизводствоПродукция.mega_Комментарий КАК Комментарий,
	|	ЗаказНаПроизводствоПродукция.Количество КАК Количество,
	|	ЗаказНаПроизводствоПродукция.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ЗаказНаПроизводствоПродукция.Номенклатура.НаименованиеПолное КАК НаименованиеПолное,
	...
	|	ЗаказНаПроизводствоПродукция.Ссылка.СтруктурнаяЕдиница КАК Подразделение,
	|	ЗаказНаПроизводствоПродукция.Ссылка.Ответственный КАК Ответственный,
	|	ЗаказНаПроизводствоПродукция.Ссылка.Организация.Префикс КАК Префикс
	|ИЗ
	|	Документ.ЗаказНаПроизводство.Продукция КАК ЗаказНаПроизводствоПродукция
	|ГДЕ
	|	ЗаказНаПроизводствоПродукция.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	Сотрудник,
	...
	ВидЦенПлановые = Справочники.ВидыЦен.НайтиПоКоду("БП-000001");
	ВидЦенЗакупочные = Справочники.ВидыЦен.НайтиПоКоду("000000001");	
	ВалютаДокумента = Константы.ВалютаУчета.Получить();
	
	Пока Шапка.Следующий() Цикл
		
		Сотрудники = Шапка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока Сотрудники.Следующий() Цикл
			...
			ОбластьИсполнитель.Параметры.Заполнить(Сотрудники);
			...
			СтрокаТабличнойЧастиЗапасы = Сотрудники.Выбрать();
			НомерСтроки = 1;
			
			ИтогоСуммаПлановая = 0;
			ИтогоСуммаЗакупочная = 0;
			Пока СтрокаТабличнойЧастиЗапасы.Следующий() Цикл 
				
				ПараметрыПолученияЦен = Новый Структура;
				ПараметрыПолученияЦен.Вставить("ВидЦен", ВидЦенПлановые);
				ПараметрыПолученияЦен.Вставить("ДатаОбработки", Шапка.ДатаДокумента);
				ПараметрыПолученияЦен.Вставить("Номенклатура", СтрокаТабличнойЧастиЗапасы.Номенклатура);
				ПараметрыПолученияЦен.Вставить("Характеристика", Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
				ПараметрыПолученияЦен.Вставить("Коэффициент", 1);
				ПараметрыПолученияЦен.Вставить("ВалютаДокумента", ВалютаДокумента);
				ПараметрыПолученияЦен.Вставить("СуммаВключаетНДС", Истина);
				ПараметрыПолученияЦен.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.Общая, Шапка.ДатаДокумента));				
				
				ЦенаПлановая = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ПараметрыПолученияЦен);
				ПараметрыПолученияЦен.Вставить("ВидЦен", ВидЦенЗакупочные);
				ЦенаЗакупочная = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ПараметрыПолученияЦен);
				
				СуммаПлановая = ЦенаПлановая * СтрокаТабличнойЧастиЗапасы.Количество;
				ИтогоСуммаПлановая = ИтогоСуммаПлановая + СуммаПлановая;
				
				СуммаЗакупочная = ЦенаЗакупочная * СтрокаТабличнойЧастиЗапасы.Количество;
				ИтогоСуммаЗакупочная = ИтогоСуммаЗакупочная + СуммаЗакупочная;
				
				ОбластьМакетаСтрока = Макет.ПолучитьОбласть("Строка");		
				
				ОбластьМакетаСтрока.Параметры.Заполнить(СтрокаТабличнойЧастиЗапасы);
				ОбластьМакетаСтрока.Параметры.НомерСтроки = НомерСтроки;
				ОбластьМакетаСтрока.Параметры.СуммаПлановая = СуммаПлановая; 
				ОбластьМакетаСтрока.Параметры.СуммаЗакупочная = СуммаЗакупочная;
				ТабличныйДокумент.Вывести(ОбластьМакетаСтрока);
			...
			ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");
			ОбластьПодвал.Параметры.Заполнить(Шапка);
			ОбластьПодвал.Параметры.СуммаПлановая = ИтогоСуммаПлановая; 
			ОбластьПодвал.Параметры.СуммаЗакупочная = ИтогоСуммаЗакупочная;
			ТабличныйДокумент.Вывести(ОбластьПодвал);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	//+mega
	ОписаниеПечатнойФормы = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "АнализВозможностиОбеспеченияПотребности");
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		
		ОписаниеПечатнойФормы.ТабличныйДокумент = Новый ТабличныйДокумент;
		ОписаниеПечатнойФормы.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_АнализВозможностиОбеспеченияПотребности";
		ОписаниеПечатнойФормы.ПолныйПутьКМакету = "Документ.ЗаказНаПроизводство.mega_ПФ_MXL_АнализВозможностиОбеспеченияПотребности";
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ru ='Требование на склад'");
		
		СформироватьАнализВозможностиОбеспеченияПотребности(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати);
		
	КонецЕсли;
	
	ОписаниеПечатнойФормы = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ЗаданиеНаПроизводствоИсполнителям");
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		
		ОписаниеПечатнойФормы.ТабличныйДокумент = Новый ТабличныйДокумент;
		ОписаниеПечатнойФормы.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_ЗаданиеНаПроизводствоИсполнителям";
		ОписаниеПечатнойФормы.ПолныйПутьКМакету = "Документ.ЗаказНаПроизводство.mega_ПФ_MXL_ЗаданиеНаПроизводство";
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ru ='Задание на производство'");
		
		СформироватьЗаданиеНаПроизводствоИсполнителям(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати);
		
	КонецЕсли;
	
	ОписаниеПечатнойФормы = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ЗаданиеНаПроизводство");
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		
		ОписаниеПечатнойФормы.ТабличныйДокумент = Новый ТабличныйДокумент;
		ОписаниеПечатнойФормы.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_ЗаданиеНаПроизводство";
		ОписаниеПечатнойФормы.ПолныйПутьКМакету = "Документ.ЗаказНаПроизводство.mega_ПФ_MXL_ЗаданиеНаПроизводство";
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ru ='Задание на производство исполнителям'");
		
		СформироватьЗаданиеНаПроизводство(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати);
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ru ='Анализ возможности обеспечения потребности'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//+mega
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "АнализВозможностиОбеспеченияПотребности";
	КомандаПечати.Представление = НСтр("ru = 'Анализ возможности обеспечения потребности'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ЗаданиеНаПроизводствоИсполнителям";
	КомандаПечати.Представление = НСтр("ru = 'Задание на производство исполнителям'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Функция КартинкаЗаписиКалендаря(Источник) Экспорт
	...
	Возврат БиблиотекаКартинок.ЗадачаРазделПроизводство;
