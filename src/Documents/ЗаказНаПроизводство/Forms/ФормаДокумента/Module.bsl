...
Процедура ОбработатьПодготовленныеДанные(АдресРезультатаЗагрузки)
	...
	Попытка
		...
		КонецЦикла;
		//+mega
		Объект[ИмяТабличнойЧасти].Свернуть("Номенклатура,Характеристика,ЕдиницаИзмерения,Спецификация,Партия,СтруктурнаяЕдиница,ЗаказПокупателя", "Количество,Резерв");
		ЗаполнитьОстаткиНаСервере();
		//-mega
