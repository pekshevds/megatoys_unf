...
Процедура СтартПриИзменении(Элемент)
	...
	Если Объект.Старт > Объект.Финиш И ЗначениеЗаполнено(Объект.Финиш) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Дата старта не может быть больше даты финиша.'"));
	Иначе
		ПриИзмененииСтарт = Объект.Старт;
	КонецЕсли;
...
Процедура ФинишПриИзменении(Элемент)
	...
	Если Объект.Финиш < Объект.Старт Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Дата финиша не может быть меньше даты старта.'"));
	Иначе
		ПриИзмененииФиниш = Объект.Финиш;
	КонецЕсли;
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.СтруктурнаяЕдиница) Тогда
		Склад = ТекущиеДанные.СтруктурнаяЕдиница;
	Иначе
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ЗаказПокупателя) Тогда
		ПараметрыОтбора.Вставить("ЗаказПокупателя", ТекущиеДанные.ЗаказПокупателя);
	Иначе
		ПараметрыОтбора.Вставить("ЗаказПокупателя", Объект.ЗаказПокупателя);
	КонецЕсли;
	ПараметрыОтбора.Вставить("НомерВариантаКП", 0);
	ПараметрыОтбора.Вставить("ИмяКолонкиСклада", "СтруктурнаяЕдиница");
	...
	ПараметрыОтбора.Вставить("ВидыЗаполненияПоЗаказНаряду", ВидыЗаполненияПоЗаказНаряду);
	
	ВидыЗаполненияПоЗаказНаряду = Новый Структура("КакВЗаказеПокупателя, КакВРасходнойНакладной");
	ВидыЗаполненияПоЗаказНаряду.КакВЗаказеПокупателя = Истина;
	ВидыЗаполненияПоЗаказНаряду.КакВРасходнойНакладной = Ложь;
	
	ПараметрыОтбора.ВидыЗаполненияПоЗаказНаряду = ВидыЗаполненияПоЗаказНаряду;
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыВРезерве Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				ПараметрыОтбора.Склад = Неопределено;
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыРазмещено И ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("Заказано", ПараметрыОтбора);
				ПоказатьВыборИзМеню(Оповещение, СписокДокументов, Элемент.ТекущийЭлемент);
...
//+mega
&НаКлиенте
Процедура ЗагрузкаИзФайлаПродукция(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата", ЭтотОбъект, НастройкиЗагрузкиДанных);
	
	НастройкиЗагрузкиДанных.Вставить("ПолноеИмяТабличнойЧасти", "ЗаказНаПроизводство.Продукция");
	НастройкиЗагрузкиДанных.Вставить("ИмяМакетаСШаблоном","ЗагрузкаИзФайлаПродукция");
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru = 'Загрузка запасов из файла'"));
	НастройкиЗагрузкиДанных.Вставить("ПоложениеЗаказаПокупателя", Объект.ПоложениеЗаказаПокупателя);
	НастройкиЗагрузкиДанных.Вставить("ПоложениеСклада", Объект.ПоложениеСклада);
	НастройкиЗагрузкиДанных.Вставить("ВидОперации", Объект.ВидОперации);
...
&НаКлиенте
Процедура mega_ЗаполнитьСотрудников(Команда)
	
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ОбработатьВыборСотрудникаЗавершение", ЭтотОбъект);
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("РежимВыборка", Истина);
	ПараметрыОткрытия.Вставить("ЗакрыватьПриВыборе", Истина);
	ПараметрыОткрытия.Вставить("МножественныйВыбор", Ложь);
	
	ОткрытьФорму("Справочник.Сотрудники.ФормаВыбора", ПараметрыОткрытия, ЭтаФорма,,,,
		ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
	
&НаКлиенте
Процедура ОбработатьВыборСотрудникаЗавершение(Результат, ДополнительныеПараметры)Экспорт 

	Если ЗначениеЗаполнено(Результат) Тогда 
		
		ЗаполнитьСотрудниковНаСервере(Результат);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСотрудниковНаСервере(Сотрудник)Экспорт 

	Для Каждого ТекСтрока Из Объект.Продукция Цикл 
		
		Если ТекСтрока.mega_Отметка Тогда 
			
			ТекСтрока.mega_Сотрудник = Сотрудник;
			ТекСтрока.mega_Отметка = Ложь;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
//-mega
...
//+mega
&НаКлиенте
Процедура mega_ОбновитьОстатки(Команда)
...
Процедура ПодборРесурсы(Команда)
	...
	ПараметрыОткрытия.Вставить("НомерПодсистемы", 2);
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	Если ПередаватьВидДокумента Тогда
		ПараметрыДиалога.Вставить("ВидДокумента", НСтр("ru = 'заказ на производство'"));
	КонецЕсли;	
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_ЗаказНаПроизводство);
...
//+mega
&НаСервере
Функция НоменклатураТабличнойЧасти(ИмяТабличнойЧасти)
...
&НаСервере
Функция ОстаткиЗапасовНаСкладах(Номенклатура)
	
	ДатаСреза = ?(Объект.Ссылка.Пустая(), ТекущаяДатаСеанса(), Объект.Дата);
	...
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница", Объект.mega_СтруктурнаяЕдиницаОстатков);
...
Процедура ЗаполнитьОстаткиНаСервере()
	...
	ЗаполнитьОстаткиГотовойПродукцииНаСервере();
	ЗаполнитьОстаткиЗапасовНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОстаткиГотовойПродукцииНаСервере()
	
	Номенклатура = НоменклатураТабличнойЧасти("Продукция");
	ОстаткиЗапасовНаСкладах = ОстаткиЗапасовНаСкладах(Номенклатура);
	Для Каждого ТекСтрока Из Объект.Продукция Цикл 
		ТекСтрока.mega_Остаток = 0;
		
		ОстаткиЗапасовНаСкладах.Сбросить();
		ПараметрыПоиска = Новый Структура("Номенклатура", ТекСтрока.Номенклатура);
		Если ОстаткиЗапасовНаСкладах.НайтиСледующий(ПараметрыПоиска) Тогда 
			
			ТекСтрока.mega_Остаток = ОстаткиЗапасовНаСкладах.Остаток;
		КонецЕсли;		
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОстаткиЗапасовНаСервере()
	
	Номенклатура = НоменклатураТабличнойЧасти("Запасы");
	ОстаткиЗапасовНаСкладах = ОстаткиЗапасовНаСкладах(Номенклатура);
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, Параметры) Экспорт
	...
	Если СтруктураДанные.ОтборТипНоменклатуры.Найти(ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ТипНоменклатуры) = Неопределено ИЛИ ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ЭтоНабор Тогда
		...
		ПараметрыШтрихкода.Вставить("ТипНоменклатуры", ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ТипНоменклатуры);
		ПараметрыШтрихкода.Вставить("ЭтоНабор", ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ЭтоНабор);
...
Процедура ОбработатьПодготовленныеДанные(АдресРезультатаЗагрузки)
	...
	Попытка
		...
		//+mega		
		Части = СтрРазделить(РезультатЗагрузки.НастройкиЗагрузкиДанных.ПолноеИмяТабличнойЧасти, ".", Ложь);
		ИмяТабличнойЧасти = Части[1];		
		//-mega
		...
		Если ИмяТабличнойЧасти = "Продукция" Тогда
			...
			ВременнаяТаблицаЗначений = Объект.Продукция.Выгрузить();
			ВременнаяТаблицаЗначений.Свернуть("Номенклатура,ЕдиницаИзмерения,Спецификация", 
											  "Количество");
			Объект.Продукция.Загрузить(ВременнаяТаблицаЗначений);		
		КонецЕсли;
		ЗаполнитьОстаткиНаСервере();
		//-mega
...
Процедура ЗаполнитьКэшЗначений()
	...
	КэшЗначений.Вставить("НаборСвойств_Документ_ЗаказНаПроизводство", НаборСвойств_Документ_ЗаказНаПроизводство);
...
Процедура ОбновитьКэшиДанныхСервер(МассивСотрудников = Неопределено)
	...
	Если ВыполнитьЗапрос Тогда
		...
		Иначе
			Запрос.УстановитьПараметр("ПериодСреза", КонецДня(ТекущаяДатаСеанса()));
		КонецЕсли;
		Запрос.Текст =
		"ВЫБРАТЬ
...
Процедура ЗаполнитьДанныеПродукцииВСтрокеОперации(Объект, СтрокаОперации, СтрокаПродукции)
	...
	Если СтрокаПродукции = Неопределено Тогда
		...
		ИзменяемыеПоля.Вставить("КлючСвязиПродукция", 0);
		ИзменяемыеПоля.Вставить("Номенклатура", ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка"));
		ИзменяемыеПоля.Вставить("Характеристика", ПредопределенноеЗначение(
			"Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка"));
		ИзменяемыеПоля.Вставить("Партия", ПредопределенноеЗначение("Справочник.ПартииНоменклатуры.ПустаяСсылка"));
		ИзменяемыеПоля.Вставить("Спецификация", ПредопределенноеЗначение("Справочник.Спецификации.ПустаяСсылка"));
	Иначе
		ИзменяемыеПоля = Новый Структура;
		ИзменяемыеПоля.Вставить("Номенклатура");
		ИзменяемыеПоля.Вставить("Характеристика");
		ИзменяемыеПоля.Вставить("Партия");
		ИзменяемыеПоля.Вставить("Спецификация");
		ИзменяемыеПоля.Вставить("КлючСвязиПродукция");
		ЗаполнитьЗначенияСвойств(ИзменяемыеПоля, СтрокаПродукции);
		ИзменяемыеПоля.КлючСвязиПродукция = СтрокаПродукции.КлючСвязи;
	КонецЕсли;
	...
	Если Объект.ПоложениеЗаказаПокупателя = ПредопределенноеЗначение(
		...
		Если СтрокаПродукции = Неопределено Тогда
			ИзменяемыеПоля.Вставить("ЗаказПокупателя", ПредопределенноеЗначение(
				"Документ.ЗаказПокупателя.ПустаяСсылка"));
		Иначе
			ИзменяемыеПоля.Вставить("ЗаказПокупателя", СтрокаПродукции.ЗаказПокупателя);
		КонецЕсли;
	Иначе
		ИзменяемыеПоля.Вставить("ЗаказПокупателя", Объект.ЗаказПокупателя);
	КонецЕсли;
...
Процедура ПоказатьДублиСтрок(Команда)
	...
	Если Не Элементы.ДекорацияОтборПоДублям.Видимость Тогда
		...
		ПараметрыОтбора.Вставить("ЗаказПокупателя", СтрокаТабличнойЧасти.ЗаказПокупателя);
...
Функция ИнформацияОбОтборе(НомерСтроки)
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Дубли строки %1'"), НомерСтроки);
