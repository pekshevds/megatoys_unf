...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если ЗачетАвансаПоЕдиномуНалоговомуСчету Тогда
		...
		Организация,
		КонецКвартала(Дата),
		Перечисления.ВидыЗаписейФормированийКУДиР.РасходыЕНП); 
		
		Если ДокументФормирования <> Неопределено Тогда
			СрокПоЕНП = РегламентированнаяОтчетностьУСН.ОпределитьСрокПлатежаПоЕНП(Дата);
			
			РежимЗаписи = ?(ДокументФормирования.ЗаписиКУДиР.Количество()>0, 
			РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.ОтменаПроведения);
			
			ДокументФормирования.Записать(РежимЗаписи);  
		КонецЕсли;
