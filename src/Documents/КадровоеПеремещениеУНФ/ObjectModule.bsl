...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.КадровыйПеревод") Тогда
		ЗаполнитьПоПереводуРеглУчет(ДанныеЗаполнения);
	Иначе
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.КадровоеПеремещениеУНФ.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ) 
	...
	|ИЗ
	|	Документ.КадровоеПеремещениеУНФ.НачисленияУдержания КАК ТаблицаНачисленияУдержания
...
// Заполнение по кадровому документу регламентированного учета
// Параметры:
//		КадровыйПеревод - ссылка на документ КадровыйПеревод
//
Процедура ЗаполнитьПоПереводуРеглУчет(КадровыйПеревод)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	МАКСИМУМ(ЕСТЬNULL(СтруктурныеЕдиницы.Ссылка, ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка))) КАК Подразделение,
	               ...
	               |	ЕСТЬNULL(КадровыйПереводНачисления.НомерСтроки, 0) КАК ПорядокНачисления,
	               ...
	               |			ПО КадровыйПереводНачисления.Начисление = ПланРасчетовНачисления.Ссылка
	               |		ПО КадровыйПеревод.Ссылка = КадровыйПереводНачисления.Ссылка
	               ...
	               |		ПО КадровыйПеревод.Подразделение = ПодразделенияОрганизаций.Ссылка
	               |ГДЕ
	               |	КадровыйПеревод.Ссылка = &ПереводРегл
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |	ЕСТЬNULL(КадровыйПереводНачисления.НомерСтроки, 0),
	               |	КадровыйПеревод.Дата
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ПорядокНачисления";
	
	Запрос.УстановитьПараметр("ПереводРегл", КадровыйПеревод);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		СтрСотрудник.Период = Выборка.ДатаНачала;
		...
		Пока Выборка.СледующийПоЗначениюПоля("ПорядокНачисления") Цикл
			...
			СтрНачисление.Актуальность = Истина;
