...
&НаКлиенте
Процедура ПодписьРуководителяНажатие(Элемент)
...
Процедура ТекущийСотрудникНачисленияУдержанияПриИзменении(Элемент)
	...
	// До исправления ошибки 30163206.
...
Процедура ЗаполнитьНачисленияУдержания(Команда)	
	...
	Если Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("КлючСвязи",
		СтрокаТабличнойЧасти.КлючСвязи)).Количество() > 0 Тогда

		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНачисленияУдержанияЗавершение", ЭтотОбъект,
			Новый Структура("СтрокаТабличнойЧасти", СтрокаТабличнойЧасти)), НСтр(
			"ru = 'Табличная часть ""Начисления и удержания"" будет очищена. Продолжить выполнение операции?'"),
...
Процедура ЗаполнитьНалогиНаДоходы(Команда)	
	...
	Если Объект.НалогиНаДоходы.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи)).Количество()
		> 0 Тогда

		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНалогиНаДоходыЗавершение", ЭтотОбъект,
			Новый Структура("СтрокаТабличнойЧасти", СтрокаТабличнойЧасти)), НСтр(
			"ru = 'Табличная часть ""Налоги на доходы"" будет очищена. Продолжить выполнение операции?'"),
			РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
Функция ПолучитьЗначениеБлокировкиСуммы(ВидНачисленияУдержания)
	...
	Возврат ВидНачисленияУдержания.Предопределенный
		ИЛИ НЕ ЗначениеЗаполнено(ВидНачисленияУдержания.Формула);
...
&НаСервере
// Процедура инициализирует управление видимостью 
// и заполняет вычисляемые значения на сервере
//
...
Процедура ОткрытьФормуРеквизитыПечати()
	...
	ОткрытьФорму("Обработка.РеквизитыПечати.Форма.РеквизитыПечатиКадровоеПеремещение", 
