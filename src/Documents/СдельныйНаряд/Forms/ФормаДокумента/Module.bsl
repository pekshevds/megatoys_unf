...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДополнительныеПараметры = УправлениеСвойствамиУНФ.ЗаполнитьДополнительныеПараметры(Объект,
		"ГруппаДополнительныеРеквизиты", , "КомандыСмТакже");
	...
	КурсВалюты = СтруктураПоВалюте.Курс;
	КратностьВалюты = СтруктураПоВалюте.Кратность;
	...
	Если ПравоДоступа("Изменение", Объект.Ссылка.Метаданные()) И Не ТолькоПросмотр Тогда
	...
	ЦеныИВалюта = ЦеныИВалютаКлиентСервер.ТекстНадписиЦеныИВалюта(Объект, ПредопределенноеЗначение(
		"Справочник.Валюты.ПустаяСсылка"), КурсНациональнаяВалюта, УчетВалютныхОпераций);
...
Процедура ОперацииПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		//+mega
		ТекущаяСтрока.Номенклатура = ГотоваяПродукцияПоУмолчанию();
		//-mega
	КонецЕсли; 
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_СдельныйНаряд);
...
//+mega
&НаСервере
Функция ГотоваяПродукцияПоУмолчанию()
	
	Возврат Справочники.Номенклатура.НайтиПоКоду("НФ-00000001");
КонецФункции
//-mega
...
Процедура ЗаполнитьКэшЗначений()
	...
	НаборСвойств_Документ_СдельныйНаряд = УправлениеСвойствами.НаборСвойствПоИмени("Документ_СдельныйНаряд");
	КэшЗначений.Вставить("НаборСвойств_Документ_СдельныйНаряд", НаборСвойств_Документ_СдельныйНаряд);
...
Процедура ОбновитьКэшиДанныхСервер(МассивСотрудников = Неопределено, МассивОпераций = Неопределено)
	...
	Запрос.УстановитьПараметр("ПериодСреза", ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, КонецДня(ТекущаяДатаСеанса())));
	Запрос.Текст =
	"ВЫБРАТЬ
...
// 
// Параметры:
//  РезультатЗакрытия - см. ЦеныИВалютаКлиентСервер.РезультатФормыЦеныИВалюта
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.ПересчитатьЦены Тогда
		...
		Если Объект.ВалютаДокумента = НациональнаяВалюта Тогда
			НовыйКурс = 1;
			СтарыйКурс = ?(РезультатЗакрытия.КурсПересчетаЦен.Курс = 0, 1, РезультатЗакрытия.КурсПересчетаЦен.Курс) * ?(
				РезультатЗакрытия.КурсПересчетаЦен.Кратность = 0, 1, РезультатЗакрытия.КурсПересчетаЦен.Кратность);
			КурсВалюты = 1;
			КратностьВалюты = 1;
		Иначе
			НовыйКурс = ?(РезультатЗакрытия.КурсПересчетаЦен.Курс = 0, 1, РезультатЗакрытия.КурсПересчетаЦен.Курс) * ?(
				РезультатЗакрытия.КурсПересчетаЦен.Кратность = 0, 1, РезультатЗакрытия.КурсПересчетаЦен.Кратность);
...
Процедура ПерезаполнитьПоВидуЦен()
	...
	|				ТОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены = КурсЦены.Валюта
	|				ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены = КурсЦены.Валюта
	|			КОНЕЦ";
