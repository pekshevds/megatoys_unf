...
Процедура ЗаполнитьПоЗаказуНаПроизводствоСборка(МассивЗаказов)
	...
	ЗаполнитьДатуЗакрытия = Ложь;
	Если ВыборкаШапка.ЗапланированыОперации Тогда
		
		Закрыт = Ложь;
		ЗаполнитьДатуЗакрытия = Истина;
	...
	Если Закрыт Или ЗаполнитьДатуЗакрытия Тогда
		Для каждого СтрокаТабличнойЧасти Из Операции Цикл
...
Процедура ЗаполнитьПоЗаказуНаПроизводствоРазборка(МассивЗаказов)
	...
	Если ВыборкаШапка.ЗапланированыОперации Тогда
		...
		|	ТаблицаОпераций.Исполнитель КАК Исполнитель,
		|	ТаблицаОпераций.НомерСтрокиОперации КАК НомерСтрокиОперации,
		|	ТаблицаОпераций.КоличествоПродукции КАК КоличествоПродукции
		|ПОМЕСТИТЬ Операции
		|ИЗ
		|	ТаблицаОпераций КАК ТаблицаОпераций";
...
Процедура ЗаполнитьПоСборкеЗапасовСборка(ДокументСсылкаСборкаЗапасов)
	...
	|	ТаблицаОпераций.Операция КАК Операция,
	|	ТаблицаОпераций.Этап КАК Этап
	|ПОМЕСТИТЬ ОперацииЗаказНаПроизводство
	|ИЗ
	|	Документ.ЗаказНаПроизводство.Продукция КАК ЗаказНаПроизводствоПродукция
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОпераций КАК ТаблицаОпераций
	|		ПО ЗаказНаПроизводствоПродукция.Спецификация = ТаблицаОпераций.Спецификация
	...
	|	
	|	ВЫБРАТЬ
	|		ОперацииЗаказНаПроизводство.Номенклатура,
	|		ОперацииЗаказНаПроизводство.Характеристика,
	|		ОперацииЗаказНаПроизводство.Операция,
	|		ОперацииЗаказНаПроизводство.Этап,
	|		ОперацииЗаказНаПроизводство.Количество
	|	ИЗ
	|		ОперацииЗаказНаПроизводство КАК ОперацииЗаказНаПроизводство
	|	ГДЕ
	|		НЕ &ОперацииПоЗаказу
	|		И &ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|	
	...
	|	ГДЕ
	|		ЗаказНаПроизводствоОперации.Ссылка = &ЗаказНаПроизводство
	|		И &ОперацииПоЗаказу
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	МассивРезультатов = Запрос.ВыполнитьПакет();
	ТаблицаОстатков = МассивРезультатов[4].Выгрузить();
