...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+mega
	Элементы.ЗапасыГруппаОтменаТоваров.Видимость = Ложь;
	//-mega
КонецПроцедуры
...
//+mega
&НаКлиенте
Процедура mega_ЗагрузкаИзExcel(Команда)
...
Процедура ДиалогОткрытияФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено И ВыбранныеФайлы.Количество() > 0 Тогда
		...
		ЗагрузитьДанныеФайлаExcelВТабличнуюЧастьЗапасы(ПоместитьВоВременноеХранилище(ДвоичныеДанные)); 
		...
		Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл 
			...
			СерииНоменклатурыУНФКлиентСервер.ОбновитьСтатусСерииВСтроке(Объект, СтрокаТабличнойЧасти, , Ложь);
...
&НаСервере
Процедура ЗагрузитьДанныеФайлаExcelВТабличнуюЧастьЗапасы(АдресВоВременномХранилище)
	...
	НомерСтроки = 2;
	Штрихкод = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 1, НомерСтроки, 1).ТекущаяОбласть.Текст);
	Пока Штрихкод <> "" Цикл 
		
		Количество = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 2, НомерСтроки, 2).ТекущаяОбласть.Текст);		
		Артикул = СокрЛП(ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 3, НомерСтроки, 3).ТекущаяОбласть.Текст);
		
		Номенклатура = Неопределено;
		
		Выборка = Справочники.НоменклатураПоставщиков.НоменклатураПоДаннымНоменклатурыПоставщика(Объект.Контрагент, , Артикул);
		...
		НоваяСтрокаТовары.mega_НомерЗаказа = Штрихкод;
		...
		Если ЗначениеЗаполнено(Номенклатура) Тогда 
			...
			НоваяСтрокаТовары.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.Общая, Объект.Дата);
			
			СтруктураДанные = Новый Структура;
			СтруктураДанные.Вставить("Номенклатура", Номенклатура);
			СтруктураДанные.Вставить("Характеристика", Неопределено);
			СтруктураДанные.Вставить("ЭтоНовыйОбъект", Истина);
			СтруктураДанные.Вставить("ДатаОбработки", Объект.Дата);
			СтруктураДанные.Вставить("Коэффициент", 1);
			СтруктураДанные.Вставить("ВалютаДокумента", Объект.ВалютаДокумента);
			СтруктураДанные.Вставить("ВидЦен", Объект.ВидЦен);
			НоваяСтрокаТовары.Цена = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДанные);
		КонецЕсли;
