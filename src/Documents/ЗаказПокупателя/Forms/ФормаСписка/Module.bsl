...
Процедура ДобавлениеНовыхРеквизитовТабличнойЧасти()
    ...
    НовыйРеквизит = Новый РеквизитФормы("Штрихкод",	
    	Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(36, ДопустимаяДлина.Переменная)),, "Штрихкод");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    ИзменитьРеквизиты(ДобавляемыеРеквизиты);		
КонецПроцедуры

&НаСервере
Процедура СоздатьРеквизитТабличнойЧасти(ИмяЭлемента, Вид, Группа)
...
&НаСервере
Функция СозданиеГруппы(ИмяГруппы)
	...
	Если ГруппаФормыГабаритыИВесБрутто = Неопределено Тогда
		...
		ГруппаФормыГабаритыИВесБрутто.Заголовок = ИмяГруппы;
...
Процедура СоздатьРеквизитыФормы()
	...
	ГруппаФормыДополнительно = СозданиеГруппы("Дополнительно");
	ДобавлениеНовыхРеквизитовТабличнойЧасти();			
	СоздатьРеквизитТабличнойЧасти("Штрихкод", ВидПоляФормы.ПолеВвода, ГруппаФормыДополнительно);
КонецПроцедуры

&НаСервере
Процедура ЗаполнениеШтрихкодаНаСервере(Ссылка)
	
	ЭтотОбъект.Штрихкод = СтрЗаменить(Строка(Ссылка.УникальныйИдентификатор()), "-", "");
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнениеШтрихкода()
	Штрихкод = "";
		
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
	
		ЗаполнениеШтрихкодаНаСервере(ТекущиеДанные.Ссылка);
	КонецЕсли;	
КонецПроцедуры
