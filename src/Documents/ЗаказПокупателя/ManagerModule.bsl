...
Процедура СформироватьЗаказПокупателя(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	|	ЗаказПокупателя.ТекстУсловийЗаказаПокупателя,
	|	ЗаказПокупателя.Работы.(
	|		НомерСтроки КАК НомерСтроки,
	|		0 КАК НомерВариантаКП,
	|		ВЫБОР
...
//+mega
Процедура ВывестиШтрихкод(ОбластьМакета, Имя, Штрихкод, ТипКода = 4)
	...
	ПараметрыШтрихкода.Ширина = Окр(ОбластьМакета.Рисунки[Имя].Ширина / КоличествоМиллиметровВПикселе);
	ПараметрыШтрихкода.Высота = Окр(ОбластьМакета.Рисунки[Имя].Высота / КоличествоМиллиметровВПикселе);
	ПараметрыШтрихкода.Штрихкод = Штрихкод;//СокрЛП(ЧисловойКодПоСсылке(Ссылка));
	ПараметрыШтрихкода.ТипКода = ТипКода; // Code128
	...
	ОбластьМакета.Рисунки[Имя].Картинка = ИзображениеШтрихкода.Картинка;
КонецПроцедуры

Функция ПолучитьДанныеДляПечатиСборочныхЛистов(МассивОбъектов)
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
	Запрос.УстановитьПараметр("СкладГП", Справочники.СтруктурныеЕдиницы.НайтиПоКоду("000000015"));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаказПокупателя.Ссылка КАК Ссылка,
	|	ЗаказПокупателя.Номер КАК Номер,
	...
	|	Запасы.НомерВариантаКП КАК НомерВариантаКП,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(Запасы.Номенклатура) = ТИП(СТРОКА)
	|			ТОГДА Запасы.Номенклатура
	|		КОГДА (ВЫРАЗИТЬ(Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|			ТОГДА Запасы.Номенклатура.Наименование
	|		ИНАЧЕ ВЫРАЗИТЬ(Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))
	|	КОНЕЦ КАК ПредставлениеНоменклатуры,
	...
	|	Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест КАК ЕдиницаИзмеренияМест,
	|	ВЫБОР
	|		КОГДА Запасы.ЕдиницаИзмерения = Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест
	|			ТОГДА ВЫБОР
	|					КОГДА Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
	|						ТОГДА 0
	|					ИНАЧЕ Запасы.Количество * Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест.Коэффициент
	|				КОНЕЦ
	...
	|							КОНЕЦ
	|					ИНАЧЕ Запасы.Количество
	|				КОНЕЦ * Запасы.Номенклатура.Вес
	|		ИНАЧЕ Запасы.Вес
	|	КОНЕЦ КАК Вес,
	|	ВЫБОР
	|		КОГДА Запасы.Объем = 0
	|			ТОГДА ВЫБОР
	|					КОГДА Запасы.ЕдиницаИзмерения = Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест
	|						ТОГДА ВЫБОР
	|								КОГДА Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
	...
	|							КОНЕЦ
	|					ИНАЧЕ Запасы.Количество
	|				КОНЕЦ * Запасы.Номенклатура.Объем
	|		ИНАЧЕ Запасы.Объем
	|	КОНЕЦ КАК Объем,
	|	Запасы.Партия КАК Партия,
	|	Запасы.mega_НомерЗаказа КАК НомерЗаказа,
	|	ЗапасыОстаткиСклада.КоличествоОстаток КАК ОстатокГП,
	|	ЗапасыОстаткиСкладов.КоличествоОстаток КАК Остаток
	|ИЗ
	|	Документ.ЗаказПокупателя.Запасы КАК Запасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.Остатки(, СтруктурнаяЕдиница = &СкладГП) КАК ЗапасыОстаткиСклада
	|		ПО Запасы.Номенклатура = ЗапасыОстаткиСклада.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.Остатки КАК ЗапасыОстаткиСкладов
	|		ПО Запасы.Номенклатура = ЗапасыОстаткиСкладов.Номенклатура
	|ГДЕ
	|	Запасы.Ссылка В(&МассивОбъектов)
	|	И (Запасы.Ссылка.ОсновнойВариантКП = 0
	|			ИЛИ Запасы.НомерВариантаКП = Запасы.Ссылка.ОсновнойВариантКП)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Запасы.Ссылка,
	|	НомерСтроки
	...
	|ГДЕ
	|	НоменклатураПоставщиков.Владелец В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ЗаказПокупателя.Контрагент КАК Ссылка
	|			ИЗ
	|				Документ.ЗаказПокупателя КАК ЗаказПокупателя
	|			ГДЕ
	|				ЗаказПокупателя.Ссылка В (&МассивОбъектов))
	...
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ЗаказПокупателяЗапасы.Номенклатура КАК Ссылка
	|			ИЗ
	|				Документ.ЗаказПокупателя.Запасы КАК ЗаказПокупателяЗапасы
	|			ГДЕ
	|				ЗаказПокупателяЗапасы.Ссылка В (&МассивОбъектов))";
...
Функция ПолучитьДанныеДляПечатиПротоколовСборки(МассивОбъектов)
	...
	|ПОМЕСТИТЬ втЗапасыЗаказовПокупателей
	|ИЗ
	|	Документ.ЗаказПокупателя.Запасы КАК Запасы
	|ГДЕ
	|	Запасы.Ссылка В(&МассивОбъектов)
	...
	|	Запасы.ЕдиницаИзмерения,
	|	Запасы.Ссылка,
	|	Запасы.НомерВариантаКП
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НоменклатураДополнительныеРеквизиты.Ссылка КАК Номенклатура,
	|	НоменклатураДополнительныеРеквизиты.Значение КАК Бренд
	|ИЗ
	...
	|	КОНЕЦ КАК Количество
	|ИЗ
	|	втЗапасыЗаказовПокупателей КАК Запасы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Запасы.Ссылка,
...
Функция ПолучитьДанныеДляПечатиРеестраНеотгруженныхПозиций(МассивОбъектов)
...
Функция ДанныеНоменклатурыПоставщика(НоменклатураПоставщиков, Поставщик, Номенклатура)
	...
	НоменклатураПоставщиков.Сбросить();
	Если НоменклатураПоставщиков.НайтиСледующий(Новый Структура("Поставщик,Номенклатура", Поставщик, Номенклатура)) Тогда 
		
		ЗаполнитьЗначенияСвойств(Структура, НоменклатураПоставщиков);
	КонецЕсли; 
	
	Возврат Структура;
КонецФункции

Процедура СформироватьСборочныйЛистВайлдберриз(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	
	ТабличныйДокумент = ПечатнаяФорма.ТабличныйДокумент;
	Макет = УправлениеПечатью.МакетПечатнойФормы(ПечатнаяФорма.ПолныйПутьКМакету);
	
	Результаты = ПолучитьДанныеДляПечатиСборочныхЛистов(МассивОбъектов);
	Шапка = Результаты[0].Выбрать();
	Запасы = Результаты[1].Выбрать();
	НоменклатураПоставщиков = Результаты[2].Выбрать();
	...
	Пока Шапка.Следующий() Цикл 
		...
		ДанныеПечати = Новый Структура;
		ДанныеПечати.Вставить("Накладная", Накладная);
		ДанныеПечати.Вставить("Контрагент", Шапка.Контрагент);
		
		ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		ОбластьМакета.Параметры.Заполнить(ДанныеПечати);
			
		Штрихкод = СтрЗаменить(XMLСтрока(Шапка.Ссылка.УникальныйИдентификатор()), "-", "");
		ВывестиШтрихкод(ОбластьМакета, "Штрихкод", Штрихкод, 16);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		КоличествоМест = 0;Объем = 0;
		Запасы.Сбросить();
		Пока Запасы.НайтиСледующий(Новый Структура("Ссылка", Шапка.Ссылка)) Цикл 
			
			ОбластьМакета = Макет.ПолучитьОбласть("Детали");							
			ОбластьМакета.Параметры.Заполнить(Запасы);
			ДанныеНоменклатурыПоставщика = ДанныеНоменклатурыПоставщика(
				НоменклатураПоставщиков, Шапка.Контрагент, Запасы.Номенклатура);					
			ОбластьМакета.Параметры.НоменклатураWB = ДанныеНоменклатурыПоставщика.Наименование;			
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			КоличествоМест = КоличествоМест + Запасы.КоличествоМест;
			Объем = Объем + Запасы.Объем;
		КонецЦикла;
		...
		ОбластьМакета.Параметры.Заполнить(ДанныеПечати);
		ВывестиШтрихкод(ОбластьМакета, "ШтрихкодЗакрыть", "9999999999994", 16);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
	КонецЦикла;
КонецПроцедуры // СформироватьСборочныйЛистВайлдберриз()

Процедура СформироватьСборочныйЛистОзон(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	Пока Шапка.Следующий() Цикл 
		...
		Пока Запасы.НайтиСледующий(Новый Структура("Ссылка", Шапка.Ссылка)) Цикл 
			...
			ДанныеНоменклатурыПоставщика = ДанныеНоменклатурыПоставщика(
				НоменклатураПоставщиков, Шапка.Контрагент, Запасы.Номенклатура);
			ОбластьМакета.Параметры.НоменклатураOzon = ДанныеНоменклатурыПоставщика.Наименование;
			ТабличныйДокумент.Вывести(ОбластьМакета);
						
			ОбластьМакета = Макет.ПолучитьОбласть("Подчерк");
			ОбластьМакета.Параметры.Заполнить(Запасы);
			ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
	КонецЦикла;
КонецПроцедуры // СформироватьСборочныйЛистОзон()

Процедура СформироватьСборочныйЛистОпт(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	Пока Шапка.Следующий() Цикл 
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
	КонецЦикла;
КонецПроцедуры // СформироватьСборочныйЛистОпт()

Процедура СформироватьСборочныйЛист(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	Пока Шапка.Следующий() Цикл 
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
	КонецЦикла;
КонецПроцедуры // СформироватьСборочныйЛист()

Процедура СформироватьПротоколСборки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	
	ТабличныйДокумент = ПечатнаяФорма.ТабличныйДокумент;
	Макет = УправлениеПечатью.МакетПечатнойФормы(ПечатнаяФорма.ПолныйПутьКМакету);
	
	Результаты = ПолучитьДанныеДляПечатиПротоколовСборки(МассивОбъектов);
	Бренды = Результаты[1].Выбрать();
	Шапка = Результаты[2].Выбрать();	
	Запасы = Результаты[3].Выгрузить();
	
	Запасы.Колонки.Добавить("Бренд");	
	Для Каждого ТекСтрока Из Запасы Цикл 
		Бренд = Справочники.ЗначенияСвойствОбъектов.ПустаяСсылка();
		
		Бренды.Сбросить();
		Если Бренды.НайтиСледующий(Новый Структура("Номенклатура", ТекСтрока.Номенклатура)) Тогда 
			
			Бренд = Бренды.Бренд;
		КонецЕсли;
		
		ТекСтрока.Бренд = Бренд;
	КонецЦикла;
	
	Запасы.Сортировать("Ссылка,Бренд,Артикул");
	...
	Пока Шапка.Следующий() Цикл 
		...
		Заголовок = СтрШаблон(
			НСтр("ru='Протокол сбоки № %1 от %2'"),
			ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс),
			ПечатьДокументовУНФ.ПредставлениеДатыВДокументах(Шапка.ДатаДокумента));
				
		ДанныеПечати = Новый Структура;
		ДанныеПечати.Вставить("Заголовок", Заголовок);
		ДанныеПечати.Вставить("Контрагент", Шапка.Контрагент);
		...
		НомерСтроки = 1;
		Строки = Запасы.НайтиСтроки(Новый Структура("Ссылка", Шапка.Ссылка));
...
Процедура СформироватьРеестрНеотгруженныхПозиций(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	
	ТабличныйДокумент = ПечатнаяФорма.ТабличныйДокумент;
	Макет = УправлениеПечатью.МакетПечатнойФормы(ПечатнаяФорма.ПолныйПутьКМакету);
	
	Результаты = ПолучитьДанныеДляПечатиРеестраНеотгруженныхПозиций(МассивОбъектов);
	Бренды = Результаты[1].Выбрать();
	Запасы = Результаты[2].Выгрузить();
	...
	Запасы.Сортировать("Бренд,Артикул");
	
	
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");	
	ОбластьМакета.Параметры.Заголовок = "Реестр неотгруженных позиций";
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
Функция ДанныеДляПечатиСпецификаций(ЗаказПокупателей)
	...
	Запрос.Текст = 
		...
		|	ДокументЗапасы.Номенклатура.Код КАК Код,
		|	ДокументЗапасы.Номенклатура.Артикул КАК Артикул,
		|	ДокументЗапасы.Номенклатура.Наименование КАК Наименование,
		|	ДокументЗапасы.Номенклатура.КатегорияНоменклатуры КАК Категория,
		|	ДокументЗапасы.Номенклатура.НаименованиеПолное КАК ПолноеНаименование,
		|	ДокументЗапасы.Номенклатура.НаименованиеПолное КАК РусскоеНаименование,
		|	ДокументЗапасы.Номенклатура.Штрихкод КАК Штрихкод,
		|	ДокументЗапасы.Номенклатура.СтранаПроисхождения КАК Страна,
		|	ДокументЗапасы.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаХраненияШт,
		|	ДокументЗапасы.Номенклатура.Длина КАК ДлинаШт,
		|	ДокументЗапасы.Номенклатура.Ширина КАК ШиринаШт,
		|	ДокументЗапасы.Номенклатура.Высота КАК ВысотаШт,
		|	ДокументЗапасы.Номенклатура.Вес КАК МассаБруттоШт,
		|	ДокументЗапасы.Номенклатура.mega_ВесНетто КАК МассаНеттоШт,
		|	ДокументЗапасы.Номенклатура.Объем КАК ОбъемШт,
		|	ДокументЗапасы.Номенклатура.mega_КоличествоНаПоддоне КАК КоличествоНаПоддонеШт,
		|	ДокументЗапасы.Цена КАК Цена,
		|	ВЫБОР
		|		КОГДА ДокументЗапасы.Номенклатура.mega_ЕдиницаИзмеренияМест = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА 1
		|		ИНАЧЕ ДокументЗапасы.Номенклатура.mega_ЕдиницаИзмеренияМест.Коэффициент
		|	КОНЕЦ КАК ШтвУп,
		|	ДокументЗапасы.Номенклатура.mega_ЕдиницаИзмеренияМест КАК ТипУпаковки,
		|	ВесИОбъемЕдиницТоваров.Вес КАК МассаБрутто,
		|	ВесИОбъемЕдиницТоваров.Объем КАК Объем,
		|	ВесИОбъемЕдиницТоваров.mega_ВесНетто КАК МассаНетто,
		|	ВесИОбъемЕдиницТоваров.mega_Высота КАК Высота,
		|	ВесИОбъемЕдиницТоваров.mega_Ширина КАК Ширина,
		|	ВесИОбъемЕдиницТоваров.mega_Длина КАК Длина,
		|	ВесИОбъемЕдиницТоваров.mega_КоличествоНаПоддоне КАК КоличествоНаПоддоне,
		|	ДокументЗапасы.Номенклатура.ТНВЭДГИСМ КАК ТНВЭД
		|ИЗ
		|	Документ.ЗаказПокупателя.Запасы КАК ДокументЗапасы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВесИОбъемЕдиницТоваров КАК ВесИОбъемЕдиницТоваров
		|		ПО ДокументЗапасы.Номенклатура = ВесИОбъемЕдиницТоваров.Номенклатура
		|			И ДокументЗапасы.Номенклатура.mega_ЕдиницаИзмеренияМест = ВесИОбъемЕдиницТоваров.ЕдиницаИзмерения
		|ГДЕ
		|	ДокументЗапасы.Ссылка В(&ЗаказПокупателей)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка
		|ИТОГИ ПО
	...
	Запрос.УстановитьПараметр("ЗаказПокупателей", ЗаказПокупателей);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	//+mega
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "СборочныйЛист_WB");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_СборочныйЛист_WB";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_СборочныйЛист_WB";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Сборочный лист для Wildberries'");
		
		СформироватьСборочныйЛистВайлдберриз(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "СборочныйЛист_OZON");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_СборочныйЛист_OZON";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_СборочныйЛист_OZON";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Сборочный лист для Ozon'");
		
		СформироватьСборочныйЛистОзон(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "СброчныйЛист_Опт");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_СброчныйЛист_Опт";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_СброчныйЛист_Опт";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Сборочный лист для Опт'");
		
		СформироватьСборочныйЛистОпт(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "СборочныйЛист");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_СборочныйЛист";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_СборочныйЛист";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Сборочный лист'");
		
		СформироватьСборочныйЛист(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, Обработки.mega_ПечатьСпецификации.ИдентификаторПечатнойФормы());
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = Обработки.mega_ПечатьСпецификации.КлючПараметровПечати();
		ПечатнаяФорма.ПолныйПутьКМакету = Обработки.mega_ПечатьСпецификации.ПолныйПутьКМакету();
		ПечатнаяФорма.СинонимМакета = Обработки.mega_ПечатьСпецификации.ПредставлениеПФ();
		
		ДанныеОбъектовПечати = ДанныеДляПечатиСпецификаций(МассивОбъектов);
		Обработки.mega_ПечатьСпецификации.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати);		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ПротоколСборки");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_ПротоколСборки";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_Протокол";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Протокол сборки'");
		
		СформироватьПротоколСборки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
		
	КонецЕсли;
	
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "РеестрНеотгруженныхПозиций");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателя_РеестрНеотгруженныхПозиций";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.mega_ПФ_MXL_РеестрНеотгруженныхПозиций";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Реестр неотгруженных позиций'");
		
		СформироватьРеестрНеотгруженныхПозиций(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//+mega
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СборочныйЛист_WB";
	КомандаПечати.Представление = НСтр("ru = 'Сборочный лист для Wildberries'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 45;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СборочныйЛист_OZON";
	КомандаПечати.Представление = НСтр("ru = 'Сборочный лист для Ozon'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 46;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СброчныйЛист_Опт";
	КомандаПечати.Представление = НСтр("ru = 'Сборочный лист для Опт'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru = 'Сборочный лист'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 51;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = Обработки.mega_ПечатьСпецификации.ИдентификаторПечатнойФормы();
	КомандаПечати.Представление = Обработки.mega_ПечатьСпецификации.ПредставлениеПФ();
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 52;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПротоколСборки";
	КомандаПечати.Представление = НСтр("ru = 'Протокол сборки'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 53;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "РеестрНеотгруженныхПозиций";
	КомандаПечати.Представление = НСтр("ru = 'Реестр неотгруженных позиций'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Функция ТекстЗапросаИнициализироватьДанныеДокументаЗаказНаряд()
	...
	|ИЗ
	|	Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|ГДЕ
