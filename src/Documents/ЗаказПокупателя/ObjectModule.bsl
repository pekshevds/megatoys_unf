...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//+mega
	УстановитьСтатусСборкиЗаказа(Ложь, ОсновнойВариантКП);
	
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		
		ПроверкаНаНаличиеОдинаковыхСтрок(Отказ)
	КонецЕсли;
	//-mega
КонецПроцедуры 

//+mega
Процедура УстановитьСтатусСборкиЗаказа(ЕстьУслуги = Ложь, ОсновнойВариантКП = 0) Экспорт
	...
	Если ЕстьУслуги Или ОсновнойВариантКП <> 0 Тогда
		...
		КонецЕсли;
		СтрокиЗаказа = Запасы.НайтиСтроки(ОтборСтрок);
	Иначе
		СтрокиЗаказа = Запасы;
	КонецЕсли;
	...
	Для Каждого СтрокаЗаказа Из СтрокиЗаказа Цикл
		...
		КоличествоСобрано = ?(СтрокаЗаказа.mega_Собран, СтрокаЗаказа.Количество, 0);
		
		Если КоличествоСобрано > 0 Тогда
			СборкаНачата = Истина;
		КонецЕсли;
		Если СтрокаЗаказа.Количество > КоличествоСобрано Тогда
...
Процедура ПроверкаНаНаличиеОдинаковыхСтрок(Отказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаказПокупателяЗапасы.Номенклатура.Артикул КАК Артикул,
	|	ЗаказПокупателяЗапасы.mega_НомерЗаказа КАК mega_НомерЗаказа,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказПокупателяЗапасы.mega_НомерЗаказа,
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыПоиска.Вставить("mega_НомерЗаказа", Выборка.mega_НомерЗаказа);
		
		Строки = Запасы.НайтиСтроки(ПараметрыПоиска);
		Если Строки.Количество() > 1 Тогда
			
			Отказ = Истина;
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон("Дублирование строк с ключами Артикул - %1, номер заказа - %2",
					Выборка.Артикул,
					Выборка.mega_НомерЗаказа)
			);
		КонецЕсли;
	КонецЦикла;
