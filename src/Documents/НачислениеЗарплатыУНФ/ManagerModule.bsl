...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаНачислениеЗарплаты, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Константы.ФункциональнаяОпцияВыполнятьРасчетНДФЛПоПрогрессивнойШкале
	|			ТОГДА ВЫРАЗИТЬ((НачислениеЗарплатыНачисленияУдержания.Сумма + НачислениеЗарплатыНачисленияУдержания.СуммаСПревышения) * КурсыВалютРасчетов.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютРасчетов.Кратность) КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ВЫРАЗИТЬ(НачислениеЗарплатыНачисленияУдержания.Сумма * КурсыВалютРасчетов.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютРасчетов.Кратность) КАК ЧИСЛО(15, 2))
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Константы.ФункциональнаяОпцияВыполнятьРасчетНДФЛПоПрогрессивнойШкале
	|			ТОГДА НачислениеЗарплатыНачисленияУдержания.Сумма + НачислениеЗарплатыНачисленияУдержания.СуммаСПревышения
	|		ИНАЧЕ НачислениеЗарплатыНачисленияУдержания.Сумма
	|	КОНЕЦ,
	|	НЕОПРЕДЕЛЕНО,
	|	НачислениеЗарплатыНачисленияУдержания.СуммаСПревышения,
	|	НачислениеЗарплатыНачисленияУдержания.Сумма
	|ИЗ
	|	Документ.НачислениеЗарплатыУНФ.НалогиНаДоходы КАК НачислениеЗарплатыНачисленияУдержания
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&МоментВремени,
	|				Валюта В
	|					(ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	&Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НачислениеЗарплатыНачисленияУдержания.ПериодРегистрации, МЕСЯЦ) КАК Период,
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(&НачислениеЗарплатыУНФ КАК СТРОКА(100))
	...
	|	ВЫРАЗИТЬ(&НачислениеЗарплатыУНФ КАК СТРОКА(100)) КАК СодержаниеПроводки
	|ИЗ
	|	ТаблицаНачисления КАК НачислениеЗарплатыНачисленияУдержания
	|ГДЕ
	...
	|	ИСТИНА,
	|	ВЫРАЗИТЬ(&НачислениеЗарплатыУНФ КАК СТРОКА(100))
	|ИЗ
	|	ТаблицаВзносов КАК НачислениеЗарплатыВзносы
	|ГДЕ
	|	(НачислениеЗарплатыВзносы.СчетЗатрат.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство)
	...
	|	ВЫБОР
	|		КОГДА НачислениеЗарплатыНачисленияВзносы.СчетЗатрат.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|			ТОГДА НачислениеЗарплатыНачисленияВзносы.Проект
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	...
	|ИЗ
	|	Документ.НачислениеЗарплатыУНФ.ПримененныеВычетыПоНДФЛ КАК ПримененныеВычетыПоНДФЛ
	|ГДЕ
	...
	|	ТаблицаНачисления.СуммаСПревышения КАК СуммаСПревышения,
	|	ТаблицаНачисления.СуммаДоПревышения КАК Сумма,
	|	ТаблицаНачисления.Сотрудник.Физлицо КАК ФизическоеЛицо,
	|	ТаблицаНачисления.СтруктурнаяЕдиница КАК Подразделение
	|ИЗ
	...
	|	Начисления.ВидОбязательногоСтрахованияСотрудников КАК ВидОбязательногоСтрахованияСотрудников,
	...
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРПоСуммарномуТарифу) КАК ВидОбязательногоСтрахованияСотрудников
	|	ИЗ
	...
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРНакопительная)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	...
	|		ВзносыЗаМесяц.ПериодРегистрации,
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРСтраховая)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	...
	|		ВзносыЗаМесяц.ПериодРегистрации,
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФФОМС)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	...
	|		ВзносыЗаМесяц.ПериодРегистрации,
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФСС)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	...
	|		ВзносыЗаМесяц.ПериодРегистрации,
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФССНесчастныеСлучаи)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	|		ВзносыЗаМесяц.ВидНачисленияУдержания = &ФССНесчастныеСлучаи
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ВзносыЗаМесяц.ПериодРегистрации,
	|		ВзносыЗаМесяц.Сумма,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРПоСуммарномуТарифу)
	|	ИЗ
	|		ТаблицаВзносов КАК ВзносыЗаМесяц
	|	ГДЕ
	|		ВзносыЗаМесяц.ВидНачисленияУдержания = &ВзносыПоЕдиномуТарифу) КАК Начисления
	|ГДЕ
	|	Начисления.Сумма <> 0
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВЫРАЗИТЬ(&ОрганизацияДляРегОтчетности КАК Справочник.Организации).РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	|ИЗ
	|	ТаблицаНачисления КАК ТаблицаНачисления
	|ГДЕ
	|	ТаблицаНачисления.ВидНачисленияУдержания = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.НалогНаДоходы)
	|	И ТаблицаНачисления.Сумма <> 0
	|	И ТаблицаНачисления.ПериодРегистрации >= &ДатаОбязательногоПримененияЕНП";
	...
	Запрос.УстановитьПараметр("ОбъединятьНакопительнуюИСтраховуюЧасть", ДокументСсылкаНачислениеЗарплаты.Дата >= УчетСтраховыхВзносовУНФ.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР());
	...
	Запрос.УстановитьПараметр("ВзносыПоЕдиномуТарифу", Справочники.ВидыНачисленийИУдержаний.ВзносыПоЕдиномуТарифу); 
	Запрос.УстановитьПараметр("УправленческиеРасходы", ПланыСчетов.Управленческий.УправленческиеРасходы);
	Запрос.УстановитьПараметр("ДатаОбязательногоПримененияЕНП", РегламентированнаяОтчетностьУСН.ДатаОбязательногоПримененияЕНП());
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыНалогоплательщиковСБюджетомПоНДФЛУНФ", МассивРезультатов[12].Выгрузить());
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаНачислениеЗарплаты, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = КурсовыеРазницыУНФ.ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоКредитамИЗаймам(Запрос.МенеджерВременныхТаблиц, НомерЗапроса, ЕстьСтруктурнаяЕдиница);
	МассивРезультатов = Запрос.ВыполнитьПакет();
...
Процедура СформироватьТаблицыПоУчетуЗаймов(ДокументСсылкаНачислениеЗарплаты, СтруктураДополнительныеСвойства)
	...
	|ИЗ
	|	Документ.НачислениеЗарплатыУНФ.ПогашениеЗаймов КАК НачислениеЗарплатыНачисленияУдержания
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&МоментВремени,
	|				Валюта В
	|					(ВЫБРАТЬ
