...
Функция ПолучитьСуммуДокумента() Экспорт
	...
	Для каждого СтрокаТЧ Из НалогиНаДоходы Цикл
		НоваяСтрока = ТаблицаНачислений.Добавить();
        НоваяСтрока.ВидНачисленияУдержания = СтрокаТЧ.ВидНачисленияУдержания;
        НоваяСтрока.Сумма = СтрокаТЧ.Сумма + СтрокаТЧ.СуммаСПревышения;
	КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СотрудникиУНФ.ОтразитьСтраховыеВзносыДоходы(Ссылка, ДополнительныеСвойства, Движения, Отказ);
	
	// Инициализация данных документа.
	Документы.НачислениеЗарплатыУНФ.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
	...
	СотрудникиУНФ.ОтразитьПримененныеВычетыПоНДФЛ(ДополнительныеСвойства, Движения, Отказ);
	СотрудникиУНФ.ОтразитьНДФЛДоходы(ДополнительныеСвойства, Движения, Отказ);
	СотрудникиУНФ.ОтразитьСтраховыеВзносыДоходы(Ссылка, ДополнительныеСвойства, Движения, Отказ);
	...
	// Контроль
	Документы.НачислениеЗарплатыУНФ.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	// Контроль
	Документы.НачислениеЗарплатыУНФ.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ, Истина);
	
КонецПроцедуры // ОбработкаУдаленияПроведения()

Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	
	Если НалогиНаДоходы.Количество() > 0 Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВидыНалогов.Ссылка КАК Ссылка,
		|	ВидыНалогов.Наименование КАК Наименование
		|ИЗ
		|	Справочник.ВидыНалогов КАК ВидыНалогов
		|ГДЕ
		|	(ВидыНалогов.СчетУчетаКВозмещению = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
		|			ИЛИ ВидыНалогов.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка))";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл   
			...
			// установка значений счетов налогов по умолчанию       
			УстановитьПривилегированныйРежим(Истина);
			ВидНалога = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
			ВидНалога.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Управленческий.Налоги");
			ВидНалога.СчетУчетаКВозмещению = ПредопределенноеЗначение("ПланСчетов.Управленческий.НалогиКВозмещению"); 
			ВидНалога.Записать();
