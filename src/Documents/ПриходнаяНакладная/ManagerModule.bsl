...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаУправленческий.СчетУчетаАвансовПокупателя.Валютный
	|			ТОГДА ТаблицаУправленческий.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
...
Процедура СформироватьТаблицуВзаиморасчетовПоОперацииВозврата(Запрос)
	...
	|		ИЛИ СУММА(ТаблицаДокумента.Сумма) <> 0
	|		ИЛИ СУММА(ТаблицаДокумента.СуммаВал) <> 0
	|		ИЛИ СУММА(ТаблицаДокумента.СуммаРег) <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	ОсвобождатьАвансАвтоматически = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаПриходнаяНакладная, "ОсвобождатьАвансАвтоматически");
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПриходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		// Интеркампани
		Запрос.УстановитьПараметр("МассивОрганизаций", ИнтеркампаниСервер.ОрганизацииОстатков(ДополнительныеСвойства.ДляПроведения.Организация));
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|			И ПродажиОбороты.НомерСтроки = РасходнаяНакладнаяЗапасы.НомерСтроки
	|ГДЕ
