...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МаршрутныйЛист") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МаршрутныйЛист", НСтр("ru = 'Маршрутный лист'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "МаршрутныйЛист"));
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Если ИмяМакета="МаршрутныйЛист" Тогда
		...
		|	МаршрутныйЛист.СлужбаДоставки КАК СлужбаДоставкиСсылка,
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаДокументов.СлужбаДоставкиСсылка) 
				И НЕ ВыборкаДокументов.СлужбаДоставкиСсылка = Справочники.СлужбыДоставки.ДоставкаСобственнымиСилами Тогда
...
Процедура ВыполнитьОтгрузку(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	Параметры.Свойство("ФормироватьДополнительно", ФормироватьДополнительно);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			КонецЕсли;
			СформироватьРасходнуюНакладную(Выборка, СозданныеНакладные, ФормироватьДополнительно);  
...
Процедура СформироватьРасходнуюНакладную(Выборка, СозданныеНакладные, ФормироватьДополнительно)
	...
	Если ФормироватьДополнительно = "УПД" Тогда
		Док.ЭтоУниверсальныйДокумент = Истина;  
	Иначе
		Док.ЭтоУниверсальныйДокумент = Ложь;  
	КонецЕсли; 
	...
	Если ФормироватьДополнительно <> Неопределено Тогда
		ПараметрыСчетаФактуры = СчетаФактурыУНФ.СоздатьСчетФактуру(Док.Ссылка, Док.Дата);
	КонецЕсли;
