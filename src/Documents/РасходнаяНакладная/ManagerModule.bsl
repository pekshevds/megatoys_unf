...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДоходыИРасходы.НаправлениеДеятельностиПродажи КАК НаправлениеДеятельности,
	|	ТаблицаДоходыИРасходы.Заказ КАК ЗаказПокупателя,
	|	ТаблицаДоходыИРасходы.СчетУчетаНДСПоРеализации КАК СчетУчета,
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	ВЫРАЗИТЬ(&ОтражениеНДС КАК СТРОКА(100)) КАК СодержаниеПроводки,
	|	0 КАК СуммаДоходов,
	|	СУММА(ТаблицаДоходыИРасходы.СуммаНДС) КАК СуммаРасходов,
	|	СУММА(ТаблицаДоходыИРасходы.СуммаНДС) КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаДоходыИРасходы
	|ГДЕ
	|	ТаблицаДоходыИРасходы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	ОсвобождатьАвансАвтоматически = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаРасходнаяНакладная, "ОсвобождатьАвансАвтоматически");
	СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ОсвобождатьАвансАвтоматически", ОсвобождатьАвансАвтоматически);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаРасходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		Если НЕ МассивРезультатов[0].Пустой()
			...
			ИЛИ НЕ МассивРезультатов[14].Пустой() Тогда
...
Функция ПолучитьДанныеДляПечатиСборочныхЛистов(МассивОбъектов)
	...
	|	РасходнаяНакладная.СуммаНДСДоставки КАК СуммаНДСДоставки,
	|	РасходнаяНакладная.СтавкаНДСДоставки КАК СтавкаНДСДоставки,
	|	ВЫРАЗИТЬ(РасходнаяНакладная.Комментарий КАК СТРОКА(1000)) КАК Комментарий
	|ИЗ
	|	Документ.РасходнаяНакладная КАК РасходнаяНакладная
	|ГДЕ
	|	РасходнаяНакладная.Ссылка В(&МассивОбъектов)
	...
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				РасходнаяНакладная.Контрагент КАК Ссылка
	|			ИЗ
	|				Документ.РасходнаяНакладная КАК РасходнаяНакладная
	|			ГДЕ
	|				РасходнаяНакладная.Ссылка В (&МассивОбъектов))
	...
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				РасходнаяНакладнаяЗапасы.Номенклатура КАК Ссылка
	|			ИЗ
	|				Документ.РасходнаяНакладная.Запасы КАК РасходнаяНакладнаяЗапасы
	|			ГДЕ
	|				РасходнаяНакладнаяЗапасы.Ссылка В (&МассивОбъектов))";
...
Функция ПолучитьДанныеДляПечатиПриложений(МассивОбъектов)
	...
	|	Запасы.Номенклатура.Артикул КАК Артикул,
	|	Запасы.Номенклатура.Штрихкод КАК Штрихкод,
	|	Запасы.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмерения,
	|	Запасы.Номенклатура.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКод,
	|	Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест КАК ЕдиницаИзмеренияМест,
	|	ВЫБОР
	|		КОГДА Запасы.ЕдиницаИзмерения = Запасы.Номенклатура.mega_ЕдиницаИзмеренияМест
	|			ТОГДА ВЫБОР
	...
	|	Запасы.СтавкаНДС.Ставка КАК СтавкаНДС
	|ИЗ
	|	Документ.РасходнаяНакладная.Запасы КАК Запасы
	|ГДЕ
	|	Запасы.Ссылка В(&МассивОбъектов)
...
Процедура СформироватьПриложениеДляЗагрузки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	
	ТабличныйДокумент = ПечатнаяФорма.ТабличныйДокумент;
	Макет = УправлениеПечатью.МакетПечатнойФормы(ПечатнаяФорма.ПолныйПутьКМакету);
	
	Результаты = ПолучитьДанныеДляПечатиПриложений(МассивОбъектов);
	Шапка = Результаты[0].Выбрать();
	Запасы = Результаты[1].Выбрать();
	Штрихкоды = Результаты[2].Выбрать();
	...
	Пока Шапка.Следующий() Цикл 
		...
		Накладная = СтрШаблон(
			НСтр("ru='Приложение к накладной № %1 от %2'"),
			ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс),
			ПечатьДокументовУНФ.ПредставлениеДатыВДокументах(Шапка.ДатаДокумента));
				
		ДанныеПечати = Новый Структура;
		ДанныеПечати.Вставить("Накладная", Накладная);
		ДанныеПечати.Вставить("Номер", XMLСтрока(Шапка.Номер));
		ДанныеПечати.Вставить("ДатаДокумента", XMLСтрока(Шапка.ДатаДокумента));
		...
		Пока Запасы.НайтиСледующий(Новый Структура("Ссылка", Шапка.Ссылка)) Цикл 
			...
			Штрихкоды.Сбросить();Штрихкод = "";
			Если Штрихкоды.НайтиСледующий(Новый Структура("Номенклатура", Запасы.Номенклатура)) Тогда 
				
				Штрихкод = Штрихкоды.Штрихкод;
			КонецЕсли;
			
			ОбластьМакета = Макет.ПолучитьОбласть("Детали");
			ОбластьМакета.Параметры.Заполнить(Запасы);			
			ОбластьМакета.Параметры.НомерСтроки = XMLСтрока(Запасы.НомерСтроки);
			ОбластьМакета.Параметры.Артикул = XMLСтрока(Запасы.Артикул);
			ОбластьМакета.Параметры.Код = XMLСтрока(Запасы.Код);
			ОбластьМакета.Параметры.Штрихкод = XMLСтрока(Штрихкод);
			ОбластьМакета.Параметры.ПредставлениеНоменклатуры = XMLСтрока(Запасы.ПредставлениеНоменклатуры);
			ОбластьМакета.Параметры.ЕдиницаИзмеренияКод = XMLСтрока(Запасы.ЕдиницаИзмеренияКод);
			ОбластьМакета.Параметры.ЕдиницаИзмерения = XMLСтрока(Запасы.ЕдиницаИзмерения);
			ОбластьМакета.Параметры.Количество = XMLСтрока(Запасы.Количество);
			ОбластьМакета.Параметры.Сумма = XMLСтрока(Запасы.Сумма);
			ОбластьМакета.Параметры.СуммаНДС = XMLСтрока(Запасы.СуммаНДС);
			ОбластьМакета.Параметры.СтавкаНДС = XMLСтрока(Запасы.СтавкаНДС);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ПриложениеДляЗагрузки");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасходнаяНакладная_ПриложениеДляЗагрузки";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.РасходнаяНакладная.mega_ПФ_MXL_ПриложениеДляЗагрузки";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Приложение к накладной (для загрузки в различные приложения)'");
		
		СформироватьПриложениеДляЗагрузки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПриложениеДляЗагрузки";
	КомандаПечати.Представление = НСтр("ru = 'Приложение к накладной (для загрузки в различные приложения)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
