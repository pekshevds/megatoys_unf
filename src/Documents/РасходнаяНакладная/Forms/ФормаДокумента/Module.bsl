...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая()
		...
		И ЭтотОбъект.ОснованиеСоздания = Неопределено Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоУниверсальныйДокумент 
		...
		И (Объект.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю) Тогда
...
//+mega
&НаКлиенте
Процедура mega_ЗаполнитьСтруктурныеЕдиницыТабличнойЧастиЗапасы(Команда)
	
	ЭтаФорма.Модифицированность = ЗаполнитьСтруктурныеЕдиницыТабличнойЧастиЗапасы();
КонецПроцедуры
//-mega
...
Функция ЗаполнитьСтруктурныеЕдиницыТабличнойЧастиЗапасы()
	...
	Запасы.Свернуть("Номенклатура");
	
	Номенклатура = Запасы.ВыгрузитьКолонку("Номенклатура"); 
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатФормыЦеныИВалюта, ДополнительныеПараметры) Экспорт
	...
	// ДисконтныеКарты
	Если ЗначениеЗаполнено(РезультатФормыЦеныИВалюта.ДисконтнаяКарта)
		И ЗначениеЗаполнено(РезультатФормыЦеныИВалюта.Контрагент)
...
Процедура ВосстановитьДанныеХарактеристикиПартииИзКэш()
	...
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Если НЕ ТабличнаяЧасть.Реквизиты.Найти("Характеристика") = Неопределено
			И Не ТабличнаяЧасть.Имя = "Подарки"
			Тогда
			МассивИменТабличныхЧастей.Добавить(ТабличнаяЧасть.Имя);
		КонецЕсли;
...
&НаСервере
Процедура ПолучитьЗапасыИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти, ЕстьХарактеристики, ЕстьПартии, РассчитатьЦены = Ложь)
...
Функция ВыполнитьРасчетСкидокНаСервере(ОбъектДляРасчета, СтруктураПараметры)
	...
	Возврат СкидкиНаценкиСерверПереопределяемый.ВыполнитьРасчетСкидокНаСервере(ОбъектДляРасчета, СтруктураПараметры);
...
Процедура ОтразитьСкидкиНаФорме(РезультатРасчета) 
	...
	Для Каждого ОписаниеДобавленнойСтроки Из РезультатРасчета.ДобавленныеСтрокиЗапасов Цикл 
		...
		КонецЕсли;
		КодАкцизнойМарки = ОписаниеДобавленнойСтроки.Значение;
		ОбработатьДобавлениеСтрокиЗапасов(ДобавленнаяСтрокаЗапасов, ИмяТЧ, КодАкцизнойМарки);
	КонецЦикла;
...
Процедура ОбновитьСостояниеПромокода()
	...
	ИспользуютсяПромокоды = ЭтоПродажа(ВидОперации) И ПолучитьФункциональнуюОпцию("ИспользоватьПромокоды");
