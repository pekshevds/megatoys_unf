...
Процедура ЗаполнитьПоОперацияПоПлатежнымКартам(ДокументСсылкаОперацияПоПлатежнымКартам) Экспорт
	...
	ЗначенияРеквизитовОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ДокументСсылкаОперацияПоПлатежнымКартам,
		"ВидОперации, СпособОплаты");
	
	ДокументСсылкаОперацияПоПлатежнымКартамВидОперации = ЗначенияРеквизитовОснования.ВидОперации;
	...
	Если ЗначенияРеквизитовОснования.СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаСБП Тогда
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДокументСсылкаОперацияПоПлатежнымКартам, ,
			"Номер, Дата, РасшифровкаПлатежа, ДополнительныеРеквизиты, 
			|ОперацияПоПлатежнымКартамШапка, НомерЧекаККМ, Проведен, 
			|ПометкаУдаления,СсылочныйНомер,ИдентификаторПлатежнойСистемы");
...
Процедура ЗаполнитьПоЧекККМ(ДокументСсылкаЧекККМ, НомерСтроки = 0)
	...
	Иначе
		...
		БанковскийСчет = ДокументСсылкаЧекККМ.ЭквайринговыйТерминал.БанковскийСчетЭквайринг;
		...
		ДоговорПоУмолчанию = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
			Ссылка, Контрагент, Организация, ВидОперации);
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	Если ТипЗнч(Выборка.ДокументОснованиеПриходнойНакладной) = Тип("ДокументСсылка.ЧекККМ")
		...
		ОперацияПоПлатежнымКартамШапка = Выборка.ДокументОснованиеПриходнойНакладной;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаСБП Тогда
		СсылочныйНомер = "";   
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		...
		Если СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаСБП Тогда
			ТекстСообщения = НСтр("ru = 'Способ оплаты должен быть выбран.'");
		Иначе
	...
	ИначеЕсли Не ЗначениеЗаполнено(ЭквайринговыйТерминал.Касса) ИЛИ
		...
		Не ЗначениеЗаполнено(ЭквайринговыйТерминал.СчетЗатрат) 
		И Не СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаСБП Тогда
		ТекстСообщения = НСтр("ru = 'В карточке эквайрингового терминала заполнены не все реквизиты. 
			|Откройте карточку эквайрингового терминала и проверьте реквизиты ""Эквайер"", ""Касса"", ""Банковский счет"" и ""Счет затрат"" (команда ""Счета учета""). 
