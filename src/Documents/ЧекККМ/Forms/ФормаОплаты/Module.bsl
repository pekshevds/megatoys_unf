...
&НаСервере
Процедура ЗаполнитьКонтактнуюИнформациюДляЧека(Очищать = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		Объект.АдресЭП = ДанныеКИКонтрагента.Получить(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		Объект.Телефон = ДанныеКИКонтрагента.Получить(Перечисления.ТипыКонтактнойИнформации.Телефон);
	ИначеЕсли Очищать Тогда
		Объект.Телефон = "";
		Объект.АдресЭП = "";
	КонецЕсли;
...
Процедура УстановитьВидимостьОтправкиЭлектронныхЧеков()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОтправкаЧека", "Видимость", ВидимостьОтправкиЧека);
	...
	Если ВидимостьОтправкиЧека Тогда
		Если НеПечататьБумажныйЧек Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Телефон", "АвтоОтметкаНезаполненного", Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресЭП", "АвтоОтметкаНезаполненного", Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НеПечататьБумажныйЧек = КассаККМ.НеПечататьБумажныйЧек;
			...
			Иначе
				Для каждого СтрокаОплатыКартой Из Объект.БезналичнаяОплата Цикл
					...
					ИначеЕсли СтрокаОплатыКартой.ВидОплаты = Перечисления.ВидыБезналичныхОплат.Бонусы Тогда
						СписатьБаллыНаСервере(НоваяСтрока);
					ИначеЕсли СтрокаОплатыКартой.ВидОплаты = Перечисления.ВидыБезналичныхОплат.Кредит Тогда
						ДобавитьКредитНаСервере(НоваяСтрока);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАвтоматическиеСкидкиНаценки") Тогда
		...
		Для Каждого СтрокаПодарка Из Объект.Подарки Цикл
			...
			СтрокаПодарка.СтатусыСерийНоменклатуры = СерииНоменклатурыУНФ.СтатусСерийНоменклатурыПриИзменении(СтруктураДанные);
		КонецЦикла;
	Иначе
		Элементы.НадписьАвтоСкидкиРассчитаны.Заголовок = "";
	КонецЕсли;
...
Процедура УстановитьВидимостьПриСозданииПоВидуКлиента()
	...
	Иначе
		...
		Если Объект.Предоплата.Количество() > 0
		 ...
		 //ИЛИ КоличествоПодарковВыдать > 0
...
Процедура НастроитьСуммуСкидкиНаСервере()
	...
	Для Каждого ТекущаяСтрока Из Объект.Запасы Цикл
		...
		Если ТекущаяСтрока.ПродажаПодарка Тогда
			КоличествоПодарковВыдать = КоличествоПодарковВыдать + 1;
		КонецЕсли;
	КонецЦикла;
	
	КоличествоПодарковВыдать = КоличествоПодарковВыдать + Объект.Подарки.Итог("Количество");
	Элементы.КоличествоПодарковВыдать.Видимость = (КоличествоПодарковВыдать > 0);
		
	СуммаСкидки = (Сумма - Объект.СуммаДокумента);
	Элементы.ГруппаСкидка.Видимость = (СуммаСкидки > 0) Или (КоличествоПодарковВыдать > 0);
...
Процедура НастроитьЭлементыНаСтраницеГруппаОплатаКартой()
	...
	Элементы.ПередатьНаТерминал.Видимость = НЕ ЭквайринговыйТерминал.Пустая() И Элементы.ЭквайринговыйТерминал.Видимость;
...
Процедура ПриОткрытии(Отказ)
	...
	ПересчитатьСуммы();
	УправлениеНебольшойФирмойКлиент.ЗаполнитьТелефонАдресЭП(ЭтаФорма, ТелефонАдресЭП, НеПечататьБумажныйЧек);
...
Процедура ОК(Команда)
	...
	ПараметрыЗакрытия.Кнопка = "Пробить чек";
	ПараметрыЗакрытия.ОбщаяОплата = ЭтоОбщаяОплата;
...
Процедура Отменить(Команда)
	...
	ПараметрыЗакрытия = ПараметрыЗакрытия();
	ПараметрыЗакрытия.Кнопка = "Отмена";
...
Процедура СписатьБаллыЗавершение(Результат, Параметры) Экспорт
	...
	СчитатьДисконтнуюКартуНажатиеЗавершение(Результат, Параметры);
	ПересчитатьСуммы();
	СписатьБаллыНаСервере();
...
Функция ПараметрыЗакрытия()
	...
	ПараметрыЗакрытия.Вставить("ОбщаяОплата", Ложь);
	ПараметрыЗакрытия.Вставить("Кнопка", "");
	ПараметрыЗакрытия.Вставить("Идентификатор", УникальныйИдентификатор);
...
Процедура ДобавитьОплатуКартой(ОднаКарта_МК = Ложь, ПробитьЧек = Ложь)
	...
	Если ИдентификаторУстройстваЭТ <> Неопределено 
		...
		ДополнительныеПараметры.Вставить("ОднаКарта_МК", ОднаКарта_МК);
		ДополнительныеПараметры.Вставить("ПробитьЧек", ПробитьЧек);
		
		Если ОднаКарта_МК Тогда
			Результат = Новый Структура;
			Результат.Вставить("ВидКарты", ВидКартыОплатаКартой);
			Результат.Вставить("НомерКарты", "");
			Результат.Вставить("Сумма", Дельта);
		...
		Иначе
			ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьОплатуКартойЗавершение", ЭтотОбъект,
...
Процедура ОплатитьКартой(Команда)
	...
	Если ДоступенОдинТерминал И НЕ ЭквайринговыйТерминал.Пустая()
		...
		Если НеПечататьБумажныйЧек Тогда
			Если Не (ЗначениеЗаполнено(Объект.Телефон) Или ЗначениеЗаполнено(Объект.АдресЭП)) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты или телефон'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьВидимостьОтФормыОплаты()
	...
	Если Объект.ФормаОплаты = ТипыДенежныхСредствНаличные Тогда
		...
		Если Объект.Подарки.Количество() > 0 Тогда
			Элементы.ГруппаОплата.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСверху;
			Для Каждого Страница Из Элементы.ГруппаОплата.ПодчиненныеЭлементы Цикл
				Если Страница = Элементы.ГруппаОплата.ТекущаяСтраница
					ИЛИ Страница = Элементы.ГруппаПодаркиКСписанию Тогда
	...
	Иначе
		...
		Элементы.ГруппаПодаркиКСписанию.Видимость = Объект.Подарки.Количество();
...
Процедура СписатьБонусыПриИзменении(Элемент)
	...
	Если СписатьБонусы И НЕ Объект.ДисконтнаяКарта.Пустая() Тогда
		ДанныеБонуснойКарты = Новый Структура;
		ДанныеБонуснойКарты.Вставить("БонуснаяКарта", Объект.ДисконтнаяКарта);
		ДанныеБонуснойКарты.Вставить("СуммаБонусов", МожноСписатьБонусов);
		ДанныеБонуснойКарты.Вставить("ВидОплаты", ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы"));
		ДанныеБонуснойКарты.Вставить("ЕстьОплатаБонусами", Истина);
...
Процедура КонтрагентПриИзмененииНаСервере()
	...
	Если НеПечататьБумажныйЧек Тогда
		ЗаполнитьКонтактнуюИнформациюДляЧека(Истина);
	КонецЕсли;
