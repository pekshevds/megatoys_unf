...
Процедура ЗаполнитьРеквизитыСообщения100(Документ, ЭлементDOM) Экспорт
	...
	Если Реквизиты.Свойство("assignmentParams") Тогда
		ЗагрузитьПараметрыНазначения(Документ, УзлыЭлементаDOM(Реквизиты.assignmentParams, ""));
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыНазначения(Документ, Назначение)
	...
	Если Назначение.Свойство("twoPrevYearsInsurers") Тогда
		Документ.Вставить("СоставСтрахователейЗаДваГода",
			Перечисления.ВариантыИзмененияСоставаСтрахователейСФР.ЗначениеИзXML(
				СериализацияБЗК.СтрокаИзXML(Назначение.twoPrevYearsInsurers)));
	КонецЕсли;
