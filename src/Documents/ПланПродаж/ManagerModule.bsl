...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПланПродаж, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА УчетПотребностиПоСкладам.Значение
	|			ТОГДА ПланПродажЗапасы.Ссылка.Склад
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта = &Валюта) КАК КурсВалютыУчета
	|		ПО (ИСТИНА),
	|	Константа.УчетПотребностиПоСкладам КАК УчетПотребностиПоСкладам
	|ГДЕ
