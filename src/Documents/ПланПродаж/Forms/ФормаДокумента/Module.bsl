...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦенообразованиеСервер.ДобавитьКолонкуИЗаполнитьМинимальныеЦеныВТЧ(ЭтотОбъект, "ТаблицаПланирования");
	...
	МобильныйКлиентУНФ.НастроитьФормуОбъектаМобильныйКлиент(Элементы, "ТаблицаПланирования");
...
Процедура ДатаПриИзменении(Элемент)
	...
	ДанныеДляИзмененияДаты = ДанныеДляИзмененияДаты(Объект.Дата, Объект.ВалютаДокумента);
	
	Если ДанныеДляИзмененияДаты.ВалютаКурсКратность.Курс <> Объект.Курс ИЛИ ДанныеДляИзмененияДаты.ВалютаКурсКратность.Кратность <> Объект.Кратность Тогда
		Объект.Курс = ДанныеДляИзмененияДаты.ВалютаКурсКратность.Курс;
		Объект.Кратность = ДанныеДляИзмененияДаты.ВалютаКурсКратность.Кратность;
...
Процедура ТаблицаПланированияЕдиницаИзмеренияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если КэшЗначений.ИспользоватьМинимальныеЦены
		И ТекущиеДанные.Свойство("МинимальнаяЦена") Тогда
		
		ТекущиеДанные.МинимальнаяЦена = ТекущиеДанные.МинимальнаяЦена * ОтношениеКоэффициентовЕдиницИзмерения(ТекущиеДанные.ЕдиницаИзмерения, ВыбранноеЗначение);
...
Процедура УстановитьПериод(Команда)
	...
	ОбщегоНазначенияУНФКлиент.РедактироватьПериод(
		Объект, Новый Структура("ДатаНачала, ДатаОкончания", "НачалоПланирования", "ОкончаниеПланирования"),
...
Процедура Подбор(Команда)
	...
	ПодборНоменклатурыВДокументахКлиент.ОткрытьФормуПодбораНоменклатуры(ЭтотОбъект, "ТаблицаПланирования");
...
Процедура ПланВыполнитьДействие(Команда)
	...
	Если ПланИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент") Тогда
		...
		ПриводитьКМинимальнымЦенам = КэшЗначений.ИспользоватьМинимальныеЦены;
		...
		Для Каждого СтрокаТаблицы Из ОтмеченныеСтроки Цикл
			...
			Для Индекс = 0 По Периоды.Количество()-1 Цикл
				...
				Если ПриводитьКМинимальнымЦенам 
					И СтрокаТаблицы.Свойство("МинимальнаяЦена") Тогда

					Если РеквизитыСценария.ПланироватьКоличество Тогда
						КоличествоПоСтроке = СтрокаТаблицы["КолонкаКоличество_" + Индекс];
					Иначе
						КоличествоПоСтроке = 1;
					КонецЕсли;

					МинимальнаяСумма = СтрокаТаблицы.МинимальнаяЦена * КоличествоПоСтроке;
					СуммаПоКолонке = СтрокаТаблицы["КолонкаСумма_" + Индекс];
					Если СуммаПоКолонке < МинимальнаяСумма Тогда
						
						СтрокаТаблицы["КолонкаСумма_" + Индекс] = МинимальнаяСумма;
	...
	ИначеЕсли ПланИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ОкруглитьЦены") Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтмеченныеСтроки Цикл
			СтрокаТаблицы.Цена = ОкруглитьЦену(СтрокаТаблицы.Цена, ПланИзменениеСтрокЗначение, Ложь);
		КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из ОтмеченныеСтроки Цикл
			...
			Для Индекс = 0 По Периоды.Количество()-1 Цикл
				...
				Если ПриводитьКМинимальнымЦенам 
					...
					МинимальнаяСумма = СтрокаТаблицы.МинимальнаяЦена * КоличествоПоСтроке;
					СуммаПоКолонке = СтрокаТаблицы["КолонкаСумма_" + Индекс];
					Если СуммаПоКолонке < МинимальнаяСумма 
						ИЛИ КоличествоПоСтроке = 0 Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, 
		"ТаблицаПланирования.Цена", 
		Новый ПолеКомпоновкиДанных("ТаблицаПланирования.МинимальнаяЦена"), ВидСравненияКомпоновкиДанных.Меньше);
		
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ТаблицаПланированияЦена");
...
Процедура ОбновитьСтруктуруТаблицыПланирования()
	...
	// 6. Сдвиг итоговых показатели в конец таблицы
...
Процедура ЗаполнитьТаблицуПериодов(ТаблицаПериоды, Знач ДатаНачала, Знач ДатаОкончания, Знач Периодичность)
	...
	ДобавлениеДатаОкончания = ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(ДатаНачала, Периодичность, 1);
	...
	Пока ДобавлениеДатаНачала < КонецДня(ДатаОкончания) Цикл
		...
		НоваяСтрока.ЗаголовокКолонки = ПланированиеКлиентСервер.СформироватьЗаголовокПериода(ДобавлениеДатаНачала,
			ДобавлениеДатаОкончания, Периодичность);
		
		ДобавлениеДатаНачала = ПланированиеКлиентСервер.РассчитатьДатуНачалаПериода(ДобавлениеДатаОкончания + 1,
			Периодичность);
		ДобавлениеДатаОкончания = ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(ДобавлениеДатаОкончания + 1,
			Периодичность, 1);
	...
	Если Форма.УчетВалютныхОпераций Тогда
		Если ЗначениеЗаполнено(Объект.ВалютаДокумента) Тогда
			...
			Если Объект.ВалютаДокумента <> УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту() Тогда
				...
				ТекстНадписи = СтрЗаменить(ТекстНадписи, "%Валюта%", СокрЛП(УправлениеНебольшойФирмойПовтИсп.ПолучитьСимвольноеПредставлениеВалюты(Объект.ВалютаДокумента)));
				ТекстНадписи = СтрЗаменить(ТекстНадписи, "%Курс%", СокрЛП(Строка(Объект.Курс)));
			Иначе
...
Функция ПолучитьСтруктуруПересчетаЦены(Объект, СтрокаТЧ)
	...
	Результат.Вставить("ДатаОбработки", Объект.Дата);
	Результат.Вставить("ВалютаДокумента", Объект.ВалютаДокумента);
	Результат.Вставить("Номенклатура", СтрокаТЧ.ОбъектПланирования);
	Результат.Вставить("Характеристика", СтрокаТЧ.Характеристика);
	Результат.Вставить("Коэффициент", 1);
	
	ЦенообразованиеКлиентСервер.ДополнитьСтруктуруДанныхНоменклатурыПолямиЦенообразования(Объект, Неопределено, Результат);
	
	Результат.Вставить("МинимальнаяЦена", ЦенообразованиеСервер.ПолучитьМинимальнуюЦенуНоменклатуры(Результат));
...
Процедура ПолучитьДанныеСервер(Действия)
	...
	Если Действия.Свойство("ЕдиницаИзмерения") Тогда
		Действия.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Действия.ЕдиницаИзмерения,
			"ЕдиницаИзмерения");
	КонецЕсли;
	
	Если Действия.Свойство("ИспользуютсяХарактеристики") Тогда
		Действия.ИспользуютсяХарактеристики = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			Действия.ИспользуютсяХарактеристики, "ИспользоватьХарактеристики");
	КонецЕсли;
	
	Если Действия.Свойство("Цена") Тогда
		
		Если Действия.Цена.Свойство("МинимальнаяЦена") Тогда
			Действия.Вставить("МинимальнаяЦена", Действия.Цена.МинимальнаяЦена);
		КонецЕсли;
		
		Действия.Цена = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(Действия.Цена);
...
Процедура ПерезаполнитьЦены(ВидЦен = Неопределено, ПараметрыОтбора = Неопределено)
	...
	Запрос.Текст =
		...
		|	МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены КАК ВалютаМинимальнойЦены,
		...
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(МинимальныеЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения) = ТИП(Справочник.ЕдиницыИзмерения)
		|			ТОГДА ВЫРАЗИТЬ(МинимальныеЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК КоэффициентЕдиницыИзмеренияВМинимальныхЦенах,
		|	ВЫБОР
		...
		|						ИЗ
		|							втТаблицаНоменклатуры)
		|					И ВидЦен = &МинимальныйВидЦен) КАК МинимальныеЦеныНоменклатурыСрезПоследних
		|		ПО втТаблицаНоменклатуры.Номенклатура = МинимальныеЦеныНоменклатурыСрезПоследних.Номенклатура
		|			И втТаблицаНоменклатуры.Характеристика = МинимальныеЦеныНоменклатурыСрезПоследних.Характеристика
		|			И (МинимальныеЦеныНоменклатурыСрезПоследних.Актуальность = ИСТИНА)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	втЦены.МинимальнаяЦена * ЕСТЬNULL(КурсыВалютыМинимальныйВидЦены.Курс, 1) * ЕСТЬNULL(КурсыВалютыДокумента.Кратность, 1) / (ЕСТЬNULL(КурсыВалютыДокумента.Курс, 1) * ЕСТЬNULL(КурсыВалютыМинимальныйВидЦены.Кратность, 1)) * втЦены.КоэффициентЕдиницыИзмеренияВДокументе / втЦены.КоэффициентЕдиницыИзмеренияВМинимальныхЦенах КАК МинимальнаяЦена
		|ИЗ
		|	втЦены КАК втЦены
		...
		|						втЦены.ВалютаЦены
		|					ИЗ
		|						втЦены)) КАК КурсыВалютыМинимальныйВидЦены
		|		ПО втЦены.ВалютаЦены = КурсыВалютыМинимальныйВидЦены.Валюта
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	...
	Запрос.УстановитьПараметр("МинимальныйВидЦен", ЦенообразованиеСерверПовтИсп.ПолучитьОбщийВидМинимальныхЦен());
	...
	Для Каждого СтрокаТаблицы Из ЗаполняемаяКоллекция Цикл
		...
		Если КэшЗначений.ИспользоватьМинимальныеЦены Тогда
			...
			СтрокаТаблицы.МинимальнаяЦена = Выборка.МинимальнаяЦена;
		...
		КонецЕсли;
		СтрокаТаблицы.Цена = ЦенообразованиеСервер.ОкруглитьЦену(
			Цена,
...
//
&НаСервереБезКонтекста
Функция ДанныеДляИзмененияДаты(ДатаНовая, ВалютаРасчетов)
...
&НаСервере
Процедура ОбновитьКурсКратность()
	
	КурсКратность = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Объект.ВалютаДокумента, Объект.Дата);
...
// Требуется для использования на клиенте без серверного вызова.
// См. ЦенообразованиеСервер.ОкруглитьЦену
//
// Параметры:
...
Процедура ЦеныИВалютаФормаЗакрыта(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.БылиВнесеныИзменения Тогда
		...
		ЗаполнитьЗначенияСвойств(Объект, Результат, "ВидЦен, ВалютаДокумента");
		ОбновитьКурсКратность();
