...
Процедура ИзменитьПериод(Форма)
	...
	Форма.ДатаНачала = НачалоДня(ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(
		Форма.НачалоПланирования, Форма.Периодичность, Форма.КоличествоПериодов * Форма.СдвигПериода) + 86400);
	Форма.ДатаОкончания = ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(
