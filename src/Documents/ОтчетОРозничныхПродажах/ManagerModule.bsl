...
Процедура УдалитьДвиженияПоРегиструЗапасыВПробитыхЧекахККМ(КассоваяСменаОбъект) Экспорт
	...
	Пока ВыборкаЧековККМ.Следующий() Цикл
		...
		//	Набор.Записать(Истина);
		//	
		//	Набор = РегистрыНакопления.ДвиженияСерииНоменклатуры.СоздатьНаборЗаписей();
		//	Набор.Отбор.Регистратор.Установить(ВыборкаЧековККМ.Ссылка);
		//	Набор.Записать(Истина);
		//КонецЕсли;
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	|	И ВременнаяТаблицаОплатаПлатежнымиКартами.ВидОплаты В (ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.БанковскаяКарта), ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.СБП), ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.Кредит), ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.ВыдачаНаличных))";
