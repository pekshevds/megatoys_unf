...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВнеоборотныйАктив.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ТаблицаДокумента.ВнеоборотныйАктив.СчетУчетаНДСПоРеализации
	...
	|	ТаблицаДокумента.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ТаблицаДокумента.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ТаблицаДокумента.Всего
	|			ИНАЧЕ ТаблицаДокумента.Всего * ТаблицаДокумента.Ссылка.Курс / ТаблицаДокумента.Ссылка.Кратность
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаВА КАК ПередачаВА
	|		ПО ТаблицаДокумента.Ссылка = ПередачаВА.Ссылка,
	|	Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	|
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПередачаВА, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияВнеоборотныеАктивыИзменение
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			КонтрольОстатковУНФ.РасчетыСПокупателями(ДокументОбъектПередачаВА, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по амортизации имущества.
		Если НЕ МассивРезультатов[1].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
			КонтрольОстатковУНФ.ВнеоборотныеАктивы(ДокументОбъектПередачаВА, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("НачислениеАмортизации", СодержанияПроводок.НачислениеАмортизации());
	Запрос.УстановитьПараметр("ПрочиеДоходы", СодержанияПроводок.ПрочиеДоходы());
	...
	|	ТаблицаДоходыИРасходы.ВнеоборотныйАктив,
	|	ТаблицаДоходыИРасходы.Проект,
	|	&ОтражениеНДС,
	|	0,
	|	ТаблицаДоходыИРасходы.СуммаНДС,
	|	ТаблицаДоходыИРасходы.СуммаНДС
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДоходыИРасходы
	|ГДЕ
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства)
	...
	РасчетыПроведениеДокументов.ПодготовитьТаблицуВзаиморасчетовСУчетомАвансов(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства, Ложь);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства)
	...
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачислениеАмортизации", СодержанияПроводок.НачислениеАмортизации());
	Запрос.УстановитьПараметр("СписаниеАмортизации", СодержанияПроводок.СписаниеАмортизации());
	Запрос.УстановитьПараметр("ПрочиеРасходы", СодержанияПроводок.ПрочиеРасходы());
	Запрос.УстановитьПараметр("ПрочиеДоходы", СодержанияПроводок.ПрочиеДоходы());
	...
	|	&ОтражениеНДС
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаУправленческий
	|ГДЕ
