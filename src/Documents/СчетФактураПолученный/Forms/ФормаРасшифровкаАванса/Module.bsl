...
&НаСервере
Процедура ПеренестиЗапасыВТабличнуюЧастьАвансов(ДокументСсылка)
	
	ТаблицаЗапасов = ДокументСсылка.Запасы.Выгрузить();
...
Процедура УстановитьПараметрыВыбора()
	...
	НовыйПараметрВидОперации = Новый ПараметрВыбора("Отбор.ВидОперации", Перечисления.ВидыОперацийСчетФактураПолученный.Аванс);
...
Функция ПараметрыДоплатыКАвансу(ДанныеСтроки)
	...
	|ГДЕ
	|	СчетФактураПолученныйАвансы.Ссылка = &КорректируемыйСФ
	|	И (СчетФактураПолученныйАвансы.СтавкаНДС = &СтавкаНДС
	|			ИЛИ СчетФактураПолученныйАвансы.СтавкаНДС = &СтавкаНДСРасчетная)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КэшЗначений.Вставить("ЭтоКорректировкаАвансаНДС20", (КэшЗначений.ВидОперации = Перечисления.ВидыОперацийСчетФактураПолученный.КорректировкаАванса));
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораЗаказаПоставщику", ЭтотОбъект);
	ОткрытьФорму("Документ.ЗаказПоставщику.Форма.ФормаВыбора", ПараметрыОткрытия, ЭтотОбъект, , , , ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораЗаказаПоставщику(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		
		ПеренестиЗапасыВТабличнуюЧастьАвансов(Результат);
...
Процедура ЗаполнитьПоСчету(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораСчетаНаОплатуПоставщика", ЭтотОбъект);
	ОткрытьФорму("Документ.СчетНаОплатуПоставщика.ФормаВыбора", ПараметрыОткрытия, ЭтотОбъект, , , , ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораСчетаНаОплатуПоставщика(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
