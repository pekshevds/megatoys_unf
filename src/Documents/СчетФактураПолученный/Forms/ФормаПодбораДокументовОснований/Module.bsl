...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОснованияСчетаФактурыПолученного, "Контрагент", Параметры.Контрагент, Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОснованияСчетаФактурыПолученного, "Контрагент", Неопределено, Истина);
	КонецЕсли;
	...
	Если Параметры.АвансыДоступны = Ложь Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Договор) Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОснованияСчетаФактурыПолученного, "Договор", Параметры.Договор, Истина);
		Иначе
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОснованияСчетаФактурыПолученного, "Договор", Неопределено, Истина);
		КонецЕсли; 
	Иначе	
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОснованияСчетаФактурыПолученного, "Договор", Неопределено, Ложь);
	КонецЕсли;
	...
	ЗаголовокКонтрагента = НСтр("ru ='Контрагент: %1 '") + ?(ЗначениеЗаполнено(Параметры.Договор), НСтр("ru ='(%2)'"), "");
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияОтборКонтрагентДоговор", "Заголовок", 
	СтрШаблон(ЗаголовокКонтрагента, Параметры.Контрагент, 
	?(ЗначениеЗаполнено(Параметры.Договор),Параметры.Договор, Неопределено)));
