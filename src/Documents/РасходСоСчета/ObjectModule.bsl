...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если РасшифровкаПлатежа.Количество() = 0 Тогда
		...
		РасшифровкаПлатежа[0].СпособЗачета = РасчетыРаботаСФормамиВызовСервера.СпособРазнесенияОплатыДляДоговора(РасшифровкаПлатежа[0].Договор, Контрагент);
			...
			Для Каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				...
				ИначеЕсли УчитыватьВНУ И СтрокаРасшифровкаПлатежа.ПризнакАванса Тогда
					...
					Если ТипЗнч(СтрокаРасшифровкаПлатежа.Документ) = Тип("ДокументСсылка.ПоступлениеВКассу")
						...
						УСНИПатентДокументаВРасшифровке =
						УправлениеНебольшойФирмойВызовСервера.ЗначенияРеквизитовОбъекта(СтрокаРасшифровкаПлатежа.Документ, "УчитыватьВНУ, Патент");
						Если НЕ УСНИПатентДокументаВРасшифровке.УчитыватьВНУ И НЕ УСНИПатентДокументаВРасшифровке.Патент.Пустая() Тогда
							ТекстСообщения = СтрШаблон(НСтр(
