...
//	
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылка) Экспорт
		
	Если ДокументСсылка.ВидОперации <> Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Нельзя ввести Отчет комитенту на основании операции ""%1""'"),
	...
	ДокументОснованияНужногоТипа = ЗначениеЗаполнено(ДокументОснование) 
	И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") И ДокументОснование.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию;
	
	Запрос = Новый Запрос;
	
	Если ДокументОснованияНужногоТипа Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НакладнаяЗапасы.Номенклатура КАК Номенклатура,
		|	НакладнаяЗапасы.Характеристика КАК Характеристика,
		|	НакладнаяЗапасы.Партия КАК Партия,
		|	НакладнаяЗапасы.Количество КАК Количество,
		|	НакладнаяЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	НакладнаяЗапасы.Цена КАК Цена,
		|	НакладнаяЗапасы.Сумма КАК Сумма,
		|	НакладнаяЗапасы.СтавкаНДС КАК СтавкаНДС,
		|	НакладнаяЗапасы.СуммаНДС КАК СуммаНДС,
		|	НакладнаяЗапасы.СерииНоменклатуры КАК СерииНоменклатуры,
		|	НакладнаяЗапасы.СтранаПроисхождения КАК СтранаПроисхождения,
		|	НакладнаяЗапасы.НомерГТД КАК НомерГТД,
		|	НакладнаяЗапасы.КлючСвязи КАК КлючСвязи,
		|	ДокОснование.Организация КАК Организация,
		|	ДокОснование.Контрагент КАК Контрагент,
		|	ДокОснование.Договор КАК Договор,
		|	ДокОснование.ВалютаРасчетов КАК ВалютаРасчетов,
		|	ДокОснование.ВидЦенКонтрагента КАК ВидЦенКонтрагента,
		|	ДокОснование.ВалютаДокумента КАК ВалютаДокумента,
		|	ДокОснование.НалогообложениеНДС КАК НалогообложениеНДС,
		|	ДокОснование.СуммаВключаетНДС КАК СуммаВключаетНДС,
		|	ДокОснование.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
		|	ДокОснование.Курс КАК Курс,
		|	ДокОснование.Кратность КАК Кратность,
		|	НакладнаяЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|ИЗ
		|	Документ.РасходнаяНакладная.Запасы КАК НакладнаяЗапасы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДокОснование КАК ДокОснование
		|		ПО (&ДокументОснование = ДокОснование.Ссылка)
		|ГДЕ
		|	НакладнаяЗапасы.Ссылка = &ДокументСсылка
		|	И НакладнаяЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если ДокументОснованияНужногоТипа Тогда
			...
			ТаблицаЗапасы = ДокументОснование.Запасы;
			...
			СтруктураПоискаЦеныПоступления = Новый Структура("Номенклатура, Характеристика, Партия, ЗаказПокупателя");
			
			Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
				НоваяСтрока = Запасы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаИзРезультатаЗапроса);
				
				СпособРасчетаКомиссионногоВознаграждения = Договор.СпособРасчетаКомиссионногоВознаграждения;
				ПроцентКомиссионногоВознаграждения  = Договор.ПроцентКомиссионногоВознаграждения;
				
				СтруктураПоискаЦеныПоступления.Номенклатура = НоваяСтрока.Номенклатура;
				СтруктураПоискаЦеныПоступления.Характеристика = НоваяСтрока.Характеристика;
				СтруктураПоискаЦеныПоступления.Партия = НоваяСтрока.Партия;
				СтруктураПоискаЦеныПоступления.ЗаказПокупателя = НоваяСтрока.ЗаказПокупателя;
				
				НайденныеСтроки = ТаблицаЗапасы.НайтиСтроки(СтруктураПоискаЦеныПоступления);
				
				Если НайденныеСтроки.Количество() Тогда
					
					НоваяСтрока.ЦенаПоступления = НайденныеСтроки[0].Цена;
					...
					НоваяСтрока.СуммаНДСВознаграждения = ?(СуммаВключаетНДС, НоваяСтрока.СуммаВознаграждения
					- (НоваяСтрока.СуммаВознаграждения) / ((СтавкаНДС + 100) / 100),
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументСсылка, "Проект") Тогда
		Проект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,"Проект");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("СуммаВознаграждения") > 0 И (ЭтоНовый() Или ИспользоватьНовуюСхемуДвижений) Тогда
		Если Не ЗначениеЗаполнено(Договор.УслугаКомиссионногоВознаграждения) И Не УдержатьКомиссионноеВознаграждение Тогда
			ТекстСообщения = НСтр("ru = 'Необходимо заполнить услугу комиссионного вознаграждения в договоре документа.
			|Услуга находится в группе - Комиссия (или в группе - Агентское вознаграждение). Договор документа должен иметь вид - С комитентом'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,, Отказ);
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Договор.ДоговорУслугиКомиссионногоВознаграждения) Тогда
			...
			|Договор для услуги комиссии выбирается в группе - Комиссия (или в группе - Агентское вознаграждение). Договор документа должен иметь вид - С комитентом'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,, Отказ);
		КонецЕсли;
	КонецЕсли;
