...
Процедура ПриОткрытии(Отказ)
	...
	Если ФормаВладельца <> Неопределено И ТипЗнч(ФормаВладельца) = Тип("ФормаКлиентскогоПриложения")
...
Процедура СоставНабораНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЭтоНабор И ВыбранноеЗначение = НоваяНоменклатураНабора Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выбранная номенклатура совпадает с текущим набором.'"));
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ГруппаСмещение",				
		"Доступность", 
		Форма.РежимРаботы = РежимРаботыРедактированиеСостава() Или Форма.РежимРаботы = РежимРаботыИзФормыНоменклатуры());
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, 
