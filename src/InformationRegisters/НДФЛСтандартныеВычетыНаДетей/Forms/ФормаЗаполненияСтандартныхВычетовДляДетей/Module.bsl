...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Код = 108 По 125 Цикл
		...
		Исключение
			Продолжить;
		КонецПопытки;
		СписокКодовВычета.Добавить(ЭлементСправочника, СтрШаблон(НСтр("ru='Код %1, %2 руб.'"), Код, Формат(
			РазмерВычетов.Получить(ЭлементСправочника), "ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0")));
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Вычеты по НДФЛ на детей (Итого: %1) руб.'"), Формат(ТаблицаВычетовНаДетей.Итог(
		"СуммаВычета"), "ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0"));
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ БылиНажатыКнопкиЗакрытия Тогда
		...
		ВариантыОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Сохранить'"));
		ВариантыОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Нет'"));
		ВариантыОтветов.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена'"));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеПриЗакрытии", ЭтотОбъект);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаВычетовНаДетей
...
Процедура ТаблицаВычетовНаДетейПриАктивизацииСтроки(Элемент)
	...
	Если Текданные <> Неопределено Тогда
		...
		Иначе
			...
			Строка.Представление = ПредставлениеВычета(Строка.КодВычета, Строка.СуммаВычета);
	...
	Итог = Формат(ТаблицаВычетовНаДетей.Итог("СуммаВычета"), "ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0");
	Заголовок = СтрШаблон(НСтр("ru = 'Вычеты по НДФЛ на детей (Итого: %1) руб.'"), Итог);
	...
	Если Модифицированность Тогда
		...
		Если Не ВыполнитьПроверку(ОписаниеОшибок) Тогда
			Для Каждого Ошибка Из ОписаниеОшибок Цикл
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(ПолеИОписаниеОшибки.Описание, , ПолеИОписаниеОшибки.Поле);
			КонецЦикла;
			Возврат;
		КонецЕсли;
...
Процедура УдалитьЗаписьСРебенком(Команда)
	...
	Иначе
		...
		Если СпрашиватьОКорректировке Тогда
			ВариантыОтветов = Новый СписокЗначений;
			ВариантыОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Скорректировать'"));
			ВариантыОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Оставить как есть'"));
			
			оп = Новый ОписаниеОповещения("ОповещениеКорректировкиКодовВычетов", ЭтотОбъект);
			
			ПоказатьВопрос(оп, НСтр("ru='В таблице изменился порядок строк.
					|Скорректировать кода вычетов в строках согласно новому порядку?'"), ВариантыОтветов);
		КонецЕсли;
	КонецЕсли;
...
Функция ВыполнитьПроверку(ОписаниеОшибок)
	...
	ОписаниеОшибок = Новый Структура;
	НомерСтроки = 1;
	НеЗаполненыПоля = Новый Массив;
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		НеЗаполненыПоля.Очистить();
		Если Не ЗначениеЗаполнено(Строка.Период) Тогда
			НеЗаполненыПоля.Добавить(НСтр("ru ='Дата заявления'"));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Строка.ДатаРожденияРебенка) Тогда
			НеЗаполненыПоля.Добавить(НСтр("ru ='Дата рождения ребенка'"));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Строка.КодВычета) Тогда
			НеЗаполненыПоля.Добавить(НСтр("ru ='Код вычета'"));
		КонецЕсли;
		ДополнитьОписаниеОшибок(ОписаниеОшибок, НомерСтроки, НеЗаполненыПоля);
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
...
&НаСервере
Процедура ДополнитьОписаниеОшибок(ОписаниеОшибок, НомерСтроки, НеЗаполненыПоля)
	
	Если Не ЗначениеЗаполнено(НеЗаполненыПоля) Тогда
		Возврат;
	КонецЕсли;
	
	КлючОписания = СтрШаблон("Строка%1", Формат(НомерСтроки, "ЧГ=;"));
	ЗначениеОписания = Новый Структура;
	ЗначениеОписания.Вставить("Поле",
		СтрШаблон("ТаблицаВычетовНаДетей[%1].ДатаРожденияРебенка", Формат(НомерСтроки - 1, "ЧН=0; ЧГ=;")));
	ПредставлениеНеЗаполненыПоля = СтрСоединить(НеЗаполненыПоля,"; ");
	ЗначениеОписания.Вставить("Описание",
		СтрШаблон(НСтр("ru = 'В строке №%1 не заполнены следующие поля: %2'"), НомерСтроки, ПредставлениеНеЗаполненыПоля));
	
	ОписаниеОшибок.Вставить(КлючОписания, ЗначениеОписания);
...
Функция КодВычетаПоПереключателям()
	...
	// W - отказ от вычета
	КодПереключателей = 1000 * ТретийРебенок + 100 * ИмеетИнвалидность + 10 * ЕдинственныйРодитель
		+ ОтказОтВычетаВторогоРодителя;
...
Функция НаименованиеИКодВычетаПоНомеруСтроки(НомерСтроки, ТекущийКод = Неопределено)
	...
	КодВычета = КодВычетаПоНомеруСтроки(НомерСтроки, ТекущийКод);
	НаименованиеВычета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КодВычета, "Наименование");
	
	Результат = Новый Структура;
	Результат.Вставить("Код", КодВычета);
	Результат.Вставить("Наименование", НаименованиеВычета);

	Возврат Результат;

КонецФункции

// Код вычета по номеру строки
// 
// Параметры:
// 	НомерСтроки - Число - номер строки
// 	ТекущийКод - СправочникСсылка.ВычетыНДФЛ, Неопределено - ссылка на элемент справочника ВычетыНДФЛ
// Возвращаемое значение:
// 	СправочникСсылка.ВычетыНДФЛ - ссылка на элемент справочника ВычетыНДФЛ
&НаСервере
...
Процедура ОпределитьРазмерыВычетовНаДату(АктуальнаяДата)
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Вычеты по НДФЛ на детей (Итого: %1) руб.'"),
		Формат(ТаблицаВычетовНаДетей.Итог("СуммаВычета"),"ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0"));
...
Функция ПредставлениеВычета(КодВычета, СуммаВычета)
	
	КомпонентыПредставления = Новый Массив;
	Если ЗначениеЗаполнено(КодВычета) Тогда
		КомпонентыПредставления.Добавить(СтрШаблон(НСтр("ru='Код %1'"), СокрЛП(КодВычета)));
	КонецЕсли;

	Если СуммаВычета > 0 Тогда
		КомпонентыПредставления.Добавить(СтрШаблон(НСтр("ru = '%1 руб.'"), Формат(СуммаВычета,
			"ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0")));
	КонецЕсли;
