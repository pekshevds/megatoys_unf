...
Процедура ПриЗаписи(Отказ, Замещение)
    ...
    Для каждого ГоловнаяОрганизация Из Организации Цикл
		УчетнаяПолитика = РегистрыСведений.НастройкиУчетаСтраховыхВзносов.СоздатьНаборЗаписей();
		УчетнаяПолитика.Отбор.Организация.Установить(ГоловнаяОрганизация);
		УчетнаяПолитика.Прочитать();
		Если УчетнаяПолитика.Количество() = 0 Тогда
			Запись = УчетнаяПолитика.Добавить();
			Запись.Организация = ГоловнаяОрганизация;
		Иначе
			Запись = УчетнаяПолитика[0] 
		КонецЕсли;
		Запись.ИспользуетсяТрудИностранцевСОсобымиВзносами = Истина;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(УчетнаяПолитика);
	КонецЦикла;
