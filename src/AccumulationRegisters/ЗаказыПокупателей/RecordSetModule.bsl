...
Процедура РассчитатьГрафикВыполненияЗаказов()
	...
	|	ЗаказыПокупателейОстатки.Склад КАК Склад,
	...
	Пока ЕстьЗаписиВВыборке Цикл
		...
		// АссистентУправления: статус заказа заказа до записи движений.
		...
		// АссистентУправления: статус заказа заказа после записи движений.
...
Процедура СформироватьТаблицуИсходныхДвижений(Отказ, Замещение)
	...
	Иначе
		...
		|	ДвиженияЗаказыПокупателейИзменение.Склад КАК Склад,
...
Процедура СформироватьТаблицуИзмененийДвижений(Отказ, Замещение)
	...
	|		ДвиженияЗаказыПокупателейПередЗаписью.Склад КАК Склад,
	...
	|		ДвиженияЗаказыПокупателейПриЗаписи.Склад,
	...
	|	ДвиженияЗаказыПокупателейИзменение.Склад,
...
// Определяет статус отгрузки заказа по набору записей ГрафикВыполненияЗаказов.
//
// Параметры:
//  ГрафикВыполненияНаборЗаписей - РегистрСведенийНаборЗаписей
// 
// Возвращаемое значение:
//   - ПеречислениеСсылка.СтатусОтгрузки
//
Функция СтатусОтгрузкиЗаказа(ГрафикВыполненияНаборЗаписей)
	
	Если ГрафикВыполненияНаборЗаписей.Количество() = 0 Тогда
		Возврат Перечисления.СтатусОтгрузки.Полная;
	Иначе
		Если ГрафикВыполненияНаборЗаписей[0].Выполнено = 0 Тогда
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если ОбменДаннымиУНФ.РегистрироватьЗаказыПокупателейКОбменуСУРМК() Тогда
		
		МассивЗаказов = ДополнительныеСвойства.ТаблицаЗаказов.ВыгрузитьКолонку("ЗаказПокупателя");
		Для Каждого Документ Из МассивЗаказов Цикл
			Источник = Документ.ПолучитьОбъект();
			Источник.ДополнительныеСвойства.Вставить("ОтложеннаяЗапись");
			РежимЗаписи = Неопределено;
			РежимПроведения = Неопределено;
			ОбменДаннымиСобытия.МеханизмРегистрацииОбъектовПередЗаписьюДокумента("ОбменСУРМК", 
				Источник, Отказ, РежимЗаписи, РежимПроведения);
			
			ПланыОбмена.ЗарегистрироватьИзменения(Источник.ОбменДанными.Получатели, Документ);
		КонецЦикла;
...
Процедура АссистентУправленияПриСрабатыванииСобытия(ЗаказПокупателя, Источник, СостояниеОтгрузкиДо, СостояниеОтгрузкиПосле)
	
	Событие = Неопределено;
	
	Если СостояниеОтгрузкиДо = СостояниеОтгрузкиПосле Тогда
		Возврат;
	КонецЕсли;
	
	Если СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Полная
		И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Отсутствует
		И ТипЗнч(Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Возврат;
	КонецЕсли;
	
	Если СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Отсутствует И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Частичная Тогда
		Событие = "ЧастичнаяОтгрузкаПоступила";
	ИначеЕсли (СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Отсутствует И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Полная) Тогда
		Событие = "ПолнаяОтгрузкаПоступила";
	ИначеЕсли (СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Частичная И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Отсутствует) Тогда
		Событие = "ЧастичнаяОтгрузкаОтмена";
	ИначеЕсли СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Частичная И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Полная Тогда
		Событие = "ПолнаяОтгрузкаПоступила";
	ИначеЕсли (СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Полная И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Отсутствует) Тогда
		Событие = "ПолнаяОтгрузкаОтмена";
	ИначеЕсли (СостояниеОтгрузкиДо = Перечисления.СтатусОтгрузки.Полная И СостояниеОтгрузкиПосле = Перечисления.СтатусОтгрузки.Частичная) Тогда
		Событие = "ПолнаяОтгрузкаОтмена";
	КонецЕсли;
	
	Если Событие <> Неопределено Тогда
		АссистентУправления.ПриСрабатыванииСобытия(ЗаказПокупателя, Событие, Источник);
	КонецЕсли;
