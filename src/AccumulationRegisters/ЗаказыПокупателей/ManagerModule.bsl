...
Процедура ДобавитьДвиженияСУчетомСкладов(Регистратор, ТаблицаДвижений) Экспорт
	...
	// Итерация 1: Строки с пустым складом или отрицательным количеством переносятся как есть
	Для каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.Склад) И КоличествоИзТаблицы(ТаблицаДвижений, СтрокаТаблицы) > 0 Тогда
			Продолжить;
		КонецЕсли; 
	...
	Для каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Склад) Тогда
			Продолжить;
		КонецЕсли; 
		Если КоличествоИзТаблицы(ТаблицаДвижений, СтрокаТаблицы) <= 0 Тогда
			Продолжить;
		КонецЕсли; 
...
Функция КоличествоИзТаблицы(ТаблицаДвижений, СтрокаТаблицы)
		
	Если ТаблицаДвижений.Колонки.Найти("ВидДвижения") = Неопределено Тогда
		КоличествоЗаписи = СтрокаТаблицы.Количество;
	ИначеЕсли СтрокаТаблицы.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
		КоличествоЗаписи = -СтрокаТаблицы.Количество;
	Иначе
		КоличествоЗаписи = СтрокаТаблицы.Количество;
	КонецЕсли;
	Возврат КоличествоЗаписи;
