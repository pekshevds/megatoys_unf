...
Процедура РассчитатьПланируемыеПлатежи(ТаблицаСчетов)
	...
	Пока ОбходПоСчетамНаОплату.Следующий() Цикл
		...
		НаборЗаписей.Отбор.СчетНаОплату.Установить(ОбходПоСчетамНаОплату.СчетНаОплату);
		
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			
			НаборЗаписей.Отбор.СчетНаОплату.Установить(ВыборкаДетальныеЗаписи.СчетНаОплату);
			
			// В некоторых ситуациях в наборе может оказаться две записи с одинаковым ключом
			// Необходимо оставить только последнюю
			Если НаборЗаписей.Количество() > 0 Тогда
				
				ОтборПоКлючу = Новый Структура;
				ОтборПоКлючу.Вставить("СчетНаОплату", ВыборкаДетальныеЗаписи.СчетНаОплату);
				ОтборПоКлючу.Вставить("Период", ВыборкаДетальныеЗаписи.Дата);
				 
				ЗаписиНабора = НаборЗаписей.Выгрузить();
				СтрокиПоКлючу = ЗаписиНабора.НайтиСтроки(ОтборПоКлючу);
				
				Для каждого СтрокаНабора Из СтрокиПоКлючу Цикл
					ЗаписиНабора.Удалить(СтрокаНабора);
				КонецЦикла;
