...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Иначе
		...
		// Текущее состояние набора и текущее состояние изменений помещаются во временную таблицу "ДвиженияЗаказыНаПроизводствоПередЗаписью",
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	|		ДвиженияЗаказыНаПроизводствоПередЗаписью.Склад КАК Склад,
	...
	|		ДвиженияЗаказыНаПроизводствоПриЗаписи.Склад,
	...
	|	ДвиженияЗаказыНаПроизводствоИзменение.Склад,
	...
	ИзмененныеЗаказы = ИзмененныеЗаказы(СтруктураВременныеТаблицы);
	Для каждого Заказ Из ИзмененныеЗаказы Цикл
		АссистентУправления.ПриСрабатыванииСобытия(Заказ, "ИзменениеСостоянияВыполнения", Отбор.Регистратор.Значение);
	КонецЦикла;
...
Функция ИзмененныеЗаказы(СтруктураВременныеТаблицы)
	
	ИзмененныеЗаказы = Новый Массив;
	
	Если НЕ СтруктураВременныеТаблицы.ДвиженияЗаказыНаПроизводствоИзменение Тогда
		Возврат ИзмененныеЗаказы;
	КонецЕсли;
	
	ТаблицаИзменение = СтруктураВременныеТаблицы.МенеджерВременныхТаблиц.Таблицы.Найти("ДвиженияЗаказыНаПроизводствоИзменение");
	Если ТаблицаИзменение = Неопределено Тогда
		Возврат ИзмененныеЗаказы;
	КонецЕсли;
	
	ТаблицаИзменение = ТаблицаИзменение.ПолучитьДанные().Выгрузить();
	ИзмененныеЗаказы = ТаблицаИзменение.ВыгрузитьКолонку("ЗаказНаПроизводство");
	ИзмененныеЗаказы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИзмененныеЗаказы);
