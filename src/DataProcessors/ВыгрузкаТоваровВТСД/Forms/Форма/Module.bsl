...
Процедура ВыгрузитьВыполнить(Команда)
	...
	Если НЕ ВыполнитьПроверкуЗаполнения() Тогда
		Возврат;
	КонецЕсли;
...
Функция ВыполнитьПроверкуЗаполнения() 
	...
	Результат = Истина;
	Если ВариантЗаполнения = "ПоДокументу" Тогда 
		Возврат Результат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.ВидЦены) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""По виду цен""'"), , "Объект.ВидЦены");
		Результат = Ложь;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.ЦеныНаДату) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""На дату""'"), , "Объект.ЦеныНаДату");
		Результат = Ложь;
	КонецЕсли;
