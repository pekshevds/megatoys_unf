...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВводДаты(Оповещение, Объект.ПериодЦен, НСтр("ru = 'На дату'"), ЧастиДаты.ДатаВремя);
...
Процедура ИндивидуальныйПриИзменении(Элемент)
	...
	КэшЗначений.ЗапретитьОбновлениеПрайсЛистаПриЗакрытии = НЕ Объект.УдалитьИндивидуальный;
	
	Объект.Автор = ?(Объект.УдалитьИндивидуальный, КэшЗначений.АвторизованныйПользователь, Неопределено);
	Если НЕ Объект.УдалитьИндивидуальный Тогда
...
Процедура Декорация1Нажатие(Элемент)
	...
	ОткрытьФормуСоставРеквизитов("ГруппаВидыЦен");
...
Процедура Декорация3Нажатие(Элемент)
	...
	ПараметрыОткрытия.Вставить("Индивидуальный",				Объект.УдалитьИндивидуальный);
	...
	ОткрытьФорму("Справочник.ПрайсЛисты.Форма.УдалитьФормаОДокументе", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещения);
...
Процедура ПредставлениеПериодаТекстом()
	...
	Иначе
		...
		ПредставлениеПериода = НСтр("ru = 'актуальная дата'");
...
Процедура УстановитьСвойстваЭлементовПоВидуЦен()
	...
	ИначеЕсли КоличествоСтрок = 1 Тогда
		...
		ТекстЗаголовка			= СокрЛП(ВыбранныеВидыЦен[0].ВидЦен);

	ИначеЕсли КоличествоСтрок = 2 Тогда

		ТекстЗаголовка			= СокрЛП(ВыбранныеВидыЦен[0].ВидЦен) + ", " + СокрЛП(ВыбранныеВидыЦен[1].ВидЦен);

	Иначе

		ТекстЗаголовка	= СокрЛП(ВыбранныеВидыЦен[0].ВидЦен) + ", " + СокрЛП(ВыбранныеВидыЦен[1].ВидЦен) + Символы.НПП
			+ СтрШаблон(НСтр("ru ='(еще %1)'"), КоличествоСтрок - 2);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаКолонки", "Заголовок",
...
Процедура ЗаполнитьПредставлениеНоменклатуры()
	...
	СоставРеквизитов = ЦенообразованиеКлиент.ПредставлениеНоменклатурыСтрокой(Объект.ПредставлениеНоменклатуры,
		ПараметрыКартинки);
...
Процедура ОткрытьФормуСоставРеквизитов(АктивнаяЗакладка)
	...
	ИмяФормыРедактированияПредставления = "Справочник.ПрайсЛисты.Форма.УдалитьФормаСоставРеквизитов";
...
Функция ПараметрыОткрытияФормыСоставаПолей(АктивнаяЗакладка, ПараметрыОткрытияФормы)
	...
	ПараметрыОткрытияФормы.Вставить("НоваяФормаАРМ", Ложь);
...
Процедура ОформлениеПрайсЛистаПолотно()
	...
	Для НомерСтроки = НомерПервойСтроки По КэшЗначений.ОформлениеГраницаПоследнейПорции Цикл
		...
		Для каждого ОписаниеКолонки Из КэшЗначений.ИменаКолонок Цикл
			...
			ИмяКолонки = ОписаниеКолонки.Значение;
