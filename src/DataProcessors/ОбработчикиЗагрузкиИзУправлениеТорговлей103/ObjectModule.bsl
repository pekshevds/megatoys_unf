...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	ОбщиеСвойства = НаборСвойств_Справочник_Контрагенты.ПолучитьОбъект();
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Выборка.Свойство.НаборСвойств) Тогда
			СвойствоОбъект = Выборка.Свойство.ПолучитьОбъект();
			СвойствоОбъект.НаборСвойств = НаборСвойств_Справочник_Контрагенты;
	...
	ОбщиеСвойства = НаборСвойств_Справочник_ФизическиеЛица.ПолучитьОбъект();
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.Свойство.НаборСвойств) Тогда
			СвойствоОбъект = Выборка.Свойство.ПолучитьОбъект();
			СвойствоОбъект.НаборСвойств = НаборСвойств_Справочник_ФизическиеЛица;
			СвойствоОбъект.ОбменДанными.Загрузка = Истина;
			СвойствоОбъект.Записать();
		КонецЕсли;
	КонецЦикла;
	...
	ОбщиеСвойства = НаборСвойств_Справочник_ДоговорыКонтрагентов.ПолучитьОбъект();
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.Свойство.НаборСвойств) Тогда
			СвойствоОбъект = Выборка.Свойство.ПолучитьОбъект();
			СвойствоОбъект.НаборСвойств = НаборСвойств_Справочник_ДоговорыКонтрагентов;
			СвойствоОбъект.ОбменДанными.Загрузка = Истина;
			СвойствоОбъект.Записать();
		КонецЕсли;
	КонецЦикла;
	...
	ОбщиеСвойства = НаборСвойств.ПолучитьОбъект();
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Пока ВыборкаПоНоменклатуре.Следующий() Цикл
		...
		|НаправлениеДеятельности, КатегорияНоменклатуры, Склад, СпособПополнения,
		|ВидСтавкиНДС, СчетУчетаЗапасов, СчетУчетаЗатрат, Ячейка, ЦеноваяГруппа, ИспользоватьХарактеристики,
	...
	// Запись в регистр иерархии групп номенклатуры
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	// Проведение документов ввода начальных остатков
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			КДОбъект.ЗарегистрироватьПредупреждение(НСтр("ru = 'Не удалось провести документ ввода начальных остатков: '")+Строка(ДокументОбъект));
		КонецПопытки;
...
Процедура ПКО_ПослеЗагрузки_КонстантыНабор(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Попытка
		...
		МассивКонстант.Добавить(Метаданные.Константы.ИспользоватьДополнительныеРеквизитыИСведения);
		...
		Для каждого Эл Из МассивКонстант Цикл
...
Процедура ПКО_ПослеЗагрузки_ФайлыНоменклатуры(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ПослеЗагрузкиПрисоединенногоФайла(Объект);
...
Процедура ПКО_ПослеЗагрузки_Организации(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если Выборка.Следующий() Тогда
		Объект.ВидСтавкиНДСПоУмолчанию = Выборка.ВидСтавкиНДС;
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_Номенклатура(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если НЕ Объект.ЭтоГруппа Тогда
		...
		КонецЕсли;
		ЗаполнитьСтавкуНДСШапки(ПараметрыОбъекта, Объект);
...
Процедура ПКО_ПослеЗагрузки_ВидыЦен(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт

	Если Не ЗначениеЗаполнено(Объект.ТипВидаЦен) Тогда
		Объект.ТипВидаЦен = Перечисления.ТипыВидовЦен.Статический;
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_ВводОстатковСобственныеТоварыНаСкладах(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ЗаполнитьАвтора(Объект);
	
	// Проведение документа.
	УстановитьРежимПроведения(Объект);
...
Процедура ПКО_ПослеЗагрузки_ЗаказПокупателя(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого ТекСтрока Из Объект.Запасы Цикл
		...
		ЗаполнитьЕдиницуИзмерения(ТекСтрока);
		
		// Заполним ставку НДС.
		ЗаполнитьСтавкуНДСПоСтроке(ПараметрыОбъекта, ПараметрыТЧ, ТекСтрока);
...
Процедура ЗаполнитьАвтора(Объект) Экспорт
...
Процедура ЗаполнитьЕдиницуИзмерения(ТекСтрока) Экспорт
...
Процедура ЗаполнитьПартию(ТекСтрока) Экспорт

	
	КДОбъект = Обработки.КонвертацияОбъектовИнформационныхБаз.Создать();
	
	// Определим статус партии от вида операции.
	Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии;
	Наименование = "Товары на комиссии";
	
	// Поиск партии по номенклатуре и контрагенту.
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ВладелецПартии", ТекСтрока.Контрагент);
	...
	Иначе
		...
		Иначе
			...
			// Если ничего не найдено то создаем новый.
			НовыйЭлемент = Справочники.ПартииНоменклатуры.СоздатьЭлемент();
			НовыйЭлемент.Наименование = Наименование;
			НовыйЭлемент.Владелец = ТекСтрока.Номенклатура;
			НовыйЭлемент.Статус = Статус;
			НовыйЭлемент.УстановитьНовыйКод();
			КДОбъект.ЗаписатьОбъектВИБ(НовыйЭлемент, ТипЗнч(НовыйЭлемент));
			
			ТекСтрока.Партия = НовыйЭлемент.Ссылка;
...
Процедура ЗаполнитьСтавкуНДСПоСтроке(ПараметрыОбъекта, ПараметрыТЧ, ТекСтрока) Экспорт

	Если ПараметрыОбъекта <> Неопределено
	   И ПараметрыТЧ.Количество() > 0 Тогда
		
		Запрос = Новый Запрос();
		Запрос.Текст = ПараметрыТЧ[ТекСтрока.НомерСтроки - 1].ТекстЗапросаСтавкаНДС;
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда
			ТекСтрока.СтавкаНДС = Выборка.СтавкаНДС;
		КонецЕсли;
...
Процедура ЗаполнитьСтавкуНДСШапки(ПараметрыОбъекта, Объект) Экспорт

	Если ПараметрыОбъекта <> Неопределено Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = ПараметрыОбъекта["ТекстЗапросаСтавкаНДС"];
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда
			
			лСтавкаНДС = Выборка.СтавкаНДС; 
			Если лСтавкаНДС.Ставка = 20 ИЛИ лСтавкаНДС.Ставка = 18 Тогда
				Если лСтавкаНДС.Расчетная Тогда
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.ОбщаяРасчетная;
				Иначе
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.Общая;
				КонецЕсли;
			ИначеЕсли лСтавкаНДС.Ставка = 10 Тогда
				Если лСтавкаНДС.Расчетная Тогда
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.ПониженнаяРасчетная;
				Иначе
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.Пониженная;
				КонецЕсли;
			ИначеЕсли лСтавкаНДС.Ставка = 0 Тогда
				Если лСтавкаНДС.НеОблагается Тогда
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.БезНДС;
				Иначе
					лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.Нулевая;
				КонецЕсли;
			Иначе
				лВидСтавкиНДС = Перечисления.ВидыСтавокНДС.БезНДС;
			КонецЕсли;		
			
			Объект.ВидСтавкиНДС = лВидСтавкиНДС;
		КонецЕсли;
...
Процедура ПослеЗагрузкиПрисоединенногоФайла(Объект) Экспорт

	Объект.Автор						= Пользователи.ТекущийПользователь();
	Объект.ДатаСоздания					= ТекущаяДата();
	Объект.ДатаМодификацииУниверсальная	= УниверсальноеВремя(ТекущаяДата());
	Объект.Изменил						= Пользователи.ТекущийПользователь();
...
Процедура УстановитьРежимПроведения(Объект) Экспорт
