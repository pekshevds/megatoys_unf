...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВыделитьЭлемент") Тогда
		...
		Элементы[Параметры.ВыделитьЭлемент].ЦветРамки = WebЦвета.ЖелтоЗеленый;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Оповестить("УстановитьГрафик", Ресурс, ИмяФормы);
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(НомерТекущегоГода) Тогда
		НомерТекущегоГода = Год(ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
...
Процедура РасписаниеПоГрафикуВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Список.Добавить("Очистить", , Ложь, БиблиотекаКартинок.ПолеВводаОчистить);
	Список.Добавить("Удалить отклонения", , Ложь);
	Список.Добавить("Удалить все отклонения", , Ложь);
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораИзМеню", ЭтотОбъект, ПараметрыОповещения);
...
Процедура ЗаписатьДанныеВРегистр(ГодЗаписи, Отказ = Ложь)
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'При записи графика произошла ошибка.
										|Дополнительное описание: %1'"), ПодробноеПредставлениеОшибки);
