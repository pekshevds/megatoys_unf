...
Процедура ВыгрузитьДанныеИнформационнойБазы(Контейнер, Обработчики, Сериализатор) Экспорт
	...
	Иначе 
		...
		Если КоличествоИзмененных >= 100 Тогда
			Если Контейнер.ФиксироватьСостояние() Тогда
				Контейнер.ДополнитьОбщееКоличествоОбъектов(КоличествоИзмененных);
			КонецЕсли;
			ВыгрузитьДанные(
				ИсключаемыеТипы,
				Контейнер,
...
Функция ЗавершитьВыгрузкуВТранзакции(ИсключаемыеТипы, Контейнер, ДополнительныйУзел, Обработчики,
	...
	Если ОбщегоНазначения.РазделениеВключено() И Метаданные.ОбщиеМодули.Найти("ОповещениеПользователейБТС") <> Неопределено Тогда
		...
		Если ФоновоеЗадание <> Неопределено Тогда
			...
			Для Каждого Сеанс Из ПолучитьСеансыИнформационнойБазы() Цикл
				...
				Оповещение.ОбластьДанных = ЗначениеРазделителяСеанса;
