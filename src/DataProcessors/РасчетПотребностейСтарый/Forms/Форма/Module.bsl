...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступноПроизводство = ПравоДоступа("Просмотр", Метаданные.Подсистемы.Производство)
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.РасчетПотребностейСтарый.Форма.ФормаНастройка", ПараметрыФормы,,,,, Оповещение);
...
Процедура ОбновитьСписокВыбораСпособПополнения()
	...
	Если ДоступныЗакупки Тогда
		Если ПолучитьФункциональнуюОпцию("ПередачаСырьяВПереработку") И ПравоДоступа("ИнтерактивноеПроведение", Метаданные.Документы.ОтчетПереработчика) Тогда
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Закупка и переработка", НСтр("ru = 'Закупка и переработка'"));
		Иначе
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Закупка", НСтр("ru = 'Закупка'"));
		КонецЕсли;
	КонецЕсли;
	
	Если ДоступноПроизводство Тогда
		Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Производство", НСтр("ru = 'Производство'"));
		Если ДоступныЗакупки Тогда
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Все", НСтр("ru = 'Все'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьДанные(РезультатЗапроса, ДатаНачала, ДатаОкончания)
	...
	ЗаполнитьТаблицуОстаткиПотребности();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТаблицуОстаткиПотребности()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОстаткиПотребности", ОстаткиПотребности.Выгрузить());
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОстаткиПотребности.Номенклатура КАК Номенклатура,
	|	ОстаткиПотребности.Характеристика КАК Характеристика,
	|	ОстаткиПотребности.ЗаказПокупателя КАК ЗаказПокупателя
	|ПОМЕСТИТЬ ОстаткиПотребности
	|ИЗ
	|	&ОстаткиПотребности КАК ОстаткиПотребности
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					(ВЫБРАТЬ
	|						ОстаткиПотребности.ЗаказПокупателя,
	|						ОстаткиПотребности.Номенклатура,
	|						ОстаткиПотребности.Характеристика
	|					ИЗ
	|						ОстаткиПотребности)) КАК ЗаказыПокупателейОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				,
	|				ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка)
	|					И (ЗаказПокупателя, Номенклатура, Характеристика) В
	|						(ВЫБРАТЬ
	|							ОстаткиПотребности.ЗаказПокупателя,
	|							ОстаткиПотребности.Номенклатура,
	|							ОстаткиПотребности.Характеристика
	|						ИЗ
	|							ОстаткиПотребности)) КАК ПотребностьВЗапасахОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|						ОстаткиПотребности.Характеристика
	|					ИЗ
	|						ОстаткиПотребности)) КАК РазмещениеЗаказовОстатки) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	ОстаткиПотребности.Загрузить(Запрос.Выполнить().Выгрузить());
		...
		Для каждого СтрокаРасшифровка Из СтруктураРасшифровка.Расшифровка Цикл
			Для каждого СтрокаЗаказа Из СтрокаРасшифровка Цикл
				...
				Если (СтрокаЗаказа.Значение.Потребность <> 0 ИЛИ СтрокаЗаказа.Значение.ПотребностьПросрочено <> 0) И МассивЗаказовПотребность.Найти(СтрокаЗаказа.Ключ) = Неопределено Тогда
					...
					Если ФОРезервированиеЗапасов Тогда
						НоваяСтрокаРазмещения = ОстаткиПотребности.Добавить();
						НоваяСтрокаРазмещения.Номенклатура = СтруктураДетализация.Номенклатура;
						НоваяСтрокаРазмещения.Характеристика = СтруктураДетализация.Характеристика;
						НоваяСтрокаРазмещения.ЗаказПокупателя = СтрокаЗаказа.Ключ;
					КонецЕсли; 
...
Процедура ОбновитьРекомендацииНаСервере()
	...
	ИсточникПотребность = Новый Соответствие;
		...
		Если Номенклатура.Дефицит > 0 Тогда
			...
			Пока НачалоДня(ТекущийПериод) <= НачалоДня(ОкончаниеПериода) Цикл
				...
				Если Номенклатура[ИмяКолонки] > 0 ИЛИ Номенклатура.Просрочено > 0 И ТекущийПериод = НачалоДня(ТекущаяДатаСеанса()) Тогда
					...
					Если ФОРезервированиеЗапасов И Номенклатура.Строки.Количество() > 2 Тогда
						СтрокиПотребность = Номенклатура.Строки[2];
						Для каждого СтрокаДетали Из СтрокиПотребность.Строки Цикл
							Если ТипЗнч(СтрокаДетали.Номенклатура) <> Тип("ДокументСсылка.ЗаказПокупателя") 
								ИЛИ НЕ ЗначениеЗаполнено(СтрокаДетали.Номенклатура) Тогда
								Продолжить;
							КонецЕсли;
							Дефицит = ?(ТекущийПериод = НачалоДня(ТекущаяДатаСеанса()), СтрокаДетали.Просрочено, СтрокаДетали[ИмяКолонки]);
							Если Дефицит = 0 Тогда
								Продолжить;
							КонецЕсли; 
							Потребность.Вставить(СтрокаДетали.Номенклатура, Дефицит);
						КонецЦикла; 
					КонецЕсли; 
					...
					ИсточникПотребность.Вставить(ИндексСтроки, Потребность);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекущийИндексСтроки <> Выборка.ИндексСтроки Тогда
			...
			ТекущаяПотребность = ИсточникПотребность.Получить(ТекущийИндексСтроки);
			Если ТипЗнч(ТекущаяПотребность)=Тип("Соответствие") И ФОРезервированиеЗапасов Тогда
				НоваяНоменклатура.Заказы = Новый ФиксированноеСоответствие(ТекущаяПотребность);
			Иначе
				НоваяНоменклатура.Заказы = Новый ФиксированноеСоответствие(Новый Соответствие);
			КонецЕсли;                     
...
Процедура СформироватьЗаказыНаСервере()
	...
	ТаблицаЗаказы.Колонки.Добавить("ЗаказыПокупателей");
	...
	Для каждого СтрокаРекомендации Из РекомендацииНоменклатура Цикл
		...
		Для каждого СтрокаНоменклатура Из РекомендацииЭлементы Цикл
			...
			Если СтрокаНоменклатура.Выбран Тогда
				...
				НоваяСтрока.ЗаказыПокупателей = СтрокаРекомендации.Заказы;
	...
	ТаблицаПотребности = ОстаткиПотребности.Выгрузить();
	ТаблицаПотребности.Индексы.Добавить("Номенклатура, Характеристика, ЗаказПокупателя");
	...
	Для каждого ПараметрыЗаказа Из ТаблицаЗаказы Цикл
		...
		Если ПараметрыЗаказа.СпособПополнения = Перечисления.СпособыПополненияЗапасов.Закупка
			...
			Если ДатаПоступленияВШапке Тогда
			...
			ДоговорПоУмолчанию = Справочники.ДоговорыКонтрагентов.ДоговорПоУмолчанию(ТекущийЗаказ.Контрагент);
			...
			Для каждого СтрокаРезультат Из РезультатПоиска Цикл
				...
				Для каждого ДанныеЗаказПокупателя Из СтрокаРезультат.ЗаказыПокупателей Цикл
					КоличествоЗаказ = Мин(ДанныеЗаказПокупателя.Значение, НоваяСтрока.Количество);
					Если КоличествоЗаказ<=0 Тогда
						Продолжить;
					КонецЕсли; 
					СтруктураПоискаПотребность = Новый Структура;
					СтруктураПоискаПотребность.Вставить("Номенклатура", НоваяСтрока.Номенклатура);
					СтруктураПоискаПотребность.Вставить("Характеристика", НоваяСтрока.Характеристика);
					СтруктураПоискаПотребность.Вставить("ЗаказПокупателя", ДанныеЗаказПокупателя.Ключ);
					СтрокиПотребность = ТаблицаПотребности.НайтиСтроки(СтруктураПоискаПотребность);
					Если СтрокиПотребность.Количество()=0 Тогда
						Продолжить;
					КонецЕсли;
					СтрокаПотребность = СтрокиПотребность[0];
					ПотребностьЗаказ = СтрокаПотребность.Остаток;
					КоличествоЗаказ = Мин(ПотребностьЗаказ, КоличествоЗаказ);
					Если КоличествоЗаказ<=0 Тогда
						Продолжить;
					КонецЕсли; 
					Если НоваяСтрока.Количество=КоличествоЗаказ Тогда
						НоваяСтрока.ЗаказПокупателя = ДанныеЗаказПокупателя.Ключ;
					Иначе
						СтрокаЗаказ = ТекущийЗаказ.Запасы.Вставить(ТекущийЗаказ.Запасы.Индекс(НоваяСтрока));
						ЗаполнитьЗначенияСвойств(СтрокаЗаказ, НоваяСтрока);
						СтрокаЗаказ.Количество = КоличествоЗаказ;
						СтрокаЗаказ.ЗаказПокупателя = ДанныеЗаказПокупателя.Ключ;
						НоваяСтрока.Количество = НоваяСтрока.Количество-КоличествоЗаказ;
						СтрокиРасчета.Добавить(СтрокаЗаказ);
					КонецЕсли;
					СтрокаПотребность.Остаток = СтрокаПотребность.Остаток-КоличествоЗаказ;
					Если ТекущийЗаказ.ПоложениеЗаказаПокупателя<>Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
				...
				Если ЗначениеЗаполнено(ПараметрыЗаказа.ВидЦен) Тогда
					...
					Для каждого СтрокаРасчета Из СтрокиРасчета Цикл
						...
						СтрокаРасчета.Цена = СтрокаРезультат.Цена;
						
						Если НЕ СтрокаРезультат.ВидЦен.ЦенаВключаетНДС Тогда
							СтрокаРасчета.Цена = (СтрокаРасчета.Цена * (100 + СтавкаНДС)) / 100;
						КонецЕсли;
						
						СтрокаРасчета.Сумма = СтрокаРасчета.Цена * СтрокаРасчета.Количество;
						СтрокаРасчета.СуммаНДС = СтрокаРасчета.Сумма - (СтрокаРасчета.Сумма) / ((СтавкаНДС + 100) / 100);
						СтрокаРасчета.Всего = СтрокаРасчета.Сумма;
			...
			Для каждого СтрокаРезультат Из РезультатПоиска Цикл
				...
				Для каждого ДанныеЗаказПокупателя Из СтрокаРезультат.ЗаказыПокупателей Цикл
					...
					Если НоваяСтрока.Количество=КоличествоЗаказ Тогда
						НоваяСтрока.ЗаказПокупателя = ДанныеЗаказПокупателя.Ключ;
					Иначе
						СтрокаЗаказ = ТекущийЗаказ.Продукция.Вставить(ТекущийЗаказ.Продукция.Индекс(НоваяСтрока));
						ЗаполнитьЗначенияСвойств(СтрокаЗаказ, НоваяСтрока);
						СтрокаЗаказ.Количество = КоличествоЗаказ;
						СтрокаЗаказ.ЗаказПокупателя = ДанныеЗаказПокупателя.Ключ;
						НоваяСтрока.Количество = НоваяСтрока.Количество-КоличествоЗаказ;
...
Процедура СформироватьПериодПотребности()
	...
	Если Месяц(ДатаКалендаряНачало) = Месяц(ДатаКалендаряОкончание) Тогда
		...
		ДеньНеделиРасписанияНачало = ОбщегоНазначенияУНФКлиент.ПредставлениеДняНедели(ДатаКалендаряНачало);
		ДеньРасписанияОкончание = Формат(ДатаКалендаряОкончание, "ДФ=дд");
		ДеньНеделиРасписанияОкончание = ОбщегоНазначенияУНФКлиент.ПредставлениеДняНедели(ДатаКалендаряНачало);
	...
	Иначе
		...
		ДеньРасписанияОкончание = Формат(ДатаКалендаряОкончание, "ДФ=дд");
		ДеньНеделиРасписанияОкончание = ОбщегоНазначенияУНФКлиент.ПредставлениеДняНедели(ДатаКалендаряОкончание);
