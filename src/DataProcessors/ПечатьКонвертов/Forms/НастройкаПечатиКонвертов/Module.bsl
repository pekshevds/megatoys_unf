...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ОбъектыПечати) Тогда
		ВызватьИсключение НСтр("ru ='Непосредственное открытие этой формы не предусмотрено.'");
	КонецЕсли;
	
	УдалитьГруппыИзОбъектовПечати();
	
	Если Не ЗначениеЗаполнено(Параметры.ОбъектыПечати) Тогда
		ВызватьИсключение НСтр("ru ='Отсутствуют данные для печати конвертов.'");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Организация", "ОтображениеПодсказки",
			ОтображениеПодсказки.Нет);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияМобильныйКлиент", "Видимость",
			Истина);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если КэшЗначений.ВыполняетсяКомандаПродолжить = Ложь Тогда
		...
		СохраняемыеВНастройкахДанныеМодифицированы = Ложь;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если РазрешитьАдресаВПроизвольнойФорме = Истина Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаАдресВПроизвольнойФорме",
			"ОтображениеПодсказки", ОтображениеПодсказки.ОтображатьСнизу);
...
&НаКлиенте
Процедура ГруппаАдресВПроизвольнойФормеРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент,
...
&НаКлиенте
Процедура ПечататьЛоготипРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент,
	НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура КомандаПродолжить(Команда)
	...
	ОбъектыПечати = ОбъектыПечатиИзВременногоХранилища();
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Формируется конверт...'"));
	
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьКонвертов", "Конверт", ОбъектыПечати, ,
		ПараметрыПечати);
...
Процедура НастройкаДлиныСтрок(Команда)
	...
	ОткрытьФорму("Обработка.ПечатьКонвертов.Форма.НастройкаДлиныСтрок", ПараметрыОткрытия, ЭтотОбъект, , , ,
		ОписаниеОповещения);
...
Процедура СообщитьПользователюОсобенностиПечатиВВебБраузере(Форма)
	...
	Если Форма.НаименованиеБраузера = "CHROME" Тогда
		ТекстСообщения = НСтр("ru='На следующем шаге, в окне предпросмотра, выберите размер бумаги'");
	ИначеЕсли Форма.НаименованиеБраузера = "MSIE" Тогда
		ТекстСообщения = НСтр("ru='На следующем шаге выберите в Параметрах страницы размер бумаги'");
	Иначе
		ТекстСообщения = НСтр("ru='На следующем шаге выберите в Свойствах принтера размер бумаги'");
	КонецЕсли;
	
	РазмерБумаги = РазмерБумаги(Форма);
	
	Форма.Элементы.ТекстОсобенностиПечатиВВебКлиенте.Заголовок = СтрШаблон("%1 %2", ТекстСообщения, РазмерБумаги);
...
Процедура ВидимостьПодсказкиАдресаВПроизвольнойФорме()
	...
	СпособОтображениеПодсказки = ?(РазрешитьАдресаВПроизвольнойФорме, ОтображениеПодсказки.ОтображатьСнизу,
		ОтображениеПодсказки.Нет);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаАдресВПроизвольнойФорме",
		"ОтображениеПодсказки", СпособОтображениеПодсказки);
...
&НаСервере
Процедура УдалитьГруппыИзОбъектовПечати()
	
	Если ТипЗнч(Параметры.ОбъектыПечати[0]) <> Тип("СправочникСсылка.Контрагенты") Тогда
		Возврат;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОбъектыПечати", Параметры.ОбъектыПечати);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Контрагенты.Ссылка
	|ИЗ
	...
	Параметры.ОбъектыПечати.Очистить();
	Если РезультатЗапроса.Пустой() Тогда
		Возврат;
	КонецЕсли;
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		Параметры.ОбъектыПечати.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция ОбъектыПечатиИзВременногоХранилища()
...
Процедура ЗаполнитьЗначенияПоУмолчанию()
	...
	КэшЗначений.Вставить("ФОИспользоватьНесколькоОрганизаций", ПолучитьФункциональнуюОпцию(
	...
	ЗаполнитьРеквизитОрганизация();
	
	РазрешитьАдресаВПроизвольнойФорме	= Ложь;
	ДополнительныеСтрокиВЗаголовках		= Ложь;
	ПечататьЛоготип						= Ложь;
	
	ЗаполнитьТиповыеДлиныСтрокКонвертов();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьРеквизитОрганизация()
	...
	Если КэшЗначений.ФОИспользоватьНесколькоОрганизаций Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СпрОрганизаций.Ссылка КАК Ссылка,
		|	Представление(СпрОрганизаций.Ссылка) КАК Представление
		|ИЗ
		|	Справочник.Организации КАК СпрОрганизаций
		|ГДЕ
		|	НЕ СпрОрганизаций.ПометкаУдаления";
	...
	КэшЗначений.ВОбъектеПечатиЕстьОрганизация = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		Параметры.ОбъектыПечати[0], "Организация");
	Если КэшЗначений.ВОбъектеПечатиЕстьОрганизация Тогда
		
		Элементы.Организация.СписокВыбора.Добавить(Справочники.Организации.ПустаяСсылка(), НСтр(
			"ru = '<Указана в документе>'"));
...
&НаСервере
Процедура ЗаполнитьТиповыеДлиныСтрокКонвертов()
