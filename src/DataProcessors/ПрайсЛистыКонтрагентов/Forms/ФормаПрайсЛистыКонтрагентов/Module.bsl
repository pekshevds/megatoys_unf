...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокНовости = НСтр("ru='Новости: Прайс-лист контрагента'");
	НастройкиЗаполнения = Новый Структура("ПолучатьНовостиНаСервере, ХранитьМассивНовостейТолькоНаСервере", Истина, Истина);
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(ЭтотОбъект, "УНФ.Обработка.ПрайсЛистыКонтрагентов", "ФормаПрайсЛистыКонтрагентов", Неопределено, ЗаголовокНовости, Ложь, НастройкиЗаполнения, "ПриОткрытии");
	...
	КэшЗначений.Вставить("ВопросСозданияПервогоПрайсЛиста",	УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ВыводитьВопросСозданииПервогоПрайсЛистаКонтрагента"));
	...
	КэшЗначений.Вставить("ЕстьПравоДобавленияНового", 		ПравоДоступа("Добавление", Метаданные.Справочники.ПрайсЛистыКонтрагентов));
	...
	ОткрыватьПрайсЛистКонтрагентовКакСписок = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ОткрыватьПрайсЛистКонтрагентовКакСписок");
	
	Если НЕ Параметры.Свойство("РабочийПрайсЛистКонтрагентов", РабочийПрайсЛистКонтрагентов) Тогда
		
		РабочийПрайсЛистКонтрагентов = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("РабочийПрайсЛистКонтрагентов");
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РабочийПрайсЛистКонтрагентов) Тогда
	...
	Иначе
		...
		Объект.ВидЦенКонтрагентаУсловие = Перечисления.ВидСравненияЗначений.Равно;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ПрайсЛистыКонтрагентов") Тогда
...
&НаКлиенте
Процедура РабочийПрайсЛистКонтрагентовПриИзменении(Элемент)
	
	Если НЕ ЗначениеЗаполнено(РабочийПрайсЛистКонтрагентов) Тогда
		
		РабочийПрайсЛистКонтрагентов = Объект.Ссылка;
		Возврат;
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВводДаты(Оповещение, Объект.ПериодЦенКонтрагента, НСтр("ru = 'На дату'"), ЧастиДаты.Дата);
...
Процедура ПредставлениеПериодаНажатиеЗавершение(НовыйПериод, ДополнительныеДанные) Экспорт
	...
	Объект.ПериодЦенКонтрагента = НовыйПериод;
...
&НаКлиенте
Процедура ФормироватьПоОтсутствиюПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВидЦенКонтрагентаСравнитьПриИзменении(Элемент)
	
	УстановитьСвойстваЭлементовДополнительногоОтбора();
	
	Если Объект.ВидЦенКонтрагентаСравнить = Ложь Тогда
		
		Объект.ВидЦенКонтрагента1		= Неопределено;
		Объект.ВидЦенКонтрагентаУсловие	= ПредопределенноеЗначение("Перечисление.ВидСравненияЗначений.Равно");
		Объект.ВидЦенКонтрагента2		= Неопределено;
...
&НаКлиенте
Процедура ВидЦенКонтрагента1ПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВидЦенКонтрагентаУсловиеПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВидЦенКонтрагента2ПриИзменении(Элемент)
...
Процедура ТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если Расшифровка.Свойство("ВидЦенКонтрагента")
...
&НаКлиенте
Процедура ОткрыватьПрайсЛистКонтрагентовКакОбработкуПриИзменении(Элемент)
	
	ПерсональныеНастройкиВызовСервера.Установить(ОткрыватьПрайсЛистКонтрагентовКакСписок,
		"ОткрыватьПрайсЛистКонтрагентовКакСписок");
...
Процедура ПрайсЛистВыбрать(Команда)
	...
	ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.ФормаВыбора", ПараметрыОткрытия, , , , , ОписаниеОповещения);
...
Процедура ПослеВыбораЭлементаНастроекДляЗаполнения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		РабочийПрайсЛистКонтрагентов = Результат[0];
		
	ИначеЕсли ТипЗнч(Результат) = Тип("СправочникСсылка.ПрайсЛистыКонтрагентов") Тогда
		
		РабочийПрайсЛистКонтрагентов = Результат;
...
Процедура Декорация1Нажатие(Элемент)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("МассивВидыЦенКонтрагентов", Новый Массив);
	
	Для каждого СтрокаТаблицы Из Объект.ОтборВидовЦенКонтрагентов Цикл
		
		ПараметрыОткрытия.МассивВидыЦенКонтрагентов.Добавить(СтрокаТаблицы.ВидЦенКонтрагента);
		
	КонецЦикла;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораВидовЦен", ЭтотОбъект);
	
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораВидовЦенКонтрагентов", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещения);
...
Процедура ПослеВыбораВидовЦен(Результат, ДополнительныеПараметры) Экспорт
	...
	Объект.ОтборВидовЦенКонтрагентов.Очистить();
	Элементы.ВидЦенКонтрагента1.СписокВыбора.Очистить();
	Элементы.ВидЦенКонтрагента2.СписокВыбора.Очистить();
	
	Для каждого ЭлементМассива Из Результат.МассивВидыЦенКонтрагентов Цикл
		
		НоваяСтрока 					= Объект.ОтборВидовЦенКонтрагентов.Добавить();
		НоваяСтрока.ВидЦенКонтрагента	= ЭлементМассива;
		
		Элементы.ВидЦенКонтрагента1.СписокВыбора.Добавить(ЭлементМассива);
		Элементы.ВидЦенКонтрагента2.СписокВыбора.Добавить(ЭлементМассива);
		
	КонецЦикла;
	
	КоличествоСтрок = Объект.ОтборВидовЦенКонтрагентов.Количество();
	...
	УстановитьСвойстваЭлементовПоВидуЦенКонтрагентов();
...
Процедура Декорация2Нажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораНоменклатурыКомпоновщик", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещения);
...
Процедура Декорация3Нажатие(Элемент)
	...
	ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.Форма.ФормаОДокументе", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещения);
...
Процедура ПредставлениеПериодаТекстом()
	...
	Если ЗначениеЗаполнено(Объект.ПериодЦенКонтрагента) Тогда
		
		ПредставлениеПериода = Формат(Объект.ПериодЦенКонтрагента, "ДЛФ=DD");
...
&НаКлиенте
Процедура УстановитьСвойстваЭлементовПоВидуЦенКонтрагентов()
	
	КоличествоСтрок = Объект.ОтборВидовЦенКонтрагентов.Количество();
	Если КоличествоСтрок = 0 Тогда
		
		ТекстЗаголовка = НСтр("ru ='Колонки (не указаны)'");
		
	ИначеЕсли КоличествоСтрок = 1 Тогда
		
		ТекстЗаголовка = СокрЛП(Объект.ОтборВидовЦенКонтрагентов[0].ВидЦенКонтрагента);
		
	ИначеЕсли КоличествоСтрок = 2 Тогда
		
		ТекстЗаголовка = СокрЛП(Объект.ОтборВидовЦенКонтрагентов[0].ВидЦенКонтрагента) + ", " + СокрЛП(Объект.ОтборВидовЦенКонтрагентов[1].ВидЦенКонтрагента);
		
	Иначе
		
		ТекстЗаголовка = СокрЛП(Объект.ОтборВидовЦенКонтрагентов[0].ВидЦенКонтрагента) + ", " + СокрЛП(Объект.ОтборВидовЦенКонтрагентов[1].ВидЦенКонтрагента) 
			+ Символы.НПП + СтрШаблон(НСтр("ru ='(еще %1)'"), КоличествоСтрок - 2);
...
Процедура УстановитьСвойстваЭлементовВыделенияПоУсловию()
	...
	БолееОдногоВидаЦен		= (Объект.ОтборВидовЦенКонтрагентов.Количество() > 1);
	ЕстьВидыЦенКонтрагентов	= (Объект.ОтборВидовЦенКонтрагентов.Количество() > 0);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УсловнаяЦена",			"Доступность",	ЕстьВидыЦенКонтрагентов);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаЦенаПоУсловию",	"Доступность",	Объект.УсловнаяЦена);
...
Процедура УстановитьСвойстваЭлементовДополнительногоОтбора()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВидЦенСравнить", "Доступность", (Объект.ОтборВидовЦенКонтрагентов.Количество() > 1));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаУсловиеСравнения", "Доступность", Объект.ВидЦенКонтрагентаСравнить);
...
Функция ОбъектВСтруктуру()
	...
	СтруктураДанных.Вставить("ПериодЦенКонтрагента",		Объект.ПериодЦенКонтрагента);
	СтруктураДанных.Вставить("ФормироватьПоОтсутствию",		Объект.ФормироватьПоОтсутствию);
	...
	СтруктураДанных.Вставить("ВидЦенКонтрагентаСравнить",	Объект.ВидЦенКонтрагентаСравнить);
	СтруктураДанных.Вставить("ВидЦенКонтрагента1",			Объект.ВидЦенКонтрагента1);
	СтруктураДанных.Вставить("ВидЦенКонтрагентаУсловие",	Объект.ВидЦенКонтрагентаУсловие);
	СтруктураДанных.Вставить("ВидЦенКонтрагента2",			Объект.ВидЦенКонтрагента2);
	
	СтруктураДанных.Вставить("ВидыЦенКонтрагентов", 		Новый Массив);
	...
	Для каждого СтрокаВидовЦен Из Объект.ОтборВидовЦенКонтрагентов Цикл
		
		СтруктураДанных.ВидыЦенКонтрагентов.Добавить(СтрокаВидовЦен.ВидЦенКонтрагента);
...
Процедура ПоказатьСоветПользователю(ЗаголовокФормы, ТекстСообщения, ОписаниеОповещения, ВидимостьБольшеНеПоказывать)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаСообщения", ПараметрыСовета, ЭтаФорма, Истина, , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ЗаполнитьСписокВыбораЭлементовСравненияЦен()
	...
	Для каждого СтрокаТаблицы Из Объект.ОтборВидовЦенКонтрагентов Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ВидЦенКонтрагента)
			И СтрокаТаблицы.ВидЦенКонтрагента <> Объект.ВидЦенКонтрагента2 Тогда
			
			Элементы.ВидЦенКонтрагента1.СписокВыбора.Добавить(СтрокаТаблицы.ВидЦенКонтрагента);
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаТаблицы.ВидЦенКонтрагента)
			И СтрокаТаблицы.ВидЦенКонтрагента <> Объект.ВидЦенКонтрагента1 Тогда
			
			Элементы.ВидЦенКонтрагента2.СписокВыбора.Добавить(СтрокаТаблицы.ВидЦенКонтрагента);
...
Процедура ОткрытьФормуЗаписиРегистра(Расшифровка)
	...
	КлючЗаписи = ЗаписьОЦенеСуществует(Расшифровка, Объект.ПериодЦенКонтрагента);
	...
	ОткрытьФорму("РегистрСведений.ЦеныНоменклатурыКонтрагентов.ФормаЗаписи", ПараметрыОткрытия, ЭтотОбъект);
...
Функция ЗаписьОЦенеСуществует(Знач Расшифровка, ПериодЦенКонтрагента)
	
	КлючЗаписи = Новый Структура("Период, ВидЦенКонтрагента, Номенклатура, Характеристика");
	...
	Если НЕ Расшифровка.Свойство("Период", КлючЗаписи.Период) Тогда
		...
		КлючЗаписи.Период = ?(ЗначениеЗаполнено(ПериодЦенКонтрагента), ПериодЦенКонтрагента, НачалоДня(ТекущаяДатаСеанса()));
		
	КонецЕсли;
	
	Расшифровка.Свойство("ВидЦенКонтрагента",	КлючЗаписи.ВидЦенКонтрагента);
	...
	Возврат ЦенообразованиеСервер.ЗаписьОЦенеКонтрагентаСуществует(КлючЗаписи);
...
Функция ЕстьСохраненныеПрайсЛисты()
	...
	Возврат Справочники.ПрайсЛистыКонтрагентов.КоличествоПрайсЛистовКонтрагентовПользователя() <> 0;
...
Процедура ИзменитьОбъектФормыПоСсылке(НовыеДанныеЭлементаСправочника = Неопределено)
	...
	НовыйОбъект = РабочийПрайсЛистКонтрагентов.ПолучитьОбъект();
	...
	Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.НастройкиПользователей) Тогда
		...
		РегистрыСведений.НастройкиПользователей.Установить(Объект.Ссылка, "РабочийПрайсЛистКонтрагентов");
...
Процедура НачатьСохранениеНастроекПрайсЛиста(Отказ, ЗавершениеРаботы)
	...
	ИначеЕсли КэшЗначений.ВопросСозданияПервогоПрайсЛиста <> Ложь
		...
		МассивСтрок[0] = НСтр("ru ='У текущего пользователя нет еще сохраненных прайс-листов поставщиков.'");
		МассивСтрок[1] = НСтр("ru ='Создать первый прайс-лист (поставщика) и сохранить в него текущие параметры?'");
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИмяПользовательскойНастройки", "ВыводитьВопросСозданииПервогоПрайсЛистаКонтрагента");
...
Процедура ПослеВопросаОСохраненииНастроекПервогоПрайсЛиста(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") Тогда
		...
		Если РезультатЗакрытия.НажатаКнопкаОК Тогда
			...
			ЗаписатьЭлементСправочникаПрайсЛисты("Сохранить");
			ПерсональныеНастройкиВызовСервера.Установить(Объект.Ссылка, "РабочийПрайсЛистКонтрагентов");
...
Процедура ЗаписатьЭлементСправочникаПрайсЛисты(Действие)
	...
	ИначеЕсли Действие = "Новый" Тогда
		...
		ТекстВопроса = НСтр("ru ='Создать новый прайс-лист поставщиков (без настроек) и продолжить работу?'");
	...
	ИначеЕсли Действие = "Скопировать" Тогда
		...
		ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.Форма.ФормаЭлемента", ПараметрыОткрытия, ЭтаФорма);
...
Процедура ОбновитьСуществующийПрайсЛист(СсылкаОбновляемого, НовыеДанныеЭлементаСправочника)
	...
	Попытка
		...
		РегистрыСведений.НастройкиПользователей.Установить(СсылкаОбновляемого, "РабочийПрайсЛистКонтрагентов");
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ПрайсЛистыКонтрагентов");
...
Процедура СформироватьПрайсЛистНаСервере(ПараметрыПроцедуры)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Прайс-лист контрагентов, формирование'");
	ПараметрыВыполнения.ЗапуститьВФоне				= Истина;
	
	ИмяМетода = "Обработки.ПрайсЛистыКонтрагентов.СформироватьПрайсЛистыКонтрагентов";
	...
	КэшЗначений.ПараметрыДлительнойОперации.ФоновоеЗаданиеИдентификатор = КэшЗначений.ПараметрыДлительнойОперации.ФоновоеЗаданиеРезультат.ИдентификаторЗадания;
...
&НаКлиенте
Процедура ОтложенноеОформлениеПрайсЛиста()
	...
	Иначе
		...
		ПодключитьОбработчикОжидания("ОтложенноеОформлениеПрайсЛиста", 0.5, Истина);
