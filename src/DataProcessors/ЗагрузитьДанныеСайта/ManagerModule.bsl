		...
		Для каждого пакетДанных Из ПакетыДанных Цикл
			...
			Если ЕстьКлассификатор Тогда
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ТипыТовара", Классификатор)
					...
					МассивКатегорийXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(Классификатор.ТипыТовара.ТипТовара);
					Для каждого КатегорияXDTO Из МассивКатегорийXDTO Цикл
						...
						Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой Тогда
							...
							СтрокаСопоставлениеКатегорий.КлассификаторИд = ПолучитьИдентификаторКлассификатора(Классификатор);
				...
				Иначе
					...
					Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой Тогда
						...
						СтрокаСопоставлениеКатегорий.Ид = ПолучитьИдентификаторКлассификатора(Классификатор);
						...
						МассивИдентификаторовКатегорий.Добавить(ПолучитьИдентификаторКлассификатора(Классификатор));
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Свойства", Классификатор)
					...
					МассивСвойствXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(Классификатор.Свойства.Свойство);
					Для каждого СвойствоXDTO Из МассивСвойствXDTO Цикл
						Свойства.Вставить(СвойствоXDTO.Наименование);
						
						СвойствоXDTOТипТовара = ?(НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ТипТовара", СвойствоXDTO),СвойствоXDTO.ТипТовара,ПолучитьИдентификаторКлассификатора(Классификатор));
						...
						Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И МассивИдентификаторовКатегорий.Найти(СвойствоXDTOТипТовара) <> Неопределено
							...
							СтрокаСопоставлениеСвойств.КлассификаторИд = ПолучитьИдентификаторКлассификатора(Классификатор);
							...
							Иначе
								СтрокаСопоставлениеСвойств.ТипТовара = ПолучитьИдентификаторКлассификатора(Классификатор);
							КонецЕсли;
			...
			Если ЕстьКаталог И НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Товар", ВесьФайл.Каталог.Товары) Тогда
				...
				МассивТоваровXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ВесьФайл.Каталог.Товары.Товар);
				Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
					...
					Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой Тогда
						...
						Иначе
							СтрокаСопоставлениеТоваров.ТипТовара = ПолучитьИдентификаторКлассификатора(Классификатор);
						КонецЕсли;
						
						СтрокаСопоставлениеТоваров.КлассификаторИд = ПолучитьИдентификаторКлассификатора(Классификатор);
					...
					Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Штрихкод", ТоварXDTO) 
						И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ТоварXDTO.Штрихкод) Тогда
			...
			Если ЕстьПакетПредложений Тогда
				...
				МассивТоварныхПредложенийXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ВесьФайл.ПакетПредложений.Предложения.Предложение);
				...
				Для каждого ТоварноеПредложениеXDTO Из МассивТоварныхПредложенийXDTO Цикл
					...
					КонецЕсли;
				КонецЦикла;
				МассивТиповЦенXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ВесьФайл.ПакетПредложений.ТипыЦен.ТипЦены);
...
Функция СоздатьУзелОбмена(Параметры, ИмяСайта, ОписаниеОшибки) Экспорт
	...
	Исключение
		ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеОшибки());
	КонецПопытки;
...
Процедура ПрочитатьКаталогиРекурсивно(спГруппы, СоответствиеГрупп, НомерПоПорядку)
	...
	Если НЕ НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Группа", спГруппы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СопоставитьДанныеСайта(Параметры)
	...
	|	И СоответствияОбъектовИнтернетМагазина.ТипОбъекта = ""Справочник.КатегорииНоменклатуры""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	СоответствияОбъектовИнтернетМагазина.КлючИнтеграции = &КлючИнтеграции
	|	И СоответствияОбъектовИнтернетМагазина.ТипОбъекта = ""Справочник.Номенклатура""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	СоответствияОбъектовИнтернетМагазина.КлючИнтеграции = &КлючИнтеграции
	|	И СоответствияОбъектовИнтернетМагазина.ТипОбъекта = ""ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ЗагрузитьПакет(пакетДанных, Соединение=Неопределено, Параметры, КаталогСайта, ОписаниеОшибки)
	...
	Если ЕстьКлассификатор Тогда
		...
		Параметры.Вставить("КлассификаторИд" , ПолучитьИдентификаторКлассификатора(Классификатор));
		
		КаталогСайта.Значение.Вставить("Наименование", Классификатор.Наименование);
		
		Если Параметры.ТипСайта = Перечисления.ПротоколыОбменаCMS.Битрикс Тогда
			ЗагрузитьКатегорию(Классификатор, НастройкаИнтеграции, Параметры, ОписаниеОшибки);
		КонецЕсли;
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ТипыТовара", Классификатор)
			...
			Для каждого КатегорияXDTO Из МассивКатегорийXDTO Цикл
				...
				Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеКатегорий") Тогда
					...
					Отбор = Новый Структура("КлассификаторИд,Ид" , ПолучитьИдентификаторКлассификатора(Классификатор) , КатегорияXDTO.Ид);
				...
				ЗагрузитьКатегорию(КатегорияXDTO, НастройкаИнтеграции, Параметры, ОписаниеОшибки);
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Группы", Классификатор) Тогда
			...
			ЗагрузитьКаталогиРекурсивно(спГруппы, Параметры.КаталогЗагруженныхТоваров, НастройкаИнтеграции, УровеньВложенности, ГруппыВерхнегоУровня);
		КонецЕсли;
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Свойство", Классификатор.Свойства) Тогда
			...
			Для каждого СвойствоXDTO Из МассивСвойствXDTO Цикл
				...
				Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеСвойств") Тогда
					...
					Отбор = Новый Структура("КлассификаторИд,Ид" , ПолучитьИдентификаторКлассификатора(Классификатор) , СвойствоXDTO.Ид);
				...
				ЗагрузитьСвойствоНоменклатуры(СвойствоXDTO, НастройкаИнтеграции, Параметры);
			КонецЦикла;
		КонецЕсли; 
	...
	Если ЕстьКаталог Тогда
		...
		ТоварыПараметры.Вставить("НастройкаИнтеграции", НастройкаИнтеграции);
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Товар", Каталог.Товары) Тогда
			...
			МассивТоваровXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(Каталог.Товары.Товар);
			Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
				...
				Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеКатегорий") Тогда
					...
					СопоставлениеКатегорий = Параметры.СопоставлениеКатегорий;
					Отбор = Новый Структура("КлассификаторИд" , ?(Классификатор = Неопределено,Каталог.ИдКлассификатора,ПолучитьИдентификаторКлассификатора(Классификатор)));
					
					Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ТипТовара", ТоварXDTO) Тогда
						Отбор.Вставить("Ид" , ТоварXDTO.ТипТовара);
					Иначе
						Отбор.Вставить("Ид" , ?(Классификатор = Неопределено,Каталог.ИдКлассификатора, ПолучитьИдентификаторКлассификатора(Классификатор)));
					КонецЕсли;
				...
				Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеТоваров") Тогда
					...
					Отбор = Новый Структура("КлассификаторИд,Ид" , ?(Классификатор = Неопределено,Каталог.ИдКлассификатора,ПолучитьИдентификаторКлассификатора(Классификатор)) , ТоварXDTO.Ид);
				...
				ТипНоменклатурыXDTO = "";
				ВидНоменклатурыXDTO = "";
				Если ИнтеграцияСИнтернетМагазиномСервер.ОбъектXDTOСодержитСвойство(ТоварXDTO, "ЗначенияРеквизитов")
					И ТоварXDTO.ЗначенияРеквизитов <> Неопределено
					И ИнтеграцияСИнтернетМагазиномСервер.ОбъектXDTOСодержитСвойство(ТоварXDTO.ЗначенияРеквизитов, "ЗначениеРеквизита") Тогда
					
					ЗначенияРеквизитовXDTO = ТоварXDTO.ЗначенияРеквизитов.ЗначениеРеквизита;
					МассивЗначенийРеквизитовXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ЗначенияРеквизитовXDTO);
				...
				Если НЕ ЗначениеЗаполнено(Номенклатура) Тогда
					Номенклатура = ИнтеграцияСИнтернетМагазиномСервер.НайтиСоздатьНоменклатуру(ТоварXDTO, ЭтоУслуга, ТоварыПараметры, , ОписаниеОшибки);
				КонецЕсли;
				...
				ЗаписатьСоответствие(ТоварXDTO, Номенклатура, НастройкаИнтеграции);
				
				ХарактеристикаНоменклатуры = ИнтеграцияСИнтернетМагазиномСервер.НайтиСоздатьХарактеристикуНоменклатуры(
				ТоварXDTO, 
				Ложь, 
				Номенклатура,
				ТоварыПараметры,
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Ид", ТоварXDTO.Группы)
					И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ТоварXDTO.Группы.Ид) Тогда
					ГруппаРодитель = СсылкаСоответствиеПоИдСайта(ТоварXDTO.Группы.Ид, "Справочник.Номенклатура", НастройкаИнтеграции, Истина);
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ТипТовара", ТоварXDTO) 
					И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ТоварXDTO.ТипТовара) Тогда
					Категория = СсылкаСоответствиеПоИдСайта(ТоварXDTO.ТипТовара, "Справочник.КатегорииНоменклатуры", НастройкаИнтеграции);
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ПолноеНаименование", ТоварXDTO)
					И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ТоварXDTO.ПолноеНаименование) Тогда
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Описание", ТоварXDTO) 
					И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ТоварXDTO.Описание) Тогда
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Штрихкод", ТоварXDTO)
					...
					НаборЗаписейШтрихкоды.Характеристика = ХарактеристикаНоменклатуры;
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТоварXDTO.ЗначенияСвойств, "ЗначенияСвойства") Тогда
					...
					МассивЗначенийСвойствXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ЗначенияСвойствXDTO);
					Для каждого ЗначениеСвойстваXDTO Из МассивЗначенийСвойствXDTO Цикл
						...
						Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеСвойств") Тогда
							...
							СопоставлениеСвойств = Параметры.СопоставлениеСвойств;
							Отбор = Новый Структура("КлассификаторИд,Ид" , ?(Классификатор = Неопределено,Каталог.ИдКлассификатора,ПолучитьИдентификаторКлассификатора(Классификатор)) , ЗначениеСвойстваXDTO.Ид);
							НайденныеСтроки = СопоставлениеСвойств.НайтиСтроки(Отбор);
							Для Каждого Стр Из НайденныеСтроки Цикл
								ЗагружатьСвойство = Стр.Загружать;
							КонецЦикла;
						...
						Если ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ЗначениеСвойстваXDTO.Значение) Тогда
							
							ВладелецПВХ = СсылкаСоответствиеПоИдСайта(ЗначениеСвойстваXDTO.Ид, "ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения", НастройкаИнтеграции);
							...
							Иначе	
								ЗначениеСвойства = СсылкаСоответствиеПоИдСайта(ЗначениеСвойстваXDTO.Значение, "Справочник.ЗначенияСвойствОбъектов", НастройкаИнтеграции);
							КонецЕсли;
				...
				Если Параметры.ЕстьКартинки И Параметры.ЗагружатьКартинки И Соединение<>Неопределено Тогда
					Если ТипЗнч(ТоварXDTO.Картинка)=Тип("Строка") Тогда
						...
						Если РезультатЗагрузки<>"" Тогда
							ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, РезультатЗагрузки);
						КонецЕсли;
					ИначеЕсли ТипЗнч(ТоварXDTO.Картинка)=Тип("СписокXDTO") Тогда
						МассивКартинокТоварXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ТоварXDTO.Картинка);
	...
	Если ЕстьПакетПредложений Тогда
		...
		// Типы цен
		МассивТипыЦенXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ВесьФайл.ПакетПредложений.ТипыЦен.ТипЦены);
		...
		Для каждого ТипЦенXDTO Из МассивТипыЦенXDTO Цикл
			...
			Если Параметры.ТипСайта = Перечисления.ПротоколыОбменаCMS.UMI Тогда
				Если Параметры.ЗагрузитьВидЦенССайта Тогда
					СсылкаВидЦен = ЗагрузитьТипЦен(ТипЦенXDTO, НастройкаИнтеграции);
					ВидыЦенСайта.Вставить(СсылкаВидЦен);
				Иначе
					ЗаписатьСоответствие(ТипЦенXDTO, Параметры.ВидЦен, НастройкаИнтеграции);
				КонецЕсли;
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Предложение", ВесьФайл.ПакетПредложений.Предложения) Тогда
			...
			МассивПредложенийXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ВесьФайл.ПакетПредложений.Предложения.Предложение);
			Для каждого ПредложениеXDTO Из МассивПредложенийXDTO Цикл
				...
				Если ИспользуетсяРазделитель Тогда
					...
					Иначе
						ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, "Не указан код товара в товарном предложении "+ПредложениеXDTO.Ид);
						Продолжить;
					КонецЕсли;
				Иначе	
				...
				Номенклатура = СсылкаСоответствиеПоИдСайта(ИдТовара, "Справочник.Номенклатура", НастройкаИнтеграции, Ложь);
				Если Номенклатура=Неопределено Тогда
					ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, "Не найден товар "+ПредложениеXDTO.Наименование+" по коду "+ПредложениеXDTO.Ид+" в товарном предложении, предложение не загружено!");
					Продолжить;
				КонецЕсли;
				...
				Если ИспользуетсяРазделитель ИЛИ Номенклатура.ИспользоватьХарактеристики Тогда
					...
					Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ХарактеристикиТовара", ПредложениеXDTO)
						...
						МассивСвойствХарактеристикиXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ПредложениеXDTO.ХарактеристикиТовара.ХарактеристикаТовара);
						Для каждого СвойствоХарактеристикиXDTO Из МассивСвойствХарактеристикиXDTO Цикл
							Если ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(СвойствоХарактеристикиXDTO.Наименование) 
								И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(СвойствоХарактеристикиXDTO.Значение) Тогда
						...
						Если ИдХарактеристики <> "" Тогда
							СсылкаХарактеристика = СсылкаСоответствиеПоИдСайта(ИдХарактеристики, "Справочник.ХарактеристикиНоменклатуры", НастройкаИнтеграции);
						КонецЕсли;
						...
						Иначе
							СсылкаХарактеристика = НайтиСоздатьХарактеристикуПоНаборуСвойств(Номенклатура, НаборыСвойствХарактеристики, ПредложениеXDTO);
							ЗаписатьСоответствие(Новый Структура("Ид",ИдХарактеристики), СсылкаХарактеристика, НастройкаИнтеграции);
						КонецЕсли;
					...
					Если ЗначениеЗаполнено(СсылкаХарактеристика) 
						...
						ИначеЕсли ТипЗнч(ПредложениеXDTO.Картинка)=Тип("СписокXDTO") Тогда
							...
							МассивКартинокХарактеристикиXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ПредложениеXDTO.Картинка);
...
Процедура ЗагрузитьПакетПроверкаСопоставления(ЗагружатьКатегорию, ЗагружатьТовар, ИдТовара, Классификатор, Параметры)
	...
	Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеТоваров") Тогда
		...
		СопоставлениеТоваров = Параметры.СопоставлениеТоваров;
		Отбор = Новый Структура("КлассификаторИд,Ид" , ПолучитьИдентификаторКлассификатора(Классификатор) , ИдТовара);
		НайденныеСтроки = СопоставлениеТоваров.НайтиСтроки(Отбор);
		Для Каждого Стр Из НайденныеСтроки Цикл
			ЗагружатьТовар = Стр.Загружать;
			...
			Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И Параметры.Свойство("СопоставлениеКатегорий") Тогда
				...
				СопоставлениеКатегорий = Параметры.СопоставлениеКатегорий;
				Отбор = Новый Структура("КлассификаторИд,Ид" , ПолучитьИдентификаторКлассификатора(Классификатор) , Стр.ТипТовара);
				НайденныеСтроки = СопоставлениеКатегорий.НайтиСтроки(Отбор);
				Для Каждого Стр Из НайденныеСтроки Цикл
					ЗагружатьКатегорию = Стр.Загружать;
				КонецЦикла;
...
Функция ПолучитьИдентификаторКлассификатора(Классификатор)
	
	Попытка
		Возврат Классификатор.Ид;
	Исключение
		Возврат "1";
	КонецПопытки;
...
Процедура ОбновитьСписокКаталоговДляВыгрузкиИВидовЦенУзлаОбмена(Параметры)
	...
	НастройкаИнтеграцииОбъект = Параметры.НастройкаИнтеграции.ПолучитьОбъект();
	// Список каталогов
	НастройкаИнтеграцииОбъект.СохраненнаяТаблицаКаталогов = Новый ХранилищеЗначения(ТаблицаКаталогов);
	// Виды цен
	НастройкаИнтеграцииОбъект.ВидыЦен.Очистить();
	Если Параметры.Свойство("ВидыЦен") Тогда
		Для каждого стр Из Параметры.ВидыЦен Цикл
			НовыйВидЦен = НастройкаИнтеграцииОбъект.ВидыЦен.Добавить();
...
Функция URLАдминзоны(Параметры, ИмяСайта, ОписаниеОшибки)
	...
	РезультатПодключения = ИнтеграцияСИнтернетМагазиномСервер.ВыполнитьПодключениеКСайту(Параметры.НастройкаИнтеграции);
	Соединение = РезультатПодключения.Соединение;
	Если Соединение = Неопределено Тогда
		ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Авторизация не выполнена.'"));
		Возврат "";
	КонецЕсли;
	...
	Если ОтветHTTP.КодСостояния<>200 Тогда
		ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Ошибка! Ответ HTTP: '")+ОтветHTTP.КодСостояния);
		Возврат "";
	КонецЕсли;
...
Процедура ЗагрузитьЦены(ПредложениеXDTO, Номенклатура, Характеристика, Параметры)
	...
	МассивЦенXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(ПредложениеXDTO.Цены.Цена);
	Для каждого ЦенаXDTO Из МассивЦенXDTO Цикл
		...
		ВидЦен = ИнтеграцияСИнтернетМагазиномСервер.СсылкаСоответствиеПоИд(ЦенаXDTO.ИдТипаЦены, "Справочник.ВидыЦен", Параметры.НастройкаИнтеграции);
		...
		ДопРеквизиты.Вставить("Период", НачалоДня(ТекущаяДатаСеанса()));
...
Функция ЗагрузитьТипЦен(ТипЦенXDTO, НастройкаИнтеграции)
	...
	ДопРеквизиты = Новый Структура;
	Если ИнтеграцияСИнтернетМагазиномСервер.ОбъектXDTOСодержитСвойство(ТипЦенXDTO, "Налог")
		И ТипЦенXDTO.Налог.Наименование="НДС" 
		И ИнтеграцияСИнтернетМагазиномСервер.ОбъектXDTOСодержитСвойство(ТипЦенXDTO.Налог, "УчтеноВСумме") Тогда
	...
	ЗаписатьСоответствие(ТипЦенXDTO, СсылкаВидЦен, НастройкаИнтеграции);
	
	Возврат СсылкаВидЦен;
	
КонецФункции	

Процедура ЗагрузитьКатегорию(КатегорияXDTO, НастройкаИнтеграции, Параметры, ОписаниеОшибки="")
	...
	Если ЗначениеЗаполнено(СсылкаКатегория) Тогда
		...
		ЗаписатьСоответствие(КатегорияXDTO, СсылкаКатегория.НаборСвойств, НастройкаИнтеграции);
		ЗаписатьСоответствие(КатегорияXDTO, СсылкаКатегория, НастройкаИнтеграции);
	...
	Иначе
		...
		ЗаписатьСоответствие(КатегорияXDTO, СсылкаНаборСвойств, НастройкаИнтеграции);
...
Функция СсылкаСоответствиеПоИдСайта(ИдОбъекта, ТипОбъекта, НастройкаИнтеграции, ЭтоГруппа=Неопределено)
	...
	Если ЭтоГруппа<>Неопределено Тогда
		...
		Запрос.Текст = Запрос.Текст + "	И ВЫРАЗИТЬ(СоответствияОбъектовИнтернетМагазина.СсылкаНаОбъект КАК Справочник.Номенклатура).ЭтоГруппа = &ЭтоГруппа";
...
Процедура ЗагрузитьКаталогиРекурсивно(спГруппы, Родитель=Неопределено, НастройкаИнтеграции, УровеньВложенности, ГруппыВерхнегоУровня)
	
	Если Не НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Группа", спГруппы) Тогда
		Возврат;
	КонецЕсли;
	
	МассивТоваровXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(спГруппы.Группа);
	Для каждого ГруппаXDTO Из МассивТоваровXDTO Цикл
		...
		ЗаписатьСоответствие(ГруппаXDTO, СсылкаГруппа, НастройкаИнтеграции);
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Группы", ГруппаXDTO) 
			И НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Группа", ГруппаXDTO.Группы) Тогда
			ЗагрузитьКаталогиРекурсивно(ГруппаXDTO.Группы, СсылкаГруппа, НастройкаИнтеграции, 1, ГруппыВерхнегоУровня);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаписатьСоответствие(ОбъектXDTO, СсылкаИБ, НастройкаИнтеграции)
	...
	Если НЕ ЗначениеЗаполнено(СистемныйИд) Тогда
		Возврат;
	КонецЕсли;
	
	РегМенеджер = РегистрыСведений.СоответствияОбъектовИнтернетМагазина.СоздатьМенеджерЗаписи();
	РегМенеджер.ИдентификаторМагазина = СистемныйИд;
	РегМенеджер.ИдентификаторБазы = ИдСайта;
	РегМенеджер.КлючИнтеграции = ПолучитьКлючПоНастройке(НастройкаИнтеграции);
...
Процедура ЗаписатьОшибку(Параметры, ОписаниеОшибки)
	...
	ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(Параметры.ОписаниеОшибки, ОписаниеОшибки);
	Параметры.Вставить("ОписаниеОшибки", ОписаниеОшибки);
		...
		Пока Истина Цикл
			...
			Иначе
				...
				Если Лев(ДанныеСтрока, 2) = "PK" Тогда
					СтрокаXML = ИнтеграцияСИнтернетМагазиномСервер.РаспаковатьZIPАрхив(ОтветСервера, ОписаниеОшибки);
				Иначе
	...
	Пока НЕ КонецПередачи Цикл
		...
		Пока Истина Цикл
			...
			Иначе
				...
				СтрокаXML = "";
				Если Лев(ДанныеСтрока, 2) = "PK" Тогда
					СтрокаXML = ИнтеграцияСИнтернетМагазиномСервер.РаспаковатьZIPАрхив(ОтветСервера, ТекстОшибки);
				Иначе
...
Функция НайтиСоздатьХарактеристикуПоНаборуСвойств(Номенклатура, НаборыСвойствХарактеристики, ПредложениеXDTO)
	...
	Если Результат.Пустой() Тогда
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Наименование", ПредложениеXDTO) Тогда
			
			ОбъектХарактеристика.Наименование = СокрЛП(ПредложениеXDTO.Наименование);
			ОбъектХарактеристика.НаименованиеДляПечати = СокрЛП(ПредложениеXDTO.Наименование);
			
		Иначе
			
			ОбъектХарактеристика.Наименование = СокрЛП(НаименованиеХарактеристики);
			ОбъектХарактеристика.НаименованиеДляПечати = СокрЛП(НаименованиеХарактеристики);
...
Процедура ЗагрузитьСвойствоНоменклатуры(СвойствоXDTO, НастройкаИнтеграции, Параметры)
	
	ДопРеквизиты = Новый Структура;
	СтруктураОтбора = Новый Структура("Наименование", СвойствоXDTO.Наименование);
	
	ПротоколОбменаCMS = НастройкаИнтеграции.ПротоколОбменаCMS;
	...
	Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Общее", СвойствоXDTO) Тогда
		Если (СвойствоXDTO.Общее = Истина ИЛИ СвойствоXDTO.Общее = НРег("true")
			...
			СсылкаНаборСвойств = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Номенклатура_Общие");
			// Общее свойство технического характера, которое загружать не нужно
			Возврат;
		Иначе
			СсылкаНаборСвойств = ИнтеграцияСИнтернетМагазиномСервер.СсылкаСоответствиеПоИд(СвойствоXDTO.ТипТовара, "Справочник.НаборыДополнительныхРеквизитовИСведений", НастройкаИнтеграции);
	...
	Если ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.UMI Тогда
		СсылкаПВХ = СсылкаСоответствиеПоИдСайта(СвойствоXDTO.СистемныйИд, "ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения", НастройкаИнтеграции);
	ИначеЕсли ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.Битрикс Тогда
		СсылкаПВХ = СсылкаСоответствиеПоИдСайта(СвойствоXDTO.Ид, "ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения", НастройкаИнтеграции);
	Иначе
		Возврат;
	КонецЕсли;
	Если СсылкаПВХ=Неопределено Тогда
		...
		ЗаписатьСоответствие(СвойствоXDTO, СсылкаПВХ, НастройкаИнтеграции);
	...
	Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ВариантыЗначений", СвойствоXDTO) Тогда
		...
		Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Справочник", СвойствоXDTO.ВариантыЗначений) Тогда
			МассивЗначенийСвойствXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(СвойствоXDTO.ВариантыЗначений.Справочник);
		ИначеЕсли НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Вариант", СвойствоXDTO.ВариантыЗначений) Тогда
			МассивЗначенийСвойствXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(СвойствоXDTO.ВариантыЗначений.Вариант);
		Иначе	
			МассивЗначенийСвойствXDTO = ИнтеграцияСИнтернетМагазиномСервер.ПолучитьМассивОбъектовСпискаXDTO(СвойствоXDTO.ВариантыЗначений);
		КонецЕсли;
		...
		Для каждого ЗначениеСвойстваXDTO Из МассивЗначенийСвойствXDTO Цикл
			...
			Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("СистемныйИд", ЗначениеСвойстваXDTO)
				И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(ЗначениеСвойстваXDTO.СистемныйИд) Тогда
				
				ЗаписатьСоответствие(ЗначениеСвойстваXDTO, СсылкаЗначениеСвойства, НастройкаИнтеграции);
			ИначеЕсли НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Вариант", ЗначениеСвойстваXDTO) Тогда
				ЗаписатьСоответствие(ЗначениеСвойстваXDTO.Вариант, СсылкаЗначениеСвойства, НастройкаИнтеграции);
...
Функция ЗагрузитьСвойствоХарактеристикиИЗначение(СвойствоXDTO, Параметры)
	...
	// Создать/найти Набор свойств
	Если Параметры.ТипСайта=Перечисления.ПротоколыОбменаCMS.Битрикс Тогда
		СтруктураОтбора = Новый Структура("Наименование",
			Параметры.КатегорияКлассификатор.НаборСвойствХарактеристики.Наименование);
		...
		СтруктураОтбора.Вставить("Родитель", НаборСвойств_Справочник_ХарактеристикиНоменклатуры);
		ДопРеквизиты = Новый Структура;
		ДопРеквизиты.Вставить("Используется", Истина);
		СсылкаНаборСвойств = СоздатьИлиНайтиСуществующий(СтруктураОтбора,
			Справочники.НаборыДополнительныхРеквизитовИСведений, ДопРеквизиты);
	Иначе
	...
	ДопРеквизиты.Вставить("Виден", Истина);
	ДопРеквизиты.Вставить("ДополнительныеЗначенияИспользуются", Истина);
	СсылкаПВХ = СоздатьИлиНайтиСуществующий(СтруктураОтбора, ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения,
		ДопРеквизиты);
	...
	Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Значение", СвойствоXDTO) 
		И ИнтеграцияСИнтернетМагазиномСервер.СвойствоXDTOЗаполнено(СвойствоXDTO.Значение) Тогда
		
		СсылкаЗначениеСвойства = СсылкаСоответствиеПоИдСайта(СвойствоXDTO.Значение, "Справочник.ЗначенияСвойствОбъектов", Параметры.НастройкаИнтеграции);
