...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ТекущийРазмер.СписокВыбора.Количество() = 1 Тогда
		ТекущийРазмер = Элементы.ТекущийРазмер.СписокВыбора[0];
	КонецЕсли;
...
Процедура ОтправитьНаПечать()
	...
	ПечатьДокументовУНФКлиент.ОбработкаКомандыПечатиЦенниковИЭтикетокНаПринтереЭтикеток(ПараметрыПечатиДляПринтераЭтикеток);
