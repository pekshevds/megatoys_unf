...
Функция ТекстТекущихСозданныхЗвонков()
	...
	ДанныеЗвонков = Запрос.Выполнить();
	ДанныеЗвонковВыборка = ДанныеЗвонков.Выбрать();
	...
	Пока ДанныеЗвонковВыборка.Следующий() Цикл
		Строки.Добавить(Символы.ПС);
		Для каждого Колонка Из ДанныеЗвонков.Колонки Цикл
			Строки.Добавить(СтрШаблон("%1: %2", Колонка.Имя, Строка(ДанныеЗвонковВыборка[Колонка.Имя])));
		КонецЦикла;
		Строки.Добавить(Символы.ПС);
	КонецЦикла;
