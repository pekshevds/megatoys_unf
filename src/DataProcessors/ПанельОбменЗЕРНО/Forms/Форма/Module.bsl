...
Процедура ОтобразитьСостояниеОбмена()
	...
	ЕстьПроблема =  Не (РезультатыЗапроса["НастройкаРегламентногоОбменаЗагрузкиСДИЗЗерноСХТП"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаЗагрузкиСДИЗЗерноСХТП"));
	...
	ЕстьПроблема = Не (РезультатыЗапроса["НастройкаРегламентногоОбменаЗагрузкиСДИЗППЗ"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаЗагрузкиСДИЗППЗ"));
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = Не (РезультатыЗапроса["НастройкаРегламентногоОбменаЗагрузкиСДИЗЗерноЭлеватор"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаЗагрузкиСДИЗЗерноЭлеватор"));
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = Не (РезультатыЗапроса["НастройкаРегламентногоОбменаЗагрузкиПартийЗерноСобственныеПартии"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаЗагрузкиПартийЗерноСобственныеПартии"));
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = Не (РезультатыЗапроса["НастройкаРегламентногоОбменаЗагрузкиОстатковПартий"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаЗагрузкиОстатковПартий"));
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = Не (РезультатыЗапроса["НастройкаРегламентногоОбменаДанными"].Пустой()
		Или ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаРегламентногоОбменаДанными"));
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаСертификатаНаСервере", Ложь);
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
	...
	ЕстьПроблема = ИнтеграцияЗЕРНО.РегЗаданияНастроены(РезультатыЗапроса, "НастройкаОтветственныхЗаПодписаниеСообщений", Ложь);
	Если ЕстьПроблема Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
