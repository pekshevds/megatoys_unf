...
&НаКлиенте
Перем ПользователиТелефонииВнутреннийИдентификаторВАТСИзменен;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_НаборКонстант" И Источник = "ОбщиеНастройкиТелефонии" Тогда
		
		Если ИспользуемаяАТС <> Неопределено И ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн") Тогда
			
			ПроверкаПодпискиНаСобытияАТСБилайн();
...
&НаКлиенте
Процедура ИспользоватьАТС_МТТПриИзменении(Элемент)
	
	ПриВыбореАТС(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.МТТ"));
	Оповестить("Запись_НаборКонстант");
	ПрочитатьЗначенияНастроек();
...
&НаКлиенте
Процедура ИспользоватьАТС_БилайнПриИзменении(Элемент)
	
	ПриВыбореАТС(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн"));
	Оповестить("Запись_НаборКонстант");
	ПрочитатьЗначенияНастроек();
...
&НаКлиенте
Процедура АТС_МТТЛогоНажатие(Элемент)
	
	ПерейтиПоСсылке(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.МТТ"));
	
КонецПроцедуры

&НаКлиенте
Процедура АТС_БилайнЛогоНажатие(Элемент)
	
	ПерейтиПоСсылке(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн"));
...
&НаКлиенте
Процедура АТС_МТТСсылкаНажатие(Элемент)
...
&НаКлиенте
Процедура АТС_БилайнСсылкаНажатие(Элемент)
...
Процедура АТС_Билайн_КлючДляАвторизацииВОблачнойАТСПриИзменении(Элемент)
	...
	ЗаписатьЗначениеНастройкиИнтеграцииАТС("НастройкиАТС_Билайн", 0);
	ПроверкаПодпискиНаСобытияАТСБилайн();
	ОбновитьСписокПользователейТелефонии(ИспользуемаяАТС);
...
Процедура ПользователиТелефонииПриИзменении(Элемент)
	...
	Если НЕ (ПользователиТелефонииПользовательИзменен = Истина
		...
		ИЛИ ПользователиТелефонииИсходящийНомерИзменен = Истина
		ИЛИ ПользователиТелефонииВнутреннийИдентификаторВАТСИзменен = Истина) Тогда
		Возврат;
	КонецЕсли;
	...
	ПользователиТелефонииВнутреннийИдентификаторВАТСИзменен = Ложь;
...
&НаКлиенте
Процедура ПользователиТелефонииВнутреннийНомерОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн") Тогда
		Элементы.ПользователиТелефонии.ТекущиеДанные.ВнутреннийИдентификаторВАТС = СписокАбонентов.НайтиПоЗначению(ВыбранноеЗначение).Представление;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПользователиТелефонииВнутреннийИдентификаторВАТСПриИзменении(Элемент)
	ПользователиТелефонииВнутреннийИдентификаторВАТСИзменен = Истина;
КонецПроцедуры
...
Процедура ПрочитатьЗначенияНастроек()
	...
	// Билайн
	НастройкиАТС_Билайн.Очистить();
	НоваяНастройка = НастройкиАТС_Билайн.Добавить();
	НоваяНастройка.Ключ = "КлючДляАвторизацииВАТСБилайн";
	НоваяНастройка.Значение = НастройкиТелефонии.КлючДляАвторизацииВАТСБилайн;
	
	НоваяНастройка = НастройкиАТС_Билайн.Добавить();
	НоваяНастройка.Ключ = "URL";
	НоваяНастройка.Значение = ТелефонияСервер.АдресОбратногоВызоваОблачнаяТелефония(Перечисления.ДоступныеАТС.Билайн);
...
Функция ИмяТаблицыНастроек(АТС)
	...
	ИначеЕсли АТС = Перечисления.ДоступныеАТС.Билайн Тогда
		Возврат "НастройкиАТС_Билайн";
	ИначеЕсли АТС = Перечисления.ДоступныеАТС.Энфорта
		...
		ИЛИ АТС = Перечисления.ДоступныеАТС.МГТС
		ИЛИ АТС = Перечисления.ДоступныеАТС.МТТ Тогда
...
Процедура ОпределитьДоступностьЭлементов()
	...
	Элементы.ИспользоватьАТС_МТТ.Доступность            = ЭтоАдминистратор;
	Элементы.ИспользоватьАТС_Билайн.Доступность         = ЭтоАдминистратор;
	...
	Элементы.ГруппаНастройкиАТС_Билайн.Доступность      = ЭтоАдминистратор;
...
Процедура ПриВыбореАТС(ВыбраннаяАТС = Неопределено)
	...
	ИспользоватьАТС_МТТ                   = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.МТТ");
	ИспользоватьАТС_Билайн                = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн");
	...
	Если ВыбраннаяАТС <> Неопределено Тогда
		...
		Попытка
			...
			Константы.ИспользоватьРегламентноеОбновлениеПодпискиАТС.Установить(ЭтоОблачнаяАТС(ИспользуемаяАТС) И ИспользуемаяАТС = Перечисления.ДоступныеАТС.Билайн);
			...
			Константы.ИспользоватьМобильнуюТелефонию.Установить(ИспользоватьМобильнаяТелефония);
...
Процедура УправлениеФормой()
	...
	Элементы.ГруппаНастройкиАТС_Билайн.Видимость             = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн");
	...
	Элементы.МобильнаяТелефонияВажно.Видимость = ЭтоМобильнаяТелефония(ИспользуемаяАТС);
	...
	Элементы.ПользователиТелефонииВнутреннийИдентификаторВАТС.Видимость = (ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн"));
	Элементы.ПользователиТелефонииВнутреннийНомер.РежимВыбораИзСписка = (ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн"));
	...
	Если ЭтоМобильнаяТелефония(ИспользуемаяАТС) Тогда
		Элементы.МаршрутизироватьВходящиеНаОтветственных.Видимость = Ложь;
	Иначе
...
Функция ИнструкцияПодключенияСсылка(АТС)
	...
	СсылкиНаСтатьи.Вставить(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.МТТ"),                   "https://its.1c.ru/db/metod81#content:8111:hdoc");
	СсылкиНаСтатьи.Вставить(ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн"),                "https://its.1c.ru/db/metod81#content:8112:hdoc");
...
&НаСервере
Процедура ОбновитьСписокПользователейТелефонии(ИспользуемаяАТС)
	
	Если ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн")
		И НастройкиАТС_Билайн <> Неопределено
		И ЗначениеЗаполнено(НастройкиАТС_Билайн[0].Значение) Тогда
		
		СписокАбонентовВАТС = ТелефонияБилайн.ЗагрузитьДанныеПоАбонентам();

		Элементы.ГруппаПользователиТелефонии.ПодчиненныеЭлементы.ПользователиТелефонии.ПодчиненныеЭлементы.ПользователиТелефонииВнутреннийНомер.СписокВыбора.Очистить();
		Если СписокАбонентовВАТС <> Неопределено Тогда
			Для каждого Абонент Из СписокАбонентовВАТС Цикл
				Элементы.ГруппаПользователиТелефонии.ПодчиненныеЭлементы.ПользователиТелефонии.ПодчиненныеЭлементы.ПользователиТелефонииВнутреннийНомер.СписокВыбора.Добавить(Строка(Абонент.Представление), Строка(Абонент.Представление));
				СписокАбонентов.Добавить(Строка(Абонент.Представление), Строка(Абонент.Значение));
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ПроверкаПодпискиНаСобытияАТСБилайн()
	
	ТелефонияБилайн.ПроверкаПодпискиНаСобытияАТС();
