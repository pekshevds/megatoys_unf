...
Процедура УстановитьВидимостьНастроекТиповОбмена()
	...
	Если ПереключательНазначениеОбмена = 0 Тогда
		...
		Элементы.НадписьДалееВыгрузкаТоваров.Видимость = Истина;
	...
	Иначе
		...
		Элементы.НадписьДалееВыгрузкаТоваров.Видимость = Ложь;
...
Процедура РедактированиеРасписанияРегламентногоЗадания()
	...
	#Если МобильныйКлиент Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Настройка расписания не доступна в мобильном",,Элементы.НастроитьРасписаниеРегламентногоЗадания);
	#Иначе
...
Процедура ВыполнитьПроверкиПриНажатииГотово(Отказ)
	...
	Если Объект.ИспользоватьРегламентныеЗадания Тогда
		...
		Если Объект.РасписаниеРегламентногоЗадания = Неопределено
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,,,,Отказ);
...
Процедура ИнициализацияОбменаДаннымиНаКлиенте()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ИнициализацияОбменаДаннымиНаКлиентеЗавершение", ЭтотОбъект);
	
	ОткрытьФорму("ПланОбмена.ОбменУправлениеНебольшойФирмойСайт.Форма.ФормаВыполнениеОбмена", ПараметрыФормы, ЭтотОбъект,,,, ОповещениеОЗавершении);
	
КонецПроцедуры

&НаКлиенте
Процедура ИнициализацияОбменаДаннымиНаКлиентеЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда 
		...
		Если Объект.ВыгружатьНаСайт Тогда 
			...
			Если ПустаяСтрока(Объект.АдресСайта) Тогда 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.АдресСайта",, Отказ);
			...
			Если ПустаяСтрока(Объект.ИмяПользователя) Тогда 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.ИмяПользователя",, Отказ);
		...
		Иначе
			...
			Если ПустаяСтрока(Объект.КаталогВыгрузки) Тогда 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.КаталогВыгрузки",, Отказ);
			...
			Если Объект.ОбменЗаказами И ПустаяСтрока(Объект.КаталогЗагрузки) Тогда 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.КаталогЗагрузки",, Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 4 Тогда 	
		...
		Если Объект.ОбменТоварами И СписокВидовЦен.Количество()=0 Тогда
			...
			Сообщение = НСтр("ru = 'Выберите вид цен'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.ВидыЦен",, Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 5 Тогда 
		...
		Если Объект.ОбменЗаказами Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.СпособИдентификацииКонтрагентов) Тогда
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.СпособИдентификацииКонтрагентов",, Отказ);
			...
			ИначеЕсли НеСоздаватьКонтрагентовПриЗагрузкеЗаказа 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение,, "Объект.КонтрагентДляПодстановкиВЗаказы",, Отказ);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.СпособУстановкиДатыОтгрузкиЗаказа = Перечисления.СпособыУстановкиДатыОтгрузкиЗаказа.НеУстанавливать;
	Объект.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.СоздаватьИскатьПоТелефонуEmail;
...
Процедура КомандаПроверитьСоединение(Команда)
	...
	ТекстПредупреждения = "";
	ОбменССайтомСлужебныйВызовСервера.ВыполнитьТестовоеПодключениеКСайту(НастройкиПодключения, ТекстПредупреждения);
