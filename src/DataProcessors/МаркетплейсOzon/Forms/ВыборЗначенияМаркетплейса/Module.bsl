...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НаименованиеАтрибутаМаркетплейса);
...
Процедура ЗаполнитьЗначенияМаркетплейса()
	...
	ДанныеФормы = Новый Структура("ИдентификаторКатегорииМаркетплейса, ИдентификаторАтрибутаМаркетплейса,
			|НаименованиеАтрибутаМаркетплейса, УчетнаяЗапись, УникальныйИдентификатор");
	ЗаполнитьЗначенияСвойств(ДанныеФормы, ЭтотОбъект);

	Если ТипЗнч( ДанныеКэша ) = Тип( "Массив" ) Тогда
		ЭтаФорма.ЗначенияМаркетплейса.Очистить();
		Для Каждого ДанныеКэшаЭлемент Из ДанныеКэша Цикл
			ЗначенияСтрока = ЭтаФорма.ЗначенияМаркетплейса.Добавить();
			ЗаполнитьЗначенияСвойств( ЗначенияСтрока, ДанныеКэшаЭлемент );
		КонецЦикла;
		
		СпозиционироватьсяНаТекущемЗначенииКлиент();
		Элементы.ДекорацияДлительнаяОперация.Видимость = Ложь;
...
&НаКлиенте
Процедура СпозиционироватьсяНаТекущемЗначенииКлиент()
...
Функция ЗаполнитьДоступныеЗначенияАтрибутаКатегорииНаСервере(ДанныеФормы, ВариантПолученияЗначений, ДанныеКэша = Неопределено)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетода,
			ПараметрыАтрибута, ДанныеФормы.УчетнаяЗапись, ВариантПолученияЗначений, ДанныеКэша);
