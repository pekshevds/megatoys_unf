...
Процедура ЗаполнитьДанныеПоКонтрагенту(ТаблицаРасшифровкаПлатежа)
	...
	Иначе
		...
		ИначеЕсли ФункциональнаяОпцияУчетВалютныхОпераций Тогда
			...
			ВыборкаДоговоры.Следующий();
			Если ВыборкаДоговоры.ВалютныйДоговор = Истина Тогда
...
Функция ПолучитьТекстЗапросаДляРаспределенияСуммыПлатежа()
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаДанныеРасчетовОсновная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.Заказ КАК Заказ,
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.Документ КАК Документ,
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.СуммаОстаток КАК СуммаОстаток,
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.СуммаВалОстаток КАК СуммаВалОстаток,
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.Договор КАК Договор,
	|	1 КАК Порядок,
	|	ИСТИНА КАК ЭтоДолг,
	|	ЛОЖЬ КАК ЭтоОстатокПоЗаказу,
	|	ЛОЖЬ КАК ЭтоЗачет,
	...
	|	0 КАК СуммаЗачета,
	|	0 КАК СуммаВал,
	|	РасчетСПокупателямиОстаткиДляОтчетаКомиссионера.СуммаРегОстаток КАК СуммаРегОстаток,
	|	1 КАК НомерОбъединения
	|ПОМЕСТИТЬ ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера
	|ИЗ
	|	РегистрНакопления.РасчетыСПокупателями.Остатки(
	|			&ПериодОстатков,
	...
	|				И Организация = &Организация
	|				И Контрагент = &Контрагент) КАК РасчетСПокупателямиОстаткиДляОтчетаКомиссионера
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РасчетСПокупателямиДляОтчетаКомиссионера.Заказ,
	|	РасчетСПокупателямиДляОтчетаКомиссионера.Документ,
	|	ВЫБОР
	|		КОГДА РасчетСПокупателямиДляОтчетаКомиссионера.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ТОГДА РасчетСПокупателямиДляОтчетаКомиссионера.Сумма
	|		ИНАЧЕ -РасчетСПокупателямиДляОтчетаКомиссионера.Сумма
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА РасчетСПокупателямиДляОтчетаКомиссионера.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ТОГДА РасчетСПокупателямиДляОтчетаКомиссионера.СуммаВал
	|		ИНАЧЕ -РасчетСПокупателямиДляОтчетаКомиссионера.СуммаВал
	|	КОНЕЦ,
	|	РасчетСПокупателямиДляОтчетаКомиссионера.Договор,
	|	1,
	|	ИСТИНА,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	...
	|	ВЫБОР
	|		КОГДА РасчетСПокупателямиДляОтчетаКомиссионера.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ТОГДА РасчетСПокупателямиДляОтчетаКомиссионера.СуммаРег
	|		ИНАЧЕ -РасчетСПокупателямиДляОтчетаКомиссионера.СуммаРег
	|	КОНЕЦ,
	|	2
	|ИЗ
	|	РегистрНакопления.РасчетыСПокупателями КАК РасчетСПокупателямиДляОтчетаКомиссионера
	|ГДЕ
	|	РасчетСПокупателямиДляОтчетаКомиссионера.Организация = &Организация
	|	И РасчетСПокупателямиДляОтчетаКомиссионера.Контрагент = &Контрагент
	|	И РасчетСПокупателямиДляОтчетаКомиссионера.Регистратор = &Ссылка
	|	И &ЭтоРасчетыСПоставщиком
	|	И ТИПЗНАЧЕНИЯ(РасчетСПокупателямиДляОтчетаКомиссионера.Документ) = ТИП(Документ.ОтчетКомиссионера)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаДанныеРасчетовОсновная.Заказ КАК Заказ,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.Документ КАК Документ,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаОстаток КАК СуммаОстаток,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаВалОстаток КАК СуммаВалОстаток,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.Договор КАК Договор,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.Порядок КАК Порядок,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.ЭтоДолг КАК ЭтоДолг,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.ЭтоОстатокПоЗаказу КАК ЭтоОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.ЭтоЗачет КАК ЭтоЗачет,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаОстатокПоЗаказу КАК СуммаОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаВалОстатокПоЗаказу КАК СуммаВалОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаЗачета КАК СуммаЗачета,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаВал КАК СуммаВал,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.СуммаРегОстаток КАК СуммаРегОстаток,
	|	ВременнаяТаблицаДанныеРасчетовОсновная.НомерОбъединения КАК НомерОбъединения
	|ПОМЕСТИТЬ ВременнаяТаблицаДанныеРасчетов
	|ИЗ
	|	ВременнаяТаблицаДанныеРасчетовОсновная КАК ВременнаяТаблицаДанныеРасчетовОсновная
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.Заказ,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.Документ,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаОстаток,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаВалОстаток,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.Договор,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.Порядок,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.ЭтоДолг,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.ЭтоОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.ЭтоЗачет,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаВалОстатокПоЗаказу,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаЗачета,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаВал,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.СуммаРегОстаток,
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера.НомерОбъединения
	|ИЗ
	|	ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера КАК ВременнаяТаблицаДанныеРасчетовДляОтчетаКомиссионера
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если ЭтоРасчетыСПоставщиком Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрНакопления.РасчетыСПоставщиками КАК РасчетСПокупателямиДляОтчетаКомиссионера"
		, "РегистрНакопления.РасчетыСПокупателями КАК РасчетСПокупателямиДляОтчетаКомиссионера");
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрНакопления.РасчетыСПоставщиками.Остатки(
		|			&ПериодОстатков,
		|			&ЭтоРасчетыСПоставщиком
		|				И ТИПЗНАЧЕНИЯ(Документ) = ТИП(Документ.ОтчетКомиссионера)
		|				И Организация = &Организация
...
Функция ПолучитьДанныеДляРаспределенияСуммыПлатежа()
	...
	Запрос.УстановитьПараметр("ЭтоРасчетыСПоставщиком", ЭтоРасчетыСПоставщиком);
	...
	Иначе
		...
		СтруктураДляВозврата.Вставить("ЗапросОстаткиВсеДанные", МРезультатов[4]);
		СтруктураДляВозврата.Вставить("КоличествоГруппировок", КоличествоГруппировок);
		СтруктураДляВозврата.Вставить("Группировки", Группировки);
		СтруктураДляВозврата.Вставить("Договоры", МРезультатов[5]);
		СтруктураДляВозврата.Вставить("ДоговорыБезОстатковПоВзаиморасчетам", МРезультатов[7]);
		СтруктураДляВозврата.Вставить("ДанныеОСчетахНаОплату", МРезультатов[9]);
	...
	Если НЕ ПоследняяГруппировка ИЛИ ЗначениеЗаполнено(ТекущийОбъект) Тогда
		...
		ИначеЕсли ПолеГруппировки = "Документ" Тогда
			...
			Если ЗначениеЗаполнено(ТекущийОбъект) Тогда
				...
				ДокументПозжеТекущего = ТекущаяВыборка.ДокументПозжеТекущего = Истина;
				...
				Если ДокументПозжеТекущего Тогда
				...
				ДокументНеПроведен = ТекущаяВыборка.ДокументНеПроведен = Истина;
				
				ТекущаяСтрокаДерева.ДокументНеОбрабатывается = (ДокументПозжеТекущего
					...
					ИЛИ (ДокументНеПроведен И ЭтоЗачетПредоплаты И ДокументПозжеТекущего);
...
Процедура ПрименитьРаспределениеСуммыПлатежа(Команда)
	...
	Результат.Вставить("ДеревоРаспределенияСуммыПлатежа", ДеревоРаспределенияСуммыПлатежа);
	Результат.Вставить("ДоговорПоУмолчанию", ДоговорПоУмолчанию);
	Результат.Вставить("СпособЗачетаПоУмолчанию", СпособЗачетаПоУмолчанию);
	Результат.Вставить("ВестиРасчетыВЦеломПоКонтрагенту", ВестиРасчетыВЦеломПоКонтрагенту);
	Результат.Вставить("ВестиРасчетыТолькоПоДоговорам", ВестиРасчетыТолькоПоДоговорам);
	Результат.Вставить("СтавкаНДСПоУмолчанию", СтавкаНДСПоУмолчанию);
	Закрыть(Результат);
...
Процедура ДеревоРаспределенияСуммыПлатежаСчетНаОплатуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ПараметрыОтбораСчетаНаОплату = Новый Структура("Контрагент", Контрагент);
		ПараметрыВыбораСчетаНаОплату = Новый Структура();
		ПараметрыВыбораСчетаНаОплату.Вставить("Отбор", ПараметрыОтбораСчетаНаОплату);
		ПараметрыВыбораСчетаНаОплату.Вставить("РежимВыбора", Истина);
