...
Функция ДанныеПоЗакрытиюПериодов(Организация, МесяцРасчета)
	...
	Запрос.УстановитьПараметр("РасчетФинансовогоРезультата", ПолучитьФункциональнуюОпцию("ИспользоватьРасчетФинансовогоРезультата"));
	Запрос.УстановитьПараметр("ИспользоватьРаспределениеЗатрат", ПолучитьФункциональнуюОпцию("ИспользоватьРаспределениеЗатрат"));
...
Функция ТекстЗапросаОперацииЗакрытияМесяца()
	...
	|	ЛОЖЬ КАК ЗачетАвансаПоЕдиномуНалоговомуСчету
	...
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	&ИспользоватьРаспределениеЗатрат,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	...
	|
	|ВЫБРАТЬ
	|	РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты.Период,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрНакопления.РасчетыПоНалогамНаЕдиномНалоговомСчете.Обороты(&НижняяГраница, &ВерхняяГраница, Месяц, Организация = &Организация) КАК РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЛОЖЬ КАК НачислениеАмортизации,
	|	МАКСИМУМ(ЗакрытиеМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчету) КАК ЗачетАвансаПоЕдиномуНалоговомуСчету
	...
	|	МАКСИМУМ(ВложенныйЗапрос.ЗачетАвансаПоЕдиномуНалоговомуСчету) КАК ЗачетАвансаПоЕдиномуНалоговомуСчетуТребуется,
	...
	|	МАКСИМУМ(ВложенныйЗапрос.ЗачетАвансаПоЕдиномуНалоговомуСчетуВыполнен) КАК ЗачетАвансаПоЕдиномуНалоговомуСчетуВыполнен
	...
	|		ТребуемыеОперации.ЗачетАвансаПоЕдиномуНалоговомуСчету КАК ЗачетАвансаПоЕдиномуНалоговомуСчету,
	...
	|		ЛОЖЬ КАК ЗачетАвансаПоЕдиномуНалоговомуСчетуВыполнен
	|	ИЗ
	...
	|		ВыполненныеОперации.ЗачетАвансаПоЕдиномуНалоговомуСчету
	|	ИЗ
...
Функция ТекстЗапросаПолучениеДанных()
	...
	|	ИСТИНА КАК ПроверкаЕдиницИзмеренияТребуется,
	...
	|	МАКСИМУМ(ВложенныйЗапрос.ЗачетАвансаПоЕдиномуНалоговомуСчетуТребуется) КАК ЗачетАвансаПоЕдиномуНалоговомуСчетуТребуется,
	...
	|	ЛОЖЬ КАК ПроверкаЕдиницИзмеренияВыполнен,
	|	МАКСИМУМ(ВложенныйЗапрос.НачислениеАмортизацииВыполнен) КАК НачислениеАмортизацииВыполнен,
	|	МАКСИМУМ(ВложенныйЗапрос.РасчетПрямыхЗатратВыполнен) КАК РасчетПрямыхЗатратВыполнен,
	|	МАКСИМУМ(ВложенныйЗапрос.РаспределениеЗатратВыполнен) КАК РаспределениеЗатратВыполнен,
	|	МАКСИМУМ(ВложенныйЗапрос.РасчетФактическойСебестоимостиВыполнен) КАК РасчетФактическойСебестоимостиВыполнен,
	...
	|		ЛОЖЬ КАК ЗачетАвансаПоЕдиномуНалоговомуСчетуТребуется,
	|		ЛОЖЬ КАК РасчетПрямыхЗатратВыполнен,
	|		ЛОЖЬ КАК РасчетФактическойСебестоимостиВыполнен,
	|		ЛОЖЬ КАК РаспределениеЗатратВыполнен,
	|		ЛОЖЬ КАК РасчетФинансовогоРезультатаВыполнен,
	...
	|		ОперацииЗакрытияМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчетуТребуется,
	...
	|		ОперацииЗакрытияМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчетуВыполнен
	|	ИЗ
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДокументЧекККМ.КассоваяСмена.СтатусКассовойСмены, ДокументЧекККМВозврат.КассоваяСмена.СтатусКассовойСмены) <> ЗНАЧЕНИЕ(Перечисление.СтатусыОтчетаОРозничныхПродажах.Открыта)
	|			ТОГДА ЕСТЬNULL(ДокументЧекККМ.Ссылка, ДокументЧекККМВозврат.Ссылка)
	|		ИНАЧЕ ЕСТЬNULL(ДокументЧекККМ.КассоваяСмена, ДокументЧекККМВозврат.КассоваяСмена)
	|	КОНЕЦ КАК Документ,
	|	ЕСТЬNULL(ДокументЧекККМ.КассаККМ, ДокументЧекККМВозврат.КассаККМ) КАК КассаККМ,
	|	ЕСТЬNULL(ДокументЧекККМ.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)) КАК Статус,
	|	ЕСТЬNULL(ДокументЧекККМ.КассоваяСмена.СтатусКассовойСмены, ДокументЧекККМВозврат.КассоваяСмена.СтатусКассовойСмены) КАК СтатусКассовойСмены
	|ИЗ
	...
	|	ЕСТЬNULL(ДокументЧекККМ.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)),
	|	ЕСТЬNULL(ДокументЧекККМ.КассоваяСмена.СтатусКассовойСмены, ДокументЧекККМВозврат.КассоваяСмена.СтатусКассовойСмены)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Месяц,
	|	День
...
Процедура ДобавитьСекции(РезультатЗапроса, РезультатРасчета, НижняяГраница, ВерхняяГраница)
	...
	Пока ВыборкаМесяцы.Следующий() Цикл
		...
		ДобавитьСтрокуСекции(ПериодДетальногоАнализа, ВыборкаМесяцы, СекцияЗачетАвансаПоЕдиномуНалоговомуСчету(), СтрокаМесяца);
	КонецЦикла;
...
Процедура ВыполнитьПроверкуИнтеркампани(РезультатЗапроса, РезультатРасчета, Компания, ТаблицаМесяцы)
	...
	Для каждого СтрокаМесяца Из ТаблицаМесяцы Цикл
		ДатаПроверки = КонецМесяца(СтрокаМесяца.Месяц);
		ТребуетсяЗакрытие = Документы.ПередачаТоваровМеждуОрганизациями.НужноЗаполнятьДокументыПередачТоваровМеждуОрганизациями(Компания, ДатаПроверки);
...
Процедура ВыполнитьПроверкуКассовыхСмен(РезультатЗапроса, РезультатРасчета, НижняяГраница, ВерхняяГраница)
	...
	Пока ВыборкаМесяцы.Следующий() Цикл
		...
		НезакрытыеСмены = Новый Массив;
		НезакрытыеСменыДни = Новый Массив;
		СменыСЧекамиВРезерве = Новый Массив;
		СменыСЧекамиВРезервеДни = Новый Массив;
		ПрочиеСменыСДвижениями = Новый Массив;
		ПрочиеСменыСДвижениямиДни = Новый Массив;
		ВыборкаДни = ВыборкаМесяцы.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаДни.Следующий() Цикл
			ДеньПредставление = Формат(ВыборкаДни.День, "ДЛФ=D");
			ВыборкаДетали = ВыборкаДни.Выбрать();
			Пока ВыборкаДетали.Следующий() Цикл
				ДанныеСмены = Новый Структура;
				ДанныеСмены.Вставить("Документ", ВыборкаДетали.Документ);
				ДанныеСмены.Вставить("КассаККМ", ВыборкаДетали.КассаККМ);
				Если ВыборкаДетали.СтатусКассовойСмены = Перечисления.СтатусыОтчетаОРозничныхПродажах.Открыта Тогда
					НезакрытыеСмены.Добавить(ДанныеСмены);
					Если НезакрытыеСменыДни.Найти(ДеньПредставление) = Неопределено Тогда
						НезакрытыеСменыДни.Добавить(ДеньПредставление);
					КонецЕсли;
				ИначеЕсли ВыборкаДетали.Статус = Перечисления.СтатусыЧековККМ.ТоварЗарезервирован Тогда
					СменыСЧекамиВРезерве.Добавить(ДанныеСмены);
					Если СменыСЧекамиВРезервеДни.Найти(ДеньПредставление) = Неопределено Тогда
						СменыСЧекамиВРезервеДни.Добавить(ДеньПредставление);
					КонецЕсли;
				Иначе
					ПрочиеСменыСДвижениями.Добавить(ДанныеСмены);
					Если ПрочиеСменыСДвижениямиДни.Найти(ДеньПредставление) = Неопределено Тогда
						ПрочиеСменыСДвижениямиДни.Добавить(ДеньПредставление);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла; 
		КонецЦикла;
		Если НезакрытыеСменыДни.Количество() > 0 Тогда
			СтрокаОшибка = СтрокаСекция.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаОшибка, СтрокаСекция);
			Подробности = Новый Структура;
			Подробности.Вставить("Тип", ТипСообщенияОткрытыеКассовыеСмены());
			Подробности.Вставить("Документы", НезакрытыеСмены);
			СтрокаОшибка.Подробности = Подробности;
			...
			СтрокаОшибка.ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаМесяц, СтрСоединить(НезакрытыеСменыДни, ", "));
		КонецЕсли;
		Если СменыСЧекамиВРезервеДни.Количество() > 0 Тогда
			СтрокаОшибка = СтрокаСекция.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаОшибка, СтрокаСекция);
			Подробности = Новый Структура;
			Подробности.Вставить("Тип", ТипСообщенияЧекиВСостоянииРезерва());
			Подробности.Вставить("Документы", СменыСЧекамиВРезерве);
			СтрокаОшибка.Подробности = Подробности;
			ШаблонСообщения = НСтр("ru = 'В %1 обнаружены чеки в состоянии ""Товар зарезервирован"" за даты: %2. Для корректного расчета фактической себестоимости следует отменить резерв.
                                    |Также в настройках программы можно включить режим автоматического удаления непробитых чеков при закрытии смены.'");
			СтрокаОшибка.ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаМесяц, СтрСоединить(СменыСЧекамиВРезервеДни, ", "));
		КонецЕсли;
		Если ПрочиеСменыСДвижениямиДни.Количество() > 0 Тогда
			СтрокаОшибка = СтрокаСекция.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаОшибка, СтрокаСекция);
			Подробности = Новый Структура;
			Подробности.Вставить("Тип", ТипСообщенияЧекиСДвижениями());
			Подробности.Вставить("Документы", ПрочиеСменыСДвижениями);
			СтрокаОшибка.Подробности = Подробности;
			ШаблонСообщения = НСтр("ru = 'В %1 обнаружены чеки с незавершенной архивацией за даты: %2. Для корректного расчета фактической себестоимости завершить архивацию.'");
			СтрокаОшибка.ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаМесяц, СтрСоединить(ПрочиеСменыСДвижениямиДни, ", "));
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыполнитьОбщиеПроверки(РезультатРасчета, Организация, НижняяГраница, ВерхняяГраница, Операции)
	...
	КонецЕсли;
	Если Операции.Найти(СекцияПроверкаЕдиницИзмерения()) <> Неопределено Тогда
		ПроверитьЕдиницыИзмерения(РезультатРасчета, Организация, НижняяГраница, ВерхняяГраница);
	КонецЕсли;
...
Процедура ПроверкаОрганизацийИДоговоровВДокументах(РезультатРасчета, Организация, НижняяГраница, ВерхняяГраница)
	...
	Если Константы.УчетПоКомпании.Получить() Тогда
		// При использовании учета по компании организация в договорах и документах не контролируется 
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЕдиницыИзмерения(РезультатРасчета, Организация, НижняяГраница, ВерхняяГраница)
	...
	|	ВЫРАЗИТЬ(ДокументЗапасы.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения) КАК ЕдиницаИзмерения,
	|	ДокументЗапасы.Количество КАК Количество,
	|	ДокументЗапасы.Ссылка КАК Документ,
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ) КАК Месяц
	|ПОМЕСТИТЬ ДвиженияНеВОсновнойЕдинице
	|ИЗ
	|	Документ.ВводНачальныхОстатков.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ДокументЗапасы.Номенклатура,
	|	ВЫРАЗИТЬ(ДокументЗапасы.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения),
	|	ДокументЗапасы.Количество,
	|	ДокументЗапасы.Ссылка,
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.КомплектацияЗапасов.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	ВЫРАЗИТЬ(Документ.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения),
	|	Документ.Количество,
	|	Документ.Ссылка,
	...
	|ГДЕ
	|	Документ.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И Документ.Проведен
	|	И Документ.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.ОприходованиеЗапасов.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.ПередачаТоваровМеждуОрганизациями.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.ПеремещениеЗапасов.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.ПриходнаяНакладная.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.РасходнаяНакладная.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.СборкаЗапасов.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.СборкаЗапасов.Продукция КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|	НАЧАЛОПЕРИОДА(ДокументЗапасы.Ссылка.Дата, МЕСЯЦ)
	|ИЗ
	|	Документ.СписаниеЗапасов.Запасы КАК ДокументЗапасы
	|ГДЕ
	|	ДокументЗапасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|	И ДокументЗапасы.Ссылка.Проведен
	|	И ДокументЗапасы.Ссылка.Дата МЕЖДУ &НижняяГраница И &ВерхняяГраница
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДвиженияНеВОсновнойЕдинице.Номенклатура КАК Номенклатура,
	|	ДвиженияНеВОсновнойЕдинице.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	СУММА(ДвиженияНеВОсновнойЕдинице.Количество) КАК Количество,
	|	ДвиженияНеВОсновнойЕдинице.Документ КАК Документ,
	|	ДвиженияНеВОсновнойЕдинице.Месяц КАК Месяц,
	|	ВЫБОР
	|		КОГДА СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент > (ВЫРАЗИТЬ(СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3)))
	|			ТОГДА СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент - (ВЫРАЗИТЬ(СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3)))
	|		ИНАЧЕ (ВЫРАЗИТЬ(СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3))) - СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент
	|	КОНЕЦ / (СУММА(ДвиженияНеВОсновнойЕдинице.Количество) * ЕдиницыИзмерения.Коэффициент) * 100 КАК Отклонение
	|ИЗ
	|	ДвиженияНеВОсновнойЕдинице КАК ДвиженияНеВОсновнойЕдинице
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	|		ПО ДвиженияНеВОсновнойЕдинице.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент > (ВЫРАЗИТЬ(ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3)))
	|				ТОГДА ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент - (ВЫРАЗИТЬ(ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3)))
	|			ИНАЧЕ (ВЫРАЗИТЬ(ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент КАК ЧИСЛО(15, 3))) - ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент
	|		КОНЕЦ / (ДвиженияНеВОсновнойЕдинице.Количество * ЕдиницыИзмерения.Коэффициент) * 100 >= 1
	|
	|СГРУППИРОВАТЬ ПО
	|	ДвиженияНеВОсновнойЕдинице.ЕдиницаИзмерения,
	|	ЕдиницыИзмерения.Коэффициент,
	|	ДвиженияНеВОсновнойЕдинице.Номенклатура,
	|	ДвиженияНеВОсновнойЕдинице.Месяц,
	|	ДвиженияНеВОсновнойЕдинице.Документ
	|
	|УПОРЯДОЧИТЬ ПО
	|	Месяц,
	|	Номенклатура,
	...
	ВыборкаМесяц = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаМесяц.Следующий() Цикл
		СтрокаСекция = СтрокаРезультата(РезультатРасчета, ВыборкаМесяц.Месяц, СекцияПроверкаЕдиницИзмерения(), Истина);
		СтрокаОшибка = СтрокаСекция.Строки.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаОшибка, СтрокаСекция);
		СтрокаОшибка.ТекстСообщения = 
			НСтр("ru = 'Были обнаружены движения запасов со значительным влиянием округления (>1%). Рекомендуется пересмотреть выбор базовой единицы в таких случаях.'");
		МассивНоменклатуры = Новый Массив;
		ВыборкаНоменклатура = ВыборкаМесяц.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаНоменклатура.Следующий() Цикл
			МассивДокументы = Новый Массив;
			ВыборкаДетали = ВыборкаНоменклатура.Выбрать();
			Пока ВыборкаДетали.Следующий() Цикл
				ОписаниеДокумента = Новый Структура;
				ОписаниеДокумента.Вставить("Документ", ВыборкаДетали.Документ);
				ОписаниеДокумента.Вставить("ЕдиницаИзмерения", ВыборкаДетали.ЕдиницаИзмерения);
				ОписаниеДокумента.Вставить("Количество", ВыборкаДетали.Количество);
				ОписаниеДокумента.Вставить("Отклонение", ВыборкаДетали.Отклонение);
				МассивДокументы.Добавить(ОписаниеДокумента);	
			КонецЦикла; 
			СтруктураОписания = Новый Структура;
			СтруктураОписания.Вставить("Документы", МассивДокументы);
			СтруктураОписания.Вставить("Номенклатура", ВыборкаНоменклатура.Номенклатура);
			МассивНоменклатуры.Добавить(СтруктураОписания);
		КонецЦикла; 
		СтруктураПодробностей = Новый Структура;
		СтруктураПодробностей.Вставить("Тип", ТипСообщенияПриблизительноеОкруглениеКоличества());
		СтруктураПодробностей.Вставить("Номенклатура", МассивНоменклатуры);
		СтрокаОшибка.Подробности = СтруктураПодробностей;
	КонецЦикла; 
...
Функция СформироватьТекстЗапросаПоДокументам(СтруктураПравил)
	...
	Для каждого КлючИЗначение Из СтруктураПравил Цикл
		...
		Если Метаданные.Документы[КлючИЗначение.Ключ].Реквизиты.Найти("Организация") <> Неопределено
			И НЕ Константы.УчетПоКомпании.Получить() Тогда
...
Процедура ПроверитьСоответствияВДокументе(ИмяДокумента, Выборка, ТаблицаРасположенияДоговоров, ТаблицаПравил, РезультатРасчета, КонтролироватьСоответствиеОрганизацийВДоговорах)
	...
	Если РасположениеДоговора.ТипРеквизита = "Реквизит" Тогда
		...
		Если КонтролироватьСоответствиеОрганизацийВДоговорах 
			...
			ВладелецДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "Владелец", Истина);
			Если ТипЗнч(ВладелецДоговора) = Тип("СправочникСсылка.Контрагенты") Тогда
				ВестиРасчетыПоДоговорам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВладелецДоговора, "ВестиРасчетыПоДоговорам", Истина);
