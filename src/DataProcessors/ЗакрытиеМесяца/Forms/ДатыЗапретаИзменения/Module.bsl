...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПереноситьДатуЗапретаРедактирования = (Константы.ПереноситьДатуЗапретаРедактирования.Получить() = Перечисления.ДаНет.Да);
	...
	ТипыОбъектовРазделов = СвойстваРазделов.Разделы;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекстВопроса) Тогда
		...
		ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(
			ЭтотОбъект, Отказ, Ложь, ТекстВопроса, "ЗакрытьФормуБезПодтверждения");
	КонецЕсли;
...
&НаКлиенте
Процедура ПереноситьДатуЗапретаРедактированияПриИзменении(Элемент)
	ПереноситьДатуЗапретаРедактированияПриИзмененииНаСервере(ПереноситьДатуЗапретаРедактирования);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПереноситьДатуЗапретаРедактированияПриИзмененииНаСервере(ПереноситьДатуЗапретаРедактирования)
	Если ПереноситьДатуЗапретаРедактирования Тогда
...
Процедура ДатыЗапретаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущийПользователь = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияСначалаВыберитеПользователя());
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ДатыЗапретаПередНачаломИзменения(Элемент, Отказ)
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияСначалаВыберитеОбъект());
...
Процедура УстановитьВидимостьЭлементовДляМобильногоКлиента()
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.УстановкаДатыЗапрета.ЦветФона = Элементы.ДатаЗапрета.ЦветФона;
		Элементы.УстановкаДатыЗапрета.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
		Элементы.СпособУказанияДатыЗапрета.ЦветФона = Элементы.ДатаЗапрета.ЦветФона;
		Элементы.СпособУказанияДатыЗапрета.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
		Элементы.ОписаниеДатыЗапрета.ЦветФона = Элементы.ДатаЗапрета.ЦветФона;
	КонецЕсли;
...
Процедура ОбщаяДатаЗапретаСОписаниемПриИзменении(Знач Контекст, РассчитатьДатуЗапрета = Истина);
	...
	Иначе
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтрШаблон(НСтр("ru = 'Запрещено изменение данных по %1'"),
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ru = 'Удалить все даты запрета, кроме установленных %1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ru = 'Значение %1 не изменяется.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ru = 'Комментарий %1 не изменяется.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ru = 'Удалить даты запрета %1?'"),
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтрШаблон(НСтр("ru = '%1 даты запрета не установлены.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтрШаблон(НСтр("ru = '%1 разделы всегда показываются.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтрШаблон(НСтр("ru = '%1 <Общая дата> всегда показывается.'"),
		ЗначениеДляВсехПользователей);
