...
Процедура СоздатьСайтПродолжить(ВхПараметры, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	ВхПараметры.Вставить("ОбменВыполнен", Ложь);
	ПоместитьВоВременноеХранилище(ВхПараметры, ФоновоеЗаданиеАдресХранилища);
...
Процедура СоздатьИнтеграцию(ВхПараметры, АдресСайта, IDсайта, Пароль = "")
	
	НоваяИнтеграция = Справочники.НастройкиИнтеграцииСИнтернетМагазином.СоздатьЭлемент();
	НоваяИнтеграция.Заполнить(ПредопределенноеЗначение("Перечисление.ТипыИсточниковИнтеграции.Сайт"));
	НоваяИнтеграция.АдресСайта = СтрШаблон("https://%1/admin/exchange/autoimport/", АдресСайта);
	НоваяИнтеграция.ВыгружатьНаСайт = Истина;
	
	Если ВхПараметры.ТипСайта=4 Тогда
		
		НоваяИнтеграция.ВидыЦен.Очистить();
		НовыйВидЦен = НоваяИнтеграция.ВидыЦен.Добавить();
		НовыйВидЦен.ВидЦен = ВхПараметры.ВидЦен;
	
		НоваяИнтеграция.ВыгружатьКартинки = Истина;
		НоваяИнтеграция.ВыгружатьОстаткиПоСкладам = ВхПараметры.ВыгружатьОстатки;
		НоваяИнтеграция.ОбменЗаказами = Истина;
		НоваяИнтеграция.ОрганизацияДляПодстановкиВЗаказы = ВхПараметры.ОсновнаяОрганизация;
		НоваяИнтеграция.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.СоздаватьИскатьПоТелефонуEmail;
		НоваяИнтеграция.ОтборГруппыКатегорииНоменклатуры = ВхПараметры.ОтборКатегорииГруппыПереключатель;
		...
		Если ТаблицаКаталогов.Количество() > 0 Тогда
			НоваяИнтеграция.ОбменТоварами = Истина;
			НоваяИнтеграция.СохраненнаяТаблицаКаталогов = Новый ХранилищеЗначения(ТаблицаКаталогов);
		КонецЕсли;
	КонецЕсли;
	НоваяИнтеграция.Логин = ВхПараметры.ИмяСайта;
	НоваяИнтеграция.Пароль = Пароль;
	НоваяИнтеграция.ВыполнятьПолнуюВыгрузкуПринудительно = НоваяИнтеграция.ЭтоНовый();
	
	НоваяИнтеграция.ПротоколОбменаCMS = ПредопределенноеЗначение("Перечисление.ПротоколыОбменаCMS.UMI");
	НоваяИнтеграция.ТипИсточникаИнтеграции = ПредопределенноеЗначение("Перечисление.ТипыИсточниковИнтеграции.Сайт");
	НоваяИнтеграция.ТипСайта = НоваяИнтеграция.ПротоколОбменаCMS;
	
	НоваяИнтеграция.ПереходАдресСайта = ВхПараметры.АдресСайта;
	НоваяИнтеграция.ПереходURLАдминЗоны = ВхПараметры.СсылкаАдминЗоны;
	НоваяИнтеграция.ИдентификаторСайта1СUMI = IDсайта;
	НоваяИнтеграция.ИспользоватьОповещения = Истина;
	НоваяИнтеграция.ОтправлятьСкрытуюКопиюВПоддержку = Истина;
	НоваяИнтеграция.УчетнаяЗаписьДляОповещения = ПредопределенноеЗначение("Справочник.УчетныеЗаписиЭлектроннойПочты.СистемнаяУчетнаяЗаписьЭлектроннойПочты");
	
	НоваяИнтеграция.ПодпискиНаСобытие.Очистить();
	НоваяПодпискаНаСобытие = НоваяИнтеграция.ПодпискиНаСобытие.Добавить();
	НоваяПодпискаНаСобытие.ЭлектроннаяПочта = ВхПараметры.EmailРегистрацииСайта;
	НоваяПодпискаНаСобытие.Ошибка = Истина;
	
	Если ВхПараметры.ЗаписьНаУслуги Тогда
		
		НоваяИнтеграция.ИдентификаторКаталогаУслуг = Новый УникальныйИдентификатор;
		
		НоваяИнтеграция.ОбменЗаписьНаУслуги = Истина;
		НоваяИнтеграция.ВидЦенУслуг = ВхПараметры.ВидЦен;
		НоваяИнтеграция.ВыгружатьДнейГрафика = 14;
	КонецЕсли;
	
	НоваяИнтеграция.ДобавитьНастройкиСоздания();
	
	Попытка
		
		НоваяИнтеграция.Записать();
		
		ВхПараметры.Вставить("НастройкаИнтеграции", НоваяИнтеграция.Ссылка);
	...
	|ГДЕ
	|	НастройкиЗаписьНаУслугиИнтернетМагазина.НастройкаИнтеграции = ЗНАЧЕНИЕ(Справочник.НастройкиИнтеграцииСИнтернетМагазином.ПустаяСсылка)
	...
	Пока Выборка.Следующий() Цикл
		...
		МЗ.Прочитать();
		МЗ.НастройкаИнтеграции = НоваяИнтеграция.Ссылка;
...
Процедура ЗаписатьКонтактнуюИнформацию(АдресСайта, ПарольСайта, ОсновнаяОрганизация, ТипСайта)
	...
	ФактАдресОрганизации = ПечатьДокументовУНФ.КонтактнаяИнформация(ОсновнаяОрганизация, 
	Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
	
	ТелефонОрганизации = ПечатьДокументовУНФ.КонтактнаяИнформация(ОсновнаяОрганизация, 
	Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации);
	
	ФаксОрганизации = ПечатьДокументовУНФ.КонтактнаяИнформация(ОсновнаяОрганизация, 
	Справочники.ВидыКонтактнойИнформации.ФаксОрганизации);
	
	EmailОрганизации = ПечатьДокументовУНФ.КонтактнаяИнформация(ОсновнаяОрганизация, 
	Справочники.ВидыКонтактнойИнформации.EmailОрганизации);
	
	SkypeОрганизации = ПечатьДокументовУНФ.КонтактнаяИнформация(ОсновнаяОрганизация, 
...
Процедура ДобавитьПараметрВТелоЗапроса(ТекстОтправки, ИмяПараметра, ЗначениеПараметра, Boundary)
	...
	Если ТекстОтправки="" Тогда
		ДобавитьСтрокуСРазделителем(ТекстОтправки, "--" + Boundary);
	КонецЕсли;
