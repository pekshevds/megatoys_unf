...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(КонтекстПечати, Параметры.КонтекстПечати, "Организация, Контрагент,
		|ПодписьОтветственного, ПредставительКонтрагента");
...
&НаКлиенте
Процедура ПодписьОтветственногоОрганизацииПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПредставительКонтрагентаПриИзменении(Элемент)
	
	ЗафиксироватьИзменениеЗначенияРеквизита("ПредставительКонтрагента");
...
Процедура ВосстановитьПодписиПоУмолчанию(Команда)
	...
	ПредыдущиеЗначения = Новый Структура("ПодписьОтветственного, ПредставительКонтрагента");
...
&НаКлиенте
Процедура СохранитьПодписьОтветственного(Команда)
	
	ЗаписатьНастройкуПользователя("ПодписьГлавногоБухгалтера", КонтекстПечати.ПодписьОтветственного);
...
Процедура ПолучитьПодписиПоУмолчаниюНаСервере()
	...
	ДокументОбъект = ДанныеФормыВЗначение(КонтекстПечати, Тип("ДокументОбъект.СверкаВзаиморасчетов"));
	Обработки.РеквизитыПечати.ПодписиПоУмолчанию(ДокументОбъект);
	
	ПодписьИзНастроек = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПодписьГлавногоБухгалтера", КонтекстПечати.Организация);
	Если ЗначениеЗаполнено(ПодписьИзНастроек) Тогда
		ДокументОбъект.ПодписьОтветственного = ПодписьИзНастроек;
	КонецЕсли;
	
	ЗначениеВДанныеФормы(ДокументОбъект, КонтекстПечати);
	
	//КонтекстПечати.ПредставительКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтекстПечати.Контрагент, "КонтактноеЛицоПодписант");
...
Процедура ЗаполнитьПодсказкиПодписей() 
	...
	Если КонтекстПечати.ПодписьОтветственного = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтекстПечати.Организация, 
		"ПодписьГлавногоБухгалтера") Тогда
		Элементы.ПодписьОтветственного.Подсказка = НСтр("ru='Подпись из реквизитов организации'");		
	Иначе
		Элементы.ПодписьОтветственного.Подсказка = НСтр("ru='Подпись из настроек пользователя'");
	КонецЕсли;
...
Процедура УстановитьКартинкиКнопок()
	...
	Элементы.СохранитьПодписьОтветственного.Картинка = КартинкаСохранения;
