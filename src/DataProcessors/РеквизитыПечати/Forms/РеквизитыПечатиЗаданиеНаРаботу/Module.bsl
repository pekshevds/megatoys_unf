...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(КонтекстПечати, Параметры.КонтекстПечати, "СтруктурнаяЕдиница, ПодписьМОЛ, Организация");
...
Процедура ВосстановитьПодписиПоУмолчанию(Команда)
	...
	ПредыдущиеЗначения = Новый Структура("ПодписьМОЛ");
...
Процедура ПолучитьПодписиПоУмолчаниюНаСервере()
	...
	ДокументОбъект = ДанныеФормыВЗначение(КонтекстПечати, Тип("ДокументОбъект.ЗаданиеНаРаботу"));
...
Процедура ЗаполнитьПодсказкиПодписей() 
	...
	Если КонтекстПечати.ПодписьМОЛ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтекстПечати.СтруктурнаяЕдиница,
		"ПодписьМОЛ") Тогда
		Элементы.ПодписьМОЛ.Подсказка = НСтр("ru='Подпись из реквизитов организации'");		
	Иначе
		Элементы.ПодписьМОЛ.Подсказка = НСтр("ru='Подпись из настроек пользователя'");
	КонецЕсли;
...
Процедура УстановитьКартинкиКнопок()
	...
	Элементы.СохранитьПодпись.Картинка = КартинкаСохранения;
