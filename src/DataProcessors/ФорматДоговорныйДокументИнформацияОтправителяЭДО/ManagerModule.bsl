	...
	Для Каждого СвойствоОбъекта Из СвойстваОбъектаXDTO Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СокрЛП(ЗначениеСвойстваОбъекта)) Тогда
				...
				НоваяСтрока.СтильФрагмента = НоваяСтрока.СтильФрагмента + СвойствоОбъекта.ЛокальноеИмя + ":"
...
Процедура ЗаменитьОписаниеСообщенияПодготовленнымиДанными(РезультатПодготовкиДанных, ПомещенныйФайл) Экспорт
	...
	ПодготовленныеПараметры = 
		ПолучитьИзВременногоХранилища(РезультатПодготовкиДанных.АдресОписанияСообщения);
	...
	РезультатПодготовкиДанных.АдресОписанияСообщения = ПоместитьВоВременноеХранилище(
		ПодготовленныеПараметры, Новый УникальныйИдентификатор());
