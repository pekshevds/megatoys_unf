...
Функция СформироватьДерево(ТаблицаДанных, Параметры)
	...
	ВключитьПорядокПоступленияЗапасов = Ложь;	
	Для каждого Измерение Из Построитель.Измерения Цикл
		Если Измерение.Имя=ИмяГруппировкиПериод() Тогда
			...
			ВключитьПорядокПоступленияЗапасов = Истина;
	...
	Если Параметры.ОбеспечениеЗаСчетОстатка Тогда
		...
		ОтборПотребность = Построитель.Отбор.Добавить("Потребность");
		ОтборПотребность.ВидСравнения 	= ВидСравнения.Равно;
		ОтборПотребность.Использование 	= Истина;
		ОтборПотребность.Значение 		= 0; 
	...
	СортироватьДеревоРекурсивно(Дерево);
	...
	Если Параметры.Группировать = ГруппировкаНоменклатура() Тогда
		...
		Пока НачалоДня(ТекущийПериод) <= НачалоДня(Параметры.Период.ДатаОкончания) Цикл
			...
			Дерево.Колонки.Добавить(ИмяКолонки, ТипКолонки);
		...
		РассчитатьИтогиНоменклатураРекурсивно(Параметры, Дерево.Строки, Дерево.Колонки);
	ИначеЕсли Параметры.Группировать = ГруппировкаНоменклатураДень() Тогда 
...
Процедура СортироватьДеревоРекурсивно(Корень, ТаблицаЗаказа = Неопределено, КлючПоиска = Неопределено)
	...
	Если ИмяГруппировки = ИмяГруппировкиЗаказ() ИЛИ ИмяГруппировки = ИмяГруппировкиЗаказРекомендации() Тогда
		...
		Для каждого Строка Из Корень.Строки Цикл
			...
			СортироватьДеревоРекурсивно(Строка, Запасы, КлючПоиска);
	...
	Иначе
		...
		Для каждого Строка Из Корень.Строки Цикл
			СортироватьДеревоРекурсивно(Строка);
		КонецЦикла; 
...
Функция РассчитатьИтогиНоменклатураРекурсивно(Параметры, Строки, Колонки)
	...
	Иначе 
		Для каждого Строка Из Строки Цикл
			Если Строка.Строки.Количество() > 0 Тогда
				ИтогиВложенных = РассчитатьИтогиНоменклатураРекурсивно(Параметры, Строка.Строки, Колонки);
...
Процедура ЗаполнитьПараметрыПоступления(Дерево, Параметры)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыПополненияЗапасов.Закупка)
	|	КОНЕЦ,
	|	НоменклатураПоставщиков.СрокПополнения,
	...
	|	НоменклатураПоставщиков.Остаток,
	|	ВЫБОР
	|		КОГДА НоменклатураПоставщиков.Владелец = ТаблицаНоменклатуры.Номенклатура.Поставщик
	|				И ТаблицаНоменклатуры.Номенклатура.СпособПополнения В (ЗНАЧЕНИЕ(Перечисление.СпособыПополненияЗапасов.Переработка), ЗНАЧЕНИЕ(Перечисление.СпособыПополненияЗапасов.Закупка))
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ,
	...
	|		ПО (ОсновныеДоговорыКонтрагента.Организация = &Организация)
	|			И (ОсновныеДоговорыКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.СПоставщиком))
	|			И (НоменклатураПоставщиков.Владелец = ОсновныеДоговорыКонтрагента.Контрагент)
	|ГДЕ
	|	(&ПоказыватьПереработку
	|			ИЛИ &ПоказыватьЗакупку)
	|	И ЕСТЬNULL(НоменклатураПоставщиков.Остаток, 0) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьВременныеТаблицыПродажиПоОстаткамТоваров(Запрос)
	...
	|			&ПланироватьПеремещения
	|				И ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|				И (Номенклатура, Характеристика, СтруктурнаяЕдиница, Организация) В
	|					(ВЫБРАТЬ
	...
	|	ВЫБОР КОГДА ПризнакиНаличияОстатковПоДатам.ЕстьОстатки = ИСТИНА ТОГДА Продажи.КоличествоОборот КОНЕЦ КАК КоличествоОборот
	...
	|			И Продажи.Организация = ПризнакиНаличияОстатковПоДатам.Организация
	|			И Продажи.Склад = ПризнакиНаличияОстатковПоДатам.Склад
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПризнакиНаличияОстатковПоДатам.Организация КАК Организация,
	|	ПризнакиНаличияОстатковПоДатам.Склад КАК Склад,
	|	ПризнакиНаличияОстатковПоДатам.Номенклатура КАК Номенклатура,
	|	ПризнакиНаличияОстатковПоДатам.Характеристика КАК Характеристика,
	|	ДниУчета.День КАК День,
	|	МАКСИМУМ(ПризнакиНаличияОстатковПоДатам.Дата) КАК ДатаПризнака,
	|	МАКСИМУМ(ДниУчета.День = ПризнакиНаличияОстатковПоДатам.Дата) КАК ИспользуетсяПризнакРассчитанныйДляТекущегоДня
	|ПОМЕСТИТЬ ДатыПризнаковНаличияОстатковДляДня
	|ИЗ
	|	ПризнакиНаличияОстатковПоДатам КАК ПризнакиНаличияОстатковПоДатам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДниУчета КАК ДниУчета
	|		ПО (ДниУчета.Склад = ПризнакиНаличияОстатковПоДатам.Склад)
	|			И (ДниУчета.День >= ПризнакиНаличияОстатковПоДатам.Дата)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПризнакиНаличияОстатковПоДатам.Организация,
	|	ПризнакиНаличияОстатковПоДатам.Номенклатура,
	|	ПризнакиНаличияОстатковПоДатам.Склад,
	|	ПризнакиНаличияОстатковПоДатам.Характеристика,
	|	ДниУчета.День
	|
	|;
	|
	...
	|		ПО ДатыПризнаков.Организация = ПризнакиПоДатам.Организация
	|			И ДатыПризнаков.Склад = ПризнакиПоДатам.Склад
	|			И ДатыПризнаков.Номенклатура = ПризнакиПоДатам.Номенклатура
	|			И ДатыПризнаков.Характеристика = ПризнакиПоДатам.Характеристика
	|			И ДатыПризнаков.ДатаПризнака = ПризнакиПоДатам.Дата
	|ГДЕ
	...
	|СГРУППИРОВАТЬ ПО
	|	ДатыПризнаков.Склад,
	|	ДатыПризнаков.Характеристика,
	|	ДатыПризнаков.Номенклатура,
	|	ДатыПризнаков.Организация
	|
	|";
