...
Процедура ПериодичностьПланированияПриИзменении(Элемент)
	...
	ПериодПроверки				= ПериодПоДатеНачалаИДлительности(ДатаНачалаПланирования, КоличествоДнейПланирования + 1, ПериодичностьПланирования); 
...
Процедура ВосстановитьНастройкиОтборов(Знач СохраненныйВариантРасчетаПотребностей = "")
	...
	Иначе
		...
		Если ОбщегоНазначения.СсылкаСуществует(ВариантРасчетаПотребностей) Тогда
...
Процедура НастроитьДополнительныеПоля(ПересчитатьКоличествоПрогноза = Ложь)
	...
	Если Не ПроизвольныйПериод Тогда
        ...
        ПериодПродаж.ДатаНачала	   = ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(
...
Функция ПериодПоДатеНачалаИДлительности(ДатаНачала, Знач КоличествоПериодов, Периодичность)
	...
	Возврат Новый СтандартныйПериод(
		...
		ПланированиеКлиентСервер.РассчитатьДатуОкончанияПериода(КонецДня(ДатаНачала), Периодичность, КоличествоПериодов));
