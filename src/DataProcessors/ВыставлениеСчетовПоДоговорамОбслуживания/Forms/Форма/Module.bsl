...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазрешеноФормированиеДокументов = ПравоДоступа("Изменение", Метаданные.Документы.СчетНаОплату);
	Если НЕ РазрешеноФормированиеДокументов Тогда
		Элементы.СформироватьДокументы.Доступность = Ложь;
		Элементы.ГруппаДатаФормированияДокументов.Доступность = Ложь;
		Элементы.ГруппаФормированиеСчетов.Доступность = Ложь;
		Элементы.ГруппаФормированиеАктов.Доступность = Ложь;
		Элементы.ШаблонСообщения.Доступность = Ложь;
	КонецЕсли;
	...
	ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, ПериодичностьПериода,
		ДатаФормированияДокументовСчета, ДатаФормированияДокументовАкты);
	
	СписокСчетаНаОплату.Параметры.УстановитьЗначениеПараметра("АктуальнаяДатаСеанса", НачалоДня(ТекущаяДатаСеанса()));
	
	ДинамическиеСпискиУНФ.ОтображатьТолькоВремяДляТекущейДаты(СписокСчетаНаОплату);
	ДинамическиеСпискиУНФ.ОтображатьТолькоВремяДляТекущейДаты(СписокАктыВыполненныхРабот);
	
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		
		Элементы.ФильтрыНастройкиИДопИнфо.ОтображатьЗаголовок = Истина;
...
&НаКлиенте
Процедура ДатаФормированияДокументовСчетаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
...
&НаКлиенте
Процедура ДатаФормированияДокументовАктыОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
	СтандартнаяОбработка)
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВыборИзМеню(
		Новый ОписаниеОповещения("ПредставлениеПериодаНажатиеЗавершение", ЭтотОбъект), ПериодичностиДоговоров, Элемент);
...
Процедура ФормированиеСчетовПечатныеФормыПредставлениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ПоказыватьКомандыПечатиПоШаблонамПечатиОфисныхДокументов", Ложь);
...
Процедура СформироватьДокументы(Команда)
	...
	Если ФормированиеСчетовОтправлятьПоПочте ИЛИ (ФормированиеАктов И ФормированиеАктовОтправлятьПоПочте) Тогда
		УчетнаяЗаписьПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
			ПользователиКлиент.ТекущийПользователь(), "ОсновнаяУчетнаяЗаписьЭлектроннойПочты");
		Если НЕ ЗначениеЗаполнено(УчетнаяЗаписьПоУмолчанию) Тогда
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПерейтиКНастройкеУчетныхЗаписейПользователя", ЭтотОбъект),
				НСтр("ru='Необходимо выбрать основную учетную запись для отправки электронных писем.
				|Перейти в настройки?'"),
...
Процедура ВыбратьПредыдущийПериод(Команда)
	...
	ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, ПериодичностьПериода,
		ДатаФормированияДокументовСчета, ДатаФормированияДокументовАкты, -1);
	ЗапуститьОбновлениеДанныхФормы();
...
Функция ПолучитьПериодичностиДоговоровОбслуживания()
	...
	Если ПериодичностиДоговоров.Количество() <> 0
		И ПериодичностиДоговоров.НайтиПоЗначению(ПериодичностьПериода) = Неопределено Тогда
...
Процедура ВыбратьПериодичность(ВыбраннаяПериодичность)
	...
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Неделя Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущая неделя'");
...
Процедура ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, Периодичность,
	ДатаФормированияДокументовСчета, ДатаФормированияДокументовАкты, Направление = 0)
...
&НаКлиенте
Функция ПроверитьКорректностьВыбораДатыФормированияДокументов(ДокументТип, НовоеЗначение,
	...
	Если НовоеЗначение >= ОтборПериод.ДатаНачала
		...
		Если ДокументТип = "Счет" Тогда
			Если ДатаФормированияДокументовСчетаКэш = Неопределено Тогда
				ДатаФормированияДокументовАктыИзменятьАвтоматически = (ДатаФормированияДокументовСчета
					= ДатаФормированияДокументовАкты);
			Иначе
				ДатаФормированияДокументовАктыИзменятьАвтоматически = (ДатаФормированияДокументовСчетаКэш
					= ДатаФормированияДокументовАкты);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			СтрШаблон(
				НСтр("ru='Дата формируемых документов должна находиться в интервале выбранного периода: с %1 по %2'"),
...
Процедура ЗапуститьОбновлениеДанныхФормы()
	...
	ДлительнаяОперация = ЗапуститьОбновлениеДанныхФормыСервер();
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
		Новый ОписаниеОповещения("ОбработатьЗавершениеФоновогоЗадания", ЭтотОбъект), ПараметрыОжидания);
...
Процедура ЗапуститьСозданиеПакетаДокументов()
	...
	ДлительнаяОперация = ЗапуститьСозданиеПакетаДокументовСервер();
...
&НаСервере
Функция ЗапуститьОбновлениеДанныхФормыСервер()
	
	ФоновоеЗаданиеИмя = "ФоновоеЗаданиеОбновлениеДанныхФормы";
	ФоновоеЗаданиеЗапущено = Истина;
	ИмяПроцедуры = "Обработки.ВыставлениеСчетовПоДоговорамОбслуживания.ПолучитьДанныеПоДоговорамОбслуживания";
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение данных по договорам обслуживания'");
...
&НаСервере
Функция ЗапуститьСозданиеПакетаДокументовСервер()
	
	ФоновоеЗаданиеИмя = "ФоновоеЗаданиеСозданиеПакетаДокументов";
	ФоновоеЗаданиеЗапущено = Истина;
	ИмяПроцедуры = "Обработки.ВыставлениеСчетовПоДоговорамОбслуживания.СоздатьПакетДокументов";
	...
	ПараметрыПроцедуры.Вставить("СписокВыполнениеДоговоровОбслуживания",
		СписокВыполнениеДоговоровОбслуживания.Выгрузить());
...
// то на клиентскую сторону возвращается массив созданных документов
// для дальнейшей обработки (например, вызов команд печати).
//
&НаКлиенте
Процедура ОбработатьЗавершениеФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Выполнение операции прервано.'");
	...
	Если ФоновоеЗаданиеИмя = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		...
		Если Результат.Свойство("АктыВыполненныхРабот") Тогда
			...
			ТекстОповещениеАкты = СтрШаблон(
				НСтр("ru='Актов выполненных работ: %1'"), Результат.АктыВыполненныхРабот.Количество());
			
			Если ФормированиеАктовПечатать И Результат.Свойство("АктыВыполненныхРаботИменаМакетов") Тогда
				УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.АктВыполненныхРабот",
					Результат.АктыВыполненныхРаботИменаМакетов, Результат.АктыВыполненныхРабот, ЭтотОбъект);
			КонецЕсли;
		...
		Если Результат.Свойство("СчетаНаОплату") Тогда
			...
			ТекстОповещениеСчета = СтрШаблон(
				НСтр("ru='Счетов на оплату: %1'"), Результат.СчетаНаОплату.Количество());
			
			Если ФормированиеСчетовПечатать И Результат.Свойство("СчетаНаОплатуИменаМакетов") Тогда
				УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.СчетНаОплату",
					Результат.СчетаНаОплатуИменаМакетов, Результат.СчетаНаОплату, ЭтотОбъект);
			КонецЕсли;
		...
		Если Результат.Свойство("ЭлектронныеПисьма") Тогда
			...
			ТекстОповещениеЭлектронныеПисьма = СтрШаблон(
				НСтр("ru='Электронных писем: %1'"), Результат.ЭлектронныеПисьма.Количество());
...
//
&НаСервере
Процедура ОбработатьЗавершениеФоновогоЗаданияНаСервере(Результат)
	
	ДанныеФормы = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	...
	Если ДанныеФормы.Свойство("СчетаНаОплату") Тогда
		...
		СчетаНаОплатуИменаМакетов = "";
		
		Для каждого КомандаПечати Из ПолучитьИзВременногоХранилища(ФормированиеСчетовКомандыПечатиАдрес) Цикл
			Если ФормированиеСчетовПечатныеФормы.НайтиПоЗначению(КомандаПечати.Идентификатор) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Если СчетаНаОплатуИменаМакетов = "" Тогда
				СчетаНаОплатуИменаМакетов = Идентификатор;
			Иначе
				СчетаНаОплатуИменаМакетов = СчетаНаОплатуИменаМакетов + "," +  Идентификатор;
			КонецЕсли;
		КонецЦикла;
		
		Результат.Вставить("СчетаНаОплату", ДанныеФормы.СчетаНаОплату);
		Результат.Вставить("СчетаНаОплатуИменаМакетов", СчетаНаОплатуИменаМакетов);
	КонецЕсли;
	
	Если ДанныеФормы.Свойство("АктыВыполненныхРабот") Тогда
		
		АктыВыполненныхРаботИменаМакетов = "";
		
		Для каждого КомандаПечати Из ПолучитьИзВременногоХранилища(ФормированиеАктовКомандыПечатиАдрес) Цикл
			Если ФормированиеАктовПечатныеФормы.НайтиПоЗначению(КомандаПечати.Идентификатор) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Если АктыВыполненныхРаботИменаМакетов = "" Тогда
				АктыВыполненныхРаботИменаМакетов = Идентификатор;
			Иначе
				АктыВыполненныхРаботИменаМакетов = АктыВыполненныхРаботИменаМакетов + "," +  Идентификатор;
			КонецЕсли;
		КонецЦикла;
		
		Результат.Вставить("АктыВыполненныхРабот", ДанныеФормы.АктыВыполненныхРабот);
		Результат.Вставить("АктыВыполненныхРаботИменаМакетов", АктыВыполненныхРаботИменаМакетов);
	КонецЕсли;
...
Процедура ВосстановитьНастройкиФормы()
	...
	Для каждого Настройка Из Настройки Цикл
		...
		ЗначениеНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, Настройка.Ключ,
			Настройка.Значение);
...
Процедура УправлениеФормой()
	...
	Если ФоновоеЗаданиеЗапущено И ФоновоеЗаданиеИмя = "ФоновоеЗаданиеОбновлениеДанныхФормы" Тогда
	...
	ИначеЕсли ФоновоеЗаданиеЗапущено И ФоновоеЗаданиеИмя = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		Элементы.СтраницыДоговоры.ТекущаяСтраница = Элементы.СтраницаОжиданиеВыполненияОперации;
	...
	Элементы.ФормированиеСчетовШаблонСообщения.Видимость = ФормированиеСчетовОтправлятьПоПочте
		Или ФормированиеАктовОтправлятьПоПочте;
	
	Элементы.СписокДоговорыОбслуживанияФормироватьАкт.Видимость = ФормированиеАктов;
	НастроитьКомандыПроставленияПометок(Элементы,, СчетчикПометокАкты);
	
	Элементы.ГруппаСписокАктыВыполненныхРабот.Видимость = ФормированиеАктов;
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьКомандыПроставленияПометок(Элементы, СчетчикПометокСчета = Неопределено,
	СчетчикПометокАкты = Неопределено)
	...
	Если СчетчикПометокАкты <> Неопределено Тогда
		Элементы.ФормированиеАктовСнятьПометки.Видимость = СчетчикПометокАкты > 0
			И Элементы.СписокДоговорыОбслуживанияФормироватьАкт.Видимость;
		Элементы.ФормированиеАктовПроставитьПометки.Видимость = СчетчикПометокАкты = 0
			И Элементы.СписокДоговорыОбслуживанияФормироватьАкт.Видимость;
	КонецЕсли;
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	НастройкаПечатныхФорм = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ОтправкаПечатныхФорм",
		КлючНастроекПечатныеФормыСчета);
...
&НаКлиенте
Процедура ПерейтиКНастройкеУчетныхЗаписейПользователя(Результат, ДополнительныеПараметры) Экспорт
