...
Процедура ПриОткрытии(Отказ)
	...
	МенеджерОборудованияКлиент.НачатьПодключениеОборудованиеПриОткрытииФормы(Неопределено, ЭтотОбъект, ПоддерживаемыеТипыОборудования);
			...
			Если СтрокаДерева.КоличествоПодчиненныхНеПроверялось > 0 Тогда
				...
				Если СтрокаДерева.ТребуетВзвешивания Тогда
					УстановитьСтатусДляВыделенныхСтрок(
						ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаДерева.ПолучитьИдентификатор()),
						НовыйСтатусПроверки);
					Возврат;
				Иначе
...
Процедура УточнитьДанныеПодобраннойПродукцииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(СтрокаИзмененийДо, ТекущиеДанные,, "Количество");
	...
	ЗаполнитьЗначенияСвойств(СтрокаИзмененийПосле, СтрокаИзмененийДо,, "Количество");
	Для Каждого ИмяСвойства Из ИзменяемыеСвойства Цикл
		Если ИмяСвойства = "КоличествоПотребительскихУпаковок"
			Или ИмяСвойства = "КоличествоПотребительскихУпаковокОСУ" Тогда
			Продолжить;
	...
	УточнитьДанныхОТоваре = Ложь; // Допустим свойства товара не изменены
	
	Если ПоддерживаетсяОбъемноСортовойУчет Тогда
		
		ИменаСвойствТовара = Новый Соответствие;
		ИменаСвойствТовара.Вставить("Номенклатура",   Истина);
		ИменаСвойствТовара.Вставить("Характеристика", Истина);
		ИменаСвойствТовара.Вставить("Серия",          Истина);
		
		ДанныеОТовареИзменены = Ложь;
		Для Каждого ИмяСвойства Из ИзменяемыеСвойства Цикл
			Если ИменаСвойствТовара[ИмяСвойства] = Истина Тогда
				ДанныеОТовареИзменены = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Если НовоеКоличествоУпаковокОСУ = КоличествоУпаковокОСУ Тогда
			ДействиеСУпаковкамиОСУ = Неопределено;
		ИначеЕсли НовоеКоличествоУпаковокОСУ > КоличествоУпаковокОСУ Тогда
			ДействиеСУпаковкамиОСУ = "Добавление";
		ИначеЕсли НовоеКоличествоУпаковокОСУ = 0 Тогда
			ДействиеСУпаковкамиОСУ = "Удаление";
		Иначе
			ДействиеСУпаковкамиОСУ = "Уменьшение";
		КонецЕсли;
		
		ВыполнитьУточнениеПоОСУ = (НовоеКоличествоУпаковокОСУ > 0 Или КоличествоУпаковокОСУ > 0);
		
	Иначе
		
		ВыполнитьУточнениеПоОСУ = Ложь;
		
	КонецЕсли;
	
	Если ВыполнитьУточнениеПоОСУ Тогда
		
		Если ЗначениеЗаполнено(ДействиеСУпаковкамиОСУ) Или КоличествоУпаковокОСУ > 0 Тогда
			
			Если ТекущиеДанные.ТребуетВзвешивания Тогда
				
				Если КоличествоУпаковокОСУ > 0 И ТекущиеДанные.КоличествоПодобраноВзвешеноОСУ = 0 Тогда
					КоличествоКодовМаркировкиБезКоличества =
						ТекущиеДанные.КоличествоПотребительскихУпаковокТребующихВзвешивания - КоличествоУпаковокОСУ;
				Иначе
					КоличествоКодовМаркировкиБезКоличества = ТекущиеДанные.КоличествоПотребительскихУпаковокТребующихВзвешивания;
				КонецЕсли;
				
				Если ДанныеВыбора.Количество = ТекущиеДанные.КоличествоПодобрано Тогда
					ДействиеСКоличеством = Неопределено;
				ИначеЕсли ДанныеВыбора.Количество > ТекущиеДанные.КоличествоПодобрано Тогда
					ДействиеСКоличеством = "Увеличение";
				ИначеЕсли ДанныеВыбора.Количество = 0 Тогда
					ДействиеСКоличеством = "Обнуление";
				Иначе
					ДействиеСКоличеством = "Уменьшение";
				КонецЕсли;
				
				Если Не (ДействиеСУпаковкамиОСУ = Неопределено И ДействиеСКоличеством = Неопределено) Тогда
					
					Если КоличествоКодовМаркировкиБезКоличества = 0 Тогда
						
						СтрокаИзмененийДо.Количество = ТекущиеДанные.КоличествоПодобраноВзвешеноОСУ;
						
						Если ДанныеВыбора.Количество = 0 Тогда
							СтрокаИзмененийПосле.Количество = 0;
						Иначе
							СтрокаИзмененийПосле.Количество =
								ДанныеВыбора.Количество - (ТекущиеДанные.КоличествоПодобраноВзвешено - ТекущиеДанные.КоличествоПодобраноВзвешеноОСУ);
						КонецЕсли;
						
						ОбщееКоличествоУпаковокДо    = 0;
						ОбщееКоличествоУпаковокПосле = 0;
						
						ДанныеДляОСУДо    = ДанныеДляАнализаИРаспределенияОСУ(ЭтотОбъект, СтрокаИзмененийДо);
						ДанныеДляОСУПосле = ДанныеДляАнализаИРаспределенияОСУ(ЭтотОбъект, СтрокаИзмененийПосле);
						
						Если ДанныеДляОСУДо.СтрокаОбъемноСортовогоКода = ДанныеДляОСУПосле.СтрокаОбъемноСортовогоКода Тогда
							
							СтрокаОбъемноСортовогоКода = ДанныеДляОСУДо.СтрокаОбъемноСортовогоКода;
							...
							ОбщееКоличествоУпаковокДо = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось
								+ СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует
								+ СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии
								+ СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось;
							...
							Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии") Тогда
								...
								СтрокаОбъемноСортовогоКода.Количество = СтрокаИзмененийПосле.Количество;
								
								Если КоличествоУпаковокОСУ > НовоеКоличествоУпаковокОСУ Тогда
									
									ИзменяемоеКоличествоУпаковок = КоличествоУпаковокОСУ - НовоеКоличествоУпаковокОСУ;
									
									СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок = НовоеКоличествоУпаковокОСУ;
									
									Если РежимПодбораСуществующихУпаковок Тогда
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии      = НовоеКоличествоУпаковокОСУ;
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось
											+ ИзменяемоеКоличествоУпаковок;
									Иначе
										КоличествоУпаковокКРаспределению = ИзменяемоеКоличествоУпаковок;
										Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось > 0 И КоличествоУпаковокКРаспределению > 0 Тогда
											КоличествоУпаковокКИзменению = Мин(КоличествоУпаковокКРаспределению, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось);
											КоличествоУпаковокКРаспределению = КоличествоУпаковокКРаспределению - КоличествоУпаковокКИзменению;
											СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось - КоличествоУпаковокКИзменению;
										КонецЕсли;
										Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии > 0 И КоличествоУпаковокКРаспределению > 0 Тогда
											КоличествоУпаковокКИзменению = Мин(КоличествоУпаковокКРаспределению, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии);
											КоличествоУпаковокКРаспределению = КоличествоУпаковокКРаспределению - КоличествоУпаковокКИзменению;
											СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии      = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии      - КоличествоУпаковокКИзменению;
											СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось + КоличествоУпаковокКИзменению;
										КонецЕсли;
									КонецЕсли;
									
								ИначеЕсли КоличествоУпаковокОСУ < НовоеКоличествоУпаковокОСУ Тогда
									
									ИзменяемоеКоличествоУпаковок     = НовоеКоличествоУпаковокОСУ - КоличествоУпаковокОСУ;
									КоличествоУпаковокКРаспределению = ИзменяемоеКоличествоУпаковок;
									Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось > 0 И КоличествоУпаковокКРаспределению > 0 Тогда
										КоличествоУпаковокКИзменению = Мин(КоличествоУпаковокКРаспределению, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось);
										КоличествоУпаковокКРаспределению = КоличествоУпаковокКРаспределению - КоличествоУпаковокКИзменению;
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось - КоличествоУпаковокКИзменению;
									КонецЕсли;
									Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует > 0 И КоличествоУпаковокКРаспределению > 0 Тогда
										КоличествоУпаковокКИзменению = Мин(КоличествоУпаковокКРаспределению, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует);
										КоличествоУпаковокКРаспределению = КоличествоУпаковокКРаспределению - КоличествоУпаковокКИзменению;
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует - КоличествоУпаковокКИзменению;
									КонецЕсли;
									...
									Если РежимПодбораСуществующихУпаковок Тогда
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии = НовоеКоличествоУпаковокОСУ;
									Иначе
										СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии
											+ (ИзменяемоеКоличествоУпаковок - КоличествоУпаковокКРаспределению);
									КонецЕсли;
							...
							Иначе
								...
								Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.Отсутствует") Тогда
									СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует;
									СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует   = 0;
								КонецЕсли;
								
								Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок < НовоеКоличествоУпаковокОСУ
									И РежимПодбораСуществующихУпаковок Тогда
									СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = НовоеКоличествоУпаковокОСУ;
								КонецЕсли;
						...
						Иначе
							...
							Если ТекущиеДанные.КоличествоПотребительскихУпаковок = 0
								И СтрокаИзмененийДо.Номенклатура = СтрокаИзмененийПосле.Номенклатура
								И СтрокаИзмененийДо.Характеристика = СтрокаИзмененийПосле.Характеристика
								И Не ЗначениеЗаполнено(СтрокаИзмененийДо.Серия)
								И ЗначениеЗаполнено(СтрокаИзмененийПосле.Серия) Тогда
								
								ДанныеШтрихкода = ИнициализироватьДанныеОСУ(СтрокаИзмененийПосле, НовоеКоличествоУпаковокОСУ);
								ДанныеШтрихкода.КоличествоВПотребительскойУпаковке = Результат.КоличествоВПотребительскойУпаковке;
						...
						Если КоличествоУпаковокОСУ > 0 Тогда
						...
						Если НовоеКоличествоУпаковокОСУ > 0 Тогда
						...
						Если Не РежимПодбораСуществующихУпаковок Тогда
							...
							Если ДанныеДляОСУДо.СтрокаОбъемноСортовогоКода = ДанныеДляОСУПосле.СтрокаОбъемноСортовогоКода
								И (СтрокаИзмененийПосле.ТребуетВзвешивания И СтрокаИзмененийПосле.Количество <> СтрокаИзмененийДо.Количество
									Или ОбщееКоличествоУпаковокПосле <> ОбщееКоличествоУпаковокДо) Тогда
					...
					Иначе
						...
						ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не реализовано.'"));
						
					КонецЕсли;
					
				КонецЕсли;
				
			ИначеЕсли ДействиеСУпаковкамиОСУ = Неопределено И ДействиеСКоличеством = Неопределено Тогда
				
				СтрокиДерева = ДеревоМаркированнойПродукции.ПолучитьЭлементы();
				
				Для Каждого СтрокаДерева Из СтрокиДерева Цикл
					
					УточнитьТоварВСтрокеДерева(СтрокаДерева, СтрокаИзмененийДо, СтрокаИзмененийПосле);
					
				КонецЦикла;
				
			ИначеЕсли ДействиеСУпаковкамиОСУ <> Неопределено Тогда
				...
				Если КоличествоУпаковокОСУ > НовоеКоличествоУпаковокОСУ Тогда
					...
					СтрокиПоВозростаниюВеса     = ДанныеДляОСУ.СтрокиПоВозростаниюВеса;
					СтрокаОбъемноСортовогоКода  = ДанныеДляОСУ.СтрокаОбъемноСортовогоКода;
					
					Если СтрокиПоВозростаниюВеса = Неопределено Тогда
						Возврат;
					КонецЕсли;
					
					Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии")
						Или СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась")
						И РежимПодбораСуществующихУпаковок Тогда
				...
				ИначеЕсли КоличествоУпаковокОСУ < НовоеКоличествоУпаковокОСУ Тогда
					...
					Если КоличествоУпаковокОСУ > 0 Тогда
						ДанныеДляОСУ = ДанныеДляАнализаИРаспределенияОСУ(ЭтотОбъект, ИнициализироватьДанныеОСУ(СтрокаИзмененийДо));
						СтрокаОбъемноСортовогоКода = ДанныеДляОСУ.СтрокаОбъемноСортовогоКода;
						Если СтрокаОбъемноСортовогоКода <> Неопределено
							И СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась") Тогда
							СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии");
							ПроверкаИПодборПродукцииИСМПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковки(СтрокаОбъемноСортовогоКода, Ложь);
							НормализироватьШтрихкодУпаковкиПоСоставуВложенныхУпаковок(ЭтотОбъект, СтрокаОбъемноСортовогоКода, СоответствиеШтрихкодовСтрокДерева);
							ПроверкаИПодборПродукцииИСМПКлиентСервер.СформироватьПредставлениеДляСтрокиДереваМаркированнойПродукции(СтрокаОбъемноСортовогоКода, ВидМаркируемойПродукции);
...
Процедура ДобавитьОбъемноСортовойКодУпрощенно(ДанныеШтрихкода)
	...
	Если СтрокаОбъемноСортовогоКода = Неопределено И СтрокиПоВозростаниюВеса.Количество() = 0 Тогда
	...
	ИначеЕсли СтрокаОбъемноСортовогоКода = Неопределено Тогда
		
		// Добавление
		// Ищем доступный остаток по строкам и переносим их на новую строку
		
		КоличественныеПоказатели = РассчитатьКоличественныеПоказателиСУчетомСтрокАгрегаторов(ДанныеШтрихкода, ДанныеДляОСУ, КоличествоВПотребительскойУпаковке, СвойстваСтрокой);
		КоличествоУпаковокКДобавлению = КоличественныеПоказатели.КоличествоУпаковокКДобавлению;
		ПереносимоеКоличествоУпаковок = КоличественныеПоказатели.ПереносимоеКоличествоУпаковок;
		КоличествоДоступныхУпаковок   = КоличественныеПоказатели.КоличествоДоступныхУпаковок;
	...
	ИначеЕсли СтрокиПоВозростаниюВеса.Количество() = 1 Тогда
		
		Если РежимПодбораСуществующихУпаковок Тогда
			
			Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии") Тогда
				
				Если (СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось + СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует) < ДанныеШтрихкода.КоличествоПотребительскихУпаковок Тогда
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось =
						СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось + ДанныеШтрихкода.КоличествоПотребительскихУпаковок
						- (СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось + СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует);
				КонецЕсли;
			...
			Иначе
				...
				Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась") Тогда
					...
					Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок < ДанныеШтрихкода.КоличествоПотребительскихУпаковок Тогда
						СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
					КонецЕсли;
		...
		Иначе
			...
			КоличествоУпаковокКРаспределению = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
			
			КоличествоДоступныхУпаковок = Мин(ДоступныйОстатокПоСтроке[СтрокаОбъемноСортовогоКода], КоличествоУпаковокКРаспределению);
			
			КоличествоУпаковокКРаспределению = КоличествоУпаковокКРаспределению - КоличествоДоступныхУпаковок;
			
			Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии") Тогда
				
			Иначе
				
				ИсходноеКоличествоУпаковок = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок;
				ИсходноеКоличество         = СтрокаОбъемноСортовогоКода.Количество;
				...
				Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок > ИсходноеКоличествоУпаковок Тогда
					
					КоличествоУпаковокКИзменению = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок - ИсходноеКоличествоУпаковок;
					
					Если СтрокаОбъемноСортовогоКода.ТребуетВзвешивания Тогда
						КоличествоПоУпаковкамКИзменению = СтрокаОбъемноСортовогоКода.Количество - ИсходноеКоличество;
					ИначеЕсли СтрокаОбъемноСортовогоКода.ПроизвольнаяЕдиницаУчета Тогда
						КоличествоПоУпаковкамКИзменению = КоличествоУпаковокКИзменению * КоличествоВПотребительскойУпаковке;
					Иначе
						КоличествоПоУпаковкамКИзменению = КоличествоУпаковокКИзменению;
					...
					Если КоличествоПоУпаковкамКИзменению >= 0 Тогда
				...
				ИначеЕсли СтрокаОбъемноСортовогоКода.ТребуетВзвешивания
					И ИсходноеКоличество <> СтрокаОбъемноСортовогоКода.Количество Тогда
					
					ДополнительныеПараметры = Новый Структура;
					ДополнительныеПараметры.Вставить("СтатусПроверки", ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась"));
					
					СтрокаИзмененийДо = Новый Структура(СвойстваСтрокой);
					ЗаполнитьЗначенияСвойств(СтрокаИзмененийДо, СтрокаОбъемноСортовогоКода);
					СтрокаИзмененийДо.Количество = ИсходноеКоличество;
					
					СтрокаИзмененийПосле = Новый Структура(СвойстваСтрокой);
					ЗаполнитьЗначенияСвойств(СтрокаИзмененийПосле, СтрокаИзмененийДо);
					СтрокаИзмененийПосле.Количество = СтрокаОбъемноСортовогоКода.Количество;
					
					ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриУточненииСопоставления(
						ЭтотОбъект,
						СтрокаИзмененийДо,
						СтрокаИзмененийПосле,
						ИсходноеКоличествоУпаковок,
						ОбщийМодульКонтекстаПиП(),
						ДополнительныеПараметры,
						КэшированныеЗначения);
				...
				СтрокаИзменений = Новый Структура(СвойстваСтрокой);
				ЗаполнитьЗначенияСвойств(СтрокаИзменений, СтрокаОбъемноСортовогоКода);
				СтрокаИзменений.Количество = СтрокаОбъемноСортовогоКода.Количество;
	...
	Иначе
		...
		// Ищем доступный остаток по строкам и переносим их на искомую строку
		...
		Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.Отсутствует") Тогда
			...
			СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок;
			СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует   = 0;
			СтрокаОбъемноСортовогоКода.СтатусПроверки                     = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась");
		...
		Если СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась") Тогда
			...
			Если РежимПодбораСуществующихУпаковок Тогда
				...
				ВыполнитьПересчетПодобраннойПродукции = Истина;
				
				Если ИсходныйСтатусПроверки = СтрокаОбъемноСортовогоКода.СтатусПроверки Тогда
					
					Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок = ДанныеШтрихкода.КоличествоПотребительскихУпаковок Тогда
						
						ВыполнитьПересчетПодобраннойПродукции = Ложь;
				...
				СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
				Если КоличествоУпаковокКДобавлению > 0 Или ПереносимоеКоличествоУпаковок > 0 Тогда
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
				КонецЕсли;
				СтрокаОбъемноСортовогоКода.Количество = КоличествоВПотребительскойУпаковке * СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхПотребительскихУпаковок;
				СтрокаОбъемноСортовогоКода.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии");
				ПроверкаИПодборПродукцииИСМПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковки(СтрокаОбъемноСортовогоКода, Ложь, Неопределено);
				...
				Если РежимПодбораСуществующихУпаковок И ВыполнитьПересчетПодобраннойПродукции Тогда
		...
		Иначе
			...
			Если КоличествоДоступныхУпаковок > 0 Тогда
				ОстатокПоСтроке = КоличествоДоступныхУпаковок;
				Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось > 0 Тогда
					ИзменениеПоСтроке = Мин(ОстатокПоСтроке, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось);
					ОстатокПоСтроке = ОстатокПоСтроке - ИзменениеПоСтроке;
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеПроверялось - ИзменениеПоСтроке;
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии      = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии      + ИзменениеПоСтроке;
				КонецЕсли;
				Если ОстатокПоСтроке > 0 И СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует > 0 Тогда
					ИзменениеПоСтроке = Мин(ОстатокПоСтроке, СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует);
					ОстатокПоСтроке = ОстатокПоСтроке - ИзменениеПоСтроке;
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхОтсутствует - ИзменениеПоСтроке;
					СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии    = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии    + ИзменениеПоСтроке;
				КонецЕсли;
			КонецЕсли;
			...
			Если КоличествоУпаковокКДобавлению > 0 Тогда
				СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии + КоличествоУпаковокКДобавлению;
			КонецЕсли;
...
&НаКлиенте
Функция РассчитатьКоличественныеПоказателиСУчетомСтрокАгрегаторов(ДанныеШтрихкода, ДанныеДляОСУ, КоличествоВПотребительскойУпаковке, СвойстваСтрокой)
	...
	Если КоличествоДоступныхУпаковок < ДанныеШтрихкода.КоличествоПотребительскихУпаковок Тогда
		...
		Пока Сч >= 0 И КоличествоУпаковокКДобавлению > 0 Цикл
			СтрокаДерева = СтрокиПоВозростаниюВеса[Сч];
			Сч = Сч - 1;
			...
			Если РежимПодбораСуществующихУпаковок
				...
				СтрокаИзменений = Новый Структура(СвойстваСтрокой);
				ЗаполнитьЗначенияСвойств(СтрокаИзменений, СтрокаДерева);
...
Функция ДанныеДляАнализаИРаспределенияОСУ(Форма, ДанныеШтрихкода, ЭлементыИзГруппировкиПоGTIN = Неопределено)
	...
	Для Каждого СтрокаДерева Из ЭлементыИзГруппировкиПоGTIN Цикл
		Если СтрокаДерева.ВидУпаковки <> ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет")
			Или Не ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Номенклатура",   Ложь)
			Или Не ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Характеристика", Ложь)
			Или Не ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Серия",          Ложь) Тогда
			Продолжить;
		КонецЕсли;
		ВесСтроки = 0;
		Если ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Номенклатура") Тогда
			ВесСтроки = ВесСтроки + 1;
		КонецЕсли;
		Если ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Характеристика") Тогда
			ВесСтроки = ВесСтроки + 1;
		КонецЕсли;
		Если ПроверкаИПодборПродукцииИСМПКлиентСервер.СвойствоСтрокСовпадает(ДанныеШтрихкода, СтрокаДерева, "Серия") Тогда
			ВесСтроки = ВесСтроки + 1;
		КонецЕсли;
		СтрокиПоВозростаниюВеса.Добавить(СтрокаДерева);
	...
	Если СтрокиПоВозростаниюВеса.Количество() > 0 Тогда
		...
		Пока Сч > 0 Цикл
			Для ТекущийИндекс = 0 По Сч - 1 Цикл
				ВесСтроки1 = ВесаСтрок[СтрокиПоВозростаниюВеса[ТекущийИндекс]];
				ВесСтроки2 = ВесаСтрок[СтрокиПоВозростаниюВеса[ТекущийИндекс + 1]];
				Если ВесСтроки1 > ВесСтроки2 Тогда
					СтрокаДерева = СтрокиПоВозростаниюВеса[ТекущийИндекс + 1];
					СтрокиПоВозростаниюВеса[ТекущийИндекс + 1] = СтрокиПоВозростаниюВеса[ТекущийИндекс];
					СтрокиПоВозростаниюВеса[ТекущийИндекс]     = СтрокаДерева;
				КонецЕсли;
			КонецЦикла;
			Сч = Сч - 1;
		КонецЦикла;
		
		СтрокаДереваСНаибольшимВесом = СтрокиПоВозростаниюВеса[СтрокиПоВозростаниюВеса.ВГраница()];
		МаксимальныйВесСтроки        = 3;
		...
		Пока Сч >= 0 Цикл
			СтрокаДерева = СтрокиПоВозростаниюВеса[Сч];
			Если СтрокаДерева.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась") Тогда
				КоличествоДоступныхУпаковок = СтрокаДерева.КоличествоПодчиненныхПотребительскихУпаковок;
			ИначеЕсли СтрокаДерева.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии") Тогда
			...
			Если КоличествоДоступныхУпаковок = 0 И СтрокаДерева <> СтрокаОбъемноСортовогоКода Тогда
				СтрокиПоВозростаниюВеса.Удалить(Сч);
			Иначе
	...
	Результат.Вставить("СтрокиПоВозростаниюВеса",     СтрокиПоВозростаниюВеса);
...
&НаКлиенте
Процедура УточнитьТоварВСтрокеДерева(СтрокаДерева, СтрокаИзмененийДо, СтрокаИзмененийПосле)
	
	ПодчиненныеСтроки = СтрокаДерева.ПолучитьЭлементы();
	
	Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
		
		УточнитьТоварВСтрокеДерева(ПодчиненнаяСтрока, СтрокаИзмененийДо, СтрокаИзмененийПосле);
...
Процедура ЗаполнитьКоличествоПоДокументуОСУКлиент(ВызовПослеАвторизации = Ложь)
	...
	Для Каждого СтрокаМаркируемойПродукции Из СтрокиЗаполнения Цикл
		...
		КоличествоКИзменению = СтрокаМаркируемойПродукции.Количество - СтрокаМаркируемойПродукции.КоличествоПодобрано;
		Если СтрокаМаркируемойПродукции.ТребуетВзвешивания Или СтрокаМаркируемойПродукции.ПроизвольнаяЕдиницаУчета Тогда
			...
			КоличествоВложенныхЕдиниц = КоличествоКИзменению * РасчитанноеКоличество.КоличествоВложенныхЕдиниц;
