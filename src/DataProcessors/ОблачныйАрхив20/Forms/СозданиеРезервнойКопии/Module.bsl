...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ОблачныйАрхив20.ДоступноИспользованиеСервисаОблачныйАрхив() Тогда
		...
		РезультатИнициализации = ИнициализацияРезервногоКопированияНаСервере(
			ПараметрыИнициализацииРезервногоКопирования(ЭтотОбъект));
		
		УстановитьПараметрПриОткрытии("РезультатИнициализации", РезультатИнициализации);
		...
		Если Параметры.ПараметрыСозданияРезервнойКопии <> Неопределено Тогда
			...
			УстановитьПараметрПриОткрытии("КомментарийЗаполнен" , Истина);
			УстановитьПараметрПриОткрытии("ЗапуститьПриОткрытии", ПараметрыСозданияРезервнойКопии.ЗапуститьПриОткрытии);
...
Процедура ПриОткрытии(Отказ)
	...
	КомментарийЗаполнен = ПрочитатьПараметрПриОткрытии("КомментарийЗаполнен", Ложь);	// Булево
	Если Не КомментарийЗаполнен Тогда
	...
	РезультатИнициализации = ПрочитатьПараметрПриОткрытии("РезультатИнициализации");	// см. ИнициализацияРучногоРезервногоКопированияНаСервере
...
Процедура ВыполнитьОбработкуНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли СтрСравнить(НавигационнаяСсылкаФорматированнойСтроки, "Action:InstallAgent") = 0 Тогда
		...
		ОткрытьФорму(
			...
			Новый Структура("УстановкаАгента"));
...
// Сохраняет данные, которые появляются в событии ПриСозданииНаСервере и должны быть использованы в событии ПриОткрытии.
//
// Параметры:
//  Ключ - Строка
...
//
&НаСервере
Процедура УстановитьПараметрПриОткрытии(Ключ, Значение)
	
	Если ПараметрыПриОткрытии = Неопределено Тогда
		РеквизитФормы        = Новый Структура();
		ПараметрыПриОткрытии = РеквизитФормы;
	Иначе
		РеквизитФормы = ПараметрыПриОткрытии;	// Структура
	КонецЕсли;
	
	РеквизитФормы.Вставить(Ключ, Значение);
	
КонецПроцедуры

// Возвращает значение ключа из параметров заполненных при открытии формы.
//
// Параметры:
//  Ключ - Строка
//  ЗначениеПоУмолчанию - Произвольный - значение по умолчанию, которое будет возвращено в случае отсутствия ключа в
...
//
&НаКлиенте
Функция ПрочитатьПараметрПриОткрытии(Ключ, ЗначениеПоУмолчанию = Неопределено)
	Перем Значение;	// Произвольный
	...
	РеквизитФормы = ПараметрыПриОткрытии;	// Структура
	РеквизитФормы.Свойство(Ключ, Значение);
...
Процедура ПрименитьРезультатОперации(РезультатОперации)
	...
	Если РезультатОперации.Статус = ОблачныйАрхив20КлиентСервер.ИдентификаторСтатусаРаботыАгентаАктивен() Тогда
		...
		ЗапуститьПриОткрытии = ПрочитатьПараметрПриОткрытии("ЗапуститьПриОткрытии", Ложь);
	...
	ИначеЕсли РезультатОперации.Статус = ОблачныйАрхив20КлиентСервер.ИдентификаторСтатусаИППОшибкаАутентификации()
		...
		Если РезультатОперации.Свойство("ДанныеАутентификации", ДанныеАутентификации)
