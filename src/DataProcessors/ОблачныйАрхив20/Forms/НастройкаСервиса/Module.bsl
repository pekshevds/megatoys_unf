...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОблачныйАрхив20.ДоступноИспользованиеСервисаОблачныйАрхив(Истина) Тогда
		...
		Если Параметры.Свойство("УстановкаАгента") Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Если ТекущаяСтраница = Элементы.СтраницаОписаниеИСоглашение Тогда
		...
		Элементы.КомандаЗакрыть.Заголовок = НСтр("ru = 'Отмена'");
...
Процедура ПриНачалеДлительнойОперации()
	...
	Если РеквизитДлительнаяОперация().Статус = "Выполняется" Тогда
...
Процедура ПрименитьРезультатДлительнойОперации(РезультатДлительнойОперации)
	...
	Если Не РезультатДлительнойОперации.Свойство("Результат")
		Или РезультатДлительнойОперации.Результат = Неопределено Тогда
	...
	Иначе
		РезультатВыполнения = РезультатДлительнойОперации.Результат;	// см. ОблачныйАрхив20.ИнициализацияНастроекОблачногоАрхиваВФоне
	КонецЕсли;
	...
	ИначеЕсли Статус = ОблачныйАрхив20КлиентСервер.ИдентификаторСтатусаИППОшибкаАутентификации()
		...
		Если РезультатВыполнения.Свойство("ДанныеАутентификации")
			И РезультатВыполнения.ДанныеАутентификации <> Неопределено Тогда
