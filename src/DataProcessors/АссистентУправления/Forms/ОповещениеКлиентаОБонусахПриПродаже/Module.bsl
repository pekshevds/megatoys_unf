...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ЗадачиАссистента
	ИдентификаторГруппы = "ОповещениеКлиентаОБонусахПриПродаже";
...
Процедура ШаблонСообщенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли СтрокаНастроек.ТипКИ = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ТипШаблона = "Письмо";
	КонецЕсли;
...
Процедура ДобавитьНовыеНастройкиЗадачи()
	...
	НоваяНастройка.СобытиеИдентификатор = "СписаниеБонусовПриПродаже";
	НоваяНастройка.ШаблонСообщения = Справочники.ШаблоныНаименований.НайтиПоНаименованию(РаботаСБонусами.НаименованиеШаблонаСообщенияНачислениеБонусов(), Истина);
...
Процедура ОпределитьНаименованиеЗадачи(Задача)
	...
	Наименование = "";
	Если Событие = "НачислениеБонусовПриПродаже" Тогда
		Наименование = НСтр("ru='Оповещение клиента о начислении бонусов при продаже'");
	ИначеЕсли Событие = "СписаниеБонусовПриПродаже" Тогда
		Наименование = НСтр("ru='Оповещение клиента о списании бонусов при продаже'");
	ИначеЕсли Событие = "СписаниеНачислениеБонусовПриПродаже" Тогда
		Наименование = НСтр("ru='Оповещение клиента о начислении и списании бонусов при продаже'");
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораЭлементовФормы()
	...
	Для Итератор = 0 По НастройкиЗадач.Количество() - 1 Цикл
		...
		Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("СписаниеБонусовПриПродаже", НСтр("ru='списании бонусов'"));
		Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("НачислениеБонусовПриПродаже", НСтр("ru='начислении бонусов'"));
		Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("СписаниеНачислениеБонусовПриПродаже", НСтр("ru='начислении и списании бонусов'"));
...
Процедура ДобавитьПустыеНастройкиЗадачи()
	...
	НоваяНастройка = НастройкиЗадач.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяНастройка, ОбщиеНастройки);
	НоваяНастройка.СобытиеИдентификатор = "НачислениеБонусовПриПродаже";
	НоваяНастройка.ШаблонСообщения = Справочники.ШаблоныСообщений.НайтиПоНаименованию(РаботаСБонусами.НаименованиеШаблонаСообщенияНачислениеБонусов(), Истина);
	...
	ЗаполнитьЗначенияСвойств(НоваяНастройка, ОбщиеНастройки);
	НоваяНастройка.СобытиеИдентификатор = "СписаниеБонусовПриПродаже";
	НоваяНастройка.ШаблонСообщения = Справочники.ШаблоныСообщений.НайтиПоНаименованию(РаботаСБонусами.НаименованиеШаблонаСообщенияСписаниеБонусов(), Истина);
	

	// 3.
	НоваяНастройка = НастройкиЗадач.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяНастройка, ОбщиеНастройки);
	НоваяНастройка.СобытиеИдентификатор = "СписаниеНачислениеБонусовПриПродаже";
	НоваяНастройка.ШаблонСообщения = Справочники.ШаблоныСообщений.НайтиПоНаименованию(РаботаСБонусами.НаименованиеШаблонаСообщенияСписаниеНачислениеБонусов(), Истина);
...
Процедура СоздатьИзменитьЗадачиАссистента()
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ГруппаЗадач) Тогда
			...
			ГруппаЗадач.Наименование = НСтр("ru = 'Оповестить клиента о бонусах при продаже'");
		...
		Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
			...
			ЗадачаОбъект.ПометкаУдаления  = СтрокаНастроек.Удалена;
