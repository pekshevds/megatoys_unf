...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ЗадачиАссистента
	ИдентификаторЗадач = "РасчетИОповещениеПользователяОНачисленныхНалогах";
...
Процедура УправлениеФормой()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Если СтрокаНастроек.ОповещатьСНачалаСобытия Тогда
			Элементы["НадписьТекстОповещения_"+ИндексНастройки].Заголовок = НСтр("ru = 'дней после начала периода оплаты'");
		Иначе
			Элементы["НадписьТекстОповещения_"+ИндексНастройки].Заголовок = НСтр("ru = 'дней до окончания срока оплаты'");
		КонецЕсли;
...
Процедура ДобавитьПустыеНастройкиЗадачи()
	...
	Элементы.НадписьТекстОповещения_0.Заголовок = НСтр("ru = 'дней после начала периода оплаты'");
...
Процедура СоздатьИзменитьЗадачиАссистента()
	...
	Попытка
		...
		Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
			...
			ЗадачаОбъект.Наименование = НСтр("ru = 'Оповещение клиента о начисленных налогах'");
