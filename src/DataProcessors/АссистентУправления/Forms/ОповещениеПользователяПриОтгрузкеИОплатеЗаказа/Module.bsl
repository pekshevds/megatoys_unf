...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификаторГруппы = "ОповещениеПользователяПриОтгрузкеИОплатеЗаказа";
...
Процедура ВыборПользователя(Результат, ДополнительныеПараметры) Экспорт
	...
	НастройкиЗадач[Число(Индекс)].Модифицированность = Истина;
...
Процедура УправлениеФормой()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Иначе
			ПолеСтатусОтгрузки.МаксимальнаяШирина = 26;
		КонецЕсли;
...
Процедура СоздатьИзменитьЗадачиАссистента()
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ГруппаЗадач) Тогда
			...
			ГруппаЗадач.Наименование = НСтр("ru = 'Оповестить пользователя при оплате и отгрузке заказа'");
		...
		Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
			...
			Если СтрокаНастроек.СтатусОтгрузки = Перечисления.СтатусОтгрузки.Полная Тогда
				ЗадачаОбъект.Наименование = НСтр("ru='Оповестить пользователя при полной отгрузке'");
			ИначеЕсли СтрокаНастроек.СтатусОтгрузки = Перечисления.СтатусОтгрузки.Частичная Тогда
				ЗадачаОбъект.Наименование = НСтр("ru='Оповестить пользователя при частичной отгрузке'");
			КонецЕсли;
...
Процедура СоздатьВспомогательныеЗадачи(ЗадачаОбъект, СтрокаНастроек)
	...
	ВспомогательнаяЗадача.СпособОповещения          = СтрокаНастроек.СпособОповещения;
