...
&НаКлиенте
Процедура Подключаемый_ПроцентОплатыПриИзменении(Элемент)
...
&НаКлиенте
Процедура Подключаемый_ДекорацияУдалитьНажатие(Элемент)
...
&НаКлиенте
Процедура Подключаемый_СпособОповещенияАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура Подключаемый_СостояниеЗаказаПриИзменении(Элемент)
	СтрокаНастроек = НастройкиЗадач.Получить(ИндексБлока(Элемент.Имя));
	СтрокаНастроек.Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СтатусПоставкиПриИзменении(Элемент)
	СтрокаНастроек = НастройкиЗадач.Получить(ИндексБлока(Элемент.Имя));
	СтрокаНастроек.Модифицированность = Истина;
КонецПроцедуры
...
Процедура ОпределитьНаименованиеЗадачи(Задача, СтрокаНастроек)
	...
	Если ЭтоЗадачаИзменениеСостоянияПриОплатеИПоставке() ИЛИ ЭтоЗадачаСообщенияПриОплатеИПоставке() Тогда
		Наименование = СтрШаблон(НСтр("ru = '%1 %2 и %3'"), СтрокаДействия, СтрокаОплаты, СтрокаОтгрузки);
	ИначеЕсли ЭтоЗадачаИзменениеСостоянияПриОплате() ИЛИ ЭтоЗадачаСообщенияПриОплате() Тогда
		Наименование = СтрШаблон(НСтр("ru = '%1 %2'"), СтрокаДействия, СтрокаОплаты);
	Иначе
		Наименование = СтрШаблон(НСтр("ru = '%1 %2'"), СтрокаДействия, СтрокаОтгрузки);
	КонецЕсли; 
...
Процедура ОбновитьЭлементыДействийАссистента()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Если ИндексНастройки > 0 Тогда
			...
			ПолеСостояниеЗаказа.УстановитьДействие("ПриИзменении","Подключаемый_СостояниеЗаказаПриИзменении");
			...
			ПолеПроцентОплаты.УстановитьДействие("ПриИзменении", "Подключаемый_ПроцентОплатыПриИзменении");
			...
			ПолеСтатусПоставки.УстановитьДействие("ПриИзменении", "Подключаемый_СтатусПоставкиПриИзменении");
			...
			ДекорацияУдалить.УстановитьДействие("Нажатие", "Подключаемый_ДекорацияУдалитьНажатие");
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиИзменениеСостоянияПриОплате()
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиИзменениеСостоянияПриПоставке()
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиИзменениеСостоянияПриОплатеИПоставке()
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиСообщенияПриОплате()
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиСообщенияПриПоставке()
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиСообщенияПриОплатеИПоставке()
