...
Процедура ПослеЗавершенияЗаменыСсылок(Задание, ДополнительныеПараметры) Экспорт
	...
	Если ЕстьНеудачныеЗамены Тогда
		...
		Активизировать();
		ОбновитьОстаткиПоНоменклатуреУНФ();
	Иначе
...
&НаКлиенте
Процедура ОбновитьОстаткиПоНоменклатуреУНФ()

	Если ТипЗнч(ОсновнойЭлемент) = Тип("СправочникСсылка.Номенклатура") И ЗначениеЗаполнено(ОсновнойЭлемент) Тогда
		ОбновитьОстаткиПоНоменклатуреУНФНаСервере();
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ОбновитьОстаткиПоНоменклатуреУНФНаСервере()
	
	ОстаткиОбновлены = РегистрыСведений.ОстаткиТоваров.ВыполнитьЗаданиеПоПересчетуОстатковВТехническомРегистре(
		ОсновнойЭлемент);

	Если Не ОстаткиОбновлены Тогда

		СтрокаСообщения = НСтр("ru = 'При обновлении остатков произошла ошибка.
							   |Необходимо повторно обновить остатки.'");
