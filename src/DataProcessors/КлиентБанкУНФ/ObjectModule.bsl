...
Функция СоздатьКонтрагента(СтрокаКонтрагент = Неопределено) Экспорт
	...
	Если НЕ ТипЗнч(СтрокаКонтрагент.Реквизит) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		СтруктураОтношений = Новый Структура();
		Обработки.КлиентБанкУНФ.ДобавитьИнформациюОбОтношенияхСКонтрагентомПоВидуОперации(СтруктураОтношений, СтрокаКонтрагент.ВидОперации);
		ЗаполнитьЗначенияСвойств(НовыйЭлемент, СтруктураОтношений);
...
// Функция возвращает реквизиты банковского счета с учетом того, являются расчеты прямыми или нет.
//
...
// Процедура вставляет в структуру показатели необходимости заполнения договора данными по умолчанию.
...
// Функция создает и возвращает таблицу значений для хранения объектов, которые созданы в процессе загрузки. Таблица
// используется, чтобы избежать дублей.
//
...
Функция Загрузить(ИмпортЗаголовок, ТаблицаРезультата, ТаблицаПомеченныхНаУдалениеКакДубль) Экспорт
	...
	Если ТипЗнч(ИмпортЗаголовок) = Тип("Структура") И ИмпортЗаголовок.Свойство("ДатаНачала") Тогда
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат Новый Структура("ЕстьПредупреждения, ОбработаноСтрок, ВсегоСтрок", ЕстьПредупреждения, 0,
			ДокументыКИмпорту.Количество());
	КонецЕсли;
		
	Если ТипЗнч(ИмпортЗаголовок) = Тип("Структура") И ИмпортЗаголовок.Свойство("ДатаКонца") Тогда
	...
	Для каждого СтрокаСекции Из ДокументыКИмпорту Цикл
		Если СтрокаСекции.Загружать Тогда
			ЗагрузитьСтроку(ЕстьПредупреждения, СтрокаСекции, ТаблицаРезультата, ТаблицаОбъектовСозданныхПриЗагрузке, Ложь);
			ОбработаноСтрок = ОбработаноСтрок + 1;
		Иначе
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ru='Пропущен, не загружается'"),
				Ложь, Ложь, НСтр("ru='• Пропущен, не загружается'"), "");
		КонецЕсли;
	...
	Возврат Новый Структура("ЕстьПредупреждения, ОбработаноСтрок, ВсегоСтрок, СообщенияПользователю", ЕстьПредупреждения, ОбработаноСтрок, ДокументыКИмпорту.Количество(), ПолучитьСообщенияПользователю());
		...
		Если ТипЗнч(СтрокаСекции.Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ СтрокаСекции.Контрагент.Пустая() Тогда
			...
			Если //ТипЗнч(СтрокаСекции.СчетКонтрагента) = Тип("Строка") И
				...
				Иначе
					...
					СчетКонтрагента = Обработки.КлиентБанкУНФ.СоздатьРасчетныйСчетПоСтруктуреДанных(СтруктураДанных);
			...
			Если (СтрокаСекции.ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С
				...
				Иначе
					...
					Договор = Обработки.КлиентБанкУНФ.СоздатьДоговорПоСтруктуреДанных(СтруктураДанных);
...
Процедура ЗаполнитьЗначениеЭкспорта(СтрокаЭкспорта, ВыборкаДляЭкспорта)
	...
	// КодНазПлатежа
	Если ДвиженияДенежныхСредствКлиентСервер.ДействуетУказаниеБанкаРоссии5286У(ВыборкаДляЭкспорта.Дата) Тогда
...
Функция Выгрузить(РасширениеРаботыСФайламиПодключено, КлючУникальности, АдресТекстаНаСервере = "", ТаблицаЗначенийДляВыгрузкиВФайл = Неопределено) Экспорт
	...
	Для каждого СтрокаСекции Из ДокументыНаЭкспорт Цикл
		...
		КоличествоСтрокСекции = СтрЧислоСтрок(Буфер);
		Для Сч = 1 По КоличествоСтрокСекции Цикл
