...
Функция РазобратьБанковскуюВыписку(КонтекстФормы) Экспорт
	...
	Если ДанныеИзБанка.Условия.Свойство("РасчСчет") Тогда
		...
		Если ДанныеИзБанка.Условия.РасчСчет.Количество() = 1 Тогда
	...
	Для каждого Документ Из ДанныеИзБанка.Документы Цикл
		...
		// Если в файле находятся выписка по нескольким счетам, распознаем и отображаем только выгруженные по указанному
		// банковскому счету.
	...
	Если НЕ (ПлатежиВБюджетКлиентСервер.ДопускаетсяПустойКБК(СтрокаЭкспорта.ВидПеречисленияВБюджет, СтрокаЭкспорта.КонтрагентНомерСчета, СтрокаЭкспорта.Дата)) Тогда
		...
		Если ПустаяСтрока(П105)
			...
			Если СтрокаЭкспорта.Дата >= '20140101' Тогда // ОКТМО действует в любом случае с 01.01.2014 
				Если НЕ ПлатежиВБюджетКлиентСервер.ЭтоЕдиныйНалоговыйПлатеж(П104) Тогда // по ЕПН - "0"
					ДобавитьЗамечание(
						СтрокаЭкспорта,
						3,
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, Документ, ПараметрыВыделенияКолонок, Объект, ТаблицаТаблицДокументов, КонтекстФормы, ПослеЗагрузкиВыписокВ1С = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
		...
		ИначеЕсли НЕ ПустаяСтрока(ИННКонтрагента) ИЛИ СтрокаДокумента.ЭтоФизическоеЛицо Тогда
			...
			|						ИЛИ (ВЫРАЗИТЬ(Контрагенты.НаименованиеПолное КАК СТРОКА(1024))) = &ИмяКонтрагента
			|						ИЛИ (ВЫРАЗИТЬ(Контрагенты.НаименованиеПолное КАК СТРОКА(1024))) = &ИмяКонтрагентаДополнительно
			|			ИНАЧЕ Контрагенты.ИНН = &КонтрагентИНН
			|					ИЛИ ВЫБОР
			|						КОГДА Контрагенты.Наименование = &ИмяКонтрагента
	...
	ОбновитьДоговорПоНайденнымДокумента(СтрокаДокумента);
...
Процедура ОбновитьДоговорПоНайденнымДокумента(СтрокаДокумента)
	
	ДоговорИзДокумента = СтрокаДокумента.Договор;
	
	Если ЗначениеЗаполнено(СтрокаДокумента.ПлатежноеПоручение) Тогда
		
		ДоговорИзДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.ПлатежноеПоручение, "Договор");
		
	ИначеЕсли ЗначениеЗаполнено(СтрокаДокумента.Заказ) Тогда
		
		ДоговорИзДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.Заказ, "Договор");
		
	КонецЕсли;
	
	СтрокаДокумента.Договор = ДоговорИзДокумента;
...
Функция ИдентификаторыФизическогоЛицаИзНаименования(Наименование, ИНН, Счет)
	...
	ЭтоИННЮридическогоЛица = ЗначениеЗаполнено(ИННПриведенный)
		И ИдентификационныеНомераНалогоплательщиковКлиентСервер.ЭтоИННЮридическогоЛица(ИНН);
	
	БалансовыйСчет = БанковскиеПравилаКлиентСервер.БалансовыйСчет(Счет);
	ЭтоСчетФизЛица = БанковскиеПравилаКлиентСервер.ЭтоСчетПереводовФизическимЛицам(БалансовыйСчет);
	...
	Если Не ПустаяСтрока(ИНН)
		...
		// Это не похоже на имя физического лица. Возможно, это как раз тот случай, когда в наименовании юридического лица
		// указали адрес.
	...
	Если ИдентификационныеНомераНалогоплательщиковКлиентСервер.ЭтоИННФизическогоЛица(ИНН) Тогда
		СоответствиеТребованиямИНН = ИдентификационныеНомераНалогоплательщиковКлиентСервер.ПроверитьСоответствиеТребованиямИНН(ИНН, Ложь);
