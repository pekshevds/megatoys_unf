...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// Счет контрагента.
	Если НЕ ПустаяСтрока(ИспользоватьСуществующийСчетКонтрагента) Тогда
	...
	// Договор.
	Если НЕ ПустаяСтрока(ИспользоватьСуществующийДоговор) Тогда
...
Процедура КомандаОК(Команда)
	...
	Если ВидОперацииИсходящие = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам") Тогда
		СтруктураДаннхЗакрытия.Вставить("Договор", ДоговорКредитаЗайма);
	Иначе 
	...
	Если ЭтоПеремещение Тогда
		...
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(ИспользоватьСуществующийСчетКонтрагента) Тогда
	...
	СтруктураДаннхЗакрытия.Вставить("СрокУплаты", СрокУплаты);
...
Функция ПолучитьДействиеНаКлиенте(ОбъектАнализа)
	...
	ИначеЕсли ОбъектАнализа = "РСчет" Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СоздаватьНовыйСчетКонтрагентаПриЗагрузкеВыпискиВ1С) Тогда
			Возврат ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С");
		ИначеЕсли ЗначениеЗаполнено(ИспользоватьСуществующийСчетКонтрагента) Тогда
			Возврат ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего");
	...
	ИначеЕсли ОбъектАнализа = "Договор" Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СоздаватьНовыйДоговорПриЗагрузкеВыпискиВ1С) Тогда
			Возврат ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С");
		ИначеЕсли ЗначениеЗаполнено(ИспользоватьСуществующийДоговор) Тогда
			Возврат ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего");
...
Процедура ИнициализироватьФорму()
	...
	СрокУплаты = Параметры.СрокУплаты;
	...
	УправлениеВидимостьюДляРасчетовПоКредитам();
	
КонецПроцедуры

&НаСервере
Процедура УправлениеВидимостьюДляРасчетовПоКредитам()
	
	Если ВидОперацииИсходящие <> Перечисления.ВидыОперацийРасходСоСчета.РасчетыПоКредитам Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаДействияСДоговоромПодчиненные.Видимость = Ложь;
	Элементы.ДоговорКредитаЗайма.Видимость = Истина;
	Элементы.Договор.Видимость = Ложь;
	Элементы.ОДоговореЦентр.Видимость = Ложь;
	Элементы.ОДоговореПраво.Видимость = Ложь;
	
	Если ЗначениеЗаполнено(КонтрагентСсылка) Тогда
		Элементы.ДоговорКредитаЗайма.ТолькоПросмотр = Ложь;
		Элементы.ДоговорКредитаЗайма.ПодсказкаВвода = "";
	Иначе
		Элементы.ДоговорКредитаЗайма.ТолькоПросмотр = Истина;
		Элементы.ДоговорКредитаЗайма.ПодсказкаВвода = НСтр("ru = 'Необходимо выбрать контрагента'");
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюОДоговореНаСервере(Контрагент, Договор, ПараметрыДоговора)
	...
	Если ТипЗнч(Параметры.Договор) = Тип("ДокументСсылка.ДоговорКредитаИЗайма") Тогда
		ДоговорКредитаЗайма = Параметры.Договор;
	КонецЕсли;
...
Процедура СформироватьЗаголовкиСтраницНаСервере()
	...
	Если МенятьЗаголовкиСтраниц Тогда
		...
		Если ЗначениеЗаполнено(СоздаватьНового) Тогда
			Элементы.ДействияСКонтрагентом.Заголовок = НСтр("ru = 'Контрагент (создать сейчас)'");
		ИначеЕсли ЗначениеЗаполнено(СоздаватьНовогоПриЗагрузкеВыпискиВ1С) Тогда
			Элементы.ДействияСКонтрагентом.Заголовок = НСтр("ru = 'Контрагент (создать при загрузке)'");
		ИначеЕсли ЗначениеЗаполнено(ИспользоватьСуществующего) Тогда
			Элементы.ДействияСКонтрагентом.Заголовок = НСтр("ru = 'Контрагент (использовать существующего)'");
		ИначеЕсли ЗначениеЗаполнено(НеСоздавать) Тогда
			Элементы.ДействияСКонтрагентом.Заголовок = НСтр("ru = 'Контрагент (НЕ создавать)'");
		Иначе
			Элементы.ДействияСКонтрагентом.Заголовок = НСтр("ru = 'Контрагент'");
		КонецЕсли;
		
		// Заголовок страницы "Р/счет".
		Если ЗначениеЗаполнено(СоздаватьНовыйСчетКонтрагента) Тогда
			Элементы.ДействияСоСчетомКонтрагента.Заголовок = НСтр("ru = 'Р/счет (создать сейчас)'");
		ИначеЕсли ЗначениеЗаполнено(СоздаватьНовыйСчетКонтрагентаПриЗагрузкеВыпискиВ1С) Тогда
			Элементы.ДействияСоСчетомКонтрагента.Заголовок = НСтр("ru = 'Р/счет (создать при загрузке)'");
		ИначеЕсли ЗначениеЗаполнено(ИспользоватьСуществующийСчетКонтрагента) Тогда
			Элементы.ДействияСоСчетомКонтрагента.Заголовок = НСтр("ru = 'Р/счет (использовать существующий)'");
		ИначеЕсли ЗначениеЗаполнено(НеСоздаватьСчетКонтрагента) Тогда
			Элементы.ДействияСоСчетомКонтрагента.Заголовок = НСтр("ru = 'Р/счет (НЕ создавать)'");
		Иначе
			Элементы.ДействияСоСчетомКонтрагента.Заголовок = НСтр("ru = 'Р/счет'");
		КонецЕсли;
		
		// Заголовок страницы "Договор".
		Если ЗначениеЗаполнено(СоздаватьНовыйДоговор) Тогда
			Элементы.ДействияСДоговором.Заголовок = НСтр("ru = 'Договор (создать сейчас)'");
		ИначеЕсли ЗначениеЗаполнено(СоздаватьНовыйДоговорПриЗагрузкеВыпискиВ1С) Тогда
			Элементы.ДействияСДоговором.Заголовок = НСтр("ru = 'Договор (создать при загрузке)'");
		ИначеЕсли ЗначениеЗаполнено(ИспользоватьСуществующийДоговор) Тогда
			Элементы.ДействияСДоговором.Заголовок = НСтр("ru = 'Договор (использовать существующий)'");
		ИначеЕсли ЗначениеЗаполнено(НеСоздаватьДоговор) Тогда
			Элементы.ДействияСДоговором.Заголовок = НСтр("ru = 'Договор (НЕ создавать)'");
		Иначе
			Элементы.ДействияСДоговором.Заголовок = НСтр("ru = 'Договор'");
		КонецЕсли;
...
Процедура НазначитьПереключатели()
	...
	Если ДействиеСоСчетомКонтрагента = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		...
		ИспользоватьСуществующийСчетКонтрагента = "";
	...
	ИначеЕсли ЗначениеЗаполнено(КонтрагентСсылка) Тогда
		СоздаватьНовыйСчетКонтрагента = "";
		ИспользоватьСуществующийСчетКонтрагента = "ИспользоватьСуществующего";
		СоздаватьНовыйСчетКонтрагентаПриЗагрузкеВыпискиВ1С = "";
		НеСоздаватьСчетКонтрагента = "";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИспользоватьСуществующего) И Не ЗначениеЗаполнено(ДоговорСсылка) Тогда
		...
		ИспользоватьСуществующийДоговор = "";
	...
	ИначеЕсли ЗначениеЗаполнено(ДоговорСсылка) Тогда
		СоздаватьНовыйДоговор = "";
		ИспользоватьСуществующийДоговор = "ИспользоватьСуществующего";
		СоздаватьНовыйДоговорПриЗагрузкеВыпискиВ1С = "";
		НеСоздаватьДоговор = "";
	КонецЕсли;
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	Если ЗначениеЗаполнено(ИспользоватьСуществующийДоговор) Тогда
...
Процедура ОпределитьВидимостьРеквизитовНалогов()
	...
	Элементы.СрокУплаты.Видимость = (ВидОперацииИсходящие = ВидОперацииНалоги);
	...
	Если НЕ ПустаяСтрока(СоздаватьНового) Тогда
		...
		Попытка
			...
			Если ТипЗнч(КонтрагентГруппа) = Тип("Строка") Тогда // авто
				Родитель = Обработки.КлиентБанкУНФ.ОпределитьАвтоГруппуКонтрагента(ДеньгиОтКонтрагентаПоступили);
			Иначе
			...
			Обработки.КлиентБанкУНФ.ДобавитьИнформациюОбОтношенияхСКонтрагентомПоВидуОперации(СтруктураДанных, ВидОперации);
			// ) Заполнили структуру данных.
			
			// Создадим контрагента по структуре данных
			КонтрагентСоздан = Обработки.КлиентБанкУНФ.СоздатьКонтрагентаПоСтруктуреДанных(СтруктураДанных);
		...
		Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Или Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
			...
			Если Не КонтрагентСсылка.Пустая() Тогда
				...
				Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Тогда
					...
					БанковскийСчетПоУмолчанию = Обработки.КлиентБанкУНФ.СоздатьРасчетныйСчетПоСтруктуреДанных(СтруктураДанных);
				...
				Если Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
					...
					ДоговорСсылка = Обработки.КлиентБанкУНФ.СоздатьДоговорПоСтруктуреДанных(СтруктураДанных);
				КонецЕсли;
...
Функция НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора = Неопределено, СписокВидовДоговора = Неопределено, МаксНомер = Неопределено)
	...
	Возврат Обработки.КлиентБанкУНФ.НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора, СписокВидовДоговора);
...
Функция НайтиСчетКонтрагента(ВладелецСчета, НомерСчета)
	...
	Возврат Обработки.КлиентБанкУНФ.НайтиСчетКонтрагента(ВладелецСчета, НомерСчета);
...
&НаСервере
Процедура ИспользоватьСуществующийСчетКонтрагентаПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ИспользоватьСуществующийСчетКонтрагентаПриИзменении(Элемент)
	
	СоздаватьНовыйСчетКонтрагента = "";
	СоздаватьНовыйСчетКонтрагентаПриЗагрузкеВыпискиВ1С = "";
	НеСоздаватьСчетКонтрагента = "";
	
	ИспользоватьСуществующийСчетКонтрагентаПриИзмененииНаСервере();
...
&НаСервере
Процедура ИспользоватьСуществующийДоговорПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ИспользоватьСуществующийДоговорПриИзменении(Элемент)
	
	СоздаватьНовыйДоговор = "";
	СоздаватьНовыйДоговорПриЗагрузкеВыпискиВ1С = "";
	НеСоздаватьДоговор = "";
	
	ИспользоватьСуществующийДоговорПриИзмененииНаСервере();
...
Процедура ВидНалогаПриИзменении(Элемент)
	Если ЗначениеЗаполнено(ВидНалога) Тогда
		СрокУплаты = ПолучитьСрокУплаты(ВидНалога, ДатаДок);
	КонецЕсли;
КонецПроцедуры
...
Процедура НастроитьФормуМобильныйКлиент()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаИнформация", "Заголовок", НСтр("ru = 'Информация'"));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИспользоватьСуществующийСчетКонтрагента",  "ВидПереключателя", ВидПереключателя.Тумблер);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИспользоватьСуществующийДоговор",  "ВидПереключателя", ВидПереключателя.Тумблер);
