...
&НаСервере
Функция УсловиеДляТаблицыСформировать()
	
	Результат = 
	"ВЫБОР КОГДА НЕ &ПоказыватьПомеченныеНаУдаление Тогда
	|	&Условие
	|	ИНАЧЕ
	|		ИСТИНА
	|КОНЕЦ";
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ОснованияЭлектронныхДокументовЭДО.Тип.Типы() Цикл
		...
		Если Метаданные.Документы.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
			ТекстУсловия = ТекстУсловия + "КОГДА ТИПЗНАЧЕНИЯ(СостоянияЭД.СсылкаНаОбъект) = ТИП(Документ." + ОбъектМетаданных.Имя + ") 
				|		ТОГДА ВЫРАЗИТЬ(СостоянияЭД.СсылкаНаОбъект КАК Документ." + ОбъектМетаданных.Имя + ").ПометкаУдаления = ЛОЖЬ
				|	";
		ИначеЕсли Метаданные.Справочники.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
			ТекстУсловия = ТекстУсловия + "КОГДА ТИПЗНАЧЕНИЯ(СостоянияЭД.СсылкаНаОбъект) = ТИП(Справочник." + ОбъектМетаданных.Имя + ") 
				|		ТОГДА ВЫРАЗИТЬ(СостоянияЭД.СсылкаНаОбъект КАК Справочник." + ОбъектМетаданных.Имя + ").ПометкаУдаления = ЛОЖЬ
				|	";
		КонецЕсли;
	...
	Результат = СтрЗаменить(Результат, "&Условие", ТекстУсловия);
...
&НаСервере
Функция ТекстПоляПометкиУдаленияДляТаблицыСформировать()
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ОснованияЭлектронныхДокументовЭДО.Тип.Типы() Цикл
		...
		Если Метаданные.Документы.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
			Текст = Текст + "КОГДА ТИПЗНАЧЕНИЯ(СостоянияЭД.СсылкаНаОбъект) = ТИП(Документ." + ОбъектМетаданных.Имя + ") 
				|		ТОГДА ВЫРАЗИТЬ(СостоянияЭД.СсылкаНаОбъект КАК Документ." + ОбъектМетаданных.Имя + ").ПометкаУдаления
				|	";
		ИначеЕсли Метаданные.Справочники.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
			Текст = Текст + "КОГДА ТИПЗНАЧЕНИЯ(СостоянияЭД.СсылкаНаОбъект) = ТИП(Справочник." + ОбъектМетаданных.Имя + ") 
				|		ТОГДА ВЫРАЗИТЬ(СостоянияЭД.СсылкаНаОбъект КАК Справочник." + ОбъектМетаданных.Имя + ").ПометкаУдаления
				|	";
		КонецЕсли;
		
	КонецЦикла;
	
	Результат = Текст + "КОНЕЦ";
...
Процедура УстановитьПараметрыСписковРазделов(Знач НастройкиОтображения)
	...
	ИмяСписка = СписокРаздела("Сформировать");
	Список = Форма[ИмяСписка];
	Список.ТекстЗапроса = СтрЗаменить(Список.ТекстЗапроса, "&ПоказыватьПомеченныеНаУдаление", УсловиеДляТаблицыСформировать());
	Список.ТекстЗапроса = СтрЗаменить(Список.ТекстЗапроса, "&ПометкаУдаления", ТекстПоляПометкиУдаленияДляТаблицыСформировать());
	Список.Параметры.УстановитьЗначениеПараметра("ПоказыватьПомеченныеНаУдаление", НастройкиОтображения.ПоказыватьПомеченныеНаУдаление);
	Список.УстановитьОграниченияИспользованияВОтборе(ОграничениеОтбора);
...
Процедура ПоказатьКомандыРаздела(Знач Раздел)
	...
	КомандыРазделов.Вставить("Сформировать", "Обновить, ЗакрытьПринудительно, ИзменитьДокумент, СформироватьЭД, 
		|СформироватьПодписатьИОтправить, КомандыСформироватьЭД, СоздатьПроизвольныйДокумент, ПоказыватьПомеченныеНаУдаление");
