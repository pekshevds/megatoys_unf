...
Процедура Подключаемый_ПроверитьВыполнениеДлительнойОперацииНаКлиенте()
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0 
		ИЛИ СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "6НДФЛ") > 0 Тогда
		Попытка
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") = 0 
		И СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "6НДФЛ") = 0 Тогда
...
Процедура ВыполнитьРасчетНажатие()
	...
	Если Результат.Свойство("ИмяФормыРезультата") Тогда
		...
		Если СтрНайти(ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0
			ИЛИ СтрНайти(ИмяФормыРезультата, "Форма6НДФЛ") > 0 Тогда			
...
Процедура ДекорацияТекстОписание22ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "СведенияПоСотрудникам" Тогда
		ОткрытьФорму("Справочник.Сотрудники.ФормаСписка");
	ИначеЕсли НавигационнаяСсылка = "НачисленияПоСотрудникам" И ИспользуетсяРегламентированнаяЗарплата = Ложь Тогда
		ОткрытьФорму("ЖурналДокументов.ДокументыПоЗарплате.ФормаСписка", Новый Структура("Организация", Организация)); 
	ИначеЕсли НавигационнаяСсылка = "НачисленияПоСотрудникам" И ИспользуетсяРегламентированнаяЗарплата Тогда	
		ОткрытьФорму("ЖурналДокументов.НачислениеЗарплаты.ФормаСписка", Новый Структура("Организация", Организация));
	КонецЕсли;
