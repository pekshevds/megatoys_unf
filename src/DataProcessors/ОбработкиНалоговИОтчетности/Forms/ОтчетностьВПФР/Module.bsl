...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(КомплектОтчетности) Тогда
		...
		Иначе
			КомплектОтчетности = РегламентированнаяОтчетностьУСН.ПолучитьКомплектОтчетностиПерсучетаПоСобытиюКалендаря(
	...
	Если ЗначениеЗаполнено(КомплектОтчетности) Тогда
		...
		Если Выборка1.Следующий() Тогда
			...
			Если Месяц(ОтчетныйПериод) = 10 И ЭтоОтчетностьРанее2013года Тогда
				...
				Элементы.ДекорацияОписаниеРаботы2.Заголовок = НСтр(
					"ru='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 в 2-х экземплярах'");
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр(
					"ru='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 на флешку или дискету (разные налоговые по-разному принимают)'");
			...
			Если Не ЭтоОтчетностьРанее2014года Тогда
				...
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр(
					"ru='Сохраните файлы РСВ-1 на флешку или дискету (разные налоговые по-разному принимают)'");
			ИначеЕсли Не ЭтоОтчетностьРанее2013года Тогда
				Элементы.ДекорацияОписаниеРаботы2.Заголовок = НСтр(
					"ru='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 в 2-х экземплярах'");
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр(
					"ru='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 на флешку или дискету (разные налоговые по-разному принимают)'");
			КонецЕсли;
	...
	ЗаписьТарифа = РегистрыСведений.ПрименяемыеТарифыСтраховыхВзносовУНФ.ПолучитьПоследнее(
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СобытиеКалендаря, "ДатаДокументаОбработкиСобытия"),
		Новый Структура("ГоловнаяОрганизация", Объект.Организация));
	Если ЗаписьТарифа.ВидТарифа = РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "УпрощенныйНалоговыйРежимПроизводство") Тогда
	...
	ДатыСобытия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.СобытиеКалендаря,
		"ДатаНачалаДокументов,ДатаОкончанияДокументов");
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДокументОтчетности, "ДатаНачала,ДатаОкончания,ВыбраннаяФорма");
			
	Если ЗначениеЗаполнено(ДокументОтчетности) И Реквизиты.ДатаОкончания >= '20150401' И Реквизиты.ВыбраннаяФорма
	...
	РегламентированнаяОтчетностьКлиентСервер.ПриИнициализацииФормыРегламентированногоОтчета(ЭтотОбъект, "ПФР");
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьТекущуюЗакладку(ЭтотОбъект, Объект);
...
Процедура ПереходСправкиПоРСВ(Элемент)
	...
	ДатаСменыСостояния = ОбщегоНазначенияКлиент.ДатаСеанса();
...
Процедура ПереходКЗаполнениюОтчетностиПФРНажатие(Элемент)
	...
	КалендарьОтчетностиКлиент.ОткрытьФормуНачалаЗаполнения(ЭтотОбъект, ПараметрыФормы);
	Закрыть();
	
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияЗаявлениеОтправленоОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОбновитьСтатус" Тогда
		оп = Новый ОписаниеОповещения("ОбновлениеСтатусаЗавершение", ЭтотОбъект);
		КонтекстЭДОКлиент.ОбновитьСтатусыЗаявленийАбонентов_ИзФормыСписка( , ЭтотОбъект.УникальныйИдентификатор, оп,
			Объект.Организация);
...
Процедура РаспечататьСправкуРСВ1(Команда)
	...
	МассивПечатаемыхОтчетов.Добавить(ДокументОтчетности);
...
Процедура ОбновитьСтатус(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОбменаПоУчетнойЗаписиЗавершение", ЭтотОбъект);
...
Процедура ОтправитьОтчетность(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриОтветеНаВопросОЗаполнении", ЭтотОбъект);
...
Процедура РаспечататьСЗВ62(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.РеестрСЗВ_6_2", "ФормаСЗВ_6_2", МассивОбъектов, ЭтотОбъект,
		Новый Структура);
...
Процедура СохранитьСЗВ(Команда)
	...
	АдресФайла = УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиСЗВ64(ДокументСЗВ64);
...
Процедура РаспечататьОписьПачек(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ОписьПачекСЗВ_6", "ФормаАДВ_6_2", МассивОбъектов, ЭтотОбъект, Новый Структура());
...
Процедура СохранитьАДВ(Команда)
	...
	АдресФайла = УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиАДВ62(ДокументАДВ);
	ПолучитьФайл(АдресФайла, ИмяФайла, Истина);
...
Процедура РаспечататьАДВ64(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПачкаДокументовСЗВ_6_3", "ФормаАДВ_6_4", МассивОбъектов,
...
Процедура РаспечататьСЗВ63(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПачкаДокументовСЗВ_6_3", "ФормаСЗВ_6_3", МассивОбъектов,
		ЭтотОбъект, Новый Структура);
КонецПроцедуры
...
Процедура РаспечататьСЗВ64(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПачкаДокументовСЗВ_6_4", "ФормаСЗВ_6_4", МассивОбъектов,
		ЭтотОбъект, Новый Структура);
...
Процедура РаспечататьАДВ65(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПачкаДокументовСЗВ_6_4", "ФормаАДВ_6_5", МассивОбъектов,
		ЭтотОбъект, Новый Структура);
...
Процедура ПриОтветеНаВопросОЗаполнении(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если Отказ Тогда
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Отправка прервана. В отчете РСВ-1 найдены ошибки. Проверьте корректность заполнения отчета и повторите снова.'"));

		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтправкаВКонтролирующийОрганПослеОтправки", ЭтотОбъект);
...
Процедура ЗаписьНаДискПослеПодключенияРасширенияРаботыСФайлами(Подключено, Параметры) Экспорт
	...
	Для Каждого ОписаниеФайла Из МассивФайлов Цикл
		ПолучаемыйФайл = Новый ОписаниеПередаваемогоФайла(ВРег(ОписаниеФайла.ИмяФайла),
			ОписаниеФайла.СсылкаНаДвоичныеДанныеФайла);
		ПолучаемыеФайлы.Добавить(ПолучаемыйФайл);
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ДокументОтчетности) Тогда
		МассивСохраняемыхОтчетов = Новый СписокЗначений;
		МассивСохраняемыхОтчетов.Добавить(ДокументОтчетности);
		
		Ошибки = Новый Массив;
		
		ФайлВыгрузкиВоВременномХранилище = Новый Структура;
		РегламентированнаяОтчетностьКлиент.ВыгрузитьОтчеты(МассивСохраняемыхОтчетов, , Ошибки, , Истина,
			ФайлВыгрузкиВоВременномХранилище);
		Если Ошибки.Количество() > 0 Тогда
			ПоказатьПредупреждение( , НСтр(
				"ru = 'Сохранение прервано: в отчете РСВ-1 найдены ошибки.
				|Проверьте корректность заполнения отчета и повторите снова.'"));
			Возврат;
		КонецЕсли;
		Если ФайлВыгрузкиВоВременномХранилище.Свойство("АдресФайлаВыгрузкиВоВременномХранилище")
			...
			ПолучаемыйФайл = Новый ОписаниеПередаваемогоФайла(ВРег(ФайлВыгрузкиВоВременномХранилище.ИмяФайлаВыгрузки),
				ФайлВыгрузкиВоВременномХранилище.АдресФайлаВыгрузкиВоВременномХранилище);
			ПолучаемыеФайлы.Добавить(ПолучаемыйФайл);
		КонецЕсли;
...
Процедура УстановитьТекущуюЗакладку(ЭтотОбъект, Объект)
	Если Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.ОтчетНеСдан") Тогда
		
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ЗадачаОзнакомиться;
		
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Ознакомиться") Тогда
		
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ЗадачаОтредактировать;
		ЭтотОбъект.Элементы.ПереходСдатьОтчетность.КнопкаПоУмолчанию = Истина;
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ОтчетностьВПФР;
		ЭтотОбъект.Элементы.ПереходВыполнил.КнопкаПоУмолчанию = Истина;
		
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Завершить") Тогда
		
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ЗадачаПроверена;
		ЭтотОбъект.Элементы.Далее.КнопкаПоУмолчанию = Истина;
		
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Завершено") Тогда
		
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ЗадачаВыполнена;
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.ПолучитьПодтверждение") Тогда
		...
		ЭтотОбъект.Элементы.СтраницаОтчетностиВПФР.ТекущаяСтраница = ЭтотОбъект.Элементы.ЗадачаОжидатьПодтверждения;
...
Функция ПолучитьМассивОписанийФайловКомплекта(ИмяФайлаВыгрузкиРСВ)
	...
	Если ЗначениеЗаполнено(ДокументАДВ) Тогда
		...
		СтруктураОписьСЗВ.Вставить("СсылкаНаДвоичныеДанныеФайла", УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиАДВ62(ДокументАДВ));
	...
	Если ЗначениеЗаполнено(ДокументСЗВ62) Тогда
		...
		СтруктураСЗВ62.Вставить("СсылкаНаДвоичныеДанныеФайла", УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиСЗВ62(ДокументСЗВ62));
	...
	ИначеЕсли ЗначениеЗаполнено(ДокументСЗВ64) Тогда
		...
		СтруктураСЗВ64.Вставить("СсылкаНаДвоичныеДанныеФайла", УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиСЗВ64(ДокументСЗВ64));
	...
	Если ЗначениеЗаполнено(ДокументСЗВ63) Тогда
		...
		СтруктураСЗВ63.Вставить("СсылкаНаДвоичныеДанныеФайла", УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиСЗВ63(ДокументСЗВ63));
	...
	Если ЗначениеЗаполнено(ДокументРаздел6РасчетаРСВ1) Тогда
		...
		СтруктураРаздела6РасчетаРСВ1.Вставить("СсылкаНаДвоичныеДанныеФайла", УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиРаздела6РСВ1(ДокументРаздел6РасчетаРСВ1));
...
Процедура ОбновитьОтправку(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОбновитьОтправкуИзПанелиОтправки(ЭтотОбъект, "ПФР");
КонецПроцедуры

&НаКлиенте
Процедура ГиперссылкаПротоколНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьПротоколИзПанелиОтправки(ЭтотОбъект, "ПФР");
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьНеотправленноеИзвещение(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОтправитьНеотправленноеИзвещениеИзПанелиОтправки(ЭтотОбъект, "ПФР");
КонецПроцедуры

&НаКлиенте
Процедура ЭтапыОтправкиНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьСостояниеОтправкиИзПанелиОтправки(ЭтотОбъект, "ПФР");
КонецПроцедуры

&НаКлиенте
Процедура КритическиеОшибкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьКритическиеОшибкиИзПанелиОтправки(ЭтотОбъект, "ПФР");
КонецПроцедуры
