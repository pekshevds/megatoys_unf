...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_УведомлениеОСпецрежимахНалогообложения" Тогда
		СозданиеУведомленияОкончание(Источник,);		
	КонецЕсли;
...
Процедура ПолучитьДанныеОтчетности()
	...
	Если УпрощенныйЗачетФиксированныхВзносов Тогда
		...
		ДатаДокументаОбработкиСобытия,
		Истина);
		ВзносСДоходов = РегламентированнаяОтчетностьУСН.ВзносыСДоходовПодлежащиеУплате(Объект.Организация, ДатаДокументаОбработкиСобытия).Всего;
	КонецЕсли;	
...
Процедура СформироватьУведомлениеУменьшениеНаСтраховыеВзносы(Команда)
	...
	ОткрытьФорму(
	ДанныеПомощника.ПутьКФорме, ПараметрыОткрытия, , , , ,);   	
