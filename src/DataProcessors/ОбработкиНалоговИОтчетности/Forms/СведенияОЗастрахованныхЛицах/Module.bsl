...
Функция ПолучитьИмяИАдресФайла(ДокументОтчетности)
	...
	Возврат Новый Структура("ИмяФайла, АдресФайла",
			...
			УчетСтраховыхВзносовУНФ.СохранитьФайлВыгрузкиСЗВМ(ДокументОтчетности));
КонецФункции
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ПФР принял СЗВ-М?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что СЗВ-М заполнены полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
