...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НалоговыйПериодПропущен = РегламентированнаяОтчетностьУСН.НалоговыйПериодПропущен(Объект.Организация,
	ДатыСобытия.ДатаОкончанияДокументов, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация, "ДатаРегистрации"));
	...
	ЗаписьКалендаряУплата = НайтиЗаписьКалендаряУплата();
...
Функция НайтиЗаписьКалендаряУплата()
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ИФНС приняла декларацию?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что Декларация заполнена полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ДекорацияОписаниеРазеделовНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок",НСтр("ru='Декларация по УСН'"));
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Завершено") Тогда
		Если ЭтаФорма.НалоговыйПериодПропущен Тогда
			ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.ЗадачаВыполненаБезДекларации;
		Иначе
