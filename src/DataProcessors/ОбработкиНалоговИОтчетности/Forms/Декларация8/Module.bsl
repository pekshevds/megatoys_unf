...
Процедура СохранитьФайлВыгрузки(Команда)
	...
	ВыполнитьКомандуФормыПоИмениАЛКО("Выгрузить");
	
КонецПроцедуры

&НаКлиенте
Процедура РаспечататьДекларацию(Команда)
	ВыполнитьКомандуФормыПоИмениАЛКО("ПоказатьБланк");
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьКомандуФормыПоИмениАЛКО(ИмяКоманды)
	...
	Форма.ИнициализацияНаСервере(Истина);
...
&НаКлиенте
Процедура ПереходКДекларации8(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.АлкогольнаяДекларация8;
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ФСРАР приняла Декларацию №8?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что Декларация №8 заполнена полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.АлкогольнаяДекларация8;
