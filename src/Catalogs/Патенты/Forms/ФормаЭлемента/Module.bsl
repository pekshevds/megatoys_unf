...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(Объект.НомерПатента) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.НомерПатента", , Отказ);
	КонецЕсли;
...
Процедура СоздатьЗадачиПоОплате() Экспорт
	...
	Если СуммаНалога = 0 Тогда
		Возврат;
	КонецЕсли;
	НачалоВторогоКвартала = Дата('20200401');
	КонецВторогоКвартала = Дата('20200630');
	...
	Если ОбъектПерваяЗадача.ДатаОкончанияСобытия >= НачалоВторогоКвартала
		И ОбъектПерваяЗадача.ДатаОкончанияСобытия <= КонецВторогоКвартала Тогда
		ОбъектПерваяЗадача.ДатаОкончанияСобытияДляПострадавших = ДобавитьМесяц(ОбъектПерваяЗадача.ДатаОкончанияСобытия, 4);
	КонецЕсли;
	...
	Иначе
		...
		Если ОбъектВтораяЗадача.ДатаОкончанияСобытия >= НачалоВторогоКвартала
			И ОбъектВтораяЗадача.ДатаОкончанияСобытия <= КонецВторогоКвартала Тогда
			ОбъектВтораяЗадача.ДатаОкончанияСобытияДляПострадавших = ДобавитьМесяц(ОбъектВтораяЗадача.ДатаОкончанияСобытия, 4);
		КонецЕсли;
	...
	Если ЗаписьПервойЗадачи = Неопределено Тогда
		...
		ЗаписьКалендаряПодготовкиОтчетности.СуммаНалога = Объект.СуммаПервогоПлатежа;
		ЗаписьКалендаряПодготовкиОтчетности.ЭтоТочныйРасчет = Истина;
	...
	Если НЕ ОдинПлатеж И ЗаписьВторойЗадачи = Неопределено Тогда
		...
		ЗаписьКалендаряПодготовкиОтчетности.Состояние = Перечисления.СостоянияСобытийКалендаря.Заполнить;
		ЗаписьКалендаряПодготовкиОтчетности.СуммаНалога = Объект.СуммаВторогоПлатежа;
		ЗаписьКалендаряПодготовкиОтчетности.ЭтоТочныйРасчет = Истина;
		ЗаписьКалендаряПодготовкиОтчетности.Записать();
	КонецЕсли;
...
Функция ПолучитьДанныеЗадачи(Событие)
	...
	Результат.ИмяФормыЗадачи = Справочники.ЗаписиКалендаряПодготовкиОтчетности.ПолучитьИмяФормыПоЗадачеИСостоянию(Событие.Задача, Результат.СтруктураСобытия);
