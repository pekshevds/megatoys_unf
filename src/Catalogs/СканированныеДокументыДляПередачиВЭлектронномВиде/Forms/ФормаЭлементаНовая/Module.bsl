...
&НаКлиенте
Процедура ЗакрытьПанельПроверьтеPDFНажатие(Элемент)
	БольшеНеПоказыватьИнформационнуюПанель();
КонецПроцедуры

&НаКлиенте
Процедура ПроверьтеPDFОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Справочник.СканированныеДокументыДляПередачиВЭлектронномВиде.Форма.ПорядокУказанияЭлПодписи");
...
Процедура ЗаполнитьПредставлениеЭлПодписи(Строка)
	...
	Иначе
		Строка.ПредставлениеЭлПодписи = Строка.ИмяФайлаПодписи;
	КонецЕсли;
...
Процедура ЗаписатьФайлыФормыВБазу()
	...
	Для каждого СтрокаТаблицыФайлов Из ТаблицаФайлов Цикл
		...
		Если ЭтоФормат_ON_DOCNPNO_1_886_00_05_02 Тогда
			ОбновитьПодпись(ПрисоединенныйФайлСсылка, СтрокаТаблицыФайлов.ИмяФайлаПодписи, СтрокаТаблицыФайлов.АдресФайлаПодписи);
		КонецЕсли;
		
	КонецЦикла;
	
	Если НЕ ЭтоФормат_ON_DOCNPNO_1_886_00_05_02 Тогда
		ОбновитьПодпись(ПрисоединенныйФайлСсылка, ИмяФайлаПодписи, АдресФайлаПодписи);
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьПодпись(ПрисоединенныйФайлСсылка, ИмяФайлаПодписи, АдресФайлаПодписи)
	
	Попытка
	
		// Очищаем перед добавлением
		ОчиститьПодписьПрисоединенногоФайла(ПрисоединенныйФайлСсылка);
		ДобавитьПодписьКПрисоединенномуФайлу(ПрисоединенныйФайлСсылка, ИмяФайлаПодписи, АдресФайлаПодписи);
...
&НаСервере
Процедура ДобавитьПодписьКПрисоединенномуФайлу(ПрисоединенныйФайлСсылка, ИмяФайлаПодписи, АдресФайлаПодписи)
...
&НаСервере
Процедура ОчиститьПодписьПрисоединенногоФайла(ПрисоединенныйФайлСсылка)
...
Процедура УправлениеЭУПроверьтеPDF() 
	...
	Элементы.ПанельПроверьтеPDF.Видимость = Видимость;
...
Процедура УправлениеФормойПриОткрытии()
	...
	Элементы.Организация.Видимость = НЕ ЭтоЗагрузкаСканаНапрямуюИзОписи;
