...
Функция ИдентификаторКлассификатораВСервисеИнтернетПоддержки() Экспорт
	...
	КлассификаторКТРУПереопределяемый.ИдентификаторКлассификатораВСервисеИнтернетПоддержки(Идентификатор);
	...
	Для Каждого МетаданныеФайла Из ФайлыКлассификатора Цикл
		...
		Если ЭтоФайлКТРУ(МетаданныеФайла.Файл) Тогда
			...
			Исключение
				...
				ИмяСобытия = НСтр("ru = 'КТРУ.Преобразование данных файла КТРУ.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого МетаданныеФайла Из ФайлыКлассификатора Цикл
		...
		Если ЭтоФайлОКПД2(МетаданныеФайла.Файл) Тогда
			...
			Исключение
				...
				ИмяСобытия = НСтр("ru = 'КТРУ.Преобразование данных файла ОКПД2.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЗагрузитьКТРУ(ДанныеКлассификатора, ДанныеЗагруженныхДочернихКлассификаторов)
	...
	Для Каждого Элемент Из ДанныеКлассификатора Цикл
		...
		Иначе
			ЗагрузитьЭлементКТРУ(Элемент, ДанныеЗагруженныхДочернихКлассификаторов, ЗначенияЛогическогоТипа, Результат);
		КонецЕсли;
...
Процедура ЗагрузитьОтменуКТРУ(ОписаниеОтмены, Результат)
	...
	ЭтоНовый = Не ЗначениеЗаполнено(ОписаниеОтмены.КТРУ);
	Если ЭтоНовый Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьЭлементКТРУ(ОписаниеЭлемента, ДанныеЗагруженныхДочернихКлассификаторов, ЗначенияЛогическогоТипа, Результат)
	
	НачатьТранзакцию();
	Попытка
		
		РезультатЗаписиКТРУ = НайтиСоздатьОбновитьКТРУ(ОписаниеЭлемента, ДанныеЗагруженныхДочернихКлассификаторов);
...
Функция НайтиСоздатьОбновитьКТРУ(ОписаниеЭлемента, ДанныеЗагруженныхДочернихКлассификаторов)
	
	ЭтоНовый = Не ЗначениеЗаполнено(ОписаниеЭлемента.КТРУ);
	Если ЭтоНовый Тогда
	...
	КлассификаторКТРУПереопределяемый.ЗаполнитьОбъектДаннымиДочернихКлассификаторов(КТРУОбъект, ОписаниеЭлемента.ДочерниеКлассификаторы, ДанныеЗагруженныхДочернихКлассификаторов);
...
Функция КонвертироватьОписаниеХарактеристики(ОписаниеХарактеристикиИзФайла, ЗначенияЛогическогоТипа)
	...
	Если ОписаниеХарактеристикиИзФайла.Количество() < 7 Тогда
		НаименованиеХарактеристики                     = ОписаниеХарактеристикиИзФайла[0];
		ЧисловаяХарактеристика                         = ОписаниеХарактеристикиИзФайла[1] = ПризнакЧисловойХарактеристики;
		ВидХарактеристики                              = Перечисления.ВидыХарактеристикКТРУ[ОписаниеХарактеристикиИзФайла[2] - 1];
		Актуально                                      = ОписаниеХарактеристикиИзФайла[3] = 1;
		ОбязательноКПрименению                         = ОписаниеХарактеристикиИзФайла[4] = 1;
		СтруктураОписанийЗначенийХарактеристикиИзФайла = ОписаниеХарактеристикиИзФайла[5];
	Иначе
		НаименованиеХарактеристики                     = ОписаниеХарактеристикиИзФайла[0];
		ЧисловаяХарактеристика                         = ОписаниеХарактеристикиИзФайла[2] = ПризнакЧисловойХарактеристики;
		ВидХарактеристики                              = Перечисления.ВидыХарактеристикКТРУ[ОписаниеХарактеристикиИзФайла[3] - 1];
		Актуально                                      = ОписаниеХарактеристикиИзФайла[4] = 1;
		ОбязательноКПрименению                         = ОписаниеХарактеристикиИзФайла[5] = 1;
		СтруктураОписанийЗначенийХарактеристикиИзФайла = ОписаниеХарактеристикиИзФайла[6];
	КонецЕсли;

	ОписаниеХарактеристики = Новый Структура;
	ОписаниеХарактеристики.Вставить("Наименование"              , НаименованиеХарактеристики);
	ОписаниеХарактеристики.Вставить("Актуально"                 , Актуально);
	ОписаниеХарактеристики.Вставить("ВидХарактеристики"         , ВидХарактеристики);
	ОписаниеХарактеристики.Вставить("ОбязательноКПрименению"    , ОбязательноКПрименению);
	...
	Если ЧисловаяХарактеристика Тогда
		...
		ОписанияЗначенийХарактеристикиИзФайла = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураОписанийЗначенийХарактеристикиИзФайла, "rangeSet");
		...
		Иначе
			...
			ОписанияЗначенийХарактеристикиИзФайла = СтруктураОписанийЗначенийХарактеристикиИзФайла.valueSet;
	...
	Иначе
		...
		ОписанияЗначенийХарактеристикиИзФайла = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураОписанийЗначенийХарактеристикиИзФайла, "qDesc");
