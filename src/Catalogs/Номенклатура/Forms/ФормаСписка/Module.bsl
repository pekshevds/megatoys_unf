...
Процедура ОтборИерархияПриАктивизацииСтроки(Элемент)
	...
	Если ОтборИерархияТекущая = НовоеЗначение Тогда
		Возврат
	КонецЕсли;
...
Процедура ВосстановитьНастройки()
	...
	Если Не РежимВыбораГруппы Тогда
...
Процедура ОбновитьОстаткиПоНоменклатуреКлиент()
	...
	ОбновитьПанельОстатков(Номенклатура, ТекущиеДанные.ЕдиницаИзмерения);
...
Процедура УстановитьВидимостьИДоступность(ЭтоСозданиеФормы = Ложь)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборОрганизация", "Видимость", Элементы.ОтборСклад.Видимость И Не УчетПоКомпанииВЦелом И ИспользоватьНесколькоОрганизаций);
...
Функция ДанныеПоПоступлениям(Ссылка)
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		СУММА(ЗаказыПоставщикамОстатки.КоличествоОстаток - ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0)) КАК Количество,
