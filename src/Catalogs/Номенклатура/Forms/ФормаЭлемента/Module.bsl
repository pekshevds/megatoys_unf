...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.Склад.Видимость И Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ГиперссылкаОткрытьФормуНастройкиПараметровНоменклатурыИСМП"
		И Источник = ЭтотОбъект Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ОповещениеДействия = Новый ОписаниеОповещения(
			"ФормаНастройкиПараметровИСМПЗавершение",
			ИнтеграцияИСМПУНФКлиент,
		...
		Если Параметр.Гиперссылка = "ГиперссылкаОткрытьФормуНастройкиПараметровНоменклатурыИСМПДляПросмотра" Тогда
		...
		ИнтеграцияИСМПКлиент.ОбработкаДействияНастройкиНоменклатуры(
			ОповещениеДействия,
...
Процедура ВидПродукцииИСПриИзменении(Элемент)
	...
	Объект.ПризнакПредметаРасчета = ПолучитьОжидаемыйПризнакПредметаРасчета(Объект.ТипНоменклатуры, Объект.ВидПродукцииИС, Объект.ВидМаркировки, Объект.ПодакцизныйТовар);
...
Процедура ТипНоменклатурыПриИзмененииСервер()
	...
	// ПризнакПредметаРасчета
	ТребуетсяПризнакПредметаРасчета = РозничныеПродажиСервер.ТребуетсяПризнакПредметаРасчета(ТипНоменклатуры);
...
//+mega
&НаКлиенте
Процедура mega_ЕдиницаИзмеренияМестПриИзменении(Элемент)
	
	mega_ЕдиницаИзмеренияМестПриИзмененииСервер()	
КонецПроцедуры

&НаСервере
Процедура mega_ЕдиницаИзмеренияМестПриИзмененииСервер()
	
	Если Не ЗначениеЗаполнено(Объект.mega_ЕдиницаИзмеренияМест) Тогда
		Объект.mega_КоэффициентЕдиницыМест = 1;
	Иначе
		Объект.mega_КоэффициентЕдиницыМест = Объект.mega_ЕдиницаИзмеренияМест.Коэффициент;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура mega_ЕдиницаИзмеренияМестНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ТипНоминалаРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура("СтрокаПоиска", "Серии");
...
Функция ОписаниеЗапросаАктуальныеЦены(Запрос, Области)
	...
	Если ФОИспользуютсяНаборы И ЭтоНаборБезРаспределенияЦен(Объект) Тогда
		...
		|						ИЗ
		|							СоставНабора)
		|					И Актуальность И НЕ ВидЦен.Недействителен) КАК ЦеныНоменклатурыСрезПоследних
...
Процедура УстановитьПризнакПодакцизныйТовар()
	...
	Объект.ПодакцизныйТовар = ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(Объект.ВидПродукцииИС)
...
Процедура ПослеВыбораРеквизитовДляРазблокирования(РазблокируемыеРеквизиты, Контекст) Экспорт
	...
	Если РазблокируемыеРеквизиты.Найти("ЕдиницаИзмерения") <> Неопределено Тогда
		ИнтеграцияИСМПУНФКлиентСервер.ЗаполнитьДанныеНастройкиПараметровНоменклатурыИСМП(ЭтотОбъект, Истина);
	КонецЕсли;
