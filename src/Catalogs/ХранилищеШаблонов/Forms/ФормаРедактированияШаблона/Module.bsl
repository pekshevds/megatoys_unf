...
Процедура ОповещениеЗаполнитьШаблонИСМППоУмолчанию(Результат, ДополнительныеПараметры) Экспорт
	...
	ТипКода = ДополнительныеПараметры.ТипКода;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипШаблона = Перечисления.ТипыШаблонов.ЭтикеткаКодМаркировкиИСМП Тогда
		...
		ПечатьЭтикетокИСМПУНФ.ДобавитьПоляКомпоновкиИдентификаторовШтрихкодовУпаковок(СхемаКомпоновкиДанных);
		...
		Элементы.ФормаЗагрузитьШаблонШтрихкодыУпаковокПоУмолчанию.Видимость = Ложь;
		
	ИначеЕсли ТипШаблона = Перечисления.ТипыШаблонов.ЭтикеткаДляШтрихкодовУпаковок Тогда
		
		СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаШтрихкодыУпаковок");
		ПечатьЭтикетокИСМПУНФ.ДобавитьПоляКомпоновкиИдентификаторовШтрихкодовУпаковок(СхемаКомпоновкиДанных);
		
		ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона этикетки штрихкода упаковки'");
		Элементы.ГруппаШаблоныПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонПодарочногоСертификатаПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонДисконтнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонРегистрационнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонИСМППоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонИСМПУпаковкаПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонШтрихкодыУпаковокПоУмолчанию.Видимость = Истина;
	...
	ПолеНабораДанных = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ШтрихАвтопроверки");
...
Процедура ЗагрузитьШаблонИСМППоУмолчанию(Команда)
	...
	СписокМакетов.Добавить("ШаблонЭтикетки_58х40_ИС", "58х40");
	СписокМакетов.Добавить("ШаблонЭтикетки_40х58_ИС", "40х58");
	СписокМакетов.Добавить("ШаблонЭтикетки_58х34_ИС", "58х34");
	СписокМакетов.Добавить("ШаблонЭтикетки_34х58_ИС", "34х58");
	СписокМакетов.Добавить("ШаблонЭтикетки_144х58_ИС", "144х58");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Шаблоны", СписокМакетов);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТипКода", 18);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполнитьШаблонИСМППоУмолчанию", ЭтотОбъект, ДополнительныеПараметры);
	
	ОткрытьФорму("Справочник.ХранилищеШаблонов.Форма.ФормаВыбораШаблона", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения);

КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьШаблонИСМПУпаковкаПоУмолчанию(Команда)
	
	СписокМакетов = Новый СписокЗначений;
	СписокМакетов.Добавить("ШаблонЭтикеткиУпаковки_58х40_ИС", "58х40");
	СписокМакетов.Добавить("ШаблонЭтикеткиУпаковка_40х58_ИС", "40х58");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Шаблоны", СписокМакетов);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТипКода", 2);
...
&НаКлиенте
Процедура ЗагрузитьШаблонШтрихкодаУпаковкиПоУмолчанию(Команда)
	
	СписокМакетов = Новый СписокЗначений;
	СписокМакетов.Добавить("ШаблонЭтикетки_100х50_ШтрихкодыУпаковок", "100х50");
	СписокМакетов.Добавить("ШаблонЭтикетки_100х100_ШтрихкодыУпаковок", "100х100");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Шаблоны", СписокМакетов);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТипКода", 99);
...
Процедура ПоместитьВТабличныйДокументШаблонПоУмолчанию(ИмяШаблона)
	...
	Если ИмяШаблона = "ШаблонЦенникаАкционного" Тогда
		
		СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаЦенникАкционный");
		
	ИначеЕсли Прав(ИмяШаблона, 3) = "_ИС" Тогда
		
		СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаИСМП");
		ПечатьЭтикетокИСМПУНФ.ДобавитьПоляКомпоновкиИдентификаторовШтрихкодовУпаковок(СхемаКомпоновкиДанных);
		
	ИначеЕсли Прав(ИмяШаблона, 18) = "_ШтрихкодыУпаковок" Тогда
