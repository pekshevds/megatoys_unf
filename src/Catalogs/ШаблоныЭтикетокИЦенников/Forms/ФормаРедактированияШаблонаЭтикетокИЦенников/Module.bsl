	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не установлена область печати'"), ЭтотОбъект, "ПолеТабличногоДокумента", , Отказ);
			Возврат;
		КонецЕсли;
		...
		Если НЕ Отказ Тогда
			Оповестить("ИзмененШаблон", АдресШаблона, ЭтотОбъект);
		КонецЕсли;
...
Процедура ЭтикеткаПоУмолчанию(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ШаблонПоУмолчаниюЗавершение", ЭтотОбъект, "ШаблонЭтикеткиПоУмолчанию");
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?'"),
...
Процедура ЦенникПоУмолчанию(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ШаблонПоУмолчаниюЗавершение", ЭтотОбъект, "ШаблонЦенникаПоУмолчанию");
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?'"),
		РежимДиалогаВопрос.ДаНет);
...
Функция ПолучитьСтруктуруШаблона()
	...
	ШаблонОбъекта = ?(ПустаяСтрока(Параметры.АдресШаблона), Неопределено, ПолучитьИзВременногоХранилища(
		Параметры.АдресШаблона));
...
Функция ПроверитьУмещение()
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Лево = 0 И ПолеТабличногоДокумента.ОбластьПечати.Право = 0) Тогда
		...
		Если КоличествоПоГоризонтали <> МассивТаблиц.Количество() Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Максимальное количество по горизонтали: %1'"),
				МассивТаблиц.Количество());
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "КоличествоПоГоризонтали", , Ошибка);
		КонецЕсли;
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Верх = 0 И ПолеТабличногоДокумента.ОбластьПечати.Низ = 0) Тогда
		...
		Если КоличествоПоВертикали <> МассивТаблиц.Количество() Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "КоличествоПоВертикали", , Ошибка);
		КонецЕсли;
...
// Процедура помещает в табличный документ рисунок штрихкода.
//
&НаСервере
...
Процедура ВставитьРисунокЛоготипа(ИмяТекущейОбласти)
	...
	ИмяМакетаКартинкаЛоготип = СтрШаблон("КартинкаЛоготип_%1", ОбщегоНазначения.КодОсновногоЯзыка());
	Логотип = Новый Картинка(Справочники.Организации.ПолучитьМакет(ИмяМакетаКартинкаЛоготип));
	...
	ПолеТабличногоДокумента.Рисунки[Индекс].Имя = ПолучитьИмяПараметраЛоготип() + СтрЗаменить(
		Новый УникальныйИдентификатор, "-", "_");
	ПолеТабличногоДокумента.Рисунки[Индекс].Расположить(ПолеТабличногоДокумента.Область(ИмяТекущейОбласти));
	ПолеТабличногоДокумента.Рисунки[Индекс].ГраницаСверху = Ложь;
	ПолеТабличногоДокумента.Рисунки[Индекс].ГраницаСлева = Ложь;
	ПолеТабличногоДокумента.Рисунки[Индекс].ГраницаСнизу = Ложь;
...
Процедура ВыборДоступногоПоля(ВыбраннаяСтрока)
	...
	Если ТипЗнч(ПолеТабличногоДокумента.ТекущаяОбласть) <> Тип("ОбластьЯчеекТабличногоДокумента") Тогда
		ПоказатьПредупреждение(Неопределено, НСтр(
			"ru = 'Для переноса поля шаблона нужно выделить ячейку или область ячеек.'"));
		Возврат;
	Иначе
		ТекущаяОбласть = ПолеТабличногоДокумента.ТекущаяОбласть;
		ОбъединитьОбласть(ТекущаяОбласть.Имя);
	...
	// Подготовка данных.
	ИмяПоляВШаблоне = Строка(КомпоновщикНастроек.Настройки.ДоступныеПоляПорядка.ПолучитьОбъектПоИдентификатору(
		ВыбраннаяСтрока).Поле);
	
	// Размещение поля в шаблоне.
	Если ИмяПоляВШаблоне = ПолучитьИмяПараметраШтрихкод() Тогда
		
		Оповещение = Новый ОписаниеОповещения("ВыборДоступногоПоляЗавершение", ЭтотОбъект, ИмяПоляВШаблоне);
	...
	Иначе
		...
		ТекущаяОбласть.Текст = СтрШаблон("%1[%2]", ТекущаяОбласть.Текст, ИмяПоляВШаблоне);
