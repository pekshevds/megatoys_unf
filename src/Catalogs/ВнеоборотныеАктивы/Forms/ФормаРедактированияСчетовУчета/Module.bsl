...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СчетУчетаНДСПоРеализации.Видимость = ПолучитьФункциональнуюОпцию("СебестоимостьБезНДС");
	
	Если ОтказИзменитьСчетУчета(Ссылка) Тогда
		Элементы.Пояснение.Заголовок = НСтр("ru = 'В базе есть движения по этому внеоборотному активу: изменение счетов учета запрещено.'");
...
Процедура ОповеститьОбИзмененииСчетовРасчетов()
	...
	СтруктураПараметры = Новый Структура("СчетУчета, СчетАмортизации, СчетУчетаНДСПоРеализации", СчетУчета, СчетАмортизации, СчетУчетаНДСПоРеализации);
