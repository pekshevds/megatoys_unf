...
Процедура ВыполнитьЗадачу(Предмет, Источник, ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		...
		ЭтоНалог = Истина;
	ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыЗаСотрудников Тогда
		ТекстНалог = НСтр("ru = 'страховые взносы за сотрудников'");
		ЭтоНалог = Истина;		
	...
	ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация8  Тогда
		ТекстНалог = НСтр("ru = 'декларацию №8 (розничная продажа пива и пр.)'");	     
	ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ПерсСведенияОФизлицах  Тогда
		ТекстНалог = НСтр("ru = 'персонифицированные сведения о физических лицах'");
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыЗаСотрудников Тогда
			...
			РезультатРасчета = РегламентированнаяОтчетностьСотрудники.ВыполнитьРасчетВзносовПоСотрудникам(Организация,ДатаДокументаОбработкиСобытия, ДополнительныеПараметры.СобытиеКалендаря);
			СуммаНалога = РезультатРасчета.СуммаНалога;
			ИмяФормыЗадачи = "Обработка.ОбработкиНалоговИОтчетности.Форма.НалогиСотрудниковСтраховыеВзносы";
		...
		ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ПерсСведенияОФизлицах Тогда
			ИмяФормыЗадачи = "Обработка.ОбработкиНалоговИОтчетности.Форма.ПерсСведенияОФизлицах";
			ТекстУведомления = НСтр(
				"ru = 'Почти сформировала "+ТекстНалог+ТекстОрганизации+" за "
				+ ПредставлениеПериода(
					ДополнительныеПараметры.ДатаНачалаДокументов,
					КонецДня(ДополнительныеПараметры.ДатаОкончанияДокументов), "ФП=Истина")+".
				|До завершения формирования персонифицированных сведений о физических лицах остался один шаг.
				|Сведения нужно предоставить в СФР не позднее "+Формат(ДополнительныеПараметры.ДатаОкончанияСобытия,"ДЛФ=DD")+"'");
			ТекстОсновнойКоманды = НСтр("ru = 'Посмотреть и отправить СЗВ-М'");
		ИначеЕсли Налог = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СЗВСтаж Тогда
			ИмяФормыЗадачи = "Обработка.ОбработкиНалоговИОтчетности.Форма.ОписанияЗадачИСтартВыполнения";
			ТекстУведомления = НСтр(
