...
Процедура НастройкиНажатие(Элемент)
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыНастроекИнтеграцияВыполнена",ЭтотОбъект);
	ОткрытьФорму("РегистрСведений.НастройкиИнтеграцииКонтактныхФорм.ФормаСписка",,,,,,ОповещениеОЗакрытии);
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработатьЗакрытиеФормыНастроекИнтеграцияВыполнена(Результат, ДополнительныеПараметры) Экспорт
...
Функция ОпубликоватьФормуНаСервере()
	...
	Соединение = Новый HTTPСоединение("script.google.com",443,,,,,ЗащищенноеСоединение);
