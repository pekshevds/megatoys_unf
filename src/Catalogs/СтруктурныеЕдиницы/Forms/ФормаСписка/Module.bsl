...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("ОрдерныйСклад") Тогда
		ОрдерныйСклад = Параметры.Отбор.ОрдерныйСклад;
		Параметры.Отбор.Удалить("ОрдерныйСклад");
	КонецЕсли;
	
	Если Параметры.Свойство("ИспользоватьОрдернуюСхемуПриПоступлении") Тогда
		ИспользоватьОрдернуюСхемуПриПоступлении = Параметры.ИспользоватьОрдернуюСхемуПриПоступлении;
	КонецЕсли;
	
	Если Параметры.Свойство("ИспользоватьОрдернуюСхемуПриОтгрузке") Тогда
		ИспользоватьОрдернуюСхемуПриОтгрузке = Параметры.ИспользоватьОрдернуюСхемуПриОтгрузке;
	КонецЕсли;
	
	Если Параметры.Свойство("ВидУчетаОрдерныхСкладов") Тогда
		ВидУчетаОрдерныхСкладов = Параметры.ВидУчетаОрдерныхСкладов;
	КонецЕсли;
	
	Список.Параметры.УстановитьЗначениеПараметра("ОрдерныйСклад", ОрдерныйСклад);
	
	Список.Параметры.УстановитьЗначениеПараметра("ВидУчетаОрдерныхСкладов", ВидУчетаОрдерныхСкладов);
	Список.Параметры.УстановитьЗначениеПараметра("ИспользоватьОрдернуюСхемуПриОтгрузке", ИспользоватьОрдернуюСхемуПриОтгрузке);
	Список.Параметры.УстановитьЗначениеПараметра("ИспользоватьОрдернуюСхемуПриПоступлении", ИспользоватьОрдернуюСхемуПриПоступлении);
	...
	СуммовойУчет = ПолучитьФункциональнуюОпцию("СуммовойУчетВРознице");
	...
	Иначе
		...
		Если НЕ (Параметры.Свойство("ВидОперации") И Параметры.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение) Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЭтаФорма.Список, "СкладУправляющейСистемы", Ложь,,, Истина);
		КонецЕсли;
	...
	Список.Параметры.УстановитьЗначениеПараметра("СписокОграниченийПоТипу", СписокОграниченийПоТипу);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) 
	...
	Если РежимВыбора Тогда
		...
		Если ОрдерныйСклад И Не ТекущиеДанные.ОрдерныйСклад Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Для выбора доступны только ордерные склады.'"));
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
		
		Если ТекущиеДанные.ОрдерныйСклад И ИспользоватьОрдернуюСхемуПриПоступлении И Не ТекущиеДанные.ИспользоватьОрдернуюСхемуПриПоступлении Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Для выбора доступны только ордерные склады с установленным признаком - Использовать при поступлении.'"));
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
		
		Если ТекущиеДанные.ОрдерныйСклад И ИспользоватьОрдернуюСхемуПриОтгрузке И Не ТекущиеДанные.ИспользоватьОрдернуюСхемуПриОтгрузке Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Для выбора доступны только ордерные склады с установленным признаком - Использовать при отгрузке.'"));
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
		
		Если ТекущиеДанные.ОрдерныйСклад И ВидУчетаОрдерныхСкладов = ПредопределенноеЗначение("Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом")
			И Не ТекущиеДанные.ВидУчетаОрдерныхСкладов = ПредопределенноеЗначение("Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом") Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Для выбора доступны только ордерные склады с видом учета - По складу в целом.'"));
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
...
Функция СписокТиповДоступныхДляВыбора()
	...
	Если УчетРозничныхПродаж И СуммовойУчет И Не СписокОграниченийПоТипу.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет")) = Неопределено Тогда
		ТипыДоступныеДляВыбора = ТипыДоступныеДляВыбора + "
...
Процедура ОбновитьНастройкиФормыСогласноРежимуОтображения(ОткрытиеФормы = Ложь)
	...
	Список.Параметры.УстановитьЗначениеПараметра("СписокОграниченийПоТипу", СписокОграниченийПоТипу);
	Список.Параметры.УстановитьЗначениеПараметра("ОграничиватьТип", ?(СписокОграниченийПоТипу.Количество(), Истина, Ложь));
	...
	Если СписокОграниченийПоТипу.Количество() Тогда
		...
		ЗначениеВидимости = Ложь;
		Если УчетРозничныхПродаж И СуммовойУчет Тогда
			Если Не СписокОграниченийПоТипу.НайтиПоЗначению(Перечисления.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет) = Неопределено Тогда
				ЗначениеВидимости = Истина;
			КонецЕсли;
		КонецЕсли;
