	...
	Если НастройкиВыгрузкиРеквизитов <> "" Тогда
		...
		Если СтруктураНастройки <> Неопределено Тогда
			Для каждого СтрокаНастроек Из СтруктураНастройки Цикл
				Если СписокДополнительныхРеквизитов <> Неопределено И СписокДополнительныхРеквизитов.Найти(СтрокаНастроек.Имя) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				СтрокаПредставление = СтрокаПредставление + СтрокаНастроек.Представление + ", ";
			КонецЦикла;
		КонецЕсли;
...
Функция ПолучитьQRКодКлиента(КодКлиента)
	...
	Возврат ПоместитьВоВременноеХранилище(ДвоичныеДанныеКартинки, УникальныйИдентификатор);
