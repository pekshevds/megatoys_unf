...
Функция СФормироватьНастройкиДляОтправки()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ИспользоватьОНас Тогда
			...
			СтруктураОНас.Вставить("СсылкаНаИнстаграм", "");
			СтруктураОНас.Вставить("СсылкаНаФейсбук", "");
		...
		Если ВыборкаДетальныеЗаписи.ИспользоватьЗаказТоваров Тогда
			...
			// Версия 2.5.0
			СтруктураЗаказТоваров.Вставить("ОтображатьТочныеОстатки", ВыборкаДетальныеЗаписи.ОтображатьТочныеОстатки);
			
			МассивСУИОрганизацийСкладскихОстатков = Новый Массив;
			ВыбранныеОрганизации = ВыборкаДетальныеЗаписи.ОрганизацииСкладскихОстатков.Выгрузить();
			Для каждого Строка Из ВыбранныеОрганизации Цикл
				МассивСУИОрганизацийСкладскихОстатков.Добавить(Строка(Строка.Организация.УникальныйИдентификатор()));
			КонецЦикла;
			СтруктураЗаказТоваров.Вставить("МассивСУИОрганизацийСкладскихОстатков", Новый ХранилищеЗначения(МассивСУИОрганизацийСкладскихОстатков, Новый СжатиеДанных(9)));
			...
			Если ВыборкаДетальныеЗаписи.ИспользоватьЯндексКассу Тогда
				...
				|	НастройкиЯндексКассы.ОтправкаЧековЧерезСервис КАК ОтправкаЧековЧерезЯндекс
				|ИЗ
				|	Справочник.НастройкиОнлайнОплат КАК НастройкиЯндексКассы
				|ГДЕ
	...
	СтруктураПакетОтправки.Вставить("ЭтоРозница", ОбщегоНазначенияУНФ.ЭтоРозница());
...
Функция ПолучитьНастройкиПубликации()
	...
	|	НастройкиПубликацииМЛК.ОтображатьТочныеОстатки КАК ОтображатьТочныеОстатки,
	|	НастройкиПубликацииМЛК.ОрганизацииСкладскихОстатков.(
	|		Ссылка КАК Ссылка,
	|		НомерСтроки КАК НомерСтроки,
	|		Организация КАК Организация
	|	) КАК ОрганизацииСкладскихОстатков
	|ИЗ
