...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Объект.ТипПоказателяРасчетов = Перечисления.ТипыПоказателейРасчета.Ручной;
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.ИмяИсточника) Тогда
		...
		МетаданныеИсточник = Метаданные[СтрЗаменить(ВидИсточникаПоМетаданным, "Регистр",
			"Регистры")][ИмяИсточникаПоМетаданным];
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ТипПоказателяРасчетов = Перечисления.ТипыПоказателейРасчета.Запрос И Не Объект.ПроизвольныйЗапрос Тогда

		Если НеВыбранПоказатель() Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Объект.ТипПоказателяРасчетов = Перечисления.ТипыПоказателейРасчета.Запрос И Объект.ПроизвольныйЗапрос Тогда
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("ИдентификаторПоказателяРасчета", Объект.ИдентификаторДляРасчетаВРасширении);
...
&НаКлиенте
Процедура ТипПараметраРасчетовПриИзменении(Элемент)

	Если Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Ручной") Тогда

		ТекстСообщения = НСтр("ru = 'После изменения настройки все реквизиты будут очищены. Продолжить?'");

		ПоказатьВопрос(Новый ОписаниеОповещения("ЗадаватьЗначениеПриРасчетеЗППриИзмененииЗавершение", ЭтотОбъект),
			ТекстСообщения, РежимДиалогаВопрос.ДаНет);
		Возврат;

	КонецЕсли;
	
	Если Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Расширение")  Тогда
		УстановитьИдентификаторДляРасчетаВРасширении();
	КонецЕсли;
...
Процедура ИсточникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ПоказателиРасчетов.Форма.ФормаВыбораИсточника", , , , , , Оповещение);

КонецПроцедуры

#КонецОбласти

#Область ОбработчикиСобытийЭлементовТаблицыФормыОтборДанныхНастройкиОтбор
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПериодыОтбораДанных

&НаКлиенте
Процедура ПериодыОтбораДанныхПриИзменении(Элемент)

	Если СтрНайти(Объект.ПредставлениеИсточника, "движения: оборот") > 0 Тогда
...
&НаКлиенте
Процедура ОбновитьПараметрыЗапроса(Команда)
...
Процедура РедактироватьЗапрос(Команда)
	...
	Если Элементы.РедактироватьЗапрос.Пометка Тогда
		...
		Иначе
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ВернутьсяКИсходномуЗапросу", ЭтотОбъект), ТекстВопроса,
...
&НаКлиенте
Процедура ШаблонНомаДнейГрафикРаботыСотрудника(Команда)
	ИнициализироватьЗаполнениеПоШаблону("НормаДнейГрафикСотрудника");
КонецПроцедуры

&НаКлиенте
Процедура ШаблонНормаЧасовГрафикСотрудника(Команда)
	ИнициализироватьЗаполнениеПоШаблону("НормаЧасовГрафикСотрудника");
КонецПроцедуры
...
Процедура ПослеРедактированияЗапросаКонструктором(ТекстЗапросаИзКонструктора, ДополнительныеПараметры) Экспорт
	...
	Если ТекстЗапросаИзКонструктора <> Неопределено И Не ПустаяСтрока(ТекстЗапросаИзКонструктора) Тогда
...
Процедура ЗадаватьЗначениеПриРасчетеЗППриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Запрос");
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьЗаполнениеПоШаблону(ИмяШаблона)
	...
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьПоШаблонуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура УправлениеСвойствомДоступность()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Источник", "Доступность",
		Не Объект.ПроизвольныйЗапрос);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроизвольныйЗапрос", "Доступность",
		ПустаяСтрока(Объект.ИмяИсточника));

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РедактироватьЗапрос", "Доступность",
		Объект.ПроизвольныйЗапрос);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КонструкторЗапроса", "Доступность",
			Объект.ПроизвольныйЗапрос И Элементы.РедактироватьЗапрос.Пометка);

	КонецЕсли;

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Запрос", "Доступность",
		Объект.ПроизвольныйЗапрос И Элементы.РедактироватьЗапрос.Пометка);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПараметрыЗапроса", "Доступность",
		Объект.ПроизвольныйЗапрос И Элементы.РедактироватьЗапрос.Пометка);

	ПараметрПоИсточнику = Не ПустаяСтрока(Объект.ИмяИсточника);

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПериодыОтбораДанных",
		"Доступность", ПараметрПоИсточнику);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Показатели",
		"Доступность", ПараметрПоИсточнику);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборДанныхНастройкиОтбор",
		"Доступность", ПараметрПоИсточнику);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборДанныхНастройкиОтборКоманднаяПанель",
		"Доступность", ПараметрПоИсточнику);
...
Процедура УстановитьТекущуюСтраницуВидимостиНаКлиенте()
	...
	Если Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Запрос") Тогда
		ТекущаяСтраницаВидимости = Элементы.СтраницаЗапросом;
	ИначеЕсли Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Расширение") Тогда
		ТекущаяСтраницаВидимости = Элементы.СтраницаРасширение;
	Иначе
		ТекущаяСтраницаВидимости = Элементы.СтраницаВручную;
	КонецЕсли;

	Элементы.УправлениеВидимостью.ТекущаяСтраница = ТекущаяСтраницаВидимости;
...
Процедура ЗаполнитьПоШаблону(ДополнительныеПараметры)
	...
	Если ИмяШаблона = "ФиксированнаяСумма" Тогда
		...
		Объект.ПроизвольныйЗапрос		= Ложь;
		Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Ручной");
	...
	ИначеЕсли ИмяШаблона = "НормаЧасовГрафикСотрудника" Тогда

		Объект.Наименование				= "Норма часов (график работы сотрудника)";
		Объект.Идентификатор 			= "НормаЧасовГрафикСотрудника";
		Объект.ПроизвольныйЗапрос		= Истина;
		Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Запрос");

		ДобавитьСтрокуВТаблицуПараметровЗапроса("ПериодРегистрации", "Период регистрации");
		ДобавитьСтрокуВТаблицуПараметровЗапроса("ГрафикРаботы", "График работы");
	...
	ИначеЕсли ИмяШаблона = "НормаДнейГрафикСотрудника" Тогда

		Объект.Наименование 			= "Норма дней (график работы сотрудника)";
		Объект.Идентификатор 			= "НормаДнейГрафикСотрудника";
		Объект.ПроизвольныйЗапрос		= Истина;
		Объект.ТипПоказателяРасчетов = ПредопределенноеЗначение("Перечисление.ТипыПоказателейРасчета.Запрос");
...
Процедура СформироватьЗапросЗаполнитьПараметры()
	...
	Для Каждого ЭлементОтбора Из ОтборДанных.Настройки.Отбор.Элементы Цикл
		СписокУсловий = СписокУсловий + ?(СписокУсловий = "", "", "
																  |	И ") + ПолучитьВидСравнения(
			ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ВидСравнения);
	КонецЦикла;
	...
	Если СтрНайти(Объект.ПредставлениеИсточника, "движения: оборот") > 0 Тогда
		...
		Если Объект.ПериодыОтбораДанных.НайтиСтроки(Новый Структура("ИмяДатаГраницы", "НачалоПериода")).Количество()
			> 0 Тогда

			УсловияПоПериоду = УсловияПоПериоду + "Период >= &НачалоПериода";

		КонецЕсли;

		Если Объект.ПериодыОтбораДанных.НайтиСтроки(Новый Структура("ИмяДатаГраницы", "КонецПериода")).Количество() > 0 Тогда

			УсловияПоПериоду = УсловияПоПериоду + ?(ПустаяСтрока(УсловияПоПериоду), "", " И ")
				+ "Период <= &КонецПериода";

		КонецЕсли;

		Если Не ПустаяСтрока(УсловияПоПериоду) Тогда

			УсловияПоПериоду = "(" + УсловияПоПериоду + ")";
			СписокУсловий = СписокУсловий + ?(СписокУсловий = "", "", "
																	  |	И ") + УсловияПоПериоду;
	...
	Для Каждого ЭлементОтбора Из ОтборДанных.Настройки.Отбор.Элементы Цикл
		...
		Если Объект.ПараметрыЗапроса.НайтиСтроки(Новый Структура("Имя", Строка(
			ЭлементОтбора.ЛевоеЗначение))).Количество() = 0 Тогда
			...
			Иначе
				...
				ПреобразоватьЗависимостиВСписке(ЭлементОтбора);
...
Процедура ЗаполнитьПараметрыПоЗапросу()
	...
	Пока НомерПодстроки > 0 Цикл
		...
		НомерВвод	 = СтрНайти(ЗапросСтрока, Ввод);
		НомерСкобки	 = СтрНайти(ЗапросСтрока, ")");
		Если НомерЗапятой = 0 И НомерПробела = 0 И НомерВвод = 0 И НомерСкобки = 0 Тогда
		...
		Иначе
			...
			КонецЕсли;
			Если НомерВвод = 0 Тогда
				НомерВвод = 9000000;
			КонецЕсли;
			Если НомерСкобки = 0 Тогда
				НомерСкобки = 9000000;
			КонецЕсли;
			КонецПараметра = МИН(НомерЗапятой, НомерПробела, НомерВвод, НомерСкобки);
...
&НаКлиенте
// Процедура формирует идентификатор параметра расчетов.
//    
...
Процедура ЗаполнитьПоказателиИПериодыОтбораДанныхИсточника()
	...
	Если Периодический Тогда
		...
		Если СтрНайти(Объект.ПредставлениеИсточника, "обороты") > 0 Или СтрНайти(Объект.ПредставлениеИсточника,
			"движения:") > 0 Тогда
...
Процедура УстановитьОтбор()
	...
	Если Не Объект.ПроизвольныйЗапрос Тогда
		...
		КопияТаблицыФормы = Объект.ПараметрыЗапроса.Выгрузить( , "Имя, Представление, ВидСравнения");
		...
		Для Каждого СтрокаОписанияОтбора Из КопияТаблицыФормы Цикл
			...
			Если СтрокаОписанияОтбора.Имя = "МоментВремени" Или СтрокаОписанияОтбора.Имя = "НачалоПериода"
				Или СтрокаОписанияОтбора.Имя = "КонецПериода" Тогда
			...
			НайденныеСтроки = Объект.ПараметрыЗапроса.НайтиСтроки(СтруктураОтбора);
			...
			ЭлементОтбора.ВидСравнения = ?(СохраненныйВидСравнения = Неопределено, ВидСравненияКомпоновкиДанных.Равно,
				СохраненныйВидСравнения);
...
&НаСервере
Процедура ПреобразоватьЗависимостиВСписке(ЭлементОтбора)
