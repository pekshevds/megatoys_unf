...
Процедура УправлениеВидимостьюДоступностью()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПредпросмотр", "Видимость", Изображения.Количество());
...
Процедура ЗаполнитьРасшифровкуПодписиНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
		...
		РасшифровкаПодписи = РегистрыСведений.ФИОФизическихЛиц.ФИОФизЛица(Период, Объект.ФизическоеЛицо);
...
Процедура ДобавитьИзображениеНаКлиенте()
	...
	Иначе
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Физическое лицо""'"), , "Объект.ФизическоеЛицо");
...
Процедура ОбработатьНажатиеПлашки(Элемент)
	...
	СтрокаИзображенияТекущиеПлашки = НайтиТекущуюСтрокуВТаблицеИзображенийНаКлиенте(Элемент.Имя);
	Если Не ЗначениеЗаполнено(ЭтаФорма[ЭтаФорма.ТекущийЭлемент.Имя]) Тогда
...
Процедура ДобавитьСтрокуТаблицыИзображенийФизическогоЛица(СсылкаНаПрисоединенныйФайл, НомерФайла, ЭтоТекущаяФаксимиле)
	...
	Если ДвоичныеДанныеКартинки <> Неопределено Тогда
		...
		НоваяСтрока.ДатаСоздания	= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаПрисоединенныйФайл, "ДатаСоздания");
...
Процедура ДобавитьИзображенияФизическогоЛицаНаСервере()
	...
	Изображения.Сортировать("ДатаСоздания");
...
Процедура ПодставитьИзображенияТройкиВПлашки(Тройка)
	...
	//Элементы.ДекорацияСледующиеКартинки.Доступность = ЗначениеЗаполнено(ЭтотОбъект.Плашка3);
	//Элементы.ДекорацияПредыдущиеКартинки.Доступность = (Тройка > 1);
	
	Элементы.ГруппаПредпросмотр.Видимость = ЗначениеЗаполнено(Плашка1) Или Тройка <> 1;
	
	Элементы.Плашка2.Видимость = ЗначениеЗаполнено(Плашка1);
	Элементы.Плашка2Отсутствует.Видимость = Не Элементы.Плашка2.Видимость;
	
	Элементы.Плашка3.Видимость = ЗначениеЗаполнено(Плашка2);
	Элементы.Плашка3Отсутствует.Видимость = Не Элементы.Плашка3.Видимость;
	
	ИзображенийБольшеДвух = Изображения.Количество() > 2;
	
	Элементы.ДекорацияПредыдущиеКартинки.Видимость = (Тройка > 1);
	Элементы.ДекорацияПредыдущиеКартинкиОтсутствуют.Видимость = НЕ Элементы.ДекорацияПредыдущиеКартинки.Видимость;
	
	Элементы.ДекорацияСледующиеКартинки.Видимость = ЗначениеЗаполнено(ЭтотОбъект.Плашка3);
	Элементы.ДекорацияСледующиеКартинкиОтсутствуют.Видимость = Не Элементы.ДекорацияСледующиеКартинки.Видимость;
...
Процедура ПросмотретьПрисоединенныйФайл()
	...
	Если Не ЗначениеЗаполнено(Объект.Факсимиле) Тогда
		Возврат
	КонецЕсли;
	
	Файл = Объект.Факсимиле;
...
&НаКлиенте
Процедура УдалитьФаксимилеПодписи(Команда)
	
	ПрисоединенныйФайл = Объект.Факсимиле;
	Если НЕ ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьФаксимилеПодписи(Команда)
...
&НаКлиенте
Процедура ПросмотретьФаксимилеПодписи(Команда)
...
&НаКлиенте
Процедура УдалитьФаксимилеПодписиИзПлажки(Команда)
	
	АдресВыбранногоИзображения = ЭтаФорма[ЭтаФорма.ТекущийЭлемент.Имя];
	
	МассивНайденныхСтрок = Изображения.НайтиСтроки(Новый Структура("Адрес", АдресВыбранногоИзображения));
	...
	ПрисоединенныйФайл = Строка.Ссылка;
	Если НЕ ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Строка.ТекущаяФаксимиле Тогда
		ОчиститьФаксимилеПодписи();
	КонецЕсли;
