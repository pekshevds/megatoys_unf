...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетаУчета(НаправлениеДеятельности) Тогда
		Элементы.Пояснение.Заголовок = НСтр("ru = 'В базе есть доходы или расходы по этому направлению деятельности: изменение счета учета выручки от продаж запрещено.'");
		Элементы.Пояснение.Видимость = Истина;
		Элементы.ГруппаСчетовУчета.ТолькоПросмотр = Истина;
		Элементы.ПоУмолчанию.Видимость = Ложь;
	КонецЕсли;
	
	Если Не ПравоДоступа("Редактирование", НаправлениеДеятельности.Метаданные()) Тогда
		Элементы.ГруппаСчетовУчета.ТолькоПросмотр = Истина;
		Элементы.ПоУмолчанию.Видимость = Ложь;
	КонецЕсли;
	
	Если НаправлениеДеятельности = Справочники.НаправленияДеятельности.Прочее Тогда
...
Процедура ОповеститьОбИзмененииСчетов()
	...
	СтруктураПараметры = Новый Структура("СчетУчетаВыручкиОтПродаж, СчетУчетаСебестоимостиПродаж, СчетУчетаПрибыли",
