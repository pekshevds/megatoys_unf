...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НастройкиВыгрузкиРеквизитов") Тогда
		...
		ЗаполнитьСписокДопРеквизитов(Параметры.НастройкиВыгрузкиДопРеквизитов, "Справочник_ХарактеристикиНоменклатуры", Не Параметры.НастройкиВыгрузкиДопРеквизитовНоменклатурыИспользовать);
		ЗаполнитьСписокДопРеквизитов(Параметры.НастройкиВыгрузкиДопРеквизитов, "Справочник_КомплектацииНоменклатуры", Не Параметры.НастройкиВыгрузкиДопРеквизитовНоменклатурыИспользовать);
		ЗаполнитьСписокДопРеквизитов(Параметры.НастройкиВыгрузкиДопРеквизитов, "Справочник_Номенклатура_Общие", Не Параметры.НастройкиВыгрузкиДопРеквизитовНоменклатурыИспользовать);
		ЗаполнитьСписокДопРеквизитов(Параметры.НастройкиВыгрузкиДопРеквизитов, "Справочник_КомплектацииНоменклатуры_Общие", Не Параметры.НастройкиВыгрузкиДопРеквизитовНоменклатурыИспользовать);
	...
	Если Параметры.Свойство("НастройкаИнтеграции") Тогда
		ЭтаФорма.НастройкаИнтеграции = Параметры.НастройкаИнтеграции;
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Для Каждого Реквизит Из СписокРеквизитов Цикл
		...
		Исключение
			ТекстОшибки = "Ошибка при заполнении дополнительного реквизита. Идентификатор некорректный: """ + Реквизит.Значение + """. Исключен из списка реквизитов.";
			ОбщегоНазначенияКлиент.СообщитьПользователю( ТекстОшибки );
			
			ОбменСВнешнимиСистемамиЛогированиеВызовСервера.ЗаписатьВЛог( ТекстОшибки, ЭтаФорма.НастройкаИнтеграции, Истина );
			Продолжить;
		КонецПопытки;
...
Процедура СнятьАктивность(Команда)
	СнятьАктивностьНаКлиенте(Элементы.СтраницыТело.ТекущаяСтраница);
КонецПроцедуры

&НаКлиенте
Процедура УстановитьАктивность(Команда)
	УстановитьАктивностьНаКлиенте(Элементы.СтраницыТело.ТекущаяСтраница);
КонецПроцедуры
#КонецОбласти
...
Процедура ЗаполнитьСписокДопРеквизитов(ТекущиеНастройки, ИмяТаблицы, ПометкаПоУмолчанию = Ложь)
	...
	Отбор = Новый Структура( "Значение" );
	...
	Для Каждого ДопРеквизит Из ТаблицаСвойств Цикл
		...
		Отбор.Значение = ИмяРеквизита;
		Если СписокРеквизитов.НайтиСтроки( Отбор ).Количество() > 0 Тогда
			// реквизит был добавлен ранее
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Процедура СнятьАктивностьНаКлиенте(ТекущаяСтраница)
...
&НаКлиенте
Процедура УстановитьАктивностьНаКлиенте(ТекущаяСтраница)
