...
//
Процедура ВыполнитьДействияПоИсточнику(Действия, Источник, Правило, Автор, ДопДанныеИсточника) Экспорт
	...
	Пока ВыборкаДействий.Следующий() Цикл
		...
		Если ТипЗнч(МенеджерИсточника) = Тип("СправочникМенеджер.РабочиеМеста") Тогда
			РеквизитыНедоступныеДляИсточника = Новый Массив;
		Иначе
		...
		ИначеЕсли ВыборкаДействий.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьEmail Тогда
			...
			ДополнительныеПараметры = Новый Структура("ПреобразовыватьHTMLДляФорматированногоДокумента", Истина);
			ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДополнительныеПараметры, ДопДанныеИсточника);
			
			Результат = ШаблоныСообщений.СформироватьСообщение(ВыборкаДействий.ШаблонСообщения,
				Источник,
		...
		ИначеЕсли ВыборкаДействий.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьSMS Тогда
			...
			Результат = ШаблоныСообщений.СформироватьСообщение(ВыборкаДействий.ШаблонСообщения, Источник, Новый УникальныйИдентификатор, ДополнительныеПараметры);
		...
		Если Не ПриемникОбъект.ПроверитьЗаполнение() Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось выполнить действие рабочего процесса: ""%1"".
				|Некорректно заполнен создаваемый объект: %2'"), ВыборкаДействий.Ссылка, МетаданныеПриемника.Представление());
		...
		Если ТребуетсяЗаписатьВложения Тогда
			Для Каждого Вложение Из Результат.Вложения Цикл
				...
				ПрисоединенныйФайлСсылка = РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, Вложение.АдресВоВременномХранилище);
				Если ЗначениеЗаполнено(Вложение.Идентификатор) Тогда
		...
		РегистрыСведений.ВыполнениеПравилРабочегоПроцесса.ЗафиксироватьВыполнениеПравила(Правило,
...
// Функция возвращает рассчитанную дату смещения
//
// Параметры:
...
Процедура ЗаполнитьУчастниковСобытия(ВыборкаНастроек, СобытиеОбъект, знач ЗначенияРеквизитовИсточника)
	...
	Если ВыборкаНастроек.НайтиСледующий(Новый Структура("ИмяРеквизита", "Контакт")) Тогда
		...
		Если ВыборкаНастроек.НайтиСледующий(Новый Структура("ИмяРеквизита", "ВидКонтактнойИнформации")) Тогда
			...
			НоваяСтрока.КакСвязаться = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(ЗначениеКонтакта, 
