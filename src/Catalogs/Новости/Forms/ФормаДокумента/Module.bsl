...
Процедура КомандаВставитьКартинку(Команда)
	...
	ФильтрФайла = "*." + ЛокальныйФорматКартинки;
	
	ПараметрыДиалога = Новый ПараметрыДиалогаПомещенияФайлов();
	ПараметрыДиалога.Заголовок = НСтр("ru = 'Выберите картинку'");
	ПараметрыДиалога.Фильтр    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Картинка (%1)|%1'"),
		ФильтрФайла);
...
//
// Параметры:
//  ОписаниеФайла - ОписаниеПомещенногоФайла, Неопределено - описание помещенного во временное хранилище файла, либо
//    Неопределено, если помещение файла было отменено.
//  ДополнительныеПараметры - Структура:
//    * ТекущаяСтрока - Число - идентификатор строки таблицы формы БинарныеДанные.
//
&НаКлиенте
Процедура ПослеЗагрузкиФайлаКакБинарныхДанных(ОписаниеФайла, ДополнительныеПараметры) Экспорт
...
//    Неопределено, если помещение файла было отменено.
//  ДополнительныеПараметры - Структура:
//    * ФорматКартинки - Строка - расширение картинки.
//
&НаКлиенте
Процедура ПослеЗагрузкиВнедреннойКартинки(ОписаниеФайла, ДополнительныеПараметры) Экспорт

	Если ОписаниеФайла <> Неопределено Тогда
		ЛокальныеДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(ОписаниеФайла.Адрес);
		...
		ТекстДляВставки = СтрЗаменить(
			ТекстДляВставки,
			"%ДанныеКартинки%",
			"data:image/" + ДополнительныеПараметры.ФорматКартинки + ";base64," + ЛокальныеДанныеФайлаСтрока64);
