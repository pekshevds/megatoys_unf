...
&НаКлиенте
Процедура НачисленияИУдержанияСчетЗатратНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ЗарплатаИПерсоналКлиент.ПараметрыВыбораСчетаЗатрат(ЭтаФорма, "НачисленияИУдержания");
...
Процедура СоздатьИтоговыйДокумент(Отказ)
	...
	Попытка
		...
		Если ПринятьНаРаботу Тогда
			...
			ЭлементБлокировкиДанных = Блокировка.Добавить("Документ.ПриемНаРаботуУНФ");
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Разделяемый;
		...
		Если ИзменитьОклад ИЛИ ИзменитьДолжность Тогда
			...
			ЭлементБлокировкиДанных = Блокировка.Добавить("Документ.КадровоеПеремещениеУНФ");
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Разделяемый;
		...
		Если ОформитьУвольнение Тогда
			...
			ЭлементБлокировкиДанных = Блокировка.Добавить("Документ.УвольнениеУНФ");
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Разделяемый;
		...
		Если ПринятьНаРаботу Тогда
			...
			Если ИспользуетсяШтатноеРасписание
				...
				Отбор = Новый Структура("Организация, СтруктурнаяЕдиница, Должность", 
					Константы.УчетПоКомпании.Компания(Организация), ПриемНаРаботуСтруктурнаяЕдиница, ПриемНаРаботуДолжность);
				...
				МенеджерЗаписи.Организация = Константы.УчетПоКомпании.Компания(Организация);
			...
			ПриемНаРаботуОбъект					= Документы.ПриемНаРаботуУНФ.СоздатьДокумент();
		...
		Если ОформитьУвольнение Тогда
			...
			УвольнениеОбъект					= Документы.УвольнениеУНФ.СоздатьДокумент();
		...
		Если ИзменитьДолжность Тогда
			...
			КадровоеПеремещениеОбъект					= Документы.КадровоеПеремещениеУНФ.СоздатьДокумент();
...
&НаКлиенте
Процедура ПриПереходеДалее(Отказ)
	Перем Ошибки;
...
Процедура ВыполнитьКонтрольШтатногоРасписания(СтруктураДанных, Ошибки, Отказ)
	...
	Запрос.УстановитьПараметр("Организация",	Константы.УчетПоКомпании.Компания(СтруктураДанных.Организация));
	...
	ИначеЕсли НЕ ВыборкаКоличествоСтавок.Следующий() Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, 
			Константы.УчетПоКомпании.Компания(СтруктураДанных.Организация), СтруктураДанных.СтруктурнаяЕдиница, СтруктураДанных.Должность);
...
Процедура ПолучитьДанныеСотрудника(СтруктураСотрудник)
	...
	Запрос.УстановитьПараметр("Организация", Константы.УчетПоКомпании.Компания(СтруктураСотрудник.Организация));
