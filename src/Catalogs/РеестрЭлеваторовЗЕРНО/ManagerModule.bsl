...
Функция ВыполнитьЗапросВСервис(ЭлементОчереди, ПараметрыОбмена) Экспорт
	...
	ПараметрыОтправкиЗапроса = ИнтеграцияЗЕРНОСлужебный.НовыеПараметрыОтправкиЗапроса();
	
	ПараметрыОтправкиЗапроса.URLЗапроса = "opendata/7708075454-zerno/meta.xml";
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса = ИнтеграцияЗЕРНОСлужебныйКлиентСервер.ПараметрыОтправкиHTTPЗапросов();
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса.Сервер                           = "opendata.mcx.ru";
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса.Порт                             = 80;
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса.ИспользоватьЗащищенноеСоединение = Ложь;
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса.ПредставлениеСервиса             = НСтр("ru = 'opendata МСХ'");
	
	РезультатЗапроса = ИнтеграцияЗЕРНОСлужебный.ПолучитьДанныеИзСервиса(ПараметрыОтправкиЗапроса);
	
	ПараметрыОбработкиРезультата = ИнтеграцияЗЕРНОСлужебный.НовыеПараметрыОбработкиРезультата();
	...
	ДанныеОбработки = ИнтеграцияЗЕРНОСлужебный.ОбработатьРезультатОтправкиHTTPЗапроса(
	...
	Если ДанныеОбработки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийЗЕРНО.Ошибка Тогда
		Возврат ДанныеОбработки;
	КонецЕсли;
	
	ТелоОтвета   = ДанныеОбработки.РезультатОтправкиЗапроса.HTTPОтвет.ПолучитьТелоКакСтроку();
	ОбъектXDTO   = ИнтеграцияИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ТелоОтвета);
	ДанныеОтвета = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO);
	
	ДанныеЗапросаРеестра = ОбщегоНазначенияКлиентСервер.СтруктураURI(ДанныеОтвета.data.dataversion.source);
	ПараметрыОтправкиЗапроса.ПараметрыЗапроса.Сервер = ДанныеЗапросаРеестра.Хост;
	ПараметрыОтправкиЗапроса.URLЗапроса              = ДанныеЗапросаРеестра.ПутьНаСервере;
	...
	ПрочитатьСодержаниеФайлаКлассификатора(ТелоОтвета, ДанныеОбработки, ПараметрыОбмена);
...
//
Функция СообщенияЗагрузкиРеестраЭлеваторов(СсылкаНаОбъект, Организация, ВидПродукции) Экспорт
...
Функция Элеватор(Идентификатор, Организация, Подразделение, ПараметрыОбмена, Наименование = Неопределено) Экспорт
...
Функция ДанныеОрганизацииПоЗаполенномуСвойству(ЭлементДанных, ПараметрыОбмена, Сопоставление = Неопределено)
	...
	Для Каждого КлючИЗначение Из ПараметрыОбмена.ПараметрыПреобразования.ТипыОрганизацийПоИменамСвойств Цикл
		...
		Если ЭлементДанных.Свойство(ПараметрыТипа.ИмяПоля, ВозвращаемоеЗначение.Данные)
			...
			ВозвращаемоеЗначение.Идентификатор  = ЭлементДанных.registrationNumber;
			Прервать;
		КонецЕсли;
	КонецЦикла;
