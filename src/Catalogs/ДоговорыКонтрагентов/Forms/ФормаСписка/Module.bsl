...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли; 
	Если Параметры.Свойство("ЭтоВзаимозачетСПокупателем") Тогда
		ЭтоВзаимозачетСПокупателем = Параметры.ЭтоВзаимозачетСПокупателем;
	Иначе
		ЭтоВзаимозачетСПокупателем = Ложь;
	КонецЕсли; 
	Если Параметры.Свойство("ЭтоВзаимозачетСПоставщиком") Тогда
		ЭтоВзаимозачетСПоставщиком = Параметры.ЭтоВзаимозачетСПоставщиком;
	Иначе
		ЭтоВзаимозачетСПоставщиком = Ложь;
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(ОтборКонтрагент) Тогда
		// Интеркампани
		Если ЭтоВзаимозачетСПоставщиком И ПередачаТоваровМеждуОрганизациями Тогда
			СписокЗначений = Новый СписокЗначений;
			СписокЗначений.Добавить(Организация);
			СписокЗначений.Добавить(ОтборКонтрагент);
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Владелец", СписокЗначений);
		ИначеЕсли ЭтоВзаимозачетСПокупателем И ПередачаТоваровМеждуОрганизациями Тогда
			ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Список.Отбор.Элементы, 
				НСтр("ru = 'Обычные и интеркампани'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИли, "Владелец", 
				ВидСравненияКомпоновкиДанных.Равно, ОтборКонтрагент);
			ГруппаИнтеркампани = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаИли.Элементы, 
				НСтр("ru = 'Интеркампани'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИнтеркампани, "Организация", 
				ВидСравненияКомпоновкиДанных.Равно, Организация);
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИнтеркампани, "ДоговорИнтеркампани", 
				ВидСравненияКомпоновкиДанных.Равно, Истина);
		ИначеЕсли Интеркампани И ЭтоПоступлениеДенег Тогда
		...
		ИначеЕсли Интеркампани Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ДоговорИнтеркампани", Истина);
		Иначе
			Если ЗначениеЗаполнено(Организация) И ТипЗнч(ОтборКонтрагент) = Тип("СправочникСсылка.Организации") Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Организация", Организация);
			КонецЕсли;
			// Конец Интеркампани
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбработатьВыборПериода_Договоры" Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ НеобходимоСброситьОтборПоПериоду Тогда
		Возврат;
	КонецЕсли;
	
	РаботаСОтборамиКлиент.СброситьОтборПоПериоду(ЭтотОбъект, "Список", "СрокДействия");
	НеобходимоСброситьОтборПоПериоду = Ложь;
	Элементы.ОтборДействуетНа.Доступность = Истина;
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	РаботаСОтборамиКлиент.ПредставлениеПериодаВыбратьПериод(ЭтотОбъект, "Список", "СрокДействия",,,"ОбработатьВыборПериода_Договоры");
...
Процедура УстановитьВидимостьИДоступность(ЭтаФорма)
	...
	УстановленОтборДействуетНа = Ложь;
	Для Каждого Отбор Из ЭтаФорма.Список.Отбор.Элементы Цикл
		Если Отбор.Представление <> "ДействуетНа" ИЛИ НЕ Отбор.Использование Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗнч(Отбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			Для Каждого ЭлементОтбора ИЗ Отбор.Элементы Цикл 
				...
				Если ЭлементОтбора.Использование Тогда
					УстановленОтборДействуетНа = Истина;
					Прервать;
				КонецЕсли;
	...
	УстановленОтборПериод = Ложь;
	Для Каждого Отбор Из ЭтаФорма.Список.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если Отбор.Представление <> "Период" ИЛИ НЕ Отбор.Использование Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗнч(Отбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			Для Каждого ЭлементОтбора ИЗ Отбор.Элементы Цикл 
				...
				Если ЭлементОтбора.Использование Тогда
					УстановленОтборПериод = Истина;
					
					Если УстановленОтборДействуетНа Тогда
						ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(
						ЭтаФорма.Список.КомпоновщикНастроек.Настройки.Отбор,, Отбор.Представление);
						ЭтаФорма.НеобходимоСброситьОтборПоПериоду = Истина;
					КонецЕсли;
	...
	Элементы.ОтборДействуетНа.Доступность = НЕ УстановленОтборПериод;
	Элементы.ПредставлениеПериода.Доступность = НЕ УстановленОтборДействуетНа;
...
Процедура УстановитьУсловноеОформление()
	...
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
		...
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДоговорИнтеркампани");
