...
&НаСервере
Функция СозданиеГруппыДляЭлементовГабаритыИВесБрутто(ИмяГруппы)
	
	ГруппаФормыГабаритыИВесБрутто = Элементы.Найти(ИмяГруппы);
	
	Если ГруппаФормыГабаритыИВесБрутто = Неопределено Тогда
		
		ГруппаФормыГабаритыИВесБрутто = Элементы.Добавить(ИмяГруппы, Тип("ГруппаФормы"));		
		ГруппаФормыГабаритыИВесБрутто.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаФормыГабаритыИВесБрутто.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаФормыГабаритыИВесБрутто.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
		ГруппаФормыГабаритыИВесБрутто.Заголовок = "Габариты и вес брутто";
		ГруппаФормыГабаритыИВесБрутто.ОтображатьЗаголовок = Истина;
		ГруппаФормыГабаритыИВесБрутто.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
		ГруппаФормыГабаритыИВесБрутто.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
		ГруппаФормыГабаритыИВесБрутто.Объединенная = Истина;		
	КонецЕсли;		
	
	Возврат ГруппаФормыГабаритыИВесБрутто;
КонецФункции

&НаСервере
Процедура ДобавлениеНовыхРеквизитов()
	
	ТипЧислоЦелоеНеотрицательное = Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(15, 0, ДопустимыйЗнак.Неотрицательный));
		
	ТипЧислоДробноеНеотрицательное = Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(15, 3, ДопустимыйЗнак.Неотрицательный));
	
	ДобавляемыеРеквизиты	= Новый Массив;		
    
    НовыйРеквизит = Новый РеквизитФормы("mega_Ширина",	
    	ТипЧислоЦелоеНеотрицательное, , "Ширина");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    
    НовыйРеквизит = Новый РеквизитФормы("mega_Высота",	
    	ТипЧислоЦелоеНеотрицательное, , "Высота");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    
    НовыйРеквизит = Новый РеквизитФормы("mega_Длина",	
    	ТипЧислоЦелоеНеотрицательное, , "Длина");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    
    НовыйРеквизит = Новый РеквизитФормы("mega_ВесБрутто",	
    	ТипЧислоДробноеНеотрицательное, , "ВесБрутто");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    
    НовыйРеквизит = Новый РеквизитФормы("mega_ВидПоддона",	
    	Новый ОписаниеТипов("ПеречислениеСсылка.mega_ВидыПоддонов"), , "ВидПоддона");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
    
    НовыйРеквизит = Новый РеквизитФормы("mega_КоличествоНаПоддоне",	
    	ТипЧислоДробноеНеотрицательное, , "КоличествоНаПоддоне");
    ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
...
&НаСервере
Процедура СоздатьРеквизитФормы(ИмяЭлемента, Группа)
	
	Если Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
		ЭлементФормы = Элементы.Добавить(ИмяЭлемента, Тип("ПолеФормы"), Группа);        
		ЭлементФормы.Вид = ВидПоляФормы.ПолеВвода;
		ЭлементФормы.ПутьКДанным = "Объект." + ИмяЭлемента;
...
Процедура СоздатьРеквизитыФормы()
	...
	ИмяГруппы = "ГабаритыИВесБрутто";
	ГруппаФормыГабаритыИВесБрутто = СозданиеГруппыДляЭлементовГабаритыИВесБрутто(ИмяГруппы);	
	
	ИмяГруппы = "КоличествоИВидПоддона";
	ГруппаФормыКоличествоИВидПоддона = СозданиеГруппыДляЭлементовГабаритыИВесБрутто(ИмяГруппы);
	
	ДобавлениеНовыхРеквизитов();	
			
	СоздатьРеквизитФормы("mega_Ширина", ГруппаФормыГабаритыИВесБрутто);
	СоздатьРеквизитФормы("mega_Высота", ГруппаФормыГабаритыИВесБрутто);
	СоздатьРеквизитФормы("mega_Длина", ГруппаФормыГабаритыИВесБрутто);
	СоздатьРеквизитФормы("mega_ВесБрутто", ГруппаФормыГабаритыИВесБрутто);
	
	СоздатьРеквизитФормы("mega_ВидПоддона", ГруппаФормыКоличествоИВидПоддона);
	СоздатьРеквизитФормы("mega_КоличествоНаПоддоне", ГруппаФормыКоличествоИВидПоддона);	
