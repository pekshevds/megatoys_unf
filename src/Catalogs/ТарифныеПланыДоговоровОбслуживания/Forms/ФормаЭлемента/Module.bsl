...
#Область ОбработчикиСобытийЭлементовТаблицыФормыУчетНоменклатуры

&НаКлиенте
Процедура УчетНоменклатурыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
Процедура УчетНоменклатурыНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти = Элементы.УчетНоменклатуры.ТекущиеДанные;
	...
	Если ЗапрашиватьСбросКоличества И СбрасыватьКоличество
		...
		Оповещение = Новый ОписаниеОповещения("УчетНоменклатурыПодтверждениеСбросаКоличестваЗавершение",
	...
	Иначе
		...
		КонецЕсли;
		УчетНоменклатурыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество);
...
&НаКлиенте
Процедура УчетНоменклатурыПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	УчетНоменклатурыНоменклатураПриИзмененииПродолжить(ДополнительныеПараметры.Элемент, НовоеКоличество);
	
КонецПроцедуры

&НаКлиенте
Процедура УчетНоменклатурыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ПредставлениеВСчете) Тогда
		СтруктураДанные.Вставить("НоменклатураВСчете", ТекущаяСтрока.ПредставлениеВСчете);
	Иначе
		СтруктураДанные.Вставить("НоменклатураВСчете", ТекущаяСтрока.Номенклатура);
		СтруктураДанные.Вставить("ПредставлениеХарактеристики", ТекущаяСтрока.Характеристика);
	КонецЕсли;
	СтруктураДанные.Вставить("ДобавлятьПериодВСодержание", ТекущаяСтрока.ДобавлятьПериодВСодержание);
	...
	Если СтруктураДанные.Свойство("НоменклатураСодержаниеПример") Тогда
		ТекущаяСтрока.НоменклатураСодержаниеПример = СтруктураДанные.НоменклатураСодержаниеПример;
	КонецЕсли;
...
Процедура УчетНоменклатурыПредставлениеЦеныНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УчетНоменклатурыПредставлениеЦеныЗавершениеВыбора", ЭтотОбъект);
...
&НаКлиенте
Процедура УчетНоменклатурыПредставлениеВСчетеПриИзменении(Элемент)
...
&НаКлиенте
Процедура УчетНоменклатурыДобавлятьПериодВСодержаниеПриИзменении(Элемент)
...
Процедура УчетЗатратПредставлениеЦеныНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УчетЗатратПредставлениеЦеныЗавершениеВыбора", ЭтотОбъект);
...
&НаКлиенте
Процедура УчетЗатратПредставлениеВСчетеПриИзменении(Элемент)
...
&НаКлиенте
Процедура УчетЗатратДобавлятьПериодВСодержаниеПриИзменении(Элемент)
...
Процедура НастройкаСбросаКоличества(Команда)
	...
	СтатистикаИспользованияФормКлиент.ПриИнтерактивномДействии(ЭтотОбъект, 
		Элементы.УчетНоменклатурыКонтекстноеМенюНастройкаСбросаКоличества, "Нажатие");
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	// 1. если формирование цены затрат выполняется без наценки - колонка недоступна для редактирования.
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
...
Процедура ИнициализироватьТаблицыОбъекта()
	...
	Для каждого Строка Из Объект.УчетНоменклатуры Цикл
		Если Строка.ФормированиеЦены = Перечисления.БиллингФормированиеЦеныНоменклатуры.ФиксированноеЗначение Тогда
			Строка.ПредставлениеЦены = Строка.Цена;
		Иначе
			Строка.ПредставлениеЦены = Строка.ФормированиеЦены;
		КонецЕсли;
		
		Если Строка.ДобавлятьПериодВСодержание Тогда
			ПараметрыНоменклатуры = Новый Структура;
			Если ЗначениеЗаполнено(Строка.ПредставлениеВСчете) Тогда
				ПараметрыНоменклатуры.Вставить("ПредставлениеНоменклатуры", Строка.ПредставлениеВСчете);
			Иначе
				ПараметрыНоменклатуры.Вставить("ПредставлениеНоменклатуры", Строка.Номенклатура);
				ПараметрыНоменклатуры.Вставить("ПредставлениеХарактеристики", Строка.Характеристика);
			КонецЕсли;
			
			ПредставлениеНоменклатуры = ПечатьДокументовУНФ.ПредставлениеНоменклатуры(ПараметрыНоменклатуры);
			Строка.НоменклатураСодержаниеПример = СодержаниеНоменклатурыСПериодом(ПредставлениеНоменклатуры);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого Строка Из Объект.УчетЗатрат Цикл
		Если Строка.ФормированиеЦены = Перечисления.БиллингФормированиеЦеныЗатрат.ФиксированноеЗначение Тогда
			Строка.ПредставлениеЦены = Строка.Цена;
		Иначе
			Строка.ПредставлениеЦены = Строка.ФормированиеЦены;
		КонецЕсли;
		...
		Если Строка.ДобавлятьПериодВСодержание Тогда
			ПредставлениеНоменклатуры = ПечатьДокументовУНФ.ПредставлениеНоменклатуры(
				Новый Структура("ПредставлениеНоменклатуры", Строка.ПредставлениеВСчете));
			Строка.НоменклатураСодержаниеПример = СодержаниеНоменклатурыСПериодом(ПредставлениеНоменклатуры);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПолучитьДанныеСтрокиНоменклатураПриИзменении(СтруктураДанные)
	...
	Если СтруктураДанные.Свойство("Номенклатура") Тогда
		ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			СтруктураДанные.Номенклатура, "ЕдиницаИзмерения,Наименование,НаименованиеПолное");
		СтруктураДанные.Вставить("ЕдиницаИзмерения", ДанныеНоменклатуры.ЕдиницаИзмерения);
	КонецЕсли;
	
	Если СтруктураДанные.Свойство("НоменклатураВСчете") И СтруктураДанные.Свойство("ДобавлятьПериодВСодержание") Тогда
		ПараметрыНоменклатуры = Новый Структура;
		ПараметрыНоменклатуры.Вставить("ПредставлениеНоменклатуры", СтруктураДанные.НоменклатураВСчете);
		Если СтруктураДанные.Свойство("ПредставлениеХарактеристики") Тогда
			ПараметрыНоменклатуры.Вставить("ПредставлениеХарактеристики", СтруктураДанные.ПредставлениеХарактеристики);
		КонецЕсли;
		
		ПредставлениеНоменклатуры = ПечатьДокументовУНФ.ПредставлениеНоменклатуры(ПараметрыНоменклатуры);
		СтруктураДанные.Вставить("НоменклатураСодержаниеПример", 
			СодержаниеНоменклатурыСПериодом(ПредставлениеНоменклатуры, СтруктураДанные.ДобавлятьПериодВСодержание));
	КонецЕсли;
...
&НаСервере
Функция СодержаниеНоменклатурыСПериодом(ПредставлениеНоменклатуры, ДобавлятьПериодВСодержание = Истина)
	
	Если НЕ ДобавлятьПериодВСодержание Тогда
		Возврат "";
	КонецЕсли;
	
	ПредставлениеНоменклатурыСПериодом = НоменклатураВДокументахКлиентСервер.СодержаниеНоменклатурыСПериодом(
		ПредставлениеНоменклатуры, ТекущаяДатаСеанса(), Перечисления.БиллингПериодичностьДоговораОбслуживания.Месяц);
	Возврат СтрШаблон(НСтр("ru='Пример: %1'"), ПредставлениеНоменклатурыСПериодом);
