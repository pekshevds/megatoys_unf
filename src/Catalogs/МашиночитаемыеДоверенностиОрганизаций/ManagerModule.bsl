...
// Определяет является ли доверенность передоверием
//
// Параметры:
//  ДанныеИзФайлаОбмена - см. МашиночитаемыеДоверенности.ДанныеИзФайлаОбмена
//
// Возвращаемое значение:
//  Булево - Признак того что является или нет передоверием
//
Функция ЭтоПередоверие(ДанныеИзФайлаОбмена) Экспорт
	
	Если НЕ ДанныеИзФайлаОбмена.Успех Тогда
		Возврат Ложь;
	КонецЕсли;
	СтруктураДанных = ДанныеИзФайлаОбмена.ДанныеДоверенности;
...
Функция НайтиДоверенности(НомерДоверенности) Экспорт
	...
	Запрос.Текст =
		...
		|	И МашиночитаемыеДоверенностиОрганизаций.ПометкаУдаления = ЛОЖЬ";
