...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Тип) Тогда
			...
			Объект.Тип = ?(Параметры.РежимПлатежнойСистемы, 
				Перечисления.ТипыСпособовОплат.СБП, 
				Перечисления.ТипыСпособовОплат.Эквайринг);
				
		КонецЕсли;
		
	КонецЕсли;
	
	НастроитьСписокТипов();
	...
	Валюта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ВалютаРасчетов");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Если ЗначениеЗаполнено(Объект.БанковскийСчетЭквайринг)
			И НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
			Валюта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчетЭквайринг, "ВалютаДенежныхСредств");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		И ЗначениеЗаполнено(Объект.Тип) Тогда
		ПриИзмененииТипа();
	КонецЕсли;
	
	Элементы.ДоговорПлатежнаяСистема.Видимость = ЗначениеЗаполнено(Объект.Эквайрер);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если Объект.Тип = Перечисления.ТипыСпособовОплат.Эквайринг Тогда
		Элементы.СтраницыСпособыОплат.ТекущаяСтраница = Элементы.СтраницаЭквайринг;
	Иначе
		Элементы.СтраницыСпособыОплат.ТекущаяСтраница = Элементы.СтраницаПлатежнаяСистема;
		Если Объект.ВидыПлатежныхКарт.Количество() > 0 Тогда
			ПроцентКомиссииШапка = Объект.ВидыПлатежныхКарт[0].ПроцентКомиссии;
			ПроцентКомиссииПриВозврате = Объект.ВидыПлатежныхКарт[0].ПроцентКомиссииПриОтмене;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ТипПлатежнаяСистемаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОрганизацияПлатежнаяСистемаПриИзменении(Элемент)
	
	ЗначениеРеквизитов = Новый Структура;
	ЗначениеРеквизитов.Вставить("Эквайрер", 	Объект.Эквайрер);
	ЗначениеРеквизитов.Вставить("Организация", 	Объект.Организация);
	ЗначениеРеквизитов.Вставить("Ссылка", 		Объект.Ссылка);
	
	НовыеЗначенияРеквизитов = РассчитатьРеквизиты("Касса,Договор,БанковскийСчетЭквайринг", ЗначениеРеквизитов);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, НовыеЗначенияРеквизитов);
...
&НаКлиенте
Процедура ЭквайрерПлатежнаяСистемаПриИзменении(Элемент)
	...
	НовыеЗначенияРеквизитов = РассчитатьРеквизиты("Договор", ЗначениеРеквизитов);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, НовыеЗначенияРеквизитов);
...
Функция СформироватьАвтоНаименование()
    ...
    Элементы.НаименованиеПлатежнаяСистема.СписокВыбора.Очистить();
	
	Если Объект.Тип = ПредопределенноеЗначение("Перечисление.ТипыСпособовОплат.СБП") Тогда
		ШаблонАвтоНаименования = НСтр("ru = 'СБП %1'");
		ЭквайрерСтрокой = ?(ЗначениеЗаполнено(Объект.Эквайрер), Строка(Объект.Эквайрер), НСтр("ru = ''"));
		СтрокаНаименования = СокрП(СтрШаблон(ШаблонАвтоНаименования, ЭквайрерСтрокой));
	ИначеЕсли Объект.Тип = ПредопределенноеЗначение("Перечисление.ТипыСпособовОплат.ЮKassa") Тогда
		СтрокаНаименования = НСтр("ru = 'ЮKassa'");
	Иначе
    ...
    Элементы.НаименованиеПлатежнаяСистема.СписокВыбора.Добавить(СтрокаНаименования);
...
&НаКлиенте
Процедура ПриИзмененииТипа()
	
	Если Объект.Тип = ПредопределенноеЗначение("Перечисление.ТипыСпособовОплат.Эквайринг") Тогда
		Элементы.СтраницыСпособыОплат.ТекущаяСтраница = Элементы.СтраницаЭквайринг;
	Иначе
		Элементы.СтраницыСпособыОплат.ТекущаяСтраница = Элементы.СтраницаПлатежнаяСистема;
		Объект.ИспользоватьБезПодключенияОборудования = Истина;
	...
	ИспользоватьБезПодключенияОборудованияПриИзменении(Неопределено);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция РассчитатьРеквизиты(СписокРеквизитов, ЗначенияФормы)
	...
	Для Каждого ИмяРеквизита Из СтрРазделить(СписокРеквизитов, ","  ) Цикл
		...
		Если ИмяРеквизита = "Договор" Тогда
			...
			ЗначениеРеквизита = ПолучитьДоговорПоУмолчанию(ЗначенияФормы.Ссылка, ЗначенияФормы.Эквайрер, ЗначенияФормы.Организация);
			ЗначенияРеквизитов.Вставить(ИмяРеквизита, ЗначениеРеквизита);
			
		ИначеЕсли ИмяРеквизита = "БанковскийСчетЭквайринг" Тогда
			
			ЗначениеРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначенияФормы.Организация, "БанковскийСчетПоУмолчанию");
			ЗначенияРеквизитов.Вставить(ИмяРеквизита, ЗначениеРеквизита);
			
		ИначеЕсли ИмяРеквизита = "Касса" Тогда

			ЗначениеРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначенияФормы.Организация, "КассаПоУмолчанию");
			ЗначенияРеквизитов.Вставить(ИмяРеквизита, ЗначениеРеквизита);
...
&НаСервере
Процедура ЗаполнитьТаблицуКомиссии()
	
	Если Объект.Тип = Перечисления.ТипыСпособовОплат.Эквайринг Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ВидыПлатежныхКарт.Очистить();
	
	СтрокаТЧ = Объект.ВидыПлатежныхКарт.Добавить();
	
	СтрокаТЧ.ВидПлатежнойКарты			= Справочники.СпособыОплаты.ВидПлатежнойКартыПоУмолчанию(Объект.Тип);
	
	Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ВидПлатежнойКарты) Тогда
		СтрокаТЧ.ВидПлатежнойКарты = Объект.Наименование;
	КонецЕсли;
	
	СтрокаТЧ.ПроцентКомиссии			= ПроцентКомиссииШапка;
	СтрокаТЧ.ПроцентКомиссииПриОтмене   = ПроцентКомиссииПриВозврате;
	СтрокаТЧ.ПроцентКомиссииПриВозврате = ПроцентКомиссииПриВозврате;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьСписокТипов()
	
	Элементы.Тип.СписокВыбора.Очистить();
	Элементы.Тип.СписокВыбора.Добавить(Перечисления.ТипыСпособовОплат.Эквайринг);
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОнлайнОплаты") Тогда
		Элементы.Тип.СписокВыбора.Добавить(Перечисления.ТипыСпособовОплат.ЮKassa);
	КонецЕсли;
	
	Элементы.Тип.СписокВыбора.Добавить(Перечисления.ТипыСпособовОплат.СБП);
	Элементы.Тип.СписокВыбора.Добавить(Перечисления.ТипыСпособовОплат.ДругаяПлатежнаяСистема);
	
	Элементы.ТипПлатежнаяСистема.СписокВыбора.Очистить();

	Для каждого Элемент Из Элементы.Тип.СписокВыбора Цикл
		Элементы.ТипПлатежнаяСистема.СписокВыбора.Добавить(Элемент.Значение);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьКассу()
