...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КОплате) Тогда
		...
		Элементы.Сумма.СписокВыбора.Добавить(КОплате, СтрШаблон("%1 (%2)", КОплате, ЗаголовокСуммы));
	КонецЕсли;
...
Процедура ОК(Команда)
	...
	Иначе
		...
		Если ВозвратОплаты Тогда
			ПараметрыЗакрытия.Вставить("ОплатаОтменена", Истина);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура СчитатьКарту(Команда)
	
	Если ВозвратОплаты Тогда
		ВернутьОплатуПоКарте();
	Иначе
...
&НаКлиенте
Процедура ВернутьОплатуПоКарте()
	
	НомерПлатежнойКарты = "";
	
	ИДЗамера = ОценкаПроизводительностиКлиент.ЗамерВремени("ОплатаКартой_ПроверитьВозможностьПечатиЧека", Истина, Ложь);
	Если ПечатьДокументовУНФКлиент.ПечатьЧекаНевозможна(ВладелецФормы, НСтр(
		"ru = 'Не удалось выполнить возврат через платежный терминал'")) Тогда
		Возврат;
	КонецЕсли;
	ОценкаПроизводительностиКлиент.ЗавершитьЗамерВремени(ИДЗамера, Ложь);
	...
	Если ИдентификаторУстройстваЭТ <> Неопределено 
		...
		ВернутьОплатуПоКартеЗавершение(Результат, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ВернутьОплатуПоКартеЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьВидыКарт()
	...
	МассивВидовПлатежныхКарт = Справочники.СпособыОплаты.ВидыПлатежныхКарт(ЭквайринговыйТерминал);
...
Процедура УправлениеФормой()
	...
	Если ВозвратОплаты Тогда
		Элементы.ОплатаПлатежнымиКартамиУдалитьОплатуКартой.Видимость = Ложь;
