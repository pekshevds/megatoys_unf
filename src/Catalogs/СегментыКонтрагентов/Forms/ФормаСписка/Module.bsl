...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РаботаСОтборами.ВосстановитьНастройкиОтборов(ЭтотОбъект, 
		Список, , , Новый Структура("ОтборПериод", "ДатаФормирования"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_СегментКонтрагентовГруппа" Тогда
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "КомандаСоставСегмента" Тогда
		...
		СтандартнаяОбработка = Ложь;
		ВыделенныйСегмент = Элементы.Список.ТекущиеДанные.Ссылка;
		ПараметрыИОтборОтчета = Новый Структура("СегментКонтрагентов", ВыделенныйСегмент);
...
Процедура ИерархияИзменить(Команда)
	...
	Если Элементы.ОтборИерархия.ТекущиеДанные = Неопределено
		Или ТипЗнч(Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов) <> Тип("СправочникСсылка.СегментыКонтрагентов")
...
Процедура ИерархияСоздатьГруппу(Команда)
	...
	Если ТипЗнч(Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов) = Тип("СправочникСсылка.СегментыКонтрагентов") Тогда
		ЗначенияЗаполнения.Вставить("Родитель", Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов);
	КонецЕсли;
	
	СтруктураЗначенияЗаполнения = Новый Структура("ЗначенияЗаполнения, ЭтоГруппа", ЗначенияЗаполнения, Истина);
	ОткрытьФорму("Справочник.СегментыКонтрагентов.ФормаГруппы",
		СтруктураЗначенияЗаполнения,
		Элементы.Список);
...
Процедура ИерархияСкопировать(Команда)
	...
	СтруктураЗначенияЗаполнения =	Новый Структура("ЗначениеКопирования, ЭтоГруппа", 
		Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов, Истина);
		
	ОткрытьФорму("Справочник.СегментыКонтрагентов.ФормаГруппы", СтруктураЗначенияЗаполнения, Элементы.Список);
...
Процедура ИерархияУстановитьПометкуУдаления(Команда)
	...
	СтрокаУточнения = ?(ПометкаУдаления, НСтр("ru='установлена'"), НСтр("ru='снята'"));
	ТекстОповещения = СтрШаблон(НСтр("ru='Пометка удаления %1'"), СтрокаУточнения);
	НавигационнуюСсылкаСегмента = ПолучитьНавигационнуюСсылку(Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов);
	
	ПоказатьОповещениеПользователя(
		ТекстОповещения,
		НавигационнуюСсылкаСегмента,
...
Процедура ОтборИерархияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) <> Тип("Массив")
		...
		Или ТипЗнч(ПараметрыПеретаскивания.Значение[0]) <> Тип("СправочникСсылка.СегментыКонтрагентов") Тогда
	...
	НоваяГруппа = ?(СтрокаИерархии.ГруппаСегментов = "БезГруппы", 
		ПредопределенноеЗначение("Справочник.СегментыКонтрагентов.ПустаяСсылка"), 
...
Процедура УстановитьОтборПоИерархии(Форма)
	...
	ЗначениеГруппыСегментов = Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов;
	ЭтоОтборПоГруппе = ТипЗнч(ЗначениеГруппыСегментов) = Тип("СправочникСсылка.СегментыКонтрагентов");
	...
	ИначеЕсли Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСегментов = "БезГруппы" Тогда
		...
		ПравоеЗначение = ПредопределенноеЗначение("Справочник.СегментыКонтрагентов.ПустаяСсылка");
	Иначе
...
Процедура ОперацииПослеФормирования()
	...
	СтрокаСообщение = СтрШаблон(СтрокаШаблон, КоличествоКонтрагентовПоПравилам, КоличествоКонтрагентовВручную);
	ОбщегоНазначенияКлиент.СообщитьПользователю(СтрокаСообщение);	
	Элементы.Список.Обновить();
...
Процедура РассчитатьКоличествоКонтрагентовВСегменте()
	КоличествоКонтрагентовВручную = СоставСегментаВручную().Количество();
	КоличествоКонтрагентовПоПравилам = СоставСегментаПоПравилам().Количество()
КонецПроцедуры

&НаСервере
Функция СоставСегментаПоПравилам()
	Возврат Справочники.СегментыКонтрагентов.ПолучитьСоставСегмента(ВыделенныйСегмент, 
