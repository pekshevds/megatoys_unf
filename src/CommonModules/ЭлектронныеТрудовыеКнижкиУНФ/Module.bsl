...
Функция ДанныеТрудовойДеятельностиСотрудников(ТолькоРазрешенные, ДокументыСЗВТД,  Сотрудники, Организация, НачалоПериода = Неопределено, ОкончаниеПериода = Неопределено, ВидыМероприятий = Неопределено, ЗаполнитьВсемиСотрудниками = Ложь) Экспорт
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	НАЧАЛОПЕРИОДА(ТекущиеКадровыеДанныеСотрудниковУНФ.ДатаПриема, МЕСЯЦ) КАК НачалоПериода,
			|	ВЫБОР
			|		КОГДА ТекущиеКадровыеДанныеСотрудниковУНФ.ДатаУвольнения = ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА ДАТАВРЕМЯ(3999, 12, 31)
			|		ИНАЧЕ КОНЕЦПЕРИОДА(ТекущиеКадровыеДанныеСотрудниковУНФ.ДатаУвольнения, МЕСЯЦ)
			|	КОНЕЦ КАК ОкончаниеПериода
			|ПОМЕСТИТЬ ВТСотрудникиПериодыДанных
			|ИЗ
			|	ВТТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанныеСотрудниковУНФ";
...
Функция ДанныеТрудовойДеятельностиСотрудниковОрганизации(ТолькоРазрешенные, МассивДокументов, Организация, НачалоПериода, ОкончаниеПериода, ВидыМероприятий = Неопределено, ЗаполнитьВсемиСотрудниками = Ложь) Экспорт
	...
	УчетСтраховыхВзносовУНФ.СформироватьТаблицуВТФизическиеЛицаРаботавшиеВОрганизации(МенеджерВременныхТаблиц, Организация, НачалоПериода, ОкончаниеПериода);
	Запрос = Новый Запрос;
				...
				Если ВыборкаДанныхДляПечати.Дата >= '20210701' Тогда
					Если ЗначениеЗаполнено(ВыборкаДанныхДляПечати.КодПоОКЗ) Тогда
						...
						СтркутраЗаполнения.Вставить("КодПоОКЗ",
							ПредставлениеКодаОКЗ(ВыборкаДанныхДляПечати.КодПоОКЗ));
...
Функция ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(МенеджерВременныхТаблиц, ТолькоРазрешенные, Организация, ВидыМероприятий = Неопределено, УстановленОтбрСотрудников = Ложь, НачалоПериода, ЗаполнитьВсемиСотрудниками)
	...
	УчетСтраховыхВзносовУНФ.СоздатьВТКадровыеДанныеСотрудников(Описатель, ТолькоРазрешенные, КадровыеДанные);
	...
	Запрос.Текст =
		...
		|	КадровыеДанные.СтраховойНомерПФР КАК СтраховойНомерПФР,
		|	ВЫБОР
		|		КОГДА КадровыеДанные.Сотрудник.ТипЗанятости = ЗНАЧЕНИЕ(Перечисление.ТипыЗанятости.Совместительство)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
	...
	СписокСвойств = ДоступныеПоляВсехВидовМероприятий() + ",Сотрудник";
	ДоступныеПоля = ИменаДоступныхПолейВидовМероприятий();
...
Функция ИменаДоступныхПолейВидовМероприятий(ДатаДокумента = Неопределено) Экспорт
	...
	ДоступныеПоля.Вставить(ПредопределенноеЗначение("Перечисление.ВидыМероприятийТрудовойДеятельности.Увольнение"),
		"КодПоОКЗ,ОснованиеУвольнения,ОснованиеУвольненияТекстОснования,ОснованиеУвольненияСтатья,ОснованиеУвольненияЧасть,ОснованиеУвольненияПункт,"
		+ "ОснованиеУвольненияПодпункт,ОснованиеУвольненияАбзац");   
...
Процедура СоздатьВТКадровыеСобытия(МенеджерВременныхТаблиц, Организация, ТолькоРазрешенные)
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Документ.ЗаявленияОПредоставленииСведенийОТрудовойДеятельности.Сотрудники КАК ЗаявлениеСотрудники
		...
		|ГДЕ
		|	НЕ ЗаявлениеСотрудники.УдалитьЗаявлениеОтозвано
...
Функция ПредставлениеПричиныУвольнения(ВыборкаДанных, ДляКадровогоПриказа = Ложь, ЭтоЕФС1 = Ложь) Экспорт 
	
	НормативныйДокумент = УчетСтраховыхВзносовУНФ.НормативныйДокументОснованияУвольнения(ВыборкаДанных.ОснованиеУвольнения);
	ПредставлениеПричины = ?(ЗначениеЗаполнено(ВыборкаДанных.ОснованиеПричиныУвольнения), ВыборкаДанных.ОснованиеПричиныУвольнения, "")
	...
	Если ЭтоЕФС1 Тогда 
		КодПричиныУвольнения = Строка(ВыборкаДанных.ПричинаУвольненияПФР);
...
Функция ЗапросВТСобытияТрудовыхКнижек(Организация, ТолькоРазрешенные)
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПриемНаРаботуУНФ.Ссылка КАК Ссылка,
		|	ПриемНаРаботуУНФ.Ссылка.Номер КАК Номер,
		|	ПриемНаРаботуУНФ.Ссылка.Дата КАК Дата,
		|	ПриемНаРаботуУНФ.Сотрудник КАК Сотрудник,
		|	ПриемНаРаботуУНФ.Сотрудник.Физлицо КАК ФизическоеЛицо,
		|	ПриемНаРаботуУНФ.Период КАК ДатаМероприятия,
		|	ПриемНаРаботуУНФ.Ссылка.Организация КАК Организация,
		|	ПриемНаРаботуУНФ.СтруктурнаяЕдиница КАК Подразделение,
		|	ПриемНаРаботуУНФ.Должность КАК Должность,
		...
		|		ПО ПриемНаРаботуУНФ.Сотрудник = КадровыеДанные.Сотрудник
		|ГДЕ
		|	ПриемНаРаботуУНФ.Ссылка.Проведен
		|	И ПриемНаРаботуУНФ.Ссылка.Организация = &Организация
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	NULL
		|ИЗ
		|	Документ.КадровоеПеремещениеУНФ.Сотрудники КАК КадровыйПеревод
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		...
		|
		|ВЫБРАТЬ
		|	УвольнениеУНФ.Ссылка,
		|	УвольнениеУНФ.Ссылка.Номер,
		|	УвольнениеУНФ.Ссылка.Дата,
		|	УвольнениеУНФ.Сотрудник,
		|	УвольнениеУНФ.Сотрудник.Физлицо,
		|	УвольнениеУНФ.Период,
		|	УвольнениеУНФ.Ссылка.Организация,
		|	NULL,
		|	NULL,
		|	NULL,
		...
		|	""""
		|ИЗ
		|	Документ.УвольнениеУНФ.Сотрудники КАК УвольнениеУНФ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		|		ПО УвольнениеУНФ.Сотрудник = КадровыеДанные.Сотрудник
		|ГДЕ
		|	УвольнениеУНФ.Ссылка.Проведен
		|	И УвольнениеУНФ.Ссылка.Организация = &Организация
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЗНАЧЕНИЕ(Справочник.КлассификаторЗанятийУНФ.ПустаяСсылка) КАК КодПоОКЗ,
		...
		|	ЗНАЧЕНИЕ(Справочник.КлассификаторЗанятийУНФ.ПустаяСсылка),
