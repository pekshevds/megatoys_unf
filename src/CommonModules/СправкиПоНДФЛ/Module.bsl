...
Функция СохранитьФайлВыгрузкиСправки2НДФЛ(Документ) Экспорт
	...
	Если ВыводитьФорму2НДФЛ2019Года(НалоговыйПериод, Документ.Дата) Тогда
		ТекстФайла = СформироватьВыходнойФайл2019(Документ, ИмяСформированногоФайла, НалоговыйПериод);
	ИначеЕсли ВыводитьФорму2НДФЛ2018Года(НалоговыйПериод, Документ.Дата) Тогда
		ТекстФайла = СформироватьВыходнойФайл2018(Документ, ИмяСформированногоФайла, НалоговыйПериод);
...
Функция ДанныеОДоходах(ТаблицаСотрудников, НалоговыйПериод, Организация, ГоловнаяОрганизация, ОКАТО_КПП, Дата) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТУведомления.НомерСправки КАК НомерСправки,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТУведомления.НомерСправки,
	|	ВТУведомления.ГруппаВычета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТПоследниеУведомления.НомерСправки КАК НомерСправки,
	...
	|		ПО (ВТУведомления.НомерСправки = ВТПоследниеУведомления.НомерСправки)
	|			И (ВТУведомления.ДатаУведомления = ВТПоследниеУведомления.ДатаУведомления)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДоходыИВычеты.НомерСправки КАК НомерСправки,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МесяцыОсобыхВычетов.НомерСправки КАК НомерСправки,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОтсортированныеУведомления.НомерСправки КАК НомерСправки,
	...
	Возврат Новый Структура("Уведомления, Доходы, Вычеты, ИТОГИ", Результаты[ВсегоЗапросов - 4].Выгрузить(), Результаты[ВсегоЗапросов - 3].Выгрузить(), Результаты[ВсегоЗапросов - 2].Выгрузить(), Результаты[ВсегоЗапросов - 1].Выгрузить());
...
Функция ДанныеПодписывающихЛиц(Организация, ДатаАктуальности, ЭтоЮрЛицо = Неопределено, РасшифровкаПодписи = Ложь) Экспорт
	...
	Если ЭтоЮрЛицо Тогда
		...
		|	ОтветственныеЛица.Роль,
		|	ФИО.Фамилия КАК Фамилия,
		|	ФИО.Имя КАК Имя,
		|	ФИО.Отчество КАК Отчество
		|ИЗ
		|	ВТОтветственныеЛица КАК ОтветственныеЛица
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&Дата, ) КАК ФИО
		|		ПО ОтветственныеЛица.ФизЛицо = ФИО.ФизическоеЛицо";
...
Функция ДанныеСотрудников(ТаблицаСотрудников, Дата, НалоговыйПериод) Экспорт
	...
	|			И (СотрудникиАдрес.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица))
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&Дата) КАК ФИОФизическихЛиц
...
Функция СправкиНДФЛДанныеДляПечати(МассивОбъектов, ПолучатьИтоговыеСуммы = Ложь) Экспорт
	...
	Если ПолучатьИтоговыеСуммы Тогда
		...
		Иначе
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	СправкаНДФЛУНФ.Ссылка,
			|	1 КАК НомерСправки
			|ПОМЕСТИТЬ ВТКоличествоСправок
			|ИЗ
			|	Документ.СправкаНДФЛУНФ КАК СправкаНДФЛУНФ
			|ГДЕ
			|	СправкаНДФЛУНФ.Ссылка В(&МассивОбъектов)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	СУММА(СправкаНДФЛУНФ.ОбщаяСуммаДохода) КАК ОбщаяСуммаДохода,
			|	СУММА(СправкаНДФЛУНФ.ОблагаемаяСуммаДохода) КАК ОблагаемаяСуммаДохода,
			|	СУММА(СправкаНДФЛУНФ.Исчислено) КАК Исчислено,
			|	СУММА(СправкаНДФЛУНФ.Удержано) КАК Удержано,
			|	СУММА(СправкаНДФЛУНФ.Задолженность) КАК Задолженность,
			|	СУММА(СправкаНДФЛУНФ.ИзлишнеУдержано) КАК ИзлишнеУдержано,
			|	СУММА(СправкаНДФЛУНФ.Перечислено) КАК Перечислено,
			|	СправкаНДФЛУНФ.Ссылка
			|ПОМЕСТИТЬ ВТСуммыНалогов
			|ИЗ
			|	Документ.СправкаНДФЛУНФ.СуммыНалогов КАК СправкаНДФЛУНФ
			|ГДЕ
			|	СправкаНДФЛУНФ.Ссылка В(&МассивОбъектов)
			|
			|СГРУППИРОВАТЬ ПО
		...
		|		ПО  Организации.ФизическоеЛицо = ФИОФизическихЛиц.ФизическоеЛицо
		|ГДЕ
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Организации.КодНалоговогоОрганаПолучателя", "ДанныеДокумента.КодНалоговогоОрганаПолучателя");
		...
		Запрос.УстановитьПараметр("ОснованиеДляПредставленияСправок","");
	...
	Пока ДанныеОрганизации.Следующий() Цикл 
		...
		Если ЭтоСправкаДляНалоговогоОргана Тогда
			...
			ИначеЕсли ДанныеОрганизации.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛЗаРеорганизованнуюОрганизацию Тогда
...
Функция СформироватьПечатнуюФорму2НДФЛ(ОбъектыПечати, МассивОбъектов, ДанныеНА, ДанныеСотрудниковДокументов, ДанныеОДоходахДокументов, ДанныеОВычетахДокументов, ДанныеОбУведомленияхДокументов) Экспорт
	...
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		...
		ВыводитьФормуСправки2022 = ВыводитьФорму2НДФЛ2022Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата);
		...
		ИначеЕсли ВыводитьФорму2НДФЛ2021Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
        ...
        ИначеЕсли ВыводитьФорму2НДФЛ2019Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
		...
		ИначеЕсли ВыводитьФорму2НДФЛ2018Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
		...
		ИначеЕсли ДанныеНалоговогоАгента.НалоговыйПериод >= 2015 Тогда			
		...
		Если ВыводитьФорму2НДФЛ2018Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) И ЗначениеЗаполнено(ДанныеНалоговогоАгента.ФормаРеорганизации) Тогда
			ОбластьМакетаШапкаПервойСтраницы.Параметры.НазваниеОрганизации = ДанныеНалоговогоАгента.НаименованиеРеорганизованнойОрганизации;
		КонецЕсли;
		Если ВыводитьФорму2НДФЛ2019Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
			
		ИначеЕсли ДанныеНалоговогоАгента.Признак = "СводнаяСправка" Тогда
		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Для каждого СтрокаТЗ Из ДоходыПоВидам Цикл
				Если ПредыдущийМесяц <> СтрокаТЗ.МесяцНалоговогоПериода 
					Или ПредыдущийКодДохода <> СтрокаТЗ.КодДохода Тогда
			...
			Если ВыводитьФорму2НДФЛ2018Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда			
				
			Иначе
				Если ЗначениеЗаполнено(СтрокаДанных.АдресРФ) Тогда
					СтруктураАдреса = РаботаСАдресами.СведенияОбАдресе(
							СтрокаДанных.АдресРФ);
			...
			КонецЕсли;
			 ...
			 // реквизиты Уведомления об имущ.вычете Раздела 4
			СтрокиУведомленийПоСотрудникам = ДанныеОбУведомленияхДокументов.Скопировать(СтруктураОтбора);
			Для Каждого Уведомление Из СтрокиУведомленийПоСотрудникам Цикл
				Если Уведомление.ГруппаВычета = Перечисления.ГруппыВычетовПоНДФЛ.Имущественные Тогда
					ОбластьМакетаУведомлениеНаИмущВычет.Параметры.НомерУведомления = Уведомление.НомерУведомления;
					ОбластьМакетаУведомлениеНаИмущВычет.Параметры.ДатаУведомления = Уведомление.ДатаУведомления;
					ОбластьМакетаУведомлениеНаИмущВычет.Параметры.КодНалоговогоОрганаУведомления = Уведомление.КодНалоговогоОрганаУведомления;
				ИначеЕсли Уведомление.ГруппаВычета = Перечисления.ГруппыВычетовПоНДФЛ.Социальные Тогда
					ОбластьМакетаУведомлениеНаСоцВычет.Параметры.НомерУведомления = Уведомление.НомерУведомления;
					ОбластьМакетаУведомлениеНаСоцВычет.Параметры.ДатаУведомления = Уведомление.ДатаУведомления;
					ОбластьМакетаУведомлениеНаСоцВычет.Параметры.КодНалоговогоОрганаУведомления = Уведомление.КодНалоговогоОрганаУведомления;
				КонецЕсли;
			КонецЦикла;
			...
			Ставка = Перечисления.НДФЛСтавкиУНФ.Ставка13;
			...
			Если ПерваяСтавка Тогда
				...
				Если ОбластьМакетаУведомлениеНаСоцВычет = Неопределено Тогда
					...
					КонецЕсли;
				ИначеЕсли ВычетыПоВидам.Количество() = 0 Или Ставка <> Перечисления.НДФЛСтавкиУНФ.Ставка13 Тогда
				...
				Иначе
					...
					Если ВычетыПоВидам.Найти(Перечисления.ГруппыВычетовПоНДФЛ.СоциальныеПоУведомлениюНО, "ГруппаВычета") <> Неопределено Тогда
						ВывестиУведомления(ВременныйТабДокумент, ОбластьМакетаУведомлениеНаСоцВычет, ДанныеОбУведомленияхДокументов, Перечисления.ГруппыВычетовПоНДФЛ.Социальные)
					ИначеЕсли ДанныеНалоговогоАгента.НалоговыйПериод >= 2015 Тогда	
						ВременныйТабДокумент.Вывести(ОбластьМакетаУведомлениеНаСоцВычет);
					КонецЕсли;
					Если ВычетыПоВидам.Найти(Перечисления.ГруппыВычетовПоНДФЛ.Имущественные, "ГруппаВычета") <> Неопределено Тогда
						ВывестиУведомления(ВременныйТабДокумент, ОбластьМакетаУведомлениеНаИмущВычет, ДанныеОбУведомленияхДокументов, Перечисления.ГруппыВычетовПоНДФЛ.Имущественные)
					Иначе	
						ВременныйТабДокумент.Вывести(ОбластьМакетаУведомлениеНаИмущВычет);
					КонецЕсли;
				КонецЕсли;
			...
			Если ВыводитьФормуСправки2022 Тогда
				...
				ОбластьМакетаРаздел6.Параметры.Заполнить(СтрокаДанных); 
				ОбластьМакетаРаздел6.Параметры.СуммаДоходаНеудержанного = 0;
				//
...
Процедура ЗаполнитьСправку2НДФЛПоСотруднику(ДокСправка) Экспорт
	
	Если Не ЗначениеЗаполнено(докСправка.НалоговыйПериод) Тогда
		Если Месяц(ТекущаяДата())> 1 Тогда
			Год = Год(ТекущаяДата());
		Иначе
			Год = Год(ТекущаяДата())-1;
		КонецЕсли;
		докСправка.НалоговыйПериод = Год;
	КонецЕсли;
	
	докСправка.Дата = ТекущаяДатаСеанса();
	докСправка.РегистрацияВНалоговомОргане = РегламентированнаяОтчетностьУСН.ПолучитьРегистрациюВИФНС(докСправка.Организация);
	...
	ЗапросПоОрганизации.УстановитьПараметр("Ссылка", докСправка.Организация);
	...
	Если Выборка.Следующий() Тогда
		...
		докСправка.КодНалоговогоОрганаПолучателя = Выборка.КодНалоговогоОрганаПолучателя;
		ЗаполнитьЗначенияСвойств(докСправка, ДанныеПодписывающихЛиц(докСправка.Организация, докСправка.Дата));
	...
	СтрокаСотрудника.Сотрудник = докСправка.Сотрудник;
	СтрокаСотрудника.Ставка = Перечисления.НДФЛСтавкиУНФ.Ставка13;
	ДанныеСотрудника = ДанныеСотрудников(ТаблицаСотрудников, докСправка.Дата, докСправка.НалоговыйПериод);
	ДанныеСотрудника.Колонки.Добавить("НомерСправки",Новый ОписаниеТипов("Число"));
	
	СтруктураДанных = ДанныеОДоходах(ДанныеСотрудника, докСправка.НалоговыйПериод, докСправка.Организация, докСправка.Организация, докСправка.ОКАТО_КПП, докСправка.Дата);
	...
	Если докСправка.НалоговыйПериод < 2011 Тогда
		Для Каждого СтрокаТЧ Из ДанныеСотрудника Цикл
	...
	Если ДанныеСотрудника.Количество() > 0 Тогда
		ЗаполнитьЗначенияСвойств(докСправка, ДанныеСотрудника[0]);
	КонецЕсли;
...
Функция ПолучитьИмяФайлаНДФЛ(СправкаНДФЛ) Экспорт
...
Функция СформироватьВыходнойФайл2011(Объект, ИмяФайла)
	...
	Для каждого СтрокаДанных Из СотрудникиОрганизации Цикл 
		...
		Для каждого СтрокаИтогов Из Итоги Цикл
			...
			// Налоговые вычеты
			Если ВычетыПоВидам.Количество() > 0 И СтрокаИтогов.Ставка = Перечисления.НДФЛСтавкиУНФ.Ставка13 Тогда
...
Функция СформироватьВыходнойФайл2015(Объект, ИмяФайла, НалоговыйПериод)
	...
	// -----------------------------------------------------------------------------
	// СВЕДЕНИЯ О НАЛОГОВОМ АГЕНТЕ
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект, "Организация, НалоговыйПериод, ОКАТО_КПП, Дата, Телефон, ДолжностьПодписавшегоЛица, СправкуПодписал, ИмяФайла, НомерКорректировки");
	СтруктураДанныхНА = ДанныеНалоговогоАгентаДляНДФЛ(СтруктураРеквизитов.Организация, СтруктураРеквизитов.НалоговыйПериод, СтруктураРеквизитов.ОКАТО_КПП, СтруктураРеквизитов.Дата, СтруктураРеквизитов.Телефон, СтруктураРеквизитов.ДолжностьПодписавшегоЛица, СтруктураРеквизитов.СправкуПодписал);
	СтруктураДанныхНА.Признак = "1";
	СтруктураДанныхНА.ОКТМО = СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(СтруктураДанныхНА.ОКТМО, "-", "Справа");
	ОрганизацияЭтоФизЛицо = Не СтруктураДанныхНА.ЭтоЮрЛицо;
	...
	ФорматСправка.НомКорр.Значение = "00"; // исходные справки
	ФорматСправка.КодНО.Значение = СтруктураДанныхНА.КодНалоговогоОргана;
	ФорматСправка.НомКорр.Значение = Формат(СтруктураРеквизитов.НомерКорректировки,"ЧЦ=2; ЧН=00; ЧВН="); 
	...
	Для каждого СтрокаДанных Из СотрудникиОрганизации Цикл 
		...
		// Адреса
		Если ЗначениеЗаполнено(СтрокаДанных.АдресРФ) Тогда
			СтруктураАдреса = РаботаСАдресами.СведенияОбАдресе(СтрокаДанных.АдресРФ);
...
Функция СформироватьВыходнойФайл2018(Объект, ИмяФайла, НалоговыйПериод)
	...
	// -----------------------------------------------------------------------------
	// СВЕДЕНИЯ О НАЛОГОВОМ АГЕНТЕ
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект, 
		"Организация, НалоговыйПериод, ОКАТО_КПП, Дата, Телефон, ДолжностьПодписавшегоЛица, СправкуПодписал, ИмяФайла,ОснованиеДляПредставленияСправок,ФормаРеорганизации,КППРеорганизованнойОрганизации,ИННРеорганизованнойОрганизации,НаименованиеРеорганизованнойОрганизации, НомерКорректировки");
	СтруктураДанныхНА = ДанныеНалоговогоАгентаДляНДФЛ(СтруктураРеквизитов.Организация, СтруктураРеквизитов.НалоговыйПериод, СтруктураРеквизитов.ОКАТО_КПП, СтруктураРеквизитов.Дата, СтруктураРеквизитов.Телефон, СтруктураРеквизитов.ДолжностьПодписавшегоЛица, СтруктураРеквизитов.СправкуПодписал);
	СтруктураДанныхНА.Признак = "1";
	Если СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛ Тогда
		СтруктураДанныхНА.Признак = "2"
	ИначеЕсли СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетность Тогда
		СтруктураДанныхНА.Признак = "1"
	ИначеЕсли СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетностьЗаРеорганизованнуюОрганизацию Тогда
		СтруктураДанныхНА.Признак = "3"
	ИначеЕсли СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛЗаРеорганизованнуюОрганизацию Тогда
	...
	СтруктураДанныхНА.Вставить("КППОтправки", "");
	СтруктураДанныхНА.Вставить("ФормаРеорганизации", СтруктураРеквизитов.ФормаРеорганизации);
	СтруктураДанныхНА.Вставить("ИННРеорганизованнойОрганизации", СтруктураРеквизитов.ИННРеорганизованнойОрганизации);
	СтруктураДанныхНА.Вставить("КППРеорганизованнойОрганизации", СтруктураРеквизитов.КППРеорганизованнойОрганизации);
	Если ЗначениеЗаполнено(СтруктураРеквизитов.ФормаРеорганизации) Тогда
		СтруктураДанныхНА.НазваниеОрганизации = СтруктураРеквизитов.НаименованиеРеорганизованнойОрганизации
	КонецЕсли;
	
	// Загружаем формат файла сведений
	ДеревоФорматаXML = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ПолучитьМакет("ФорматНДФЛ2018XML");
	ТекстФорматаXML = ДеревоФорматаXML.ПолучитьТекст();
	...
	Для каждого СтрокаДанных Из СотрудникиОрганизации Цикл 
		...
		Для каждого СтрокаИтогов Из Итоги Цикл
			...
			СтруктураСГДНалПер.НалБаза.Значение = СтрокаИтогов.ОблагаемаяСуммаДохода;
			СтруктураСГДНалПер.НалИсчисл.Значение = СтрокаИтогов.Исчислено;
			Если Не ЗначениеЗаполнено(СтруктураРеквизитов.ОснованиеДляПредставленияСправок)
				ИЛИ СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетность
				ИЛИ СтруктураРеквизитов.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетностьЗаРеорганизованнуюОрганизацию Тогда
				СтруктураСГДНалПер.НалУдерж.Значение = СтрокаИтогов.Удержано;
				СтруктураСГДНалПер.НалПеречисл.Значение = СтрокаИтогов.Перечислено;
				СтруктураСГДНалПер.НалУдержЛиш.Значение = СтрокаИтогов.ИзлишнеУдержано;
			КонецЕсли;
...
Функция СформироватьВыходнойФайл2019(Объект, ИмяФайла, НалоговыйПериод)
	...
	// Загружаем формат файла сведений
	ДеревоФорматаXML = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ПолучитьМакет("ФорматНДФЛ2019XML");
	ТекстФорматаXML = ДеревоФорматаXML.ПолучитьТекст();

	ДеревоФормата = ЗагрузитьXMLВДокументDOM(ТекстФорматаXML);
	ФорматФайла = ЗагрузитьФорматНабораЗаписей(ДеревоФормата, "Файл");
	...
	УзелДокумент = ДобавитьУзелВДеревоXML(Узел2НДФЛ, "Документ", "", АтрибутыВыгружаемогоУзла(ФорматСправка));
	
	// Данные о налоговом агенте
	НаборНалоговыйАгент = ФорматСправка.СвНА.Значение;
	...
	УзелНалоговыйАгент = ДобавитьУзелВДеревоXML(УзелДокумент, "СвНА","", Атрибуты);
	ДобавитьИнформациюВДеревоБезУдаленияАтрибутов(УзелНалоговыйАгент, НаборНалоговыйАгент);
	...
	УзелПодписант = ДобавитьУзелВДеревоXML(УзелДокумент, "Подписант","", Атрибуты);
	ДобавитьИнформациюВДеревоБезУдаленияАтрибутов(УзелПодписант, НаборЗаписейПодписант);
	
	// НДФЛ-2
	ОтборЗаписейРегистров = Новый Структура("ДокументОснование", Объект);
	СведенияОДоходахФизическихЛиц = ПакетЗапросов.СведенияОДоходах;
	СведенияОВычетахФизическихЛиц = ПакетЗапросов.СведенияОВычетах;
	
	СтруктураДанныхСправки = ОбщегоНазначения.СкопироватьРекурсивно(ФорматСправка.НДФЛ__2.Значение);
	...
	Для каждого СтрокаДанных Из СотрудникиОрганизации Цикл 
		...
		СтруктураДанныхСправки.НомКорр.Значение = Формат(СтруктураРеквизитов.НомерКорректировки,"ЧЦ=2; ЧН=00; ЧВН="); 
		...
		НаборЗаписейПолучателиДохода.Статус.Значение =1;
		...
		НаборЗаписейДокУдЛичности.КодУдЛичн = ?(Описание = Неопределено, "", Описание);
		НаборЗаписейДокУдЛичности.СерНомДок = СокрЛП(СтрокаДанных.СерияДокумента) + " " + СокрЛП(СтрокаДанных.НомерДокумента);
		
		НаборЗаписейСведенияОДоходах = ОбщегоНазначения.СкопироватьРекурсивно(ФорматСправка.НДФЛ__2.Значение.СведДох.Значение);
		СтруктураДанныхСправки.Удалить("СведДох");
		...
		Узел2НДФЛ = ДобавитьУзелВДеревоXML(УзелДокумент, "НДФЛ-2", , Атрибуты);
		ДобавитьИнформациюВДеревоБезУдаленияАтрибутов(Узел2НДФЛ, СтруктураДанныхСправки);
		
		// Сведения о доходах не заполняются если справка отменяющая
		Если СтруктураРеквизитов.НомерКорректировки = 99 Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого СтрокаИтогов Из Итоги Цикл
			...
			УзелСведенияОДоходах = ДобавитьУзелВДеревоXML(Узел2НДФЛ, "СведДох","", Атрибуты);
			ДобавитьИнформациюВДерево(УзелСведенияОДоходах, НаборЗаписейСведенияОДоходах);
...
Функция ИзвлечьДоходыПоСтавке(Ставка, СтатусНалогоплательщика, ДоходыПоВидам)
	...
	Для каждого СтрокаТЗ Из ДоходыПоВидам Цикл
		...
		СтавкаДохода = Перечисления.НДФЛСтавкиУНФ.Ставка13;
...
Функция ПреобразоватьСтрокуВСтруктуру(СтрокаПолей)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоСтрокаXML(СтрокаПолей) Тогда
		Результат = УчетСтраховыхВзносовУНФ.СтруктураПолейАдресаИзСпискаПолейКонтактнойИнформации(СтрокаПолей, УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтрокаПолей));
	Иначе
