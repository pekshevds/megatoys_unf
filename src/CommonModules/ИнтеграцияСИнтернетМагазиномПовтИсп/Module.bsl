...
//	СправочникСсылка.КлючиИнтеграций - найденная валюта.
//
Функция ПолучитьКлючПоНастройке(НастройкаИнтеграции) Экспорт
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаИнтеграции, "КлючИнтеграции");
	
КонецФункции

Функция ЭтоДокумент(ТипСсылкиНаДокументСтрокой) Экспорт
	ТипСсылкиНаДокумент = ЗначениеИзСтрокиВнутр(ТипСсылкиНаДокументСтрокой);
	Возврат Документы.ТипВсеСсылки().СодержитТип(ТипСсылкиНаДокумент);
КонецФункции

Функция ПолучитьУзелОбменаПоНастройке(НастройкаИнтеграции) Экспорт
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаИнтеграции, "УзелОбменаССайтом");
...
// Определяет, используется ли обмен с интернет магазином.
//
Функция ИспользоватьИнтеграциюСИнтернетМагазином() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат Константы.ИспользоватьИнтеграциюСИнтернетМагазином.Получить();
...
Функция ОтображатьКнопкуСортировкиНаФорме() Экспорт
	...
	Если НЕ ПравоДоступа("Редактирование", Метаданные.ПланыОбмена.ОбменУправлениеНебольшойФирмойСайт)
		Или НЕ ПравоДоступа("Редактирование", Метаданные.Справочники.НастройкиИнтеграцииСИнтернетМагазином) Тогда 
		Возврат Ложь;
	КонецЕсли;
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	НастройкиИнтеграцииСИнтернетМагазином.Ссылка
	|ИЗ
	|	Справочник.НастройкиИнтеграцииСИнтернетМагазином КАК НастройкиИнтеграцииСИнтернетМагазином
	|ГДЕ
	|	НастройкиИнтеграцииСИнтернетМагазином.ВыгружатьКартинки
	...
	// проверим Ozon
	Если ПравоДоступа("Редактирование", Метаданные.Справочники.УчетныеЗаписиМаркетплейсов) Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
