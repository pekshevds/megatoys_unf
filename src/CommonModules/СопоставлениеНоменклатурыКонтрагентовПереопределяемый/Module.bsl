...
//
// Параметры:
//  Форма           - ФормаКлиентскогоПриложения                - форма, из обработчика события которой происходит вызов
//                                                                процедуры.
//  ТекущийОбъект   - СправочникОбъект.НоменклатураКонтрагентов - объект номенклатуры контрагента.
//  ПараметрыЗаписи - Структура                                 - структура параметров записи.
//
...
//   * ЕдиницаИзмеренияПоУмолчанию - ОпределяемыйТип.УпаковкаНоменклатурыБЭД       - ссылка базовой единицы измерения
//                                                                                   номенклатуры ИБ, которую необходимо
//                                                                                   отобразить пользователю в подсказке.
//
Процедура ПриИзмененииНоменклатурыСопоставления_НоменклатураКонтрагентов(Форма, ДанныеСопоставления) Экспорт
	
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриИзмененииНоменклатурыСопоставления_НоменклатураКонтрагентов(Форма, ДанныеСопоставления);
	// КлиентЭДО конец
...
//  УпаковкаОтбор       - ОпределяемыйТип.УпаковкаНоменклатурыБЭД       - упаковка/единица измерения. Если в форму
//                                                                        выбора номенклатуры контрагента передавалась
//                                                                        пустая упаковка/единица измерения - она будет
//                                                                        перезаполнена единицей хранения номенклатуры,
//                                                                        если номенклатура была передана в параметрах.
...
//  Владелец       - ОпределяемыйТип.ВладелецНоменклатурыБЭД       - владелец, для которого необходимо сформировать
//                                                                   список выбора.
//  Номенклатура   - ОпределяемыйТип.НоменклатураБЭД               - номенклатура предприятия для фильтрации
//                                                                   номенклатуры контрагента.
//  Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - характеристика номенклатуры предприятия для
//                                                                   фильтрации номенклатуры контрагента.
//  Упаковка       - ОпределяемыйТип.УпаковкаНоменклатурыБЭД       - упаковка номенклатуры предприятия для фильтрации
//                                                                   номенклатуры контрагента.
//
// Параметры:
...
//  СвойстваУпаковки - Структура из см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовыеСвойстваУпаковки -
//                     свойства упаковки.
//
Процедура ПриОпределенииСвойствУпаковкиЕдиницыИзмеренияНоменклатуры(Знач Упаковка, Знач Номенклатура, СвойстваУпаковки) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОпределенииСвойствУпаковкиЕдиницыИзмеренияНоменклатуры(Упаковка, Номенклатура, СвойстваУпаковки);
	// КлиентЭДО конец
...
Процедура НайтиНаименованиеЕдиницыИзмеренияПоКодуОКЕИ(КодОКЕИЕдиницыИзмерения, НаименованиеЕдиницыИзмерения) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.НайтиНаименованиеЕдиницыИзмеренияПоКодуОКЕИ(КодОКЕИЕдиницыИзмерения, НаименованиеЕдиницыИзмерения);
	// КлиентЭДО конец
...
//  ВладелецУпаковкиНоменклатура - Булево                                  - признак того, что владельцем упаковки
...
Процедура ВладелецУпаковкиЕдиницыИзмеренияНоменклатура(Знач Упаковка, ВладелецУпаковкиНоменклатура) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ВладелецУпаковкиЕдиницыИзмеренияНоменклатура(
		Упаковка, ВладелецУпаковкиНоменклатура);
	// КлиентЭДО конец
...
Процедура БазоваяЕдиницаИзмеренияНоменклатуры(Знач Номенклатура, ЕдиницаИзмерения) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.БазоваяЕдиницаИзмеренияНоменклатуры(Номенклатура, ЕдиницаИзмерения);
	// КлиентЭДО конец
...
Процедура ПриОпределенииСвойствНоменклатурыИнформационнойБазы(Знач НаборНоменклатуры, СвойстваНоменклатурИБ) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОпределенииСвойствНоменклатурыИнформационнойБазы(НаборНоменклатуры, СвойстваНоменклатурИБ);
	// КлиентЭДО конец
...
Процедура ПриОпределенииПредставленийСопоставленияНоменклатуры(Представления) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОпределенииПредставленийСопоставленияНоменклатуры(Представления);
	// КлиентЭДО конец
...
Процедура ПриОпределенииСтруктурыНоменклатурыИнформационнойБазы(СтруктураНоменклатуры) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОпределенииСтруктурыНоменклатурыИнформационнойБазы(СтруктураНоменклатуры);
	// КлиентЭДО конец
...
Процедура ПриОпределенииШтрихкодовНоменклатурыКонтрагента(Знач НоменклатураИБ, Штрихкод, ДругиеШтрихкодыНоменклатуры) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОпределенииШтрихкодовНоменклатурыКонтрагента(НоменклатураИБ, Штрихкод,
		ДругиеШтрихкодыНоменклатуры);
	// КлиентЭДО конец
...
Процедура ТекстЗапросаПоискаНеСопоставленнойНоменклатурыКонтрагентовПоНоменклатуреИБ(ТекстЗапроса, ТаблицаТоваровПоУмолчанию) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаПоискаНеСопоставленнойНоменклатурыКонтрагентовПоНоменклатуреИБ(ТекстЗапроса, ТаблицаТоваровПоУмолчанию);
	// КлиентЭДО конец
...
//   * СсылкаНаДокумент                     - ДокументСсылка                                - документ, участвующий в
//       сопоставление номенклатуры БЭД.
//   * НоменклатураКонтрагента              - СправочникСсылка.НоменклатураКонтрагентов     - ссылка номенклатуры контрагента.
//   * НомерСтроки                          - Строка                                        - номер строки.
//   * Номенклатура                         - ОпределяемыйТип.НоменклатураБЭД               - ссылка номенклатуры.
//   * Характеристика                       - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - ссылка характеристики.
//   * Упаковка                             - ОпределяемыйТип.УпаковкаНоменклатурыБЭД       - ссылка упаковки. Если
//                                                                                            Номенклатура заполнена в
//                                                                                            документа, упаковка должна
//                                                                                            быть заполнена. Необходимо
//                                                                                            передавать упаковку или
//                                                                                            базовую единицу измерения номенклатуры.
//   * НоменклатураПоиск                    - ОпределяемыйТип.НоменклатураБЭД               - ссылка номенклатуры, по
//                                                                                            которой необходимо искать
//                                                                                            номенклатуру контрагента.
//                                                                                            Может отличаться от
//                                                                                            номенклатуры в документе,
//                                                                                            например, когда в
//                                                                                            документа товар другого качества.
//   * УпаковкаПоиск                        - ОпределяемыйТип.УпаковкаНоменклатурыБЭД      -  ссылка упаковки, по
//       которой необходимо искать номенклатуру контрагента.
//   * ХарактеристикаПоиск                  - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - ссылка характеристики, по
//       которой необходимо искать номенклатуру контрагента.
//   * ИмяТабличнойЧасти                    - Строка                                        - имя табличной части, где
//                                                                                            располагается колонка с
//                                                                                            типом Справочник
//                                                                                            НоменклатураКонтрагентов в
//                                                                                            документе. Если колонка не
//                                                                                            используется необходимо
//                                                                                            передать пустую строку.
//   * ИмяКолонкиНоменклатурыКонтрагента    - Строка                                        - имя колонки с типом
//                                                                                            Справочник
//                                                                                            НоменклатураКонтрагентов в
//                                                                                            документе. Если колонка не
//                                                                                            используется необходимо
//                                                                                            передать пустую строку.
//   * ПредставлениеНоменклатурыКонтрагента - Строка                                        - представление колонки с
//                                                                                            типом Справочник
//                                                                                            НоменклатураКонтрагентов в
//                                                                                            документе, которое
//                                                                                            необходимо вывести
//                                                                                            пользователю. Если колонка
//                                                                                            не используется необходимо
//                                                                                            передать пустую строку.
// Если в табличной части товаров в документе нет колонки с типом справочник НоменклатураКонтрагентов:
//  - установить ОткрыватьФормуДляИзмененияСопоставления = Истина, чтобы в случае неоднозначного сопоставления или
//  неверного вывелась форма
// для редактирования сопоставления. 
//  - описать логику получения номенклатуры контрагента для данных документа.
//
// Параметры:
//  НаборДокументов                         - ДокументСсылка, Массив из ДокументСсылка - документы,в которых необходимо
//      проверить сопоставление номенклатуры контрагентов БЭД.
//  ТаблицаСопоставления                    - ТаблицаЗначений                          - переопределяемая таблица
//      сопоставления номенклатуры контрагентов.
//  ОткрыватьФормуДляИзмененияСопоставления - Булево                                   - признак необходимости открыть
//                                                                                       форму для редактирования
//                                                                                       сопоставления номенклатуры
//                                                                                       контрагента при наличии
//                                                                                       неоднозначного сопоставления
//                                                                                       или неверного. По умолчанию
//                                                                                       установлено Истина.
//
Процедура СформироватьТаблицуСопоставленияНоменклатурыКонтрагентовВДокументах(
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.СформироватьТаблицуСопоставленияНоменклатурыКонтрагентовВДокументах(
...
//   * СсылкаНаДокумент        - ДокументСсылка                                - документ, участвующий в сопоставление
//                                                                               номенклатуры БЭД.
//   * НоменклатураКонтрагента - СправочникСсылка.НоменклатураКонтрагентов     - ссылка номенклатуры контрагента.
...
//
// Параметры:
//  НоменклатураКонтрагента - Структура из см.
//      СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагента        - данные номенклатуры контрагента.
//  НоменклатураИБ          - Структура из см.
//      СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураИнформационнойБазы - данные номенклатуры
//                                                                                              информационной базы.
//  Отказ                   - Булево                                                                               - при
...
//                                             передана информация для сопоставления, тогда отражение в учете будет
...
Процедура ПриОтраженииВУчетеЭлектронногоДокументаСНоменклатурой(Знач Документ, Настройки) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриОтраженииВУчетеЭлектронногоДокументаСНоменклатурой(Документ, Настройки);
	// КлиентЭДО конец
...
Процедура ЗначениеНаименованияДанныхСопоставленияНоменклатурыБЭД(Знач СсылкаНаОбъект, ЗначениеНаименования) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ЗначениеНаименованияДанныхСопоставленияНоменклатурыБЭД(СсылкаНаОбъект, ЗначениеНаименования);
	// КлиентЭДО конец
...
Процедура ЗначенияНаименованийДанныхСопоставленияНоменклатурыБЭД(Знач НаборСсылокНаОбъекты, СоответствиеЗначенийНаименований) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ЗначенияНаименованийДанныхСопоставленияНоменклатурыБЭД(НаборСсылокНаОбъекты, СоответствиеЗначенийНаименований);
	// КлиентЭДО конец
...
Процедура ОпределитьНаименованиеТипаОбъектаСопоставленияНоменклатурыБЭД(Знач СсылкаНаОбъект, НаименованиеТипаОбъекта) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ОпределитьНаименованиеТипаОбъектаСопоставленияНоменклатурыБЭД(СсылкаНаОбъект, НаименованиеТипаОбъекта);
	// КлиентЭДО конец
...
Процедура ТекстЗапросаПоискаВариантовСопоставленияПоШтрихкодамКомбинаций(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаПоискаВариантовСопоставленияПоШтрихкодамКомбинаций(ТекстЗапроса);
	// КлиентЭДО конец
...
//   * Номенклатура              - ОпределяемыйТип.НоменклатураБЭД - ссылка сопоставленной номенклатуры по штрихкоду
//                                                                   номенклатуры контрагента и информационной базой.
//   * ШтрихкодСопоставлен       - Булево                          - признак сопоставленного штрихкода.
//   * ЭтоНашаНоменклатура       - Булево                          - признак нашей номенклатуры, иначе номенклатура
//                                                                   контрагента - из ТаблицаНатуральныхКлючей.
//
...
//   * ИдентификаторНоменклатуры - Строка                          - хеш номенклатуры контрагента из ТаблицаНатуральныхКлючей.
//   * Номенклатура              - ОпределяемыйТип.НоменклатураБЭД - ссылка сопоставленной номенклатуры по артикулу
//                                                                   номенклатуры контрагента и информационной базой.
//   * АртикулСопоставлен        - Булево                          - признак сопоставленного артикула.
//   * ЭтоНашаНоменклатура       - Булево                          - признак нашей номенклатуры, иначе номенклатура
//                                                                   контрагента - из ТаблицаНатуральныхКлючей.
//
//  ТаблицаСопоставленныхПоНаименованию - необходимо найти номенклатуру ИБ по наименованию номенклатуры, которая с
//   признаком ЭтоНашаНоменклатура = Истина:
//   * Наименование              - Строка                          - наименование номенклатуры контрагента из ТаблицаНатуральныхКлючей.
//   * ИдентификаторНоменклатуры - Строка                          - хеш номенклатуры контрагента из ТаблицаНатуральныхКлючей.
//   * Номенклатура              - ОпределяемыйТип.НоменклатураБЭД - ссылка сопоставленной номенклатуры по наименованию
//                                                                   (полному или краткому) номенклатуры контрагента и
...
Процедура ИнициализацияТекстаЗапросаПоискаСопоставленияПоНатуральнымКлючам(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ИнициализацияТекстаЗапросаПоискаСопоставленияПоНатуральнымКлючам(ТекстЗапроса);
	// КлиентЭДО конец
...
Процедура ТекстЗапросаОтбораХарактеристикНоменклатурыБЭДПоВладельцу(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаОтбораХарактеристикНоменклатурыБЭДПоВладельцу(ТекстЗапроса);
	// КлиентЭДО конец
...
Процедура ТекстЗапросаПоискаВариантовСопоставленияХарактеристикиСНашейНоменклатурой(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаПоискаВариантовСопоставленияХарактеристикиСНашейНоменклатурой(ТекстЗапроса);
	// КлиентЭДО конец
...
Процедура ТекстЗапросаОтбораСтрокСопоставленияДляЗаполненияХарактеристик(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаОтбораСтрокСопоставленияДляЗаполненияХарактеристик(ТекстЗапроса);
	// КлиентЭДО конец
...
//
// Рекомендации поиска:
//  Упаковки рекомендуется искать по полному наименованию (если используется), по наименованию и по наименованию и/или
//  коду базовой единицы измерения, отобрав упаковки по владельцу (если владельцем упаковки является номенклатура).
...
//  * НоменклатураИБ        - ОпределяемыйТип.НоменклатураБЭД         - ссылка сопоставленной номенклатуры ИБ.
//  * ИдентификаторУпаковки - Строка                                  - идентификатор упаковки по данным номенклатуры контрагента.
//  * УпаковкаИБ            - ОпределяемыйТип.УпаковкаНоменклатурыБЭД - найденная упаковка или единица измерения по
//                                                                      данным номенклатуры контрагента.
//
// Параметры:
//  ТекстЗапроса - Строка - текст запроса по данным информационной базы.
//
Процедура ТекстЗапросаПоискаВариантовСопоставленияУпаковкиНоменклатурыБЭД(ТекстЗапроса) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ТекстЗапросаПоискаВариантовСопоставленияУпаковкиНоменклатурыБЭД(ТекстЗапроса);
	// КлиентЭДО конец

КонецПроцедуры

// Выполняется при поиске вариантов сопосотавления номенклатуры перед ПодобратьВариантыСопоставленияПоНатуральнымКлючам.
...
Процедура ПриЗаполненииФормыНоменклатурыПоДаннымКонтрагента(Знач НоменклатураКонтрагента, Форма) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриЗаполненииФормыНоменклатурыПоДаннымКонтрагента(НоменклатураКонтрагента, Форма);
	// КлиентЭДО конец
...
Процедура ПриЗаполненииФормыХарактеристикиПоДаннымКонтрагента(Знач НоменклатураКонтрагента, Форма) Экспорт
	...
	// КлиентЭДО начало
	СопоставлениеНоменклатурыКонтрагентовКЭДО.ПриЗаполненииФормыХарактеристикиПоДаннымКонтрагента(НоменклатураКонтрагента, Форма);
	// КлиентЭДО конец
