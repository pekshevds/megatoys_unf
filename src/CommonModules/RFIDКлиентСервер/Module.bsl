...
Процедура ЗаполнитьФлагиРаботыСМеткой(ОбрабатываемаяСтрока, GTIN, ТекущаяМетка, НастройкиИспользованияСерий,
	ЭтоМаркировкаПерсонифицированнымиКиЗ, ТекстСообщения) Экспорт
	...
	Если ЗначениеЗаполнено(GTIN)
		...
		Если ОбрабатываемаяСтрока.ЗаполненRFIDTID Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр(
					"ru = 'При маркировке неперсонифицированными КиЗ номер серии должен быть сгенерирован по TID RFID-метки.
					|При генерации произошла ошибка: %ТекстОшибки%. Обратитесь к администратору.'");
