...
#Область ПодсказкаВключенияВалют

// Процедура открывает диалог включения опции нескольких валют при попытке изменить валюту
//
Процедура ОткрытьДиалогВключенияНесколькихВалют() Экспорт
	
	ОповещениеОВыборе = Новый ОписаниеОповещения("ПереходКНастройкамВалюты", ЭтотОбъект);
	
	ПараметрыДиалога = РасчетыРаботаСФормамиВызовСервера.ПараметрыДиалогаВключенияУчетаПоВалютам();
	
	Если ПараметрыДиалога.ЭтоВопрос Тогда
		ПоказатьВопрос(ОповещениеОВыборе, ПараметрыДиалога.ТекстСообщения, ПараметрыДиалога.Кнопки);
	Иначе
		ПоказатьПредупреждение(, ПараметрыДиалога.ТекстСообщения);
	КонецЕсли;
	
КонецПроцедуры

// Процедура обработчик оповещения результата выбора в диалоге валюты
//
// Параметры:
//  ВыбранныйВариант		 - Булево	 - Определяет необходимость перехода к настройкам валюты
//  ДополнительныеПараметры	 - Неопределено	 - Дополнительные параметры обработчика
//
Процедура ПереходКНастройкамВалюты(ВыбранныйВариант, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйВариант = Ложь ИЛИ ВыбранныйВариант = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("СтрокаПоиска", НСтр("ru = 'Валюты'"));
...
Процедура РасшифровкаПлатежаСуммаПлатежаПриИзмененииЗавершениеНаКлиенте(СтрокаТабличнойЧасти,
	...
	Иначе
		СтрокаТабличнойЧасти.Курс = ?(
			...
			СтрокаТабличнойЧасти.СуммаПлатежа * Кратность / СтрокаТабличнойЧасти.СуммаРасчетов * Курс * СтрокаТабличнойЧасти.Кратность);
	КонецЕсли;
...
Процедура ОткрытьФормуИтоговКонтекстноеМеню(Форма, ИмяФормыРодителя) Экспорт
	...
	Если ИмяФормыРодителя = "БанкИКасса" ИЛИ ИмяФормыРодителя = "Банк" ИЛИ ИмяФормыРодителя = "Касса" Тогда
		...
		Если Форма.ИмяФормы = "Обработка.БанкИКасса.Форма.ФормаСписка" Тогда
			ПараметрыИтогов.Вставить("ОтборПериод", 	 		 Форма.ОтборПериодБанковскиеВыписки);
		Иначе
...
Процедура ПриИзмененииКурсаВСтрокеРасшифровки(Форма, ИмяТабличнойЧасти, ИмяПоля = "") Экспорт
	...
	РассчитатьСуммуПлатежаНаКлиенте(Форма, СтрокаТабличнойЧасти, ИмяПоля);
...
// Заполняет поле Договор в переданной строке таблицы формы
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ТекущиеДанные - ДанныеФормыЭлементКоллекции
Процедура ЗаполнитьДоговорВНовойСтрокеРасшифровки(Форма, ТекущиеДанные) Экспорт
	
	Если Форма.Объект.Контрагент = Форма.ВладелецДоговораПоУмолчанию Тогда
		ТекущиеДанные.Договор = Форма.ДоговорКонтрагентаПоУмолчанию;
	ИначеЕсли Форма.ДоговорКонтрагентаПоУмолчанию.Пустая() Тогда
		ТекущиеДанные.Договор = ДоговорыКонтрагентовУНФВызовСервера.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
			Форма.Объект.Ссылка, Форма.Объект.Контрагент, Форма.Объект.Организация, Форма.Объект.ВидОперации);
	КонецЕсли;
