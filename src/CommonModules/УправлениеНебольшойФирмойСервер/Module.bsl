...
Процедура СообщитьОбОшибкахПроведенияПоРегиструПрослеживаемыеТовары(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Номенклатура: %НоменклатураХарактеристикаПартия% (%СтранаПроисхождения%), с комплектующим %Комплектующие%
		|РНПТ: %РНПТ%'");
	...
	ЕстьСвойствоКомплектующие = Не ВыборкаЗаписей.Владелец().Колонки.Найти("Комплектующие") = Неопределено;
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ЕстьСвойствоКомплектующие И ВыборкаЗаписей.ПрослеживаемыйКомплект Тогда
			ПредставлениеКомплектующего = ?(ЗначениеЗаполнено(ВыборкаЗаписей.Комплектующие), ВыборкаЗаписей.Комплектующие, НСтр("ru = '<не выбрано>'"));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Комплектующие%", ВыборкаЗаписей.Комплектующие);
		Иначе
			ТекстСообщения = СтрЗаменить(ТекстСообщения, НСтр("ru = 'с комплектующим %Комплектующие%'"), "");
		КонецЕсли;
