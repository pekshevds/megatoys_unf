...
Функция ЕстьПравоНаОрганПоОрганизации(Организация, Орган, ПроверятьДляТекущегоПользователя = Истина) Экспорт
	...
	ПроверятьДоступ = 
		...
		И Организация.УчетнаяЗаписьОбмена.ЭтоМультиРежим;
...
Функция ПоддерживаетсяМультирежим() Экспорт
	...
	Поддерживается = 
		НЕ ЭтоИнтеграцияСБанком
		И НЕ ЭтоРежимОграниченнойФункциональности
		И НЕ ЭтоРежимБесплатнойНулевойОтчетности;
...
Функция ИмяТекущегоПользователя() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Возврат Пользователи.СвойстваПользователяИБ(Пользователи.ТекущийПользователь().ИдентификаторПользователяИБ).Имя
