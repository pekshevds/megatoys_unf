...
Функция ИменаСобытийЖР(Приложение) Экспорт
	...
	// Имя общего события - ИнтеграцияУпралениеПерсоналом.
...
Функция ИмяСобытияОтключение(НазваниеГруппыСобытийЖР) Экспорт
...
Функция ИмяОбщегоСобытияЖР() Экспорт
	...
	Возврат НСтр("ru = 'ИнтеграцияУпралениеПерсоналом'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОбъектыДляРегистрацииПриЗаписиНовыхПравил(Приложение, ДанныеДляРегистрации)
	...
	ТипыОсновныхОбъектов = ИнтеграцияУправлениеПерсоналом.ТипыОсновныхОбъектов();
	...
	ТекущиеВыгружаемые = ИнтеграцияУправлениеПерсоналом.ОбъектыВыгружаютсяВПриложение(ВыгружаемыеФизическихЛиц, Приложение);
	...
	Для каждого ФизическоеЛицо Из НовыеФизическиеЛица Цикл
		...
		НоваяСтрока.ТипОбъекта 	= Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.ФизическоеЛицо;
	КонецЦикла;
	...
	// Сотрудники.
	Отбор.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Сотрудник;
	...
	Для каждого СтрокаТЗ Из ТаблицаСотрудники Цикл
		Если СтрокаТЗ.Удалить Тогда
			...
			НоваяСтрока.ТипОбъекта 			= Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Сотрудник;
	...
	// Добавляем новые выгружаемые как изменения.
	ТекущиеВыгружаемые = ИнтеграцияУправлениеПерсоналом.ОбъектыВыгружаютсяВПриложение(ВыгружаемыеСотрудники, Приложение);
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника И ДоступенРасширенныйФункционал() Тогда
		...
		Отбор.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.ГрафикРаботы;
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника И ИнтеграцияУправлениеПерсоналом.Используется1СПерсонал() Тогда
	...
	Для каждого ТипОбъекта Из ТипыОбъектов Цикл
		...
		// Добавляем новые выгружаемые как изменения.
		ТекущиеВыгружаемые = ИнтеграцияУправлениеПерсоналом.ОбъектыВыгружаютсяВПриложение(ВыгружаемыеОбъектыПоТипу, Приложение);
	...
	ИсключаемыеТипы = ИнтеграцияУправлениеПерсоналом.ТипыОсновныхОбъектов();
...
Процедура ДополнитьВыгружаемыеОбъекты1СПерсонал(ИсходнаяТаблица)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Должность)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Подразделение)
...
Функция ОбъектыДляРегистрацииПриЗаписиПустыхПравил(Приложение, Сотрудники)
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ВыгружаемыеОбъекты.Добавить(), Выборка);
		НоваяСтрока.УсловноВыгружается 	= Истина;
	КонецЦикла;
...
Процедура ЗарегистрироватьИзменениеКадровойИстории(ТаблицаСотрудников, ПриложениеДляОбработки = Неопределено) Экспорт
	...
	Для каждого Приложение Из ИспользуемыеПриложения Цикл
		...
		Если Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
			ОбъектыДляРегистрацииУП = Интеграция1СПерсонал.ОбъектыДляРегистрацииОбмена(ДанныеДляРегистрации);
		Иначе
			ОбъектыДляРегистрацииКС = ИнтеграцияКабинетСотрудника.ОбъектыДляРегистрацииОбмена(ДанныеДляРегистрации);
		КонецЕсли;
...
// 			* Значение - Булево
//
Функция ВсеПозицииПравилВыгрузки(Приложения) Экспорт
...
// 			* Приложение - ПерчислениеСсылка.ПриложенияДляИнтеграции
...
Функция ДанныеДляРегистрацииОбъектов(ПараметрыПолученияДанных)
		...
		Иначе
			...
			Иначе
				...
				ИначеЕсли ФизическиеЛицаУсловноВыгружаемыхСотрудников[ФизическоеЛицо] <> Неопределено Тогда
					// текущие условно выгрудаемые сотрудники, добавим их позиции
					Для каждого СтрокаТЗ Из РабочиеМестаФизическогоЛица Цикл
						ПозицииУсловноВыгружаемыхСотрудников.Добавить(СтрокаТЗ.ДолжностьПоШтатномуРасписанию);
						Если ЗначениеЗаполнено(СтрокаТЗ.ГрафикРаботы) Тогда
							ГрафикиУсловноВыгружаемыхСотрудников.Добавить(СтрокаТЗ.ГрафикРаботы);
	...
	Для каждого Ссылка Из ГрафикиВыгружаемыхСотрудников Цикл
		...
		НоваяСтрока.Ссылка 		= Ссылка;
		НоваяСтрока.ТипОбъекта 	= Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.ГрафикРаботы;
	КонецЦикла;
...
Функция ОбъектыПозицийШтатногоРасписанияПоСсылкам(Приложение, Позиции)
	...
	Иначе
		ТипСсылки = Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.ОбъектПубликацииКабинетСотрудника.Тип)
	КонецЕсли;
	ОбъектыПозиций = Новый ТаблицаЗначений;
	ОбъектыПозиций.Колонки.Добавить("Ссылка", 		ТипСсылки);
	ОбъектыПозиций.Колонки.Добавить("ТипОбъекта", 	Новый ОписаниеТипов("ПеречислениеСсылка.ТипыОбъектовИнтеграцияУпралениеПерсоналом"));
	...
	Для каждого Позиция Из Позиции Цикл
		...
		НоваяСтрока.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.ДолжностьПоШтатномуРасписанию;
	КонецЦикла;
	...
	Для каждого Должность Из Должности Цикл
		...
		НоваяСтрока.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Должность;
	КонецЦикла;
	...
	Для каждого Организация Из Организации Цикл
		...
		НоваяСтрока.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Организация;
		Если ПубликоватьСтруктуруЮридическихЛиц Тогда
			НоваяСтрока = ОбъектыПозиций.Добавить();
			НоваяСтрока.Ссылка = Организация;
			НоваяСтрока.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Подразделение;
...
//
Функция ПодразделенияОрганизацииПозиций(Позиции) Экспорт
...
Функция ДанныеГрафиковРаботы(ГрафикиРаботыСотрудников, Приложение)  Экспорт
	...
	Возврат ИнтеграцияУправлениеПерсоналомВнутренний.ДанныеГрафиковРаботы(ГрафикиРаботыСотрудников, Приложение);
...
// 		Приложения - Массив, ПерчислениеСсылка.ПриложенияДляИнтеграции
//
// Возвращаемое значение:
// 		Соответствие
// 			* Ключ - СправочникСсылка.ФизическоеЛицо
// 			* Значение - Соответствие
// 				* Ключ - ПерчислениеСсылка.ПриложенияДляИнтеграции
// 				* Значение - Структура
//					* БлокирующиеОшибки - Массив - строки с представление блокирующх ошибок
//					* Ошибки - Массив - строки с представление ошибок
//					* ФизическоеЛицо - СправочникСсылка.ФизическоеЛицо, Неопределеное
...
Процедура СохранитьНовыеВидыКонтактнойИнформации(ВидКИМобильныйТелефон, ВидКИАдресЭлектроннойПочты, СообщениеОбОшибке) Экспорт
	...
	РегистрироватьУправлениеПерсоналом = Новый ТаблицаЗначений;
	РегистрироватьУправлениеПерсоналом.Колонки.Добавить("Ссылка", 		Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.СсылкаИзменение1СПерсонал.Тип));
	РегистрироватьУправлениеПерсоналом.Колонки.Добавить("ТипОбъекта", 	Новый ОписаниеТипов("ПеречислениеСсылка.ТипыОбъектовИнтеграцияУпралениеПерсоналом"));
	РегистрироватьУправлениеПерсоналом.Колонки.Добавить("ВерсияДанных", Новый ОписаниеТипов("Строка"));
	
	РегистрироватьКабинетСотрудника = Новый ТаблицаЗначений;
	РегистрироватьКабинетСотрудника.Колонки.Добавить("Ссылка", 		Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.СсылкаИзменениеКабинетСотрудника.Тип));
	РегистрироватьКабинетСотрудника.Колонки.Добавить("ТипОбъекта", 	Новый ОписаниеТипов("ПеречислениеСсылка.ТипыОбъектовИнтеграцияУпралениеПерсоналом"));
	РегистрироватьКабинетСотрудника.Колонки.Добавить("ВерсияДанных", Новый ОписаниеТипов("Строка"));
...
Процедура СохранитьНастройкуПубликуемойСтруктурыКомпании(ПубликоватьСтруктуруЮридическихЛиц, СообщениеОбОшибке) Экспорт
	...
	РегистрироватьУправлениеПерсоналом = Используется1СПерсонал();
	ВыгружаемыеОбъектыУП 	= Неопределено;
	ИзмененияДляОбменаУП 	= Неопределено;
	...
	РегистрироватьКабинетСотрудника = ИспользуетсяКабинетСотрудника();
	ВыгружаемыеОбъектыКС 	= Неопределено;
	ИзмененияДляОбменаКС 	= Неопределено;
	РолиИсполнителейЗадач 	= Неопределено;
...
Функция ДанныеДляРегистрацииПриИзмененииСтруктуруКомпании(Приложение)
	...
	Иначе
		// Будет установлена настройка ПубликоватьСтруктуруЮридическихЛиц.
		НовыеПодразделения = ИнтеграцияУправлениеПерсоналом.ПодразделенияОрганизацииПозиций(ПозицииШР);
	КонецЕсли;
...
Функция ЗарегистрироватьОбъектыКОбмену(Объекты, Приложение, ТипОбъекта) Экспорт
	...
	Если ТипОбъектаВыгружаетсяПоПравилам(ТипОбъекта) Тогда
		...
		Если ЗначениеЗаполнено(НевыгружаемыеОбъекты) Тогда
			ЗаголовокСообщения = НСтр("ru = 'Объекты не соответсвует правилам выгрузки.'");
			СообщениеСтроки = "";
...
Функция РассчитатьИзмененияОбъектовКОбмену(Приложение, ОбъектыКРегистрации, ТипОбъекта)
	...
	Для каждого Ссылка Из ОбъектыКРегистрации Цикл
		...
		Если ПубликоватьСтруктуруЮридическихЛиц И ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Организация Тогда
			НоваяСтрока = ИзмененияДляОбмена.Добавить();
			НоваяСтрока.Ссылка 			= Ссылка;
			НоваяСтрока.ТипОбъекта 		= Перечисления.ТипыОбъектовИнтеграцияУпралениеПерсоналом.Подразделение;
			НоваяСтрока.ВерсияДанных 	= Строка(Новый УникальныйИдентификатор);
...
Функция ОчиститьДанныеПриОтключенииПриложения(Приложение) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Приложение", Приложение); 
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СобытияОбменаУпралениеПерсоналом");
		ЭлементБлокировки.УстановитьЗначение("Приложение", Приложение);
...
Процедура ПроверитьРуководителейОрганизаций(БылиОшибки) Экспорт
	...
	Для каждого СтрокаТЗ Из ТекущиеРуководители Цикл
		ОтветственныеЛица = ПодписиДокументов.ОтветственныеЛицаОрганизации(СтрокаТЗ.Организация, "Руководитель", ТекущаяДатаСеанса());
		ОтветственныеЛица.Руководитель = ?(Не ЗначениеЗаполнено(ОтветственныеЛица.Руководитель), Справочники.ФизическиеЛица.ПустаяСсылка(),ОтветственныеЛица.Руководитель);
		ОтветственныеЛица.РуководительДолжность = ?(Не ЗначениеЗаполнено(ОтветственныеЛица.РуководительДолжность), Справочники.Должности.ПустаяСсылка(),ОтветственныеЛица.РуководительДолжность);
		Если СтрокаТЗ.Руководитель <> ОтветственныеЛица.Руководитель
			Или СтрокаТЗ.Должность <> ОтветственныеЛица.РуководительДолжность Тогда
			НоваяСтрока = ОбновлениеОрганизаций.Добавить();
			НоваяСтрока.Организация 	= СтрокаТЗ.Организация;
			НоваяСтрока.Руководитель 	= ОтветственныеЛица.Руководитель;
			НоваяСтрока.Должность 		= ОтветственныеЛица.РуководительДолжность;
		КонецЕсли;
		Если СтрокаТЗ.Руководитель <> ОтветственныеЛица.Руководитель Тогда
...
Функция ПодразделенияБезРуководителя(Организации) Экспорт
