...
Процедура ПубликуемаяНоменклатураТорговыхПредложений(Номенклатура) Экспорт
	РегистрыСведений.ТорговыеПредложенияТорговойПлощадки.ПубликуемаяНоменклатураТорговыхПредложений(Номенклатура);
КонецПроцедуры
...
// 
// Параметры:
//  ПараметрыЗапросаКатегорий - см. ОписаниеПараметровДереваКатегорий
...
Процедура ЗаполнитьПараметрыДереваКатегорийПоПараметрамПоискаПредложений(ПараметрыЗапросаКатегорий, ПараметрыПоиска)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыЗапросаКатегорий, ПараметрыПоиска, "ВНаличии, СоСкидкой, Организация");
...
// 
// Параметры:
//  ПараметрыЗапросаДерева - см. ОписаниеПараметровДереваКатегорий
//  Отказ - Булево
// 
// Возвращаемое значение:
...
//  Контрагенты              - Неопределено, Массив из ОпределяемыйТип.УчастникЭДО - выбранный поставщик.
...
Функция ПолучитьХарактеристикиКатегорииВФоне(
		Идентификатор, Контрагенты, УникальныйИдентификатор, ИдентификаторЗадания) Экспорт
	
	ИнтеграцияБСПБЭД.ОтменитьВыполнениеЗадания(ИдентификаторЗадания);
	ИдентификаторЗадания = "";
	
	// Заполнение параметров запроса.
	ПараметрыЗапроса = НовыйПараметрыЗапросаХарактеристикКатегорий();
	ПараметрыЗапроса.categoryId = Идентификатор;
	
	Если ЗначениеЗаполнено(Контрагенты) Тогда
		
		ИдентификаторыКонтрагентов = 
			БизнесСеть.ИдентификаторыКонтрагентов(Контрагенты).Данные.ВыгрузитьКолонку("Идентификатор");
		Если ИдентификаторыКонтрагентов.Количество() > 0 Тогда
			ПараметрыЗапроса.Вставить("suppliers", ИдентификаторыКонтрагентов);
		КонецЕсли;
...
//
Функция НовыйПараметрыЗапросаХарактеристикКатегорий()
...
Процедура СинхронизацияТорговыхПредложений(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Пока ВыборкаПрайсЛисты.Следующий() Цикл
		...
		ИначеЕсли ЧастичнаяСинхронизация Тогда
				...
				ПубликуемаяНоменклатураТорговыхПредложений(СписокНоменклатуры);
...
Функция ОбработатьОшибкуСервиса(Соединение, ЗапросСервиса, КодСостояния, Данные, Ошибка, Отказ)
	...
	ПодробныйТекстОшибки = СтрШаблон("%1%2%3 %4%2Код состояния: %5%6%7", 
	...
	Попытка
		...
		Для Каждого СтрокаТаблицаТоваров Из ТаблицаТоваров Цикл
			...
			ИначеЕсли ИнформацияОбОшибках.ЕстьОшибки
					И Не СтрокаТаблицаТоваров.ЕстьОшибки Тогда
				ИзменяемыеРеквизиты.Вставить("СостояниеПубликацииТорговыхПредложений",
					Перечисления.СостоянияПубликацииПозицииТорговогоПредложения.Ошибка);
