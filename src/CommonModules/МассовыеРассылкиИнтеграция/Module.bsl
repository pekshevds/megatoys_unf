...
// Обновляет промокоды переданным контактам.
//
// Параметры:
//  ДанныеАдреснойКниги - Структура - См. НовыйДанныеАдреснойКнигиСервиса().
// 
...
//  Булево - Истина, если выполнено успешно.
//
Функция ОбновитьПромокоды(ДанныеАдреснойКниги) Экспорт
	Ошибки = Неопределено;
	ЕстьОшибки = Ложь;
	
	НастройкаИнтеграции = НастройкаИнтеграции();
	МодульИнтеграции().ОбновитьПромокодыПолучателей(НастройкаИнтеграции, ДанныеАдреснойКниги, Ошибки);
...
Функция НовыйТаблицаПолучателей() Экспорт
	...
	ДанныеПолучателя.Колонки.Добавить("Промокод", НовыйТипСтрока());
...
Функция НовыйДанныеРассылки() Экспорт
	...
	ДанныеРассылки.Вставить("ПредлагаемаяСкидка", Неопределено);
	...
	ДанныеРассылки.Получатели.Колонки.Добавить("Промокод", НовыйТипСтрока());
...
Процедура ДобавитьПромокодВДанныеПолучателя(ДанныеПолучателя, СтрокаРассылки) Экспорт
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаРассылки, "Промокод") Тогда
		ДанныеПолучателя.Вставить("smallbiz_1c_promocode", СтрокаРассылки.Промокод);
	КонецЕсли;
КонецПроцедуры
#КонецОбласти
