...
Процедура Обслуживание_ПриСозданииНаСервере(Форма) Экспорт
	...
	// ИнтернетПоддержкаПользователей.ОблачныйАрхив20
	СписокПодсистем.Добавить(
...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ПараметрыОблачногоАрхива = Новый Структура();
	ПараметрыОблачногоАрхива.Вставить("ОблачныйАрхивПодключен", Истина);
	...
	Если ПараметрыАрхивирования.РаботаРезервногоКопированияЗаблокирована = Неопределено
		...
		ПараметрыОблачногоАрхива.Вставить("ПоказатьФормуВосстановленияРаботыОблачногоАрхива");
		
	КонецЕсли;
	
	// Запуск автоматического резервного копирования
	ДлительнаяОперация = ОбновитьВремяДоАвтоматическогоРезервногоКопированияВФоне(ПараметрыАрхивирования);
	ПараметрыОблачногоАрхива.Вставить("АвтоматическоеАрхивирование", ДлительнаяОперация);
	...
	Если Не ПустаяСтрока(НастройкиКопирования.ЗаданиеID) Тогда
		...
		Если НастройкиКопирования.Пользователь = ИмяПользователя Тогда
			Если ПравоДоступа("Просмотр", Метаданные.Обработки.ОблачныйАрхив20) Тогда
				...
				ПараметрыОблачногоАрхива.Вставить(
					"ПоказатьФормуСтатусаРучногоРезервногоКопирования",
					НастройкиКопирования);
			Иначе
	...
	Если Не ПараметрыОблачногоАрхива.Свойство("ПоказатьФормуСтатусаРучногоРезервногоКопирования") Тогда
...
Процедура СброситьНеобязательныеПоляСервиса(Данные, НеобязательныеПоля)
	...
	Для Каждого ТекПоле Из СписокПолей Цикл
		...
		Если Данные.Свойство(ТекПоле)
			И Не ЗначениеЗаполнено(Данные[ТекПоле]) Тогда
