...
Процедура СоздатьМЧД(ОповещениеОЗавершении = Неопределено) Экспорт
	ОткрытьФорму("Справочник.МЧД003.Форма.СозданиеДоверенности", , , , , , ОповещениеОЗавершении);
КонецПроцедуры
...
Процедура ОбработатьРезультатРучногоПодтвержденияПодписания(Результаты, ДополнительныеПараметры) Экспорт
	...
	ЭлектронныеДокументыЭДОКлиент.НачатьВыполнениеДействийПоЭДО(
		ДополнительныеПараметры.ОповещениеУспешногоЗавершения, ПараметрыВыполненияДействийПоЭДО); 
...
Процедура ОткрытьСписокМЧД(Организация = Неопределено) Экспорт
	...
	Оповестить("ПоказатьДоверенностиОрганизации", Организация);
...
Процедура ПодписатьЗарегистрироватьМЧД(ЗарегистрироватьВРеестреФНС,
	...
	ПараметрыВыполненияДействийПоЭДО.ДополнительныеДанныеОбъектов.Вставить(
		ДополнительныеПараметры.СсылкаНаДоверенность, ДополнительныеДанные);
...
//
// Параметры:
//   РезультатПодписания - Неопределено - будет прислано в случае прерывания подписания пользователем на форме ожидания,
//    						Структура - при завершении со следующими параметрами:
