...
Функция ДляТипаДокументаЕстьФорматНоНетРегламента(ТипДокумента) Экспорт
	...
	Если 	ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент
...
Функция ДляВидаДокументаЕстьФормализованныйФорматФНС(Знач ВидДокумента) Экспорт
	...
	// Форматы формализованные привязаны к АктСверкиВзаиморасчетов.
	Если ТипДокументаДляПоиска = Перечисления.ТипыДокументовЭДО.АктСверки Тогда
		ТипДокументаДляПоиска = Перечисления.ТипыДокументовЭДО.АктСверкиВзаиморасчетов;
		ПараметрыПоиска = НовыеПараметрыПоискаВидаДокумента(ТипДокументаДляПоиска);
		ВидДокументаДляПоиска = НайтиВидДокумента(ПараметрыПоиска);
	КонецЕсли;
...
Процедура ДополнитьСтрокуHTMLШтампамиПодписей(СтрокаHTML, Сообщение) Экспорт
...
Функция ПредставлениеДанныхСообщения(ВидДокумента, ДанныеФайла, ДанныеФайлаОтвета = Неопределено, ПараметрыВизуализации = Неопределено, КонтекстДиагностики = Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеВида.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент
...
// Возвращаемое значение:
//  Структура:
//   * ПредставлениеДокумента - ТабличныйДокумент, Строка, Неопределено - Представление данных
...
Функция ПредставлениеДанныхСообщенияПоСсылке(Сообщение, СообщениеОтвета = Неопределено, СообщениеЗапроса = Неопределено, 
	...
	ИначеЕсли ОписаниеВида.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент
		...
		Возврат Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПредставлениеДанныхДокумента(ДвоичныеДанныеФайла);
...
// 
// Возвращаемое значение:
//  Массив из см. ИнтеграцияЭДОКлиентСервер.НовыеДанныеЭлектронногоДокументаДляОтраженияВУчете
Функция ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы) Экспорт
	...
	Запрос.Текст =
		...
		|	СообщениеЭДО.Направление КАК Направление,
		|	ЭлектронныйДокументВходящийЭДО.Представление как ПредставлениеДокумента
		|ИЗ
		...
		|	ЭлектронныйДокументИсходящийЭДО.Представление как ПредставлениеДокумента
		|ИЗ
...
Процедура ПодменитьФайлНаФайлСВизуализацией(
	...
	ШтампЭП = ДанныеШтампаЭППоДаннымСообщенияЭДО(ДанныеФайла.Владелец);
	АдресСШтампом = РаботаСФайламиБЭД.ДобавитьШтампЭПВФайлPDF(ДвоичныеДанныеФайлаPDF, ШтампЭП, УникальныйИдентификатор);
		
	Если АдресСШтампом = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
// 	КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// Возвращаемое значение:
// 	См. ЭлектронныеДокументыЭДО.НовыйРезультатДействийПоЭДО
...
Функция СоздатьДокументПоФайлу(ПараметрыСоздания, ОписаниеФайла) Экспорт
	...
	МаршрутПодписания = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыСоздания, "МаршрутПодписания");
	Если НастройкиОтправки.МаршрутПодписания = МаршрутыПодписанияБЭД.МаршрутУказыватьПриСоздании()
		И Не ЗначениеЗаполнено(МаршрутПодписания)
...
//  * ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
Функция ВыборкаДокументовДляФормированияОтвета(ОбъектыДействий) Экспорт  // АПК:581 - экспортная, так как используется в сервисе.
...
Процедура СформироватьОтветПоДокументу(ЭлектронныйДокумент, РезультатДействий, ДанныеОбъектаУчета = Неопределено) Экспорт  // АПК:581 - экспортная, так как используется в сервисе.
...
//                                  - См. ЭлектронныеДокументыЭДОКлиентСервер.ДанныеРучногоФормированияТитула
Процедура СформироватьОтветПоДокументамПакета(ПакетДокументов, РезультатДействий, ДанныеОбъектовУчетаПоДокументам = Неопределено) Экспорт  // АПК:581 - экспортная, так как используется в сервисе.
...
Функция ОпределитьДоверителя(ДополнительныйКонтекстПроверки, СвойстваПодписи, ВсегоПодписей) Экспорт
...
Функция ИдентификаторыУчастниковЭДО(ЭлектронныйДокумент) Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Результат.ИдентификаторПолучателя = ВыборкаДетальныеЗаписи.ИдентификаторКонтрагента;
	КонецЕсли;
...
Функция ДвоичныеДанныеСообщения(Сообщение)
	...
	ДвоичныеДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(Выборка.ОсновнойФайл, Ложь);
...
// Это подпись получателя.
//
// Параметры:
//  НаправлениеСообщения - см. Перечисления.НаправленияЭДО - Направление сообщение
//  ЭтоИсходящийДокумент - Булево - Признак исходящего документа
//
// Возвращаемое значение:
//  Булево
Функция ЭтоПодписьПолучателя(НаправлениеСообщения, ЭтоИсходящийДокумент)
	
	Возврат (НаправлениеСообщения = Перечисления.НаправленияЭДО.Входящий И ЭтоИсходящийДокумент = Истина)
		Или (НаправлениеСообщения = Перечисления.НаправленияЭДО.Исходящий И ЭтоИсходящийДокумент = Ложь);
	
КонецФункции

// Это подпись отправителя.
//
// Параметры:
//  НаправлениеСообщения - см. Перечисления.НаправленияЭДО - Направление сообщение
//  ЭтоИсходящийДокумент - Булево - Признак исходящего документа
...
// Возвращаемое значение:
//  Булево
Функция ЭтоПодписьОтправителя(НаправлениеСообщения, ЭтоИсходящийДокумент)
	
	Возврат (НаправлениеСообщения = Перечисления.НаправленияЭДО.Входящий И ЭтоИсходящийДокумент = Ложь)
		Или (НаправлениеСообщения = Перечисления.НаправленияЭДО.Исходящий И ЭтоИсходящийДокумент = Истина);
	
КонецФункции

// Получить ключ владельца подписи
//
// Параметры:
//  НаправлениеСообщения - см. Перечисления.НаправленияЭДО - Направление сообщение
//  ЭтоИсходящийДокумент - Булево - Признак исходящего документа
//  Владелец - Строка - Имя владельца подписи
//
// Возвращаемое значение:
//  Строка
Функция ПолучитьКлючВладельцаПодписи(НаправлениеСообщения, ЭтоИсходящийДокумент, Владелец)
	
	Если ЭтоПодписьПолучателя(НаправлениеСообщения, ЭтоИсходящийДокумент) Тогда
		Префикс = "2e1715b4-1f8f-485c-b92b-fbada03795af_";
	ИначеЕсли ЭтоПодписьОтправителя(НаправлениеСообщения, ЭтоИсходящийДокумент) Тогда
		Префикс = "169e4d90-4756-40cf-9e07-dc86e360e789_";
	Иначе
		Префикс = "2b91a5d9-037c-4d8b-83b5-37e3df1781e4_";
	КонецЕсли;
	
	Возврат Префикс + Владелец;
...
Функция ПодписиПоНаправлению(СообщениеСхемы, ЭтоИсходящийДокумент, ПодписиПолучателя, 
	...
	Если ЭтоПодписьПолучателя(СообщениеСхемы.Направление, ЭтоИсходящийДокумент) Тогда
		Возврат ПодписиПолучателя;
	ИначеЕсли ЭтоПодписьОтправителя(СообщениеСхемы.Направление, ЭтоИсходящийДокумент) Тогда
		...
		Иначе
			...
			Для Каждого СообщениеИОП Из СообщенияИОП Цикл
				...
				Для Каждого Подпись Из ПодписиСообщения Цикл
					КлючВладельцаПодписи = ПолучитьКлючВладельцаПодписи(СообщениеИОП.Направление,
						ДанныеДляФормированияШтампа.ЭтоИсходящийДокумент, Подпись.СвойстваПодписи.Владелец);
						
					ПодписиПоВладельцу.Вставить(КлючВладельцаПодписи, ПодписиПоНаправлению);
				КонецЦикла;
			КонецЦикла;
			...
			Если ЗначениеЗаполнено(ДанныеОтправителя) Тогда
				...
				Для Каждого Подпись Из ПодписиСообщения Цикл
					КлючВладельцаПодписи = ПолучитьКлючВладельцаПодписи(ДанныеОтправителя.Направление,
						ДанныеДляФормированияШтампа.ЭтоИсходящийДокумент, Подпись.СвойстваПодписи.Владелец);
						
					ПодписиПоНаправлению = ПодписиПоВладельцу[КлючВладельцаПодписи];
			...
			Если ЗначениеЗаполнено(ДанныеПолучателя) Тогда
				...
				Для Каждого Подпись Из ПодписиСообщения Цикл
					КлючВладельцаПодписи = ПолучитьКлючВладельцаПодписи(ДанныеПолучателя.Направление,
						ДанныеДляФормированияШтампа.ЭтоИсходящийДокумент, Подпись.СвойстваПодписи.Владелец);
...
Функция ПроверитьПодписьФайлаПоМЧД(ФайлСообщения, СвойстваПодписи, ВсегоПодписей)
	...
	ДополнительныйКонтекст = СведенияОПодписанномОбъекте(ФайлСообщения);
...
Функция ПроверитьПодписьПоМЧД(ФайлСообщения, НаправлениеСообщения, ОписаниеФайла, СвойстваПодписи,
	...
	Если Не ЗначениеЗаполнено(НомераМЧД) Тогда
		...
		Если ЗначениеЗаполнено(СтарыйРезультатПроверки) И ЗначениеЗаполнено(СтарыйРезультатПроверки.Доверенность) Тогда
			...
			Иначе
				ЭтоМЧДКонтрагента = 
					МашиночитаемыеДоверенности.ЭтоМЧДКонтрагента(СтарыйРезультатПроверки.Доверенность);
	...
	Для Каждого НомерМЧД Из НомераМЧД Цикл
		ИННДоверителяИзДопПолей = СоответствиеИННДоверителяСНомерамиМЧД.Получить(НомерМЧД);
		Если ИННДоверителяИзДопПолей = Неопределено
			Или ИННДоверителяИзДопПолей = Лев(ИННДоверителя, 10) // Есть случаи неполного ИНН для доверителей ИП
			Тогда
			СоответствиеИННДоверителяСНомерамиМЧД.Вставить(НомерМЧД, ИННДоверителя);
		КонецЕсли;
	КонецЦикла;
...
Функция СвойстваДоверенностейИзРезультатовПроверокПодписей(РезультатыПроверок)
	...
	// При отсутствии доверенности, проверки будут осуществляться по пустой ссылке из структуры, определяя её наличие
...
Функция ЗапросСообщенийИПакетовДляОбработкиДействия(ОбъектыДействий, Отбор)
	...
	ТекстыЗапросов = Новый Массив;
	ИмяВТ = "ВТ_СообщенияДляОбработки";
	
	ОписаниеЗапросаСообщений = ЗапросСообщенийДляОбработкиДействия(ОбъектыДействий, Отбор, ИмяВТ,
		"Ссылка, ЭлектронныйДокумент, ТипЭлементаРегламента");
		
	ТекстыЗапросов.Добавить(ОписаниеЗапросаСообщений.Текст);
	ТекстЗапросаПакетов = ТекстЗапросаИдентификаторовПакетовПоСообщениямЭДО(ИмяВТ);
...
Функция ТекстЗапросаИдентификаторовПакетовПоСообщениямЭДО(ИмяВременнойТаблицыСообщений)
	...
	|ИЗ
	|	&ИмяВременнойТаблицы КАК СообщениеЭДО
...
// Параметры:
//  ПараметрыФормирования - см. НовыеПараметрыФормированияДокумента
...
Процедура ПроизвестиДействияСДополнительнымиФайлами(ПараметрыФормирования, ДополнительныйФайл)
	...
	Если ОписаниеСообщения.Направление <> ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Внутренний")
		...
		Если Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ЭтоФайлФормата(
			ОписаниеСообщения.Данные.Документ.ИмяФайла) Тогда
			
			ОписаниеСообщения.Данные.Документ.ИмяФайла       = ДополнительныйФайл.ИмяФайла;
			ОписаниеСообщения.Данные.Документ.ДвоичныеДанные = ДополнительныйФайл.ДвоичныеДанные;
			
			Содержание = ФорматыЭДО.ПрочитатьСодержаниеДокумента(ОписаниеСообщения.Данные.Документ);
...
Функция ТекстЗапросаНастроекОтправкиОбъектовУчета()
	...
	Иначе
		...
		|	ОписанияОбъектовУчета.Организация КАК Отправитель,
		|	ОписанияОбъектовУчета.Контрагент КАК Получатель,
		|	ЕСТЬNULL(НастройкиОтправкиСДоговором.Договор, &ПустойДоговор) КАК Договор,
		...
		|		И НастройкиОтправкиБезДоговора.Договор = &ПустойДоговор
...
//  * ОшибкиФормирования - Массив из см. НовоеОписаниеОшибкиФормирования
Функция СформироватьОтвет(ЭлектронныйДокумент, КонтекстДиагностики, ОписаниеДанныхЭД = Неопределено)
...
Функция ЗапросДанныхМЧДДляПодписания(ПараметрыВыполнения, КонтекстДиагностики)
	...
	ЭтоМЧД003 = ТипЗнч(ПараметрыВыполнения.ОбъектыДействий.МЧД[0]) = Тип("СправочникСсылка.МЧД003");
	
	Если Не ЭтоМЧД003 Тогда
		ОписанияЗапросов.Добавить(ЗапросМЧДДляПодписания("СообщенияДляОбработки", ОбъектыДействий));
	Иначе
		ЗапросДляПодписанияМЧД003 =
			Справочники.МЧД003.ЗапросДляПодписания("СообщенияДляОбработки", ОбъектыДействий);
	...
	|	СообщенияДляОбработки.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
	...
	ТекстПоляОрганизацииИНН = СтрШаблон("СообщенияДляОбработки.Организация.%1", ИмяРеквизитаИННОрганизации);
...
Функция ОбработатьРезультатыЗапросаДанныхМЧДДляПодписания(ПараметрыВыполнения, РезультатыЗапроса, КонтекстДиагностики)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.СтатусВРеестреФНС) Тогда
			ДвоичныеДанные = МашиночитаемыеДоверенности.ПолучитьДвоичныеДанныеМЧД(Выборка.Ссылка);
		Иначе
			ДвоичныеДанные = МашиночитаемыеДоверенности.ПолучитьДвоичныеДанныеЗаявленияНаОтзыв(Выборка.Ссылка);
		КонецЕсли;
...
//     ** Значение - Строка
//
Функция СоздатьНаборСертификатовДляПодписанияМЧД(ИННОрганизации,
	...
	Для Каждого СтрокаТаблицы Из СертификатыДляПодписания Цикл
		...
		ЭтоСертификатДоверителя = МашиночитаемыеДоверенности.ЭтоСертификатДоверителя(
...
Функция НайтиСоздатьНаборСертификатовДляПодписания(Выборка, НаборыСертификатов, СертификатыДляПодписания,
	...
	ОбезличенныеСертификатыНайдены = Ложь;
	...
	Если ИспользуютсяОбезличенныеСертификаты
		...
		Если СтрокиОбезличенногоСертификата.Количество() > 0 Тогда
			...
			ОбезличенныеСертификатыНайдены = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	Если Не ОбезличенныеСертификатыНайдены Тогда
		СтрокиСертификата = НайденныеСтроки.НайтиСтроки(Новый Структура("ОбезличенныйСертификат", Ложь));
		НайденныеСтроки = НайденныеСтроки.Скопировать(СтрокиСертификата);
	КонецЕсли;
...
Функция ПараметрыПодписанияМЧД(Выборка)
	...
	ИмяФайла = МашиночитаемыеДоверенности.ПолучитьИмяФайлаМЧД(Выборка.Ссылка);
	...
	Параметры.Вставить("ОрганизацияИНН", Выборка.ИННОрганизации);
	Параметры.Вставить("ЭтоИнтеркампани", Ложь);
	Возврат Параметры;
КонецФункции
...
//  ** Сертификат - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//  ** Сообщение - ДокументСсылка.СообщениеЭДО
// * СообщенияВФорматеНеПоддержанномОператором - ТаблицаЗначений:
//  ** Сообщение - ДокументСсылка.СообщениеЭДО
//  ** Формат - Строка
...
Процедура ДобавитьОшибкуДокументыНедоступныДляПодписанияНаСервереПоДоверенности(КонтекстДиагностики, 
	...
	СертификатыИСообщения.Свернуть("Сертификат");
	...
	Пока ВыборкаСертификатов.Следующий() Цикл
		...
		ДополнительныеДанные = Новый Структура;
		ДополнительныеДанные.Вставить("Сертификат", ВыборкаСертификатов.Сертификат);
		ПодробноеПредставление = СтрСоединить(МассивСтрок, Символы.ПС);
		
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, ВидОшибки,
			ПодробноеПредставление, ПодробноеПредставление, ДополнительныеДанные);
...
Функция ВидОшибкиДокументыНедоступныДляПодписанияНаСервереПоДоверенности(Сертификаты)
	...
	ВидОшибки.ОписаниеРешения = ТекстОшибкиСертификатомМожноПодписатьТолькоПоМЧД(Сертификаты);
...
Функция ЗаполнитьПодписантовИДоверенностиПоСертификату(ДанныеДляЗаполнения,
	...
	Для Каждого Элемент Из ДанныеДляЗаполнения Цикл
		...
		ТипКлюча = ТипЗнч(Элемент.Ключ);
		
		Если ТипКлюча = Тип("СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций")
			Или ТипКлюча = Тип("СправочникСсылка.МЧД003") Тогда
			
			Если ТипКлюча = Тип("СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций") Тогда
				ИмяФайла = Справочники.МашиночитаемыеДоверенностиОрганизаций.ПолучитьИмяФайлаМЧД(Элемент.Ключ);
			ИначеЕсли ТипКлюча = Тип("СправочникСсылка.МЧД003") Тогда
				ИмяФайла = Справочники.МЧД003.ПолучитьИмяФайлаМЧД(Элемент.Ключ);
			КонецЕсли;
...
Функция ОбновитьДанныеПриПодписании(ПодписанноеСообщение, РезультатыПроверокПодписей, РезультатДействий)
	...
	Попытка
		...
		КонтекстВыполненияДействия = СоздатьКонтекстВыполненияДействияПоСообщению(ПодписанноеСообщение.Ссылка,
			Действие);
...
Процедура ЗагрузитьДанныеОбъектовПакета(ДанныеОбъектовПакета, ПроверенныеПодписи, КонтекстДиагностики, РезультатЗагрузки)
	...
	Попытка
		...
		КонтекстВыполненияДействия = СоздатьКонтекстВыполненияДействияПоПакету(ДанныеОбъектовПакета.Ключ,
			Действие);
