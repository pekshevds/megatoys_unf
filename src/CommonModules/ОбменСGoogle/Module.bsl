...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	НастройкаСинхронизацияGoogleCalendar.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьОбменСGoogleCalendar;
	
КонецПроцедуры

// Подписка для записи объекта владельца записи календаря.
//
// Параметры:
//  Объект - ДокументОбъект.ЗадачаСотрудника, СправочникОбъект.ЗаписиКалендаряСотрудника
//
...
Процедура ПриЗаписиИсточникаЗаписиКалендаря(Объект) Экспорт
	...
	ДанныеОбмена = ДанныеЗаписиКалендаряGoogle(Объект);
	ЗаполнитьПараметрыОбменGoogleВЗаписи(Объект, ДанныеОбмена);
	ЗаполнитьКлючевыеПоляЗаписиКалендаряGoogle(Объект, ДанныеОбмена);
	ЗарегистрироватьИзменениеВПланеОбмена(Объект, ДанныеОбмена);
	ДанныеОбмена.Записать();
	
КонецПроцедуры

// Возвращает соответствие номера картинки календаря Gooogle и её цвета
// 
// Возвращаемое значение:
//  Соответствие:
//  Число - Цвет
//
Функция ЦветаПоНомеруКартинки() Экспорт
...
Функция ЭтоУчетнаяЗаписьGoogle(УчетнаяЗаписьЭлектроннойПочты) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СеансовыеДанныеGoogle.УчетнаяЗаписьЭлектроннойПочты КАК УчетнаяЗаписьЭлектроннойПочты
	|ИЗ
	...
	Запрос.УстановитьПараметр("ОбластьДоступа", Перечисления.ОбластиДоступаGoogle.Почта);
...
Процедура ЗаданиеСинхронизироватьКалендарь() Экспорт
	...
	СинхронизироватьВсеGoogleКалендари();
	ОбменСВнешнимиКалендарями.СинхронизироватьDAVКалендари();
...
Процедура СинхронизироватьВсеGoogleКалендари()
...
Процедура ЗагрузитьДанныеGoogleCalendar(СеансовыеДанные, Пользователь)
	...
	Для Каждого ТекОписаниеКалендаря Из ОписанияВсехКалендарей["items"] Цикл
		...
		Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен= "" Цикл
			...
			КалендарьСотрудника = СсылкаНаКалендарьСотрудника(
				ТекОписаниеКалендаря, Пользователь, Счетчик);
			Счетчик = Счетчик + 100;
...
Процедура СоздатьОтсутствующиеКалендариВGoogle(СеансовыеДанные, Пользователь)
	...
	Пока Выборка.Следующий() Цикл
		...
		УстановитьЦветКалендаряВGoogle(
			Выборка.КалендарьСотрудника, Пользователь, СеансовыеДанные, ЦветаПоНомеруКартинки, Счетчик);
...
Процедура УстановитьЦветКалендаряВGoogle(
	КалендарьСотрудника,
	Пользователь,
	СеансовыеДанные,
	ЦветаПоНомеруКартинки,
	Счетчик)
	...
	colorId = Неопределено;
	Для Каждого КлючИЗначение Из ЦветаПоНомеруКартинки Цикл
		Если КлючИЗначение.Значение = НайденнаяНастройка.ЦветКалендаря Тогда
			colorId = КлючИЗначение.Ключ;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если colorId = Неопределено Тогда
		colorId = СлучайноеЧисло(Счетчик, 1, 24);
	КонецЕсли;
	
	ПараметрыКалендаря["colorId"] = Строка(colorId);
...
Процедура ВыгрузитьДанныеGoogleCalendar(СеансовыеДанные, Пользователь)
	...
	|	И ЗаписиКалендаряСотрудникаИзменения.Узел <> &ЭтотУзел
	|	И ЗаписиКалендаряСотрудникаИзменения.Узел ССЫЛКА ПланОбмена.ОбменСGoogleCalendar
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ЗадачаСотрудникаИзменения.Узел ССЫЛКА ПланОбмена.ОбменСGoogleCalendar
	|ИТОГИ ПО
	|	Узел");
...
Функция ТелоПакетногоЗапросаИзменениеДанныхКалендаря(ВыборкаДанных, ГраницаСекций)
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		ОписаниеЗаписи = ОписаниеЗаписиКалендаря(Данные.Ссылка);
		
		Если Не ЗначениеЗаполнено(ОписаниеЗаписи.Начало) Или Не ЗначениеЗаполнено(ОписаниеЗаписи.Окончание) Тогда
			Продолжить;
		КонецЕсли;
		
		ДанныеДляИзменения = Новый Соответствие;
		
		ИдентификаторУНФ = ДанныеОбмена.Идентификатор;
		...
		ДанныеДляИзменения["id"] = ИдентификаторУНФ;
		...
		Если ИмяМетодаHTTP(Данные, ДанныеОбмена) = "DELETE" Тогда
			...
			ИмяМетодаHTTP(Данные, ДанныеОбмена),		// 4
...
//   * ЦветКалендаря - Цвет - цвет, принятый от Google,
...
Процедура ЗаписатьНастройкиКалендаря(КалендарьСотрудника, ДанныеЗаполнения)
	...
	НоваяНастройка.ЦветКалендаря = ДанныеЗаполнения.ЦветКалендаря;
...
Функция НастройкиДоступныхКалендарей(ПользовательИБ)
	...
	ТаблицаНастроекПоУмолчанию.Колонки.Добавить("ЦветКалендаря", Новый ОписаниеТипов("Цвет"));
...
//  Счетчик - Число - нужен для генерации случайного числа для конкретного календаря
// 
// Возвращаемое значение:
...
//
Функция СсылкаНаКалендарьСотрудника(ОписаниеКалендаря, Пользователь, Счетчик)
	...
	Иначе
		...
		СотрудникиПользователя = РегистрыСведений.СотрудникиПользователя.ПолучитьСотрудниковПользователя(Пользователь);
	...
	ЦветКалендаря = ОпределитьЦветПоДаннымЗаполнения(ДанныеЗаполнения, Счетчик);
	ДанныеЗаполнения.Вставить("ЦветКалендаря", ЦветКалендаря);
...
Функция ОпределитьЦветПоДаннымЗаполнения(ДанныеЗаполнения, Счетчик)
	
	colorId = Неопределено;
	ДанныеЗаполнения.Свойство("colorId", colorId);
	Если colorId = Неопределено Тогда
		colorId = СлучайноеЧисло(Счетчик, 1, 24);
	Иначе
		ОписаниеТипа = Новый ОписаниеТипов("Число");
		colorId = ОписаниеТипа.ПривестиЗначение(colorId);
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ЦветаПоНомеруКартинки Цикл
		Если colorId = КлючИЗначение.Ключ Тогда
			ПринятыйЦвет = КлючИЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Если ПринятыйЦвет = Неопределено Тогда
		ПринятыйЦвет = ЦветаПоНомеруКартинки[1];
	КонецЕсли;
	Возврат ПринятыйЦвет;
...
Процедура ЗаполнитьКлючевыеПоляЗаписиКалендаряGoogle(Объект, ДанныеОбмена)
...
Процедура ЗарегистрироватьИзменениеВПланеОбмена(Объект, ДанныеОбмена)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Календарь, "СинхронизироватьСGoogle") <> Истина Тогда
		Возврат;
	КонецЕсли;
	
	УзелДляКалендаряGoogle = ПланыОбмена.ОбменСGoogleCalendar.УзелДляКалендаряGoogle(Объект.Календарь);
	
	Объект.ОбменДанными.Получатели.Добавить(УзелДляКалендаряGoogle);
	
	Если ДанныеОбмена <> Неопределено И Не ЗначениеЗаполнено(ДанныеОбмена.ETag) Тогда
		УдалитьДублированиеРегистрацииИзменений(Объект.Ссылка, УзелДляКалендаряGoogle);
	КонецЕсли;
	
КонецПроцедуры

// Удаляет регистрацию изменений в случае, когда у объекта был изменен календарь перед выгрузкой
Процедура УдалитьДублированиеРегистрацииИзменений(Ссылка, УзелДляКалендаряGoogle)
...
Функция ЗаписьКалендаряПоИдентификатору(Идентификатор, Календарь)
	...
	Запрос.УстановитьПараметр("Ключ", КлючИзИдентификатора(Идентификатор, ТипЗнч(Справочники.ЗаписиКалендаряСотрудника)));
	Запрос.УстановитьПараметр("Календарь", Календарь);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		Результат = Документы.ЗадачаСотрудника.СоздатьДокумент();
...
Функция ОписаниеЗаписиКалендаря(Данные)
	
	СекундВСутках = 86400;
	Результат = НовыйОписаниеЗаписиКалендаря();
	
	Если ТипЗнч(Данные) = Тип("СправочникСсылка.ЗаписиКалендаряСотрудника") Тогда
		Результат.Наименование = Данные.Наименование;
		Результат.Описание = Данные.Описание;
		Результат.Начало = Данные.Начало;
		Результат.Окончание = Данные.Окончание;
	Иначе
		Результат.Наименование = Данные.Наименование;
		Результат.Описание = Данные.Описание;
		Результат.Начало = Данные.ДатаНачала;
		Результат.Окончание = Данные.ДатаОкончания;
		
		Если Не ЗначениеЗаполнено(Результат.Окончание) Тогда
			Результат.Окончание = Результат.Начало + СекундВСутках;
		КонецЕсли;
	КонецЕсли;
...
Функция СлучайноеЧисло(ДобавлениеКГенератору, От, До)
	
	ГСЧ = Новый ГенераторСлучайныхЧисел(ТекущаяДата() - Дата(1, 1, 1) + ДобавлениеКГенератору);
