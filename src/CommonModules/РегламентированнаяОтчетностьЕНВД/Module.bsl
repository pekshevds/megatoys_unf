...
Процедура ЗаполнитьДокументПоказателейЕНВДНаОснованииПредыдущегоПериода(Организация, ДатаПериода, ПоказателиБудутЗаполненыИзПредыдущегоПериода) Экспорт
	ОбъектТекущийДокументПоказателей = ПолучитьДокументПоказателейЕНВД(Организация, ДатаПериода).ПолучитьОбъект();
	
	// Проверяем заполненность показателей
	ПоказателиБудутЗаполненыИзПредыдущегоПериода = Ложь;
	Для Каждого Строка Из ОбъектТекущийДокументПоказателей.ПоказателиЕНВД Цикл
		Если Строка.ЗначениеФизическогоПоказателя1 = 0 И Строка.ВыработкаДней1 > 0 Тогда
			ПоказателиБудутЗаполненыИзПредыдущегоПериода = Истина;
			Прервать;
		КонецЕсли;
	...
	// Если показатели заполнены, ничего не меняем
	Если НЕ ПоказателиБудутЗаполненыИзПредыдущегоПериода Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектТекущийДокументПоказателей.ЗаполнитьДанныеДокумента();
	
	ДокументЗаПредыдущийПериод = ПолучитьДокументПоказателейЕНВД(Организация, КонецМесяца(ДобавитьМесяц(ДатаПериода,-3)));
	
	Для Каждого СтрокаПоказатель Из ОбъектТекущийДокументПоказателей.ПоказателиЕНВД Цикл
		СтрокиНайденногоДокумента = ДокументЗаПредыдущийПериод.ПоказателиЕНВД.НайтиСтроки(Новый Структура("ВидДеятельностиЕНВД",СтрокаПоказатель.ВидДеятельностиЕНВД));
		Если СтрокиНайденногоДокумента.Количество() > 0 Тогда
			СтрокаПредыдущегоПериода = СтрокиНайденногоДокумента[0];
			СтрокаПоказатель.ЗначениеФизическогоПоказателя1 = СтрокаПредыдущегоПериода.ЗначениеФизическогоПоказателя1;
			СтрокаПоказатель.ЗначениеФизическогоПоказателя2 = СтрокаПредыдущегоПериода.ЗначениеФизическогоПоказателя2;
			СтрокаПоказатель.ЗначениеФизическогоПоказателя3 = СтрокаПредыдущегоПериода.ЗначениеФизическогоПоказателя3;
		КонецЕсли;
	КонецЦикла;
	ОбъектТекущийДокументПоказателей.Записать();
	...
	Если СтруктураДекларацииПоЕНВД.ДокументОтчетности <> Неопределено Тогда
		СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если Год(СтруктураДекларацииПоЕНВД.ДатаДокументаОбработкиСобытия)> 2013 И СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2012Кв1"
				...
				ИЛИ Год(СтруктураДекларацииПоЕНВД.ДатаДокументаОбработкиСобытия)> 2016 И СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
				ИЛИ Год(СтруктураДекларацииПоЕНВД.ДатаДокументаОбработкиСобытия)> 2017 И СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2017Кв1"
				ИЛИ (СтруктураДекларацииПоЕНВД.ДатаДокументаОбработкиСобытия > Дата(2018,7,1)) И СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2018Кв1"
				ИЛИ (СтруктураДекларацииПоЕНВД.ДатаДокументаОбработкиСобытия > Дата(2018,10,1)) И СтруктураДекларацииПоЕНВД.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2018Кв3" Тогда
...
Функция ВыполнитьРасчетЕНВД(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация, "ЮридическоеФизическоеЛицо,ИПИспользуетТрудНаемныхРаботников, ПоддержкаИП");
	...
	|	ПоказателиЕНВДПоказателиЕНВД.ЗначениеФизическогоПоказателя1 КАК ФизПоказатель1,
	|	ПоказателиЕНВДПоказателиЕНВД.ЗначениеФизическогоПоказателя2 КАК ФизПоказатель2,
	|	ПоказателиЕНВДПоказателиЕНВД.ЗначениеФизическогоПоказателя3 КАК ФизПоказатель3,
	...
	|	ПоказателиЕНВДПоказателиЕНВД.ВыработкаДней1,
	|	ПоказателиЕНВДПоказателиЕНВД.ВыработкаДней2,
	|	ПоказателиЕНВДПоказателиЕНВД.ВыработкаДней3,
	...
	Пока Выборка.Следующий() Цикл
		...
		Строка.ДнейВсего = Выборка.ДнейВсего1+Выборка.ДнейВсего2+Выборка.ДнейВсего3;
		Строка.ДнейОтработано = Выборка.ВыработкаДней1+Выборка.ВыработкаДней2+Выборка.ВыработкаДней3;
		Строка.БазоваяДоходность = Выборка.БазоваяДоходность;
		Строка.СтавкаЕНВД = Выборка.СтавкаЕНВД;
		Строка.К2Скорректированный = Выборка.КоэффициентК2*(Выборка.ФизПоказатель1*Выборка.ВыработкаДней1/Выборка.ДнейВсего1 + Выборка.ФизПоказатель2*Выборка.ВыработкаДней2/Выборка.ДнейВсего2 + Выборка.ФизПоказатель3*Выборка.ВыработкаДней3/Выборка.ДнейВсего3);
		Строка.ФизПоказатель = 1;
		Строка.СуммаНалога = Окр(Строка.БазоваяДоходность * К1 * Строка.К2Скорректированный * Строка.СтавкаЕНВД/100);
	КонецЦикла;
	...
	Для Каждого Строка Из СвернутаяЕНВД Цикл
		РегистрыСведений.РасходыНаККТУменьшающиеНалог.ПолучитьСуммуРасходовУменьшающиеНалог(Справочники.ЗаписиКалендаряПодготовкиОтчетности.ПолучитьЗаписьКалендаря(Организация,СобытиеКалендаря), , СуммаРасходовКВычету, Строка.ОКАТО, Строка.КодИФНС);
		Если РеквизитыОрганизации.ПоддержкаИП
			И НачалоДня(ПериодРасчета) = Дата('20200630') Тогда
			Строка.СуммаНалога = 0;
		Иначе
			Строка.СуммаНалога =  Макс(0, Строка.СуммаНалога - СуммаРасходовКВычету);
		КонецЕсли;
	КонецЦикла;
	...
	СтруктураРасчетаЕНВД.СуммаНалога = СвернутаяЕНВД.Итог("СуммаНалога");
	...
	Если СобытиеКалендаря <> Неопределено Тогда
		...
		КалендарьОтчетности.ЗаписатьСостояниеСобытияКалендаря(
			...
			"",
			СвернутаяЕНВД.Итог("СуммаНалога"));
