...
Процедура ПодобраннаяПродукцияПриДобавленииСтроки(Форма, СтрокаИзменений, Знач КоличествоКодовМаркировки, МодульКонтекста, ДополнительныеПараметры = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	ИначеЕсли СтрокаАгрегатор2 = Неопределено Тогда
		...
		Если УточнитьКоличествоИС Тогда
			ДобавляемоеКоличествоУпаковокИС    = ?(СвойстваВидаПродукции.ОтдельноеПолеКоличестваКодов, КоличествоПотребительскихУпаковокВСтруктуреУпаковок, 0);
		...
		Иначе
			ДобавляемоеКоличествоУпаковокИС    = 0;
			КоличествоКРаспределениюИС         = 0;
			КоличествоУпаковокКРаспределениюИС = 0;
		КонецЕсли;
		
		Если СвойстваВидаПродукции.РежимПодбораСуществующихУпаковок Тогда
		...
		Пока Не (СтрокаАгрегатор1 = Неопределено Или КоличествоКРаспределению = 0) Цикл
			...
			Если Не (УточнитьКоличествоИС Или СвойстваВидаПродукции.РежимПодбораСуществующихУпаковок) Тогда
				ДоступноеКоличествоИС = 0;
				Если СтрокаАгрегатор1.КоличествоИС > СтрокаАгрегатор1.КоличествоПодобрано Тогда
					ДоступноеКоличествоИС = СтрокаАгрегатор1.КоличествоИС - СтрокаАгрегатор1.КоличествоПодобрано;
				КонецЕсли;
		...
		Если Не (УточнитьКоличествоИС Или СвойстваВидаПродукции.РежимПодбораСуществующихУпаковок) Тогда
			Если ОбщееКоличествоКРаспределениюИС > ДобавляемоеКоличествоИС Тогда
			...
			Если СвойстваВидаПродукции.ОтдельноеПолеКоличестваКодов И ОбщееКоличествоУпаковокКРаспределениюИС > 0 Тогда
				НоваяСтрока.КоличествоПотребительскихУпаковокВСтруктуреУпаковок =
...
Процедура ЗаполнитьНоменклатуруВДеревеПоGTIN(СтрокаДерева, СтрокаИзменений, Форма, МодульКонтекста, КэшированныеЗначения)
	...
	Если Не ЗначениеЗаполнено(СтрокаДерева.Номенклатура)
		...
		ПроверкаИПодборПродукцииИСМПКлиентСервер.УстановитьДоступностьУпаковкиДляПроверки(СтрокаДерева, Форма.ДоступныеДляПроверкиУпаковки);
...
Процедура ОпределитьТипУпаковки(СтрокаДерева, ВключаяПодчиненные, ПараметрыЗаполнения, МодульКонтекста)
	...
	Если ПодчиненныеСтроки.Количество() = 0 Тогда
		...
		ИначеЕсли (СтрокаДерева.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Логистическая") Или ЭтоКодОСУ Или ЭтоГруппировкаОСУПоGTIN)
