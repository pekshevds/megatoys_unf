...
Функция ВыборкаСуммПоказателейБюджета(ВидОтчета, ДокументБюджет, БюджетМодифицирован) Экспорт
	...
	Иначе
		...
		Если ВидОтчета = Перечисления.ВидыФинансовыхОтчетов.Баланс И ДокументБюджет.Проведен Тогда
			Запрос.Текст = ТекстЗапросаБалансПоПроведенномуБюджету();
			Запрос.УстановитьПараметр("СценарийПланирования", ДокументБюджет.СценарийПланирования);
			Запрос.УстановитьПараметр("НачалоПериода", ДокументБюджет.ДатаНачала);
			Запрос.УстановитьПараметр("КонецПериода", ДокументБюджет.ДатаОкончания);
		КонецЕсли;
	...
	Если ВидОтчета = Перечисления.ВидыФинансовыхОтчетов.ДоходыРасходы Тогда
		...
		Если СценарийПланирования = Справочники.СценарииПланирования.Фактический Тогда
			...
			ТекстЗапроса = 
				...
				|				И НАЧАЛОПЕРИОДА(ДоходыИРасходыОбороты.Период, МЕСЯЦ) = Закрытия.Период
...
// Возвращает таблицу подключаемых показателей
//
// Параметры:
//     ДанныеОтчета - Структура - параметры формирования отчета
// Возвращаемое значение:
//     ТаблицаЗначений - Таблица подключаемых показателей
//
Функция ПолучитьТаблицуПодключаемыхПоказателей(ДанныеОтчета) Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.ПоказателиБизнеса КАК ПоказателиБизнеса
		|ГДЕ
		|	ПоказателиБизнеса.СпособПолученияДанныхИсточника = ЗНАЧЕНИЕ(Перечисление.СпособыПолученияДанныхИсточника.Расширение)
		|	И НЕ ПоказателиБизнеса.ПометкаУдаления
		|	И ПоказателиБизнеса.ВидОтчета = &ВидОтчета";
	
	Запрос.УстановитьПараметр("ВидОтчета", ДанныеОтчета.ВидОтчетаОтбор);
	...
	ТаблицаРезультат = ИнициализироватьТаблицуРезультат();
	ПараметрыПодключаемойПроцедуры = СформироватьПараметрыПодключаемойПроцедуры(ДанныеОтчета, ТаблицаРезультат);
	
	ВыборкаПодключаемыхПоказателей = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаПодключаемыхПоказателей.Следующий() Цикл
		
		ИмяПроцедуры = ИмяПроцедурыПоИдентификатору(ВыборкаПодключаемыхПоказателей.ИдентификаторПоказателя);
		
		// Устанавливаем ссылку на текущий показатель
		ПараметрыПодключаемойПроцедуры[0] = ВыборкаПодключаемыхПоказателей.Ссылка;
		
		Попытка
			ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяПроцедуры, ПараметрыПодключаемойПроцедуры);
		Исключение
			ТекстОшибки = НСтр("ru='Процедура показателя %1 не найдена. Проверьте подключено ли расширение.'");
			ЗаписьЖурналаРегистрации(НСтр("ru='ПоказателиБизнеса.ПолучитьТаблицуПодключаемыхПоказателей'",
									ОбщегоНазначения.КодОсновногоЯзыка()),
									УровеньЖурналаРегистрации.Ошибка,
									Метаданные.ОбщиеМодули.ПоказателиБизнеса,
									,
									СтрШаблон(ТекстОшибки, ВыборкаПодключаемыхПоказателей.Ссылка));
		КонецПопытки;
...
//
// Параметры:
//     ДанныеОтчета - Структура - параметры формирования расшифровки
// Возвращаемое значение:
//     ТаблицаЗначений - Таблица расшифровки подключаемого показателя
//
Функция ПолучитьРасшифровкуПодключаемогоПоказателя(ДанныеОтчета) Экспорт
	
	ТаблицаРасшифровка = ИнициализироватьТаблицуРасшифровка();
	ПараметрыПодключаемойПроцедуры = СформироватьПараметрыПодключаемойПроцедуры(ДанныеОтчета, ТаблицаРасшифровка, Истина);
	
	ИмяПроцедуры = ИмяПроцедурыПоИдентификатору(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеОтчета.Показатель, "ИдентификаторПоказателя"), Истина);
	
	// Устанавливаем ссылку на текущий показатель
	ПараметрыПодключаемойПроцедуры[0] = ДанныеОтчета.Показатель;
	
	Попытка
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяПроцедуры, ПараметрыПодключаемойПроцедуры);
	Исключение
		ТекстОшибки = "Процедура расшифровки показателя %1 не найдена. Проверьте подключено ли расширение.";
		ЗаписьЖурналаРегистрации(НСтр("ru='ПоказателиБизнеса.ПолучитьРасшифровкуПодключаемогоПоказателя'",
								ОбщегоНазначения.КодОсновногоЯзыка()),
								УровеньЖурналаРегистрации.Ошибка,
								Метаданные.ОбщиеМодули.ПоказателиБизнеса,
								,
								СтрШаблон(ТекстОшибки, ДанныеОтчета.Показатель));
	КонецПопытки;
...
Функция ТекстЗапросаДоходыРасходыПоБюджету(ПоСсылке)
	...
	ТекстЗапроса = ТекстЗапроса + 
					...
					|	СУММА(ВЫБОР
					|			КОГДА ДоходыИРасходы.ТипПоказателя = ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейБизнеса.ПустаяСсылка)
					|					ИЛИ ДоходыИРасходы.ТипПоказателя = ПоказателиБизнеса.Ссылка.ТипПоказателя
					|				ТОГДА ДоходыИРасходы.Сумма
					|			ИНАЧЕ 0
					|		КОНЕЦ) КАК Сумма
					|ИЗ
...
Функция ТекстЗапросаДенежныйПотокПоБюджету(ПоСсылке)
	...
	ТекстЗапроса = ТекстЗапроса +
					...
					|	СУММА(ВЫБОР
					|			КОГДА ПоступленияВыбытия.ТипПоказателя = ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейБизнеса.Расход)
					|				ТОГДА -ПоступленияВыбытия.Сумма
					|			ИНАЧЕ ПоступленияВыбытия.Сумма
					|		КОНЕЦ) КАК Сумма
					|ИЗ
...
Функция ТекстЗапросаДанныеТаблицДоходыРасходы(ПоСсылке)
	...
	Если ПоСсылке Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ
		               ...
		               |	ПрямыеЗатраты.Счет,
		               |	ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейБизнеса.ПустаяСсылка),
		               |	ПрямыеЗатраты.Сумма
		               |ИЗ
...
Функция ТекстЗапросаДенежныйПотокПланФакт()
	...
	|			ИНАЧЕ -1
	|		КОНЕЦ * ВЫБОР
	|			КОГДА ПоказателиПоСценариям.Коэффициент = 0
	|				ТОГДА 1
	|			ИНАЧЕ ПоказателиПоСценариям.Коэффициент
	|		КОНЕЦ) КАК СуммаПлан,
	|	СУММА(ВЫБОР
	|			КОГДА ДвиженияДенежныхСредствОбороты.СценарийПланирования <> ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический)
	|				ТОГДА 0
...
Функция ТекстЗапросаБаланс()
	...
	ТекстЗапроса = 
		...
		|ИЗ
		|	ПоказателиБизнеса КАК ПоказателиБизнеса
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Управленческий.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, МЕСЯЦ, ДвиженияИГраницыПериода, , СценарийПланирования = &СценарийПланирования) КАК УправленческийОбороты
...
Функция ТекстЗапросаБалансПоПроведенномуБюджету()
	...
	ТекстЗапроса = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	УправленческийОбороты.Период КАК ДатаПланирования,
		|	ПоказателиБизнеса.Показатель КАК Показатель,
		|	ПоказателиБизнеса.ИдентификаторПоказателя КАК ИдентификаторПоказателя,
		|	СУММА(ВЫБОР
		|			КОГДА ПоказателиБизнеса.СпособПолученияДанныхИсточника = ЗНАЧЕНИЕ(Перечисление.СпособыПолученияДанныхИсточника.ДтСальдо)
		...
		|ИЗ
		|	ПоказателиБизнеса КАК ПоказателиБизнеса
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Управленческий.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Регистратор, ДвиженияИГраницыПериода, , СценарийПланирования = &СценарийПланирования) КАК УправленческийОбороты
		|		ПО ПоказателиБизнеса.ИсточникДанных = УправленческийОбороты.Счет
		|ГДЕ
		|	УправленческийОбороты.Период <> ДАТАВРЕМЯ(1, 1, 1)
		|	И УправленческийОбороты.Регистратор = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	УправленческийОбороты.Период,
		|	ПоказателиБизнеса.Показатель,
...
#Область ПодключаемыеПоказатели
...
Функция ИмяПроцедурыПоИдентификатору(ИдентификаторПоказателя, ЭтоРасшифровка = Ложь)
	...
	Если ЭтоРасшифровка Тогда
		ИмяПроцедуры = СтрШаблон("ПодключаемыеПроцедуры.ПолучитьРасшифровкуПоказателя_%1", АдаптированныйИдентификатор);
	Иначе
		ИмяПроцедуры = СтрШаблон("ПодключаемыеПроцедуры.ПолучитьПоказатель_%1", АдаптированныйИдентификатор);
	КонецЕсли;
...
Функция СформироватьПараметрыПодключаемойПроцедуры(ДанныеОтчета, ТаблицаРезультат, ЭтоРасшифровка = Ложь)
	
	ПараметрыОтчета = Новый Структура;
	
	Если ЭтоРасшифровка Тогда
		ПериодОтчета = Новый СтандартныйПериод;
		ПериодОтчета.ДатаНачала = ДанныеОтчета.НачалоПериода;
		ПериодОтчета.ДатаОкончания = ДанныеОтчета.КонецПериода;
		ПараметрыОтчета.Вставить("ПериодОтчета", ПериодОтчета);
		ПараметрыОтчета.Вставить("Группировка", ДанныеОтчета.Группировка);
	Иначе
		ПараметрыОтчета.Вставить("ПериодОтчета", ДанныеОтчета.ПериодОтчета);
		ПараметрыОтчета.Вставить("ПланФакт", ДанныеОтчета.ПланФакт);
		ПараметрыОтчета.Вставить("Группировка", ДанныеОтчета.ГруппировкаАнализаБизнеса);
	КонецЕсли;
	
	ПараметрыОтчета.Вставить("Периодичность", ДанныеОтчета.Периодичность);
	ПараметрыОтчета.Вставить("Сценарий", ДанныеОтчета.СценарийПланирования);
	
	Отбор = Новый Структура;
	
	Для каждого СтрокаМетки Из ДанныеОтчета.ДанныеМеток  Цикл
		
		Если НЕ Отбор.Свойство(СтрокаМетки.ИмяПоляОтбора) Тогда
			Отбор.Вставить(СтрокаМетки.ИмяПоляОтбора, Новый Массив);
		КонецЕсли;
		
		Отбор[СтрокаМетки.ИмяПоляОтбора].Добавить(СтрокаМетки.Метка);
	...
	ПараметрыПроцедуры.Добавить(Справочники.ПоказателиБизнеса.ПустаяСсылка());
	ПараметрыПроцедуры.Добавить(ПараметрыОтчета);
	ПараметрыПроцедуры.Добавить(ТаблицаРезультат);
	
	Возврат ПараметрыПроцедуры;
	
КонецФункции

Функция ИнициализироватьТаблицуРезультат()
	
	ТаблицаРезультат = Новый ТаблицаЗначений;
	ТаблицаРезультат.Колонки.Добавить("Период", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ТаблицаРезультат.Колонки.Добавить("Показатель", Новый ОписаниеТипов("СправочникСсылка.ПоказателиБизнеса"));
	ТаблицаРезультат.Колонки.Добавить("ИдентификаторПоказателя", ОбщегоНазначения.ОписаниеТипаСтрока(36));
	ТаблицаРезультат.Колонки.Добавить("Аналитика", ОбщегоНазначения.ОписаниеТипаВсеСсылки());
	ТаблицаРезультат.Колонки.Добавить("Сумма", ОбщегоНазначения.ОписаниеТипаЧисло(30, 2));
	ТаблицаРезультат.Колонки.Добавить("СуммаФакт", ОбщегоНазначения.ОписаниеТипаЧисло(30, 2));
	ТаблицаРезультат.Колонки.Добавить("СуммаПлан", ОбщегоНазначения.ОписаниеТипаЧисло(30, 2));
	
	Возврат ТаблицаРезультат;
	
КонецФункции

Функция ИнициализироватьТаблицуРасшифровка()
	
	ТаблицаРезультат = Новый ТаблицаЗначений;
	ТаблицаРезультат.Колонки.Добавить("Период", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ТаблицаРезультат.Колонки.Добавить("Документ", ОбщегоНазначения.ОписаниеТипаВсеСсылки());
	ТаблицаРезультат.Колонки.Добавить("Сумма", ОбщегоНазначения.ОписаниеТипаЧисло(30, 2));
	ТаблицаРезультат.Колонки.Добавить("Аналитика", ОбщегоНазначения.ОписаниеТипаВсеСсылки());
	
	ТаблицаРезультат.Колонки.Добавить("Проект", Новый ОписаниеТипов("СправочникСсылка.Проекты"));
	ТаблицаРезультат.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.СтруктурныеЕдиницы"));
	ТаблицаРезультат.Колонки.Добавить("Содержание", ОбщегоНазначения.ОписаниеТипаСтрока(500));
