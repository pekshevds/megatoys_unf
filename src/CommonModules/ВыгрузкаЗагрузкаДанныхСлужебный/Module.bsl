...
Функция ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(Знач ИнформацияОВыгрузке) Экспорт
	...
	СовместимаСТекущейКонфигурацией = 
		ИнформацияОВыгрузке.Configuration.Name = Метаданные.Имя;
	
	// Переопределяемая процедура
	ВыгрузкаЗагрузкаДанныхПереопределяемый.ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(
		ИнформацияОВыгрузке, СовместимаСТекущейКонфигурацией);
	
	Возврат СовместимаСТекущейКонфигурацией;
...
// Получить имя последнего каталога в архиве.
// 
// Параметры:
//  Архив - см. ZipАрхивы.ПрочитатьАрхив
//  ИмяКаталогаБезНомера - Строка - Имя искомого каталога без номера
// 
// Возвращаемое значение:
//  Строка
Функция ПолучитьИмяПоследнегоКаталогаВАрхиве(Архив, ИмяКаталогаБезНомера) Экспорт
	
	ИмяКаталога = ИмяКаталогаБезНомера;
	МаксимальныйНомерКаталога = 0;
	ТипЧисло = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный));
	
	Для Каждого ЭлементАрхива Из Архив.КаталогФайлов Цикл
		
		ИмяФайла = ЭлементАрхива.Ключ;
		
		Если Не СтрНачинаетсяС(ИмяФайла, ИмяКаталогаБезНомера) Тогда
			Продолжить;
		КонецЕсли;
		
		ЧастиИмениФайла = СтрРазделить(ИмяФайла, "/", Ложь);
		ПолноеИмяКаталога = ЧастиИмениФайла[0];
		НомерКаталогаСтрокой = СтрЗаменить(ПолноеИмяКаталога, ИмяКаталогаБезНомера, "");
		НомерКаталога = ТипЧисло.ПривестиЗначение(НомерКаталогаСтрокой);
		
		Если НомерКаталога > МаксимальныйНомерКаталога Тогда
			ИмяКаталога = ПолноеИмяКаталога;
			МаксимальныйНомерКаталога = НомерКаталога;
		КонецЕсли;
...
Функция НачатьЗагрузкуДанныхТекущейОбластиИзАрхива(Знач ДанныеФайла, Знач ПараметрыЗагрузки, Знач АдресРезультата = Неопределено) Экспорт 
	...
	ТипРезервнойКопии = ТипРезервнойКопии(ПараметрыЗагрузки);
	
	ПараметрыЗагрузки.Вставить(
		"ЗагружаемыеТипы",
		ЗагружаемыеТипы(ТипРезервнойКопии));
	...
	ПроверитьВыгрузкаВАрхивеСовместимаСТекущейВерсиейКонфигурации(
		ИнформацияОВыгрузке,
		ТипРезервнойКопии <> ТипРезервнойКопииПолная());
...
Функция РезультатПроверкиВозможностиПродолженияЗагрузкиДанныхОбласти(
	ИнформацияОПроцедуреЗагрузки,
	ХешСуммаИсточника = Неопределено,
	ХешСуммаПараметров = Неопределено)
	...
	Если Результат.ЭтоПродолжениеЗагрузки Тогда
		...
		ПроверитьВозможностьПродолженияЗагрузкиПоХешСуммеИсточника(
			Результат,
			ИнформацияОПроцедуреЗагрузки,
			ХешСуммаИсточника);
		
		ПроверитьВозможностьПродолженияЗагрузкиПоХешСуммеПараметров(
			Результат,
			ИнформацияОПроцедуреЗагрузки,
			ХешСуммаПараметров);
...
Процедура ПроверитьВозможностьПродолженияЗагрузкиПоХешСуммеИсточника(
	РезультатПроверки,
	ИнформацияОПроцедуреЗагрузки,
	ХешСуммаИсточника)
	
	Если ХешСуммаИсточника = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РезультатПроверки.РазличаетсяХешСуммаИсточника
		= ИнформацияОПроцедуреЗагрузки.ХешСуммаИсточника <> ХешСуммаИсточника;
	
КонецПроцедуры

Процедура ПроверитьВозможностьПродолженияЗагрузкиПоХешСуммеПараметров(
	РезультатПроверки,
	ИнформацияОПроцедуреЗагрузки,
	ХешСуммаПараметров)
	
	Если ХешСуммаПараметров = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РезультатПроверки.РазличаетсяХешСуммаПараметров
...
Процедура ЗаписатьСобытиеОтсутствияЗаписиПредыдущегоСостояния(ИдентификаторСостояния)
	...
	ЗаписьЖурналаРегистрации(
		...
		УровеньЖурналаРегистрации.Предупреждение,,
		ИдентификаторСостояния,
		НСтр("ru = 'Восстановление состояние пропущено'"));
