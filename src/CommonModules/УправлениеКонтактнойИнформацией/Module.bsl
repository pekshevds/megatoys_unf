...
Процедура ПриСозданииНаСервере(Форма, Объект, ДополнительныеПараметры = Неопределено, УдалитьПоложениеЗаголовкаКИ = "",
	...
	// УНФ
	УправлениеКонтактнойИнформациейПереопределяемый.ПриСозданииНаСервере(Форма, Объект, ДополнительныеПараметры);
	// Конец УНФ
...
Процедура ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма , ДанныеВладельцевКИ, ДополнительныеПараметры = Неопределено, УдалитьПоложениеЗаголовкаКИ = "",
	...
	УправлениеКонтактнойИнформациейПереопределяемый.ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма, ДанныеВладельцевКИ, ДополнительныеПараметры);
...
Процедура ПриЧтенииНаСервереСвязанныхВладельцевКИ(Форма, ДанныеВладельцевКИ, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	Для каждого СтрокаДанныеВладельцевКИ Из КоллекцияДанныеВладельцевКИ Цикл
		...
		Если ИспользуетсяКонтактнаяИнформация = Ложь Тогда
			МассивУдаляемыхРеквизитов = Параметры.ДобавленныеРеквизиты;
		Иначе
			УдалитьКомандыИЭлементыФормыБезопасно(Форма, ИмяЭлементаДляРазмещения);
...
Процедура СоздатьРеквизитыКонтактнойИнформации(Знач Форма, Знач ДобавляемыеРеквизиты, Знач ИмяОбъекта, Знач РазмещеныНаФорме,
	Знач КонтактнаяИнформация, Знач ОтложеннаяИнициализация, Знач ОбработкаНавигационнойСсылки, Знач ПостфиксИмяРеквизита = "")
		...
		Иначе
			...
			Если СозданныйРеквизит = Неопределено Тогда
				...
				Если Не ОтложеннаяИнициализация Тогда
					...
					ДобавляемыеРеквизиты.Добавить(
						Новый РеквизитФормы(ОбъектКонтактнойИнформации.ИмяРеквизита + ПостфиксИмяРеквизита, ТипРеквизита,, ОбъектКонтактнойИнформации.Наименование, Истина));
					// Конец УНФ
					Если ЕстьПолеКомментарий Тогда					
						// УНФ
						//ДобавляемыеРеквизиты.Добавить(
						...
						ДобавляемыеРеквизиты.Добавить(
							Новый РеквизитФормы(ОбъектКонтактнойИнформации.ИмяРеквизитаКомментарий + ПостфиксИмяРеквизита, ТипРеквизита,, ОбъектКонтактнойИнформации.Наименование, Истина));
						// Конец УНФ
					КонецЕсли;
				КонецЕсли;
			Иначе
	...
	// Конец 3.1.9
		...
		Иначе
			...
			ИначеЕсли ПоследняяГруппа <> Неопределено И НЕ ЕстьКнопкаКонтактнойИнформации(ПоследняяГруппа, Истина)
				...
				Декорация = Форма.Элементы.Добавить("ОтступДобавить" + ИмяЭлементаДляРазмещения, Тип("ДекорацияФормы"), ГруппаКоманды);
				// Конец УНФ
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено, ПостфиксИмяРеквизита = "")
	// УНФ Добавлен параметр ПостфиксИмяРеквизита.
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		Если ТипЗнч(ДобавляемыйВид)= Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			...
			ИнформацияОВидеКИ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДобавляемыйВид, "Тип, Наименование, ВидРедактирования, ВидПоляДругое, ВводитьНомерПоМаске, МаскаНомераТелефона, ПометкаУдаления, ХранитьИсториюИзменений");
	...
	ОтборПоВиду = Новый Структура("Вид, ЭтоИсторическаяКонтактнаяИнформация, ИмяЭлементаДляРазмещения", ДобавляемыйВид, Ложь, ИмяЭлементаДляРазмещения);
	// Конец УНФ
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКонтактнойИнформации, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,
	ЭтоРеквизитТабличнойЧасти = Ложь, ЗаполнятьЗначениеРеквизита = Истина, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация", ПостфиксИмяРеквизита = "")
	// УНФ добавлен параметр ПостфиксИмяРеквизита.
...
Процедура УдалитьКомандыИЭлементыФормыБезопасно(Форма, ИмяЭлементаДляРазмещения)
...
Функция КонтактнаяИнформацияИзРеквизитовФормы(Форма, Объект, ИмяЭлементаДляРазмещения = Неопределено)
	// УНФ добавлен параметр ИмяЭлементаДляРазмещения.
