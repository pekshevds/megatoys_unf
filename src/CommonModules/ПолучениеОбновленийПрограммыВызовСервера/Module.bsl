	...
	Если Не РаботаВПрограммеЗапрещена Тогда
		...
		Иначе
			...
			Если ТипЗнч(НастройкиОповещения) = Тип("Структура")
				...
				И НастройкиОповещения.Свойство("РекомендуемаяВерсияПлатформы")
				И ИнтернетПоддержкаПользователей.ИмяКонфигурации() = НастройкиОповещения.МетаданныеИмя
				И ИнтернетПоддержкаПользователей.ВерсияКонфигурации() = НастройкиОповещения.МетаданныеВерсия
				И РекомендуемаяВерсияПлатформы = НастройкиОповещения.РекомендуемаяВерсияПлатформы Тогда
