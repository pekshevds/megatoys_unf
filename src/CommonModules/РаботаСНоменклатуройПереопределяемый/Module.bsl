...
Процедура СоздатьНоменклатуру(ДанныеЗаполнения, НоменклатураСсылка) Экспорт
	...
	СтавкаНДС = СтавкаНДСПоДаннымНоменклатурыСервиса(ДанныеЗаполнения.СтавкаНДС, НоменклатураОбъект.КатегорияНоменклатуры);
...
Процедура ПолучитьОтличияНоменклатуры(ДанныеНоменклатурыСервиса, ДанныеНоменклатурыБазы, ТаблицаОтличийРеквизитов) Экспорт
	...
	Если ВидыНоменклатуры.Количество() Тогда
		ВидНоменклатуры = ВидыНоменклатуры[0];
		Если ВидНоменклатуры <> ДанныеНоменклатурыБазы.ВидНоменклатуры Тогда 
			РаботаСНоменклатурой.ДобавитьСтрокуВТаблицуОтличий(ТаблицаОтличийРеквизитов,
				"КатегорияНоменклатуры", ВидНоменклатуры, ДанныеНоменклатурыБазы.ВидНоменклатуры, НСтр("ru = 'Категория'"), ДанныеНоменклатурыСервиса.Категория.Наименование);
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеНоменклатурыБазы.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас
		...
		// Проверка ставки НДС
		СтавкаНДССервиса = СтавкаНДСПоДаннымНоменклатурыСервиса(ДанныеНоменклатурыСервиса.СтавкаНДС, ВидНоменклатуры);
...
//    * ВидНоменклатурыНаименование - Строка - пользовательское представление набора доп.реквизитов. Пример: "Столы (вид
//                                             номенклатуры)".
...
//                                      Если передано Неопределено, то нужно получить доп.реквизиты по всем видам
//                                      номенклатуры без отбора.
// 
Процедура ДополнительныеРеквизитыНоменклатурыДляВыгрузки(ДополнительныеРеквизиты, ВидыНоменклатуры) Экспорт
	...
	|			ГДЕ
	|				Наборы.Ссылка = &НаборСвойств_Справочник_Номенклатура_Общие)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	Запрос.УстановитьПараметр("НаборСвойств_Справочник_Номенклатура_Общие", УправлениеСвойствами.НаборСвойствПоИмени(
...
//                                      значения - пользовательские представления товарных групп, например: "Товары
//                                                 легкой промышленности и одежда",
...
Процедура ИнициализацияЗапросаОтбораНоменклатурыСХарактеристиками(ТекстЗапроса) Экспорт
	...
	|ГДЕ
	|	ХарактеристикиНоменклатуры.Владелец.ИспользоватьХарактеристики
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	Характеристика
	...
	|{ГДЕ
	|	СправочникНоменклатура.Ссылка.* КАК Номенклатура,
	|	СправочникНоменклатура.Ссылка.КатегорияНоменклатуры.* КАК КатегорияНоменклатуры,
	...
	|	ВИДЫХАРАКТЕРИСТИК (ВЫБРАТЬ
	|			ДополнительныеРеквизитыИСведения.Ссылка КАК Ссылка,
	|			ДополнительныеРеквизитыИСведения.Наименование + "" ("" + ДополнительныеРеквизитыИСведения.НаборСвойств.Наименование +
	|				"")"" КАК Наименование,
	|			ДополнительныеРеквизитыИСведения.ТипЗначения КАК ТипЗначения
	|		ИЗ
	|			ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения)
...
Функция СтавкаНДСПоДаннымНоменклатурыСервиса(Значение, КатегорияНоменклатуры)
	...
	Если ТипЗнч(КатегорияНоменклатуры) = Тип("СправочникСсылка.КатегорииНоменклатуры") Тогда
		ВидСтавкиНДС = КатегорияНоменклатуры.ВидСтавкиНДС;
		Если ЗначениеЗаполнено(ВидСтавкиНДС) Тогда
			Возврат Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиНДС, ТекущаяДатаСеанса());
		КонецЕсли;
	КонецЕсли;
