...
// Возвращает результат чтения из временного файла, в коллекцию НовыйДанныеИзБанка()
...
//  ОдиночныйРазделительСтрок - Булево - Истина, если строки файла разделены одним символом ПС (LF); Ложь, если строки
//                                       файла разделены двумя символами ВК+ПС (CR+LF)
// 
...
Функция ПрочитатьФайл(ИмяФайла, Кодировка, ОдиночныйРазделительСтрок)
	...
	// При обходе файла будем отмечать, к какой секции относится конкретная строка файла, и накапливать все данные,
	// относящиеся к этой секции. Когда из данных файла будет понятно, что секция закончилась,
...
//
// Параметры:
//  ВерсияФормата - Строка - Версия формата файла выписки.
// 
// Возвращаемое значение:
//  Структура - содержит коллекцию, предназначенную для хранения данных файла банковской выписки в формате обмена 1С
//
Функция НовыйДанныеИзБанка(ВерсияФормата = "1.03") Экспорт
	...
	ТипНомерСчета     = БанковскиеПравилаКлиентСервер.ТипНомерСчета();
	ТипНомерКорСчета  = БанковскиеПравилаКлиентСервер.ТипМеждународныйНомерСчета(); // Допускается передавать номер счета контрагента в национальном или международном формате
	...
	ДанныеИзБанка.Документы.Колонки.Добавить("Идентификатор", КлиентБанкКлиентСервер.ТипИдентификатор()); // Должен быть уникальным в рамках коллекции
...
// Возвращает реквизиты плательщика и получателя
//
// Возвращаемое значение:
//  Массив - содержит значения "Плательщик" и "Получатель"
//
...
Функция ОписаниеФайлаОбмена() Экспорт
	...
	Описание.Секции.Колонки.Добавить("Элементы",              Новый ОписаниеТипов("Соответствие")); // Теги вложенных элементов. Должны соответствовать требованиям к ключам структуры. Приводятся к верхнему регистру.
	...
	// Сначала опишем эти преобразования, а затем по данным о преобразованиях заполним ОписаниеСекции.Элементы
	...
	// Приведем теги в колонках таблиц значений к верхнему регистру
	...
	// Заполним ОписаниеСекции.Элементы по Описание.ПравилаРазмещенияДанныхОДокументах
...
Функция ЭлементыСтруктурированногоНазначенияПлатежа()
	...
	ЭлементыСтруктурированногоНазначенияПлатежа.Добавить("ДополнУсловия"); // АПК:1036
...
Процедура ПеренестиВДанныеБанка(ДанныеИзБанка, СтекСекций, ОписаниеФайлаОбмена)
		...
		Для Каждого ИмяРазделаДанных Из РазделыДанных Цикл
			...
			Для Каждого ЭлементДанных Из РазделДанных Цикл
				Если Секция.Данные.Свойство(ЭлементДанных.Ключ) Тогда
					...
					РазделДанных[ЭлементДанных.Ключ] = КлиентБанкКлиентСервер.ПривестиСтроку(Секция.Данные[ЭлементДанных.Ключ], ТипЗначения);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	...
	ИначеЕсли Секция.Описание.Имя = "Остатки" Тогда
		...
		Для Каждого Колонка Из ДанныеИзБанка.Остатки.Колонки Цикл
			Если Секция.Данные.Свойство(Колонка.Имя) Тогда
				СтрокаДанныхИзБанка[Колонка.Имя] = КлиентБанкКлиентСервер.ПривестиСтроку(Секция.Данные[Колонка.Имя], Колонка.ТипЗначения);
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли Секция.Описание.Имя = "Документ" Тогда
		...
		// Часть данных размещаем особым образом в соответствии с требованиями к коллекции - "упаковываем", помещаем в колонки
		// с другими именами
		Для Каждого РазмещениеРеквизита Из ОписаниеФайлаОбмена.ПравилаРазмещенияДанныхОДокументах Цикл
			...
			Если ЗначениеСтрокой <> Значение И Не ЗначениеЗаполнено(Значение) Тогда
				...
				Значение = КлиентБанкКлиентСервер.ПривестиСтроку(ЗначениеСтрокой, ТипЗначения);
			КонецЕсли;
		...
		Для Каждого Участник Из Участники Цикл
			...
			Если Не ПустаяСтрока(ИНН) И Не РеквизитыУчастника.Свойство("КПП") Тогда
				Если СтрДлина(ИНН) > ИдентификационныеНомераНалогоплательщиковКлиентСервер.МинимальнаяДлинаИНН()
					И Не ИдентификационныеНомераНалогоплательщиковКлиентСервер.ЭтоИННФизическогоЛица(ИНН)
					И Не ИдентификационныеНомераНалогоплательщиковКлиентСервер.ЭтоИННЮридическогоЛица(ИНН) Тогда
		...
		// Формат не предполагает указывать отрицательные суммы, однако отдельные банки указывают, причем для всех расходных
		// или для всех кредитовых операций. Поэтому считаем указание отрицательной суммы ошибкой, которую можем исправить.
...
//   метод нужно вызывать многократно - для каждой строки файла, а конструирование коллекции для возвращаемого
//   значения - относительно затратная операция.
//
Процедура РазделитьСтрокуФайла(Строка, Тег, Значение)
	...
	Если Не ПустаяСтрока(Значение) Тогда
		// Недопустимые символы встречаются редко, а поиск существенно быстрее, чем замена, так как при замене строка
		// передается по значению. Поэтому оптимальнее искать до вызова УдалитьНедопустимыеСимволыXML, даже если в случае
		// недопустимого символа придется искать еще раз.
...
Процедура ОчиститьТолькоНулиВСтроке(Значение)
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		Если СтрНайти(Значение, "0") > 0 // Для оптимизации делаем лишний поиск в строке: он очень быстрый, а нули встречаются не всегда
			И КлиентБанкКлиентСервер.ТолькоНулиВСтроке(Значение) Тогда
			Значение = "";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ИсправитьОшибкиПлательщикСчет(СтрокаДанныхИзБанка) Экспорт
	...
	Если Не БанковскиеПравилаКлиентСервер.СтрокаСоответствуетФорматуБанковскогоСчета(СтрокаДанныхИзБанка.ПлательщикСчет)
		Или Не СтрокаДанныхИзБанка.ПлательщикРеквизиты.Свойство("Наименование") Тогда
		Возврат;
	КонецЕсли;
		
	БалансовыйСчет = БанковскиеПравилаКлиентСервер.БалансовыйСчет(СтрокаДанныхИзБанка.ПлательщикСчет);
	Если БанковскиеПравилаКлиентСервер.ЭтоСчетРасчетовФилиалаБанкаСКлиентамиБанка(БалансовыйСчет) Тогда
		Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка";
	ИначеЕсли БанковскиеПравилаКлиентСервер.ЭтоКорсчетБанкаНерезидента(БалансовыйСчет) Тогда
		Проблема = "РасчетыЧерезБанкНерезидент";
	ИначеЕсли БанковскиеПравилаКлиентСервер.ЭтоКорСчетКредитныхОрганизацийКорреспондентов(БалансовыйСчет)
		Или БанковскиеПравилаКлиентСервер.ЭтоКорсчетВКредитнойОрганизацииКорреспонденте(БалансовыйСчет) Тогда
	...
	Для Каждого ВозможныйНомерСчета Из СловаПлательщика Цикл
		...
		Если Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка" Или Проблема = "РасчетыЧерезБанкКорреспондент" Тогда
			...
			Если БанковскиеПравилаКлиентСервер.СтрокаСоответствуетФорматуБанковскогоСчета(ВозможныйНомерСчета) Тогда
		...
		ИначеЕсли Проблема = "РасчетыЧерезБанкНерезидент" Тогда
			...
			Если БанковскиеПравилаКлиентСервер.СтрокаСоответствуетФорматуIBAN(ВозможныйНомерСчета)
				И БанковскиеПравилаКлиентСервер.ПроверитьКонтрольныйКлючIBAN(ВозможныйНомерСчета) Тогда
				НомерСчета = ВозможныйНомерСчета;
				Прервать;
			КонецЕсли;
...
Функция ТипФайла(ИмяФайла)
	...
	Пока СтрокаТекста <> Неопределено Цикл
		...
		Если НЕ КлиентБанкКлиентСервер.ТолькоСимволыВСтроке(СтрокаТекста) Тогда
			Возврат "cp866";
			Прервать;
		КонецЕсли;
