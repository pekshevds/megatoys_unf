...
//  ДополнятьПериод				 - Булево				 - Признак использования дополнения
//  ПодбиратьПериодАвтоматически - Белево				 - Признак автоматического подбора периодичности по периоду
//
Процедура НастроитьДинамическийПериод(СхемаКомпоновкиДанных, ПараметрыОтчета, ДополнятьПериод = Ложь, ПодбиратьПериодАвтоматически = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПараметрПериодичность.Значение) И НЕ ПодбиратьПериодАвтоматически Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьМакетОформленияОтчета(НастройкиОтчета) Экспорт
	...
	МакетОформления = "ОформлениеОтчетовСтрогий";
