...
Функция ПредставлениеЗаголовкаПодчиненногоСчетаФактуры(ТекущийДокументСсылка, Источник, ПараметрыОповещения, КэшЗначений) Экспорт
	...
	Если ТипЗнч(ПараметрыОповещения) = Тип("Структура") Тогда
		Если ПараметрыОповещения.Свойство("ЭтоУниверсальныйДокумент") Тогда
			УПД = ПараметрыОповещения.ЭтоУниверсальныйДокумент;
		Иначе
			УПД = Ложь;
		КонецЕсли;
		
		Если ПараметрыОповещения.Свойство("Корректировочный") Тогда
			Корректировочный = ПараметрыОповещения.Корректировочный;
		Иначе
	...
	Если ТекущийДокументУказанОснованиемСФ Тогда
		...
		ПредставлениеЗаголовка = РаботаСФормойДокументаКлиентСервер.СформироватьНадписьСчетФактура(ПараметрыОповещения.ПредставлениеСчетФактуры,,, УПД, Корректировочный);
