...
Функция ЭффективныеСтавкиВзносовПоДаннымОценочныхОбязательств(Организация, Период, ФизическиеЛица, НастройкаРезервов)
	...
	|	ВЫРАЗИТЬ(ОценочныеОбязательстваПоСотрудникамОстатки.СуммаРезерваСтраховыхВзносовОстаток / ОценочныеОбязательстваПоСотрудникамОстатки.СуммаРезерваОстаток * 100 КАК ЧИСЛО(15, 4)) КАК СтавкаСтраховыхВзносов,
	|	ВЫРАЗИТЬ(ОценочныеОбязательстваПоСотрудникамОстатки.СуммаРезерваФССНесчастныеСлучаиОстаток / ОценочныеОбязательстваПоСотрудникамОстатки.СуммаРезерваОстаток * 100 КАК ЧИСЛО(15, 4)) КАК СтавкаФСС_НС
	|ИЗ
	|	РегистрНакопления.ОценочныеОбязательстваПоСотрудникам.Остатки(
	|			&Период,
	|			Организация = &Организация
