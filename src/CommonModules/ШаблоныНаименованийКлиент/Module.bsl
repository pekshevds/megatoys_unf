...
Процедура НаименованиеОбработкаВыбора(Форма, ВыбранноеЗначение, КатегорияНоменклатуры = Неопределено, Наименование, ВидНаименования) Экспорт
	...
	ОткрытиеИзХарактеристики = Ложь;
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		ОткрытиеИзХарактеристики = Форма.ИмяФормы = "Справочник.ХарактеристикиНоменклатуры.Форма.ФормаЭлемента";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Шаблон) Тогда
		ПараметрыФормы = Новый Структура;
		Если КатегорияНоменклатуры <> Неопределено Тогда
			Если ОткрытиеИзХарактеристики Тогда
				ПараметрыФормы.Вставить("Отбор", Новый Структура("Владелец", КатегорияНоменклатуры));
			Иначе
				ПараметрыФормы.Вставить("Отбор", Новый Структура("КатегорияНоменклатуры", КатегорияНоменклатуры));
			КонецЕсли;
		Иначе
			ПараметрыФормы.Вставить("Отбор", Новый Структура("ВидНаименования", ВидНаименования));
		КонецЕсли;
...
Процедура ОбновитьСписокСвойствОбъектаПриИзмененииСтандартногоРеквизита(Форма, ИмяРеквизита) Экспорт
	...
	Если ИмяРеквизита = "ВидДоговора" Тогда
		РедактируемоеСвойствоЗначение = НРег(РедактируемоеСвойствоЗначение);
	КонецЕсли;
	...
	Если ЭлементСпискаСвойств = Неопределено Тогда
		СвязанныеРеквизиты = ПолучитьСвязанныеЭлементы(Форма, ИмяРеквизита);
		Для Каждого ЭлементСпискаСвойств Из СвязанныеРеквизиты Цикл
			ЭлементСпискаСвойств.Представление = Формат(РедактируемоеСвойствоЗначение, ФорматСвойстваОбъекта(ЭлементСпискаСвойств.Значение));
		КонецЦикла;
	Иначе
		ЭлементСпискаСвойств.Представление = Формат(РедактируемоеСвойствоЗначение, ФорматСвойстваОбъекта(ЭлементСпискаСвойств.Значение));
	КонецЕсли;
...
Функция ПолучитьСвязанныеЭлементы(Форма, ИмяРеквизита)
	
	СвязанныеРеквизиты = Новый Массив;
	
	СписокСвойств = Новый Структура();
	СписокСвойств.Вставить("ДатаНачала", "ДатаДоговора");
	СписокСвойств.Вставить("ДатаОкончания", "СрокДействия");
	СписокСвойств.Вставить("ДатаНачалаСокр", "ДатаДоговора");
	СписокСвойств.Вставить("ДатаОкончанияСокр", "СрокДействия");
	СписокСвойств.Вставить("Организация", "Владелец");
	СписокСвойств.Вставить("Валюта", "ВалютаРасчетов");
	
	Для Каждого Свойство Из СписокСвойств Цикл
		Если Свойство.Значение = ИмяРеквизита Тогда
			ЭлементСпискаСвойств = Форма.ШаблоныНаименованийНастройки.СписокСвойств.НайтиПоЗначению(Свойство.Ключ);
			Если ЭлементСпискаСвойств <> Неопределено Тогда
				СвязанныеРеквизиты.Добавить(ЭлементСпискаСвойств);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ФорматСвойстваОбъекта(ИмяСвойства)
	
	ФорматыСвойств = Новый Соответствие;
	ФорматыСвойств.Вставить("ДатаНачала", "ДФ=dd.MM.yyyy");
	ФорматыСвойств.Вставить("ДатаОкончания", "ДФ=dd.MM.yyyy");
	ФорматыСвойств.Вставить("ДатаНачалаСокр", "ДФ=dd.MM.yy");
	ФорматыСвойств.Вставить("ДатаОкончанияСокр", "ДФ=dd.MM.yy");
	
	Возврат ФорматыСвойств.Получить(ИмяСвойства);
...
Процедура ОбновитьНаименование(Форма, Наименование, ВидНаименования) Экспорт
	...
	Для каждого Шаблон Из Форма.ШаблоныНаименованийКэш Цикл
		...
		Если ВидНаименования = ПредопределенноеЗначение("Перечисление.ВидыНаименованийОбъектов.ХарактеристикаДляПечати") Тогда
			СписокСвойств = СписокСвойствСПолнымНаименованием(Форма);
		Иначе
...
Функция СписокСвойствСПолнымНаименованием(Форма)
	
	СписокСвойств = Форма.ШаблоныНаименованийНастройки.СписокСвойств; 
	КопияСписка = СписокСвойств.Скопировать();
	...
	Для каждого Свойство Из Форма.Свойства_ТаблицаСвойстваИЗначения Цикл
		...
		ЗначениеСписка = КопияСписка.НайтиПоЗначению(НаименованиеСвойства);
		Если ЗначениеСписка <> Неопределено И ТипЗнч(ЗначениеСвойства) = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
			ПолноеНаименование = УправлениеНебольшойФирмойВызовСервера.ЗначениеРеквизитаОбъекта(ЗначениеСвойства, "ПолноеНаименование");
			Если НЕ ПустаяСтрока(ПолноеНаименование) Тогда
				ЗначениеСписка.Представление = ПолноеНаименование; 
	...
	Если ЕстьИзменения Тогда
		Результат = КопияСписка;
	Иначе
		Результат = СписокСвойств;
	КонецЕсли;		
