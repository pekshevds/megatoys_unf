...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	// Игнорировать отбор видимости для предопределенных вариантов
	ОтображатьВсеПредопределенные = ?(ПараметрыПоиска.Свойство("ОтображатьВсеПредопределенные"), ПараметрыПоиска.ОтображатьВсеПредопределенные, Ложь);
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Запрос.УстановитьПараметр("ОтображатьВсеПредопределенные", ОтображатьВсеПредопределенные);
...
Функция ТекстЗапросаОтчетовСУточненнымиОтборами()
	...
	|		ИЛИ (&ОтображатьВсеПредопределенные И РазмещениеВариантовОтчетов.Пользовательский = ЛОЖЬ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
