...
Функция СформироватьСтрокуИсточникОтчета(ИмяФормы, ИмяМакетаАтрибутов) Экспорт
	...
	Возврат (ДопАтрибуты.OKUD + "_" + ДопАтрибуты.code + "_" + ДопАтрибуты.version);
...
Процедура ЗагрузитьСправочникиШаблонаВРегистр(ИмяФормы, ИсточникОтчета, ИмяМакетаШаблона) Экспорт
	...
	Для Каждого УзелСправочник Из СправочникиШаблона Цикл
		ЗагрузитьСправочникВРегистр(ИсточникОтчета, УзелСправочник);
	КонецЦикла;
...
Процедура ЗагрузитьСправочникВРегистр(ИсточникОтчета, УзелСправочник)
	...
	Для Каждого УзелЭлементСправочника Из УзлыЭлементыСправочника Цикл
		...
		НовЗапись = РегистрыСведений.СправочникиШаблоновОтчетовСтатистики.СоздатьМенеджерЗаписи();
...
Функция СуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС, КПП = "") Экспорт	
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	КОЛИЧЕСТВО(РегламентированныйОтчет.Ссылка) КАК Ссылка
	               |ИЗ
	               |	Документ.РегламентированныйОтчет КАК РегламентированныйОтчет
	               |ГДЕ
