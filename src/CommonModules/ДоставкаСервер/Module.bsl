...
Процедура ЗаполнитьДеревоПараметровРасчета(ПараметрыРасчетов, ТолькоЧисловые = Ложь) Экспорт
	...
	СтрокаРодитель.Синоним = НСтр("ru = 'Заказ'");
	...
	ДобавитьВложенныеРеквизиты(СтрокаРодитель, Метаданные.Документы.ЗаказПокупателя,
		НаборСвойств_Документ_ЗаказПокупателя, ИменаРеквизитов, ТолькоЧисловые);
	СтрокаРодитель = ПараметрыРасчетов.ПолучитьЭлементы().Добавить();
	СтрокаРодитель.Идентификатор = НСтр("ru = 'Контрагент'");
	СтрокаРодитель.Синоним = НСтр("ru = 'Контрагент'");
	...
	ДобавитьВложенныеРеквизиты(СтрокаРодитель, Метаданные.Справочники.Контрагенты,
		НаборСвойств_Справочник_Контрагенты, ИменаРеквизитов, ТолькоЧисловые);
	...
	"ВЫБРАТЬ
	|	ПараметрыРасчетовДоставки.Ссылка КАК Параметр,
	|	ПараметрыРасчетовДоставки.Идентификатор КАК Идентификатор,
	|	ПараметрыРасчетовДоставки.Наименование КАК Наименование
	|ИЗ
	|	Справочник.ПараметрыРасчетовДоставки КАК ПараметрыРасчетовДоставки
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаПараметр.Синоним = Выборка.Наименование;
	КонецЦикла; 
...
Процедура ДобавитьВложенныеРеквизиты(СтрокаРодитель, МетаданныеОбъекта, НаборСвойств, ИменаРеквизитов, ТолькоЧисловые)
	...
	Для каждого Реквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		...
		Если ТолькоЧисловые И НЕ Реквизит.Тип.СодержитТип(Тип("Число")) Тогда
			Продолжить;
		КонецЕсли;
		ДанныеРеквизита = Новый Структура("Идентификатор,Синоним,Тип");
		ЗаполнитьЗначенияСвойств(ДанныеРеквизита, Реквизит);
		ДанныеРеквизита.Синоним = СтрокаРодитель.Идентификатор+"."+Реквизит.Синоним;
		ДанныеРеквизита.Идентификатор = СтрокаРодитель.Идентификатор+"."+Реквизит.Имя;
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеРеквизита = Новый Структура("Идентификатор,Синоним,Тип");
		ЗаполнитьЗначенияСвойств(ДанныеРеквизита, Реквизит);
		ДанныеРеквизита.Тип = Выборка.ТипЗначения;
		ДанныеРеквизита.Синоним = СтрокаРодитель.Идентификатор+"."+Выборка.Имя;
		ДанныеРеквизита.Идентификатор = СтрокаРодитель.Идентификатор + "." + Выборка.Имя;
		МассивРеквизитов.Добавить(ДанныеРеквизита);
	КонецЦикла;
	
	Для Каждого ЭлементСписка Из МассивРеквизитов Цикл
		СтрокаРеквизит = СтрокаРодитель.ПолучитьЭлементы().Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаРеквизит, ЭлементСписка);
	КонецЦикла; 
