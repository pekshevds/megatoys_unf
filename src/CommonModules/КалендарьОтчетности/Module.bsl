...
Функция ЗарегистрироватьНовыеСобытияКалендаряОтчетностиКИсполнению(Организация = Неопределено) Экспорт
	...
	|	СобытияКРегистрацииПоОрганизациям.ЭтоДоходыМинусРасходы КАК ЭтоДоходыМинусРасходы
	|ИЗ
	|	(ВЫБРАТЬ
	|		КалендарьПодготовкиОтчетности.СобытиеКалендаря КАК СобытиеКалендаря,
	|		ОрганизацииАбонента.Ссылка КАК Организация,
	|		СистемыНалогообложенияОрганизаций.ОбъектНалогообложения = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовНалогообложения.ДоходыМинусРасходы) КАК ЭтоДоходыМинусРасходы
	|	ИЗ
	...
	|	И СобытияКРегистрацииПоОрганизациям.СобытиеКалендаря.Код = """"
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура РассчитатьСуммыНалоговПоДаннымИБ() Экспорт
	...
	Пока ЗадачиОплатаТорговыйСбор.Следующий() Цикл
		РегламентированнаяОтчетностьСотрудники.ВыполнитьРасчетВзносовПоСотрудникамПредварительный(ЗадачиОплатаНалогиСотрудников.Организация, ЗадачиОплатаНалогиСотрудников.ДатаОкончанияСобытия, ЗадачиОплатаНалогиСотрудников.ДатаДокументаОбработкиСобытия, ЗадачиОплатаНалогиСотрудников.Ссылка);
	КонецЦикла;
