...
Процедура ДобавитьПолучателейВАдреснуюКнигу(НастройкаИнтеграции, ДанныеАдреснойКниги, Ошибки) Экспорт
	...
	ОбработаноКонтактов = -1;
	РазмерОднойПорцииКонтактов = 15;
	ВсегоКонтактов = ДанныеАдреснойКниги.Получатели.Количество();
	ЕстьПорцииКонтактов = ВсегоКонтактов <> 0;
	
	Пока ЕстьПорцииКонтактов Цикл
		
		НачальныйИндексКонтакта = ОбработаноКонтактов + 1;
		КонечныйИндексКонтакта = НачальныйИндексКонтакта + РазмерОднойПорцииКонтактов;
		Если КонечныйИндексКонтакта > ВсегоКонтактов - 1 Тогда
			КонечныйИндексКонтакта = ВсегоКонтактов - 1;
		КонецЕсли;
		
		Если НачальныйИндексКонтакта > КонечныйИндексКонтакта Тогда
			Прервать;
		КонецЕсли;
		...
		Для ИндексКонтакта = НачальныйИндексКонтакта По КонечныйИндексКонтакта Цикл
			...
			ОбработаноКонтактов = ОбработаноКонтактов + 1;
		КонецЦикла;
...
Процедура ОбновитьПромокодыПолучателей(НастройкаИнтеграции, ДанныеАдреснойКниги, Ошибки) Экспорт
	Отказ = Ложь;
	
	НайтиСоздатьПользовательскиеПоляСервиса(НастройкаИнтеграции, Ошибки);
	ПользовательскиеПоля = МассовыеРассылкиИнтеграция.ПользовательскиеПоляАдреснойКниги();
	ПоляПромокода = ПользовательскиеПоля.НайтиСтроки(Новый Структура("Имя", "smallbiz_1c_promocode"));
	Если ПоляПромокода.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого ОписаниеПоля Из ПоляПромокода Цикл
		ПередаваемыеПоля.Добавить(ОписаниеПоля.Имя);
	КонецЦикла;
