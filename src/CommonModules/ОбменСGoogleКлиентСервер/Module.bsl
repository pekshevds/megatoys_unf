...
Функция АдресПеренаправления(ИдентификацияПриложения) Экспорт
	...
	Если ТипЗнч(ИдентификацияПриложения.redirect_uris) = Тип("Массив")
		И ИдентификацияПриложения.redirect_uris.Количество() <> 0 Тогда
		АдресПеренаправления = ИдентификацияПриложения.redirect_uris[0];
		Если ЗначениеЗаполнено(АдресПеренаправления)
			И ЭтоСтандартныйАдресПеренаправленияСерверногоВебПриложения(АдресПеренаправления) Тогда
			АдресПеренаправления = Неопределено;
		КонецЕсли;
	Иначе
		АдресПеренаправления = Неопределено;
	КонецЕсли;
	...
	Возврат АдресПеренаправленияНастольногоПриложения();
...
Функция АдресПеренаправленияНастольногоПриложения()
	
	Возврат "http://127.0.0.1:23451";
	
КонецФункции

Функция ЭтоСтандартныйАдресПеренаправленияСерверногоВебПриложения(Знач АдресПеренаправления)
	
	АдресПеренаправления = НРег(АдресПеренаправления);
	Если СтрНайти(АдресПеренаправления, "localhost") Тогда
		Возврат Истина;
	КонецЕсли;
	Если СтрНайти(АдресПеренаправления, "127.0.0.1") Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
