...
Процедура ОтозватьСогласиеПриУвольнении(Источник) Экспорт
	...
	Если ТаблицаФизическихЛиц.Количество() = 0 Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("КэшТаблицаИзменившихсяДанных") Тогда
			...
			ТаблицаОтмененыхУвольнений = Новый ТаблицаЗначений;
			ТаблицаОтмененыхУвольнений.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
			ТаблицаОтмененыхУвольнений.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
			ТаблицаОтмененыхУвольнений.Колонки.Добавить("ДатаУвольнения", Новый ОписаниеТипов("Дата"));
			Для Каждого СтрокаИзменений Из Источник.ДополнительныеСвойства.КэшТаблицаИзменившихсяДанных Цикл
				Если СтрокаИзменений.СтароеЗначениеВидСобытия = Перечисления.ВидыКадровыхСобытий.Увольнение
					...
					НоваяСтрока = ТаблицаОтмененыхУвольнений.Добавить();
			...
			Запрос = Новый Запрос();
			Запрос.УстановитьПараметр("ТаблицаОтмененыхУвольнений", ТаблицаОтмененыхУвольнений);
			Запрос.Текст = "ВЫБРАТЬ
				...
				|ИЗ
				|	&ТаблицаОтмененыхУвольнений КАК Увольнение
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
...
Процедура ЗарегистрироватьДокументыКЭДОКПересчетуСостояний(ДокументыКЭДО)
	...
	Если РезультатыПроверки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПроблемы Из РезультатыПроверки Цикл
			...
			Если СтрокаПроблемы.ЕстьНеПроверенныеПодписи Тогда
				Проблема = МодульКонтрольВеденияУчета.ОписаниеПроблемы(СтрокаПроблемы.ДокументКЭДО, ПараметрыПроверки);
				Проблема.Ответственный = СтрокаПроблемы.Ответственный;
				Проблема.УточнениеПроблемы = НСтр("ru = 'Документ подписан не проверенными подписями.'");
				КонтрольВеденияУчетаБЗК.ЗаписатьПроблему(Проблема, ПараметрыПроверки);
			КонецЕсли;
...
Функция РезультатыПроверкиФорматОсновногоДокумента(Знач ДатаНачалаПроверки, Знач ПроверяемыеОбъекты, Знач ЛимитПроблем, РезультатыПроверки = Неопределено)
	...
	// Проверка того, что документы являются печатными формами
	ПечатныеФормы = ОтобратьФайлыПечатныхФорм(ЭлектронныеДокументы);
	СчетчикПроблем = 0;
	Для Каждого ПечатнаяФорма Из ПечатныеФормы Цикл
		СтруктураПоиска = Новый Структура("ЭлектронныйДокумент", ПечатнаяФорма);
		НайденныеСтроки = РезультатыПроверки.НайтиСтроки(СтруктураПоиска);
		Если НайденныеСтроки.Количество() > 0 Тогда
			НайденныеСтроки[0].ДокументНеЯвляетсяПечатнойФормой = Ложь;
...
Функция РезультатыПроверкиЧтоВсеСотрудникиОзнакомлены(Знач ДатаНачалаПроверки, Знач ПроверяемыеОбъекты, Знач ЛимитПроблем, РезультатыПроверки = Неопределено)
	...
	Запрос.Текст =
		...
		|			И (ПодписиДокументовКЭДО.РезультатСогласования = ЗНАЧЕНИЕ(Перечисление.РезультатыСогласованияБЗК.Согласовано))
		|ГДЕ
