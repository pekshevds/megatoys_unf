...
Процедура ИнтерактивноПодготовитьОтчетДляПросмотраНаЭкране(Знач ПараметрыЗадания, Знач АдресРезультата) Экспорт
	
	ОтчетТабДок = ОтчетКакТабличныйДокумент(ПараметрыЗадания);
	ПоместитьВоВременноеХранилище(Новый ХранилищеЗначения(ОтчетТабДок, Новый СжатиеДанных), АдресРезультата);
	
КонецПроцедуры

Процедура ИнтерактивноОтправитьОтчетПоЭлектроннойПочте(Знач ПараметрыЗадания, Знач АдресРезультата) Экспорт
	
	// Данный вызов используется для асинхронного вызова из формы настройки отчета "Состояние компании".
	// При отправке отчета по почте во временное хранилище ничего помещать не требуется.
	ОтправитьОтчетПоЭлектроннойПочте(ПараметрыЗадания);
...
Процедура ОтправитьОтчетПоЭлектроннойПочте(Знач ПараметрыЗадания)
	...
	Формат(ПараметрыЗадания.ПериодОтчета, "Л=ru; ДЛФ=DD")));
...
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - ссылка на элемент справочника учетные записи
//                                                                   электронной почты,
...
Функция ОтчетКакТабличныйДокумент(Знач ПараметрыЗадания)
	...
	ОбластьЗаголовокОтчета.Параметры.ПериодОтчета = Формат(ПараметрыЗадания.ПериодОтчета, "Л=ru; ДЛФ=DD");
...
Процедура ЗаполнитьСодержимоеСекции(Знач НастройкаСекции, Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач ОписанияВсехСекций)
	...
	Параметры.Добавить(ТабличныйДокумент);
	Параметры.Добавить(Организация);
	Параметры.Добавить(ПериодОтчета);
	Параметры.Добавить(НастройкаСекции);
	
	ОбщегоНазначения.ВыполнитьМетодКонфигурации(
	СтрШаблон("СостояниеКомпании.Подключаемый_Секция%1", НастройкаСекции.ТипСекции),
	Параметры);
...
Процедура Подключаемый_СекцияЧистыеАктивы(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
// Возвращает описание статей баланса в виде структуры, где:
//   * Ключ - наименование статьи баланса,
...
Функция СтатьиБаланса(Знач Счет, Знач ТипСчета)
	...
	ИначеЕсли ТипСчета = Перечисления.ТипыСчетов.ВнеоборотныеАктивы Тогда
		
		Результат.Вставить("Активы", 1);
		Результат.Вставить("Имущество", 1);
		
	ИначеЕсли ТипСчета = Перечисления.ТипыСчетов.АмортизацияВнеоборотныхАктивов Тогда
	...
	ИначеЕсли ТипСчета = Перечисления.ТипыСчетов.КредитыИЗаймы Тогда
		...
		Если Счет = ПланыСчетов.Управленческий.РасчетыПоПредоставленнымЗаймам Тогда
		
			Результат.Вставить("Активы", 1);
			Результат.Вставить("РасчетыПоПредоставленнымЗаймам", 1);
		
		КонецЕсли;
		
		Если Счет = ПланыСчетов.Управленческий.РасчетыПоДолгосрочнымКредитамИЗаймам
			ИЛИ Счет = ПланыСчетов.Управленческий.РасчетыПоКраткосрочнымКредитамИЗаймам Тогда
		
			Результат.Вставить("Обязательства", -1);
			Результат.Вставить("РасчетыПоКредитам", -1);
...
Функция НовыеПараметрыСекцииЧистыеАктивы(Знач ПериодОтчета)
	...
	Результат.Вставить("РасчетыПоПредоставленнымЗаймам", 0);
	Результат.Вставить("РасчетыПоПредоставленнымЗаймамКНачалуМесяца", 0);
	Результат.Вставить("РасчетыПоПредоставленнымЗаймамКНачалуГода", 0);
	...
	Результат.Вставить("РасчетыПоКредитам", 0);
	Результат.Вставить("РасчетыПоКредитамКНачалуМесяца", 0);
	Результат.Вставить("РасчетыПоКредитамКНачалуГода", 0);
...
Процедура Подключаемый_СекцияДеньги(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияТовары(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияТоварыНаСобственныхСкладах(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияТоварыУРеализаторов(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура ВывестиТоварыПоМестамХраненияРекурсивно(ТабличныйДокумент, МакетТовары, ОбластьШапка, ОбластьКатегория, ОбластьСтрока, СводнаяТаблица, КолонкиПоМестамХранения, МестаХраненияПоКолонкам, ИндексКолонки)
	...
	ОбластьОтступ = МакетТовары.ПолучитьОбласть("КолонкаМестоХранения|СтрокаОтступ");
	...
	Для ТекИндексКолонки = ИндексКолонки По ИндексКолонки + КоличествоКолонокНаСтранице() - 1 Цикл
		...
		ТабличныйДокумент.Присоединить(ОбластьОтступ);
...
Процедура Подключаемый_СекцияДебиторкаДолгиПокупателей(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияДебиторкаАвансыПоставщикам(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияКредиторкаДолгиПоставщикам(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияКредиторкаАвансыОтПокупателей(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияПродажиВыручка(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияПродажиКоличество(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияПродажиДинамика(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
...
Процедура Подключаемый_СекцияПродажиПокупатели(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
	...
	Если ПокупателиОстаток > 0 Тогда
		...
		ОбластьОстальные.Параметры.Остальные = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(";%1 покупатель;;%1 покупателя;%1 покупателей;%1 покупателя", ПокупателиОстаток);
...
Процедура Подключаемый_СекцияПродажиСамыеПродаваемыеТовары(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции) Экспорт
