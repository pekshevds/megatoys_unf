...
Процедура ДобавитьКомандыПечати(НазначениеШаблона, КомандыПечати, СписокФорм, МестоРазмещения, Порядок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КомандаПечати.ФункциональныеОпции = "ИспользоватьШаблоныПечатиОфисныхДокументов";
	КонецЦикла;
...
Процедура ОпределитьВидимостьКомандШаблоновПечати(КомандаОткрытьСписок) Экспорт
	...
	ИспользуютсяШаблоныПечати = Константы.ИспользоватьШаблоныПечатиОфисныхДокументов.Получить();
	КомандаОткрытьСписок.Видимость = ПравоДоступа("Просмотр", Метаданные.Справочники.ШаблоныПечатиОфисныхДокументов) 
		И ИспользуютсяШаблоныПечати;
...
Функция ДопустимыеФорматыДаты() Экспорт
	...
	ДопустимыеФорматы.Добавить("ДЛФ=ДД");
	ДопустимыеФорматы.Добавить("ДЛФ=Д");
	
	ТекущаяДатаСеанса = ТекущаяДатаСеанса();
	Для каждого КлючЗначение Из ДопустимыеФорматы Цикл
		ПримерДатыПоФормату = Формат(ТекущаяДатаСеанса, КлючЗначение.Значение);
		КлючЗначение.Представление = ПримерДатыПоФормату;
	КонецЦикла;
...
Функция ДопустимыеФорматыДатыВремени() Экспорт
	...
	ДопустимыеФорматы = Новый СписокЗначений;
	ДопустимыеФорматы.Добавить("ДЛФ=ДДВ");
	ДопустимыеФорматы.Добавить("ДЛФ=ДД");
	ДопустимыеФорматы.Добавить("ДЛФ=Д");
	ДопустимыеФорматы.Добавить("ДЛФ=В");
	...
	Для каждого ОписаниеПараметра Из ПараметрыДопРеквизитов Цикл
		...
		Для каждого СтрДопРеквизит Из ЗначенияСвойств Цикл
			Если СтрСравнить(СтрДопРеквизит.Свойство.Наименование, ИмяДопРеквизита) = 0 Тогда
				Если ТипЗнч(СтрДопРеквизит.Значение) = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
					ЗначенияСвойстваДопРеквизита = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрДопРеквизит.Значение, "Наименование,ПолноеНаименование");
					Если Не ПустаяСтрока(ЗначенияСвойстваДопРеквизита.ПолноеНаименование) Тогда
						ЗначениеДляПечати = ЗначенияСвойстваДопРеквизита.ПолноеНаименование;
					Иначе
						ЗначениеДляПечати = ЗначенияСвойстваДопРеквизита.Наименование;
					КонецЕсли;
				Иначе
					ЗначениеДляПечати = СтрДопРеквизит.Значение;
				КонецЕсли;
				ОписаниеПараметра.Значение = ЗначениеДляПечати;
...
Процедура ЗакончитьВыводТаблицыДокумента(ОписаниеПараметров, ПараметрыВывода) Экспорт
	...
	Если НЕ ПараметрыВывода.Свойство("ВыводТаблицы") Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru='Ошибка формирования: В шаблоне ""%1"" для таблицы ""%2"" отсутствует строка с заголовками колонок.'"),
...
Процедура ПриДобавленииПараметровШаблона(ОписаниеПараметров)
	...
	ДобавитьПараметрШаблона(ПараметрыОбщие, "ТекущаяДата", НСтр("ru='Дата формирования'"), "ДФ=ДДВ", ДопустимыеФорматыДаты(), Новый ОписаниеТипов("Дата"));
...
Функция ПолучитьАтрибутыИзображения(ДанныеЧтения)
	...
	Иначе
		...
		Если((Символ(Символ1) = "M" И Символ(Символ2) = "M" И Символ3 = 0 И Символ4 = 42) ИЛИ (Символ(Символ1) = "I" И Символ(Символ2) = "I" И Символ3 = 42 И Символ4 = 0)) Тогда //TIFF
			...
			// Image header
			СмещениеImageFileDirectory = 0;
			СмещениеImageFileDirectory = ПрочитатьЗначениеБайтаИзПотока(ЧтениеДанных, 4, ПорядокБайтовBigEndian);
			
			ЧтениеДанных.Пропустить(СмещениеImageFileDirectory - 8);
	...
	АтрибутыИзображения.Вставить("ТипИзображения", ОбъектMimeType);
	АтрибутыИзображения.Вставить("Высота", ?(ОбъектMimeType = Null, 0, Высота));
	АтрибутыИзображения.Вставить("Ширина", ?(ОбъектMimeType = Null, 0, Ширина));
...
Функция НовыйРазмерКартинки(ШиринаРамки, ВысотаРамки, ШиринаКартинки, ВысотаКартинки)
	...
	Иначе
		НоваяВысота = ВысотаРамки;
		НоваяШирина = НоваяВысота * ШиринаКартинки / ВысотаКартинки;
	КонецЕсли;
...
Функция НовыйПараметрыВывода(ЗаписьXML, СтруктураДокумента)
	...
	ПараметрыВывода.Вставить("Отказ", Ложь);
...
Функция ПараметрыПоИмениТаблицы(СписокПараметров, ИмяТаблицы)
	
	Результат = Новый Массив;
	Для каждого ОписаниеПараметра Из СписокПараметров Цикл
		Если ОписаниеПараметра.ТаблицаВладелец = ИмяТаблицы Тогда
...
Процедура НайтиОписаниеПараметровРекурсивно(ОписаниеПараметров, ПрефиксКлючПараметра, НайденныеОписания)
...
Функция ИмяПараметраВидКонтактнойИнформации(ВидКонтактнойИнформацииНаименование)
...
Процедура ПроверитьВложенностьТаблицДокументаТабличныхЧастей(ПараметрыЧтенияXML, ПараметрыВывода)
	
	Если ПараметрыЧтенияXML.Таблица.ОткрытоЧтениеВложенной И ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц > 1 И ПараметрыЧтенияXML.Таблица.СодержитДанныеТЧ Тогда
		ТекстШаблона = НСтр("ru='Ошибка формирования: В шаблоне ""%1"" в таблицу ""%2"" вложена другая таблица документа.'");
	ИначеЕсли ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц > 1 И ПараметрыЧтенияXML.Таблица.СодержитДанныеТЧ Тогда
		ТекстШаблона = НСтр("ru='Ошибка формирования: В шаблоне ""%1"" таблица ""%2"" вложена в другую таблицу документа.'");
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекстОшибки = СтрШаблон(ТекстШаблона, ПараметрыВывода.СтруктураДокумента.Шаблон, ПараметрыЧтенияXML.Таблица.ТаблицаВладелец);
	ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ПараметрыВывода.Ошибки,, ТекстОшибки, Неопределено);
	ПараметрыВывода.Отказ = Истина;
...
Функция НовыйПараметрыЧтенияXMLДокументаDOCX()
	...
	ПараметрыЧтенияТаблицы.Вставить("ОткрытоЧтениеВложенной", Ложь);
	...
	ПараметрыЧтенияТаблицы.Вставить("ВложенностьТаблиц",      0);
	ПараметрыЧтенияТаблицы.Вставить("ТаблицаВладелец",        "");
...
Процедура ОбновитьСвойстваЧтенияXMLДокументаDOCX(ЧтениеXML, ПараметрыЧтенияXML)
	...
	КэшВложенностьТаблиц = ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц;
	...
	Если ПараметрыЧтенияXML.Таблица.ОткрытоЧтение Или КэшТаблицаПродолжаетсяЧтение И ЧтениеXML.Имя = "w:tbl" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц = ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц + 1;
	КонецЕсли;
	Если ПараметрыЧтенияXML.Таблица.ЗакрытоЧтение Или КэшТаблицаПродолжаетсяЧтение И ЧтениеXML.Имя = "w:tbl" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
		ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц = ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц - 1;
	КонецЕсли;
	
	ПараметрыЧтенияXML.Таблица.ОткрытоЧтениеВложенной = ПараметрыЧтенияXML.Таблица.ВложенностьТаблиц > КэшВложенностьТаблиц;
...
Процедура ПоместитьВБуферТаблицыСтрокиДокументаDOCX(СтруктураДокумента, ПараметрыВывода, ОписаниеПараметров)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ПараметрыВывода.Отказ Тогда
			Возврат;
		КонецЕсли;
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ОбновитьСвойстваЧтенияXMLДокументаDOCX(ЧтениеXML, ПараметрыЧтенияXML);
			ПроверитьВложенностьТаблицДокументаТабличныхЧастей(ПараметрыЧтенияXML, ПараметрыВывода);
			...
			Если ПараметрыЧтенияXML.Таблица.ПродолжаетсяЧтение И ЗначениеЗаполнено(ПараметрыЧтенияXML.ИмяПараметра) Тогда
				...
				Если ОписаниеПараметра <> Неопределено И ОписаниеПараметра.ТаблицаВладелец <> Неопределено Тогда
					ПараметрыЧтенияXML.Таблица.СодержитДанныеТЧ = Истина;
					ПараметрыЧтенияXML.Таблица.ТаблицаВладелец = ОписаниеПараметра.ТаблицаВладелец;
					ПроверитьВложенностьТаблицДокументаТабличныхЧастей(ПараметрыЧтенияXML, ПараметрыВывода);
...
Процедура ДобавитьВыбранныеПараметрыВШаблонDOCX(СтруктураДокумента, ОписаниеПараметров, ДобавляемыеПараметрыДокумента, ДобавляемыеПараметрыТаблицыДокумента, ДобавляемыеПараметрыКартинкиДокумента)
	...
	ОписаниеДобавляемыхПараметровТаблиц = Новый Массив;
	ИменаДобавляемыхТаблиц = Новый Массив;
	ИменаНедостающихТаблиц = Новый Массив;
	ИменаСуществующихТаблиц = Новый Массив;
	Для каждого СтрокаПараметра Из ДобавляемыеПараметрыТаблицыДокумента Цикл
		ОписаниеПараметра = ОписаниеПараметра(ОписаниеПараметров, СтрокаПараметра.Значение);
		ОписаниеДобавляемыхПараметровТаблиц.Добавить(ОписаниеПараметра);
		Если ИменаДобавляемыхТаблиц.Найти(ОписаниеПараметра.ТаблицаВладелец) = Неопределено Тогда
			ИменаДобавляемыхТаблиц.Добавить(ОписаниеПараметра.ТаблицаВладелец);
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого КлючЗначение Из ТаблицыСодержащиеДанныеТЧ Цикл
		ИменаСуществующихТаблиц.Добавить(КлючЗначение.Значение);
	КонецЦикла;
	ИменаНедостающихТаблиц = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ИменаДобавляемыхТаблиц, ИменаСуществующихТаблиц);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
				...
				Если ДобавляемыеПараметрыТаблицыДокумента.Количество() > 0 Тогда
					Если ПараметрыЧтенияXML.ЭлементТекущийИмя = "w:tr" И ЧтениеXML.Имя = "w:rsidR" Тогда
						АтрибутRsidR = ЧтениеXML.Значение;
					КонецЕсли;
				КонецЕсли;
			...
			НужноДобавитьНовуюТаблицуВНачалоДокумента = (ДобавляемыеПараметрыТаблицыДокумента.Количество() > 0
				ИЛИ ДобавляемыеПараметрыКартинкиДокумента.Количество() > 0)
				И ИменаНедостающихТаблиц.Количество() <> 0;
			
			Если НужноДобавитьНовуюТаблицуВНачалоДокумента Тогда
				Для каждого ИмяТаблицы Из ИменаНедостающихТаблиц Цикл
					ОтобранныеПараметрыПоИмениТаблицы = ПараметрыПоИмениТаблицы(ОписаниеДобавляемыхПараметровТаблиц, ИмяТаблицы);
					ТекстДобавления = КодДобавляемыхПараметровТаблицыДокументаDOCX(ОтобранныеПараметрыПоИмениТаблицы, АтрибутRsidR);
		...
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			...
			Если ЧтениеXML.Имя = "w:tr" Тогда
				ИмяТаблицы = ТаблицыСодержащиеДанныеТЧ.Получить(ПараметрыЧтенияXML.Таблица.ПорядковыйНомер);
				Если ИмяТаблицы <> Неопределено Тогда
					ОтобранныеПараметрыПоИмениТаблицы = ПараметрыПоИмениТаблицы(ОписаниеДобавляемыхПараметровТаблиц, ИмяТаблицы);
					ТекстДобавления = КодДобавляемыхПараметровТаблицыДокументаDOCX(ОтобранныеПараметрыПоИмениТаблицы, АтрибутRsidR);
...
Функция КодДобавляемыхПараметровТаблицыДокументаDOCX(ДобавляемыеПараметры, АтрибутRsidR)
	
	ТекстыДобавления = Новый Структура("ЯчейкаЗаголовок,ЯчейкаЗначение", "", "");
	
	Для каждого ОписаниеПараметра Из ДобавляемыеПараметры Цикл
		НовыйТекст = КодПараметраТаблицыДокументаDOCX(ОписаниеПараметра, АтрибутRsidR);
...
Функция КодПараметраТаблицыДокументаDOCX(ОписаниеПараметра, АтрибутRsidR)
	...
	ТекстДобавления.ЯчейкаЗаголовок = СтрШаблон(
		...
		|	</w:p>
		|</w:tc>",
		ПредставлениеПараметра, АтрибутRsidR);
	...
	Если ОписаниеПараметра.Вид = ВидПараметраТекст() Тогда
		ТекстДобавления.ЯчейкаЗначение = СтрШаблон(
			...
			|	</w:p>
			|</w:tc>",
			КлючПараметра, ПредставлениеПараметра, АтрибутRsidR);
	ИначеЕсли ОписаниеПараметра.Вид = ВидПараметраИзображение() Тогда
		ТекстДобавления.ЯчейкаЗначение = СтрШаблон(
			...
			|	</w:p>
			|</w:tc>",
			АтрибутRsidR, КлючПараметра, ОписаниеПараметра.ПараметрыКартинки.rID);
	КонецЕсли;
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если ПораВставлятьОписаниеПараметров Тогда
				...
				Если ЧтениеXMLИмя <> "text:variable-decls" Тогда
					...
					КонецЕсли;
					Если ДобавляемыеПараметрыТаблицыДокумента.Количество() <> 0 Тогда
						Для каждого ИмяТаблицы Из ИменаДобавляемыхТаблиц Цикл
							ОтобранныеПараметрыПоИмениТаблицы = ПараметрыПоИмениТаблицы(ОписаниеДобавляемыхПараметровТаблиц, ИмяТаблицы);
							ТекстДобавления = КодДобавляемыхПараметровТаблицыДокументаODT(ОтобранныеПараметрыПоИмениТаблицы);
...
Функция КодДобавляемыхПараметровТаблицыДокументаODT(ДобавляемыеПараметры)
	...
	Для каждого Параметр Из ДобавляемыеПараметры Цикл
		НовыйТекст = КодПараметраТаблицыДокументаODT(Параметр.Ключ, Параметр.Представление);
		ТекстыДобавления.ЯчейкаЗаголовок = ТекстыДобавления.ЯчейкаЗаголовок + НовыйТекст.ЯчейкаЗаголовок;
		ТекстыДобавления.ЯчейкаЗначение  = ТекстыДобавления.ЯчейкаЗначение + НовыйТекст.ЯчейкаЗначение;
	КонецЦикла;
