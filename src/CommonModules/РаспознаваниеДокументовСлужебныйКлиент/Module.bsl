...
Функция КонтекстСозданияРаспознанногоДокумента(Форма, ПараметрыОперации)
	...
	////
	ЭтоСчетНаОплату = ПараметрыОперации.ТипДокумента = РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("СчетНаОплатуПоставщикаБРД")
		Или ПараметрыОперации.ТипДокумента = РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("СчетНаОплатуПокупателюБРД");
	...
	ТребуетсяПересчетСкидкиВЦену =Ложь;
	РаспознаваниеДокументовКлиентПереопределяемый.ТребуетсяПересчетСкидкиВЦену(ЭтоСчетНаОплату, Форма, ТребуетсяПересчетСкидкиВЦену);
...
Процедура ЗадатьВопросУстановкаФункциональныхОпцийПоСкидкам(Контекст)
	...
	ОткрытьФорму("Обработка.СчетНаОплатуРаспознаваниеДокументов.Форма.ВопросУстановкаФункциональныхОпцийПоСкидкам", ПараметрыФормы, Контекст.Форма, , , , ОповещениеОЗавершении);
...
Процедура ПриПолученииРезультатаОВключенииСкидок(Результат, Контекст) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если Контекст.Форма.Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияРаспознанногоДокумента.Входящий") Тогда
