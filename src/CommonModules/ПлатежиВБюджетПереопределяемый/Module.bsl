...
Функция КонтекстПлатежногоДокумента(ИсточникДанных) Экспорт
	...
	Контекст.НазначениеПлатежа = Справочники.ВидыНалогов.НазначениеПлатежа(
	...
	Иначе
		Контекст.КБК = Справочники.ВидыНалогов.КБК(
			ИсточникДанных.Налог,
			ИсточникДанных.ВидНалоговогоОбязательства,
			Контекст.Период,
	...
	Иначе
		Контекст.ФизическоеЛицо = Не ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоЮрЛицо(ИсточникДанных.Организация);
	КонецЕсли;
	...
	Если ПравилаЗаполнения.ЗаполнятьКодТерриторииПоРегистрации Тогда
		РегистрацияВНалоговомОргане = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникДанных.Организация, "РегистрацияВНалоговомОргане");
		Контекст.КодТерриторииПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РегистрацияВНалоговомОргане, "КодПоОКТМО");
		Если ИсточникДанных.Свойство("РегистрацияВНалоговомОргане") И ЗначениеЗаполнено(ИсточникДанных.РегистрацияВНалоговомОргане) Тогда
			Контекст.КодТерритории = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникДанных.РегистрацияВНалоговомОргане, "КодПоОКТМО");
		Иначе
...
Функция ДублиУИН(ИдентификаторПлатежа, Ссылка) Экспорт
	...
	Запрос.УстановитьПараметр("СтатусПлательщикаПоИспЛисту",
		ПлатежиВБюджетКлиентСервер.СтатусПлательщикаПоИсполнительномуЛистуВБюджет());
