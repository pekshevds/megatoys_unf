...
Процедура ЗаполнитьСлужебныеРеквизиты(Форма, ИмяКолонкиКоличество = "Количество", ИмяТабличнойЧасти = "Товары") Экспорт
	...
	КолонкаКоличество = АкцизныеМарки.Колонки.Найти("Количество");
	Если КолонкаКоличество = Неопределено Тогда
		...
		КонецЦикла;
	КонецЕсли;
	КолонкаСправка2 = АкцизныеМарки.Колонки.Найти("Справка2");
	Если КолонкаСправка2 = Неопределено Тогда
	...
	Для Каждого Выборка Из АлкогольнаяПродукция Цикл
		...
		СтрокаТЧ = Источник[ИмяТабличнойЧасти][Выборка.НомерСтроки - 1 + ДополнениеКИндексу];
