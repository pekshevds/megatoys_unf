...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	ПараметрыИПП             = Неопределено;	// ФиксированнаяСтруктура
	ПараметрыОблачногоАрхива = Неопределено;	// ФиксированнаяСтруктура
	НастройкиКопирования     = Неопределено;	// см. ОблачныйАрхив20.НовыйНастройкаРучногоРезервногоКопирования
	
	Если Не ПараметрыРаботыКлиента.Свойство("ИнтернетПоддержкаПользователей", ПараметрыИПП)
		Или Не ПараметрыИПП.Свойство("ОблачныйАрхив", ПараметрыОблачногоАрхива) Тогда
		
		Возврат;
		
	КонецЕсли;
	
	ПараметрыОблачногоАрхива = ОписаниеПараметровОблачногоАрхиваПриЗапуске(ПараметрыОблачногоАрхива);
	Если ПараметрыОблачногоАрхива.ОблачныйАрхивПодключен Тогда
		
		// Сценарий "Перемещение информационной базы"
		Если ПараметрыОблачногоАрхива.Свойство("ПоказатьФормуВосстановленияРаботыОблачногоАрхива") Тогда
		...
		Если ПараметрыОблачногоАрхива.Свойство("ПоказатьФормуСтатусаРучногоРезервногоКопирования", НастройкиКопирования) Тогда
			
			ПараметрыФормы = Новый Структура();
			ПараметрыФормы.Вставить("НастройкиКопирования"     , НастройкиКопирования);
...
// Возвращает описание структуры обязательных параметров облачного архива, возвращаемые методом
// СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске.
//
// Параметры:
//  Параметры - ФиксированнаяСтруктура:
//    * ОблачныйАрхивПодключен - Булево
//    * АвтоматическоеАрхивирование - см. ОблачныйАрхив20.ОбновитьВремяДоАвтоматическогоРезервногоКопированияВФоне
//
// Возвращаемое значение:
...
//    * АвтоматическоеАрхивирование - см. ОблачныйАрхив20.ОбновитьВремяДоАвтоматическогоРезервногоКопированияВФоне
//
Функция ОписаниеПараметровОблачногоАрхиваПриЗапуске(Параметры)
...
Процедура ПоказатьВопросОЗавершенииРаботы(ОписаниеОповещенияОЗавершении, ПараметрыВопроса)
	...
	Оповестить(
		ИмяСобытияОповещенияОбновитДиалогВопросОЗавершенииРаботы(),
		ПараметрыВопроса);
...
//  Строка
//
Функция ИмяСобытияОповещенияОбновитДиалогВопросОЗавершенииРаботы() Экспорт
