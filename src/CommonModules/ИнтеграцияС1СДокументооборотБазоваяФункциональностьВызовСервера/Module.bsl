...
Процедура ЗаполнитьСтрокуДереваПодписей(НоваяСтрока, НомерСтроки, Подпись, ДанныеВладельцаПодписи,
	...
	ФорматнаяСтрока = НСтр("ru = 'ДФ=""дд.ММ.гггг ЧЧ:мм""'"); //@skip-check module-nstr-camelcase
...
Функция ПрисоединитьПечатнуюФормуОбъектаКДокументу(ВнешнийОбъект, ID, Тип, Представление, ИмяКоманды,
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ПечатнаяФорма.ТабличныйДокумент <> Неопределено
			...
			ДатаНомер = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВнешнийОбъект, "Дата, Номер");
