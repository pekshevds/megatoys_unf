...
//
Функция РассчитатьПотенциальноВозможныйДоход(ПотенциальноВозможныйГодовойДоход, ДатаНачала, ДатаОкончания, НалоговыйВычет = Ложь) Экспорт
	...
	СуммаВычета = 0;
	//// Проверяем налоговый вычет
	Если НалоговыйВычет Тогда
		НачалоВторогоКвартала = Дата('20200401');
		ОкончаниеВторогоКвартала = Дата('20200630');
		Если ДатаНачала < НачалоВторогоКвартала Тогда
			НачальнаяДатаВычета = НачалоВторогоКвартала;
		ИначеЕсли ДатаНачала <= ОкончаниеВторогоКвартала Тогда
			НачальнаяДатаВычета = ДатаНачала;
		Иначе
			НачальнаяДатаВычета = Неопределено;
		КонецЕсли;
		
		Если ДатаОкончания > ОкончаниеВторогоКвартала Тогда
			КонечнаяДатаВычета = ОкончаниеВторогоКвартала;
		ИначеЕсли ДатаОкончания >= НачалоВторогоКвартала Тогда
			КонечнаяДатаВычета = ДатаОкончания;
		Иначе
			КонечнаяДатаВычета = Неопределено;
		КонецЕсли;
		Если НачальнаяДатаВычета <> Неопределено
			И КонечнаяДатаВычета <> Неопределено Тогда
			КоличествоДнейВычета = (КонечнаяДатаВычета - НачальнаяДатаВычета)/ОдинДень +1;
			СуммаВычета = ПотенциальноВозможныйГодовойДоход / 366 * КоличествоДнейВычета;
		КонецЕсли;
	КонецЕсли;
	
	
	Возврат Окр(ПотенциальноВозможныйДоход - СуммаВычета, 2, РежимОкругления.Окр15как20);
