...
Функция ПолучитьСписокХОзОперацийРасходДС() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетРасчетовСБанкомЭквайрером") Тогда
		...
		СписокОпераций.Добавить(Справочники.ХозяйственныеОперации.ВозвратОплатыНаПлатежныеКарты, Представление); // Эквайринг
	КонецЕсли;
...
Функция ПолучитьОстатокНаСчете(БанковскийСчет, Организация) Экспорт
	...
	Запрос.Текст = 
		...
		|				И БанковскийСчетКасса = &БанковскийСчет) КАК ДенежныеСредстваОстатки";
...
// Получает значение реквизита "СуммаДокумента" переданной ссылки на документ
//
// Параметры:
//  Объект	 - ДокументСсылка, ДокументОбъект, ДанныеФормыСтруктура - Объект, представляющий документ, или содержащий реквизит-документ, сумму которого необходимо получить
//	ИспользоватьРеквизит - Строка - Не обязательный. Имя реквизита объекта, содержащего ссылку на документ. Документ должен содержать реквизит "СуммаДокумента"
// 
// Возвращаемое значение:
// 		Число - сумма документа. Если на Приходную накладную уже выбивался чек на возврат, возвращен будет 0 (т.е. контроля суммы не ведется)
//
Функция ПолучитьСуммуПоДокументу(Объект, ИспользоватьРеквизит = "") Экспорт
	Результат = 0;
	ПоследовательностьРеквизитов = Новый Массив;
	Ссылка = Объект;
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
		Если НЕ ИспользоватьРеквизит = "" Тогда
			ПоследовательностьРеквизитов.Добавить(ИспользоватьРеквизит);
		КонецЕсли;
	Иначе
		Если ИспользоватьРеквизит = "" Тогда
			Возврат Объект[ИспользоватьРеквизит];
		Иначе
			Ссылка = Объект[ИспользоватьРеквизит];
		КонецЕсли;
	КонецЕсли;
	ПоследовательностьРеквизитов.Добавить("СуммаДокумента");
	ПолноеИмяРеквизита = СтрСоединить(ПоследовательностьРеквизитов, ".");
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Если на товарный документ уже выбивали чек возврата, то все последующие чеки на выплату денег должны быть с ПСР
		// "Оплата кредита".
		Результат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, ПолноеИмяРеквизита);
	КонецЕсли;
	Возврат Результат;
