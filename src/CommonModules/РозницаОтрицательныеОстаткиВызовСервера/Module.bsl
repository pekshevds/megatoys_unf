...
Функция ЕстьОтрицательныеОстатки(СсылкаНаДокумент, Организация, СтруктурнаяЕдиница) Экспорт
	...
	ЕстьПравоНаПолучениеОстатков = ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗапасыНаСкладах);
	Если НЕ ЕстьПравоНаПолучениеОстатков Тогда
		Возврат "";
	КонецЕсли;
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("МассивОрганизаций", ИнтеркампаниСервер.ОрганизацииОстатков(Организация));
