...
//
// Параметры:
//  ОбъектНастройки - КомпоновщикНастроекКомпоновкиДанных, НастройкиКомпоновкиДанных  - откуда брать настройки.
//
// Возвращаемое значение:
//   Булево - признак использования отбора по сегменту.
//
Функция ВключитьОтборПоСегментуНоменклатурыВСКД(ОбъектНастройки) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектНастройки) = Тип("НастройкиКомпоновкиДанных") Тогда
		НастройкиОсновнойСхемы = ОбъектНастройки;
	КонецЕсли;
...
Процедура СформироватьСоставыОбновляемыхСегментовКонтрагентов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ИмяСобытия = НСтр("ru='Ошибка при формировании состава сегмента контрагентов в регламентном задании'", 
