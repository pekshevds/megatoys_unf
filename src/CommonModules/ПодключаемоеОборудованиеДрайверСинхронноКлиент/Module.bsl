...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, ПараметрыПодключения, Команда, ДанныеОперации) Экспорт;
	...
	ИначеЕсли Команда = "DisplayStatusRM" Тогда // Вывести статус рабочего места на дисплей покупателя
		ВывестиСтатусРабочегоМестаНаДисплейПокупателя(ПараметрыПодключения.ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, РезультатВыполнения);
...
Процедура НачатьПодключениеУстройства_Завершение(ОбъектДрайвера, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ТипОборудования = "ЭлектронныеВесы" Тогда
			ВремРезультатВыполнения = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании();
			ПараметрыВесов(ОбъектДрайвера, ПараметрыПодключения, Неопределено, ВремРезультатВыполнения); 
...
// Процедура  осуществляет вывод статуса рабочего места на дисплей покупателя.
//                 
Процедура ВывестиСтатусРабочегоМестаНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, РезультатВыполнения)
	
	Если ПараметрыПодключения.РевизияИнтерфейса < 4001  Тогда
		РезультатВыполнения.Результат = Ложь;                         
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Устройство не поддерживает данную операцию.'"); 
		Возврат;
	КонецЕсли;
	...
	Попытка                                                                                    
		Результат = ОбъектДрайвера.ОтобразитьСтатусРабочегоМеста(ПараметрыПодключения.ИДУстройства, СтатусРабочегоМеста, ЗвуковойСигнал);
		Если Результат Тогда
			РезультатВыполнения.Результат = Истина;
		Иначе
			ОписаниеОшибки = "";
	...
	Исключение
		РезультатВыполнения.Результат = Ложь;
		ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		РезультатВыполнения.ОписаниеОшибки = СтрШаблон(НСтр("ru='Ошибка вызова метода <%1>.'"), "ОбъектДрайвера.ОтобразитьСтатусРабочегоМеста") + Символы.ПС + ОписаниеОшибки;
	КонецПопытки;
...
// Процедура возвращает параметры весов.
//
Процедура ПараметрыВесов(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, РезультатВыполнения)
	
	Если ПараметрыПодключения.РевизияИнтерфейса >= 4001 Тогда
		Попытка         
			РезультатОперацииXML = "";
			Результат = ОбъектДрайвера.ПараметрыВесов(ПараметрыПодключения.ИДУстройства, РезультатОперацииXML);
			Если Результат Тогда
				ПараметрыВесов = ПрочитатьКорневойЭлементXML(РезультатОперацииXML);
		...
		Исключение      
			ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			РезультатВыполнения.ОписаниеОшибки = СтрШаблон(НСтр("ru='Ошибка вызова метода <%1>.'"), "ПараметрыВесов") + Символы.ПС + ОписаниеОшибки;
		КонецПопытки;
	Иначе              
		РезультатВыполнения.Результат = Истина;
...
Процедура Тарировать(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, РезультатВыполнения)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 4001 И НЕ ПараметрыПодключения.ВесыПоддерживаютТарирование Тогда
		РезультатВыполнения.Результат = Ложь;
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Оборудование не поддерживает данную операцию.'"); 
		Возврат;
	КонецЕсли;
