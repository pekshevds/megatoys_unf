...
Функция СозданыОтборНажатие(Форма, ИмяСписка, ВариантПериода, Элемент, Префикс = "") Экспорт
	...
	Для Индекс = 0 По Форма[Префикс + "ОтборСозданы"].Количество()-1 Цикл
		...
		Если ЭлементСпискаПериодов.Пометка Тогда
			ЭлементОтображенияПериода.ЦветФона = ОбщегоНазначенияКлиент.ЦветСтиля("ФонАктивногоЗначенияОтбора");
		Иначе
...
Процедура ВыборВариантаОтбора(Форма, Команда, Префикс) Экспорт
	...
	Иначе
		...
		Элементы[Команда.Имя].ЦветФона = ОбщегоНазначенияКлиент.ЦветСтиля("ФонАктивногоЗначенияОтбора");
