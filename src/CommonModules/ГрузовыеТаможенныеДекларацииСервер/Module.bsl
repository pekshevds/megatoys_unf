...
Процедура ПриСозданииНаСервереДляДокументаСПрослеживаемостью(ЭтаФорма, ИменаТЧ, КэшЗначений, ИмяТабличнойЧасти = "Запасы") Экспорт 
	...
	Если КэшЗначений.Свойство("РеализацияВЕАЭС") Тогда
		...
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, ИмяТабличнойЧасти + "КодТНВЭД"); 
	...
	Иначе
		...
		СтруктураИмен = Новый Структура("ИмяПоляПроверки, ИмяПоляОформления", "Объект." + ИмяТабличнойЧасти + ".СтранаПроисхождения", ИмяТабличнойЧасти + "НомерГТДПриРНПТ"); 
