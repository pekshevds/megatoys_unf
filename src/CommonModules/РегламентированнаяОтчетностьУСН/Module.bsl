...
Процедура ВыполнитьФормированиеПоДоходам(Организация, ПериодФормирования, Результат, МенеджерВТ) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""0. Поступление в кассу с видами операций, которые входят в массив"" КАК Комментарий,
	...
	|
	|ВЫБРАТЬ
	|	""2. Поступление на счет с видами операций, которые входят в массив"",
	|	ПоступлениеНаСчет.Ссылка,
	...
	|
	|ВЫБРАТЬ
	|	""3. Поступление на счет от курьерской компании или почты (фактическая сумма)"",
	...
	|
	|ВЫБРАТЬ
	|	""4. Поступление на счет от курьерской компании или почты (удержаная сумма)"",
	...
	|
	|ВЫБРАТЬ
	|	""5. Расход со счета с операциями из массива"",
	|	РасходСоСчета.Ссылка,
	...
	|
	|ВЫБРАТЬ
	|	""6. Расход из кассы с операциями из массива"",
	...
	|
	|ВЫБРАТЬ
	|	""7. Отчет комиссионера, в котором удержано вознаграждение"",
	...
	|
	|ВЫБРАТЬ
	|	""8. Отчет комитенту, в котором удержано вознаграждение"",
	...
	|
	|ВЫБРАТЬ
	|	""9. Взаимозачет с операцией Взаимозачет"",
	...
	|
	|ВЫБРАТЬ
	|	""10. Прочий взаимозачет между прочим контрагентом и поставщиком"",
	|	Взаимозачет.Ссылка,
...
Процедура ВыполнитьФормированиеПоПрочимРасходам(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Для Каждого Строка Из РезультатЗапроса Цикл
			...
			ПлательщикЕНП = ПлательщикЕНП(
			ДокументФормирования.Организация,
			ПериодФормирования);
				
			// Записи КУДиР по ЕНП формирует закрытие месяца
			Если Строка.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Налоги И ПлательщикЕНП Тогда
				Продолжить;
			КонецЕсли;
...
Функция ВыполнитьРасчетЕдиногоНалога(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено, ФиксироватьСтатусСобытия=Истина) Экспорт
	...
	КонецЕсли;  	
	СтруктураРасчетаЕдиногоНалога.НалоговыйВычетЕН = НалоговыйВычет;
	...
	ВсегоКУплатеЕН = Окр(Макс(0,СтруктураРасчетаЕдиногоНалога.ВсегоКУплатеЕН - СтруктураРасчетаЕдиногоНалога.СуммаАвансовыхПлатежейЗаГодЕН - НалоговыйВычет));
...
Функция ВыполнитьРасчетАвансовогоПлатежа(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если НЕ УчетВалютныхОпераций ИЛИ НациональнаяВалюта = ВалютаУчета Тогда
		...
		|						КалендарьПодготовкиОтчетности.Задача = ЗНАЧЕНИЕ(Справочник.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН)
		|						И КалендарьПодготовкиОтчетности.ДатаНачалаДокументов = &НачалоПериода
		|						И КалендарьПодготовкиОтчетности.ДатаОкончанияДокументов <= &ОкончаниеПериода)) КАК РасчетыПоНалогамОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|						КалендарьПодготовкиОтчетности.Задача = ЗНАЧЕНИЕ(Справочник.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН)
		|						И КалендарьПодготовкиОтчетности.ДатаНачалаДокументов = &НачалоПериода
		|						И КалендарьПодготовкиОтчетности.ДатаОкончанияДокументов <= &ОкончаниеПериода)) КАК РасчетыПоНалогам
		|
		|СГРУППИРОВАТЬ ПО
	...
	ВсегоКУплате = Окр(Макс(0,СтруктураРасчетаАвансовогоПлатежа.ВсегоКУплатеАП - СтруктураРасчетаАвансовогоПлатежа.СуммаАвансовыхПлатежейАП - НалоговыйВычет));
	
	// Регистрация документа начисления
	Если ВсегоКУплате > 0 Тогда 
		ЗарегистрироватьВзаиморасчетыСБюджетом(СтруктураРасчетаАвансовогоПлатежа, ВсегоКУплате);
	КонецЕсли;
	
	// учитываем возможность отрицательного уведомления	               
	СтруктураРасчетаАвансовогоПлатежа.НалогДляУведомления = Окр(СтруктураРасчетаАвансовогоПлатежа.ВсегоКУплатеАП - СтруктураРасчетаАвансовогоПлатежа.АвансовыеПлатежиИсчисленные);	
...
Процедура РассчитатьСуммуПлатежа(НачалоПериода, ОкончаниеПериода, Организация, СтруктураДанных, ЭтоАвансовыйПлатеж = Истина)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СуммаРасходов = СуммаРасходовПовышенная + (Выборка.СуммаРасходов + Выборка.НДСОборот);
	...
	Если СтруктураФорма4ФСС.ДокументОтчетности <> Неопределено Тогда
		СтруктураФорма4ФСС.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если (ПериодРасчета >= '20130630' И СтруктураФорма4ФСС.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2012Кв1")
				...
				ИЛИ (ПериодРасчета >= '20220630' И СтруктураФорма4ФСС.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2017Кв3")Тогда
				// Форма документа отчетности не соответствует периоду формирования
	...
	Если СтруктураРасчетПоСтраховымВзносам.ДокументОтчетности <> Неопределено Тогда
		СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если (ПериодРасчета >= '20200101' И СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2017Кв1")
				...
				ИЛИ (ПериодРасчета >= '20230101' И СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2022Кв1") Тогда
			// Форма документа отчетности не соответствует периоду формирования
...
Функция ВыполнитьРасчетВзносовВПФРиФСС(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	УпрощенныйЗачетФиксированныхВзносов = РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодРасчета);
	...
	ИначеЕсли Год(ПериодРасчета) >= 2023 Тогда	
		ВзносыПоЕдиномуТарифуПодлежащиеУплате = ВзносыПоЕдиномуТарифуПодлежащиеУплате(Организация, ПериодРасчета);
		...
		Если РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодРасчета)
			ИЛИ ГодовойРасчет Тогда
		...
		Иначе
			...
			(КоличествоДней/День(КонецМесяца(ДатаНачалаПрименения))+КоличествоМесяцевНеполных)*ВзносыПоЕдиномуТарифуПодлежащиеУплате/12;    
		КонецЕсли;
...
Функция ВзносСДоходовПодлежащийУплатеЗаПериод(Организация, НалоговыйПериод, ПериодСобытия)
	...
	Если ЗначениеЗаполнено(СрокУплаты) И ВзносСДоходовПодлежитУплатеВТекущемПериоде(НалоговыйПериод, СрокУплаты) Тогда
		...
		ВзносыНачислено = ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация, ПериодСобытия);
		ДатаПереходаНаЕНС = '20230101';
		
		Если ПериодСобытия < ДатаПереходаНаЕНС Тогда
			...
			Если ЗначениеЗаполнено(ВзносыУплаченоСДатамиДокументаОплаты) Тогда
				...
				Для Каждого ТекущаяСтрока Из ВзносыУплаченоСДатамиДокументаОплаты Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Результат = ВзносыНачислено.ПФРСвыше300тр - ОплаченоДоПереходаНаЕНС;
			КонецЕсли;
		Иначе
			Результат = ВзносыНачислено.ПФРСвыше300тр;
		КонецЕсли;
...
Функция ВзносСДоходовПодлежитУплатеВТекущемПериоде(НалоговыйПериод, СрокУплаты)
	...
	Если ГодНалоговогоПериода = ГодПереходаНаЕНС И ГодНалоговогоПериода = (ГодСрокаУплаты - 1) Тогда
		...
		Возврат Истина;
	ИначеЕсли ГодНалоговогоПериода = ГодПереходаНаЕНС + 1 И ГодНалоговогоПериода = ГодСрокаУплаты Тогда
		// Для 2024 г. считаем, что взнос с доходов за 2023 г. был учтен в базе 2023 г.
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Иначе
		...
		Если Год(ПериодРасчета) < 2023 Тогда
			ПредельнаяСуммаВзноса = ТекущиеВзносы.РазмерПФР*8 
		Иначе
			ПредельнаяСуммаВзноса = 257061;  
		КонецЕсли;
        	
		МаксимальнаяСуммаСтраховыхВзносов = ПредельнаяСуммаВзноса - СуммаФиксированныхВзносов;
	КонецЕсли;	
	...
	ЭтоПредельнаяСуммаВзноса = (СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР >= МаксимальнаяСуммаСтраховыхВзносов);
	СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР = ?(ЭтоПредельнаяСуммаВзноса, МаксимальнаяСуммаСтраховыхВзносов, СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР);
...
Функция РассчитатьФиксированныеСтраховыеВзносыПосле2023(Знач Период, Организация = Неопределено)
	...
	РазмерыСтраховыхВзносов = РазмерыСтраховыхВзносов(Период, Организация);
	МесячныйВзносЕдиныйТариф   = РазмерыСтраховыхВзносов.ЕдиныйТариф / 12;
	МесячныйВзносФСС   = РазмерыСтраховыхВзносов.ФСС / 12;
	...
	Для НомерМесяца = ПервыйМесяц По 12 Цикл
		...
		// Сумма взносов за первый месяц деятельности
		СуммаВзносаЕдиныйТарифЗаМесяц    = МесячныйВзносЕдиныйТариф * ДнейУплатыВзносов/ДнейВМесяце;
		
		// Сумма взносов до конца года
		СуммаВзносаЕдиныйТарифВсего    = СуммаВзносаЕдиныйТарифВсего + СуммаВзносаЕдиныйТарифЗаМесяц;
		
		Если НомерМесяца <= НомерМесяцаРасчета Тогда
			// Сумма взносов до конца текущего месяца
			СуммаВзносаЕдиныйТариф   = СуммаВзносаЕдиныйТариф + СуммаВзносаЕдиныйТарифЗаМесяц;
		КонецЕсли;
...
Функция РазмерыСтраховыхВзносов(СтраховойПериод, Организация)
	...
	СтраховыеВзносы = Новый Структура("ПФР, ФФОМС, ФСС, ЕдиныйТариф, ПредельнаяСуммаВзноса", 0, 0, 0, 0);
	...
	ИначеЕсли ГодСтраховогоПериода = 2024 Тогда
		...
		СтраховыеВзносы.ЕдиныйТариф = 45900;		
	Иначе
...
Функция ВыполнитьРасчетНалогаПоПатенту(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ИначеЕсли ПериодРасчета = Выборка.ДатаВторогоПлатежа Тогда
			СтруктураРасчетаПатент.СуммаПатент = Выборка.СуммаПервогоПлатежа + Выборка.СуммаВторогоПлатежа;
		КонецЕсли;
...
Функция ПоказателиРасходовСтраховыеВзносы(Организация, Патент, ДатаНачала, ДатаОкончания, СтруктураРасчетаПатент, СобытиеКалендаря) Экспорт
	...
	Если УпрощенныйЗачетФиксированныхВзносов(Организация, Срок) Тогда
		...
		СтруктураРасчетаПатент.ВсегоРасходыСтраховыеВзносы = Цел(ДоляДоходовПоПатенту * (ВзносПоЕдиномуТарифу
		+ ВзносСДоходов));
...
Функция РасходыНаСтраховыеВзносыЗаГод(Организация, ДатаНачала, ДатаОкончания)
	...
	Возврат Цел(СуммаУплаченоВзносы);
...
Процедура ВыполнитьФормированияЗаписейПатента(Патент) Экспорт
	...
	|	И (ОтчетОРозничныхПродажах.ПоложениеНастроекНалоговогоУчета = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|			ИЛИ ОтчетОРозничныхПродажах.Патент = ЗНАЧЕНИЕ(Справочник.Патенты.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ВыполнитьРасчетАвансовогоПлатежаПредварительный(Организация, ПериодРасчета, ДатаДокументаОбработкиСобытия, ЗаписьСобытия) Экспорт
	...
	Если ТаблицаПоНалогу.Количество() = 0 Тогда
		...
		СобытиеКалендаряОбъект.Заблокировать();
