...
Функция УчастникиСБПДляВозврата() Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ НастройкиУчастниковСБП.ПлатежныйАгрегатор";
...
Функция ДинамическаяСсылка(
	...
	Если Результат.Свойство("СуммаОплаты") Тогда
		Результат.Удалить("СуммаОплаты");
	КонецЕсли;
...
Функция ИдентификаторыОперацииОплаты(ДокументОперации) Экспорт
	...
	Возврат ""
		...
		+ ":"
		+ ИдентификаторыОплаты.ИдентификаторОперации;
