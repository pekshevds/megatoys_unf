...
Процедура ОбновитьПромокодыПолучателей(НастройкаИнтеграции, ДанныеАдреснойКниги, Ошибки) Экспорт	
	КлючПоляПромокод = "smallbiz_1c_promocode";
	ПользовательскиеПоля = МассовыеРассылкиИнтеграция.ПользовательскиеПоляАдреснойКниги();
	ПоляПромокода = ПользовательскиеПоля.НайтиСтроки(Новый Структура("Имя", КлючПоляПромокод));
	...
	Для каждого Получатель Из ДанныеАдреснойКниги.Получатели Цикл
		...
		ПредставлениеПоля = ПредставлениеПользовательскогоПоля(ПоляПромокода, КлючПоляПромокод);
		ДанныеПолучателяДляВыгрузки.Вставить(ПредставлениеПоля, Получатель[КлючПоляПромокод]);
		...
		// Отправлять можно только одно изменение переменной за раз
		ПараметрыОтправки = ПараметрыОтправкиЗапроса(НастройкаИнтеграции);
		ПараметрыОтправки.АдресРесурса = СтрШаблон("addressbooks/%1/emails/variable", ДанныеАдреснойКниги.Идентификатор);
