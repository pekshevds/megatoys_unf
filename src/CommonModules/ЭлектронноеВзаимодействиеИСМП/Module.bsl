...
Функция ЧастичноеСодержимоеИКодыОСУ(ТаблицаУпаковок, ТаблицаТоваров, ПараметрыСканирования) Экспорт
	...
	Результат = ЧастичноеСодержимое(ТаблицаУпаковок);
	...
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		...
		СтрокиРезультат = Результат.НайтиСтроки(Поиск);
	...
	ОшибкиНетGTIN = Новый Массив;
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		ЗаполнитьЗначенияСвойств(Поиск, СтрокаТЧ);
		Если Результат.НайтиСтроки(Поиск).Количество() Тогда
			Продолжить;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаТЧ.GTIN) Тогда
			ОшибкиНетGTIN.Добавить(ИнтеграцияИС.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика));
		КонецЕсли;
	...
	КонецЦикла;
	Если ОшибкиНетGTIN.Количество() > 0 Тогда
		ОшибкиНетGTIN.Вставить(0,НСтр("ru = 'Отсутствует GTIN для автоматического формирования кодов ОСУ для:'"));
		ВызватьИсключение СтрСоединить(ОшибкиНетGTIN, Символы.ПС);
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из ДобавленныеСтроки Цикл
		Если СтрокаТЧ.ТребуетВзвешивания Тогда
			...
			Если ДанныеПоВидуПродукции.ТребуетсяОбновлениеКлючаСессии Тогда
				ВызватьИсключение НСтр("ru = 'Для автоматического формирования кодов ОСУ для весового товара требуется доступный ключ сессии сервиса ИС МП'");
			КонецЕсли;
...
Процедура ЗаполнитьСведенияОМаркировкеУКД2020(Приемник, Источник, ДанныеШтрихкодовУпаковокДо, ДанныеШтрихкодовУпаковокПосле) Экспорт
	...
	Если КодыОСУДо.Количество() И Не КодыОСУПосле.Количество() И Приемник.Количество = 0 Тогда
		...
		Для Каждого СтрокаУпаковки Из КодыОСУДо Цикл
			РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
				СтрокаУпаковки.ЗначениеШтрихКода, СтрокаУпаковки.ВидПродукции, ЭлектронноеВзаимодействиеИСМППовтИсп.ПараметрыРазбора());
			Если РезультатРазбора <> Неопределено
				И РезультатРазбора.Свойство("ДанныеРазбора") И РезультатРазбора.ДанныеРазбора <> Неопределено Тогда
				...
				ШтрихкодированиеИСМПСлужебный.ЗаменитьЗначенияШтрихкодаНаОбъемноСортовойУчет(
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтруктураДанных), "ЗначениеШтрихкода", "КоличествоУпаковок");
...
Функция СформироватьКодОСУИзGTIN(СтрокаТовары, СтрокаУпаковки, Поле = "Количество")
	...
	Возврат СтрШаблон(
		"%1%2%3%4",
		"02", СтрокаУпаковки.ЗначениеШтрихкода, "37", Формат(КоличествоПоТегу37,"ЧДЦ=0; ЧГ=;"));
