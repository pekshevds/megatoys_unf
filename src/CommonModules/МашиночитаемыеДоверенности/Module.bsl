...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если ВыбраннаяФорма = Неопределено Тогда
			ВыбраннаяФорма = Метаданные.Справочники.МЧД003.Формы.СозданиеДоверенности;
		КонецЕсли;
...
Функция ПодписиСОшибкамиМЧД(ПодписанныйОбъект) Экспорт
	...
	Для Каждого СвойстваПодписи Из КоллекцияПодписей Цикл
		...
		Доверитель = ЭлектронныеДокументыЭДО.ОпределитьДоверителя(Сведения, СвойстваПодписи, ВсегоПодписей);
	...
	Если (ДанныеДоверенности.СтатусВРеестреФНС <> Перечисления.СтатусыМашиночитаемойДоверенностиВРеестреФНС.Отправлено
		...
		ИначеЕсли СведенияСтатусаДоверенности.СтатусДоверенности = "DECLINED"
			...
			Если Не ЗначениеЗаполнено(ДатаОтзыва)
				И Не ЗначениеЗаполнено(ДанныеДоверенности.ДатаОтзыва)
				Тогда
...
Функция ДатаОтзываДоверенности(НомерДоверенности, ДоверительИНН, ТокенДоступа = "") Экспорт
	...
	ДанныеПодписиЗаявленияНаОтмену = Неопределено;
	
	СведенияОДоверенности = ПолучитьСведенияДоверенностиНаСервереМЧД(НомерДоверенности, ДоверительИНН, ТокенДоступа); 
	Если Не СведенияОДоверенности.Ошибка Тогда
		ДанныеПодписиЗаявленияНаОтмену = СведенияОДоверенности.ПолныеДанные.ДанныеПодписиЗаявленияНаОтмену;
	КонецЕсли;

	Возврат ДатаОтзываДоверенностиПоДаннымПодписиЗаявленияНаОтмену(ДанныеПодписиЗаявленияНаОтмену);

КонецФункции

// Возвращает дату отзыва доверенности по данным подписи завявления на отмену.
// 
// Параметры:
...
// Возвращаемое значение:
//  Дата
Функция ДатаОтзываДоверенностиПоДаннымПодписиЗаявленияНаОтмену(ДанныеПодписиЗаявленияНаОтмену) Экспорт

	ДатаОтзыва = Дата(1, 1, 1);
	
	Если ЗначениеЗаполнено(ДанныеПодписиЗаявленияНаОтмену) Тогда
		ДатаОтзыва = ЭлектроннаяПодпись.ДатаПодписания(ДанныеПодписиЗаявленияНаОтмену);
	КонецЕсли;
...
Функция ДанныеСнилсПодписантов(МассивМЧД) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Файл <> Неопределено Тогда
			Результат.Вставить(Выборка.Ссылка, УбратьИзСтрокиВсеНеЦифры(
				Справочники.МЧД003.ПолучитьСНИЛСЛицДействующихБезДоверенностиПоФайлу(Выборка.Файл.Получить())));
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
...
Функция ОбщиеСвойстваДоверенностей(Доверенности) Экспорт
	...
	Запрос.Текст = 
		...
		|	МашиночитаемыеДоверенностиКонтрагентов.ДоверительИНН КАК ДоверительИНН,
		|	МашиночитаемыеДоверенностиКонтрагентов.Верна,
		|	МашиночитаемыеДоверенностиКонтрагентов.Отозвана,
		|	МашиночитаемыеДоверенностиКонтрагентов.ДатаОтзыва,
		|	МашиночитаемыеДоверенностиКонтрагентов.СтатусВРеестреФНС,
		...
		|ВЫБРАТЬ
		|	МЧД003.Ссылка КАК Ссылка,
		|	"""" КАК НомерРодительскойДоверенности,
		|	МЧД003.НомерДоверенности КАК НомерДоверенности,
		|	МЧД003.ДатаВыдачи КАК ДатаВыдачи,
		|	МЧД003.СрокДействия КАК ДатаОкончания,
		|	"""" КАК Доверитель,
		|	ЕСТЬNULL(МЧД003Доверители.ИНН, """") КАК ДоверительИНН,
		|	МЧД003.Верна КАК Верна,
		|	МЧД003.ДатаПрекращения > ДАТАВРЕМЯ(1, 1, 1) КАК Отозвана,
		|	МЧД003.ДатаПрекращения КАК ДатаОтзыва,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыМашиночитаемойДоверенностиВРеестреФНС.ПустаяСсылка) КАК СтатусВРеестреФНС,
		|	МЧД003.ПолномочияОграничены КАК ПолномочияОграничены,
		|	МЧД003.ВариантЗаполненияПолномочий =
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыЗаполненияПолномочийМЧД.КлассификаторФНС) КАК ПолномочияУказаныИзКлассификатора
		|ИЗ
		|	Справочник.МЧД003 КАК МЧД003
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МЧД003.Доверители КАК МЧД003Доверители
		|		ПО МЧД003.Ссылка = МЧД003Доверители.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МЧД003.Представители КАК МЧД003Представители
		|		ПО МЧД003.Ссылка = МЧД003Представители.Ссылка
		|ГДЕ
		|	МЧД003.Ссылка В (&Доверенности)
		|	И (МЧД003Доверители.Ссылка В (&Доверенности)
		|	И МЧД003Доверители.НомерСтроки = 1)
		|	И (МЧД003Представители.Ссылка В (&Доверенности)
		|	И МЧД003Представители.НомерСтроки = 1)";
...
Функция НомераДоверенностейПоУчастникам(ДоверительИНН, ПредставительИНН, ДействительныеНаДату,
	ЭлектронныйДокумент = Неопределено) Экспорт
	
	Результат = НомераДоверенностейB2B(ДоверительИНН, ПредставительИНН, ДействительныеНаДату, ЭлектронныйДокумент);
	Справочники.МЧД003.ДополнитьНомерамиДоверенностейПоУчастникам(
...
//  * НомераДоверенностей - Массив из Строка
//  * ОшибкиПроверкиПолномочий - Массив из см. НоваяОшибкаПроверкиПолномочий
Функция НомераДоверенностейB2B(ДоверительИНН, ПредставительИНН, ДействительныеНаДату,
	ЭлектронныйДокумент = Неопределено)
	...
	Запрос.Текст = 
		...
		|	МашиночитаемыеДоверенностиКонтрагентов.ВариантЗаполненияПолномочий =
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыЗаполненияПолномочийМЧД.КлассификаторФНС) КАК ПолномочияУказаныИзКлассификатора,
		|	ЕСТЬNULL(ПравилаПроверкиПолномочийПоМЧД.ПравилоПроверки, ЗНАЧЕНИЕ(Справочник.ПравилаПроверкиПолномочийМЧД.ПустаяСсылка)) КАК ПравилоПроверки
		|ИЗ
		|	Справочник.МашиночитаемыеДоверенностиКонтрагентов КАК МашиночитаемыеДоверенностиКонтрагентов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПравилаПроверкиПолномочийПоМЧД КАК ПравилаПроверкиПолномочийПоМЧД
		|		ПО (ПравилаПроверкиПолномочийПоМЧД.Доверенность = МашиночитаемыеДоверенностиКонтрагентов.Ссылка)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.МашиночитаемыеДоверенностиКонтрагентов.Представители КАК МашиночитаемыеДоверенностиКонтрагентовПредставители
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДоверенностьДействительнаПоСвойствам(СвойстваДоверенности, ДействительныеНаДату) Тогда
...
Функция ДанныеДоверенностиДляКонтейнера(Доверенность) Экспорт
	Возврат СправочникМЧД(Доверенность).ДанныеДоверенностиДляКонтейнера(Доверенность);
КонецФункции
...
Процедура ЗаписатьРезультатПроверкиПодписи(ПодписанныйОбъект, ХешПодписи, РезультатПроверки) Экспорт
	...
	ПроверкаМЧДВыполнена = ЗначениеЗаполнено(РезультатПроверки.Доверенность) И РезультатПроверки.ПроверкаВыполнена
		И РезультатПроверки.ПротоколПроверки <> Неопределено;
	ЕстьОшибкиПроверкиПолномочий = РезультатПроверки.ПротоколПроверки <> Неопределено
			И РезультатПроверки.ПротоколПроверки.ОшибкиПроверкиПолномочий.Количество() > 0;
	Если ПроверкаМЧДВыполнена Или ЕстьОшибкиПроверкиПолномочий Тогда
...
Функция ИННДоверителя(Ссылка) Экспорт
	...
	Иначе
		ДвоичныеДанныеДоверенности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "XMLфайлМЧД").Получить();
...
Функция ЗагрузитьМЧДИзФайла(АдресХранилища, ТребуетсяПроверкаМЧДНаКлиенте = Ложь, ОбновлятьСуществующий = Ложь,
	...
	РезультатЧтения = ПрочитатьДанныеXML(Данные.ДанныеДоверенности);
...
Процедура ОбновитьКлассификаторПолномочийФНС_МЧД003(ДанныеКлассификатора) Экспорт
	...
	Попытка
		...
		Пока ВыборкаДетальныеЗаписиРодителей.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаДетальныеЗаписиРодителей.Родитель) И
				ЗначениеЗаполнено(ВыборкаДетальныеЗаписиРодителей.Ссылка) Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КодКлассификатора) И
				ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Ссылка) Тогда
...
Функция ПрочитатьАрхив(ДанныеАрхива) Экспорт
	...
	ВерсияФорматаМЧД = ВерсияФорматаФайлаМЧД(ДанныеФайлов.ДанныеДоверенности);
	Если ДанныеФайлов.Подписи.Количество() = 0 
		Или ДанныеФайлов.Подписи.Количество() > 1 
		И ВерсияФорматаМЧД <> МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003() Тогда
			
		ВидОперации = НСтр("ru = 'Чтение архива машиночитаемой доверенности'");
		ШаблонОшибки = НСтр("ru = 'Ошибка при чтении архива: %1'");
		
		Если ДанныеФайлов.Подписи.Количество() = 0 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибки, НСтр("ru = 'нет файла подписи'"));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибки, НСтр("ru = 'несколько файлов подписи'"));
		КонецЕсли;
	...
	ДанныеДляЗагрузкиМЧД.ДанныеПодписи = ДанныеФайлов.Подписи[0];
	ДанныеДляЗагрузкиМЧД.ДанныеПодписей = ДанныеФайлов.Подписи;
...
//                    * Подписи - Массив Из ДвоичныеДанные
Функция ПрочитатьАрхивДоверенности(ДвоичныеДанныеАрхива)
	...
	Для Каждого РасширениеПодписи Из МассивРасширений Цикл
		...
		ФайлыПодписейСРасширением = Новый Массив;
		Для Индекс = 0 По ВсеФайлыСРасширением.ВГраница() Цикл
			...
			Если ЭтоПодписьКвитанции
				Или ЭтоЗаявлениеНаОтмену Тогда
				Продолжить;
			КонецЕсли;
			ФайлыПодписейСРасширением.Добавить(ВсеФайлыСРасширением[Индекс].ПолноеИмя);
		КонецЦикла;
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФайлыПодписей, ФайлыПодписейСРасширением, Истина);
	КонецЦикла;
	...
	Для Каждого ФайлПодписи Из ФайлыПодписей Цикл
		...
		Если Подписи.Найти(ДанныеПодписи) = Неопределено Тогда
			Подписи.Добавить(ДанныеПодписи);
		КонецЕсли;
		
	КонецЦикла;
	
	ДанныеФайлов.Вставить("Подписи", Подписи);
...
Функция ПолучитьРодительскиеДанныеДоверенности(НомерДоверенности,
	...
	Если ЗначениеЗаполнено(СведенияДоверенности.ПолныеДанные) Тогда
		...
		ПравилоПроверки = РегистрыСведений.ПравилаПроверкиПолномочийПоМЧД.ПравилоПроверки(РезультатЗагрузки.МЧД);
	...
	Если ОбъектЧтение.Элементы.Количество() <> 0 Тогда
		...
		Для Каждого ЭлементАрхива Из ОбъектЧтение.Элементы Цикл
			...
			Если ЭтоДоверенность Или ЭтоПодписьДоверенности Или ЭтоПодписьЗаявленияНаОтмену Тогда
				...
				Иначе
					ДанныеПодписи = ПолучитьДвоичныеДанныеИзBase64ДвоичныхДанных(ДвоичныеДанные);
			...
			Если Не ПустаяСтрока(НаименованиеФайлаДоверенности) 
				...
				Для Каждого ЭлементАрхива Из ОбъектЧтение.Элементы Цикл
					Если ЭтоПодписьМЧД(ЭлементАрхива, НаименованиеФайлаДоверенности) Тогда
							ДвоичныеДанные = Новый ДвоичныеДанные(КаталогРаспаковки + ЭлементАрхива.Имя);
							Результат.ДанныеПодписи = ПолучитьДвоичныеДанныеИзBase64ДвоичныхДанных(ДвоичныеДанные);
					ИначеЕсли ЭтоПодписьЗаявленияНаОтмену(ЭлементАрхива, НаименованиеФайлаЗаявленияНаОтмену) Тогда
							ДвоичныеДанные = Новый ДвоичныеДанные(КаталогРаспаковки + ЭлементАрхива.Имя);
							Результат.ДанныеПодписиЗаявленияНаОтмену = 
...
Функция ПроверитьПолномочияДоверенности(Доверенность, ЭлектронныйДокумент) Экспорт
	...
	Иначе
		...
		Если ТипДоверенности = Тип("СправочникСсылка.МашиночитаемыеДоверенностиКонтрагентов") Тогда
			...
			Если Не РезультатЧтения.Успех Или РезультатЧтения.ДанныеДоверенности = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Не удалось получить данные доверенности.'");
				ВывестиОшибкуНекорректныхПравилПроверки(Доверенность, ЭлектронныйДокумент);
				Результат.ТекстОшибки = НСтр("ru = 'Проверка полномочий завершилась с ошибкой.
			...
			Полномочия = РезультатЧтения.ДанныеДоверенности.Полномочия;
			
		Иначе
			
			Полномочия = Доверенность.Полномочия;
...
Функция ЗаполнитьСведенияОПредставителеМЧД(СведенияОбУполномоченномПредставителе, ТипУполномоченногоПредставителя)
		...
		Если ТипУполномоченногоПредставителя  = "ЮЛ" Тогда
			...
			Если ПустаяСтрока(СведенияОрганизации) Тогда
				СведенияОрганизации = СведенияОбУполномоченномПредставителе.СвОрг;
			КонецЕсли;
		...
		Если Представитель.Отчество <> Неопределено Тогда
...
Функция ДанныеИзФайлаОбменаВУтвержденномФормате(ВходящиеДанные, ЭтоДоверенностьОрганизации = Неопределено)
	...
	Для Каждого СведенияОбУполномоченномПредставителе Из Документ.СвУпПред Цикл
		...
		ДанныеДоверенности.Представители.Добавить(ЗаполнитьСведенияОПредставителеМЧД(
			СведенияОбУполномоченномПредставителе.СвПред[0], ТипУполномоченногоПредставителя));
...
Функция ТребуетсяПерезаполнениеМЧД(Доверенность, НовыеДанные, ФорматДоверенности = "") Экспорт
	...
	Иначе
		ДатаОтзыва = ДатаОтзываДоверенностиПоДаннымПодписиЗаявленияНаОтмену(
			НовыеДанные.ДанныеПодписиЗаявленияНаОтмену);
...
Процедура ЗаполнитьРеквизитыОтзыва(Доверенность, ДанныеДляЗагрузки) Экспорт
	...
	ДатаОтзыва = ДатаОтзываДоверенностиПоДаннымПодписиЗаявленияНаОтмену(
		ДанныеДляЗагрузки.ДанныеПодписиЗаявленияНаОтмену);
...
Функция НайтиДоверенности(НомерДоверенности, ИННДоверителя, СправочникМЧД = Неопределено)
	
	Результат = Справочники.МЧД003.НайтиДоверенности(НомерДоверенности, ИННДоверителя);
	
	Если СправочникМЧД <> Неопределено Тогда
		
		Доверенности = СправочникМЧД.НайтиДоверенности(НомерДоверенности);
...
Функция СведенияОСтатусахДоверенностей(СправочникМЧД, ДанныеМЧД)
	...
	Для Каждого ДанныеДоверенности Из ДанныеМЧД Цикл
		...
		Доверенности = НайтиДоверенности(НомерДоверенности, ИННДоверителя, СправочникМЧД);
		...
		РезультатЧтения = ПрочитатьДанныеXML(СведенияИзРеестра.ПолныеДанные.ДанныеВыгрузки);
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	Попытка
		...
		ДанныеXDTO = Результат.ДанныеДоверенности;
...
Функция ЭтоМЧД003(Доверенность)
