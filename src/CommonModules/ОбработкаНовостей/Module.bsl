			...
			Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
				...
				Попытка
					...
					ПараметрыПодключения = Новый Структура;
						...
						ПараметрыПодключения.Вставить("Пользователь", Логин);
						ПараметрыПодключения.Вставить("Пароль"      , Пароль);
						ПараметрыПодключения.Вставить("Прокси"      , Прокси);
						ПараметрыПодключения.Вставить("Таймаут"     , НастройкиПолученияНовостей.HTTPСоединениеТаймаутСоединенияСекунд);
						ПараметрыПодключения.Вставить("ЗащищенноеСоединение",
							?(ВРег(Протокол) = ВРег("https"),
								ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(),
								Неопределено));

					HTTPСоединение = ИнтернетПоддержкаПользователей.СоздатьHTTPСоединение(
