...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыУНФМП.ПервыйЗапуск";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение базы начальными данными.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "*";
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Процедура = "СинхронизацияЗагрузкаНедозаписанныхДанныхМП.НайтиНедозаписанныеДанные";
...
Процедура ЗаполнитьСтавкиНДСПервыйЗапуск()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(СтавкиНДСМП.Ссылка) КАК Количество
	|ИЗ
	...
	// 10%
	СтавкаНДС = Справочники.СтавкиНДСМП.СоздатьЭлемент();
...
Процедура ДобавитьКассуККМ() Экспорт
	
	Если ПолучитьКоличествоКассККМ() = 0 Тогда
		КассаККМ = Справочники.КассыККММП.СоздатьЭлемент();
		КассаККМ.Наименование = НСтр("ru='Касса ККМ'");
		КассаККМ.РозничнаяТочка = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.СтруктурныеЕдиницыМП.ОсновнойСклад");
		КассаККМ.Записать();
	КонецЕсли;
