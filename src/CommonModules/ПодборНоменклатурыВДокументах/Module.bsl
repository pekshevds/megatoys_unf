...
Функция ПолучитьЦенуИЕдиницуИзмеренияНоменклатурыПоВидуЦен(СтруктураДанных) Экспорт
	...
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЦеныНоменклатурыВалюта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КурсВалютыВидЦен.Валюта КАК Валюта,
	|	КурсВалютыВидЦен.Курс КАК Курс,
	|	КурсВалютыВидЦен.Кратность КАК Кратность
	...
	|			Валюта В
	|				(ВЫБРАТЬ
	|					Цены.ЦеныНоменклатурыВалюта
	|				ИЗ
	|					Цены КАК Цены)) КАК КурсВалютыВидЦен
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КурсВалютыДокумента.Курс КАК Курс,
	|	КурсВалютыДокумента.Кратность КАК Кратность
	|ПОМЕСТИТЬ КурсВалютыДокумента
	|ИЗ
	...
	|	Цены.ЦеныНоменклатурыВалюта КАК ЦеныНоменклатурыВалюта,
	|	Цены.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	Цены.ПорядокОкругления КАК ПорядокОкругления,
	|	Цены.ОкруглятьВБольшуюСторону КАК ОкруглятьВБольшуюСторону,
	|	ЕСТЬNULL(Цены.Цена * КурсВалютыВидЦен.Курс * КурсВалютыДокумента.Кратность / (КурсВалютыДокумента.Курс * КурсВалютыВидЦен.Кратность) * ЕСТЬNULL(&Коэффициент, 1) / ЕСТЬNULL(Цены.ЕдиницаИзмерения.Коэффициент, 1), 0) КАК Цена
	|ИЗ
	|	Цены КАК Цены
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсВалютыВидЦен КАК КурсВалютыВидЦен
	|		ПО Цены.ЦеныНоменклатурыВалюта = КурсВалютыВидЦен.Валюта,
	|	КурсВалютыДокумента КАК КурсВалютыДокумента";
