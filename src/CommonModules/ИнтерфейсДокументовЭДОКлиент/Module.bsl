	...
	Если НЕ ЗначениеЗаполнено(ПараметрыПросмотра.ЭлектронныйДокумент)
		...
		Если НастройкиФормирования.Направление <> ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Внутренний") Тогда
			...
			Если НастройкиФормирования.НастройкиОтправки.Формат = "ON_DOGDOC_1_999_01_01_01_01" Тогда
				...
				ФормаДоговорнойДокумент101Клиент = ФормаДоговорнойДокумент101Клиент();
				ОбработчикЗавершения = Новый ОписаниеОповещения("ЗапроситьФайлЗавершение", ФормаДоговорнойДокумент101Клиент,
					 ПараметрКоманды);
				
				ПараметрыПодготовки = ФормаДоговорнойДокумент101Клиент.ПараметрыПодготовкиФайла();
				ПараметрыПодготовки.ОбработчикЗавершения = ОбработчикЗавершения;
				ПараметрыПодготовки.Данные = ПараметрКоманды;
				
				ФормаДоговорнойДокумент101Клиент.ЗапроситьИПодготовитьФайл(ПараметрыПодготовки);
...
#Область ФорматДоговорнойДокумент101_ВыполнениеКлиентскихМетодов

// Для вызова клиентских процедур формата договорного документа PDF/A3
// 
// Возвращаемое значение:
//  см. Обработка.ФорматДоговорнойДокумент101.Форма.ВыполнениеКлиентскихМетодов
//
Функция ФормаДоговорнойДокумент101Клиент() Экспорт
	
	ИмяПараметра = "ЭлектронноеВзаимодействие.ФормаФорматДоговорнойДокумент101";
	...
	Если Форма = Неопределено Тогда
		Форма = ПолучитьФорму("Обработка.ФорматДоговорнойДокумент101.Форма.ВыполнениеКлиентскихМетодов");
...
Процедура ОткрытьНовыйЭлектронныйДокументПоПодготовленнымДанным(ОбъектУчета, НастройкиФормирования, РезультатПодготовкиДанных)
	...
	Если НастройкиФормирования.Направление <> ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Внутренний") Тогда
		...
		Если НастройкиФормирования.НастройкиОтправки.Формат = "ON_DOGDOC_1_999_01_01_01_01" Тогда
			...
			ФормаДоговорнойДокумент101Клиент = ФормаДоговорнойДокумент101Клиент();
			ОбработчикЗавершения = Новый ОписаниеОповещения("ЗагрузитьИзФайлаЗавершение", ФормаДоговорнойДокумент101Клиент,
				ПараметрыФормы);
...
//    ** Параметры - См. ИнтерфейсДокументовЭДО.ПараметрыПереформированияДокумента
//    ** ПараметрыДействийПоЭДО - См. ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
...
// 
// Параметры:
//  ДопДанные - См. ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
//  Результат - Структура:
//   * КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//   * Контекст - Структура:
//    ** ОповещениеОЗавершении - ОписаниеОповещения
...
Функция СтрокаСообщенияПоРезультатамВыгрузкиДляФНС(РезультатВыгрузки, КаталогВыгрузки)
	...
	Если РезультатВыгрузки.Значение.КоличествоВФормализованномВиде <> 0 Тогда
		...
		ПутьКАрхиву = КаталогВыгрузки + "\" + РезультатВыгрузки.Значение.НаименованиеАрхиваФормализованных;
	...
	Если РезультатВыгрузки.Значение.КоличествоСНанесениемШтамповЭП <> 0 Тогда
		ШаблонСтроки = НСтр("ru='%1 - выгружены с нанесением штампов ЭП. Документы необходимо распечатать,'") 
			...
			|Путь к архиву: <a href = ""%2"">%3</a>'");
		ПутьКАрхиву = КаталогВыгрузки + "\" + РезультатВыгрузки.Значение.НаименованиеАрхиваСНанесениемШтампов;
		ТекстСтроки = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			ШаблонСтроки, 
	...
	Если РезультатВыгрузки.Значение.КоличествоСОтдельнымиШтампамиЭП <> 0 Тогда
		ШаблонСтроки = НСтр("ru='%1 - выгружены с отдельными штампами ЭП в виде картинок. Необходимо нанести штампы'")
			...
			|Путь к архиву: <a href = ""%2"">%3</a>'");
		ПутьКАрхиву = КаталогВыгрузки + "\" + РезультатВыгрузки.Значение.НаименованиеАрхиваСОтдельнымиШтампами;
		ТекстСтроки = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			ШаблонСтроки, 
	...
	Если РезультатВыгрузки.Значение.КоличествоНеВыгружено <> 0 Тогда
		...
		ПутьКФайлу = КаталогВыгрузки + "\" + РезультатВыгрузки.Значение.НаименованиеФайлаСНевыгруженными;
		ТекстСтроки = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			ШаблонСтроки, 
