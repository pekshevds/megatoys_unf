...
// Заполняется при наличии в конфигурации библиотеки БЭД:
// Заполняет настройки подключения организации к ЭДО.
//
// Параметры:
...
//
// Пример:
//   ОбменСКонтрагентами.ЕстьПравоНастройкиОбмена();
//
Процедура ЕстьПравоНастройкиЭДО(ЕстьПраво = Ложь) Экспорт
	...
	Если ЕстьПодсистемаБизнесСеть Тогда
		...
		ЕстьПраво = МодульБизнесСеть.ПравоВыполненияОбменаДокументами();
	Иначе 
		ЕстьПраво = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьСведенияОСотруднике(Знач Сотрудник, Знач МассивПоказателей = Неопределено, Знач ДатаЗначения = Неопределено) Экспорт
	...
	СведенияОСотруднике = Новый Структура("Фамилия, Имя, Отчество, СтраховойНомерПФР,СНИЛС, АдресРегистрации, Телефон,
		|ЭлектроннаяПочта, АдресМестаЖительства_XML, АдресМестаПребывания_XML, АдресФактическогоПроживания_XML, АдресЗаПределамиРФНаРусском_XML,
		|АдресЗаПределамиРФНаИностранном_XML, Пол, Гражданство, МестоРождения, ДатаРождения");
	Удостоверение = Новый Структура("Серия, Номер, ДатаВыдачи, КемВыдан, ВидДокумента, КодПодразделения, СрокДействия");
	...
	|	ФИОФизическихЛицСрезПоследних.ФизическоеЛицо.СтраховойНомерПФР КАК СтраховойНомерПФР,
	...
	|	КонтактнаяИнформацияЭлектроннаяПочта.Представление КАК ЭлектроннаяПочта,
	...
	|			И (КонтактнаяИнформацияАдрес.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица))
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица.КонтактнаяИнформация КАК КонтактнаяИнформацияЭлектроннаяПочта
	|		ПО (КонтактнаяИнформацияЭлектроннаяПочта.Ссылка = &ФизическоеЛицо)
	|			И (КонтактнаяИнформацияЭлектроннаяПочта.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailФизическиеЛица))
	|		ЛЕВОЕ СОЕДИНЕНИЕ  РегистрСведений.ДокументыФизическихЛиц.СрезПоследних КАК ДокументыФизическихЛицСрезПоследних
	|		ПО (ДокументыФизическихЛицСрезПоследних.Физлицо = &ФизическоеЛицо)
	...
	Попытка
		...
		Если Выборка.Следующий() Тогда
			...
			СведенияОСотруднике.Вставить("ЭлектроннаяПочта", Выборка.ЭлектроннаяПочта);
			СведенияОСотруднике.Вставить("Пол", Выборка.Пол);
			СведенияОСотруднике.Вставить("Гражданство", Выборка.Гражданство);
			СведенияОСотруднике.Вставить("ДатаРождения", Выборка.ДатаРождения);
			
			Удостоверение.Вставить("Серия", Выборка.Серия);
			Удостоверение.Вставить("Номер", Выборка.Номер);
			Удостоверение.Вставить("ДатаВыдачи", Выборка.ДатаВыдачи);
			Удостоверение.Вставить("КемВыдан", Выборка.КемВыдан);
			Удостоверение.Вставить("ВидДокумента", Выборка.ВидДокумента);
			Удостоверение.Вставить("КодПодразделения", Выборка.КодПодразделения);
		КонецЕсли;
	Исключение
	КонецПопытки;
	СведенияОСотруднике.Вставить("УдостоверениеЛичности", Удостоверение);
...
// 
Функция ПолучитьФизЛицоПоСНИЛС(СНИЛС, ФизЛицоРезультат) Экспорт  
	
	ФизЛицоРезультат = Справочники.ФизическиеЛица.ПустаяСсылка();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл        
		ФизЛицоРезультат = ВыборкаДетальныеЗаписи.ФизическоеЛицо;
	КонецЦикла; 
	
	Возврат ФизЛицоРезультат;	
...
Процедура ОпределитьПодключаемыеНаправленияСдачиОтчетности(Организация, Направления) Экспорт
	ИспользуетсяНаемныйТруд = Ложь;
	РегламентированнаяОтчетностьПереопределяемый.ПолучитьПризнакИспользованияИПТрудаНаемныхРаботников(Организация, ИспользуетсяНаемныйТруд);
	
	Для каждого Направление Из Направления Цикл
		Если Направление.Орган <> Перечисления.ТипыКонтролирующихОрганов.ФНС
			И Направление.Орган <> Перечисления.ТипыКонтролирующихОрганов.ПФР
			И Направление.Орган <> Перечисления.ТипыКонтролирующихОрганов.ФСРАР
			И Направление.Орган <> Перечисления.ТипыКонтролирующихОрганов.ФСС 
			И Направление.Орган <> Перечисления.ТипыКонтролирующихОрганов.ФСГС Тогда
			
			Направление.Доступен   = Ложь;
			Направление.Подключить = Ложь;
		ИначеЕсли (Направление.Орган = Перечисления.ТипыКонтролирующихОрганов.ПФР
			ИЛИ Направление.Орган = Перечисления.ТипыКонтролирующихОрганов.ФСС)
			И Не ИспользуетсяНаемныйТруд Тогда
			Направление.Доступен   = Ложь;
			Направление.Подключить = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьОрганизациюНаПустоеНаименование(Организация, ОрганизацияИмеетПустоеНаименование = Истина) Экспорт
	...
	ОрганизацияИмеетПустоеНаименование = ПустаяСтрока(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "Наименование" ));
...
Процедура ПолучитьЗначенияКонстантИнтеграции(ФлагиУчета = Неопределено) Экспорт
	Если ФлагиУчета = Неопределено Тогда
		ФлагиУчета = Новый Структура();
	КонецЕсли;
...
Процедура УпроститьФормуРегОтчетности(ФормаРО) Экспорт
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		ФормаРО.Элементы.Декорация2.Видимость = Ложь;
		ФормаРО.Элементы.ГиперссылкаНастройки.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
