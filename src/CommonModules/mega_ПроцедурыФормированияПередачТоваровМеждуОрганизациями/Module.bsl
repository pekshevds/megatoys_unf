...
Процедура ЗапуститьПроцедуруФормированияПередачТоваровМеждуОрганизациями()Экспорт
	
	ПериодРегистрации = КонецМесяца(ТекущаяДатаСеанса());
	
	ТаблицаПолучателей = ИницализацияТаблицыПолучателей();
	ЗаполнениеТаблицыПолучателей(ТаблицаПолучателей, ПериодРегистрации);
		
	Для Каждого ТекСтрока Из ТаблицаПолучателей Цикл 
		
		СозданныеДокументы = ИнициализацияТаблицыСозданныеДокументы();
		//@skip-check query-in-loop
		СформироватьДокументыСервер(ТекСтрока.Получатель, СозданныеДокументы, ПериодРегистрации);
	КонецЦикла;
КонецПроцедуры
...
Функция ИнициализацияТаблицыСозданныеДокументы()
	
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("ДокументПередачаТоваров", Новый ОписаниеТипов("ДокументСсылка.ПередачаТоваровМеждуОрганизациями"));
		
	Возврат ТаблицаЗначений;
КонецФункции

Функция ИницализацияТаблицыПолучателей()
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("Получатель", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	
	Возврат ТаблицаЗначений;
КонецФункции

Процедура ЗаполнениеТаблицыПолучателей(ТаблицаПолучателей, ПериодРегистрации)
...
Процедура СформироватьДокументыСервер(ОрганизацияПолучатель, СозданныеДокументы, ПериодРегистрации)
