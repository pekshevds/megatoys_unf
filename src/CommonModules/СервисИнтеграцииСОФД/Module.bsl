...
Функция ОперацияПодключенияКассы(ПараметрыОперации)
	...
	ОФДСлужебный.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Операция подключения кассы.'"),
		Ложь);
...
Функция ОперацияОтключенияКассы(ПараметрыОперации)
	...
	ОФДСлужебный.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Операция отключения кассы.'"),
		Ложь);
...
Функция ПроанализироватьОтветСервисаОФД(ОтветСервиса, ИменаСвойствСоЗначениямиДата = Неопределено)
	
	РезультатОперации = ОФДСлужебный.НовыйРезультатОперации();
	
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Получен ответ ОФД:
			|%1'"),
		ОтветСервиса);
...
Функция СписокКассПоДаннымСервиса(
	...
	РезультатАнализа = ПроанализироватьОтветСервисаОФД(РезультатЗапроса.ОтветСервиса);
...
Функция ОперацияПолучениеОборотовПоСмене(
	...
	РезультатАнализа = ПроанализироватьОтветСервисаОФД(РезультатЗапроса.ОтветСервиса, ИменаСвойствСоЗначениямиДата);
	...
	Попытка
		...
		Для Каждого ДанныеЧекаИзСервиса Из ДанныеСервиса.receipts Цикл
			...
			Для Каждого ДанныеПоТоваруИзСервиса Из ДанныеЧекаИзСервиса.items Цикл
				...
				Если ДанныеПоТоваруИзСервиса.Свойство("productCode") Тогда
				...
				Если ДанныеПоТоваруИзСервиса.measureData.Свойство("measureValue") Тогда
				...
				Если ДанныеПоТоваруИзСервиса.Свойство("agentData")
				...
				Если ДанныеПоТоваруИзСервиса.Свойство("supplierData")
...
Функция НовыйЗаголовкиВызоваОперации() Экспорт
	...
	Заголовки.Вставить(
		...
		КодироватьСтроку(ИнтернетПоддержкаПользователей.ИмяКонфигурации(), СпособКодированияСтроки.КодировкаURL));
	...
	Заголовки.Вставить("X-IB-User-Name",          Строка(ИмяПользователя()));
...
Функция СформироватьИдентификатор(ИсходнаяСтрока)
	...
	ДопустимыеСимволы = "АБВГДЕЁЖЗИКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ";
