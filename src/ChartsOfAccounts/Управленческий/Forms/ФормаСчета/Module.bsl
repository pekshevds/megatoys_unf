...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.АналитикаДоходовИРасходов) Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не найден тип аналитики учета доходов и расходов.'");
	...
	Если ОтказИзменитьАналитикуПрочихДоходовИРасходов(Объект.Ссылка) Тогда
		Элементы.ГруппаАналитикаУчетаПрочихДоходовИРасходов.Подсказка = НСтр(
			"ru = 'В базе есть движения по этому счету. Изменение аналитики не рекомендуется.'");
	КонецЕсли;
