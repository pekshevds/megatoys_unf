...
Процедура НазначитьПравилоДляВыделенныхУстройствНаСервере(Устройства, ПравилоОбмена)
	...
	Для Каждого Устройство Из Устройства Цикл
		...
		Если ТипЗнч(Устройство) = Тип("СправочникСсылка.КассыККМ") Тогда
			УстройствоОбмена = ?(ЗначениеЗаполнено(Устройство.ПодключаемоеОборудование), Устройство.ПодключаемоеОборудование, Устройство.ОфлайнОборудование);
		Иначе
			УстройствоОбмена = Устройство;
		КонецЕсли;
		УстройствоОбменаОбъект = УстройствоОбмена.ПолучитьОбъект();
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УстройствоОбмена, "ПравилоОбмена") Тогда
			УстройствоОбменаОбъект.ПравилоОбмена = ПравилоОбмена;
			УстройствоОбменаОбъект.Записать();
		КонецЕсли;
...
&НаКлиенте
Процедура ВесыКартинкиВыгрузить(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ВесыКартинкиВыгрузитьЗавершение", ЭтотОбъект, Новый Структура("РеквизитФормы", "ФайлЗагрузки"));
	НачатьПодключениеРасширенияРаботыСФайлами(Оповещение); 
	
КонецПроцедуры

&НаКлиенте
Процедура ВесыКартинкиВыгрузитьЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	ПодключаемоеОборудованиеOfflineКлиент.АсинхронныйВыгрузитьКартинкиВОборудованиеOffline(ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток"), Элементы.Весы.ВыделенныеСтроки,,, ОповещениеОВыполнении, Истина);
...
Процедура ЗагрузитьОтчетыЗаПериод(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ЭтоКассаЭвотор = МенеджерОфлайнОборудованияВызовСервера.ПодключаемоеОборудованиеЭвоторОФД(ТекущиеДанные.ПодключаемоеОборудование);
...
&НаКлиенте
Процедура ЗагрузитьОтчетыЗаПериодОФД(Команда)
	
	ТекущиеДанные = Элементы.КассыОФД.ТекущиеДанные;
	
	Если ТекущиеДанные <> Неопределено Тогда
		
		ЭтоКассаЭвотор = МенеджерОфлайнОборудованияВызовСервера.ПодключаемоеОборудованиеЭвоторОФД(ТекущиеДанные.ОфлайнОборудование);
		
		Если ЭтоКассаЭвотор Тогда
			ИдентификаторУстройства = ?(ПустаяСтрока(ТекущиеДанные.ОфлайнОборудование), Неопределено, ТекущиеДанные.ОфлайнОборудование);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ИдентификаторУстройства", ИдентификаторУстройства);
			
			ОповещениеПриЗавершении = Новый ОписаниеОповещения("КассыОФДЗагрузкаОтчетовЗавершение", ЭтотОбъект);
			ОткрытьФорму("Справочник.ОфлайнОборудование.Форма.ФормаНастройки1СЭвоторККМОфлайнПроизвольногоПериодаЗагрузки", ПараметрыФормы, ЭтотОбъект,,,, ОповещениеПриЗавершении, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ТекстСообщения = НСтр("ru='Для данного типа устройства данная команда недоступна.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
...
&НаКлиенте
Процедура СкладОФДПриИзменении(Элемент)
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(КассыККМ.Отбор, "Склад", СкладОФД, ВидСравненияКомпоновкиДанных.Равно,, ЗначениеЗаполнено(СкладОФД));
КонецПроцедуры

&НаКлиенте
Процедура КассыОФДОтобразитьСопоставленныеТовары(Команда) 
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("СопоставлениеПослеЗагрузки", Ложь);
	
	ОткрытьФорму("РегистрСведений.СопоставлениеНоменклатурыОФД.Форма.ФормаСопоставленияТоваров", ПараметрыОткрытия, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура КассыОФДЗагрузитьОтчеты(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ОповещениеОВыполнении = Новый ОписаниеОповещения(
			...
			ЭтотОбъект);
		МенеджерОфлайнОборудованияКлиент.НачатьЗагрузкуДанныхИзККМ(ТекущиеДанные.ОфлайнОборудование, УникальныйИдентификатор, ОповещениеОВыполнении);
