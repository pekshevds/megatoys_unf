...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокАвансовДолгов
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокОтобранныеАвансыИДолги
...
Процедура ДобавитьИРассчитатьСтрокуАванса(СуммаРасчетов, ТекущаяСтрока)
	...
	СтруктураПоиска = Новый Структура("Документ, Заказ, Договор", ТекущаяСтрока.Документ, ТекущаяСтрока.Заказ, ТекущаяСтрока.Договор);
...
Процедура ДобавитьСтрокуВОтобранные(ТекущаяСтрока)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "СуммаРасчетов") Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗапрашиватьСумму Тогда
		...
		ДополнительныеПараметры = Новый Структура("ТекущаяСтрока, СуммаРасчетов", ТекущаяСтрока, СуммаРасчетов);
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуЦеныИВалютаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Функция ЗаполнитьРасшифровкуПлатежа(ТолькоДолги)
	...
	ДоговорПоУмолчанию = Справочники.ДоговорыКонтрагентов.ПоКонтрагентуПоОрганизации(Контрагент, Компания,
