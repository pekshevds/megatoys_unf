...
Процедура ИзменитьДобавитьСтрокуВКорзину(СтрокаСпискаТоваров, СтрокаХарактеристикПартий, СтрокаКорзины, Количество, Цена, СвободныйОстаток, ЕдИзмерения = Неопределено)
	...
	Иначе
		...
		СсылкаНаХарактеристику = ?(НЕ СтрокаХарактеристикПартий = Неопределено
		, СтрокаХарактеристикПартий.ХарактеристикаСсылка, ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка"));
		...
		СтруктураДанных.Вставить("Характеристика", 	СсылкаНаХарактеристику);
		...
		Если Цена=0 И ИспользуютсяЦены Тогда
			
			// Требуется расчет цены при получении данных номенклатуры
		...
		ПересчитатьВВалюту = (Валюта <> ВалютаВидаЦен);
		ПересчитатьНДС = (СуммаВключаетНДС <> ЦенаВключаетНДС);
		
		Если Цена=0 И ИспользуютсяЦены Тогда
			
			Цена = ДанныеНоменклатуры.Цена;
			// Цена рассчитана с учетом валюты и флага включения НДС
		...
		Если ПересчитатьВВалюту Тогда
			
			ПредварительнаяЦена = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(ПредварительнаяЦена, КурсВалютаВидаЦен, КурсВалютаДокумента, КратностьВалютаВидаЦен, КратностьВалютаДокумента);
			
		КонецЕсли; 
		
		ПредварительнаяЦена = ОкруглитьЦену(ПредварительнаяЦена, ВидЦенПорядокОкругления, ВидЦенОкруглятьВБольшуюСторону);
		
		НоваяСтрока.Цена = ПредварительнаяЦена;
...
&НаКлиенте
// Формирует структуру данных подбора из реквизитов формы
//
...
Процедура ЗаполнитьПараметрыОткрытияФормыЗначениямиПоУмолчанию(СтруктураПараметровПоУмолчанию, Параметры)
	...
	СтруктураПараметровПоУмолчанию.Вставить("Организация", 
		?(Константы.УчетПоКомпании.Получить(), Справочники.Организации.ОрганизацияКомпания(), ЗначениеДляПараметраОрганизация));
	...
	Если Параметры.Свойство("ЕстьНалогообложение") Тогда
		Если Параметры.ЕстьНалогообложение Тогда
		
		// Налогообложение НДС
		СтруктураПараметровПоУмолчанию.Вставить("НалогообложениеНДС",
				НалогиУНФ.НалогообложениеНДС(СтруктураПараметровПоУмолчанию.Организация,
	...
	Если Параметры.Свойство("ВидОперацииЗаказНаряд") Тогда
		...
		Элементы.ОК.Заголовок= НСтр("ru = 'Создать ""Заказ-наряд""'");
	Иначе
		Элементы.ОК.Заголовок= СтрШаблон(НСтр("ru = 'Создать ""%1""'"),
			Метаданные.Документы[Параметры.ВидНовогоДокумента].Синоним);
	КонецЕсли;
...
Функция ЗаписатьПодборВХранилище() 
	...
	Для каждого СтрокаЗагрузки Из ОтобранныеЗапасы Цикл
		...
		Если ИспользуютсяНормы Тогда
			...
			СтрокаЗагрузки.Коэффициент = 1;
			СтрокаЗагрузки.Кратность = СтрокаЗагрузки.Количество;
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаЗагрузки.Номенклатура, "ФиксированнаяСтоимость,ТипНоменклатуры");
			СтрокаЗагрузки.ФиксированнаяСтоимость = ЗначенияРеквизитов.ФиксированнаяСтоимость ИЛИ (ЗначенияРеквизитов.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Работа);
			...
			Если СтрокаЗагрузки.ФиксированнаяСтоимость Тогда
				СтрокаЗагрузки.Количество = 1;
			Иначе
				СтрокаЗагрузки.Количество = ЦенообразованиеСервер.ПолучитьНормуВремениРаботы(СтруктураДанные);
			КонецЕсли; 
			
			Если ИспользуетсяВидРабот И НЕ СтрокаЗагрузки.ФиксированнаяСтоимость Тогда // Для фиксированной стоимости не используются нормы времени и цены по виду работ
				...
				СтрокаЗагрузки.Цена 	= ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДанные);
				...
				ИначеЕсли ЗначениеЗаполнено(СтрокаЗагрузки.Номенклатура.ВидСтавкиНДС) Тогда
					
					СтрокаЗагрузки.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(СтрокаЗагрузки.Номенклатура.ВидСтавкиНДС, Период);
					
				Иначе
					
					СтрокаЗагрузки.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОрганизацияДокумента.ВидСтавкиНДСПоУмолчанию, Период)
		...
		Если ИспользуютсяСпецификации Тогда
			СтрокаЗагрузки.Спецификация = Справочники.Спецификации.СпецификацияПоУмолчанию(СтрокаЗагрузки.Номенклатура,
	...
	РегистрыСведений.НастройкиПользователей.Установить(Элементы.СписокЗапасовЗапрашиватьКоличествоИЦены.Пометка, "ЗапрашиватьКоличествоИЦену");
	
	Если СтруктураНастроек.ЗапоминатьТекущуюИерархию Тогда
		
		Если ЗначениеЗаполнено(ГруппаНоменклатуры) Тогда
			
			РегистрыСведений.НастройкиПользователей.Установить(ГруппаНоменклатуры, "ГруппаОтбора");
			
		ИначеЕсли ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
			
			РегистрыСведений.НастройкиПользователей.Установить(КатегорияНоменклатуры, "ГруппаОтбора");
			
		Иначе
			
			РегистрыСведений.НастройкиПользователей.Установить(Неопределено, "ГруппаОтбора");
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	ИначеЕсли ЗначениеЗаполнено(СтруктураДанные.Номенклатура.ВидСтавкиНДС) Тогда
		...
		ДанныеСтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(СтруктураДанные.Номенклатура.ВидСтавкиНДС, ?(ЗначениеЗаполнено(СтруктураДанные.ДатаОбработки), СтруктураДанные.ДатаОбработки, ТекущаяДатаСеанса()));
		
	Иначе
		
		ДанныеСтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(СтруктураДанные.Организация.ВидСтавкиНДСПоУмолчанию, ?(ЗначениеЗаполнено(СтруктураДанные.ДатаОбработки), СтруктураДанные.ДатаОбработки, ТекущаяДатаСеанса()));
	...
	СтруктураНоменклатуры.Вставить("Цена", ?(СтруктураДанные.Свойство("ВидЦен"), ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДанные), 0));
...
Процедура ПерейтиКХарактеристикам(СтрокаСпискаТоваров, ОткрытьБезОстатка = Ложь)
	...
	МассивГрупп = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(СписокХарактеристик.Отбор, , "Номенклатура");
	Если МассивГрупп.Количество()=0 Тогда
		ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(СписокХарактеристик.Отбор.Элементы, "Номенклатура", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	Иначе
		ГруппаОтбора = МассивГрупп[0];
		ГруппаОтбора.Использование = Истина;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаОтбора, "Номенклатура", СтрокаСпискаТоваров.Номенклатура, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаОтбора, "КатегорияНоменклатуры", СтрокаСпискаТоваров.КатегорияНоменклатуры, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
	...
	Иначе 
		...
		Иначе
			...
			Если УчетЦенВРазличныхЕдиницахИзмерения = Истина Тогда
				...
				Если СтрокаСпискаТоваров.Свойство("ЦенаПоКлассификатору") Тогда
					...
					ЦенаДляКорзины = СтрокаСпискаТоваров.ЦенаПоКлассификатору;
			...
			Иначе
				...
				Если СтрокаСпискаТоваров.Свойство("Цена") Тогда
					...
					ЦенаДляКорзины = СтрокаСпискаТоваров.Цена;
		...
		ИзменитьТоварВКорзине(СтрокаСпискаТоваров, , , 1, ЦенаДляКорзины);
...
// для номенклатуры, у которой ведется учет по характеристикам, но 
...
Процедура УстановитьВидимостьПолейДинамическогоСписка(СтруктураНастроек)
	...
	ПоказОстатковТЧ = (ПоказыватьВТаблице И СтруктураНастроек.ТребуетсяПоказОстатков);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокХарактеристикОстаток",			"Видимость", (ПоказОстатковТЧ И СтруктураНастроек.ПоказыватьОстатки));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокХарактеристикРезерв",				"Видимость", (ПоказОстатковТЧ И СтруктураНастроек.ПоказыватьРезерв И НЕ ЗапросПоСкладу));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокХарактеристикСвободныйОстаток",	"Видимость", (ПоказОстатковТЧ И СтруктураНастроек.ПоказыватьСвободныйОстаток));
	
	ОстаткиХарактеристикиВидимость = (Элементы.Найти("СписокХарактеристикОстаток") <> Неопределено И Элементы.СписокХарактеристикОстаток.Видимость)
										ИЛИ (Элементы.Найти("СписокХарактеристикРезерв") <> Неопределено И Элементы.СписокХарактеристикРезерв.Видимость);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокХарактеристикГруппаОстатки",		"Видимость", ОстаткиХарактеристикиВидимость);
...
Функция ПолучитьНастройкиПодбора()
	...
	Возврат Новый Структура("ЗапрашиватьКоличествоИЦену, ПоказыватьОстатки, ПоказыватьРезерв, ПоказыватьСвободныйОстаток, ПоказыватьЦены, ТребуетсяПоказОстатков, СпособВыводаОстатков, ЗапоминатьТекущуюИерархию, ПоказыватьНаборы",
				...
				(ПолучитьФункциональнуюОпцию("ИспользоватьНаборы") И ПоказатьНаборы));
...
Процедура УстановитьТекстЗапросаСпискаЗапасов(СтруктураНастроек)
	...
	|	НоменклатураСписок.Ссылка.КатегорияНоменклатуры КАК КатегорияНоменклатуры,
	...
	|	НоменклатураСписок.ПодконтрольнаяПродукцияВЕТИС КАК ПодконтрольнаяПродукцияВЕТИС,
	...
	Если ЗапросСЦенами Тогда
		...
		|								И (Номенклатура.ТипНоменклатуры <> Значение(Перечисление.ТипыНоменклатуры.Работа) ИЛИ Номенклатура.ФиксированнаяСтоимость)) КАК ЦеныНоменклатурыСрезПоследних
	...
	|	И &УсловиеОтбораПоТипамНоменклатуры
	|	И НЕ НоменклатураСписок.ЭтоНабор
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если СтруктураНастроек.ПоказыватьНаборы Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НЕ НоменклатураСписок.ЭтоНабор", "Истина");
...
Процедура УстановитьТекстЗапросаСпискаХарактеристик(СтруктураНастроек)
	...
	Если ЗапросСОстатками Тогда
		...
		|	ЗНАЧЕНИЕ(Справочник.КатегорииНоменклатуры.ПустаяСсылка) КАК КатегорияНоменклатуры,
	...
	Иначе
		...
		|	ДеталиДанных.КатегорияНоменклатуры КАК КатегорияНоменклатуры,
		|	ДеталиДанных.Номенклатура.СтранаПроисхождения КАК СтранаПроисхождения,
		|	ЕСТЬNULL(ДеталиДанных.Номенклатура.Недействителен, ЛОЖЬ) КАК Недействителен,
		...
		|	(ВЫБРАТЬ
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ХарактеристикиНоменклатуры.Владелец, ПартииНоменклатуры.Владелец) ССЫЛКА Справочник.Номенклатура
		|				ТОГДА ЕСТЬNULL(ХарактеристикиНоменклатуры.Владелец, ПартииНоменклатуры.Владелец)
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|		КОНЕЦ КАК Номенклатура,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ХарактеристикиНоменклатуры.Владелец, ПартииНоменклатуры.Владелец) ССЫЛКА Справочник.КатегорииНоменклатуры
		|				ТОГДА ЕСТЬNULL(ХарактеристикиНоменклатуры.Владелец, ПартииНоменклатуры.Владелец)
		|			ИНАЧЕ НЕОПРЕДЕЛЕНО
		|		КОНЕЦ КАК КатегорияНоменклатуры,
		|		ЕСТЬNULL(ХарактеристикиНоменклатуры.Наименование, """") КАК Характеристика,
		...
		|		ЕСТЬNULL(ПартииНоменклатуры.Наименование, """") КАК Партия,
		|		ЕСТЬNULL(ПартииНоменклатуры.Ссылка, ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)) КАК ПартияСсылка
		|	ИЗ
		...
		|					ИЛИ ХарактеристикиНоменклатуры.Владелец = ПартииНоменклатуры.Владелец.КатегорияНоменклатуры)
		|				И (&УсловиеОтбораПоТипамНоменклатурыХарактеристики)
		|				И (&УсловиеОтбораПоТипамНоменклатурыПартии)
		|	ГДЕ
		|		&УсловиеОтбораПоСтатусамПартий
		|		И ХарактеристикиНоменклатуры.Недействителен = ЛОЖЬ) КАК ДеталиДанных";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				"&УсловиеОтбораПоТипамНоменклатурыХарактеристики",
				?(ЗапросСТипамиНоменклатуры, "(ХарактеристикиНоменклатуры.Владелец ССЫЛКА Справочник.КатегорииНоменклатуры ИЛИ ХарактеристикиНоменклатуры.Владелец.ТипНоменклатуры В(&ТипНоменклатуры))", "ИСТИНА"));
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				"&УсловиеОтбораПоСтатусамПартий",
				?(ЗапросСоСтатусамиПартий, "(ЕСТЬNULL(ПартииНоменклатуры.Ссылка, ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) ИЛИ ПартииНоменклатуры.Статус В (&СтатусПартии))", "ИСТИНА"));
	...
	МассивГрупп = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(СписокХарактеристик.Отбор, , "Номенклатура");
	Если МассивГрупп.Количество()>0 Тогда
		МассивГрупп[0].Использование = Ложь;
	КонецЕсли; 
...
Процедура СформироватьИспользуемыеТипыНоменклатуры(ПараметрыОткрытияФормы)
	...
	Если ПараметрыОткрытияФормы.Свойство("ТипНоменклатуры") 
		...
		НовыйПараметр = Новый ПараметрВыбора("Дополнительно.ОграничениеТипа", МассивОграниченияТипНоменклатуры);
		МассивПараметров.Добавить(НовыйПараметр);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНаборы") Тогда
		...
		Если НЕ (ПараметрыОткрытияФормы.Свойство("ПоказатьНаборы")
			И ПараметрыОткрытияФормы.ПоказатьНаборы = Истина) Тогда
...
// по текущей строке динамического списка
//
// Назначение - используется когда цены и остатки выводятся отдельно от табличного поля
//
// Параметры:
...
//
Процедура ОбновитьДанныеФормыНаСервере(ПрочитатьНастройкуПользователяГруппаОтбора = Ложь)
	
	СтруктураНастроек = ПолучитьНастройкиПодбора();
	
	// Иерархия номенклатуры
	Если ПрочитатьНастройкуПользователяГруппаОтбора Тогда
		
		ЗначениеОтбора = ?(СтруктураНастроек.ЗапоминатьТекущуюИерархию,
		...
		Если ЗначениеЗаполнено(ЗначениеОтбора) Тогда
			...
			Если ТипЗнч(ЗначениеОтбора) = Тип("СправочникСсылка.Номенклатура") Тогда
				...
				ГруппаНоменклатуры = ЗначениеОтбора;
				КатегорияНоменклатуры = Неопределено;
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИерархия", "Видимость", Истина);
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КатегорияИерархия", "Видимость", Ложь);
			...
			Иначе
				...
				КатегорияНоменклатуры = ЗначениеОтбора;
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИерархия", "Видимость", Ложь);
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КатегорияИерархия", "Видимость", Истина);
				
				Элементы.КатегорияИерархия.ТекущаяСтрока = КатегорияНоменклатуры;
	...
	ИначеЕсли ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
		...
		ОбновитьОтборПоГруппеДинамическихСписков(Истина, КатегорияНоменклатуры);
		
	Иначе
		
		ОбновитьОтборПоГруппеДинамическихСписков(Ложь, Неопределено);
...
//
Процедура ОбновитьОтборПоГруппеДинамическихСписков(ПрименитьОтбор, ЭлементОтбора)
	
	Если ПрименитьОтбор Тогда
		
		Если ТипЗнч(ЭлементОтбора) = Тип("СправочникСсылка.Номенклатура") Тогда
			
			УправлениеНебольшойФирмойКлиентСервер.УдалитьЭлементОтбораСписка(СписокЗапасов, "Номенклатура.КатегорияНоменклатуры");
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокЗапасов,
				"Номенклатура.Родитель", ЭлементОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);
			
		Иначе
			
			УправлениеНебольшойФирмойКлиентСервер.УдалитьЭлементОтбораСписка(СписокЗапасов, "Номенклатура.Родитель");
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокЗапасов,
				"Номенклатура.КатегорияНоменклатуры", ЭлементОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);
...
Процедура ОК(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("ОбщаяФормаФормаПодбора");
	...
	Если НЕ ЗначениеЗаполнено(РезультатЗапросаВНовыйДокумент) Тогда
		...
		Закрыть(Новый Структура("АдресТоваровВХранилище", АдресЗапасовВХранилище));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Организация = Константы.УчетПоКомпании.Компания(ПараметрыОткрытияФормы.ОрганизацияДокумента);
	КонецЕсли;
	...
	Если ПараметрыОткрытияФормы.Свойство("ВидЦен") Тогда
		...
		Если ЗначениеЗаполнено(ВидЦен) Тогда
			...
			Если Валюта <> ВалютаВидаЦен Тогда
				...
				СтруктураКурсовВалют = ЦенообразованиеСервер.ПолучитьКурсыВалют(ВалютаВидаЦен, Валюта, Период);
		...
		Элементы.СписокЗапасовЦена.Заголовок = НСтр("ru = 'Цена'") + Символы.ПС + ТекстПредставлениеЦены;
		Элементы.СписокЗапасовЦенаПоКлассификатору.Заголовок = НСтр("ru = 'Цена'") + Символы.ПС + ТекстПредставлениеЦены;
		
		Элементы.СписокХарактеристикЦена.Заголовок = НСтр("ru = 'Цена'") + Символы.ПС + ТекстПредставлениеЦены;
		Элементы.СписокХарактеристикЦенаПоКлассификатору.Заголовок = НСтр("ru = 'Цена'") + Символы.ПС + ТекстПредставлениеЦены;
	...
	ПоказатьНаборы = Ложь;
	Если ПараметрыОткрытияФормы.Свойство("ПоказатьНаборы") Тогда
		
		ПоказатьНаборы = ПараметрыОткрытияФормы.ПоказатьНаборы;
	...
	ОбновитьДанныеФормыНаСервере(Истина);
	...
	Если ПараметрыОткрытияФормы.Свойство("ПодконтрольнаяПродукцияВЕТИС") Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокЗапасов, "ПодконтрольнаяПродукцияВЕТИС", ПараметрыОткрытияФормы.ПодконтрольнаяПродукцияВЕТИС);
	КонецЕсли;
...
Процедура СписокХарактеристикВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ СтрокаСпискаТоваров = Неопределено Тогда
		...
		Иначе
			...
			ЦенаДляКорзины = ?(УчетЦенВРазличныхЕдиницахИзмерения, Элемент.ТекущиеДанные.ЦенаПоКлассификатору, Элемент.ТекущиеДанные.Цена);
			
		КонецЕсли;
		
		ИзменитьТоварВКорзине(СтрокаСпискаТоваров, Элемент.ТекущиеДанные, , 1, ЦенаДляКорзины);
...
Процедура ЗапасыИерархияПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.ЗапасыИерархия.ТекущиеДанные <> Неопределено Тогда
		...
		ГруппаНоменклатуры = Элементы.ЗапасыИерархия.ТекущиеДанные.НоменклатураСсылка;
...
&НаКлиенте
Процедура КатегорияИерархияПриАктивизацииСтроки(Элемент)
	
	ГруппаНоменклатуры = Неопределено;
	Если Элементы.КатегорияИерархия.ТекущиеДанные <> Неопределено Тогда
		
		КатегорияНоменклатуры = Элементы.КатегорияИерархия.ТекущиеДанные.КатегорияСсылка;
...
&НаКлиенте
Процедура ИерархияНоменклатуры(Команда)
	
	Если Элементы.ЗапасыИерархия.Видимость Тогда
	...
	Элементы.КатегорияИерархия.ТекущаяСтрока = Неопределено;
	
КонецПроцедуры

&НаКлиенте
Процедура ИерархияКатегорий(Команда)
	
	Если Элементы.КатегорияИерархия.Видимость Тогда
	...
	Элементы.ЗапасыИерархия.ТекущаяСтрока = Неопределено;
