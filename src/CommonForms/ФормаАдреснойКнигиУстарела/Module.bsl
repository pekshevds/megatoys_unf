...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокФизЛица.Параметры.УстановитьЗначениеПараметра("ТипыКИ", ТипыКИ);
	
	Элементы.СправочникЛиды.Видимость = Константы.ИспользоватьПодсистемуCRM.Получить();
...
Процедура ПеренестиВВыбранные(Команда)
	...
	ИначеЕсли Элементы.Справочники.ТекущаяСтраница = Элементы.СправочникФизЛица Тогда
		ТекущаяТаблица = Элементы.СписокФизЛица;
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПеренос(знач Контрагенты)
	...
	ИначеЕсли ТипЗнч(Контрагенты[0]) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ДобавитьВыбранныхФизЛиц(Контрагенты);
	КонецЕсли;
...
Процедура ДобавитьВыбранныхФизЛиц(знач ФизЛица)
	...
	|	И ФизЛицаКонтактнаяИнформация.Тип В(&ТипыКИ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
...
|	ФизЛицо";
	...
	Пока ВыборкаФизЛиц.Следующий() Цикл
		...
		ФизЛицоЕстьВВыбранных = Ложь;
		СтрокиПервыйУровень = ВыбранныеАдресаты.ПолучитьЭлементы();
		Для Каждого СтрокаПервыйУровень Из СтрокиПервыйУровень Цикл
			Если СтрокаПервыйУровень.Значение = ВыборкаФизЛиц.ФизЛицо Тогда
				ФизЛицоЕстьВВыбранных = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ФизЛицоЕстьВВыбранных Тогда
			Продолжить;
		КонецЕсли;
	
		СтрокаПервыйУровень = СтрокиПервыйУровень.Добавить();
		СтрокаПервыйУровень.Пометка = Истина;
		СтрокаПервыйУровень.Значение = ВыборкаФизЛиц.ФизЛицо;
		СтрокаПервыйУровень.ПредставлениеЗначения = ВыборкаФизЛиц.ФизЛицоПредставление;
		СтрокаПервыйУровень.ИндексКартинки = 1;
		
		СтрокиВторойУровень = СтрокаПервыйУровень.ПолучитьЭлементы();
		
		ВыборкаКИ_ФизЛиц = ВыборкаФизЛиц.Выбрать();
		КИФизЛицаНезаполнено = Ложь;
		
		Пока ВыборкаКИ_ФизЛиц.Следующий() Цикл
			
			Если ВыборкаКИ_ФизЛиц.ВидКИ = Неопределено Тогда
				КИФизЛицаНезаполнено = Истина;
				Продолжить;
			КонецЕсли;
			
			СтрокаВторойУровень = СтрокиВторойУровень.Добавить();
			СтрокаВторойУровень.Пометка = Истина;
			СтрокаВторойУровень.Значение = ВыборкаКИ_ФизЛиц.ВидКИ;
			СтрокаВторойУровень.ПредставлениеЗначения = ВыборкаКИ_ФизЛиц.ЗначениеКИ;
			СтрокаВторойУровень.ИндексКартинки = ВыборкаКИ_ФизЛиц.ИндексКартинки;
			
		КонецЦикла;
				
		Если КИФизЛицаНезаполнено Тогда
			СтрокаПервыйУровень.Пометка = Ложь;
		КонецЕсли;
...
Функция ПреобразоватьДеревоАдресатовВТаблицуЗначений(Дерево)
	...
	ТаблицаАдресатов.Колонки.Добавить("Контакт", Новый ОписаниеТипов("СправочникСсылка.Контрагенты,СправочникСсылка.КонтактныеЛица,СправочникСсылка.Лиды,Строка,СправочникСсылка.КонтактыЛидов, СправочникСсылка.ФизическиеЛица"));
	...
	Для Каждого СтрокаПервыйУровень Из СтрокиПервыйУровень Цикл
		...
		Для Каждого СтрокаВторойУровень Из СтрокиВторойУровень Цикл
			...
			ИначеЕсли (ТипЗнч(СтрокаВторойУровень.Значение) = Тип("СправочникСсылка.Контрагенты")
				...
				ИЛИ ТипЗнч(СтрокаВторойУровень.Значение) = Тип("СправочникСсылка.ФизическиеЛица"))
