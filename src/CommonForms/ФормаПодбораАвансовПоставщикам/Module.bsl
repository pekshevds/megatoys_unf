...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗаказВШапке И Контрагент.ВестиРасчетыПоЗаказам Тогда // заказ в шапке
		...
		НоваяСтрока.ВсегоРасч = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(
			...
			Если ТипЗнч(НоваяСтрока.Заказ) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
				СтрокаЗаказаПокупателя = СписокЗаказовПокупателей.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаЗаказаПокупателя, НоваяСтрока);
				СтрокаЗаказаПокупателя.Заказ = Параметры.Заказ;
			КонецЕсли;
	...
	ИначеЕсли ЕстьЗаказ И Контрагент.ВестиРасчетыПоЗаказам Тогда // заказ в табличной части
		...
		Если Параметры.Свойство("Заказ")
		   И ТипЗнч(Параметры.Заказ) = Тип("Массив") Тогда
			...
			ТаблицаЗаказовПокупателя = СписокЗаказовПокупателей.Выгрузить();
			Для каждого ЭлементМассива Из Параметры.Заказ Цикл
				...
				СтрокаЗаказов.ВсегоРасч = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(
					...
					Если ТипЗнч(ЭлементМассива.Заказ) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
						СтрокаЗаказаПокупателя = ТаблицаЗаказовПокупателя.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаЗаказаПокупателя, СтрокаЗаказов);
						СтрокаЗаказаПокупателя.Заказ = ЭлементМассива.Заказ;
					КонецЕсли;
					
				КонецЦикла;
				
			ТаблицаЗаказовПокупателя.Свернуть("Заказ", "Всего, ВсегоРасч");
			СписокЗаказовПокупателей.Загрузить(ТаблицаЗаказовПокупателя);
...
// Предоплата. Рассчитывает курс и кратность.
//
&НаКлиенте
...
Функция ПолучитьДанныеДокументПриИзменении(Документ)
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда 
		СуммаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "Оплаты").Итог("СуммаРасчетов");
	...
	ИначеЕсли ЭтоНакладныеИлиКорректировки Тогда
		...
		СтруктураДанные.Вставить("СуммаРасчетов", СуммаДокумента);
	Иначе
		СуммаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "РасшифровкаПлатежа").Итог("СуммаРасчетов");
		СтруктураДанные.Вставить("СуммаРасчетов", СуммаДокумента);
	КонецЕсли;
...
Процедура ЗаполнитьПредоплату()
	...
	ТаблицаЗаказов = СписокЗаказов.Выгрузить();
	ТаблицаЗаказовПокупателей = СписокЗаказовПокупателей.Выгрузить();
	...
	|		ВЫБОР
	|			КОГДА РасчетыСПокупателямиОстатки.Заказ = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ РасчетыСПокупателямиОстатки.Заказ
	|		КОНЕЦ,
	|		ЕСТЬNULL(-РасчетыСПокупателямиОстатки.СуммаОстаток, 0),
	|		ЕСТЬNULL(-РасчетыСПокупателямиОстатки.СуммаВалОстаток, 0)
	|	ИЗ
	...
	|					И Контрагент = &Контрагент
	|					И Договор = &Договор
	|					И ТИПЗНАЧЕНИЯ(Документ) = ТИП(Документ.ОтчетКомиссионера)) КАК РасчетыСПокупателямиОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ДвиженияРасчетыСПокупателями.Договор,
	|		ДвиженияРасчетыСПокупателями.Документ,
	|		ДвиженияРасчетыСПокупателями.Документ.Дата,
	|		ВЫБОР
	|			КОГДА ДвиженияРасчетыСПокупателями.Заказ = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ ДвиженияРасчетыСПокупателями.Заказ
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ДвиженияРасчетыСПокупателями.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -ЕСТЬNULL(ДвиженияРасчетыСПокупателями.Сумма, 0)
	|			ИНАЧЕ ЕСТЬNULL(ДвиженияРасчетыСПокупателями.Сумма, 0)
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ДвиженияРасчетыСПокупателями.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -ЕСТЬNULL(ДвиженияРасчетыСПокупателями.СуммаВал, 0)
	|			ИНАЧЕ ЕСТЬNULL(ДвиженияРасчетыСПокупателями.СуммаВал, 0)
	|		КОНЕЦ
	|	ИЗ
	|		РегистрНакопления.РасчетыСПокупателями КАК ДвиженияРасчетыСПокупателями
	|	ГДЕ
	|		ДвиженияРасчетыСПокупателями.Регистратор = &Ссылка
	|		И ДвиженияРасчетыСПокупателями.Период <= &Период
	|		И ДвиженияРасчетыСПокупателями.Организация = &Организация
	|		И ДвиженияРасчетыСПокупателями.Контрагент = &Контрагент
	|		И ТИПЗНАЧЕНИЯ(ДвиженияРасчетыСПокупателями.Документ) = ТИП(Документ.ОтчетКомиссионера)) КАК РасчетыСПоставщикамиОстатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
		...
		Если ТипЗнч(ВыборкаРезультатаЗапроса.Документ) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
			ЗаказПокупателя = ?(ВыборкаРезультатаЗапроса.Заказ = Неопределено, Документы.ЗаказПокупателя.ПустаяСсылка(), ВыборкаРезультатаЗапроса.Заказ);
			НайденнаяСтрока = ТаблицаЗаказовПокупателей.Найти(ЗаказПокупателя, "Заказ");
		Иначе
	...
	Если ВыдаватьСообщение Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Таблица документов к зачету не заполнена, т.к. долг по документу = 0.'");
		Сообщение.Сообщить();
		ВыдаватьСообщение = Ложь;
	КонецЕсли;
	
	Если ВыдаватьСообщение И Предоплата.Количество() = 0 И СуммаДокумента = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Таблица документов к зачету не заполнена, т.к. сумма документа = 0.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ЗаполнитьАвансы()
	...
	|		ЕСТЬNULL(-РасчетыСПокупателямиОстатки.СуммаОстаток, 0),
	|		ЕСТЬNULL(-РасчетыСПокупателямиОстатки.СуммаВалОстаток, 0),
	|		ЕСТЬNULL(-РасчетыСПокупателямиОстатки.СуммаРегОстаток, 0)
	|	ИЗ
	|		РегистрНакопления.РасчетыСПокупателями.Остатки(
	|				,
	|				Организация = &Организация
	...
	|		ВЫБОР
	|			КОГДА ДвиженияДокументаРасчетыСПокупателями.Заказ = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ ДвиженияДокументаРасчетыСПокупателями.Заказ
	|		КОНЕЦ,
	|		ВЫБОР
	...
	|		И ТИПЗНАЧЕНИЯ(ДвиженияДокументаРасчетыСПокупателями.Документ) = ТИП(Документ.ОтчетКомиссионера)
	|		И ДвиженияДокументаРасчетыСПокупателями.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)) КАК РасчетыСПоставщикамиОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСПоставщикамиОстатки.Документ,
	|	РасчетыСПоставщикамиОстатки.Заказ,
	...
	|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаРегОстаток, 0) КАК СуммаРегОстаток
	|	ИЗ
	|		ВременнаяТаблицаРасчетыСПоставщикамиОстатки КАК РасчетыСПоставщикамиОстатки
