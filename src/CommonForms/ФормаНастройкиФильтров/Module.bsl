...
&НаКлиенте
Процедура ВыбранныеОтборыПриИзменении(Элемент)
	
	Если ВыбранныеОтборы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПоследняяСтрока = ВыбранныеОтборы[ВыбранныеОтборы.Количество() -1];
	
	Если ПоследняяСтрока <> Неопределено И ПоследняяСтрока.Заголовок = "" Тогда
		ВыбранныеОтборы.Удалить(ВыбранныеОтборы.Количество() -1);
	КонецЕсли;
