...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьПодсказки = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить("ФормаСпискаОтчетов", 
		"ВидимостьПодсказкиСкрытыеОтчеты", Истина);
...
&НаКлиенте
Процедура ДекорацияПредупреждениеЗакрытьНажатие(Элемент)
	
	ДекорацияПредупреждениеЗакрытьНажатиеСервер();
	
КонецПроцедуры

&НаСервере
Процедура ДекорацияПредупреждениеЗакрытьНажатиеСервер()

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПредупреждения", "Видимость", Ложь);
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить("ФормаСпискаОтчетов", "ВидимостьПодсказкиСкрытыеОтчеты", Ложь);
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий.Вставить("ИзПанелиОтчетов", КэшЗначений.ПутьКПодсистеме)
	Иначе
		...
		КонецЕсли;
		Комментарий.Вставить("ПутьКПодсистеме", КэшЗначений.ПутьКПодсистеме);
