...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФормаПараметры.Вставить("ЭтоМобильныйКлиент", ОбщегоНазначения.ЭтоМобильныйКлиент());
	
	НастройкиОтчета = ОтчетыКлиентСервер.НастройкиОтчетаПоУмолчанию();
	НастройкиОтчета.Вставить("Контекстный", (Параметры.Свойство("Контекст") И ЗначениеЗаполнено(Параметры.Контекст)) 
		ИЛИ (Параметры.Свойство("ОтборРасшифровки") И Параметры.ОтборРасшифровки.Количество()>0));
	...
	НастройкиОтчета.Вставить("Внешний", ТипЗнч(НастройкиОтчета.ОтчетСсылка) = Тип("Строка") 
		ИЛИ ТипЗнч(НастройкиОтчета.ОтчетСсылка) = Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки"));
	...
	НастройкиОтчета.Вставить("ФормироватьНепосредственно", НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный 
		ИЛИ (Параметры.Свойство("ФормироватьНепосредственно") И Параметры.ФормироватьНепосредственно));
	...
	Если НЕ РежимРасшифровки И Параметры.Свойство("Вариант") И ТипЗнч(Параметры.Вариант) = Тип("НастройкиКомпоновкиДанных") Тогда
		// Обновление настроек при открытии в новом окне
	...
	// Проверка наличия варианта в списке выбора
	...
	Для каждого Макет Из БиблиотекаМакетовОформленияКомпоновкиДанных Цикл
		Если Макет = БиблиотекаМакетовОформленияКомпоновкиДанных.ОформлениеОтчетовЗеленый Тогда
			ПредставлениеМакета = НСтр("ru = 'Лес'");
		ИначеЕсли Макет = БиблиотекаМакетовОформленияКомпоновкиДанных.Основной Тогда
			ПредставлениеМакета = НСтр("ru = 'Классический'");
		Иначе
			ПредставлениеМакета = Макет.Представление;
		КонецЕсли; 
		Элементы.МакетОформления.СписокВыбора.Добавить(Макет.Имя, ПредставлениеМакета);
	КонецЦикла;
	Элементы.МакетОформления.СписокВыбора.СортироватьПоПредставлению();
	
	// Экспорт контактов в рассылки
	ЗаполнитьДанныеИОформлениеДляЭкспорта();
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если СуффиксФормыИсточника = ВРег("ФормаНастроекОтчета")
				...
				Или СуффиксФормыИсточника = ВРег("УдалитьФормаВариантаОтчетаУНФ")
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	ИначеЕсли ЗначениеЗаполнено(Вариант) И Вариант.Пользовательский Тогда
		СохраненныеНастройки = Вариант.Настройки.Получить();
		Если ТипЗнч(СохраненныеНастройки) = Тип("НастройкиКомпоновкиДанных") Тогда
			Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(СохраненныеНастройки);
		КонецЕсли; 
	...
	НастройкиОтчета.Вставить("КлючЗамеров", КлючЗамеров);
	// СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаЗаПериод() Тогда
		Диалог = Новый ДиалогРедактированияСтандартногоПериода;
		Диалог.Период = Период;
		Диалог.Показать(Оповещение);
	ИначеЕсли НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаНаДату() Тогда
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	// Экспорт контактов отчета в рассылки
	МожноВключитьВРассылку = ТипЗнч(Данные.Значение) = Тип("СправочникСсылка.Контрагенты") ИЛИ ТипЗнч(Данные.Значение) = Тип("СправочникСсылка.КонтактныеЛица");
	Если Данные.ЭтоСсылка И МожноВключитьВРассылку Тогда
		ПодменюРассылки = Новый СписокЗначений;
		ПодменюРассылки.Добавить("СозданиеЗадачиПоРезультатамОтчета", НСтр("ru = 'Создать задачу'")); 
		ПодменюРассылки.Добавить("ДобавлениеВЕмейлРассылку", НСтр("ru = 'Добавить в рассылку E-mail'")); 
		ПодменюРассылки.Добавить("ДобавлениеВСмсРассылку",НСтр("ru = 'Добавить в рассылку SMS'"));
		Если ИспользоватьМассовыеРассылкиИнтеграция Тогда
			ПодменюРассылки.Добавить("ЭкспортВСервис", НСтр("ru = 'Экспортировать в "+Строка(СервисМассовойРассылки)+"'"));
		КонецЕсли;
		ДополнительныеДействия.Добавить(ПодменюРассылки, НСтр("ru = 'Отправить'"));
	КонецЕсли;
	...
	Если ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда
		...
		Для каждого Поле Из Поля Цикл
			Если Поле.Значение = Null Тогда
				Продолжить;
			КонецЕсли;
			Если Результат.Получить(Поле.Поле) = Неопределено Тогда
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыполненноеДействие = "ДобавлениеВЕмейлРассылку" Тогда
		ДанныеРасшифровки = ДанныеРасшифровкиВыделеннойОбласти();
		МассивКонтактов = МассивДанныхДляЭкспорта(ДанныеРасшифровки);
		МассовыеРассылкиКлиент.ДобавитьКонтактыВМассовыеРассылки(МассивДанныхДляЭкспорта(ДанныеРасшифровки), ВидСвязиЕмейл);
	ИначеЕсли ВыполненноеДействие = "ДобавлениеВСмсРассылку" Тогда
		ДанныеРасшифровки = ДанныеРасшифровкиВыделеннойОбласти();
		МассивКонтактов = МассивДанныхДляЭкспорта(ДанныеРасшифровки);
		МассовыеРассылкиКлиент.ДобавитьКонтактыВМассовыеРассылки(МассивКонтактов, ВидСвязиСмс);
	ИначеЕсли ВыполненноеДействие = "СозданиеЗадачиПоРезультатамОтчета" Тогда
		ДанныеРасшифровки = ДанныеРасшифровкиВыделеннойОбласти();
		МассивКонтактов = МассивДанныхДляЭкспорта(ДанныеРасшифровки);
		СоздатьЗадачу(МассивКонтактов);
	ИначеЕсли ВыполненноеДействие = "ЭкспортВСервис" Тогда
		ДанныеРасшифровки = ДанныеРасшифровкиВыделеннойОбласти();
		МассивКонтактов = МассивДанныхДляЭкспорта(ДанныеРасшифровки);
...
Процедура ОткрытьОтчетРасшифровку(Расшифровка, Данные, Стр)
	...
	Для каждого ЭлементОтбора Из Данные.Настройки.Отбор.Элементы Цикл
		Если ТипЗнч(ЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") 
			ИЛИ НЕ ЭлементОтбора.Использование
			ИЛИ ТипЗнч(ЭлементОтбора.ЛевоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли; 
		Если ЭлементОтбора.ВидСравнения <> ВидСравненияКомпоновкиДанных.Равно 
			...
			Продолжить;
		КонецЕсли;
		ПараметрыОткрытия.ОтборРасшифровки.Вставить(Строка(ЭлементОтбора.ЛевоеЗначение), ЭлементОтбора.ПравоеЗначение);
	КонецЦикла; 
	ПараметрыОткрытия.Вставить("Отбор", Новый Структура);
	Для каждого Параметр Из Данные.Настройки.ПараметрыДанных.Элементы Цикл
		Если Параметр.Использование = Ложь ИЛИ Параметр.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыОткрытия.Отбор.Вставить(Строка(Параметр.Параметр), Параметр.Значение);
	КонецЦикла; 
...
Процедура ПриИзмененииВидаДиаграммы()
	...
	ЭлементСтруктуры = ЭлементСтруктурыДиаграмма(Отчет.КомпоновщикНастроек.Настройки.Структура, ЗначениеЗаполнено(
		ВидДиаграммы));
...
Процедура РазвернутьДоПриИзменении(Элемент)
	...
	СвернутьГруппировки();
	РазвернутьДоПриИзмененииСервер();
...
Функция СписокДополнительныхКолонок()
	...
	Для каждого Показатель Из Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		Если НастройкиОтчета.Свойство("ТолькоРесурсыОстатков") И НастройкиОтчета.ТолькоРесурсыОстатков Тогда
			...
			Если НЕ ПолеСхемы=Неопределено И ТипЗнч(ПолеСхемы) <> Тип("ВычисляемоеПолеСхемыКомпоновкиДанных") И НЕ ПолеСхемы.Роль.ТипОстатка=ТипОстаткаКомпоновкиДанных.КонечныйОстаток Тогда
				Продолжить;
			КонецЕсли; 
		КонецЕсли; 
...
Процедура ПриИзмененииИерархииГруппировки(ИмяЭлемента)
	...
	Если НЕ Стр.ДополнительныеПараметры.Свойство("ТипГруппировки") Тогда
		Стр.ДополнительныеПараметры.Вставить("ТипГруппировки", ТипГруппировкиКомпоновкиДанных.Иерархия);
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
...
Процедура ПриИзмененииПоказателяСервер(Идентификатор, ИмяЭлемента)
	...
	ОбновитьВидимостьДекорацииАктуальностиСебестоимости();
...
&НаКлиенте
Процедура ЭкспортироватьКонтактыОтчета(Команда)
	
	Если КонтактыОтчета.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Нет данных для экспорта'"));
		Возврат;
	КонецЕсли;
	
	МассивКонтактов = Новый Массив;
	Для Каждого Контакт Из КонтактыОтчета Цикл
...
&НаКлиенте
Процедура СоздатьЗадачуПоРезультатамОтчета(Команда)
	Если КонтактыОтчета.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Нет данных для экспорта'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВключитьРезультатВЕмейлРассылку(Команда)
	...
	МассовыеРассылкиКлиент.ДобавитьКонтактыВМассовыеРассылки(МассивКонтактов,ПредопределенноеЗначение("Перечисление.ВидыКаналовСвязи.Email"));

КонецПроцедуры

&НаКлиенте
Процедура ВключитьРезультатВСмсРассылку(Команда)
	...
	МассовыеРассылкиКлиент.ДобавитьКонтактыВМассовыеРассылки(МассивКонтактов, ПредопределенноеЗначение("Перечисление.ВидыКаналовСвязи.SMS"));

КонецПроцедуры

&НаКлиенте
Процедура ОткрытьВНовомОкнеВеб(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Вариант", Отчет.КомпоновщикНастроек.Настройки);
	ПараметрыОткрытия.Вставить("Ключ", Вариант);
	ОткрытьФорму(НастройкиОтчета.ПолноеИмя + ".Форма", ПараметрыОткрытия, , Истина, Неопределено);
...
Процедура ПериодПриИзмененииСервер()
	...
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта)
		И НЕ НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаБезПериода() Тогда 
		ХранилищеСистемныхНастроек.Сохранить(
		"ПериодОтчета",
		НастройкиОтчета.ИдентификаторВарианта + ?(НастройкиОтчета.Контекстный, "_Контекстный", ""),
		Период);
	КонецЕсли;
	
	// Обновление динамических периодов
	Если НастройкиОтчета.ПоказыватьГруппуСтрокиНаФормеОтчета И ДинамическийПериодВГруппировках() Тогда
		Если ПолеГруппировкиКолонок = ИмяГруппировкиДинамическийПериод() 
			ИЛИ ПолеСравнения = ИмяГруппировкиДинамическийПериод() Тогда
			ОбновитьГруппировкиИРеквизиты(, Истина);
		Иначе
			ОбновитьГруппировкиИРеквизиты(Истина);
		КонецЕсли; 
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если Элементы.ПредставлениеПериода.Видимость Тогда
		ПредставлениеПериода = ОтчетыУНФКлиентСервер.ПредставлениеПериодаВЗаголовке(
...
&НаСервере
Процедура ОбновитьВидимостьДекорацииАктуальностиСебестоимости()
	
	ПоляВыведены = Ложь;
	Если НЕ НеАнализироватьСебестоимость 
		И НастройкиОтчета.Свойство("ПоляСебестоимости")
		И НЕ ПустаяСтрока(НастройкиОтчета.ПоляСебестоимости) Тогда
		Поля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НастройкиОтчета.ПоляСебестоимости, ",", , Истина);
		Для каждого Поле Из Поля Цикл
			СтрокаНастроек = СтрокаОписанияПоля(ТипПоказатель(), Поле, ПоляНастроек);
			Если СтрокаНастроек = Неопределено ИЛИ СтрокаНастроек.Скрыт Тогда
				Продолжить;
			КонецЕсли;
			Для каждого КлючИЗначение Из СтрокаНастроек.Реквизиты Цикл
				Если ЭтотОбъект[КлючИЗначение.Ключ] Тогда
					ПоляВыведены = Истина;
					Прервать;
				КонецЕсли; 
			КонецЦикла; 
			Если ПоляВыведены Тогда
				Прервать;
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли; 
	...
	Если ПоляВыведены Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаАктуальностьСебестоимости", "Видимость", СебестоимостьНеактуальна());
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаАктуальностьСебестоимости", "Видимость", Ложь);
	КонецЕсли; 
	
КонецПроцедуры

&НаСервере
Функция СебестоимостьНеактуальна()
	
	Если Период.ДатаНачала >= НачалоМесяца(ТекущаяДатаСеанса())  Тогда
		// За текущий месяц анализ не требуется
		Возврат Ложь;
	КонецЕсли; 
	
	// Период запроса
	КонецПериода = Период.ДатаОкончания;
	Если КонецПериода > ТекущаяДатаСеанса() ИЛИ НЕ ЗначениеЗаполнено(КонецПериода) Тогда
		КонецПериода = КонецМесяца(ДобавитьМесяц(ТекущаяДатаСеанса(), -1));
	КонецЕсли; 
	Если НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаНаДату() Тогда
	...
	СтрокиФильтра = НаложенныеФильтры.НайтиСтроки(СтруктураОтбора);
	Для каждого СтрокаФильтра Из СтрокиФильтра Цикл
		Если ТипЗнч(СтрокаФильтра.Значение) = Тип("СписокЗначений") Тогда
			Для каждого ЭлементФильтра Из СтрокаФильтра.Значение Цикл
				Если НЕ ЗначениеЗаполнено(ЭлементФильтра.Значение) Тогда
					Продолжить;
				КонецЕсли;
				Если СтрокаФильтра.ПризнакИсключения Тогда
					ИсключенныеОрганизации.Добавить(ЭлементФильтра.Значение);
				Иначе
					Организации.Добавить(ЭлементФильтра.Значение);
				КонецЕсли; 
			КонецЦикла;
		ИначеЕсли ТипЗнч(СтрокаФильтра.Значение) = Тип("СправочникСсылка.Организации") И ЗначениеЗаполнено(СтрокаФильтра.Значение) Тогда
			Если СтрокаФильтра.ПризнакИсключения Тогда
				ИсключенныеОрганизации.Добавить(СтрокаФильтра.Значение);
			Иначе
				Организации.Добавить(СтрокаФильтра.Значение);
			КонецЕсли; 
		КонецЕсли;
	КонецЦикла;
	...
	НеактуальныеДанные = ЗакрытиеМесяца.НеактуальныеДанныеСебестоимостиВПериоде(НачалоПериода, КонецПериода, Организации);
	Если НеактуальныеДанные = Неопределено Тогда
		ПервыйНеактуальныйМесяц = '0001-01-01';
		ПерваяНеактуальнаяОрганизация = Справочники.Организации.ПустаяСсылка();
		Возврат Ложь;
	Иначе
		ПервыйНеактуальныйМесяц = НеактуальныеДанные.Месяц;
		ПерваяНеактуальнаяОрганизация = НеактуальныеДанные.Организация;
		Возврат Истина;
	КонецЕсли; 
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	Если ПолнаяИнициализация Тогда
		...
		Элементы.МакетОформления.Видимость = НастройкиОтчета.РазрешитьРедактироватьСКД;
		...
		Элементы.ГруппаСравнение.Видимость = НастройкиОтчета.ИспользоватьСравнение И НастройкиОтчета.РежимПериода=ОтчетыУНФКлиентСервер.РежимПериодаЗаПериод();
		...
		Элементы.ЗапомнитьНастройки.Видимость = НастройкиОтчета.ИспользуетХранилищеВариантов;
		...
		Элементы.ПредставлениеПериода.Видимость = НЕ (НастройкиОтчета.РежимПериода=ОтчетыУНФКлиентСервер.РежимПериодаБезПериода());
		Элементы.ПредставлениеПериода.Заголовок = ?(НастройкиОтчета.РежимПериода=ОтчетыУНФКлиентСервер.РежимПериодаНаДату(), 
		НСтр("ru = 'На дату'"), 
		НСтр("ru = 'Период'"));
		
		Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
			Элементы.НовоеОкно.Видимость = Ложь;
		Иначе
			Элементы.ОткрытьВНовомОкнеВеб.Видимость = Ложь;
		КонецЕсли; 
	...
	Если ПолнаяИнициализация Тогда
		...
		ИЛИ Элементы.МакетОформления.Видимость;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭлементСтруктурыДиаграмма(ЭлементыСтруктуры, Создать = Ложь)
...
&НаСервере
Функция ЭлементСтруктурыГруппировкаТаблица(ЭлементыСтруктуры)
	
	Для Каждого ЭлементСтруктуры Из ЭлементыСтруктуры Цикл
		Если ТипЗнч(ЭлементСтруктуры) = Тип("ТаблицаКомпоновкиДанных") Или ТипЗнч(ЭлементСтруктуры) = Тип(
			"ГруппировкаКомпоновкиДанных") Тогда
			Возврат ЭлементСтруктуры;
		КонецЕсли;
	КонецЦикла;
	Возврат Неопределено;
...
Функция НестандартнаяДиаграмма(Настройки)
	...
	ЭлементДиаграмма = ЭлементСтруктурыДиаграмма(Настройки.Структура);
...
Процедура ПроверитьСовместимостьВарианта()
	...
	// Диаграммы
	Если НЕ ЭлементСтруктурыДиаграмма(Отчет.КомпоновщикНастроек.Настройки.Структура)=Неопределено Тогда
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		Если ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") 
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				Если Строка(Поле.Поле) = ИмяГруппировкиДинамическийПериод() Тогда
					Возврат Истина;
				КонецЕсли; 
			КонецЦикла; 
		ИначеЕсли ТипЗнч(Элемент) = Тип("ДиаграммаКомпоновкиДанных") Тогда
...
Процедура ОпределитьПараметрыОтчетаПоУмолчанию()
	...
	Если НЕ НастройкиОтчета.Свойство("РежимПериода") Тогда
		НастройкиОтчета.Вставить("РежимПериода", ОтчетыУНФ.РежимОтображенияПериода(ОтчетОбъект));
	КонецЕсли; 
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	ПараметрыПериодов = ОтчетыУНФ.ПараметрыПериодовСКД(ОтчетОбъект);
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		"УдалитьФормаОтчетаУНФ.УстановитьПараметрыИОтборыРасшифровки",
...
Функция ПризнакИспользованияПериодичности(ОтчетОбъект)
	...
	Если НЕ НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаЗаПериод() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ЭтоОтчетУНФ", Истина);
...
Процедура Сформировать()
	...
	Если ВыполнятьЗамеры Тогда
		...
		КонецЕсли; 
		Комментарий = ИмяОперации + " (формирование)";
	...
	ЗаголовокОтчета = ОтчетыУНФКлиентСервер.Заголовок(
...
&НаКлиенте
Процедура ОперацииПослеФормирования() Экспорт
	...
	// Формирование списка контактов для экспорта в массовые рассылки
	СформироватьКонтактыДляЭкспорта();
...
Процедура СвернутьГруппировки()
	...
	Если НЕ ПустаяСтрока(РазвернутьДо) Тогда
		...
		Если РазвернутьДо="*" Тогда
			
			ОтчетТабличныйДокумент.ПоказатьУровеньГруппировокСтрок(9999);
			
		Иначе
			
			КоличествоГруппировок = Элементы.РазвернутьДо.СписокВыбора.Количество()-2;
	...
	Для каждого ИмяПараметра Из ИменаПериодическихПараметров Цикл
		...
		Если ПараметрПериод <> Неопределено Тогда
			Если ТипЗнч(ПараметрПериод.Значение) = Тип("СтандартныйПериод") Тогда
				...
				Если НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаНаДату()
	...
	Для каждого Элемент Из Структура Цикл
		...
		Если ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") 
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				...
				Если ИмяПоля <> ИмяГруппировкиДинамическийПериод() 
					И ИмяПоля <> ИмяГруппировкиМесяцГода() 
					И ИмяПоля <> ИмяГруппировкиДеньМесяцГода() 
					И ИмяПоля <> ИмяГруппировкиНеделяГода() Тогда
...
Процедура ОбновитьНастройкиНаФорме()
	...
	ВосстановитьДинамическиеПериодыРекурсивно(Отчет.КомпоновщикНастроек.Настройки.Структура);
	...
	// Макет оформления
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) Тогда
		МакетОформленияИзНастроек = ХранилищеСистемныхНастроек.Загрузить("МакетОформления", Строка(НастройкиОтчета.ИдентификаторВарианта));
	Иначе
		МакетОформленияИзНастроек = Неопределено;
	КонецЕсли; 
	Параметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("МакетОформления"));
	Если ТипЗнч(МакетОформленияИзНастроек)=Тип("Строка") Тогда
		МакетОформления = МакетОформленияИзНастроек;
		Если Параметр<>Неопределено Тогда
			Если ПустаяСтрока(МакетОформленияИзНастроек) Тогда
				Параметр.Использование = Ложь;
			Иначе
				Параметр.Использование = Истина;
				Параметр.Значение = МакетОформленияИзНастроек;
			КонецЕсли; 
		КонецЕсли; 
	ИначеЕсли Параметр<>Неопределено И Параметр.Использование Тогда
		МакетОформления = Параметр.Значение;
	Иначе
		МакетОформления = "";
	КонецЕсли;
...
Процедура ОбновитьНастройкиДиаграммыНаФорме()
	...
	Если ИмяТочки = ИмяГруппировкиДинамическийПериод() Тогда
		...
		КонецЕсли; 
		Если ТипЗнч(ИмяТочки) = Тип("ПеречислениеСсылка.Периодичность") И ЗначениеЗаполнено(ИмяТочки) Тогда
			СтрокаДлительностьПериода = ОбщегоНазначения.ИмяЗначенияПеречисления(ИмяТочки);
			ИмяТочки = "Период" + СтрокаДлительностьПериода;
			ЭлементДиаграмма.Точки[0].ПоляГруппировки.Элементы[0].Поле = Новый ПолеКомпоновкиДанных(ИмяТочки);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ОбновитьНастройкиДиаграммы()
	...
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	ЭлементДиаграмма = ЭлементСтруктурыДиаграмма(Настройки.Структура, Истина);
	Если ЭлементДиаграмма = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ДиаграммаТочки) = Тип("ПеречислениеСсылка.Периодичность") Тогда
		ИмяГруппировкиТочки = ИмяГруппировкиДинамическийПериод();
...
Функция ОбновитьДополнениеГруппировки(Группировка, Схема)
	...
	Если ИмяПоля = ИмяГруппировкиДинамическийПериод() Тогда
		...
		Если ПараметрСхемыПериодичность <> Неопределено И ЗначениеЗаполнено(ПараметрСхемыПериодичность.Значение) Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяПоля = ИмяГруппировкиМесяцГода() Тогда
		УстановитьТипДополнения(ПолеГруппировки, ТипДополненияПериодаКомпоновкиДанных.Месяц);
	ИначеЕсли ИмяПоля = ИмяГруппировкиДеньМесяцГода() Тогда
		УстановитьТипДополнения(ПолеГруппировки, ТипДополненияПериодаКомпоновкиДанных.День);
	Иначе
		ПолеСхемы = ОтчетыУНФ.ПолеСхемы(Схема, ИмяПоля);
		Если ТипЗнч(ПолеСхемы) <> Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") ИЛИ ПолеСхемы.Роль.НомерПериода = 0 Тогда
			УстановитьТипДополнения(ПолеГруппировки, ТипДополненияПериодаКомпоновкиДанных.БезДополнения);
		Иначе
...
Процедура ОбновитьПериодНаФорме()
	...
	Если ЭтоПерваяЗагрузкаВарианта() Тогда
		Период = ОтчетыУНФ.ПериодИзПараметровОтчета(Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ЭтоПерваяЗагрузкаВарианта()
	
	Возврат ПоляНастроек.ПолучитьЭлементы().Количество() = 0;
...
&НаСервере
Процедура ВосстановитьДинамическиеПериодыРекурсивно(ЭлементыСтруктуры)
	
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		Если ТипЗнч(Элемент) = Тип("ТаблицаКомпоновкиДанных") Тогда
			ВосстановитьДинамическиеПериодыРекурсивно(Элемент.Строки);
			ВосстановитьДинамическиеПериодыРекурсивно(Элемент.Колонки);
		ИначеЕсли ТипЗнч(Элемент) = Тип("ДиаграммаКомпоновкиДанных") Тогда
			ВосстановитьДинамическиеПериодыРекурсивно(Элемент.Серии);
			ВосстановитьДинамическиеПериодыРекурсивно(Элемент.Точки);
		ИначеЕсли ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") 
			ИЛИ ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных")
			ИЛИ ТипЗнч(Элемент) = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Тогда
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				Если ТипЗнч(Поле) = Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
					Если Поле.Поле = Новый ПолеКомпоновкиДанных(ИмяГруппировкиДинамическийПериод()) Тогда
						ПараметрСхемыПериодичность = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Периодичность"));
				...
				КонецЕсли; 
			КонецЦикла;
			ВосстановитьДинамическиеПериодыРекурсивно(Элемент.Структура);
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") 
			...
			Иначе
				...
				Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
					Если ТипЗнч(Поле)<>Тип("ПолеГруппировкиКомпоновкиДанных") ИЛИ Поле.Использование Тогда
						Продолжить;
					КонецЕсли;
					ДоступноеПоле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляГруппировок.НайтиПоле(Поле.Поле);
					Если ДоступноеПоле=Неопределено Тогда
						ПоляКУдалению.Добавить(Поле);
					КонецЕсли; 
				КонецЦикла; 
				Для каждого Поле Из ПоляКУдалению Цикл
					Элемент.ПоляГруппировки.Элементы.Удалить(Поле);
				КонецЦикла;
				ИсправитьГруппировкиРекурсивно(Элемент.Структура);
			КонецЕсли;
		КонецЕсли; 
	КонецЦикла; 
...
Функция ДобавитьОписаниеРеквизита(Путь, НестандартныйОбработчик = Ложь, ИдентификаторРодителя, ОбновлятьФорму = Истина)
	...
	Если НЕ ПустаяСтрока(СтрРодитель.Поле) Тогда
		...
		Если СтрРодитель.ДополнительныеПараметры.Свойство("ТипГруппировки") 
			И НЕ СтрРодитель.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
...
&НаСервере
Функция ДинамическийПериодЗаголовок()
	
	Шаблон = НСтр("ru = '%1 (выбран динамически)'");
	ДополнительныеСвойства = Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства;
	Если ДополнительныеСвойства.Свойство("Сравнение") 
		И ЗначениеЗаполнено(ДополнительныеСвойства.Сравнение) 
		И ТипЗнч(ДополнительныеСвойства.Сравнение) = Тип("ПеречислениеСсылка.Периодичность") Тогда
		ТекущаяПериодичность = ДополнительныеСвойства.Сравнение;
	Иначе
		...
		Если ТипЗнч(ЗначениеСравнения) = Тип("ЗначениеПараметраКомпоновкиДанных") Тогда
			ТекущаяПериодичность = ЗначениеСравнения.Значение;
		Иначе
			ТекущаяПериодичность = Неопределено;
		КонецЕсли; 
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТекущаяПериодичность) Тогда
		ТекущаяПериодичность = УправлениеНебольшойФирмойОтчеты.ПолучитьЗначениеПериодичности(Период.ДатаНачала, Период.ДатаОкончания);
	КонецЕсли;
	Возврат СтрШаблон(Шаблон, ТекущаяПериодичность);
...
Процедура ДобавитьОписанияГруппировокРекурсивно(ЭлементыСтруктуры, ЭтоСтрока = Истина)
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			Если ЗаголовокГруппировки = ИмяГруппировкиДинамическийПериод() Тогда
				ЗаголовокГруппировки = ДинамическийПериодЗаголовок();
			КонецЕсли; 
			Если ПустаяСтрока(ЗаголовокГруппировки) Тогда
				ЗаголовокГруппировки = ЗаголовокГруппировкиДетальныеЗаписи();
			КонецЕсли;
			...
			ИначеЕсли Элемент.ПоляГруппировки.Элементы.Количество() = 0 Тогда 
				Стр.Поле = "";
				Стр.ДополнительныеПараметры.Вставить("ТипГруппировки", ТипГруппировкиКомпоновкиДанных.Элементы);
			Иначе
				// Добавление описаний отдельных полей группировок, если их несколько
				Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
		...
		ИначеЕсли НЕ ЭтоСтрока И ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			Если НЕ ДополнительныеСвойства.Свойство("ГруппировкаКолонок") И НЕ ДополнительныеСвойства.Свойство("Сравнение") Тогда
				...
				Если ИмяПоля = ИмяГруппировкиДинамическийПериод() Тогда
					...
					Иначе
						...
						Если НЕ ЗначениеЗаполнено(ПолеСравнения) Тогда
							ПолеСравнения = УправлениеНебольшойФирмойОтчеты.ПолучитьЗначениеПериодичности(Период.ДатаНачала, Период.ДатаОкончания);
						КонецЕсли; 
					КонецЕсли; 
				Иначе
			...
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") И НРег(ИмяПоля) = НРег(ДополнительныеСвойства.Сравнение) Тогда
				...
				КонецЕсли; 
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") 
				И ИмяПоля = ИмяГруппировкиДинамическийПериод() 
				И ТипЗнч(ДополнительныеСвойства.Сравнение) = Тип("ПеречислениеСсылка.Периодичность") Тогда
				КоличествоПоказателей = КоличествоПоказателейСКД(Отчет.КомпоновщикНастроек.Настройки.Выбор.Элементы);
				Если КоличествоПоказателей > 1 Тогда
					// отключение режима сравнения
...
Процедура УстановитьПредставлениеКнопкиИерархииГруппировки(Кнопка, Стр)
	...
	Если НЕ Стр.ДополнительныеПараметры.Свойство("ТипГруппировки") 
		ИЛИ Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
...
Процедура ОбновитьГруппировкиИРеквизиты(ТолькоГруппировкиСтрок = Ложь, ТолькоГруппировкиКолонок = Ложь)
	...
	СуществующийЭлемент = ЭлементСтруктурыГруппировкаТаблица(Структура);
		...
		Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			...
			Для каждого ОписаниеРеквизита Из Стр.Реквизиты Цикл
				...
				Если Стр.ПолучитьЭлементы().Количество()=0 Тогда
					Если НЕ ПустаяСтрока(Стр.Поле) Тогда
					...
					Для каждого СтрРеквизит Из Стр.ПолучитьЭлементы() Цикл
						Для каждого ОписаниеПоля Из СтрРеквизит.Реквизиты Цикл
							Если НЕ ЭтаФорма[ОписаниеПоля.Ключ] Тогда
								Продолжить;
							КонецЕсли;
							Если ПустаяСтрока(СтрРеквизит.Поле) Тогда
								Продолжить;
							КонецЕсли; 
							Поле = ТекущийЭлементСтруктуры.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
							Поле.Поле = Новый ПолеКомпоновкиДанных(СтрРеквизит.Поле);
							Поле.Использование = Использование;
							Если СтрРеквизит.ДополнительныеПараметры.Свойство("ТипГруппировки") Тогда
				...
				Если Стр.Заголовок = ИмяГруппировкиДинамическийПериод() Тогда
					Стр.Заголовок = ДинамическийПериодЗаголовок();
				КонецЕсли; 
				Если ПустаяСтрока(Стр.Заголовок) Тогда
					Стр.Заголовок = ЗаголовокГруппировкиДетальныеЗаписи();
				КонецЕсли; 
				ОбновитьЗаголовокЭлемента(Стр);
...
&НаСервере
Процедура РазвернутьДоПриИзмененииСервер()
	
	// Сохранение макета оформления по умолчанию для варианта
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) Тогда
		ХранилищеСистемныхНастроек.Сохранить("РазвернутьДо", НастройкиОтчета.ИдентификаторВарианта, РазвернутьДо);
	КонецЕсли; 
...
Процедура ДобавитьГруппировкуСравнения(СтруктураКолонок)
	...
	Если ТипЗнч(ПолеСравнения)=Тип("ПеречислениеСсылка.Периодичность") Тогда
		...
		ИмяПоля = ИмяГруппировкиДинамическийПериод();
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Дата")) Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно 
			И (Стр.Реквизиты[Идентификатор].ДатаОкончания > Значение ИЛИ НЕ ЗначениеЗаполнено(Стр.Реквизиты[Идентификатор].ДатаОкончания)) Тогда
			Стр.Реквизиты[Идентификатор].ДатаОкончания = Значение;
		КонецЕсли; 
...
Процедура УстановитьЗначениеФильтра(Поле, Значение, ВидСравнения = Неопределено, ОбновлятьВПанели = Истина, ЗаменитьЗначение = Ложь)
	...
	ДоступноеПоле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(ПолеКД);
	Если ДоступноеПоле=Неопределено Тогда
		Возврат;
	КонецЕсли; 
	...
	Для каждого ЭлементОтбор Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если ЭлементОтбор.ПредставлениеПользовательскойНастройки=СтрНаложенногоФильтра.ИмяКнопки Тогда
			Если Найти(ЭлементОтбор.ПредставлениеПользовательскойНастройки, "ДобавленноеПоле_")>0 Тогда
				МассивКУдалению.Добавить(ЭлементОтбор);
			Иначе
		...
		ИначеЕсли ЭлементОтбор.ПравоеЗначение=СтрНаложенногоФильтра.Значение Тогда
			МассивКОтключению.Добавить(ЭлементОтбор);
		КонецЕсли;
		// Сброс отборов в панели настроек
		ОбновитьРеквизитыФильтровВПанели(СтрНаложенногоФильтра.Поле, СтрНаложенногоФильтра.Значение,, Истина);
	КонецЦикла; 
	Для каждого ЭлементОтбор Из МассивКОтключению Цикл
		ЭлементОтбор.Использование = Ложь;
	КонецЦикла; 
	Для каждого ЭлементОтбор Из МассивКУдалению Цикл
		Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Удалить(ЭлементОтбор);
	КонецЦикла; 
	
	ОбновитьОтображениеОтметокФильтров();
	Если СтрНаложенногоФильтра.Поле = "Организация" Тогда
		ОбновитьВидимостьДекорацииАктуальностиСебестоимости();
	КонецЕсли; 
...
Процедура ОпределитьЭлементыИРеквизитыПараметра(Стр, Параметр = Неопределено, ДоступныйПараметр)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗначения.Типы().Получить(0)) Тогда
		// Ссылочный тип
		Обязательный = (ТипЗнч(ДоступныйПараметр)=Тип("ДоступныйПараметрКомпоновкиДанных") И ДоступныйПараметр.ЗапрещатьНезаполненныеЗначения);
		ПараметрыВыбора = ОпределитьПараметрыВыбора(ДоступныйПараметр);
		КоличествоЭлементов = ОтчетыУНФ.ОпределитьКоличествоЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
		Если КоличествоЭлементов=0 И НЕ Обязательный Тогда
			// Ни одного объекта в БД
			Возврат;
		КонецЕсли; 
		Если КоличествоЭлементов<3 И НЕ Обязательный Тогда
			Значения = ОтчетыУНФ.ПолучитьСписокЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
			СтрокаЗаголовка = "";
			Для каждого Элемент Из Значения Цикл
				СтрокаЗаголовка = СтрокаЗаголовка+Строка(Элемент);
	...
	Для каждого ЭлементОтбор Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		ИначеЕсли ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.МеньшеИлиРавно 
			...
			ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Дата") Тогда
				...
				Стр.Значение.ДатаОкончания = ?(НЕ ЗначениеЗаполнено(ЭлементОтбор.ПравоеЗначение), ЭлементОтбор.ПравоеЗначение, ЭлементОтбор.ПравоеЗначение);
			КонецЕсли; 
		ИначеЕсли ТипЗнч(ЭлементОтбор.ПравоеЗначение)=Тип("СписокЗначений") И ЭлементОтбор.ПравоеЗначение.Количество()<=КоличествоОбъединяемыхЭлементов Тогда
...
// Алгоритм для отображения отметок фильтров (плашек).
// При группировке "ГоризонтальнаяЕслиВозможно" когда горизонтально не помещается,
// платформа просто размещает элементы вертикально.
// Здесь реализован перенос на следующую строку не помещающихся элементов.
&НаСервере
Процедура ОбновитьОтображениеОтметокФильтров()
	
	ЭтоИнтерфейсТакси = (ФормаПараметры.ВариантИнтерфейса = ВариантИнтерфейсаКлиентскогоПриложения.Такси);
	ЗаполнитьТаблицуНаложенныхФильтров();
	
	Если ФормаПараметры.ЭтоМобильныйКлиент Тогда
		МаксимальнаяДлинаСтрокиОтметок = 35;
	ИначеЕсли ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
	...
	Если Элементы.ПредставлениеПериода.Видимость Тогда
		Если ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
			КоэффициентДлины = 0.75;
		Иначе
			КоэффициентДлины = 0.8;
		КонецЕсли;
		Элементы.ПредставлениеПериода.Ширина = Окр(СтрДлина(ПредставлениеПериода) * КоэффициентДлины + ШиринаКнопки);
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		Если СтрДлина(Стр.Заголовок) > МаксимальнаяДлинаЗаголовка Тогда
			ЗаголовокЭлемента = Лев(Стр.Заголовок, МаксимальнаяДлинаЗаголовка - 2) + "...";
		Иначе
			ЗаголовокЭлемента = Стр.Заголовок;
		КонецЕсли; 
		
		ДлинаЭлемента = ОтчетыУНФКлиентСервер.ШиринаОтметки(ЗаголовокЭлемента, ЭтоИнтерфейсТакси, ФормаПараметры.ЭтоМобильныйКлиент);
		...
		Если Стр.Заголовок <> ЗаголовокЭлемента Тогда
...
Функция СформироватьСписокЭлементов(Текст, УправляющиеСимволы)
	...
	Если СтрДлина(Текст)>=3 
		...
		Если НастройкиОтчета.ПоляПоиска.Количество()=0 Тогда
			// Выполняем поиск по полям, присутствующим в списке фильтров
			СтруктураОтбора = Новый Структура;
			СтруктураОтбора.Вставить("Тип", ТипФильтр());
			Строки = СтрокиДерева(ПоляНастроек, СтруктураОтбора);
			Для каждого Стр Из Строки Цикл
...
Процедура ДобавитьЭлементВыбораВСписок(Список, СтрПоля, Значение, УправляющиеСимволы)
	...
	КонецЦикла; 
	СтруктураЗначения = Новый Структура;
	СтруктураЗначения.Вставить("Поле", СтрПоля.Поле);
	СтруктураЗначения.Вставить("ВведенноеЗначение", Значение);
	СтруктураЗначения.Вставить("УправляющиеСимволы", УправляющиеСимволы);
	...
	Если Не ПустаяСтрока(УправляющиеСимволы) Тогда
		Список.Добавить(СтруктураЗначения, ЗаголовокПоля+" "+УправляющиеСимволы+" "+Строка(Значение));
	Иначе
		СтруктураЗначения.Вставить("УправляющиеСимволы", "=");
		Список.Добавить(СтруктураЗначения, ЗаголовокПоля+" = "+Строка(Значение));
	КонецЕсли; 
...
Процедура НаложитьОтборПоДобавленномуПолю(Поле, Значение, УправляющиеСимволы = "")
	...
	ИначеЕсли Поле="ДобавленноеПоле_Сегмент" Тогда
		...
		Если ПолеКонтрагент=Неопределено Тогда
			Возврат;
		КонецЕсли; 
		МассивСегментов = Справочники.СегментыКонтрагентов.ПолучитьСоставСегмента(Значение);
		...
		ГруппаОтборов.Представление = ПредставлениеСегмента(Значение);
...
&НаСервере
Функция ПредставлениеСегмента(Знач Значение)
...
Функция ДобавитьОписаниеНастройки(ЭлементыДобавления, Тип, ИмяПоля, Представление = "", ЭлементСхемы = Неопределено)
	...
	КонецЕсли;
	Если (Тип=ТипПараметр() ИЛИ Тип=ТипФильтр()) И ДоступноеПоле=Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
...
Процедура ОбновитьДополнительныйСвойстваСКД()
	...
	ИначеЕсли НастройкиОтчета.РежимПериода = ОтчетыУНФКлиентСервер.РежимПериодаЗаПериод() Тогда
	...
	Если ДополнительныеСвойства.Свойство("ЭтоОтчетУНФ") Тогда
		ДополнительныеСвойства.Удалить("ЭтоОтчетУНФ");
	КонецЕсли;
...
#Область ЭкспортКонтактовОтчетаВРассылку

&НаСервере
Процедура ЗаполнитьДанныеИОформлениеДляЭкспорта()
	
	СервисМассовойРассылки = МассовыеРассылкиИнтеграция.ПодключенныйСервис();
	ИспользоватьМассовыеРассылкиИнтеграция = ПолучитьФункциональнуюОпцию("ИспользоватьМассовыеРассылкиИнтеграция") И ЗначениеЗаполнено(СервисМассовойРассылки);
	ВидСвязиЕмейл = Перечисления.ВидыКаналовСвязи.Email;
	ВидСвязиСмс   = Перечисления.ВидыКаналовСвязи.SMS;
	
	Если ИспользоватьМассовыеРассылкиИнтеграция Тогда
		ЗаголовокКомандыЭкспорта = НСтр("ru = 'Экспортировать результат отчета в "+МассовыеРассылкиИнтеграция.ПредставлениеСервиса(СервисМассовойРассылки) +"'");
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭкспортироватьКонтактыОтчета", "Заголовок", ЗаголовокКомандыЭкспорта);
	КонецЕсли;

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭкспортироватьКонтактыОтчета", "Видимость", ИспользоватьМассовыеРассылкиИнтеграция);
	Элементы.Переместить(Элементы.ГруппаИспользоватьРезультатОтчета, Элементы.ГруппаОтправить);
	
КонецПроцедуры

&НаСервере
Процедура СформироватьКонтактыДляЭкспорта()
	
	ИспользуемыеНастройки = Отчет.КомпоновщикНастроек.Настройки;
	Если НЕ ЭтоАдресВременногоХранилища(ОтчетДанныеРасшифровки) Тогда
		Возврат;
	КонецЕсли; 
	ИспользуемыеДанныеРасшифровки =  ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	
	КонтактыОтчета.Очистить();
	...
	Для Каждого ЭлементРасшифровки Из ИспользуемыеДанныеРасшифровки.Элементы Цикл
		...
		Если ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда
			...
			Для Каждого ЗначениеПоля Из ЗначенияПолей Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначениеПоля.Значение) ИЛИ (ТипЗнч(ЗначениеПоля.Значение) <> ТИП("СправочникСсылка.Контрагенты") 
					И ТипЗнч(ЗначениеПоля.Значение) <> ТИП("СправочникСсылка.КонтактныеЛица")) Тогда 
					Продолжить; 
				КонецЕсли;
				
				ЗначениеУжеЕсть = КонтактыОтчета.НайтиСтроки(Новый Структура("Контакт", ЗначениеПоля.Значение));
				Если ЗначениеУжеЕсть.Количество() <> 0 Тогда
					Продолжить;
				КонецЕсли;
				
				НоваяСтрока = КонтактыОтчета.Добавить();
				НоваяСтрока.Контакт = ЗначениеПоля.Значение;
...
&НаСервере
Функция МассивДанныхДляЭкспорта(ДанныеРасшифровки)
	
	ВыделенныеОбъекты = Новый Массив;
	Для Каждого Расшифровка Из  ДанныеРасшифровки Цикл
		
		Данные = ПолучитьДанныеРасшифровки(Расшифровка);
		Если Данные = Неопределено ИЛИ НЕ Данные.Свойство("Значение") Тогда
			Продолжить;
		КонецЕсли;
		
		Если ТипЗнч(Данные.Значение) = Тип("СправочникСсылка.Контрагенты") ИЛИ  ТипЗнч(Данные.Значение) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
			ВыделенныеОбъекты.Добавить(Данные.Значение);
		КонецЕсли;
...
&НаКлиенте
Функция ДанныеРасшифровкиВыделеннойОбласти()
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл
		...
		Для НомерКолонки = ВыделеннаяОбластьЛево По ВыделеннаяОбластьПраво Цикл
			Для НомерСтроки = ВыделеннаяОбластьВерх По ВыделеннаяОбластьНиз Цикл
				...
				Если Ячейка.Видимость = Истина Тогда
					Если Ячейка.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Колонки И НЕ Ячейка.Расшифровка = Неопределено Тогда
						ДанныеРасшифровки.Добавить(Ячейка.Расшифровка);
					Иначе
						Продолжить;
					КонецЕсли;
				КонецЕсли;
...
&НаКлиенте
Процедура СоздатьЗадачу(ПараметрЗадачи)
	
	ЗначенияЗаполнения = Новый Структура;
	ЗначенияЗаполнения.Вставить("Основание", ПараметрЗадачи);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяГруппировкиДинамическийПериод()
	Возврат "ДинамическийПериод";
КонецФункции 

&НаКлиентеНаСервереБезКонтекста
Функция ИмяГруппировкиМесяцГода()
	Возврат "МесяцГода";
КонецФункции 

&НаКлиентеНаСервереБезКонтекста
Функция ИмяГруппировкиДеньМесяцГода()
	Возврат "ДеньМесяцГода";
КонецФункции 

&НаКлиентеНаСервереБезКонтекста
Функция ИмяГруппировкиНеделяГода()
	Возврат "НеделяГода";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЗаголовокГруппировкиДетальныеЗаписи()
