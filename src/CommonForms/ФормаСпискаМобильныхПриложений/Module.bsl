...
&НаКлиенте
Процедура Розница_МК_GPНажатие(Элемент)
	ОткрытьСсылкуНаРесурс(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура Розница_МК_ASНажатие(Элемент)
	ОткрытьСсылкуНаРесурс(Элемент.Имя);
КонецПроцедуры
...
Процедура НастроитьФормуПодМобильноеПриложение()
	...
	ИдентификаторыПриложений.Вставить("Группа_Розница_МК", 9);
...
Процедура УстановитьСсылкиНаМаркетплейсы()
	...
	// Мобильный клиент УНФ
	АдресПубликации_УНФ_МК_GP = МобильныйКлиентУНФКлиентСервер.АдресПубликацииВGooglePlay(Истина, Ложь);
	СсылкиНаМаркетплейсы.Вставить("УНФ_МК_GP", АдресПубликации_УНФ_МК_GP);
	
	АдресПубликации_УНФ_МК_AS = МобильныйКлиентУНФКлиентСервер.АдресПубликацииВAppStore(Истина, Ложь);
	СсылкиНаМаркетплейсы.Вставить("УНФ_МК_AS", АдресПубликации_УНФ_МК_AS);
	
	// Мобильный клиент Розница
	АдресПубликации_Розница_МК_GP = МобильныйКлиентУНФКлиентСервер.АдресПубликацииВGooglePlay(Истина, Истина);
	СсылкиНаМаркетплейсы.Вставить("Розница_МК_GP", АдресПубликации_Розница_МК_GP);
	
	АдресПубликации_Розница_МК_AS = МобильныйКлиентУНФКлиентСервер.АдресПубликацииВAppStore(Истина, Истина);
	СсылкиНаМаркетплейсы.Вставить("Розница_МК_AS", АдресПубликации_Розница_МК_AS);
...
Процедура УстановитьНастройкиКатегорийДляЭлементов()
	...
	НастройкиКатегорийЭлементов.Вставить("Группа_Розница_МК", ТекФильтр);
	...
	ТекФильтр = СтрЗаменить(ПолныйФильтр, "УслугиСервисы", 	"");
	ТекФильтр = СтрЗаменить(ТекФильтр,	  "Производство", 	"");
	ТекФильтр = СтрЗаменить(ТекФильтр, 	"ОптоваяТорговля",  "");
...
Процедура УстановитьНастройкиСценариевДляЭлементов()
	...
	Иначе
		Если ЭтоМобильныйКлиент Тогда
			ПолноценныйУчетПредставление 		= НСтр("ru = 'Все функции «большой» 1С:Розница'");
		Иначе
			ПолноценныйУчетПредставление 		= НСтр("ru = 'Доступ ко всем функциям «большой» 1С:Розница'");
		КонецЕсли;
	КонецЕсли;
	...
	ТекФильтр = СтрЗаменить(ПолныйФильтр, "УчетВМикроБизнесе", "");
	ТекФильтр = СтрЗаменить(ТекФильтр, 	  "НовыйКаналПродаж",  "");
	НастройкиСценариевЭлементов.Вставить("Группа_Розница_МК", ТекФильтр);
...
Процедура УстановитьНастройкиВесаПриложений_ПоУмолчанию()
	...
	ВесЭлементовПоУмолчанию.Вставить("Группа_Розница_МК", 	9);
...
Процедура УстановитьНастройкиВесаПриложений_ПоКатегории()
	...
	НастройкиВеса.Вставить("Группа_Розница_МК", 8);
	...
	// Розница
	НастройкиВеса = Новый Структура;
	НастройкиВеса.Вставить("Группа_Розница_МК", 6);
	...
	НастройкиВеса = Новый Структура;
	НастройкиВеса.Вставить("Группа_Розница_МК", 7);
	НастройкиВеса.Вставить("Группа_УНФ_МК", 7);
	НастройкиВеса.Вставить("Группа_ЗАК", 	4);
	НастройкиВеса.Вставить("Группа_МКс", 	5);
	НастройкиВеса.Вставить("Группа_КЛД",	3);
	НастройкиВеса.Вставить("Группа_КК", 	1);
...
Процедура УстановитьНастройкиВесаПриложений_ПоСценарию()
	...
	// Новый канал продаж
	НастройкиВеса = Новый Структура;
	НастройкиВеса.Вставить("Группа_Розница_МК", 0);
...
Процедура УстановитьВидимостьЭлементов()
	...
	ПредопределеннаяВидимостьПриложений.Вставить("Группа_УНФ_МК", 	ЭтоУНФ);
	ПредопределеннаяВидимостьПриложений.Вставить("Группа_Розница_МК", НЕ ЭтоУНФ);
...
Процедура НастроитьСинхронизациюКЛДНажатие(Элемент)
	...
	МобильноеПриложение = НастроитьСинхронизациюКЛДСервер();
	ПараметрыФормы = Новый Структура;
	ПараметрыОтбора = Новый Структура("МобильноеПриложение", МобильноеПриложение);
	ПараметрыФормы.Вставить("Отбор", ПараметрыОтбора);
	ОткрытьФорму("ПланОбмена.ОбменСМобильнымиПриложениями.Форма.ФормаСписка",ПараметрыФормы, , "ОбменСМобильнымиПриложениями_Кладовщик");
	
КонецПроцедуры

&НаСервере
Функция НастроитьСинхронизациюКЛДСервер()
	...
	Возврат МобильноеПриложениеПоВариантуНастройки(МассивВариантов);
...
Процедура ИнструкцияКЛДНажатие(Элемент)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://its.1c.ru/db/metod81#content:7761:hdoc");
...
Процедура НастроитьСинхронизациюЗАКНажатие(Элемент)
	...
	МобильноеПриложение = НастроитьСинхронизациюЗАКСервер();
	ПараметрыФормы = Новый Структура;
	ПараметрыОтбора = Новый Структура("МобильноеПриложение", МобильноеПриложение);
	ПараметрыФормы.Вставить("Отбор", ПараметрыОтбора);
	ОткрытьФорму("ПланОбмена.ОбменСМобильнымиПриложениями.Форма.ФормаСписка",ПараметрыФормы, , "ОбменСМобильнымиПриложениями_Заказы");
	
КонецПроцедуры

&НаСервере
Функция НастроитьСинхронизациюЗАКСервер()
	...
	Возврат МобильноеПриложениеПоВариантуНастройки("Заказы");
	
КонецФункции

&НаСервере
Функция МобильноеПриложениеПоВариантуНастройки(ВариантНастройки) 
	...
	Запрос.Текст = "ВЫБРАТЬ 
	               ...
	               |ГДЕ
	               |	МобильныеПриложения.ВариантНастройки В (&ВариантНастройки)
	...
	Если Не Результат.Пустой() Тогда
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				МассивПриложений.Добавить(Выборка.Ссылка);
			КонецЦикла;                             
...
Процедура ИнструкцияЗАКНажатие(Элемент)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://its.1c.ru/db/metod81#content:7818:hdoc");
	
КонецПроцедуры

&НаСервере
Процедура НастроитьТекстыНаФорме()
	
	Элементы.КраткоеОписаниеКК_УНФ.Видимость = ЭтоУНФ И Не ЭтоМобильныйКлиент;
	Элементы.КраткоеОписаниеКК_Розница.Видимость = Не ЭтоУНФ И Не ЭтоМобильныйКлиент;
	
	Элементы.Группа7ЗаголовокИКартинка1.Видимость = Не ЭтоМобильныйКлиент;
	
	Если ЗначениеЗаполнено(ПолучитьСсылкуНаКонструктор()) Тогда
		Элементы.ПерейтиВКонструктор.Заголовок = НСтр("ru = 'Перейти в конструктор мобильного приложения'");
	КонецЕсли;
