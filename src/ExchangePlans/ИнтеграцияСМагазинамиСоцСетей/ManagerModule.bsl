...
Процедура ОбработатьЦеныТоваров(СоответствиеИдентификаторов, РезультатЗапросаЦена, ВыгружатьЦены)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		Попытка
			...
			ИначеЕсли ВыборкаДетальныеЗаписи.СтараяЦена = 0 Тогда
				//ОписаниеНоменклатуры.Вставить("old_price","0");
			КонецЕсли;
		Исключение
		КонецПопытки;
...
Процедура ПроверитьОбъединитьХарактеристикуВГруппы(МассивХарактеристик, ОписаниеУзелОбмена, СпособОбменаСМагазином)
	...
	//Если СпособОбменаСМагазином =
	//ПредопределенноеЗначение("Перечисление.СпособыОбменаСМагазиномСоцСетей.ТолькоЦеныИКоличество") Тогда
	Возврат;
	//КонецЕсли;
...
Функция ПолучитьПодборкиТоваров(ОписаниеУзелОбмена) Экспорт
	...
	РезультатМассив = ПолучитьОбщиеДанныеРекурсивно(ОписаниеУзелОбмена, СтруктураЗапросаHTTP, АдресРесурса);
...
Процедура ЗагрузитьЗаказы(ОписаниеУзелОбмена) Экспорт
	...
	АдресРесурса, 0, Истина, ОписаниеПроверкиЗначения);
...
Функция ПолучитьВсеТоварыИзМагазина(ОписаниеУзелОбмена, ПолучатьВсеДанные = Истина, КраткаяИнформация = Ложь, ПараметрыСтрокой = "") Экспорт
	...
	МассивНоменклатурДляОбработки = ПолучитьОбщиеДанныеРекурсивно(ОписаниеУзелОбмена, СтруктураЗапросаHTTP, АдресРесурса);
...
Функция ОбработатьТовар(ОписаниеТовара, ОписаниеУзелОбмена, КраткаяИнформация = Истина, ЭтоХарактеритика = Ложь) Экспорт
	...
	Если НЕ ЭтоХарактеритика И МассивВариантов <> Неопределено Тогда
		...
		Для Каждого СтрОписаниеВарианта Из МассивВариантов Цикл
			...
			//Если
			//ИнтеграцияСМагазинамиСоцСетейСервер.ПроверитьНаТипЗначение(СтрОписаниеВарианта.Получить("is_main"),Тип("Булево"),Ложь) Тогда
	...
	Если ОписаниеУзелОбмена.ЗагружатьОстатки И Не ЭтоЗагрузкаЗаказа Тогда
		...
		Если МассивОстатков.Количество() Тогда
			...
			Для Каждого СтрОстаток Из МассивОстатков Цикл
				...
				НовСтр.СтруктурнаяЕдиница = ОписаниеУзелОбмена.СкладДляПодстановкиВЗаказы;
...
Функция ПолучитьОбщиеДанныеРекурсивно(ОписаниеУзелОбмена, СтруктураЗапросаHTTP, АдресРесурса, знач Смещение = 0, знач ИспользоватьСмещение = Истина, ОписаниеПроверкиЗначения = Неопределено)
