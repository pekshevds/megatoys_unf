...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторСервиса) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не указан идентификатор сервиса (agent_id)'",
			ОбщегоНазначения.КодОсновногоЯзыка()),,,, Отказ);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОК(Команда)
	...
	Если Подпись = ТекущаяПодпись Тогда
		ТекстПредупреждения = НСтр("ru = 'Ключевое слово не изменено'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Изменено ключевое слово'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	КонецЕсли;
