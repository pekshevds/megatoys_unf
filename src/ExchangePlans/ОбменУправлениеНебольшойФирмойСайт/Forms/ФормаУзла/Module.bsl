...
Процедура СоздатьВидыОплатВ1СНаСервере()
	...
	Для каждого стр Из Объект.СоответствиеВидовОплат Цикл
		...
		ОбТерминал = Справочники.СпособыОплаты.СоздатьЭлемент();
		...
		Если НЕ ЗначениеЗаполнено(Эквайрер) Тогда
			...
			ДоговорПоУмолчанию = Справочники.ДоговорыКонтрагентов.ПоКонтрагентуПоОрганизации(Эквайрер,
				ОбТерминал.Организация, СписокВидовДоговора);
