...
Функция ПолучитьТекстЗапросаПлатежныйКалендарь(Объект, ДанныеМеток) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПлатежныйКалендарь.ПериодСекунда КАК ПериодСекунда,
	|	ПлатежныйКалендарь.ПериодДень КАК ПериодДень,
	...
	|	ПлатежныйКалендарь.СуммаОборот КАК СуммаВалОборот,
	|	ПлатежныйКалендарь.СуммаОплатыОборот КАК СуммаОплатыВалОборот,
	|	ВЫРАЗИТЬ(ПлатежныйКалендарь.СуммаОборот * КурсыВалютДокумента.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютДокумента.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаОборот,
	|	ВЫРАЗИТЬ(ПлатежныйКалендарь.СуммаОплатыОборот * КурсыВалютДокумента.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютДокумента.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаОплатыОборот,
	|	ВЫРАЗИТЬ(ПлатежныйКалендарь.СуммаОборот * КурсыВалютДокумента.Курс / КурсыВалютДокумента.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаРегОборот,
	|	ВЫРАЗИТЬ(ПлатежныйКалендарь.СуммаОплатыОборот * КурсыВалютДокумента.Курс / КурсыВалютДокумента.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаОплатыРегОборот
	...
	|				И СчетНаОплату <> ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)) КАК ПлатежныйКалендарь
	...
	|		ПО (КурсыВалютДокумента.Валюта = ПлатежныйКалендарь.Валюта)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	СчетНаОплату
	...
	|			ТОГДА ПлатежныйКалендарьОплата.БанковскийСчетКасса
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СчетКассаДляПеремещения,
	...
	|	СУММА(ПлатежныйКалендарьОплата.СуммаОплатыВалОборот) КАК СуммаОплатыВалОборот,
	|	СУММА(ПлатежныйКалендарьОплата.СуммаОплатыРегОборот) КАК СуммаОплатыРегОборот
	|ПОМЕСТИТЬ ПлатежныйКалендарьОплата
	|ИЗ
	|	ИсходныеДанные КАК ПлатежныйКалендарьОплата
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА ПлатежныйКалендарьОплата.СчетНаОплату ССЫЛКА Документ.ПеремещениеДСПлан
	...
	|			ТОГДА ВЫБОР
	|					КОГДА СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыОборот, 0)) >= 0
	...
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыОборот, 0)) < 0
	|						ТОГДА 0
	|					ИНАЧЕ СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыОборот, 0))
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СУММА(ВЫБОР
	|							КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПлатежныйКалендарь.СуммаВалОборот) > 0
	|			ТОГДА ВЫБОР
	|					КОГДА СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	|											И ПлатежныйКалендарь.Регистратор > ПлатежныйКалендарьДвижения.Регистратор
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаВалОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыВалОборот, 0)) >= 0
	|						ТОГДА МАКСИМУМ(ПлатежныйКалендарь.СуммаВалОборот)
	|					КОГДА СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	|											И ПлатежныйКалендарь.Регистратор > ПлатежныйКалендарьДвижения.Регистратор
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаВалОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаВалОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыВалОборот, 0)) < 0
	|						ТОГДА 0
	|					ИНАЧЕ СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаВалОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаВалОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыВалОборот, 0))
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СУММА(ВЫБОР
	|							КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|	КОНЕЦ КАК СуммаВал,
	|	ВЫБОР
	|		КОГДА МАКСИМУМ(ПлатежныйКалендарь.СуммаРегОборот) > 0
	|			ТОГДА ВЫБОР
	|					КОГДА СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	|											И ПлатежныйКалендарь.Регистратор > ПлатежныйКалендарьДвижения.Регистратор
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаРегОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыРегОборот, 0)) >= 0
	|						ТОГДА МАКСИМУМ(ПлатежныйКалендарь.СуммаРегОборот)
	|					КОГДА СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	|											И ПлатежныйКалендарь.Регистратор > ПлатежныйКалендарьДвижения.Регистратор
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаРегОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаРегОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыРегОборот, 0)) < 0
	|						ТОГДА 0
	|					ИНАЧЕ СУММА(ВЫБОР
	|								КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	|										ИЛИ ПлатежныйКалендарь.ПериодСекунда = ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|									ТОГДА ЕСТЬNULL(ПлатежныйКалендарьДвижения.СуммаРегОборот, 0)
	|								ИНАЧЕ 0
	|							КОНЕЦ) + МАКСИМУМ(ПлатежныйКалендарь.СуммаРегОборот) - МАКСИМУМ(ЕСТЬNULL(ПлатежныйКалендарьОплата.СуммаОплатыРегОборот, 0))
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СУММА(ВЫБОР
	|							КОГДА ПлатежныйКалендарь.ПериодСекунда > ПлатежныйКалендарьДвижения.ПериодСекунда
	...
	|ИЗ
	|	ИсходныеДанные КАК ПлатежныйКалендарь
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсходныеДанные КАК ПлатежныйКалендарьДвижения
	|		ПО ПлатежныйКалендарь.СчетНаОплату = ПлатежныйКалендарьДвижения.СчетНаОплату
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПлатежныйКалендарьОплата КАК ПлатежныйКалендарьОплата
	|		ПО ПлатежныйКалендарь.СчетНаОплату = ПлатежныйКалендарьОплата.СчетНаОплату
	|			И (ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ПлатежныйКалендарьОплата.СчетНаОплату) = ТИП(Документ.ПеремещениеДСПлан)
	|					ТОГДА ПлатежныйКалендарь.БанковскийСчетКасса = ПлатежныйКалендарьОплата.СчетКассаДляПеремещения
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА &ОтображатьБолееРанниеПлатежи
	|				И ОстаткиПланируемыхПлатежей.Период < &НачалоПериода
	...
	|	СУММА(ОстаткиПланируемыхПлатежей.СуммаВал) КАК СуммаВалОборот,
	|	СУММА(ОстаткиПланируемыхПлатежей.Сумма) КАК СуммаОборот,
	|	СУММА(ОстаткиПланируемыхПлатежей.СуммаРег) КАК СуммаРегОборот
	|ПОМЕСТИТЬ Прогноз
	|ИЗ
	|	ОстаткиПланируемыхПлатежей КАК ОстаткиПланируемыхПлатежей
	|ГДЕ
	|	ОстаткиПланируемыхПлатежей.Сумма <> 0
	|	И (&ОтображатьБолееРанниеПлатежи
	|			ИЛИ ОстаткиПланируемыхПлатежей.Период >= &НачалоПериодаОтбор)
	|	И &ОтборПоКонтрагенту
	|	И Организация = &ОтборыПлатежныйКалендарь
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА &ОтображатьБолееРанниеПлатежи
	...
	ШаблонОтбораОбороты = "ОстаткиПланируемыхПлатежей.ИмяПоляОтбора В (&ИмяПоляОтбора)";
	...
	Для каждого СтрокаОтбора Из ДанныеМеток Цикл
		...
		Если СписокОтборыПлатежныйКалендарь.Найти(СтрокаОтбора.ИмяПоляОтбора) <> Неопределено Тогда
			...
			ОтборыПлатежныйКалендарь = ОтборыПлатежныйКалендарь + СтрЗаменить(ШаблонОтбораОбороты, "ИмяПоляОтбора", СтрокаОтбора.ИмяПоляОтбора);
		КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Организация = &ОтборыПлатежныйКалендарь",	ОтборыПлатежныйКалендарь);
