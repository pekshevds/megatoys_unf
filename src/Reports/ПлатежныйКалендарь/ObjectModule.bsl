...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если ВнешниеНаборыДанных.ТаблицаПлатежей.Количество() = 0
		И ВнешниеНаборыДанных.ТаблицаОстатков.Количество() > 0 Тогда
		
		ДополнитьНаборыПриОтсутствииДвижений(ВнешниеНаборыДанных);
		НастроитьОтображениеГруппировокОтчетаПриОтсутствииДвижений(Настройки);
...
Процедура ДобавитьСтрокуТаблицНаборов(Выборка, ТипАналитики, ВнешниеНаборы, ПараметрыГруппировок)
	...
	Если ОтображатьБолееРанниеПлатежи Тогда
	...
	Если ТипАналитики = Перечисления.АналитикаПлатежногоКалендаря.Документ 
		 И (НЕ ЗначениеЗаполнено(Выборка.Документ) 
		 ИЛИ (НЕ ЗначениеЗаполнено(Выборка.Период) И НЕ ОтображатьБолееРанниеПлатежи)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДополнитьНаборыПриОтсутствииДвижений(ВнешниеНаборыДанных)
	...
	Для каждого АналитикаПлатежа Из ВнешниеНаборыДанных.ТаблицаОстатков Цикл
		
		Для каждого Период Из ПериодыПланирования Цикл
			
			НоваяСтрока = ВнешниеНаборыДанных.ТаблицаПлатежей.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, АналитикаПлатежа);
			НоваяСтрока.Период = НачалоДня(Период.Значение);
			НоваяСтрока.Сумма = 0;
			
			Если ТипЗнч(АналитикаПлатежа.БанковскийСчетКасса) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
...
Процедура НастроитьОтображениеГруппировокОтчетаПриОтсутствииДвижений(Настройки)
	...
	Если ТекущаяГруппировка.Имя = "Документ" Тогда
		ТекущаяГруппировка.Состояние = Отключена;
	КонецЕсли;
