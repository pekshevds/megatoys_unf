...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.ГруппаПоМенеджерамПодробно.ОтображатьЗаголовок = Истина;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ОтборПериодВид = ОтчетыУНФКлиентСервер.ПолучитьВидСтандартногоПериода(ОтборПериодЗначение, Элементы.ОтборПериодВид.СписокВыбора);
...
Процедура ОтборПериодВидПриИзменении(Элемент)
	...
	Иначе
		...
		КонецЕсли;
		НачалоПериода = ОтчетыУНФКлиентСервер.НачалоПериодаОтчета(ОтборПериодВид, НачалоПериода);
		КонецПериода  = ОтчетыУНФКлиентСервер.КонецПериодаОтчета(ОтборПериодВид, НачалоПериода);
...
Процедура ОтборПериодПредставлениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если НачалоПериода = '00010101' Тогда
		НачалоПериода = ОтчетыУНФКлиентСервер.НачалоПериодаОтчета(ОтборПериодВид, ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
...
Процедура ДиаграммаНовыеПоПериодамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Новые контрагенты за период'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ДиаграммаПоМенеджерамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Контрагенты по менеджерам'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ОбновитьДиаграммуНовые()
	...
	Если ЗначениеЗаполнено(ОтборСегмент) Тогда
		...
		Запрос.УстановитьПараметр("СоставСегмента", Справочники.СегментыКонтрагентов.ПолучитьСоставСегмента(ОтборСегмент));
	КонецЕсли;
...
Процедура ЗагрузитьРезультатВыбораПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора)
	...
	Иначе
		НачалоПериода = ОтчетыУНФКлиентСервер.НачалоПериодаОтчета(ПараметрыВыбора.ВидПериода, Результат.Значение);
		КонецПериода  = ОтчетыУНФКлиентСервер.КонецПериодаОтчета(ПараметрыВыбора.ВидПериода, Результат.Значение);
