...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	РежимРасшифровки = (КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("РежимРасшифровки") 
		И КомпоновщикНастроек.Настройки.ДополнительныеСвойства.РежимРасшифровки)
		ИЛИ (КомпоновщикНастроек.ФиксированныеНастройки.ДополнительныеСвойства.Свойство("РежимРасшифровки") 
	...
	ПараметрыПериод = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СтПериод"));
	Если НЕ ПараметрыОтчета.Свойство("НачалоПериода") И ПараметрыПериод <> Неопределено И ПараметрыПериод.Использование Тогда
		ПараметрыОтчета.Вставить("НачалоПериода", ПараметрыПериод.Значение.ДатаНачала);
	КонецЕсли; 
	Если НЕ ПараметрыОтчета.Свойство("КонецПериода") И ПараметрыПериод <> Неопределено И ПараметрыПериод.Использование Тогда
		ПараметрыОтчета.Вставить("КонецПериода", ПараметрыПериод.Значение.ДатаОкончания);
	КонецЕсли;
	ПараметрДанных = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПоМестамХраненияДенег"));
	Если ПараметрДанных.Использование Тогда
		ПараметрыОтчета.Вставить("ПоМестамХраненияДенег", ПараметрДанных.Значение = "ПоМестамХраненияДенег");
	Иначе
		ПараметрыОтчета.Вставить("ПоМестамХраненияДенег", Ложь);
	КонецЕсли;
	...
	ПараметрДанных = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Периодичность"));
	Если РежимРасшифровки Тогда
		ТабличныйДокумент = СформироватьРасшифровку(ПараметрыОтчета);
	ИначеЕсли ПараметрДанных.Использование И ЗначениеЗаполнено(ПараметрДанных.Значение) Тогда
		ПараметрыОтчета.Вставить("Сравнение", ПараметрДанных.Значение);
...
Функция СформироватьОтчетДенежныйПоток(ПараметрыОтчета, ДанныеРасшифровки)
	...
	ВывестиЗаголовок(ПараметрыОтчета, Отчет, Макет);
...
Функция СформироватьОтчетДенежныйПотокВДинамике(ПараметрыОтчета, ДанныеРасшифровки)
	...
	ПараметрыОтчета.Вставить("Сравнивать", ОтчетыУНФ.ЗначениеПараметраДанных(Настройки, 
		"Сравнивать", Ложь));
	ПараметрыОтчета.Вставить("ПоказыватьАбсолютноеИзменение", ОтчетыУНФ.ЗначениеПараметраДанных(Настройки, 
		"АбсолютноеОтклонение", Ложь));
	ПараметрыОтчета.Вставить("ПоказыватьОтносительноеИзменение", ОтчетыУНФ.ЗначениеПараметраДанных(Настройки, 
		"ОтносительноеОтклонение", Ложь));
	...
	СоответствиеГоризонтальныхРасшифровок = Новый Соответствие;
	...
	Для каждого Элемент Из СписокДат Цикл
		...
		СоответствиеГоризонтальныхРасшифровок.Вставить(Элемент.Значение, РасшифровкаПериод);
		...
		Если СписокДат.Индекс(Элемент) > 0 Тогда
			Если ПараметрыОтчета.Свойство("Сравнивать") И ПараметрыОтчета.Сравнивать Тогда
	...
	Для каждого СтрСчетКасса Из ИсходныеДанные.Строки Цикл
		...
		Для каждого Элемент Из СписокДат Цикл
			...
			СтрокиОстатков = ТаблицаОстатков.НайтиСтроки(СтруктураОтбораОстатков);
			Если НЕ СтрокиОстатков.Количество() = 0 Тогда
				СуммаОстаток = СтрокиОстатков[0].НачальныйОстаток;
			Иначе
				СуммаОстаток = 0;
			КонецЕсли; 
		...
		Если СтрокиОстатков.Количество() > 0 Тогда
			Область.Параметры.Сумма = СтрокиОстатков[0].НачальныйОстаток;
		КонецЕсли;  
		Отчет.Присоединить(Область);
		Для каждого СтрПриходРасход Из СтрСчетКасса.Строки Цикл
			...
			Для каждого Элемент Из СписокДат Цикл
				...
				СуммаПредыдущая1 = ВывестиОбластьОтчета(Отчет, Макет, ИмяОбласти, ПараметрыОтчета, ТаблицаИтогов, Отбор, СуммаПредыдущая1, 
					ДанныеРасшифровки, РасшифровкаПриходРасход, СоответствиеГоризонтальныхРасшифровок[Элемент.Значение],, 
					СписокДат.Индекс(Элемент)=0);
			КонецЦикла;
			Область = Макет.ПолучитьОбласть(?(СтрПриходРасход.ПриходРасход,
			...
			Для каждого СтрХозОперация Из СтрПриходРасход.Строки Цикл
				...
				Для каждого Элемент Из СписокДат Цикл
					...
					Отбор = СтруктураОтбора(СтрХозОперация, "ПриходРасход,ХозОперация" 
						+ ?(ПараметрыОтчета.ПоМестамХраненияДенег, ",БанковскийСчетКасса", ""));
					Отбор.Вставить("ДинамическийПериод", Элемент.Значение);
					СуммаПредыдущая2 = ВывестиОбластьОтчета(Отчет, Макет, ИмяОбласти, ПараметрыОтчета, ТаблицаИтогов, Отбор,  
						СуммаПредыдущая2, ДанныеРасшифровки, РасшифровкаХозОперация, 
						СоответствиеГоризонтальныхРасшифровок[Элемент.Значение], 1, СписокДат.Индекс(Элемент) = 0);
				КонецЦикла;
				...
				Для каждого СтрАналитика Из СтрХозОперация.Строки Цикл
					...
					Для каждого Элемент Из СписокДат Цикл
						...
						Отбор = СтруктураОтбора(СтрАналитика, "ПриходРасход,ХозОперация,Аналитика" 
							+ ?(ПараметрыОтчета.ПоМестамХраненияДенег, ",БанковскийСчетКасса", ""));
						Отбор.Вставить("ДинамическийПериод", Элемент.Значение);
						СуммаПредыдущая3 = ВывестиОбластьОтчета(Отчет, Макет, ИмяОбласти, ПараметрыОтчета, ТаблицаИтогов, Отбор, 
							СуммаПредыдущая3, ДанныеРасшифровки, РасшифровкаАналитика, СоответствиеГоризонтальныхРасшифровок[Элемент.Значение], 
							2, СписокДат.Индекс(Элемент) = 0);
					КонецЦикла;
					...
					Для каждого СтрВалюта Из СтрАналитика.Строки Цикл
						...
						Для каждого Элемент Из СписокДат Цикл
							...
							Отбор = СтруктураОтбора(СтрВалюта, "ПриходРасход,ХозОперация,Аналитика,Валюта" 
								+ ?(ПараметрыОтчета.ПоМестамХраненияДенег, ",БанковскийСчетКасса", ""));
							Отбор.Вставить("ДинамическийПериод", Элемент.Значение);
							Итоги = РассчитатьИтог(ТаблицаИтогов, Отбор);
							Область.Параметры.СуммаВал = Итоги.СуммаВал;
							Область.Параметры.Валюта = ?(Итоги.СуммаВал <> 0, СтрВалюта.Валюта, "");
							ДобавитьРасшифровку(ДанныеРасшифровки, РасшифровкаВалюта, СоответствиеГоризонтальныхРасшифровок[Элемент.Значение], Итоги, Область, "Сумма");
						...
						Область.Параметры.СуммаВал = Итоги.СуммаВал;
						Область.Параметры.Валюта = ?(Итоги.СуммаВал<>0, СтрВалюта.Валюта, "");
						ДобавитьРасшифровку(ДанныеРасшифровки, РасшифровкаВалюта,, Итоги, Область, "Сумма");
						Отчет.Присоединить(Область, 3,, Ложь);
					КонецЦикла; 
				КонецЦикла; 
			КонецЦикла;  
		...
		Для каждого Элемент Из СписокДат Цикл
			...
			СтрокиОстатков = ТаблицаОстатков.НайтиСтроки(СтруктураОтбораОстатков);
			Если НЕ СтрокиОстатков.Количество()=0 Тогда
				СуммаОстаток = СтрокиОстатков[0].КонечныйОстаток;
			Иначе
				СуммаОстаток = 0;
			КонецЕсли; 
			Область.Параметры.Сумма = СуммаОстаток;
		...
		Область = Макет.ПолучитьОбласть("Остаток|Период");
		Если СтрокиОстатков.Количество()>0 Тогда
			Область.Параметры.Сумма = СтрокиОстатков[0].КонечныйОстаток;
		КонецЕсли;  
		Отчет.Присоединить(Область);
		// Итоги
		...
		Для каждого Элемент Из СписокДат Цикл
			...
			Если ПараметрыОтчета.ПоМестамХраненияДенег Тогда
				...
				Отбор.Вставить("ДинамическийПериод", Элемент.Значение);
				СуммаПредыдущая0 = ВывестиОбластьОтчета(Отчет, Макет, ИмяОбласти, ПараметрыОтчета, ТаблицаИтогов, Отбор, СуммаПредыдущая0, 
					ДанныеРасшифровки, РасшифровкаСчетКасса, СоответствиеГоризонтальныхРасшифровок[Элемент.Значение],, 
					СписокДат.Индекс(Элемент)=0, "СуммаДляИтогов");
			Иначе
...
Процедура ВывестиЗаголовок(ПараметрыОтчета, Отчет, Макет)
	
	ЕстьФильтры = Ложь;
	
	ПараметрЗаголовок = КомпоновщикНастроек.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(
		Новый ПараметрКомпоновкиДанных("Заголовок"));
	Если ПараметрЗаголовок <> Неопределено И ПараметрЗаголовок.Использование 
		И НЕ ПустаяСтрока(ПараметрЗаголовок.Значение) Тогда
		ЗначенияПолей = Новый Структура;
		ЗначенияПолей.Вставить("Заголовок", ПараметрЗаголовок.Значение);
		Если ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода) Тогда
			ЗначенияПолей.Вставить("Период", ПредставлениеПериода(ПараметрыОтчета.НачалоПериода, 
		...
		Область.Параметры.Заполнить(ЗначенияПолей);
		Отчет.Вывести(Область);
	КонецЕсли;
	
	ПараметрВыводитьОтбор = КомпоновщикНастроек.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(
		Новый ПараметрКомпоновкиДанных("ВыводитьОтбор"));
	Если ПараметрВыводитьОтбор <> Неопределено И ПараметрВыводитьОтбор.Использование 
		И ПараметрВыводитьОтбор.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Тогда 
		...
		Для каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			...
			КонецЕсли;     
			ЗначенияПолей = Новый Структура;
			ЗначенияПолей.Вставить("Описание", Строка(ЭлементОтбора.ЛевоеЗначение));
			ЗначенияПолей.Вставить("Значение", Строка(ЭлементОтбора.ПравоеЗначение));
			Область.Параметры.Заполнить(ЗначенияПолей);
			Отчет.Вывести(Область);
			ЕстьФильтры = Истина;
		КонецЦикла;
...
Функция РассчитатьОстатки(ПараметрыОтчета, Настройки)
	...
	Иначе
		...
		|	Период"
	КонецЕсли;
	НаложитьОтборыРекурсивно(Построитель, Настройки.Отбор.Элементы);
...
Процедура НаложитьОтборыРекурсивно(Построитель, ОтборыЭлементы)
	...
	Для каждого ОтборЭлемент Из ОтборыЭлементы Цикл
		...
		Если ТипЗнч(ОтборЭлемент)=Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			НаложитьОтборыРекурсивно(Построитель, ОтборЭлемент.Элементы);
			Продолжить;
		КонецЕсли;
