...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Попытка
		МодульЗаявлениеОВвозеТоваров = ОбщегоНазначения.ОбщийМодуль("Документы.ЗаявлениеОВвозеТоваров");
		МодульЗаявлениеОВвозеТоваров.ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(Новый Массив);
	Исключение
		Элементы.ФормаЗаполнитьНаОснованииЗаявленияОВвозе.Видимость = Ложь;
	КонецПопытки;
...
Функция ОбычныеРеквизитыДляСохранения()
	РеквизитыДляСохранения =  "РегНомерАннулируемойФормы,Графа4,Графа5Код,Графа5Наименование,Графа6Код,Графа6Наименование,Графа7Код,Графа7Сумма,"
			...
			+"Графа1Ошибка,Графа2Ошибка,Графа3Ошибка";
...
&НаКлиенте
Процедура ЗаполнитьНаОснованииЗаявленияОВвозе(Команда)
	ОО = Новый ОписаниеОповещения("ЗаполнитьНаОснованииЗаявленияОВвозеПослеВыбора", ЭтотОбъект);
...
&НаКлиенте
Процедура ЗаполнитьНаОснованииЗаявленияОВвозеПослеВыбора(Результат, ДополнительныеПараметры) Экспорт 
	...
	Иначе 
		ЗаполнитьДаннымиЗаявленияОВвозе(Результат);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьНаОснованииЗаявленияОВвозеПослеИзмененияПериода(Результат, РеквизитыДокумента) Экспорт 
	...
	ЗаполнитьДаннымиЗаявленияОВвозе(РеквизитыДокумента.Ссылка);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьДаннымиЗаявленияОВвозе(Ссылка)
	Попытка
		МодульЗаявлениеОВвозеТоваров = ОбщегоНазначения.ОбщийМодуль("Документы.ЗаявлениеОВвозеТоваров");
		МассивДокументов = Новый Массив;
		МассивДокументов.Добавить(Ссылка);
		ДанныеЗаявленияОВвозе = МодульЗаявлениеОВвозеТоваров.ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(
			МассивДокументов);
			
		ЗагрузитьДокументы(ДанныеЗаявленияОВвозе.ТаблицаДокументы);
		ЗагрузитьТовары(ДанныеЗаявленияОВвозе.ТаблицаТоварыДляВыгрузки);
		ЗагрузитьСведенияОЗаявителе(ДанныеЗаявленияОВвозе.РеквизитыШапка);
		ЗагрузитьГрафа1(ДанныеЗаявленияОВвозе.РеквизитыШапка);
		ЗагрузитьГрафа2(ДанныеЗаявленияОВвозе.РеквизитыШапка);
		ЗагрузитьГрафа3(ДанныеЗаявленияОВвозе.РеквизитыШапка);
		ЗагрузитьПрочиеДанные(ДанныеЗаявленияОВвозе.РеквизитыШапка);
		ДоступностьПолей(ЭтотОбъект);
		УстановитьЗаголовок();
		Модифицированность = Истина;
	Исключение
		ОбщегоНазначения.СообщитьПользователю("Не удалось выполнить заполнение на основании заявления о ввозе");
	КонецПопытки;
КонецПроцедуры
...
Функция СтруктураТовар(СтрокаТовар)
	...
	ПоказателиГрафы.Вставить("RFOGRN", "");
	ПоказателиГрафы.Вставить("RFINN", "");
	ПоказателиГрафы.Вставить("RFKPP", "");
	...
	ПоказателиГрафы.Вставить("ОКАТО", "");
...
&НаСервере
Процедура ЗагрузитьТовары(ТаблицаТоварыДляВыгрузки)
	Товары.Очистить();
	Для Каждого Стр Из ТаблицаТоварыДляВыгрузки Цикл
		ЗаполнитьЗначенияСвойств(Товары.Добавить(),
			ФормаУчетаПеремещенияТоваровТС.СформироватьСтруктуруТовараПриЗагрузке(СтруктураТовар(Стр)));
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьСведенияОЗаявителе(Реквизиты)
	Заявитель_Фамилия = Реквизиты.ОргФамилияИсп;
	Заявитель_Имя = Реквизиты.ОргИмяИсп;
	Заявитель_Отчество = Реквизиты.ОргОтчествоИсп;
	Заявитель_Телефон = Реквизиты.ОргТелефонИсп;
	Заявитель_ЭлектроннаяПочта = Реквизиты.ОргАдресЭлПочты;
	Заявитель_Должность = Реквизиты.ОргДолжностьИсп;
	
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Реквизиты.Организация) Тогда 
		СтрокаСведений = "ИННЮЛ,НаимЮЛПол,КППЮЛ,НаимЮЛСокр,ОГРН";
	...
	Заявитель_СтранаКод = Реквизиты.ОргСтранаНаименование;
	Заявитель_СтранаНаименование = Реквизиты.ОргСтранаКод;
...
Функция ОбщиеПоказателиГраф23(Реквизиты)
	...
	ПоказателиГрафы.Вставить("BusinessEntityTypeName", "-");
...
Процедура ЗагрузитьГрафа2(Реквизиты)
	РезультатЗагрузки = ФормаУчетаПеремещенияТоваровТС.СформироватьСтруктуруГраф123ПриЗагрузке(ОбщиеПоказателиГраф23(Реквизиты), "Раздел2");
...
Процедура ЗагрузитьГрафа3(Реквизиты)
	РезультатЗагрузки = ФормаУчетаПеремещенияТоваровТС.СформироватьСтруктуруГраф123ПриЗагрузке(ОбщиеПоказателиГраф23(Реквизиты), "Раздел3");
