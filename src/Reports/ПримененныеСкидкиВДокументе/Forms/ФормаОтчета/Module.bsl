...
Процедура ПриОткрытии(Отказ)
	...
	Если Не СкидкиРассчитаны Тогда
		Если Элементы.ДекорацияПредупреждение.Видимость Тогда
			Элементы.ДекорацияПредупреждение.Заголовок = СтрШаблон(НСтр(
				"ru = '%1 В документе не рассчитаны скидки, наценки.'"), Элементы.ДекорацияПредупреждение.Заголовок);
		Иначе
			Элементы.ДекорацияПредупреждение.Видимость = Истина;
			Элементы.ДекорацияПредупреждение.Заголовок = НСтр("ru = 'В документе не рассчитаны скидки, наценки.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура Сформировать()
	...
	СуммаИтог = 0;
	Для Каждого ТекущееИмяТЧ Из МассивИменТЧ Цикл
		СуммаИтог = СуммаИтог + ДокументОбъект[ТекущееИмяТЧ].Итог("СуммаАвтоматическойСкидки");
	КонецЦикла;
	ОбластьИтогоСуммаИВыполнено.Параметры.Сумма = СуммаИтог;
