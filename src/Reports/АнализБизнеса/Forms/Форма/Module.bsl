		...
		Для каждого КлючИЗначение Из Параметры.Отбор Цикл
			...
			ИначеЕсли КлючИЗначение.Ключ = "КонецПериода" Тогда
				Если Год(КлючИЗначение.Значение) = Год(ТекущаяДатаСеанса()) Тогда
					Отчет.ПериодОтчета.ДатаОкончания = КонецМесяца(КлючИЗначение.Значение);
				Иначе
			...
			ИначеЕсли КлючИЗначение.Ключ = "ПланФакт" Тогда 
				Отчет.ПланФакт = КлючИЗначение.Значение;
			ИначеЕсли КлючИЗначение.Ключ = "Группировка" Тогда 
				Отчет.Группировка = КлючИЗначение.Значение;
			ИначеЕсли КлючИЗначение.Ключ = "АктивныйОтчет" Тогда 
				АктивнаяГиперссылка = КлючИЗначение.Значение;
			ИначеЕсли КлючИЗначение.Ключ = "Периодичность" Тогда 
				Отчет.Периодичность = КлючИЗначение.Значение;
			КонецЕсли; 
		КонецЦикла;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыБюджета" Тогда
		...
		Если Отчет.ПланФакт <> ПредопределенноеЗначение("Перечисление.ПланФакт.Факт") Тогда
			...
			Если  ЗначениеЗаполнено(Параметр.НачалоПериода)
				И ЗначениеЗаполнено(Параметр.КонецПериода) 
				И ЗначениеЗаполнено(Параметр.СценарийПланирования)
				И ЗначениеЗаполнено(Параметр.Периодичность) Тогда
...
Процедура ДекорацияВидОтчетаНажатие(Элемент)
	...
	Если Элемент.Имя = "ДекорацияАссистент" Тогда
		Элементы.ГруппаАссистент.Видимость = Истина;
	...
	Иначе
		Элементы.ГруппаАссистент.Видимость = Ложь;
...
&НаКлиенте
Процедура ВыводитьОтклонениеПриИзменении(Элемент)
	ОбновитьОтчеты();
КонецПроцедуры
...
&НаКлиенте
Процедура ДекорацияТекстПодсказкаЦельОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СтрокаПоиска", "Бюджетирование");
...
Процедура ДекорацияСебестоимостьНеактуальнаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтруктураОткрытия.Вставить("Месяц", Отчет.ПериодОтчета.ДатаНачала);
...
Процедура ОбновитьОтображениеЭлементовНаКлиенте()
	...
	Элементы.АктивныйПериод.Доступность = ЗначениеЗаполнено(Отчет.СценарийПланирования);
	Элементы.ВыводитьОтклонение.Видимость = Отчет.ПланФакт = ПредопределенноеЗначение("Перечисление.ПланФакт.ПланФакт");
...
Процедура ОбновитьОтчеты()
	...
	Если Отчет.ПланФакт = Перечисления.ПланФакт.План Тогда
		Элементы.ГруппаАктуальностьСебестоимости.Видимость = Ложь;
	Иначе
		РезультатПроверки = ЗакрытиеМесяца.НеактуальныеДанныеСебестоимостиВПериоде(Отчет.ПериодОтчета.ДатаНачала, Отчет.ПериодОтчета.ДатаОкончания, Новый Массив);
		Элементы.ГруппаАктуальностьСебестоимости.Видимость = РезультатПроверки <> Неопределено;
	КонецЕсли;
...
Функция ИнициализироватьДанныеОтчета()
	...
	ДанныеОтчета.Вставить("ВыводитьОтклонение", Отчет.ВыводитьОтклонение);
