...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если КлючВарианта = "Т13" Тогда
		...
		Период = НастройкиОтчета.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период")).Значение;
		Если Период.ДатаНачала <> НачалоМесяца(Период.ДатаНачала) Или Период.ДатаОкончания <> КонецМесяца(Период.ДатаОкончания) Тогда
			ВызватьИсключение НСтр("ru = 'Период отчета необходимо задавать кратно месяцам'");
		КонецЕсли;
	...
	Для каждого СтрокаОрганизации Из СтрокиДанных Цикл
		...
		Для каждого СтрокаСотрудника Из СтрокаОрганизации.Строки Цикл
			...
			Для каждого СтрокаДня Из СтрокаСотрудника.Строки Цикл
				...
				Если СтрокаДня.ПредпраздничныйДень И СтрокаДня.Часы <> 0 Тогда
					// Предпраздничный день короче на 1 час.
					ДанныеДня.Часы = Макс(0, Окр(8 * СтрокаСотрудника.ДоляНеполногоРабочегоВремени, 2) - 1);
				Иначе
					ДанныеДня.Часы = Окр(СтрокаДня.Часы * СтрокаСотрудника.ДоляНеполногоРабочегоВремени, 2);
				КонецЕсли;
