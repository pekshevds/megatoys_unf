...
Функция ОпределитьНаличиеДвиженийПоРегистратору() 
	...
	Если ЭтоУстановкаЦенНоменклатуры Тогда
		ТаблицаДвижений = Новый ТаблицаЗначений;
		ТаблицаДвижений.Колонки.Добавить("Имя", ОбщегоНазначения.ОписаниеТипаСтрока(150));
		Если Отчет.Документ.Проведен Тогда				
			Запись = ТаблицаДвижений.Добавить();
			Запись.Имя = ВРег("РегистрСведений.ЦеныНоменклатуры");
		КонецЕсли;                  		
...
Процедура ОбработатьВыводДанныхПоМассиву(СписокПолей, ТаблицаРесурсов, ТаблицаИзмерений, ТаблицаРеквизитов, ТаблицаВидДвижений = Неопределено, Знач ИмяРегистра, СинонимРегистра)
	...
	ПоляРегистратора = "ГДЕ Рег.Регистратор = &ДокументОтчета
					|	 И Рег.Активность";
	Если ЭтоУстановкаЦенНоменклатуры Тогда
		ПоляРегистратора = "ГДЕ Рег.ДокументУстановки = 	&ДокументОтчета";
	КонецЕсли;
...
Процедура СформироватьОтчет()
	...
	Если ЭтоУстановкаЦенНоменклатуры Тогда
		...
		ДвиженияДокумента.Добавить(Метаданные.РегистрыСведений.ЦеныНоменклатуры);		
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Документ") Тогда
		...
		Если Отчет.Документ.Метаданные().Имя = "УстановкаЦенНоменклатуры" Тогда
			ЭтоУстановкаЦенНоменклатуры = Истина;
		КонецЕсли;	
	КонецЕсли; 
