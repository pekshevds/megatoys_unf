...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Результат.Область(2, 2, 2, 2).Текст = НСтр(
	...
	Если ТипЗнч(Параметры.Отбор) = Тип("Структура") Тогда
		...
		Для каждого ЭлементОтбораИзПараметров Из Параметры.Отбор Цикл
			...
			УстановитьЭлементОтбораКомпоновкиДанныхНаСервере(
				ПолучитьСоответствиеЭлементовОтбораНастроекКомпоновкиНаСервере(), ЭлементОтбораИзПараметров.Ключ,
				Отчет.КомпоновщикНастроек.ПользовательскиеНастройки, ЭлементОтбораИзПараметров.Значение,
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиПриИзменении(Элемент)
	...
	ОбъектКомпоновкиДанных = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ПолучитьОбъектПоИдентификатору(
		ИдентификаторКомпоновкиДанных);

	Если Не ОбъектКомпоновкиДанных = Неопределено И ТипЗнч(ОбъектКомпоновкиДанных) = Тип(
...
Функция ПолучитьСоответствиеЭлементовОтбораНастроекКомпоновкиНаСервере()
	...
	Для каждого ЭлементОтбораКомпоновки Из ОтборКомпоновки.Элементы Цикл
		...
		СоответствиеОтбораКомпоновки.Вставить(ЭлементОтбораКомпоновки.ЛевоеЗначение,
			ЭлементОтбораКомпоновки.ИдентификаторПользовательскойНастройки);
...
Процедура УстановитьНадписьПериода()
	...
	ИначеЕсли Месяц(НачалоПериода) = Месяц(КонецПериода) Тогда
		...
		ПредставлениеПериода = ДеньРасписанияНачало + " - " + ДеньРасписанияОкончание + " " + МесяцРасписанияОкончание
			+ ", " + ГодРасписания;
	...
	Иначе
		...
		Если Год(НачалоПериода) = Год(КонецПериода) Тогда
			ГодРасписания = Формат(Год(КонецПериода), "ЧГ=0");
			ПредставлениеПериода = ДеньРасписанияНачало + " " + МесяцРасписанияНачало + " - " + ДеньРасписанияОкончание
		...
		Иначе
			...
			ПредставлениеПериода = ДеньРасписанияНачало + " " + МесяцРасписанияНачало + " " + ГодРасписанияНачало
